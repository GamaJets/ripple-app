...
import {
  daysUntilExpiry, inviteState, isRedeemable, type MemberInvite,
} from './memberInvites';

/** What one invitation looks like on the member's own screen. */
export interface GymInviteCard {
  id: string;
  /** Title Case, and names the gym only when a name was actually read. */
  title: string;
  /** Sentence case. What accepting does, which plan is attached, and — only
   *  when one was recorded — how long the invitation stays open. */
  note: string;
  /** Whether `accept_member_invite` would take it. The button's enabled state,
   *  and the same two conditions the SQL applies. */
  canAccept: boolean;
  /** Past its date. Kept separate from `!canAccept` because the sentence for a
   *  lapsed invitation is different from the sentence for one being redeemed. */
  lapsed: boolean;
}

/** The gym's own name when something could read it, and null when nothing
 *  could. Two different sources, both optional, neither invented. */
export interface GymNames {
  /** tenantId → name. Absent, or a null value, both mean "not known". */
  byTenant?: Map<string, string | null>;
}

const gymLabel = (inv: MemberInvite, names?: GymNames): string | null => {
 ...
 */
export function inviteWindowLine(
  inv: Pick<MemberInvite, 'expiresAt'>,
  now: number = Date.now(),
): string | null {
  const d = daysUntilExpiry(inv, now);
...
export const ACCEPT_NOTE =
  'Accepting adds you to their member list and opens your membership.';
 ...
 */
export function gymInviteCards(
  invites: MemberInvite[],
  names?: GymNames,
  now: number = Date.now(),
): GymInviteCard[] {
  const open = invites.filter((i) => isRedeemable(i, now));
  const rest = invites.filter((i) => !isRedeemable(i, now));
 ...
 */
export function acceptedMessage(gymName: string | null): string {
 ...
 */
export function acceptFailedMessage(reason?: string | null): string {
  ...
  if (r.includes('not signed in')) return 'You are signed out, so nothing was accepted. Sign in with the address the gym invited and try again.';
  if (r.includes('not found')) return 'That invitation is no longer on the gym’s list, so nothing was accepted. Ask them to send it again.';
  if (r.includes('not addressed to you')) return 'That invitation was sent to a different email address, so nothing was accepted. It has to be accepted from an account signed up with the address the gym invited.';
  if (r.includes('already accepted')) return 'You had already accepted that invitation, so nothing changed just now. Your membership is on the Membership screen.';
