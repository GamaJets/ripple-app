...
export function targetIndex(heights: readonly number[], from: number, dy: number): number {
  if (!Number.isInteger(from) || from < 0 || from >= heights.length) return from;
  if (!Number.isFinite(dy)) return from;
  if (!heights.every((h) => Number.isFinite(h) && h > 0)) return from;
 ...
 */
export function shifts(heights: readonly number[], from: number, to: number): number[] {
  const out = heights.map(() => 0);
  if (from === to) return out;
  if (from < 0 || from >= heights.length || to < 0 || to >= heights.length) return out;
  const h = heights[from];
  if (!Number.isFinite(h)) return out;
  if (to > from) for (let i = from + 1; i <= to; i += 1) out[i] = -h;
  else for (let i = to; i < from; i += 1) out[i] = h;
  return out;
}
