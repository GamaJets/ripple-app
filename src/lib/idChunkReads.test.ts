...
import { fetchMySessions } from './trainerSessions';
import { fetchCloses } from './gymClose';
import { fetchDocuments } from './gymDocs';
import { fetchMarks } from './gymReconcile';
...
interface Asked { table: string; in: string[] | null; limit: number | null }
...
function fakeSb(answer: (q: Asked) => { data: any[] | null; error: unknown }) {
  ...
  const from = (table: string) => {
    const q: Asked = { table, in: null, limit: null };
    asked.push(q);
    const chain: any = {
      select: () => chain,
      eq: () => chain,
...
const inCalls = (asked: Asked[], table: string) =>
  asked.filter((q) => q.table === table && q.in !== null).map((q) => q.in as string[]);

function chunkedProperly(asked: Asked[], table: string, all: string[], what: string) {
  const calls = inCalls(asked, table);
  eq(calls.length, Math.ceil(all.length / ID_CHUNK),
    `${what} is asked in chunks — one \`.in()\` of ${all.length} uuids is past the 8KB request line and comes back as a 414 dressed as an empty result`);
  ok(calls.every((c) => c.length <= ID_CHUNK),
    `and no ${what} chunk carries more than ${ID_CHUNK} ids`);
...
async function main() {
  /* ── 1 · fetchGymTrainers: three reads keyed on one id list ──────────────── */
  {
    ...
    const { sb, asked } = fakeSb((q) => {
      if (q.table === 'trainers') return { data: trainerIds.map((id) => ({ id })), error: null };
      if (q.table === 'profiles') {
        return {
    ...
    const roster = await fetchGymTrainers(sb, 'gym');
    ...
    ok(roster.every((t) => t.name !== 'Trainer'),
      'every coach is named — a 414 on one giant `.in()` would name none of them, and "Trainer" is the fallback that would hide it');
    ok(roster.every((t) => t.since !== null),
      'and every one has a joined-on date — `since: null` is documented as "the record does not say", which a refused request must not be able to claim of the whole roster');
    ok(roster.every((t) => t.clients === 2),
      'every coach carries their real client count, including the ones in the last short chunk');
    ok(roster.every((t) => t.delivered30 === 1 && t.unmarked30 === 1 && t.sessions30 === 2),
      'and their real session counts — this is the read payroll is priced from, so a chunk that never went out is money not paid');
    ...
    ok(inCalls(asked, 'clients').length > 0
      && asked.filter((q) => q.table === 'clients').every((q) => q.limit === ROW_CAP + 1),
      'each client chunk still asks for cap + 1, so a chunk that truncates is still detectable');
    ok(asked.filter((q) => q.table === 'sessions').every((q) => q.limit === ROW_CAP + 1),
      'and so does each session chunk');
  }
  ...
  /* ── 2 · fetchGymTrainers still throws when a chunk is refused ───────────── */
  {
    ...
    const { sb } = fakeSb((q) => {
      ...
      if (q.table === 'profiles') {
        ...
        if (profileCalls === 2) return { data: null, error: { message: 'refused' } };
    ...
    let threw = false;
    try { await fetchGymTrainers(sb, 'gym'); } catch { threw = true; }
  ...
  /* ── 3 · fetchMySessions names every person, over both id columns ────────── */
  {
    ...
    const N = ID_CHUNK; // 150 sessions → 300 distinct ids → three chunks.
    ...
    const { sb, asked } = fakeSb((q) => {
      if (q.table === 'sessions') return { data: rows, error: null };
      return { data: (q.in ?? []).map((id) => ({ id, full_name: `Name ${id.slice(-3)}` })), error: null };
    });

    const out = await fetchMySessions(sb, trainerId, '2026-08-01T00:00:00Z');
    ...
    ok(out.every((s) => s.clientName !== null),
      'and every session is labelled with its client — a single `.in()` of 151 ids is a request line the proxy answers 414 to, and the swallow in `fetchNames` would turn that into a queue of "Client"');
   ...
   * it does not LOOK like a failure either. */
  {
    ...
    const { sb, asked } = fakeSb((q) => {
      if (q.table === 'gym_month_closes') return { data: rows, error: null };
      return { data: (q.in ?? []).map((id) => ({ id, full_name: `Owner ${id.slice(-3)}` })), error: null };
    });

    const closes = await fetchCloses(sb, 'gym');
    eq(closes.length, N, 'every month close comes back');
    chunkedProperly(asked, 'profiles', staff, 'the close/reopen name lookup');
    ok(closes.every((c) => c.closedByName !== null),
      'and every close names who made it — one giant `.in()` is a 414 that names none of them, on the record an auditor reads');
  }
   ...
   * from it, so this crosses the request line early and stays across it. */
  {
    ...
    const out = await fetchDocuments(sb, 'gym');
    eq(out.length, N, 'every document comes back');
    chunkedProperly(asked, 'profiles', uploaders, 'the uploader name lookup');
    ok(out.every((d) => d.uploadedByName !== null),
      'and every document says who filed it — the paragraph above that read argues about ONE unreadable name, never about all of them');
  }
  ...
  /* ── 5 · fetchMarks: the answers an owner has already given ──────────────── */
  {
    ...
    const { sb, asked } = fakeSb((q) => {
      if (q.table === 'gym_reconcile_marks') return { data: rows, error: null };
      return { data: (q.in ?? []).map((id) => ({ id, full_name: `Owner ${id.slice(-3)}` })), error: null };
    });

    const marks = await fetchMarks(sb, 'gym');
    eq(marks.size, N, 'every answer already given comes back');
    chunkedProperly(asked, 'profiles', markers, 'the marked-by name lookup');
    ok([...marks.values()].every((m) => m.markedByName !== null),
      'and every answer says who gave it — otherwise the screen that exists to stop an owner re-asking cannot say who to ask');
  }
