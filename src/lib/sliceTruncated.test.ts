...
import {
  sliceLoading, sliceReady, sliceFailed, slicePartial, sliceCapped,
  rowsOf, rowsToShow, isWholeSlice,
  brokenParts, pendingParts, truncatedParts, completeness,
  partialWarning, truncationWarning, truncationNote, sliceNote, sliceDash,
  buildDossier, memberIds,
  type MemberRecord, type Slice,
} from './memberView';
import { capLimit, capped, ROW_CAP } from './rowCap';
import { closeBlockers, truncatedCloseParts } from './monthEnd';
import { staffCompleteness, staffTruncationWarning, staffWarning, truncatedStaffParts } from './staffView';
import type { Membership, GymPayment } from './gymRecord';
...
const nums = (n: number): number[] => Array.from({ length: n }, (_, i) => i);

{
  const s = slicePartial(nums(5), 3);
...

{
  ...
  const whole = sliceCapped(nums(3), 3);
  ...
  const cut = sliceCapped(nums(capLimit(3)), 3);
  eq(cut.state, 'partial', 'a read that came back at cap + 1 is partial');
  eq(cut.state === 'partial' ? cut.rows.length : -1, 3,
    'and the probe row is trimmed off — it was asked for to be counted, not shown');
  eq(cut.state === 'partial' ? cut.rows[cut.rows.length - 1] : -1, 2,
    'so the last row on the page is the last row of the cap, not the probe');
}

eq(sliceCapped(null, 3).state, 'ready', 'a null read is an empty one, not a truncated one');
eq(sliceCapped(undefined, 3).state, 'ready', 'and so is an absent one');
...
eq(sliceCapped(nums(ROW_CAP), ROW_CAP).state, 'ready', 'a set exactly the size of the cap is whole');
eq(sliceCapped(nums(ROW_CAP + 1), ROW_CAP).state, 'partial', 'one past it is not');
...
for (const n of [0, 1, 2, 3, 4, 10]) {
  const viaCapped = capped(nums(n), 3);
  const viaSlice = sliceCapped(nums(n), 3);
  eq(viaSlice.state === 'partial', viaCapped.truncated, `${n} rows: sliceCapped agrees with capped()`);
  eq(JSON.stringify(rowsToShow(viaSlice)), JSON.stringify(viaCapped.rows),
    `${n} rows: and hands back the same rows`);
}

/* ── the two accessors, and why there are two ──────────────────────────── */

const part = slicePartial(nums(5), 3);

eq(rowsOf(part), null, 'rowsOf REFUSES a prefix — a figure over part of a set is a wrong figure');
eq(rowsOf(sliceLoading<number>()), null, 'and refuses a read still in flight');
eq(rowsOf(sliceFailed<number>('no')), null, 'and refuses a read that failed');
eq(rowsOf(sliceReady(nums(2)))?.length, 2, 'and hands over a whole read');

eq(rowsToShow(part)?.length, 3, 'rowsToShow hands over the prefix, because a list of real rows is useful');
eq(rowsToShow(sliceReady(nums(2)))?.length, 2, 'and a whole read too');
eq(rowsToShow(sliceLoading<number>()), null, 'but not a read still in flight');
eq(rowsToShow(sliceFailed<number>('no')), null, 'and not a failure — an empty table under a failure is the lie');

ok(isWholeSlice(sliceReady([])), 'an empty read that succeeded is whole');
ok(!isWholeSlice(part), 'a prefix is not whole, which is what every `=== ready` gate already asks');
ok(!isWholeSlice(sliceLoading()), 'nor is a read in flight');
ok(!isWholeSlice(sliceFailed('no')), 'nor is a failure');

/* ── four sentences, never three ───────────────────────────────────────── */

eq(sliceNote(sliceReady([]), 'the door log'), null, 'a whole read needs no excuse');
eq(sliceNote(sliceLoading(), 'the door log'), 'reading the door log…', 'loading says loading');
eq(sliceNote(sliceFailed('boom'), 'the door log'), 'the door log could not be read', 'failed says failed');
{
  const note = sliceNote(part, 'the door log');
...
eq(sliceDash(sliceReady([])), null, 'a whole read has a figure, not a dash');
eq(sliceDash(sliceLoading()), '…', 'loading is an ellipsis');
eq(sliceDash(sliceFailed('boom')), 'not read', 'a failure is "not read"');
eq(sliceDash(part), 'part read', 'a prefix is "part read" — the rows exist, which is a different errand');
ok(sliceDash(part) !== sliceDash(sliceFailed('boom')),
  'and the two are never the same words, or the reader cannot tell a fault from a ceiling');

{
  const n = truncationNote('the payment runs', 100);
...
const payment = (n: number): GymPayment => ({
  id: `pay-${n}`,
  memberId: MEMBER,
  amountCents: 1000,
  currency: 'AED',
  takenAt: `2026-0${(n % 9) + 1}-01T00:00:00Z`,
} as unknown as GymPayment);

const record = (payments: Slice<GymPayment>): MemberRecord => ({
  memberships: sliceReady([membership(MEMBER)]),
  payments,
  visits: sliceReady([]),
  bookings: sliceReady([]),
  sessions: sliceReady([]),
...
{
  const whole = record(sliceReady([payment(1), payment(2)]));
  eq(buildDossier(MEMBER, whole).paidCents, 2000, 'a whole payments read totals');
  eq(completeness(whole), 'whole', 'and the page is whole');
  eq(truncationWarning(whole), null, 'with nothing to warn about');
  eq(truncatedParts(whole).length, 0, 'and no truncated parts');
}

{
  const cut = record(slicePartial([payment(1), payment(2), payment(3)], 3));
  const d = buildDossier(MEMBER, cut);
  ...
  eq(memberIds(cut)?.length, 1, 'and the roster still names the member');

  eq(completeness(cut), 'truncated', 'the page says it is a prefix');
  eq(truncatedParts(cut).join(','), 'payments', 'and names the part that was cut off');
  eq(brokenParts(cut).length, 0, 'nothing is BROKEN — the read succeeded');
  eq(partialWarning(cut), null, 'so the failure banner stays silent');

  const w = truncationWarning(cut);
  ...
  ok(w !== partialWarning(record(sliceFailed('boom'))),
    'the two banners are never the same sentence — one is a fault to chase, one is a figure to stop quoting');
}
...

{
  ...
  const brokenAndCut: MemberRecord = {
    ...record(slicePartial([payment(1)], 1)),
    visits: sliceFailed('boom'),
  };
  eq(completeness(brokenAndCut), 'broken', 'a failure outranks a truncation');
  ...
  // change.
  const loadingAndCut: MemberRecord = {
    ...record(slicePartial([payment(1)], 1)),
    visits: sliceLoading(),
  };
  eq(completeness(loadingAndCut), 'loading', 'a read still in flight outranks a truncation');
  eq(pendingParts(loadingAndCut).join(','), 'visits', 'and is named as pending');
  eq(truncatedParts(loadingAndCut).join(','), 'payments', 'while the truncation is still recorded');
}
...

{
  ...
  const rec = {
    payments: slicePartial([], 1000),
    invoices: sliceReady([]),
    sessions: sliceReady([]),
    memberships: sliceReady([]),
    passes: sliceReady([]),
  } as unknown as Parameters<typeof truncatedCloseParts>[0];
  ...
  ok(!blockers.some((b) => b.kind === 'read_failed'),
    'and it is NOT reported as a failure: the read succeeded, which is what makes it dangerous');
}

{
  ...
  // last of them.
  const rec = {
    trainers: slicePartial([], 1000),
    sessions: sliceReady([]),
    shifts: sliceReady([]),
    clients: sliceReady([]),
    activity: sliceReady([]),
    classes: sliceReady([]),
  } as unknown as Parameters<typeof staffCompleteness>[0];
  eq(staffCompleteness(rec), 'truncated', 'a staff page over a prefix is not a whole picture');
  eq(truncatedStaffParts(rec).join(','), 'trainers', 'and it names the part');
  ok(staffTruncationWarning(rec) != null, 'and it has a sentence of its own');
  ok(staffWarning(rec) == null, 'while the failure banner stays silent — nothing failed');
}
