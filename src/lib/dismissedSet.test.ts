...
import {
  nextDismissed, packDismissed, parseDismissed, sameIds, withDismissed,
} from './dismissedSet';
...
{
  eq(ids(parseDismissed('["a","b"]')), 'a,b', 'a stored list reads back');
  eq(parseDismissed(null).size, 0, 'nothing stored is an empty set');
  eq(parseDismissed(undefined).size, 0, 'and so is undefined');
  eq(parseDismissed('{not json').size, 0, 'bytes that are not JSON are an empty set, not a crash');
  eq(parseDismissed('{"a":1}').size, 0, 'JSON that is not an array is an empty set');
  eq(parseDismissed('"a"').size, 0, 'nor is a bare string one id');
  eq(
    ids(parseDismissed('["a",null,3,"",{"x":1},"b"]')),
    'a,b',
    'a member that is not a non-empty string is dropped, never coerced: an id of "null" would look like a match',
  );
}
...
{
  const s = new Set(['a']);
  ok(sameIds(s, s), 'a set is the same as itself');
  ok(sameIds(new Set(['a', 'b']), new Set(['b', 'a'])), 'order is not part of the answer');
  ok(!sameIds(new Set(['a']), new Set(['a', 'b'])), 'a set that grew is not the same');
  ok(!sameIds(new Set(['a', 'b']), new Set(['a'])), 'nor one that shrank');
  ok(!sameIds(new Set(['a', 'b']), new Set(['a', 'c'])), 'nor the same size with a different member');
  ok(sameIds(new Set(), new Set()), 'two empty sets are the same');
}
...

{
  ...
  // back.
  eq(nextDismissed(prev, new Set(['b', 'a'])), prev, 'an unchanged reload returns the object already held, not an equal copy');

  const empty = new Set<string>();
  eq(nextDismissed(empty, new Set()), empty, 'and that holds when there is nothing in it — the ordinary case on a fresh install');

  let held = new Set(['a', 'b']);
  const first = held;
  for (let i = 0; i < 50; i += 1) held = nextDismissed(held, new Set(['a', 'b']));
...

{
  ...
  eq(nextDismissed(prev, grown), grown, 'an id added comes through');
  const shrunk = new Set(['a']);
  eq(nextDismissed(new Set(['a', 'b']), shrunk), shrunk, 'and an id removed — another device declined something, and this one has to hear');
}
...
{
  const prev = new Set(['a']);
  const out = withDismissed(prev, 'b');
  ...
  eq(withDismissed(prev, 'a'), prev, 'handling the same invitation twice does not produce a second render');
  eq(withDismissed(prev, ''), prev, 'and an empty id is not an id');
}

/* ── the bytes ─────────────────────────────────────────────────────────── */

{
  eq(packDismissed(new Set(['b', 'a', 'c'])), '["a","b","c"]', 'stored sorted');
  eq(
    packDismissed(new Set(['b', 'a'])),
    packDismissed(new Set(['a', 'b'])),
    'so the same ids in a different order write identical bytes',
  );
  eq(packDismissed(new Set()), '[]', 'an empty set is a real answer and is written as one');
  const round = new Set(['x', 'y', 'z']);
  ok(sameIds(parseDismissed(packDismissed(round)), round), 'and what goes in comes back');
}
