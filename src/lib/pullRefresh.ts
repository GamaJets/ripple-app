...
/** What the refresher needs from the world, so a test can supply all of it. */
export interface RefreshDeps {
  /** The screen's own reload. May return a promise or nothing. */
  reload: () => void | Promise<unknown>;
  /** Drives the RefreshControl's `refreshing` prop. */
  setRefreshing: (on: boolean) => void;
  /** Defaults to `Date.now`. */
  now?: () => number;
  /** Defaults to `setTimeout`. Return value deliberately ignored — nothing here
   *  cancels a timer, because a fired-late ceiling is harmless and a cancelled
   *  one is a leak waiting to be got wrong. */
...
export interface Refresher {
  /** Hand this to `RefreshControl`'s `onRefresh`. */
  onRefresh: () => void;
  /** True while a spin is in progress. For assertions; the UI reads state. */
  busy: () => boolean;
}
 ...
 */
export function makeRefresher(deps: RefreshDeps): Refresher {
  const now = deps.now ?? (() => Date.now());
  const schedule = deps.schedule ?? ((fn: () => void, ms: number) => { setTimeout(fn, ms); });
  const floorMs = deps.floorMs ?? MIN_SPIN_MS;
  const ceilingMs = deps.ceilingMs ?? MAX_SPIN_MS;

  // Which spin is on screen. Only the spin that owns the number may end it.
  let generation = 0;
  let busy = false;

  const end = (mine: number) => {
    if (!busy || mine !== generation) return;
    busy = false;
  ...
  const onRefresh = () => {
    ...
    void Promise.resolve()
      ...
      .then(() => new Promise<void>((r) => schedule(() => r(), Math.max(0, floorMs - (now() - started)))))
      .then(() => end(mine));
  };
