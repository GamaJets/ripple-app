...
import {
  endCoachingBrief, bulkReport, namesWithRest,
  type EndTarget, type WriteOutcome,
} from './bulkActions';
...
const linked = (name: string): EndTarget => ({ clientId: `id-${name}`, name, handAdded: false });
const hand = (name: string): EndTarget => ({ clientId: `id-${name}`, name, handAdded: true });

/* ── the count is in front of them ─────────────────────────────────────── */

const forty: EndTarget[] = Array.from({ length: 40 }, (_, i) => linked(`Client${i}`));
const big = endCoachingBrief(forty);
...
const mixed = endCoachingBrief([linked('Ana'), linked('Ben'), hand('Cara')]);
...
const allLinked = endCoachingBrief([linked('Ana'), linked('Ben')]);
ok(!/added by hand/i.test(allLinked.body), 'no hand-added clients, no sentence about deleted rows');
const allHand = endCoachingBrief([hand('Ana'), hand('Ben')]);
...
const one = endCoachingBrief([linked('Ana')]);
...
const okRow = (name: string): WriteOutcome => ({ clientId: `id-${name}`, name, ok: true, why: null });
const badRow = (name: string, why: string): WriteOutcome => ({ clientId: `id-${name}`, name, ok: false, why });

const all = bulkReport('end', [okRow('Ana'), okRow('Ben')]);
...
const nothing = bulkReport('end', [badRow('Ana', 'the server refused it.'), badRow('Ben', 'the server refused it.')]);
...
ok(/nothing has changed/i.test(nothing.body),
  'a coach who thinks a failed removal half-landed has to check everybody by hand');

const part = bulkReport('end', [okRow('Ana'), badRow('Ben', 'the server refused it.')]);
...
const many = bulkReport('end', Array.from({ length: 12 }, (_, i) => badRow(`C${i}`, 'the server refused it.')));
...
eq((many.body.match(/the server refused it\./g) ?? []).length, 1,
  'and the reason is stated once, because forty identical sentences is a wall nobody reads');

/* ── the naming helper ─────────────────────────────────────────────────── */

eq(namesWithRest(['Ana']), 'Ana', 'one name is a name');
ok(/and/.test(namesWithRest(['Ana', 'Ben'])), 'two are joined');
ok(/and 1 more/.test(namesWithRest(['A', 'B', 'C', 'D', 'E', 'F', 'G'])),
  'past the limit the remainder is counted — a truncation that just stops is false in the direction that makes a coach relax');
