...
import { makeSubmitGate } from './submitOnce';
...
(async () => {
  /* ── the second tap, inside the same frame ───────────────────────────── */
  {
    ...
    const gate = makeSubmitGate({ setBusy: (b) => busyLog.push(b) });
    let ran = 0;
    const job = held();
  ...
  {
    const gate = makeSubmitGate({ setBusy: () => {} });
    let ran = 0;
  ...
  /* ── a job that throws ───────────────────────────────────────────────── */
  {
    ...
    const seen: unknown[] = [];
    const gate = makeSubmitGate({ setBusy: () => {}, onError: (e) => seen.push(e) });
