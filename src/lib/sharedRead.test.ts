...
import { createSharedRead, SAME_LAUNCH_MS, type ReadOutcome } from './sharedRead';
...
function deferrable<T>() {
  let runs = 0;
  const settles: ((o: ReadOutcome<T>) => void)[] = [];
  const run = () => {
    runs += 1;
    return new Promise<ReadOutcome<T>>((resolve) => { settles.push(resolve); });
  };
  return {
    run,
...
async function main() {
  ...
  {
    const d = deferrable<string>();
    const s = createSharedRead<string>();
  ...
  {
    const d = deferrable<string | null>();
    const s = createSharedRead<string | null>();
  ...
  // one attempt and that retry a lie.
  {
    const t = 1_000;
    const d = deferrable<string>();
    const s = createSharedRead<string>({ now: () => t });
    const a = s.read('u1', d.run);
  ...
  /* ── rule 3: a held answer expires ────────────────────────────────────── */
  {
    let t = 1_000;
    const d = deferrable<string>();
    const s = createSharedRead<string>({ freshMs: 5_000, now: () => t });
    const a = s.read('u1', d.run);
    d.settle({ ok: true, value: 'Ada' });
    await a;
    await flush();
  ...
  /* ── a window of zero shares only what genuinely overlaps ─────────────── */
  {
    let t = 0;
    const d = deferrable<string>();
    const s = createSharedRead<string>({ freshMs: 0, now: () => t });
    const a = s.read('u1', d.run);
    const b = s.read('u1', d.run);
  ...
  // the old name back on three screens.
  {
    const t = 1_000;
    const d = deferrable<string>();
    const s = createSharedRead<string>({ freshMs: 60_000, now: () => t });
  ...
  {
    const t = 1_000;
    const d1 = deferrable<string>();
    const d2 = deferrable<string>();
    const s = createSharedRead<string>({ freshMs: 60_000, now: () => t });
    const a = s.read('u1', d1.run); d1.settle({ ok: true, value: 'Ada' }); await a;
    const b = s.read('u2', d2.run); d2.settle({ ok: true, value: 'Grace' }); await b;
    await flush();
    s.forget();
  ...
  // row that does not exist on every provider, for ever.
  {
    const t = 1_000;
    const d = deferrable<string | null>();
    const s = createSharedRead<string | null>({ freshMs: 60_000, now: () => t });
  ...
  // somebody's old name.
  ok(SAME_LAUNCH_MS > 0 && SAME_LAUNCH_MS <= 10_000,
    `the shipped window is a launch fan-out and not a cache — got ${SAME_LAUNCH_MS}ms`);
}
