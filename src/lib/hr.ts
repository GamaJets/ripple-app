...
export function demoHrSeries(age?: number | null, nowMs: number = Date.now()): HrSample[] {
  const m = maxHr(age);
  const out: HrSample[] = [];
  const start = nowMs - 45 * 60 * 1000;
  for (let i = 0; i <= 45; i++) {
    const x = i / 45;
    let pct = 0.56 + 0.22 * Math.min(1, i / 8);           // warm-up ramp to ~aerobic
    pct += 0.17 * Math.sin(i / 2.1) * (x > 0.18 && x < 0.85 ? 1 : 0); // work intervals into orange/red
    if (i > 40) pct -= 0.02 * (i - 40);                    // cooldown
