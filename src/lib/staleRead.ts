...
/**
 * The four states of a screen that reads once and can be asked again.
 *
 *  'loading' — nothing has landed yet and nothing has failed. First read only.
 *  'ready'   — the most recent read landed. What is on screen is current.
 *  'stale'   — rows from an earlier read are held, and the most recent attempt
 *              did NOT come back. The rows are real and were whole; they are
 *              older than the screen, and the screen has to say so.
 *  'failed'  — nothing has ever landed, and the read failed. The only state in
 *              which a screen genuinely has nothing, and it must not be worded
 *              as an empty result.
 */
export type ReadState = 'loading' | 'ready' | 'stale' | 'failed';
 ...
 */
export function readState(held: unknown[] | null | undefined, lastAttemptFailed: boolean): ReadState {
  if (held == null) return lastAttemptFailed ? 'failed' : 'loading';
  return lastAttemptFailed ? 'stale' : 'ready';
}

/** True when there are rows to draw — and therefore rows to count and sum. */
export function hasRows(s: ReadState): boolean {
 ...
 */
export function canSayEmpty(s: ReadState): boolean {
  return hasRows(s);
}

/** True when what is on screen is confirmed current. */
export function isCurrent(s: ReadState): boolean {
  return s === 'ready';
}
