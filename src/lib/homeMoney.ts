...
import { sumTaken, type Pot, type TakenRow } from './coachMoney';
import type { AgeingBook } from './coachInvoice';
import { isWhole, type LoadStatus } from '../ui/loadStatus';

/** What the home screen knows about the coach's unpaid book. */
export interface HomeMoney {
  /** Everything being chased: past a due date on the document, or past a chase
   *  date the coach set for themselves. Null when no count may be stated. */
  overdue: number | null;
  /** How many of those are past a date the CLIENT was actually shown — the
   *  only half that can go in a demand. Null when no count may be stated. */
  pastStated: number | null;
  /** Days past due on the worst one, or null when there is none or it cannot
   *  be read. `ageingBook` sorts longest-overdue first, so this is its head. */
  worstDays: number | null;
  /** What those overdue invoices come to, one line per currency, or null when
   *  no figure may be stated. Over the OVERDUE rows only — see the header. */
  pots: readonly Pot[] | null;
  /** Overdue invoices carrying an amount with no currency on it. In no pot. */
  unlabelled: number;
  /** Overdue invoices carrying no amount at all. In no pot. */
  unpriced: number;
   ...
   *  figure on it, because silence here reads as "nobody owes you anything". */
  failed: boolean;
}

/** What the home screen may say about the book it has already read. */
export function homeMoney(book: AgeingBook, status: LoadStatus): HomeMoney {
  ...
  const taken = whole
    ? sumTaken(rows.map(({ invoice }): TakenRow => ({
  ...
  return {
    ...
    worstDays: whole && Number.isFinite(worst) ? (worst as number) : null,
 ...
 */
export function homeMoneyNote(m: HomeMoney): string {
  if (m.failed) {
