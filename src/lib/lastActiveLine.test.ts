...
eq(lastActiveLine('3d ago'), 'Last active 3d ago.',
  'an aged value still reads the way it always did');
eq(lastActiveLine('45m ago'), 'Last active 45m ago.',
  'and minutes as well as days');
...
for (const unread of ['—', '', null, undefined]) {
  ...
  ok(/not a statement that they have been quiet/.test(s),
    `and refuses the inference a coach would otherwise draw — ${JSON.stringify(unread)}`);
}
 ...
 */
eq(lastActiveLine('no activity yet'), 'Nothing has been recorded for them yet.',
  'an empty record says so plainly');
for (const hand of ['added by you', 'just added']) {
  const s = lastActiveLine(hand);
  ok(/added them by hand/.test(s), `a hand-added client is described as one — ${hand} gave ${JSON.stringify(s)}`);
  ok(s !== lastActiveLine('no activity yet'),
    `and is NOT collapsed into "nothing recorded" — ${hand}`);
}

/* ── whitespace is not a fifth state ───────────────────────────────────── */
eq(lastActiveLine('  3d ago  '), 'Last active 3d ago.', 'padding is trimmed rather than printed');
eq(lastActiveLine('   '), lastActiveLine(null), 'a blank string is the unread case, not an aged one');
