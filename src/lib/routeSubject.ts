...
/** What expo-router can actually hand back for a single route param. */
export type RouteParam = string | string[] | null | undefined;
 ...
 */
export function subjectOf(p: RouteParam): string | null {
  if (typeof p !== 'string') return null;   // absent, or a repeated key
  const s = p.trim();
  return s === '' ? null : s;
}
