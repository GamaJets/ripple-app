...
import type { BlockResult, BlockOutcome } from './blockRange';
import type { LoadStatus } from '../ui/loadStatus';

/**
 * A period the coach is busy, and the whole of what this app keeps about a
 * calendar entry. Two numbers. There is deliberately no id: an event id is not
 * a time, the rows on screen are keyed by the period they cover instead, and a
 * field that is never held cannot be logged by mistake.
 */
export interface BusySpan {
  /** Epoch milliseconds at the start. */
  readonly startMs: number;
  /** Epoch milliseconds at the end. Always greater than `startMs`. */
 ...
 */
export function toBusySpan(raw: unknown): BusySpan | null {
  ...
  try {
    ...
    startMs = msOf((raw as { startDate?: unknown }).startDate);
    endMs = msOf((raw as { endDate?: unknown }).endDate);
  } catch {
    return null;
  }
...
function midnightPlus(from: string, days: number): Date | null {
...
export function busyWindow(fromDay: string, days: number): { fromMs: number; toMs: number } | null {
  if (!Number.isInteger(days) || days < 1 || days > MAX_BUSY_DAYS) return null;
...
/**
 * One period on one day that the coach may choose to block.
 *
 * A calendar entry that runs past midnight becomes one of these per day it
 * touches, because `block_time` blocks inside a day and because a coach reads
 * their week a day at a time.
 */
export interface BusyCandidate {
  /** Stable for a given period, and derived only from the period. React needs
   *  a key and this app will not hold an event id to give it one. */
  key: string;
  /** Local `YYYY-MM-DD`. */
  day: string;
  /** Minutes from local midnight, 0-1439. */
  startMin: number;
  /** Minutes from local midnight, 1-1440. 1440 is the end of the day. */
  endMin: number;
  /** The period covers the whole local day. Derived, never read off the entry:
   *  see the header. Shown so a birthday reminder is recognisable as one, and
   *  never chosen for the coach. */
  allDay: boolean;
  /** How many entries were merged into this period. Shown as a count and never
   *  as anything about what they were. */
  entries: number;
}
...
const minsOf = (d: Date): number => d.getHours() * 60 + d.getMinutes();
 ...
 */
export function busyCandidates(
  ...
  for (let i = 0; i < days; i++) {
    ...
    const pieces: { startMin: number; endMin: number; entries: number }[] = [];
    ...
    const merged: { startMin: number; endMin: number; entries: number }[] = [];
...
export const candidateMinutes = (c: BusyCandidate): number => c.endMin - c.startMin;
 ...
 */
export function candidateTimeLabel(c: BusyCandidate): string {
  if (c.allDay) return 'All day';
  const from = fmtClock(Math.floor(c.startMin / 60), c.startMin % 60);
 ...
 */
const SEVERITY: Record<BlockOutcome, number> = {
  'already-blocked': 0,
  blocked: 1,
  booked: 2,
  failed: 3,
...
export function foldByDay(results: readonly BlockResult[]): BlockResult[] {
  const byDay = new Map<string, BlockResult>();
  const order: string[] = [];
...
/* ── what the sheet is showing, and it is never "nothing found" ───────────── */

/** Whether the phone was asked, and what it said. 'unavailable' is the binary
 *  that predates the dependency — a different thing from a refusal, and the
 *  coach is told which. */
export type BusyPermission = 'unknown' | 'granted' | 'denied' | 'unavailable';

/**
 * The state the sheet is in.
 *
 * 'denied' and 'empty' are separate and that separation is the point. Both
 * hold an empty list; one of them means "your diary is clear" and the other
 * means "we were not allowed to look". Telling a coach their fortnight is free
 * because iOS refused us is the recurring defect src/ui/loadStatus.ts was
 * written about, and it would land here on a screen whose whole job is to stop
 * a double booking.
 */
export type BusyView = 'unavailable' | 'ask' | 'loading' | 'denied' | 'failed' | 'empty' | 'list';

export function busyView(a: {
  supported: boolean;
  /** Whether the coach has pressed the button that asks. Nothing is read
   *  before that: the promise below has to be readable BEFORE the operating
   *  system's own prompt appears, not after it. */
  asked: boolean;
  status: LoadStatus;
  ...
  if (!a.supported || a.permission === 'unavailable') return 'unavailable';
  if (!a.asked) return 'ask';
  ...
  if (a.permission === 'denied') return 'denied';
 ...
 */
export const BUSY_NOTES: Record<Exclude<BusyView, 'unavailable'>, string> = {
 ...
 */
export const BUSY_PRIVACY_NOTE =
  'Repple reads when you are busy and nothing else. No title, no notes, no location and nobody you are meeting is read, shown, saved or sent, and nothing is ever written back to your calendar. A client only sees that you are unavailable, never why.';
...
export function busyBlockLabel(picked: number): string | null {
  if (!Number.isInteger(picked) || picked < 1) return null;
