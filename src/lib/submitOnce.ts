...
/** What the gate needs from the world, so a test can supply all of it. */
export interface SubmitDeps {
  /** Drives the button's `disabled` and its label. Called with true as the job
   *  starts and false as it settles, whichever way. */
  setBusy: (on: boolean) => void;
  /** Where an unexpected throw goes. Defaults to dropping it — see the header;
   *  screens pass `reportError`. */
  onError?: (e: unknown) => void;
}

export interface SubmitGate {
  /** Run the job unless one is already running. Resolves when this call's job
   *  has settled, or immediately when the gate was shut. */
  run: (job: () => void | Promise<unknown>) => Promise<void>;
  /** True while a job is in flight. For assertions; the UI reads the state. */
  busy: () => boolean;
  /** True when the LAST call to `run` was turned away because one was already
   *  in flight. For a screen that wants to say "still sending…" rather than
 ...
 * had gone.
 */
export function makeSubmitGate(deps: SubmitDeps): SubmitGate {
  ...
  const run = async (job: () => void | Promise<unknown>): Promise<void> => {
    if (running) { turnedAway = true; return; }
    running = true;
    turnedAway = false;
    deps.setBusy(true);
    try {
