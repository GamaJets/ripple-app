...
import { chaseGroups, chaseMessage, chaseMessageCaveat } from './chaseList';
...
const bookOf = (rows: CoachInvoice[], status: 'ready' | 'partial' | 'error' | 'loading' = 'ready') =>
  ageingBook(rows, status, TODAY);
...
/* ── grouping ───────────────────────────────────────────────────────────── */
{
  ...
  const { groups, withheld } = chaseGroups(bookOf([a1, a2, b1, notYet, noDate, settled]), 'ready');
...
/* ── the read that is not allowed to produce a note ─────────────────────── */
{
  ...
  for (const status of ['partial', 'loading', 'error'] as const) {
    const b = chaseGroups(bookOf(rows, status), status);
    ok(!!b.withheld, `${status}: a sentence says why`);
    for (const g of b.groups) {
      eq(g.pots, null, `${status}: no amount is stated per person`);
      eq(chaseMessage(g, 'Sam'), null, `${status}: no note is written`);
    }
  }
  const part = String(chaseGroups(bookOf(rows, 'partial'), 'partial').withheld);
  const failed = String(chaseGroups(bookOf(rows, 'error'), 'error').withheld);
  const loading = String(chaseGroups(bookOf(rows, 'loading'), 'loading').withheld);
  ...
  eq(chaseGroups(bookOf(rows, 'partial'), 'partial').groups.length, 1, 'a truncated read still shows who it did read');
}
...
/* ── the note ───────────────────────────────────────────────────────────── */
{
  ...
  const g = chaseGroups(bookOf([one, two]), 'ready').groups[0];
  const note = String(chaseMessage(g, 'Sam Okafor'));
  ...
  const unsigned = String(chaseMessage(g, null));
  ...
  const single = chaseGroups(bookOf([one]), 'ready').groups[0];
  const solo = String(chaseMessage(single, 'Sam'));
  ...
  eq(chaseMessage(null, 'Sam'), null, 'no group is no note');
}
...
/* ── the chase date that must never leave the phone ─────────────────────── */
{
  ...
  const g = chaseGroups(bookOf([priv]), 'ready').groups[0];
  ...
  const note = String(chaseMessage(g, 'Sam'));
...
/* ── two currencies are two amounts ─────────────────────────────────────── */
{
  ...
  const note = String(chaseMessage(g, null));
...
/* ── an amount with no currency on it ───────────────────────────────────── */
{
  ...
  const caveat = String(chaseMessageCaveat(g));
  ok(/no amount recorded/.test(caveat), 'the coach is told before they send it');
  eq(chaseMessageCaveat({ ...g, unlabelled: 0 }), null, 'nothing missing is nothing to say');
  ok(/^One of these/.test(String(chaseMessageCaveat(g))), 'one reads as one');
  ok(/^2 of these/.test(String(chaseMessageCaveat({ ...g, unlabelled: 2 }))), 'two reads as two');
}

/* ── nothing outstanding ────────────────────────────────────────────────── */
{
  const b = chaseGroups(bookOf([]), 'ready');
  eq(b.groups.length, 0, 'nobody owes anything and there is no group');
  eq(b.withheld, null, 'and nothing is withheld about it');
  eq(chaseGroups(null, 'ready').groups.length, 0, 'a missing book is answered rather than thrown at');
}
