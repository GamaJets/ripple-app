...
import { replyWait, COACH_QUEUE_NOTE } from './replyWait';
import { WAITING_HOURS } from './awaitingReply';
...
const base = {
  ...
  peerReadAt: null as string | null,
  now: NOW,
  canSend: true,
  them: 'Dayne',
};
...
const theirs = (h: number) => ({ mine: false, createdAt: at(h), delivered: true });
/** Mine, delivered. */
const mine = (h: number) => ({ mine: true, createdAt: at(h), delivered: true });

/* ── the silences, which are most of the answers ───────────────────────── */

eq(replyWait({ ...base, messages: [] }).kind, 'silent',
  'an empty thread has nobody waiting in it');
eq(replyWait({ ...base, messages: [mine(50), theirs(48)] }).kind, 'silent',
  'the coach spoke last, so there is nothing to wait for');
eq(replyWait({ ...base, messages: [mine(WAITING_HOURS - 1)] }).kind, 'silent',
  'under a day is a coach on a gym floor, not a silence');
eq(replyWait({ ...base, messages: [mine(100)], canSend: false }).kind, 'silent',
  'a closed or blocked thread is explained by the screen, not by this');
eq(replyWait({ ...base, messages: [mine(100)], status: 'loading' }).kind, 'silent',
  'a read in flight states no silence');
eq(replyWait({ ...base, messages: [mine(100)], status: 'error' }).kind, 'silent',
  'and neither does one that failed — a reply may exist that did not come back');

{
  ...
  const r = replyWait({ ...base, messages: [mine(WAITING_HOURS)] });
  ok(r.kind !== 'silent', 'exactly a day is a wait');
  eq(replyWait({ ...base, messages: [mine(WAITING_HOURS - 0.01)] }).kind, 'silent',
    'a moment under it is not');
}

{
  ...
  const r = replyWait({ ...base, messages: [{ mine: true, createdAt: at(72), delivered: false }] });
...
}
{
  ...
  eq(replyWait({ ...base, messages: [{ mine: true, createdAt: at(-5), delivered: true }] }).kind, 'silent',
    'a message stamped in the future is not a wait');
  eq(replyWait({ ...base, messages: [{ mine: true, createdAt: at(-72), delivered: true }] }).kind, 'silent',
    'and a badly wrong clock does not become a three-day one');
}
{
  const r = replyWait({ ...base, messages: [{ mine: true, createdAt: 'not a date', delivered: true }] });
...

{
  ...
  const r = replyWait({ ...base, messages: [mine(50)] });
  ...
  ok(!/has not opened|hasn’t opened|ignored|no reply from/i.test(note),
    'nothing here accuses the coach of anything');
  ok(note.includes(COACH_QUEUE_NOTE), 'and the member is told the app has already noticed');
}

{
  ...
  const r = replyWait({ ...base, messages: [mine(30)], peerReadAt: at(20) });
...

{
  ...
  const r = replyWait({ ...base, messages: [mine(30)], peerReadAt: at(40) });
...

{
  ...
  const exact = at(30);
  const r = replyWait({ ...base, messages: [{ mine: true, createdAt: exact, delivered: true }], peerReadAt: exact });
  eq(r.kind, 'seen', 'a watermark exactly equal to the message is a read');
}

{
  const r = replyWait({ ...base, messages: [mine(30)], peerReadAt: 'not a date' });
...

{
  ...
  const r = replyWait({ ...base, messages: [theirs(60), mine(50)], status: 'partial' });
...
{
  const r = replyWait({ ...base, messages: [mine(50)], them: 'Your coach' });
  const note = r.kind === 'silent' ? '' : r.note;
  ok(/whether Your coach has opened it/.test(note),
    'an unnamed coach is described rather than dashed — see scripts/check-prose.mjs');
...
{
  const r = replyWait({ ...base, messages: [mine(74)] });
  eq(r.kind === 'silent' ? null : Math.round(r.waitedMs / HOUR), 74,
    'the span is exposed so a caller can decide how loudly to draw it');
}
