...
import { gradeLift, levelFor, type LiftGrade } from './strengthLevel';
...
const kindOf = (g: LiftGrade) => g.kind;

/* ── the reported case: a real lift, no bodyweight ─────────────────────── */

const noBw = gradeLift(200, null, DEAD);
...
const below = gradeLift(60, 100, DEAD);
...
eq(kindOf(gradeLift(0, 80, DEAD)), 'unlogged', 'a zero best means nothing was found for this lift');
eq(kindOf(gradeLift(null, 80, DEAD)), 'unlogged', 'a null best means nothing was found');
eq(kindOf(gradeLift(undefined, 80, DEAD)), 'unlogged', 'an undefined best means nothing was found');
...
eq(kindOf(gradeLift(0, null, DEAD)), 'unlogged', 'no lift and no bodyweight is still no lift');
...
eq(kindOf(gradeLift(200, 0, DEAD)), 'ungradable', 'a zero bodyweight cannot grade a lift');
eq(kindOf(gradeLift(200, -80, DEAD)), 'ungradable', 'a negative bodyweight cannot grade a lift');
eq(kindOf(gradeLift(200, NaN, DEAD)), 'ungradable', 'a NaN bodyweight cannot grade a lift');
eq(kindOf(gradeLift(NaN, 80, DEAD)), 'unlogged', 'a NaN best is not a lift');

/* ── the grading itself, unchanged ─────────────────────────────────────── */

const g = gradeLift(200, 100, DEAD);
...
ok(gradeLift(300, 100, DEAD).kind === 'graded'
  && (gradeLift(300, 100, DEAD) as { level: number }).level === DEAD.length - 1,
  '3× bodyweight tops the scale');

// A ratio exactly on a multiple counts as having reached it — a 100 kg lifter
// who deadlifts 150 is Novice, not "nearly Novice".
eq(levelFor(1.5, DEAD), 1, 'a ratio sitting exactly on a standard reaches it');
eq(levelFor(0.99, DEAD), -1, 'a ratio under the first standard is below the scale');
eq(levelFor(0, DEAD), -1, 'zero is below the scale — which is why zero must never be a stand-in for unknown');
