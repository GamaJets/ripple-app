// The coach's program, kept on the phone, for the room it is trained in.
//
// ── The gap ────────────────────────────────────────────────────────────────
//
...
// — the program their coach wrote for them — has no copy anywhere. On a cold
...
// cache". What this file adds is the three judgements that are about programs
...
//      whole answer is a member whose program was on the part we never read
//      being told, from the device, that they have none.
//
//   2. THAT AN EMPTY ANSWER IS AN ANSWER. A member taken off their program
...
/**
 * How old a cached program may be and still be worth drawing.
 *
 ...
 *
 * WHAT THE AGE ACTUALLY MEASURES. Not how old the program is — how long it
 ...
 * would take a member's program away in week two of an eight-week block, and
 * hand them the generic auto-generated program in its place — which is
 ...
 * program built from the member's own profile is the better answer.
 */
 ...
 *
 * Driven by `programs`, because a start date without a program is nothing to
 ...
 *
 * A row missing its program is dropped rather than failing the whole file: a
