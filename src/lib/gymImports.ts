 ...
 */
export function paymentKeyContent(p: PaymentRow): string {
  return [
 ...
 * purpose.
 */
export function paymentImportKey(p: PaymentRow, nth: number): string {
  return `${paymentKeyContent(p)}#${nth}`;
}

/** One line, ready to write: its number in the sheet and its key. */
export interface KeyedPayment {
  line: number;
  payment: PaymentRow;
  key: string;
}
 ...
 */
export function keyPaymentRows(
  rows: { line: number; payment: PaymentRow }[],
): KeyedPayment[] {
...
export type ImportKind = 'payments' | 'plans' | 'members';

export interface ImportRun {
  id: string;
  kind: ImportKind;
  startedAt: string;
  /** Null on a run that never reported back — which is the half-landed import
   *  this record exists to make visible, not a tidy absence. */
  finishedAt: string | null;
  rowsOffered: number;
  rowsWritten: number;
  /** Lines the database recognised as already imported. */
  rowsSkipped: number;
  rowsFailed: number;
...
export async function startImportRun(
  ...
  if (!id) throw new Error('The import run could not be opened, so nothing was written.');
  return id;
}
...
export async function finishImportRun(
  sb: Queryable,
  runId: string,
  counts: { written: number; skipped: number; failed: number; note?: string | null },
): Promise<void> {
  ...
  assertWrote('That import receipt', r);
}

export async function fetchImportRuns(
  sb: Queryable, tenantId: string, limit = 20,
): Promise<ImportRun[]> {
...
export interface ImportOutcome {
  ...
  written: number;
  /** Rows it recognised as already imported and skipped. Not failures. */
  skipped: number;
  /** Lines it refused, with the reason, named by their number in the sheet. */
  failed: { line: number; why: string }[];
}
...
export async function importPayments(
  ...
  opts: {
    currency: string;
    memberIdFor: (p: PaymentRow) => string | null;
    recordedBy?: string | null;
  },
): Promise<ImportOutcome> {
  const out: ImportOutcome = { written: 0, skipped: 0, failed: [] };

  const asRow = (k: KeyedPayment) => ({
    tenant_id: tenantId,
  ...
  for (let i = 0; i < rows.length; i += BATCH) {
    ...
    if (!error) {
      const landed = new Set((data ?? []).map((r: any) => r.import_key));
    ...
    for (const k of slice) {
      const { data: one, error: oneErr } = await sb
        ...
        .select('import_key');
      if (oneErr) out.failed.push({ line: k.line, why: (oneErr as any)?.message ?? 'write failed' });
      else if ((one ?? []).length) out.written += 1;
      else out.skipped += 1;
    }
  }
...
export async function undoImportRun(
  sb: Queryable, tenantId: string, runId: string,
): Promise<number> {
  const { data, error } = await sb
    .from('gym_payments')
    .delete()
 ...
 */
export async function importedRowCount(
  sb: Queryable, tenantId: string, runId: string,
): Promise<number> {
  const { data, error } = await sb
    .from('gym_payments')
    ...
    .limit(capLimit());
  if (error) throw error;
  return assertWhole(data, "this import run's payments").length;
}
