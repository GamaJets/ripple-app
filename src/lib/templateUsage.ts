...
import { programSignature } from './groupProgram';
import type { Program } from './programs';
import type { LoadStatus } from '../ui/loadStatus';

/** The little of a template this needs. The screen's own `ProgramTemplate`
...
export interface TemplateUse {
  ...
  /**
   * The line for the library row, or null when there is nothing to say.
   *
   * Null rather than "nobody is on this one" for a template nobody is on AND
   * that nothing was ever built from. A library of twenty rows each carrying a
   * sentence about absence is a screen shouting at a coach for having a library.
   */
  line: string | null;
}

export interface LibraryUsage {
  /** Keyed by template id. Every template handed in has an entry. */
  byId: Readonly<Record<string, TemplateUse>>;
  /**
   * Why no template carries a count, or null when they all do.
   *
   * One sentence for the whole screen rather than one per row: the reason is a
   * fact about a single read, and twenty copies of it is twenty times the noise
...
export function templateUsage(
  templates: readonly UsableTemplate[] | null | undefined,
  programs: Readonly<Record<string, Program>> | null | undefined,
  status: LoadStatus,
): LibraryUsage {
  const list = Array.isArray(templates) ? templates : [];
  const byId: Record<string, TemplateUse> = {};

  if (status !== 'ready') {
    for (const tpl of list) byId[tpl.id] = EMPTY;
    return {
      byId,
  ...
  for (const [clientId, program] of Object.entries(programs ?? {})) {
    ...
    if (seen) seen.push(clientId); else byTitle.set(title, [clientId]);
  }
 ...
 */
function usageLine(on: number, from: number): string | null {
  if (!on && !from) return null;
  if (on && from) {
    return `${people(on)} training this now, and ${from} on ${from === 1 ? 'an edited copy' : 'edited copies'} of it.`;
  }
  if (on) return `${people(on)} training this now.`;
