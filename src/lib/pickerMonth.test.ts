...
import { monthTick, monthTickStart } from './pickerMonth';
...
const at = (y: number, m: number, d: number, h = 12, min = 0) => new Date(y, m, d, h, min, 0, 0).getTime();
...
{
  const first = monthTick(at(2026, 8, 1, 0, 0));
  eq(monthTick(at(2026, 8, 15)), first, 'the middle of September is September');
  eq(monthTick(at(2026, 8, 30, 23, 59)), first, 'and the last minute of it still is');
  eq(monthTick(at(2026, 8, 1, 0, 0) + 1), first, 'one millisecond past midnight on the 1st has not moved it');
}

/* ── one more at the boundary ─────────────────────────────────────────────── */

{
  eq(monthTick(at(2026, 9, 1, 0, 0)) - monthTick(at(2026, 8, 30, 23, 59)), 1,
    'midnight on 1 October is exactly one month past the last minute of September — this is the moment a month picker must gain a month');
  eq(monthTick(at(2027, 0, 1)) - monthTick(at(2026, 11, 31)), 1,
    'and across the year boundary, where a naive month-only counter goes backwards by eleven');
  eq(monthTick(at(2026, 11, 1)) - monthTick(at(2026, 0, 1)), 11, 'January to December of one year is eleven months');
}

/* ── never backwards ──────────────────────────────────────────────────────── */

{
  let prev = monthTick(at(2025, 0, 1));
  let rises = 0;
  for (let i = 1; i < 36; i++) {
...
{
  for (const [y, m, d] of [[2026, 8, 17], [2026, 0, 1], [2026, 11, 31], [2024, 1, 29]] as const) {
    const tick = monthTick(at(y, m, d));
    const start = monthTickStart(tick);
    eq(monthTick(start.getTime()), tick, `${y}-${m + 1}: the start of a tick's month is in that month`);
...
{
  const before = monthTick(Date.now());
  const dflt = monthTick();
  const after = monthTick(Date.now());
  ok(dflt === before || dflt === after,
    'called with no argument it reads the clock — the same shape every other library function in this tree uses, so a call site that passes nothing is still answering about now');
}
