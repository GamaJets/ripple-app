...
// has no coach.
import { clientWeek, clientWeekLine, type ClientWeek } from './clientBlock';
import { blockPosition } from './programStart';
import { programWeeks, withWeeks } from './programBlock';
...
const block = (n: number): Program => {
  const base: Program = { title: 'A block', focus: [], note: '', days: [{ day: 'Mon', focus: 'Push', exercises: [] }] };
  return withWeeks(base, Array.from({ length: n }, (_, i) => ({
...
const phases: { name: string; w: ClientWeek }[] = [
...
eq(at(null, '2026-09-01', 8), { index: 0, count: 8, reason: 'no-date' },
  'no start date is week one, which is what every assignment in this app has always shown');
eq(at('the 3rd', '2026-09-01', 8), { index: 0, count: 8, reason: 'unreadable' },
  'and an unreadable one is week one too, reported as its own reason rather than folded into the first');
eq(at('2026-09-08', '2026-09-01', 8), { index: 0, count: 8, reason: 'not-started' },
  'a block dated for next week is trainable NOW, on week one — the date does not hold it back');
eq(at('2026-01-01', '2026-09-01', 8), { index: 7, count: 8, reason: 'ended' },
  'and a block that has run out stays on its last week rather than emptying the tab');
...
// scheduled.
{
  ...
  const w = clientWeek(stale, 3);
  eq(w.index, 2, 'but against a three week block it is clamped to the last week that exists');
  ok(w.index < programWeeks(block(3)).length, 'which is inside the block');
}
eq(clientWeek(blockPosition('2026-09-01', '2026-09-15', 8), 0).count, 1,
  'a block claiming no weeks at all is one week, never zero');

/* ── the sentence ───────────────────────────────────────────────────────── */

{
  const w = at('2026-09-01', '2026-09-15', 8);
  const line = clientWeekLine(w, w.index) ?? '';
...
}
{
  ...
  const line = clientWeekLine(w, w.index) ?? '';
  ok(/nothing to wait for/i.test(line),
    'a block dated for later says out loud that week one is already theirs');
}
{
...
}
{
  ...
  const line = clientWeekLine(w, w.index) ?? '';
  ok(/cannot read/i.test(line) && /week one/i.test(line),
    'an unreadable date says both what happened and what is on screen because of it');
}
{
...
// reader, so this walks the whole join rather than trusting the number.
{
  ...
  const w = clientWeek(blockPosition('2026-09-01', '2026-09-22', 4), weeks.length);
  ...
  eq(weeks[0].days[0].focus, p.days[0].focus,
    'while week one is still `days`, which is what a phone that has not been updated renders');
}
