...
import { currencyStepDone } from './currencyStep';
...
const set = (currency: string, from: 'gym' | 'own' = 'own'): MyCurrency =>
  ({ currency, from, gap: null, canSetOwn: false });
const gap = (g: MyCurrencyGap): MyCurrency => ({ currency: null, from: null, gap: g, canSetOwn: false });

/* ── the step ticks ────────────────────────────────────────────────────── */

eq(currencyStepDone(set('GBP', 'own')), true, 'a coach who set their own currency has done it');
eq(currencyStepDone(set('AED', 'gym')), true, 'a coach whose gym set one has done it');

// The whole point of the migration. Before this, `own` resolved to false and
// the step could never tick for an independent coach.
ok(currencyStepDone(set('JPY', 'own')) !== false,
  'setting your own currency is never reported as not done');

/* ── the step is genuinely not done ────────────────────────────────────── */

eq(currencyStepDone(gap('own-unset')), false, 'no gym and nothing chosen is not done');
...
eq(currencyStepDone(gap('gym-unset')), false, 'a gym that has set none is not done');
...
eq(currencyStepDone(gap('reading')), null, 'a read in flight is unknown, not not-done');
eq(currencyStepDone(gap('unreadable')), null, 'a failed read is unknown, not not-done');
eq(currencyStepDone(gap('unavailable')), null, 'an unapplied migration is unknown, not not-done');
eq(currencyStepDone(gap('nowhere')), null, 'no coach record is unknown, not not-done');
eq(currencyStepDone(null), null, 'a read that never came back is unknown');
eq(currencyStepDone(undefined), null, 'undefined is the same');
...
for (const g of ['reading', 'unreadable', 'unavailable', 'nowhere'] as MyCurrencyGap[]) {
  ok(currencyStepDone(gap(g)) !== false, `${g} is never drawn as not done`);
  ok(currencyStepDone(gap(g)) !== true, `${g} is never drawn as done either`);
}

/* ── nothing is ticked on a blank ──────────────────────────────────────── */

eq(currencyStepDone({ currency: '   ', from: 'own', gap: null, canSetOwn: false }), null,
  'whitespace is not a currency, and with no gap to name it the answer is unknown');
