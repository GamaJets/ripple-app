...
import {
  HORIZON_DAYS, QUIET_AFTER_DAYS, STREAK_WORTH_PROTECTING, motivationNudges, type NudgeInput,
} from './motivationNudge';
...
const base: NudgeInput = { now: NOW, streak: 5, trainedToday: false, daysSinceLastSession: 1, eveningHour: 19 };
const n = (over: Partial<NudgeInput> = {}) => motivationNudges({ ...base, ...over });

// ── an unread log says nothing ───────────────────────────────────────────
{
  eq(n({ streak: null }).length, 0,
    'AN UNREAD TRAINING LOG PRODUCES NO NUDGE — a banner computed from a failed read cannot be taken back');
  ...
  eq(n({ streak: null, trainedToday: null, daysSinceLastSession: null }).length, 0,
    'a wholly unread log is silent, which is the state a refused read leaves behind');
}
...
// ── the streak nudge ─────────────────────────────────────────────────────
{
  ...
  eq(n({ streak: 1 }).length, 0,
    `a streak of one is a session, not a pattern (threshold ${STREAK_WORTH_PROTECTING})`);
  eq(n({ streak: STREAK_WORTH_PROTECTING })[0].kind, 'streak-risk', 'and the threshold itself qualifies');
...
// broke at midnight. An hour from now still lands on the day it is about.
{
  ...
  const out = motivationNudges({ ...base, now: late });
  eq(out.length, 1, 'a streak at 10pm is still worth a nudge');
  ok(out[0].at.getTime() > late, 'and it is still in the future');
  ok(out[0].at.getTime() - late <= 61 * 60 * 1000,
    'AN HOUR FROM NOW, NOT TOMORROW EVENING — the streak breaks at midnight, so tomorrow is too late to be a reminder');
}

// ── the quiet week ───────────────────────────────────────────────────────
{
  const quiet = n({ streak: 0, daysSinceLastSession: QUIET_AFTER_DAYS });
  eq(quiet.length, 1, 'a week with nothing logged is worth a nudge');
  eq(quiet[0].kind, 'quiet-week', 'and it is the quiet-week one');
  ok(quiet[0].body.includes(String(QUIET_AFTER_DAYS)), 'which says how long it has been');
  ...
  ok(!/motivat|lazy|slipp|giving up|falling off/i.test(quiet[0].body),
    `AND NEVER AS A DIAGNOSIS OF THE PERSON — got "${quiet[0].body}"`);

  eq(n({ streak: 0, daysSinceLastSession: QUIET_AFTER_DAYS - 1 }).length, 0,
    'six days is a rest, not a quiet week');
  ...
  eq(n({ streak: 0, daysSinceLastSession: Infinity }).length, 0,
    'A MEMBER WHO HAS NEVER LOGGED ANYTHING IS NOT NUDGED about a week they never had');
}
  ...
  for (const streak of [0, 1, 2, 5, 40]) {
    for (const since of [0, 1, 6, 7, 30, Infinity]) {
      for (const today of [true, false]) {
        const out = motivationNudges({ ...base, streak, trainedToday: today, daysSinceLastSession: since });
        ok(out.length <= 1, `at most one nudge (streak ${streak}, since ${since}, trained ${today}) — got ${out.length}`);
        for (const x of out) ok(x.at.getTime() > NOW, `and it is in the future (streak ${streak}, since ${since})`);
      }
    }
  }
}
...
{
  eq(HORIZON_DAYS, 1,
    'NOTHING IS ARMED BEYOND TODAY — a local notification a week out fires after the member stopped opening the app, computed from a log that was current a week ago, and would tell somebody who trained on Tuesday that they had not trained in a week');
}
