...
import { gymDayBounds, NO_ZONE_NOTE } from './gymZone';

/** Whose clock a window's instants were built on. */
export type WindowBasis = 'gym' | 'device';

/** The shape both `MonthWindow` and `TaxPeriod` already have. Structural on
 *  purpose: this module is not going to grow an opinion about which of them a
 *  screen is holding. */
export interface DayBounded {
  label: string;
  firstDay: string;
  lastDay: string;
  fromIso: string;
...
export interface AtGym<T extends DayBounded> {
  /** The same window, with `fromIso`/`toIso` moved onto the gym's clock where
   *  there was one to move them onto. Every other field is untouched — a
   *  calendar day is a calendar day in any zone. */
  window: T;
  basis: WindowBasis;
 ...
 */
export function cutAtGym<T extends DayBounded>(w: T, zone: string | null | undefined): AtGym<T> {
  const first = gymDayBounds(w.firstDay, zone);
  const last = gymDayBounds(w.lastDay, zone);
  if (!first || !last) {
    return {
      window: w,
      basis: 'device',
