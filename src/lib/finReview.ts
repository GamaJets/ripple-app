...
export function reviewFinances(f: FinInputs, currency: string | null): FinReview {
  ...
  if (membersKnown) {
    ...
    if (growthPct >= 1.5) strengths.push({ tone: 'good', title: `Growing ${num1(growthPct)}% net this month`, detail: `${f.newMembers} joined vs ${f.churnedMembers} left. Momentum is positive: a good time to invest in referrals or a new branch.` });
