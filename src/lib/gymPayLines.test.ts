...
// sent.
import {
  fetchTrainerPay, addClassPay, addAdjustment,
  payRateBlocker, adjustmentBlocker, classPayAmount, classPayBlocker,
  ADJUSTMENT_KINDS,
  type AdjustmentKind, type ClassPayKind,
} from './gymPay';
...
interface Asked {
  table: string;
  /** The column list a read asked for. */
  select: string | null;
  /** Every `.eq()`, in order — the tenant filter is one of these. */
  eqs: Array<[string, unknown]>;
  limit: number | null;
  /** The row an insert carried, or null for a read. */
  insert: Record<string, unknown> | null;
  /** Whether the write asked PostgREST to count what it wrote. */
  counted: boolean;
}

type Answer = { data?: any[] | null; error?: unknown; count?: number | null };

function fakeSb(answer: (q: Asked) => Answer) {
  const asked: Asked[] = [];
  const from = (table: string) => {
    const q: Asked = { table, select: null, eqs: [], limit: null, insert: null, counted: false };
    asked.push(q);
    const chain: any = {
      select: (cols: string) => { q.select = cols; return chain; },
      eq: (col: string, v: unknown) => { q.eqs.push([col, v]); return chain; },
      limit: (n: number) => { q.limit = n; return chain; },
      insert: (row: Record<string, unknown>, o?: { count?: string }) => {
        q.insert = row; q.counted = o?.count === 'exact'; return chain;
      },
      then: (res: (v: Answer) => unknown) => {
        const a = answer(q);
...
const payRow = (o: Record<string, unknown> = {}) => ({
...
async function readPay(rows: any[], tenantId = 'gym-1') {
  const { sb, asked } = fakeSb(() => ({ data: rows, error: null }));
  const index = await fetchTrainerPay(sb, tenantId);
  return { index, asked };
}

/** What one call to `addClassPay` sent. */
async function sentClassPay(line: Parameters<typeof addClassPay>[2], count: number | null = 1) {
  const { sb, asked } = fakeSb(() => ({ error: null, count }));
  await addClassPay(sb, 'gym-1', line);
  return asked[0];
}

/** What one call to `addAdjustment` sent. */
async function sentAdjustment(a: Parameters<typeof addAdjustment>[2], count: number | null = 1) {
  const { sb, asked } = fakeSb(() => ({ error: null, count }));
  await addAdjustment(sb, 'gym-1', a);
  return asked[0];
}

const threw = async (run: () => Promise<unknown>): Promise<Error | null> => {
...
async function main() {
  /* ── fetchTrainerPay: the query ──────────────────────────────────────────── */
  {
    ...
    eq(asked[0].table, 'gym_trainer_pay',
      'what the GYM pays, never trainers.session_fee — that column is the coach’s own price list for their own clients');

    const tenant = asked[0].eqs.filter(([c]) => c === 'tenant_id');
    eq(tenant.length, 1, 'the read is scoped to one tenant');
    eq(tenant[0][1], 'gym-1',
      'and to the tenant asked for — without this filter one gym prices its payroll off another gym’s rates');
    ...
    eq(asked[0].limit, ROW_CAP + 1,
      'capped one past the row cap, so a truncated read can be told from a complete one');
  ...
  {
    const rows = Array.from({ length: ROW_CAP + 1 }, (_, i) => payRow({ trainer_id: `t${i}` }));
    const { sb } = fakeSb(() => ({ data: rows, error: null }));
    const e = await threw(() => fetchTrainerPay(sb, 'gym-1'));
    ok(e != null,
      'a read that came back at its limit REFUSES — a truncated one drops the coaches whose rows fell off the end back onto the gym’s standard fee, which for a senior coach is a smaller payslip that looks exactly like a correct one');

    const whole = Array.from({ length: ROW_CAP }, (_, i) => payRow({ trainer_id: `t${i}` }));
    const { index } = await readPay(whole);
    eq(index.size, ROW_CAP, 'and a set that merely ends AT the cap is a whole set');
  }
  ...
  /* ── fetchTrainerPay: a refused read is not an empty one ─────────────────── */
  {
    ...
    const e = await threw(() => fetchTrainerPay(sb, 'gym-1'));
    ok(e != null,
      'an error throws rather than returning an empty index — an empty index prices every coach at the gym’s standing fee and nothing on screen says the rates were never read');
  }
  ...
  /* ── fetchTrainerPay: what the row means ─────────────────────────────────── */
  {
    ...
    eq(index.get('t2')?.sessionRateCents, 0,
      'a rate of ZERO survives the read as zero — it is a claim the gym made, not an absent rate');
    eq(index.get('t3')?.sessionRateCents, null,
      'and an absent rate stays null, which falls back to the gym’s fee rather than paying nothing');
  ...
  /* ── fetchTrainerPay: a class_pay_kind nobody knows pays 30 times over ───── */
  {
    ...
    eq(pay.classPayKind, null,
      'a counting method this product does not know is not a counting method');

    ok(classPayBlocker(pay, 30, false) != null,
      'so the class is REFUSED with "this gym has not said what it pays this coach to teach" rather than costed against a rule nobody wrote');
    ...
    // class of thirty.
    eq(classPayAmount('per_head' as unknown as ClassPayKind, 3000, 30), 90_000,
      'because anything that is not per_class is multiplied — 30.00 for the class would have been 900.00 for teaching it');
  }
  ...
  /* ── addClassPay: the amount is taken, never recomputed ──────────────────── */
  {
    ...
    eq(q.insert?.amount_cents, 7000,
      'the amount handed in is the amount written down, even when the rate and the headcount would multiply to another one');
  }
  ...
  /* ── addClassPay: attendees on a flat-rate line ──────────────────────────── */
  {
    ...
    eq(q.insert?.attendees, null,
      'a flat-rate line carries NO headcount even when the caller offers one — the register did not enter into the amount, and the constraint refuses the row outright, so the coach is paid nothing for a class everybody thinks was queued');
    ...
    eq(zero.insert?.attendees, 0,
      'a per-attendee line for a class nobody came to records nought, not nothing — nought is a register that was taken');
  }
  ...
  /* ── addClassPay: counted, and refused ───────────────────────────────────── */
  {
    ...
    const e1 = await threw(() => addClassPay(refused, 'gym-1', {
      classId: 'k1', trainerId: 't1', payKind: 'per_class',
      rateCents: 3000, attendees: null, amountCents: 3000, currency: 'GBP', createdBy: null,
    }));
    ...
    // catches.
    ok(/row-level security/.test((e1 as any)?.message ?? ''),
      'and throws what the server said, which is the half of the sentence the owner can act on');

    const { sb: silent } = fakeSb(() => ({ error: null, count: 0 }));
    const e2 = await threw(() => addClassPay(silent, 'gym-1', {
      classId: 'k1', trainerId: 't1', payKind: 'per_class',
      rateCents: 3000, attendees: null, amountCents: 3000, currency: 'GBP', createdBy: null,
    }));
    ok(e2 != null,
      'and so does an insert the server accepted while writing nothing — the owner sees the class move to "on payroll", the run finds no line, and the coach is not paid');
  }
  ...
  /* ── addAdjustment: the sign comes from the kind, never from the typing ──── */
  {
    ...
    const takesAway: Record<AdjustmentKind, boolean> = {
    ...
    for (const kind of ADJUSTMENT_KINDS) {
      ...
      eq(q.insert?.amount_cents, takesAway[kind] ? -5000 : 5000,
        `a ${kind} of 50.00 is stored ${takesAway[kind] ? 'negative' : 'positive'} — the sign is the meaning of the kind, and a screen that asked somebody to type the minus will one day be handed a plus`);
    ...
    eq(twice.insert?.amount_cents, -5000,
      'a deduction handed in already negative stays a deduction — applying the sign twice would pay the coach an extra 100.00');
    ...
    eq(wrongWay.insert?.amount_cents, 5000,
      'and a bonus typed with a minus in front of it is still a bonus');
  }
  ...
  /* ── addAdjustment: the rest of the row ──────────────────────────────────── */
  {
    ...
    eq(q.insert?.currency, 'KWD',
      'in the currency it was recorded in — a three-place currency reaches the column exactly as it was passed, with nothing converted and nothing assumed');
    eq(q.insert?.note, 'parking at the away class',
      'the reason is trimmed, because a note of spaces is a line a coach queries and nobody can answer');
    ...
    eq(q.insert?.applies_on, '2026-09-01',
      'the date the adjustment applies ON is the date that was chosen, not the date it was typed');
    eq(q.insert?.created_by, 'u2', 'and whoever recorded it');

    ok(q.counted, 'the insert asks the server how many rows it wrote');

    const { sb: silent } = fakeSb(() => ({ error: null, count: 0 }));
    const e = await threw(() => addAdjustment(silent, 'gym-1', {
      trainerId: 't7', kind: 'deduction', amountCents: 1250, currency: 'GBP',
      note: 'kit', appliesOn: '2026-09-01', createdBy: null,
    }));
    ok(e != null,
      'an insert the server accepted while writing nothing throws — a deduction that reports itself recorded and is not there never comes off the run');

    const { sb: refused } = fakeSb(() => ({ error: { message: 'new row violates row-level security policy' } }));
    const e2 = await threw(() => addAdjustment(refused, 'gym-1', {
      trainerId: 't7', kind: 'deduction', amountCents: 1250, currency: 'GBP',
      note: 'kit', appliesOn: '2026-09-01', createdBy: null,
    }));
    ok(/row-level security/.test((e2 as any)?.message ?? ''),
      'and a refusal reaches the owner in the server’s own words rather than as a sentence written here');
  }
  ...
  /* ── the sentence about the gym’s currency, which nobody could reach ─────── */
  {
    ...
    const s = payRateBlocker('45', '', '', null);
    ok(s != null, 'a rate with no currency behind it is still refused');
    ok(!/^Session rate:/.test(s ?? ''),
      'and not as a field the parser could not read — the missing thing is the GYM’S currency, not the typing');
    ok(/currency/i.test(s ?? ''),
      'the sentence names what is missing');
    ok(/actually paid/.test(s ?? ''),
      'and says why it cannot be skipped: a rate is what somebody is actually paid');

    const c = payRateBlocker('', '8', 'per_attendee', null);
    ok(c != null && !/^Class rate:/.test(c) && /actually paid/.test(c),
      'the class rate reaches the same sentence — either box is a rate');

    eq(payRateBlocker('0', '', '', null), s,
      'a rate of zero is a rate, and gets the same answer — it is the currency that is missing, not the amount');

    eq(payRateBlocker('', '', '', null), null,
      'clearing both rates needs no currency, because nothing is left denominated — a gym without a currency must still be able to reach that state');

    eq(payRateBlocker('  ', ' ', '', null), null,
      'and a box left with a stray space in it is still a cleared box — the same emptiness parseRate reads as "clear", so the two cannot disagree about whether a currency is needed');

    ok(/counted|pays nothing/.test(payRateBlocker('', '', 'per_class', null) ?? ''),
      'and a counting method with no rate beside it is still answered on its own terms, not with the currency sentence');

    // With a currency the ordinary refusals are unchanged, which is the half a
    // reordering could quietly break.
    eq(payRateBlocker('45', '', '', 'GBP'), null, 'a session rate in a stated currency saves');
    eq(payRateBlocker('', '8', 'per_attendee', 'GBP'), null, 'and so does a class rate with its counting method');
    ok(/^Session rate:/.test(payRateBlocker('forty five', '', '', 'GBP') ?? ''),
      'a rate that cannot be read is still reported as the rate that cannot be read, box and all');
    ok(payRateBlocker('', '8', '', 'GBP') != null,
      'and a class rate with no counting method is still refused — "80" and "8 a head" are the same digits and different money');
  }

  {
    const a = adjustmentBlocker('50', 'covered Saturday', null);
    ok(a != null, 'an adjustment with no currency behind it is refused');
    ok(/currency/i.test(a ?? '') && !/Nothing can be worked out from it/.test(a ?? ''),
      'and told that the gym has not set its currency, rather than that the amount could not be read');

    ok(/Enter the amount/.test(adjustmentBlocker('', 'covered Saturday', null) ?? ''),
      'an empty box with no currency is still answered about the box — that is the one the owner can fix from where they are standing');
    ok(/Enter the amount/.test(adjustmentBlocker('   ', 'covered Saturday', null) ?? ''),
      'and so is a box holding only spaces');

    eq(adjustmentBlocker('50', 'covered Saturday', 'GBP'), null, 'an amount and a reason in a stated currency records');
    ok(adjustmentBlocker('50', '   ', 'GBP') != null, 'and a reason of spaces is no reason');
