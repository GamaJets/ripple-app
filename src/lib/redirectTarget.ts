...
export const HOSTILE_SCHEMES: readonly string[] = ['javascript', 'data', 'vbscript', 'file'];
 ...
 */
export const parseRedirectAllow = (raw: string | null | undefined): string[] => {
...
const under = (url: string, entry: string): boolean => {
  if (!url.startsWith(entry)) return false;
...
export const checkRedirect = (
  ...
  if (offered === undefined || offered === null || offered === '') return { ok: true, url: fallback };
  ...
  if (!url) return { ok: true, url: fallback };
  ...
  if (!allow.length) return { ok: true, url };

  const lower = url.toLowerCase();
  if (allow.some((entry) => under(lower, entry))) return { ok: true, url };
