...
import {
  ...
  refundHasSettled, refundsToMirror, refundTakenAt, refundNote,
  gymRefundRow, overReversedBy, refusalNote, refundStateForOrder,
  type LedgerMethod, type RefundFacts, type ReversibleSale,
...
const full: RefundFacts = { id: 're_1', amountCents: 9000, currency: 'gbp', createdSeconds: REFUNDED_ON, status: 'succeeded' };
const part: RefundFacts = { id: 're_2', amountCents: 2000, currency: 'gbp', createdSeconds: REFUNDED_ON, status: 'succeeded' };

/* ── which refunds are money that has actually moved ───────────────────────── */

ok(refundHasSettled({ status: 'succeeded' }), 'a settled refund is money that has gone back');
ok(!refundHasSettled({ status: 'pending' }), 'a pending refund has not, and mirroring it would understate the gym');
ok(!refundHasSettled({ status: 'failed' }), 'a failed refund never happened');
ok(!refundHasSettled({ status: 'canceled' }), 'nor did a cancelled one');
ok(refundHasSettled({ status: null }), 'a refund stating no status at all is taken as settled rather than silently skipped');
ok(refundHasSettled({ status: ' SUCCEEDED ' }), 'and the test is not upset by case or spacing');
...
eq(refundsToMirror([full, part], []).length, 2, 'a charge with two refunds and nothing recorded mirrors both');
eq(refundsToMirror([full, part], ['re_1']).length, 1, 'a refund already in the ledger is not written again');
eq(refundsToMirror([full, part], ['re_1'])[0].id, 're_2', 'and it is the OTHER one that is left to write');
eq(refundsToMirror([full, part], ['re_1', 're_2']).length, 0,
  'a redelivery of an event whose refunds are all recorded writes nothing at all');
eq(refundsToMirror([full, full], []).length, 1,
  'the same refund handed twice in one delivery is collapsed — two inserts of one id would collide with each other, which is a different failure from the one the index is there to raise');
eq(refundsToMirror([full, { ...part, status: 'pending' }], []).length, 1,
  'a pending refund is not mirrored, so a retry after it settles is what records it');
eq(refundsToMirror([{ ...full, id: '  ' }], []).length, 0,
  'a refund with no id cannot be recorded once and only once, so it is not recorded at all here');
eq(refundsToMirror(null, null).length, 0, 'and an event carrying no refunds asks for nothing');
...
const partial = gymRefundRow({ sale: SALE, refund: part, fallbackAtISO: AT }).row!;
...
eq(partial.taken_at, '2026-09-14T00:00:00.000Z',
  'dated when the refund was MADE — part 180: a refund handed back in September belongs in September, not backdated into a filed August');

const whole = gymRefundRow({ sale: SALE, refund: full, fallbackAtISO: AT }).row!;
eq(whole.amount_cents, -9000, 'a full refund reverses the whole sale, because that is what was refunded');

const twoPartials = [
  gymRefundRow({ sale: SALE, refund: part, fallbackAtISO: AT }).row!,
  gymRefundRow({ sale: SALE, refund: { ...part, id: 're_3' }, fallbackAtISO: AT }).row!,
];
eq(twoPartials[0].amount_cents + twoPartials[1].amount_cents, -4000,
  'two partial refunds of 2000 net to 4000 off the ledger and not to 6000 — this is what keying on the refund rather than assigning the running total buys');

/* ── when a reversal cannot honestly be written ────────────────────────────── */

const wrongMoney = gymRefundRow({ sale: SALE, refund: { ...part, currency: 'aed' }, fallbackAtISO: AT });
eq(wrongMoney.row, null, 'a refund in a currency the payment was not taken in is refused, never netted');
ok((wrongMoney.refusal ?? '').includes('AED') && (wrongMoney.refusal ?? '').includes('GBP'),
  'and the refusal names both currencies, because it is read by an owner and not by a log');

eq(gymRefundRow({ sale: SALE, refund: { ...part, amountCents: 0 }, fallbackAtISO: AT }).row, null,
  'a zero reversal reverses nothing — legal on the sale side for a discounted joining fee, refused here by part 180’s CHECK');
eq(gymRefundRow({ sale: SALE, refund: { ...part, amountCents: null }, fallbackAtISO: AT }).row, null,
  'and a refund stating no amount is not guessed at');
eq(gymRefundRow({ sale: { ...SALE, currency: '' }, refund: part, fallbackAtISO: AT }).row, null,
  'a payment with no currency on it cannot be reversed in a currency invented here');
ok(gymRefundRow({ sale: SALE, refund: { ...part, currency: null }, fallbackAtISO: AT }).row != null,
  'but a refund that simply states no currency takes the payment’s, which is the only money it could have been in');
ok((gymRefundRow({ sale: SALE, refund: { ...part, id: '' }, fallbackAtISO: AT }).refusal ?? '').length > 0,
  'a refund with no id is refused with a reason rather than written unkeyed');

eq(refundTakenAt({ createdSeconds: null }, AT), AT,
  'a refund stating no date falls back to the event’s instant, not to now()');
eq(refundTakenAt({ createdSeconds: 0 }, AT), AT, 'and neither does the epoch count as a date');
ok(refundNote('re_9').includes('re_9'),
  'the note carries the string that finds this refund in the Stripe dashboard, which is the only useful thing it can say');
ok(refundNote('').length > 0, 'and says something even with nothing to point at');

/* ── more back than was ever taken ─────────────────────────────────────────── */

eq(overReversedBy(9000, 0, 2000), 0, 'an ordinary partial refund is not an over-reversal');
eq(overReversedBy(9000, 7000, 2000), 0, 'nor is the instalment that finishes one off exactly');
eq(overReversedBy(9000, 7000, 2500), 500, 'past the sale, the excess is reported in minor units');
eq(overReversedBy(9000, -7000, 2500), 500, 'and reversals stored negative are read as the amounts they are');
eq(overReversedBy(NaN, 0, 2000), 2000, 'a sale with no readable amount treats the whole refund as unaccounted for');

/* ── what goes on the order, and why it is not the ledger ──────────────────── */

const state = refundStateForOrder({ amountRefundedCents: 4000, currency: 'gbp', atISO: AT });
...
eq(refundStateForOrder({ amountRefundedCents: 4000, currency: null, atISO: AT }).refundedCurrency, null,
  'a refund stating no currency records none rather than borrowing the order’s — parts/150 removed the defaults for exactly this');
eq(refundStateForOrder({ amountRefundedCents: 4000, currency: 'pound', atISO: AT }).refundedCurrency, null,
  'and something that is not an ISO code would fail the write, so it is not attempted');
eq(refundStateForOrder({ amountRefundedCents: null, atISO: AT, currency: 'gbp' }).refundedCents, 0,
  'an event stating no total records nothing refunded rather than NaN');

eq(refusalNote([]), null, 'nothing refused leaves no note — which is what clears one an earlier delivery left');
eq(refusalNote(['a', 'a']), 'a', 'the same reason twice says nothing the first one did not');
eq(refusalNote(['a', 'b']), 'a b',
  'two refunds refused for different reasons both get said, because a note reporting only the last would send an owner to fix half of it');
...
eq(onlineOrderProblem({ ...PAID }), null, 'a sale with no refund on it is not an exception');
eq(onlineOrderProblem({ ...PAID, refundedCents: null, reversedCents: 0 }), null,
  'and neither is one Stripe has never mentioned a refund for');
eq(onlineOrderProblem({ ...PAID, refundedCents: 0, reversedCents: 0 }), null,
  'nor a charge whose refund total came back as nothing');
eq(onlineOrderProblem({ ...PAID, refundedCents: 2000, reversedCents: 2000 }), null,
  'a refund that reached the ledger is not an exception — it is an ordinary reversal');
eq(onlineOrderProblem({ ...PAID, refundedCents: 2000, reversedCents: -2000 }), null,
  'and the reversal is stored negative, so the comparison reads its size and not its sign');

const unmirrored = onlineOrderProblem({ ...PAID, refundedCents: 2000, reversedCents: 0 }) ?? '';
ok(unmirrored.includes('not in the payment record'),
  'a refund Stripe made that never reached the ledger is listed, because nothing else on any screen looks wrong');
ok(unmirrored.includes('still counts this money as taken'),
  'and it says what the consequence is, which is the figure the owner is reading');

const half = onlineOrderProblem({ ...PAID, refundedCents: 4000, reversedCents: 2000 }) ?? '';
ok(half.includes('only part'),
  'a partly mirrored refund says so — an owner told "not in the payment record" would go looking for a reversal that is sitting right there');

const withWhy = onlineOrderProblem({
...
ok((onlineOrderProblem({ status: 'paid', inLedger: false, failureNote: null, refundedCents: 5000, reversedCents: 0 }) ?? '')
  .includes('Paid, and not in the payment record'),
  'a sale that never reached the ledger reports THAT and not the refund — there is one thing to fix, not two');
...
/* ── THE INVARIANT, ASSERTED RATHER THAN INSPECTED ─────────────────────────
 *
 * Everything above checks one call at a time. This checks the property that
 * actually matters, by running the webhook's loop over a fake ledger that
 * enforces part 800's unique index the way Postgres would:
 *
 *   replaying the same event twice, and processing two partial refunds in
 *   either order, both leave the same net figure.
 *
 * The fake is deliberately dumb — an array and a Set — because the thing under
 * test is the KEYING decision, not a database. Two shapes were weighed for it:
 *
 *   · ONE ROW PER STRIPE REFUND ID, each row the true delta. Chosen.
 *   · ONE ROW PER CHARGE holding `amount_refunded`, replaced on each event.
 *     Rejected twice over. It contradicts supabase/parts/180 — a correction is
 *     a new row, never an edit to one that stands — and `amount_refunded` is a
 *     RUNNING TOTAL, so a second partial refund carrying it would reverse the
 *     first one a second time. The coach-side path gets away with the running
 *     total only because it ASSIGNS to a column; assignment is idempotent and
 *     an accumulating table cannot assign.
 */

type FakeLedger = { rows: { stripe_refund_id: string; amount_cents: number }[] };

/** The webhook's loop, over a ledger that refuses a duplicate refund id. */
const deliver = (ledger: FakeLedger, refunds: readonly RefundFacts[]): void => {
  const already = ledger.rows.map((r) => r.stripe_refund_id);
  for (const one of refundsToMirror(refunds, already)) {
    const built = gymRefundRow({ sale: SALE, refund: one, fallbackAtISO: AT });
    ...
    if (ledger.rows.some((r) => r.stripe_refund_id === built.row!.stripe_refund_id)) continue;
    ledger.rows.push({ stripe_refund_id: built.row.stripe_refund_id, amount_cents: built.row.amount_cents });
  }
};
const net = (ledger: FakeLedger): number =>
  SALE.amountCents + ledger.rows.reduce((a, r) => a + r.amount_cents, 0);

const a2000: RefundFacts = { id: 're_a', amountCents: 2000, currency: 'gbp', createdSeconds: REFUNDED_ON, status: 'succeeded' };
const b3000: RefundFacts = { id: 're_b', amountCents: 3000, currency: 'gbp', createdSeconds: REFUNDED_ON + 86400, status: 'succeeded' };

// 1. THE SAME EVENT, TWICE. Stripe retries a delivery it did not get a 200 for,
//    and it retries one it did.
const once: FakeLedger = { rows: [] };
deliver(once, [a2000]);
const twice: FakeLedger = { rows: [] };
deliver(twice, [a2000]);
deliver(twice, [a2000]);
...
const forwards: FakeLedger = { rows: [] };
deliver(forwards, [a2000]);
deliver(forwards, [a2000, b3000]);

const backwards: FakeLedger = { rows: [] };
...
eq(SALE.amountCents - runningTotals.reduce((a, b) => a + b, 0), 2000,
  'keying rows on the running total would leave 2000 rather than 4000 — the first refund taken off twice');

// 4. A DELIVERY THAT ARRIVES AFTER EVERYTHING IS ALREADY RECORDED.
const settled: FakeLedger = { rows: [] };
...
eq(refundStateForOrder({ amountRefundedCents: 5000, currency: 'gbp', atISO: AT }).refundedCents, reversed,
  'Stripe’s running total and the sum of the reversals agree once every refund is mirrored — which is exactly when the exception stops being raised');
eq(onlineOrderProblem({ status: 'paid', inLedger: true, failureNote: null, refundedCents: 5000, reversedCents: reversed }), null,
  'so a fully mirrored charge raises nothing, and a derived exception clears itself the moment the last row lands');
