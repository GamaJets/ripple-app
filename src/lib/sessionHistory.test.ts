...
import {
  hasEnded, wasBooked, pastVerdict, pastSessions, tallyPast,
  readBoundary, monthCoverage, rangeCoverage, monthCoverageNote, emptyHistoryLine,
  PAST_STATE_LABEL, PAST_STATE_NOTE, PAST_STATES, CLIENT_CANCELLED_GAP_NOTE,
  type HistoryRow, type PastState,
} from './sessionHistory';
...
const at = (y: number, m1to12: number, d: number, h = 9): string =>
  new Date(y, m1to12 - 1, d, h, 0, 0, 0).toISOString();

const row = (over: Partial<HistoryRow> & { startsAt: string }): HistoryRow => ({
...
// graded on.
{
  ...
  const failed = emptyHistoryLine('error');
  ok(/could not read/i.test(failed), 'a failed read says the read failed');
  ok(/not a record of nothing happening/i.test(failed),
    'and says so in as many words, because that is exactly how a member would otherwise read it');

  const partial = emptyHistoryLine('partial');
  ...
  for (const s of ALL_STATUSES) {
    ok(emptyHistoryLine(s, 'one-to-ones').includes('one-to-ones'),
      `the subject reaches the '${s}' sentence`);
  }
}

/* ── 2. an unmarked session is its own state ───────────────────────────────── */
{
  const unmarked = pastVerdict(row({ startsAt: at(2026, 8, 10), outcome: null }));
  ...
  eq(pastVerdict(row({ startsAt: at(2026, 8, 10), outcome: 'completed', outcomeAt: at(2026, 8, 11) })).state,
    'delivered', "'completed' is delivered");
  eq(pastVerdict(row({ startsAt: at(2026, 8, 10), outcome: 'no_show' })).state,
    'missed', "'no_show' is not attended");
  eq(pastVerdict(row({ startsAt: at(2026, 8, 10), outcome: 'cancelled' })).state,
    'cancelled', "'cancelled' is cancelled");
  eq(pastVerdict(row({ startsAt: at(2026, 8, 10), outcome: 'late_cancelled' })).state,
    'late_cancelled', 'and a late cancellation stays distinct from an in-time one');
  ...
  ok(PAST_STATE_LABEL.cancelled !== PAST_STATE_LABEL.late_cancelled,
    'the two cancellations do not read as the same thing');
  ...
  eq(pastVerdict(row({ startsAt: at(2026, 8, 10), outcome: 'teleported' })).state,
    'unmarked', 'an outcome this build has never heard of is not silently delivered');

  // The wording is gymSessions' own, so the payroll blocker and this screen are
  // plainly about the same thing.
  ok(/still needs an outcome recorded/i.test(PAST_STATE_NOTE.unmarked),
    'unmarked borrows the payroll blocker’s words rather than inventing new ones');
  ok(!/delivered|cancelled/i.test(PAST_STATE_LABEL.unmarked),
    'and its label does not lean on either of the states it is not');

  // Every state has both a label and a note, so no row can render with a hole
  // where its outcome goes.
  for (const s of PAST_STATES) {
    ok((PAST_STATE_LABEL[s as PastState] || '').length > 0, `${s} has a label`);
    ok((PAST_STATE_NOTE[s as PastState] || '').length > 0, `${s} has a sentence`);
  }
}
...
// come out short with nothing on screen saying why.
{
  ...
  const approved = pastVerdict(row({
  ...
  // and objected to.
  const both = pastVerdict(row({ startsAt: at(2026, 8, 10), outcome: null, approvalState: 'disputed' }));
...
/* ── 4. cancelled and disputed sessions stay in the list ───────────────────── */
{
  ...
  const past = pastSessions(rows, NOW);
  ...
  eq(t.delivered + t.cancelled + t.late_cancelled + t.missed + t.unmarked, t.total,
    'the states partition the list — nothing is counted twice and nothing is lost');
}
...
{
  ok(!wasBooked(row({ startsAt: at(2026, 8, 3), status: 'available' })),
    'an open slot nobody took is not a session');
  ok(!wasBooked(row({ startsAt: at(2026, 8, 3), status: 'blocked' })),
    'and neither is time the coach blocked out');
  ok(wasBooked(row({ startsAt: at(2026, 8, 3), status: 'booked' })),
    'a booked slot is');
  ...
  ok(wasBooked(row({ startsAt: at(2026, 8, 3), status: 'available', outcome: 'cancelled' })),
    'and so is any row carrying an outcome, whatever its slot state now says');

  const mixed: HistoryRow[] = [
    row({ startsAt: at(2026, 8, 3), status: 'available' }),
    row({ startsAt: at(2026, 8, 4), status: 'booked' }),
  ];
  eq(pastSessions(mixed, NOW).length, 1, 'and only the booked one reaches the history');
}
...
/* ── 6. history ends at the END of a session, not its start ────────────────── */
{
  ...
  ok(!hasEnded(inProgress, NOW), 'a session being delivered right now is not history');
  ok(hasEnded(row({ startsAt: start.toISOString(), durationMin: 20 }), NOW), 'one that finished is');
  // A duration we cannot read must not be guessed at an hour: the start is the
  // only instant actually known.
  ok(hasEnded(row({ startsAt: start.toISOString(), durationMin: null }), NOW),
    'with no readable duration the start is what is judged, not an invented hour');
  ok(!hasEnded(row({ startsAt: 'not-a-date' }), NOW), 'an unparseable start is never in the past');
  eq(pastSessions([row({ startsAt: 'not-a-date' })], NOW).length, 0,
    'and it does not reach a history list as a session with no date');
}
...
/* ── 7. the boundary of a truncated read ───────────────────────────────────── */
{
  ...
  const whole = readBoundary(rows, false);
  ...
  const cut = readBoundary(rows, true);
  ...
  eq(readBoundary([], true).bounded, false, 'a truncated read with no rows names no boundary');
  eq(readBoundary([{ startsAt: 'rubbish' }], true).oldestISO, null, 'and neither does one with no readable date');
}
...
// month, so it says which day it is complete from instead.
{
  ...
  const b = readBoundary([{ startsAt: at(2026, 6, 14) }, { startsAt: at(2026, 8, 20) }], true);

  eq(monthCoverage(2026, 7, b, 'partial'), 'covered', 'August is entirely inside the read');
  eq(monthCoverage(2026, 6, b, 'partial'), 'covered', 'so is July');
  eq(monthCoverage(2026, 5, b, 'partial'), 'edge', 'June is the month the read stops inside');
  eq(monthCoverage(2026, 4, b, 'partial'), 'beyond', 'May was never asked for');
  eq(monthCoverage(2025, 11, b, 'partial'), 'beyond', 'and neither was December of the year before');

  // A read that came back whole covers everything, however old.
  const wholeB = readBoundary([{ startsAt: at(2026, 6, 14) }], false);
  eq(monthCoverage(1998, 0, wholeB, 'ready'), 'covered',
    'a complete read means an empty month really is empty — this must stay a no-op for everybody under the cap');

  // The one that matters most: nothing is "covered" while the read has failed
  // or is still in flight, whatever the boundary says.
  for (const s of ['error', 'loading'] as LoadStatus[]) {
    eq(monthCoverage(2026, 7, b, s), 'unknown', `a month under '${s}' is unknown, never covered`);
    eq(monthCoverage(2026, 7, wholeB, s), 'unknown', `even with a whole read behind it, '${s}' knows nothing`);
    eq(monthCoverage(2026, 7, readBoundary([], false), s), 'unknown', `and an empty read under '${s}' is not an empty month`);
  }
}
...
/* ── 8b. the edge month, asserted on its own so the label cannot drift ─────── */
{
  ...
  const b = readBoundary([{ startsAt: at(2026, 6, 14) }], true);
  eq(monthCoverage(2026, 5, b, 'partial'), 'edge', 'the month the read stops inside is half-known');
  eq(monthCoverage(2026, 6, b, 'partial'), 'covered', 'the month after it is whole');
  eq(monthCoverage(2026, 4, b, 'partial'), 'beyond', 'the month before it was never read');
  ...
  const onTheFirst = readBoundary([{ startsAt: new Date(2026, 5, 1, 0, 0, 0, 0).toISOString() }], true);
  eq(monthCoverage(2026, 5, onTheFirst, 'partial'), 'covered',
    'a read reaching local midnight on the 1st covers that whole month');
  eq(monthCoverage(2026, 4, onTheFirst, 'partial'), 'beyond', 'and the month before it is still beyond');
}
...
// of them.
{
  ...
  const b = readBoundary([{ startsAt: noonOnTheFirst.toISOString() }], true);
  eq(monthCoverage(2026, 5, b, 'partial'), 'edge',
    'a boundary at local noon on the 1st leaves that month an edge month in every zone');
  eq(monthCoverage(2026, 4, b, 'partial'), 'beyond', 'and the previous month beyond, in every zone');
  eq(monthCoverage(2026, 6, b, 'partial'), 'covered', 'and the next one covered, in every zone');

  // The last local instant of a month belongs to that month, not the next.
  const lastMoment = new Date(2026, 5, 30, 23, 59, 59, 999);
  const lb = readBoundary([{ startsAt: lastMoment.toISOString() }], true);
  eq(monthCoverage(2026, 5, lb, 'partial'), 'edge', '23:59 on the 30th is still June');
  eq(monthCoverage(2026, 6, lb, 'partial'), 'covered', 'and July is untouched by it');

  // December → January is the wrap the year arithmetic gets wrong.
  const newYear = new Date(2026, 0, 3, 9, 0, 0, 0);
  const nb = readBoundary([{ startsAt: newYear.toISOString() }], true);
  eq(monthCoverage(2026, 0, nb, 'partial'), 'edge', 'January 2026 is the edge month');
  eq(monthCoverage(2025, 11, nb, 'partial'), 'beyond', 'December 2025 is beyond it');
  eq(monthCoverage(2026, 1, nb, 'partial'), 'covered', 'February 2026 is inside it');
}
...
/* ── 10. an arbitrary run of days, for the week views ──────────────────────── */
{
  ...
  const b = readBoundary([{ startsAt: at(2026, 8, 26) }], true);   // boundary mid-week
  eq(rangeCoverage(monday, nextMonday, b, 'partial'), 'edge', 'the week the read stops inside is half-known');
  eq(rangeCoverage(nextMonday, nextMonday + 7 * 86_400_000, b, 'partial'), 'covered', 'the week after is whole');
  eq(rangeCoverage(monday - 7 * 86_400_000, monday, b, 'partial'), 'beyond', 'the week before was never read');
  eq(rangeCoverage(monday, nextMonday, b, 'error'), 'unknown', 'and a failed read knows nothing about any week');
}
...
/* ── 11. what the screen actually says at the boundary ─────────────────────── */
{
  ...
  eq(monthCoverageNote('covered', b, label), null,
    'a covered month says nothing — a note on every month is a note nobody reads');

  const beyond = monthCoverageNote('beyond', b, label)!;
  ok(/missing from this screen rather than absent from your record/i.test(beyond),
    'a month beyond the read says the sessions are missing here, not missing from the record');
  ok(beyond.includes('['), 'and names the day the history is loaded back to');

  const edge = monthCoverageNote('edge', b, label)!;
  ...
  for (const c of ['beyond', 'edge', 'unknown'] as const) {
    ok(!/is blank/i.test(monthCoverageNote(c, b, label)!),
      `'${c}' does not claim the month is blank`);
  }

  const unknown = monthCoverageNote('unknown', b, label)!;
  ok(/not a statement that nothing happened/i.test(unknown),
    'and a month under a failed read says outright that blank is not the same as empty');
  ...
  for (const c of ['beyond', 'edge', 'unknown'] as const) {
    const note = monthCoverageNote(c, b, label)!;
  ...
  const noDate = readBoundary([], true);
  for (const c of ['beyond', 'edge'] as const) {
    const note = monthCoverageNote(c, noDate, label)!;
    ok(note.length > 0 && !note.includes('undefined') && !note.includes('null'),
      `'${c}' with no boundary date still reads as a sentence`);
  }
}
...
{
  ok(/cancelled yourself/i.test(CLIENT_CANCELLED_GAP_NOTE), 'the gap is named as the member’s own cancellations');
  ok(/not listed here/i.test(CLIENT_CANCELLED_GAP_NOTE), 'and stated as an omission rather than left to be inferred');
  ok(!/all|every|complete/i.test(CLIENT_CANCELLED_GAP_NOTE), 'and it never claims the list is complete');
}
