...
/** One claimed row, as the dispatcher's `returning` clause hands it over. */
export interface DigestRow {
  /** The coach's switch this row answers to — `CoachChannel`, as a string,
   *  because the dispatcher reads it out of a column and this module never
   *  needs to know which one it is. */
  channel: string;
  title: string;
  body: string;
  /** '' for none, matching the dispatcher's `coalesce(btrim(route), '')`. */
  route: string;
}

/** One post to send-push. */
export interface DigestPush {
  channel: string;
  title: string;
  body: string;
  /** '' when the group has no route. The dispatcher sends `data: {}` for that,
   *  exactly as it does today. */
  route: string;
  /**
   * How many written rows this one push stands for.
   *
   * A COUNT and not an estimate: it is the number of rows the dispatcher's
   * `update … returning` actually claimed, which is the server's own answer to
   * "what did you write". One means the push is that row, untouched.
   */
  stands_for: number;
}
 ...
 */
export function digestBody(body: string, more: number): string {
  const note = digestNote(more);
  const b = (body ?? '').trim();
  if (!note) return b.slice(0, BODY_MAX);
  const room = BODY_MAX - note.length;
  ...
  if (room <= CUT.length) return note.trim().slice(0, BODY_MAX);
 ...
 */
export function digestKey(r: DigestRow): string {
 ...
 */
export function digestPushes(rows: readonly DigestRow[]): DigestPush[] {
  ...
  for (const r of rows ?? []) {
    ...
    if (!body || !channel) continue;
    const key = digestKey({ ...r, channel, body });
    const g = groups.get(key);
    if (g) g.push({ ...r, channel, body });
    else groups.set(key, [{ ...r, channel, body }]);
  }
  const out: DigestPush[] = [];
 ...
 */
export function digestSaving(rows: readonly DigestRow[]): { before: number; after: number } {
