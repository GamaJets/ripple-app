...
const UID_HOST = BRAND.webOrigin.replace(/^https?:\/\//, '').replace(/[\/?#].*$/, '');
...
const PRODID = `-//${BRAND.label}//Sessions//EN`;
...
export function buildIcs(events: IcsEvent[], calName = BRAND.label, nowMs: number = Date.now()): string {
  const stamp = icsStamp(new Date(nowMs).toISOString());
  const lines: string[] = ['BEGIN:VCALENDAR', 'VERSION:2.0', `PRODID:${PRODID}`, 'CALSCALE:GREGORIAN', 'METHOD:PUBLISH', `X-WR-CALNAME:${icsEsc(calName)}`];
  events.forEach((e, i) => {
    const start = new Date(e.start);
    const end = new Date(start.getTime() + (e.durationMin || 60) * 60000);
    lines.push('BEGIN:VEVENT', `UID:${BRAND.id}-${icsStamp(e.start)}-${i}@${UID_HOST}`, `DTSTAMP:${stamp}`, `DTSTART:${icsStamp(e.start)}`, `DTEND:${icsStamp(end.toISOString())}`, `SUMMARY:${icsEsc(e.title)}`);
