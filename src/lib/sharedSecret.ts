 ...
 */
export const secretConfigured = (expected: string | null | undefined): boolean =>
...
export const secretMatches = (
  ...
  if (offered.length !== want.length) return false;
  ...
  let diff = 0;
  for (let i = 0; i < want.length; i++) diff |= offered.charCodeAt(i) ^ want.charCodeAt(i);
  return diff === 0;
};
