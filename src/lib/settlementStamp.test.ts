...
import { recordSettlement } from './gymSessions';
import { fetchDemand } from './gymRota';
import { fetchGymTrainers } from './gymTrainers';
import { ROW_CAP, capLimit } from './rowCap';
...
interface Asked {
  table: string;
  /** The `.limit()` the caller asked for, or null if it asked for none. Null is
   *  the whole point of recording it: an absent limit is the defect. */
  limit: number | null;
  in: string[] | null;
...
function fakeSb(answer: (q: Asked) => Answer) {
  ...
  const from = (table: string) => {
    ...
    const chain: any = {
      ...
      insert: (row: Record<string, unknown>) => { q.insert = row; return chain; },
      ...
      then: (res: (v: Answer) => unknown) => res(answer(q)),
    };
    return chain;
  };
...
async function main() {
  ...
  {
    const N = ID_CHUNK * 2 + 7;
    const sessionIds = ids(N, 'sess');
    const { sb, asked } = fakeSb((q) => {
      if (q.table === 'payroll_settlements') return { data: { id: 'settle-1' }, error: null };
      return { data: null, error: null, count: (q.in ?? []).length };
    });
    const id = await recordSettlement(sb, 'tenant-1', { ...RUN, sessionIds });
    eq(id, 'settle-1', 'the settlement id is handed back');

    const writes = asked.filter((q) => q.table === 'sessions' && q.update);
    eq(writes.length, Math.ceil(N / ID_CHUNK), 'the sessions are stamped in chunks, the last one short');
    ok(writes.every((q) => (q.in ?? []).length <= ID_CHUNK),
      `no chunk carries more than ${ID_CHUNK} ids — past about two hundred the request line is a 414`);
    ok(writes.every((q) => q.counted),
      'every chunk asks PostgREST to count what it changed; RLS filters rather than refuses, so an uncounted PATCH cannot tell a stamp from a no-op');
    eq(writes.reduce((a, q) => a + (q.in ?? []).length, 0), N,
      'every id is sent exactly once across the chunks');
  ...
  /* ── the total is the DEDUPLICATED one, on both writes ──────────────────── */
  {
    ...
    const { sb, asked } = fakeSb((q) => {
      if (q.table === 'payroll_settlements') return { data: { id: 'settle-2' }, error: null };
      return { data: null, error: null, count: (q.in ?? []).length };
    });
    const why = await said(() => recordSettlement(sb, 'tenant-1', { ...RUN, sessionIds }));
    ...
    eq(asked.find((q) => q.table === 'payroll_settlements')?.insert?.sessions_count, 5,
      'and the permanent payment record counts rows, not mentions');
  }
  ...
  /* ── a short count is a partial stamp, named as one ─────────────────────── */
  {
    ...
    let seen = 0;
    const { sb } = fakeSb((q) => {
      if (q.table === 'payroll_settlements') return { data: { id: 'settle-3' }, error: null };
      seen += 1;
    ...
    const why = await said(() => recordSettlement(sb, 'tenant-1', { ...RUN, sessionIds: ids(N, 'part') }));
    ok(why.includes(`only ${ID_CHUNK} of ${N} sessions were stamped`),
      `the partial stamp is counted across chunks and named — got ${JSON.stringify(why)}`);
    ok(why.includes('settled twice'),
      'and it says what the unstamped remainder costs, which is the reason to reload before trying again');
  }
  ...
  /* ── a chunk that is refused says how much had already landed ───────────── */
  {
    ...
    let seen = 0;
    const { sb } = fakeSb((q) => {
      if (q.table === 'payroll_settlements') return { data: { id: 'settle-4' }, error: null };
      seen += 1;
      if (seen === 1) return { data: null, error: null, count: ID_CHUNK };
      return { data: null, error: { message: 'URI too long' }, count: null };
    });
    const why = await said(() => recordSettlement(sb, 'tenant-1', { ...RUN, sessionIds: ids(N, 'fail') }));
    ok(why.includes(`after ${ID_CHUNK} of ${N}`),
      `a refused chunk is reported with the running total attached — got ${JSON.stringify(why)}`);
  ...
  {
    const { sb } = fakeSb((q) => {
      if (q.table === 'payroll_settlements') return { data: { id: 'settle-5' }, error: null };
      return { data: null, error: null, count: null };
    });
    const why = await said(() => recordSettlement(sb, 'tenant-1', { ...RUN, sessionIds: ids(3, 'nocount') }));
    ok(why.includes('did not say how many'),
      `an uncounted write is named as uncounted, not added in as a zero — got ${JSON.stringify(why)}`);
  }

  /* ── fetchDemand: both reads are bounded, and a truncated week refuses ──── */
  {
    const { sb, asked } = fakeSb(() => ({ data: [], error: null }));
    await fetchDemand(sb, 'tenant-1', '2026-08-03T00:00:00Z', '2026-08-10T00:00:00Z');
    const classes = asked.find((q) => q.table === 'gym_classes');
    const sessions = asked.find((q) => q.table === 'sessions');
    eq(classes?.limit, capLimit(), 'the classes read asks for one row past the cap, so a full page and a cut one differ');
    eq(sessions?.limit, capLimit(), 'and so does the one-to-ones read');
  }
  {
    // The probe row came back: the week is bigger than the read.
    const rows = Array.from({ length: ROW_CAP + 1 }, () => ({
      title: 'Spin', trainer_id: 't1', starts_at: '2026-08-04T18:00:00Z', duration_min: 45,
    }));
    const { sb } = fakeSb((q) => (q.table === 'gym_classes'
      ? { data: rows, error: null }
      : { data: [], error: null }));
    const why = await said(() => fetchDemand(sb, 'tenant-1', 'a', 'b'));
    ok(why.includes('the classes booked this week'),
      `a truncated demand read refuses by name rather than handing back a prefix — got ${JSON.stringify(why)}`);
  }
  {
    const rows = Array.from({ length: ROW_CAP + 1 }, () => ({
    ...
    const { sb } = fakeSb((q) => (q.table === 'sessions'
      ...
      : { data: [], error: null }));
    const why = await said(() => fetchDemand(sb, 'tenant-1', 'a', 'b'));
    ok(why.includes('the one-to-ones booked this week'),
      `and the two halves of demand refuse separately, so the owner is told which — got ${JSON.stringify(why)}`);
    ok(why.includes('Refusing to report a figure'),
      'with the sentence rowCap.ts writes, which is the one an owner sees');
  }

  /* ── fetchGymTrainers: the roster is bounded and the names are checked ──── */
  {
    const { sb, asked } = fakeSb((q) => {
      if (q.table === 'trainers') return { data: [{ id: 't1' }], error: null };
      if (q.table === 'profiles') return { data: [{ id: 't1', full_name: 'Ada', created_at: '2026-01-01' }], error: null };
      return { data: [], error: null };
    });
    const out = await fetchGymTrainers(sb, 'tenant-1');
    eq(asked.find((q) => q.table === 'trainers')?.limit, capLimit(),
      'the trainers read asks for one row past the cap, like every read under it');
    eq(out.length, 1, 'and a whole read still comes back');
    eq(out[0]?.name, 'Ada', 'with the name off profiles');
  }
  {
    const rows = Array.from({ length: ROW_CAP + 1 }, (_, i) => ({ id: `t${i}` }));
    const { sb } = fakeSb((q) => (q.table === 'trainers' ? { data: rows, error: null } : { data: [], error: null }));
    const why = await said(() => fetchGymTrainers(sb, 'tenant-1'));
    ok(why.includes("this gym's trainers"),
      `a truncated roster refuses rather than paying a short payroll — got ${JSON.stringify(why)}`);
  }
  {
    const { sb } = fakeSb((q) => {
      if (q.table === 'trainers') return { data: [{ id: 't1' }], error: null };
      if (q.table === 'profiles') return { data: null, error: { message: 'permission denied for table profiles' } };
      return { data: [], error: null };
    });
    const why = await said(() => fetchGymTrainers(sb, 'tenant-1'));
    ok(why.includes('permission denied'),
      `a refused profiles read throws instead of reporting every coach as having no join date — got ${JSON.stringify(why)}`);
  }
