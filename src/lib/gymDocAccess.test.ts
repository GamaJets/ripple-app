...
import {
  documentAudience, STAFF_READABLE_KINDS, AUDIENCE_LABEL, DOCUMENT_KINDS,
  objectRemoved, absentFromListing, deleteDocument, openDocument, recordDocumentRead,
  GYM_DOCS_BUCKET, SIGNED_URL_TTL_S, type DocumentKind, type GymDocument,
} from './gymDocs';
...
const doc = (over: Partial<GymDocument> = {}): GymDocument => ({
  id: 'doc-1',
  memberId: null,
...
for (const kind of DOCUMENT_KINDS) {
  eq(documentAudience({ memberAttached: true, kind }), 'owner',
    `a ${kind} filed against a member is the owner's alone`);
}

eq(documentAudience({ memberAttached: false, kind: 'service_report' }), 'staff',
  "an engineer's report on a machine is the floor's — that is the case part 185 was right about");
eq(documentAudience({ memberAttached: false, kind: 'photo' }), 'staff',
  'and so is the photograph of the broken machine');
eq(documentAudience({ memberAttached: false, kind: 'certificate' }), 'staff',
  'first aid, gas safety and fire certificates are what staff need to see are current');
eq(documentAudience({ memberAttached: false, kind: 'insurance' }), 'staff',
  'as is the public-liability schedule');

eq(documentAudience({ memberAttached: false, kind: 'contract' }), 'owner',
  'a loose contract is a lease or a supplier agreement and is not the floor\'s business');
eq(documentAudience({ memberAttached: false, kind: 'incident' }), 'owner',
  'an incident report is an account of somebody being hurt, and there is no version of it a trainer needs the file of');
eq(documentAudience({ memberAttached: false, kind: 'other' }), 'owner',
  "'other' is where a GP letter lands when nobody picked a kind, so it fails closed");

ok((['owner', 'staff'] as const).every((a) => AUDIENCE_LABEL[a]), 'both audiences have words a screen shows');
...
eq(documentAudience({ memberAttached: true, kind: 'incident' }), 'owner',
  'an erased member\'s incident report does not become gym-wide paperwork when their id goes');
...
if (partFile) {
  ...
  if (m) {
    ...
    eq(inSql.join('|'), [...STAFF_READABLE_KINDS].join('|'),
      'the kinds a trainer may read differ between src/lib/gymDocs.ts and supabase/parts/390');
  }
  ...
  ok(/drop policy if exists gym_documents_staff_r/.test(sql),
    'the tenant-wide staff read policy must be dropped by name, not just replaced');
  ...
  ok(/create policy gymdoc_obj_read on storage\.objects[\s\S]{0,300}gym_doc_object_readable\(name\)/.test(sql),
    'the storage read policy must defer to gym_doc_object_readable, not carry its own copy of the rule');
  ok(/create policy gym_documents_read on public\.gym_documents[\s\S]{0,200}gym_doc_readable\(tenant_id, member_attached, kind\)/.test(sql),
    'and the table read policy must defer to gym_doc_readable');
}
...
ok(objectRemoved(PATH, [{ name: PATH }]), 'storage naming the full key is a removal');
ok(objectRemoved(PATH, [{ name: BASE }]), 'and so is storage naming the leaf');
ok(!objectRemoved(PATH, []), 'an EMPTY list is not a removal — that is what a refused delete looks like');
ok(!objectRemoved(PATH, null), 'and neither is nothing at all');
ok(!objectRemoved(PATH, [{ name: 'some-other-file.pdf' }]), 'a different file going is not this one going');

ok(absentFromListing(PATH, []), 'a folder that does not list it is a folder it has gone from');
ok(!absentFromListing(PATH, [{ name: BASE }]), 'a folder that still lists it is a delete that did not happen');
ok(!absentFromListing(PATH, null), 'and a listing that could not be read is not evidence of absence');

/* ── removing one ──────────────────────────────────────────────────────────── */

interface Calls { removed: string[][]; listed: number; deleted: string[] }

function fakeDb(opts: {
  removeError?: unknown; removeData?: unknown;
  listError?: unknown; listData?: unknown;
  deleteError?: unknown; deleteCount?: number | null;
}) {
  const calls: Calls = { removed: [], listed: 0, deleted: [] };
  const sb = {
    from: (table: string) => ({
      delete: (_o?: unknown) => ({
    ...
    storage: {
      from: (b: string) => {
        eq(b, GYM_DOCS_BUCKET, 'the file is looked for in the gym bucket');
        return {
          remove: (paths: string[]) => {
            calls.removed.push(paths);
            return Promise.resolve({ data: opts.removeData ?? [{ name: paths[0] }], error: opts.removeError ?? null });
          },
          list: (_dir: string, _o?: unknown) => {
            calls.listed += 1;
...
void (async () => {
  {
    ...
    const why = await threw(deleteDocument(sb as any, { id: 'doc-1', storagePath: PATH }));
...

  {
    ...
    const why = await threw(deleteDocument(sb as any, { id: 'doc-1', storagePath: PATH }));
    ok(why != null && /cannot be confirmed gone is not gone/.test(why),
      'a listing that could not be read is not permission to claim the file went');
  }

  {
    ...
    const why = await threw(deleteDocument(sb as any, { id: 'doc-1', storagePath: PATH }));
    ok(why != null && /HAS been deleted from storage/.test(why),
      'when only the row survives the owner is told the file has gone');
    ok(why != null && /Press Remove again/.test(why), 'and told how to finish it');
  }

  /* ── opening one ─────────────────────────────────────────────────────────── */

  function fakeOpen(opts: { insertError?: unknown; signError?: unknown }) {
    const seen = { inserted: [] as any[], signed: 0 };
    const sb = {
      from: (table: string) => ({
        insert: (row: any) => {
      ...
      storage: {
        from: (_b: string) => ({
          createSignedUrl: (p: string, ttl: number) => {
            seen.signed += 1;
            eq(ttl, SIGNED_URL_TTL_S, 'the link is short-lived');
...

  {
    ...
    const url = await openDocument(sb as any, 'tenant-1', d, 'owner-1');
...

  {
    ...
    const why = await threw(openDocument(sb as any, 'tenant-1', d, 'owner-1'));
...

  {
    ...
    const { sb, seen } = fakeOpen({});
    await openDocument(sb as any, 'tenant-1', doc({ kind: 'certificate' }), 'owner-1');
...

  {
    ...
    const why = await threw(openDocument(sb as any, 'tenant-1', doc(), 'owner-1'));
    ok(why != null && /could not be opened/.test(why), 'a link that will not mint is reported rather than swallowed');
  }

  {
    const { sb, seen } = fakeOpen({});
    await recordDocumentRead(sb as any, 'tenant-1', doc({ memberAttached: true }), null);
    eq(seen.inserted[0].read_by, null,
      'a null actor is written as null rather than invented — the database refuses it, which is the point');
  }
