 ...
 */
export const REFERRAL_LINK_BASE = `${BRAND.joinOrigin}/join`;
...
export function isPlausibleReferralCode(input: string): boolean {
...
export function referralLink(code: string): string {
  return `${REFERRAL_LINK_BASE}?r=${encodeURIComponent(normaliseReferralCode(code))}`;
}
