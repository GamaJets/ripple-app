 ...
 * make easy.
 */
export const LOG_BACK_DAYS = 13;

/** One day the coach may file the session under. */
export interface LogDay {
  /** Local `YYYY-MM-DD`. The value the picker holds. */
  day: string;
  /** What the chip says. */
  label: string;
  /** 0 is today. */
 ...
 */
export function logDayOptions(now: Date, back: number = LOG_BACK_DAYS): LogDay[] {
  const out: LogDay[] = [];
 ...
 */
export function logStamp(day: string, hour: number, now: Date): string | null {
  const d = localDate(day);
  if (!d) return null;
 ...
 */
export function logStampProblem(day: string, hour: number, now: Date): string | null {
 ...
 */
export function logWhenLine(day: string, hour: number, now: Date, who: string): string {
  const at = logStamp(day, hour, now);
  if (!at) return 'Pick the day this session happened.';
