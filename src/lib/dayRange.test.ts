...
import { dayAllowed, monthHasAllowedDay, dayRefusal, type DayRange } from './dayRange';
...
ok(dayAllowed('2026-09-07', undefined), 'with no range an ordinary day is allowed');
ok(dayAllowed('2026-09-07', null), 'a null range is no range');
ok(dayAllowed('2026-09-07', {}), 'an empty range object is no range');
ok(dayAllowed('2024-02-29', {}), 'a leap day that exists is allowed');
...
for (const bad of ['', '   ', 'next friday', '2026-9-7', '2026-02-30', '2023-02-29', '2026-13-01', '2026-00-10', '2026-09-07T00:00:00Z', null, undefined]) {
  ok(!dayAllowed(bad, {}), `"${String(bad)}" is not a day and cannot be allowed`);
  ok(!dayAllowed(bad, { min: '2000-01-01', max: '2099-12-31' }), `"${String(bad)}" is not a day even inside a wide range`);
  ...
  const why = dayRefusal(bad, { min: '2026-01-01' }, plain);
...
ok(!isStartDate('2026-02-30'), 'the underlying reader refuses 30 February');
ok(!dayAllowed('2026-02-30', null), '30 February is not a day a range can allow');

/* ── both ends are INCLUSIVE, asserted by walking off them ────────────────── */

const span: DayRange = { min: '2026-09-03', max: '2026-09-07' };
eq(dayAllowed('2026-09-02', span), false, 'the day before the floor is out');
eq(dayAllowed('2026-09-03', span), true, 'the floor itself is in');
eq(dayAllowed('2026-09-05', span), true, 'the middle is in');
eq(dayAllowed('2026-09-07', span), true, 'the ceiling itself is in');
eq(dayAllowed('2026-09-08', span), false, 'the day after the ceiling is out');
...
// issued today.
const oneDay: DayRange = { min: '2026-09-03', max: '2026-09-03' };
eq(dayAllowed('2026-09-03', oneDay), true, 'a one-day range allows its day');
eq(dayAllowed('2026-09-02', oneDay), false, 'and nothing before it');
eq(dayAllowed('2026-09-04', oneDay), false, 'and nothing after it');
...
const backwards: DayRange = { min: '2026-09-07', max: '2026-09-03' };
for (let d = 1; d <= 30; d++) {
  ok(!dayAllowed(isoFromParts(2026, 8, d), backwards), `a backwards range allows no day, including the ${d}th`);
}

/* ── one end only ─────────────────────────────────────────────────────────── */

const floorOnly: DayRange = { min: '2026-09-03' };
ok(!dayAllowed('2026-09-02', floorOnly), 'a floor refuses below it');
ok(dayAllowed('2099-12-31', floorOnly), 'and has no ceiling — a coach who agreed to wait until December sets December');

const ceilOnly: DayRange = { max: '2026-09-03' };
ok(dayAllowed('1999-01-01', ceilOnly), 'a ceiling refuses nothing below it');
ok(!dayAllowed('2026-09-04', ceilOnly), 'and refuses above it');
...
for (const r of junkRanges) {
  for (let m = 0; m < 12; m++) {
    for (let d = 1; d <= daysInMonth(2026, m); d++) {
      const iso = isoFromParts(2026, m, d);
      eq(dayAllowed(iso, r), dayAllowed(iso, null), `an unreadable range must behave as no range on ${iso}`);
    }
    ok(monthHasAllowedDay(2026, m, r), `and must leave every month reachable — ${m}`);
  }
  eq(dayRefusal('2026-06-15', r, plain), null, 'and must refuse nothing readable');
}

// Half unreadable is half a range: the readable end still bites.
const halfJunk: DayRange = { min: '2026-09-03', max: 'whenever' };
ok(!dayAllowed('2026-09-02', halfJunk), 'a readable floor still holds when the ceiling is junk');
ok(dayAllowed('2030-01-01', halfJunk), 'and the junk ceiling stops nothing');
...
const sweepRanges: (DayRange | null)[] = [
  null,
  {},
  { min: '2026-01-01' },
  { min: '2026-02-01' },
  ...
  for (let y = 2025; y <= 2027; y++) {
    for (let m = 0; m < 12; m++) {
      ...
      for (let d = 1; d <= daysInMonth(y, m); d++) {
        if (dayAllowed(isoFromParts(y, m, d), r)) { any = true; break; }
      }
      sweepPairs++;
      eq(monthHasAllowedDay(y, m, r), any,
        `the two-end shortcut must agree with the full sweep for ${y}-${m + 1} under ${JSON.stringify(r)}`);
    }
  }
}
...
ok(monthHasAllowedDay(2024, 1, { min: '2024-02-29' }), 'a leap February is reachable by its 29th alone');
ok(!monthHasAllowedDay(2026, 1, { min: '2026-03-01' }), 'a February wholly below a March floor has nothing in it');
ok(!monthHasAllowedDay(2026, 1, { max: '2026-01-31' }), 'and one wholly above a January ceiling has nothing either');
...
ok(monthHasAllowedDay(2026, 1, { min: '2026-02-29' }), 'a floor on a day that does not exist blocks nothing');
...
eq(monthHasAllowedDay(2026, 12, { min: '2027-01-01', max: '2027-01-31' }),
  monthHasAllowedDay(2027, 0, { min: '2027-01-01', max: '2027-01-31' }),
  'month 12 of 2026 is January 2027');

/* ── the refusal, and that it is exactly the negation of the permission ───── */

const refusalRanges: (DayRange | null)[] = sweepRanges;
for (const r of refusalRanges) {
  for (let m = 0; m < 12; m++) {
    for (let d = 1; d <= daysInMonth(2026, m); d++) {
      const iso = isoFromParts(2026, m, d);
      const allowed = dayAllowed(iso, r);
      const why = dayRefusal(iso, r, plain);
...
const low = dayRefusal('2026-09-02', { min: '2026-09-03', max: '2026-09-30' }, plain);
...
const high = dayRefusal('2026-10-01', { min: '2026-09-03', max: '2026-09-30' }, plain);
...
const shouty = dayRefusal('2026-09-02', { min: '2026-09-03' }, (iso) => `<<${iso}>>`);
...
// for.
eq(dayRefusal('2026-09-02', { min: 'soon' }, plain), null, 'an unreadable floor refuses nothing and names nothing');
...
eq(dayAllowed('2026-01-01', { min: '2026-01-01', max: '2026-01-01' }), true,
  'the boundary day is in, in every zone this suite runs under');
eq(dayAllowed('2025-12-31', { min: '2026-01-01' }), false,
  "and new year's eve is out of a range that starts on new year's day, in every zone");
