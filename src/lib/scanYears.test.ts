...
for (const y of [2026, 2027, 2031, 2040]) {
  const ys = yearsAround(new Date(y, 0, 1));
...
{
  const ys = yearsAround(new Date(2027, 0, 1));
  ...
  ok(!ys.includes(2019) || ys[0] !== 2019 || ys.indexOf(2027) !== 0,
    'the first entry is never silently the answer');
}

/* ── it reaches back far enough, and one year forward ────────────────────── */

{
  const ys = yearsAround(new Date(2026, 5, 1));
  ...
  ok(ys.every((y: number, k: number) => k === 0 || y === ys[k - 1] + 1), 'strictly consecutive');
}

/* ── a stored date outside the window widens it, never refuses it ────────── */

{
  const old = yearsAround(new Date(2026, 5, 1), 2004);
  ...
  const ahead = yearsAround(new Date(2026, 5, 1), 2030);
...
{
  const ys = yearsAround(new Date(2026, 5, 1), 2011);
...
{
  const a = yearsAround(new Date(2026, 5, 1), null);
  const b = yearsAround(new Date(2026, 5, 1));
  eq(a, b, 'a null year to include is the same as none');
  const inRange = yearsAround(new Date(2026, 5, 1), 2020);
  eq(inRange, b, 'a year already inside the window widens nothing');
}

if (errors.length) { errors.forEach((e) => console.error(e)); console.error(`scanYears: ${errors.length} failure(s)`); process.exit(1); }
