...
import {
  LEVEL_NOTE, LEVEL_TITLE, ROSTER_SPLIT_DAYS, ROSTER_WINDOW_DAYS,
  judgeRosterRow, rankRosterExercise, rosterExerciseLine, type RosterExerciseRow,
} from './rosterExercise';
...
const row = (over: Partial<RosterExerciseRow>): RosterExerciseRow => ({
...
eq(judgeRosterRow(row({})).level, 'unseen',
  'a client with no outings in the window has not done it — which may be the most important row on this screen and is not stalling');

eq(judgeRosterRow(row({ recentOutings: 3, recentTopKg: 100 })).level, 'new',
  'logged only in the recent half has nothing to compare against, and "stalled" over one data point is a coin toss');

eq(judgeRosterRow(row({ recentOutings: 3, priorOutings: 3 })).level, 'no-load',
  'logged in both halves with no weight anywhere is press-ups, and a tonnage of nothing is not a plateau');
eq(judgeRosterRow(row({ recentOutings: 3, priorOutings: 3, priorTopKg: 100 })).level, 'no-load',
  'and so is a movement whose loads have stopped being recorded');

eq(judgeRosterRow(row({ recentOutings: 3, priorOutings: 3, recentTopKg: 100, priorTopKg: 100 })).level, 'holding',
  'the same top set in both halves is holding — the one this screen is for');
eq(judgeRosterRow(row({ recentOutings: 3, priorOutings: 3, recentTopKg: 102.5, priorTopKg: 100 })).level, 'climbing',
  'a heavier top set is climbing');
eq(judgeRosterRow(row({ recentOutings: 3, priorOutings: 3, recentTopKg: 95, priorTopKg: 100 })).level, 'dropping',
  'a lighter one is dropping, which a coach reads differently and often knows the reason for');
...
eq(judgeRosterRow(row({ recentOutings: 1, priorOutings: 1, recentTopKg: 101, priorTopKg: 100 })).level, 'climbing',
  'a single kilogram more is more — a tolerance band would swallow the microload prescribed to fix a stall');

/* ── the figures carried alongside ──────────────────────────────────────── */

const climbing = judgeRosterRow(row({ recentOutings: 2, priorOutings: 2, recentTopKg: 110, priorTopKg: 100, recentE1rmKg: 127.5 }));
...
eq(Math.round(climbing.changePct as number), 10, 'and the change as a number, for deltaLabel to put a sign and an arrow on');
eq(judgeRosterRow(row({ recentOutings: 1, recentTopKg: 100 })).changePct, null,
  'a change from nothing is not a percentage, it is a first measurement');
eq(judgeRosterRow(row({ recentOutings: 1, priorOutings: 1, recentTopKg: 100, priorTopKg: 0 })).changePct, null,
  'and a change from zero has nothing to be a percentage of');
...
const ranked = rankRosterExercise(['ann', 'bob', 'cat', 'dan'], [
...
eq(ranked.map((c) => c.clientId), ['ann', 'dan', 'bob', 'cat'],
  'and the ones a coach must act on lead: not doing it, going backwards, not moving, then going up');
...
const sameBand = rankRosterExercise(['x', 'y'], [
...
eq(exerciseSlug('Bench Press'), 'bench-press', 'the slug rule the migration transcribes');
eq(exerciseSlug('Bent-Over Row'), 'bent-over-row', 'punctuation collapses to one hyphen');
eq(exerciseSlug('Bent-over Row'), 'bent-over-row', 'and case does not matter');
eq(exerciseSlug('  Push-up  '), 'push-up', 'surrounding space leaves no leading or trailing hyphen');
eq(exerciseSlug('Squat   Jump'), 'squat-jump', 'nor does a run of spaces leave two');
eq(exerciseSlug('!!!'), '', 'a name with nothing alphanumeric in it has no slug, and matches nothing');

/* ── the line, and the two reads either of which makes it a lie ─────────── */

const line = rosterExerciseLine('ready', 'ready', ranked, 'bench press');
...
ok(new RegExp(String(ROSTER_WINDOW_DAYS)).test(line), 'with the window it was measured over');

// A coach told "3 of 40 are stalled" off a partial roster acts on a denominator
// that is not their book.
ok(/not about them/i.test(rosterExerciseLine('error', 'ready', ranked, 'bench press')),
  'a failed aggregate names the read rather than the clients');
ok(/could not be read in full/i.test(rosterExerciseLine('ready', 'partial', ranked, 'bench press')),
  'and a partial roster refuses to say how many of the book this covers');
ok(!/\d+ clients?/.test(rosterExerciseLine('ready', 'partial', ranked, 'bench press')),
  'with no count anywhere in that sentence');

eq(ROSTER_SPLIT_DAYS * 2, ROSTER_WINDOW_DAYS,
  'the window splits in half, so neither side is given an advantage by being longer');
...
eq(new Set(Object.values(LEVEL_TITLE)).size, 6, 'each band has its own heading');
eq(new Set(Object.values(LEVEL_NOTE)).size, 6, 'and its own sentence');
ok(Object.values(LEVEL_NOTE).every((n) => n === n.toLowerCase() || !/^[A-Z]/.test(n)),
  'the notes are prose and are sentence case, not button labels');
