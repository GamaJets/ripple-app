...
same(blockDates({ from: '2026-09-01', days: 1, repeatWeeks: 1 }), ['2026-09-01'],
  'one day is one day — the behaviour that already existed');
same(blockDates({ from: '2026-09-01', days: 3, repeatWeeks: 1 }),
  ['2026-09-01', '2026-09-02', '2026-09-03'], 'three consecutive days');
same(blockDates({ from: '2026-09-06', days: 1, repeatWeeks: 3 }),
  ['2026-09-06', '2026-09-13', '2026-09-20'], 'every Sunday for three weeks');
...
const overlapping = blockDates({ from: '2026-09-01', days: 10, repeatWeeks: 2 });
...
same(blockDates({ from: '2026-01-30', days: 4, repeatWeeks: 1 }),
  ['2026-01-30', '2026-01-31', '2026-02-01', '2026-02-02'], 'a range crosses a month end');
same(blockDates({ from: '2026-12-30', days: 4, repeatWeeks: 1 }),
  ['2026-12-30', '2026-12-31', '2027-01-01', '2027-01-02'], 'and a year end');
// 2028 is a leap year.
ok(blockDates({ from: '2028-02-27', days: 4, repeatWeeks: 1 }).includes('2028-02-29'),
...
for (const from of ['2026-03-23', '2026-10-19', '2026-11-02', '2026-04-01']) {
  const days = blockDates({ from, days: 14, repeatWeeks: 1 });
...
// appointment.
same(blockDates({ from: '2026-09-01', days: 0, repeatWeeks: 1 }), [], 'zero days blocks nothing');
same(blockDates({ from: '2026-09-01', days: -3, repeatWeeks: 1 }), [], 'and neither does a negative');
same(blockDates({ from: '2026-09-01', days: 1.5, repeatWeeks: 1 }), [], 'or half a day');
same(blockDates({ from: '2026-09-01', days: MAX_BLOCK_DAYS + 1, repeatWeeks: 1 }), [],
  'past the cap is refused outright, not silently trimmed to the cap');
eq(blockDates({ from: '2026-09-01', days: MAX_BLOCK_DAYS, repeatWeeks: 1 }).length, MAX_BLOCK_DAYS,
  'at the cap it works');
same(blockDates({ from: '2026-09-01', days: 1, repeatWeeks: 0 }), [], 'zero repeats is nothing');
same(blockDates({ from: '2026-09-01', days: 1, repeatWeeks: MAX_BLOCK_WEEKS + 1 }), [],
  'and so is more than a term');
same(blockDates({ from: 'not a date', days: 3, repeatWeeks: 1 }), [], 'an unparseable start covers nothing');
same(blockDates({ from: '', days: 3, repeatWeeks: 1 }), [], 'and so does an empty one');

/* ── the button says what it will do ────────────────────────────────────── */

eq(blockPlanLabel({ from: '2026-09-01', days: 1, repeatWeeks: 1 }), 'Block This Day', 'one day');
eq(blockPlanLabel({ from: '2026-09-01', days: 5, repeatWeeks: 1 }), 'Block 5 Days', 'five days');
eq(blockPlanLabel({ from: '2026-09-06', days: 1, repeatWeeks: 4 }), 'Block 4 Days', 'four Sundays are four days');
eq(blockPlanLabel({ from: '2026-09-01', days: 0, repeatWeeks: 1 }), null,
  'a plan covering nothing has no label, which is the caller’s cue to disable the button');

/* ── the summary, which is the point ────────────────────────────────────── */

const r = (day: string, outcome: BlockResult['outcome'], withdrawn = 0): BlockResult => ({ day, outcome, withdrawn });

const clean = summariseBlocks([r('2026-09-01', 'blocked', 2), r('2026-09-02', 'blocked', 1)]);
...
ok(blockSummaryLine(clean).startsWith('2 days blocked'), 'the sentence opens with what happened');
ok(blockSummaryLine(clean).includes('3 open slots were withdrawn'), 'and says what it cost');

const mixed = summariseBlocks([
...
const line = blockSummaryLine(mixed);
...
const allBooked = summariseBlocks([r('2026-09-01', 'booked'), r('2026-09-02', 'booked')]);
ok(blockSummaryLine(allBooked).startsWith('Nothing was blocked'),
  'a run where every day was refused never opens with a success');
...
const manyBooked = summariseBlocks(
  ['01', '02', '03', '04', '05', '06'].map((d) => r(`2026-09-${d}`, 'booked')),
);
const manyLine = blockSummaryLine(manyBooked);
...
for (const s of [clean, mixed, allBooked, manyBooked, summariseBlocks([])]) {
  const l = blockSummaryLine(s);
