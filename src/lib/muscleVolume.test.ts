...
import { muscleBoard, unmatchedNote, EMPTY_BOARD, type GroupedExercise } from './muscleVolume';
...
const CATALOGUE: GroupedExercise[] = [
 ...
 */
{
  ...
  const b = muscleBoard(log, CATALOGUE, { sinceMs: NOW - WEEK, nowMs: NOW, history: HISTORY, catalogueWhole: true });
  ...
  eq(b.groups[0].exercises.length, 2,
    'the movements are listed as they were written, because that is what the member typed');
 ...
 */
{
  ...
  const b = muscleBoard(log, CATALOGUE, { sinceMs: NOW - WEEK, nowMs: NOW, catalogueWhole: true });
  ...
  ok(b.unmatched.includes('Mystery Machine'),
    'and so is one that IS in the catalogue with no muscle group on it — an unfiled row is unfiled either way');
  const note = unmatchedNote(b);
  ...
  eq(unmatchedNote({ ...EMPTY_BOARD, groups: [] }), null, 'and there is no sentence when nothing is missing');
}
 ...
 */
{
  ...
  const whole = muscleBoard(log, CATALOGUE, { sinceMs: NOW - WEEK, nowMs: NOW, catalogueWhole: true });
  ...
  const partial = muscleBoard(log, CATALOGUE, { sinceMs: NOW - WEEK, nowMs: NOW, catalogueWhole: false });
  eq(partial.untrained, null,
    'a catalogue we have not seen the end of supports no claim about an untrained muscle');
  eq(partial.groups.length, 1, 'though everything we DID read is still on the board — the rows are real');

  const nothing = muscleBoard([], CATALOGUE, { sinceMs: NOW - WEEK, nowMs: NOW, catalogueWhole: true });
  eq(nothing.groups.length, 0, 'a week with nothing logged has no groups');
  ok(nothing.untrained != null && nothing.untrained.length === 4,
    'and every group in the catalogue is named as untrained rather than drawn as a row of noughts');
}
...
/* ── THE WINDOW ───────────────────────────────────────────────────────────── */
{
  ...
  ok(!b.unmatched.length,
    'an entry whose timestamp will not parse is left out entirely rather than filed under today');

  const wider = muscleBoard(log, CATALOGUE, { sinceMs: NOW - 60 * 86_400_000, nowMs: NOW, catalogueWhole: true });
 ...
 */
{
  ...
  eq(b.groups[0].volumeKg, null,
    'with no tonnage — null, never a well-formed 0 — because seconds times kilograms is not a mass moved');

  const unpriced = muscleBoard(
    [{ t: at('2026-08-30'), exercise: 'Pull-up', sets: [[10, 0], [8, 0]], bw: [true, true] }],
    CATALOGUE,
    { sinceMs: NOW - WEEK, nowMs: NOW, history: [], catalogueWhole: true },
  );
  ...
  const priced = muscleBoard(
    [{ t: at('2026-08-30'), exercise: 'Pull-up', sets: [[10, 0]], bw: [true] }],
    CATALOGUE,
    { sinceMs: NOW - WEEK, nowMs: NOW, history: HISTORY, catalogueWhole: true },
  );
