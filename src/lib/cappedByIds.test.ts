...
const idList = (n: number, prefix = 'id') => Array.from({ length: n }, (_, i) => `${prefix}-${i}`);
...
(async () => {
  {
    ...
    const res = await readCappedByIds<{ id: string }>(ids, (chunk) => {
      sent.push(chunk);
      return Promise.resolve({ data: chunk.map((id) => ({ id })), error: null });
    });
    eq(sent.length, Math.ceil(1000 / ID_CHUNK), 'a thousand ids go out in ceil(1000 / ID_CHUNK) requests, not one');
    ok(sent.every((c) => c.length <= ID_CHUNK), `no request carries more than ID_CHUNK (${ID_CHUNK}) ids — that is the whole 414 argument`);
...

  {
    ...
    ok(ID_CHUNK * 39 < 8 * 1024, `${ID_CHUNK} uuids is about ${ID_CHUNK * 39} bytes of query string, which must stay inside the 8KB request line proxies enforce`);
  }

  {
    const sent: string[][] = [];
    const res = await readCappedByIds<{ id: string }>([], (chunk) => {
      sent.push(chunk);
  ...
  {
    const sent: string[][] = [];
    await readCappedByIds<{ id: string }>(
      ['a', 'b', null, 'a', undefined, 'c', ''],
      (chunk) => { sent.push(chunk); return Promise.resolve({ data: [], error: null }); },
    );
  ...
  {
    const res = await readCappedByIds<{ id: string }>(
      idList(300),
    ...
    eq(res.rows.length, 10 * Math.ceil(300 / ID_CHUNK), 'and every chunk is trimmed to the cap — the probe row is a signal, never a row to draw');
...

  {
    ...
    const res = await readCappedByIds<{ id: string }>(
      idList(ID_CHUNK * 2),
      () => { call += 1; return Promise.resolve({ data: call === 1 ? idList(6).map((id) => ({ id })) : [], error: null }); },
      { cap: 5 },
    );
  ...
  {
    let calls = 0;
    const res = await readCappedByIds<{ id: string }>(idList(ID_CHUNK * 4), (chunk) => {
      calls += 1;
      if (calls === 2) return Promise.resolve({ data: null, error: { message: 'refused' } });
      return Promise.resolve({ data: chunk.map((id) => ({ id })), error: null });
    });
    ...
    eq(res.rows.length, ID_CHUNK, 'the rows that did come back are handed over anyway, for a caller that can use them');
...

  {
    ...
    const res = await readCappedByIds<{ id: string }>(['a'], () => Promise.resolve({ data: null, error: null }));
  ...
  {
    let seen = 0;
    await readCappedByIds<{ id: string }>(['a'], () => { seen += 1; return Promise.resolve({ data: [], error: null }); });
    eq(seen, 1, 'one chunk for one id');
    ok(ROW_CAP === 1000, 'the default cap is ROW_CAP, which is PostgREST\'s own ceiling — if that changes, the callers reasoning about a 1000-row page need to know');
  }
