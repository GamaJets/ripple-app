...
import {
  ...
  type ProfileRole, type StaffRole,
} from './staffRoles';
...
eq(STAFF_ROLES.length, 2, 'there are two grantable staff roles');
ok(STAFF_ROLES.includes('trainer') && STAFF_ROLES.includes('receptionist'),
  'and they are coach and reception');
ok(!(STAFF_ROLES as string[]).includes('owner'),
  'ownership is NOT grantable from a staff roster — a function that makes a second owner makes a compromised account permanent');
ok(!(STAFF_ROLES as string[]).includes('client'),
  'and neither is membership, which is not a grant of anything');
...
for (const r of ['owner', 'trainer', 'client', 'receptionist'] as ProfileRole[]) {
  ok(!!ROLE_LABEL[r], `${r} has a label`);
}
eq(ROLE_LABEL.trainer, 'Coach', 'a trainer is called a coach on screen, as everywhere else in this product');
eq(ROLE_LABEL.receptionist, 'Reception', 'and the new one is the desk');

// The distinction that makes the role worth having at all.
ok(/roster row/.test(STAFF_ROLE_NOTE.trainer) && /payroll/.test(STAFF_ROLE_NOTE.trainer),
  'the coach note says they get a roster row and a place in payroll');
ok(/no roster row|no book/.test(STAFF_ROLE_NOTE.receptionist),
  'and the reception note says they do not');
ok(/paid/.test(STAFF_ROLE_NOTE.receptionist),
  'and that they see nothing about what anybody is paid, which is the case D5 named');

/* ── the reach table ───────────────────────────────────────────────────── */

ok(STAFF_ROLE_REACH.length > 0, 'there is a reach table to show an owner');
for (const r of STAFF_ROLE_REACH) {
  ok(!!r.what && !!r.owner && !!r.trainer && !!r.receptionist,
    `"${r.what}" answers for every role — a blank cell would be read as "no" and might be "yes"`);
}
const byWhat = (s: string) => STAFF_ROLE_REACH.find((r) => r.what.toLowerCase().includes(s));
...
const grant = (over: Partial<Parameters<typeof grantBlocker>[0]> = {}) => grantBlocker({
...
ok(!!elsewhere && /another gym/.test(elsewhere),
  'somebody who belongs to another gym is refused rather than quietly moved out of it');
...
ok(!!unread && /failed query/.test(unread),
  'an account that could not be READ blocks the grant, and says it is a failed query rather than a missing person');
eq(grant({ subjectRole: null }), null,
  'while an account read successfully that simply has no role is grantable — the two must not collapse');

/* ── who may be removed, and what removing does not do ─────────────────── */

const revoke = (over: Partial<Parameters<typeof revokeBlocker>[0]> = {}) => revokeBlocker({
...
ok(!!me && /nobody who can administer/.test(me),
  'an owner is not removable — a gym with nobody who can administer it cannot be repaired from inside the product');
ok(!!revoke({ subjectRole: 'owner' }), 'in either spelling of the same case');

const member = revoke({ subjectRole: 'client' });
ok(!!member && /member of this gym rather than staff/.test(member),
  'a member has no staff access to take away');
...
ok(!!withBook && /would NOT take away/.test(withBook),
  'and the refusal says what the removal would fail to do rather than merely that it is not allowed');
ok(!!withBook && /follows the book rather than the gym/.test(withBook),
  'and why — the access reads clients.trainer_id and has no gym in it');
...
ok(!!unknownBook && /unknown, not empty/.test(unknownBook),
  'an unread book blocks the removal and says it is unknown rather than empty');

ok(/roster row is kept/.test(revokeConsequence('trainer')),
  'removing a coach keeps their roster row — one who left is not one who never existed');
ok(/Nothing they delivered is deleted/.test(revokeConsequence('trainer')),
  'and deletes nothing they delivered');
ok(/door log keeps their entries/.test(revokeConsequence('receptionist')),
  'and removing a receptionist keeps the door entries they made');
ok(/stop belonging to this gym/.test(revokeConsequence('receptionist')),
  'while both say the access closes immediately');
...
(async () => {
  {
    ...
    const out = await grantStaffRole(sb, THEM, 'trainer', 'Started Monday');
  ...
  }
  {
    ...
    const out = await grantStaffRole(sb, THEM, 'receptionist');
  ...
  }
  {
    ...
    await grantStaffRole(sb, THEM, 'receptionist');
  ...
  }
  {
    ...
    try { await grantStaffRole(sb, THEM, 'trainer'); } catch (e: any) { threw = e?.message ?? null; }
    ok(!!threw && /already belongs to another gym/.test(threw),
      'a refusal is thrown with the database’s own words, not replaced by a house string');
  }
  {
    const { calls, sb } = rpc({ was: 'receptionist', clients_on_book: 0 });
    const out = await revokeStaffRole(sb, THEM, 'Left in September');
  ...
  }
  {
    ...
    const out = await revokeStaffRole(sb, THEM);
  ...
  }
  {
    ...
    let threw: string | null = null;
    try { await revokeStaffRole(sb, THEM); } catch (e: any) { threw = e?.message ?? null; }
  ...
  const g = grantFromRow(row);
  ok(!!g && g.role === 'receptionist' && g.subjectId === THEM, 'a complete row reads back');
  ok(!!g && isLiveGrant(g), 'and one with no revocation is the grant in force');

  eq(grantFromRow(null), null, 'no row is no grant');
  ...
  eq(grantFromRow({ ...row, granted_at: null }), null, 'a grant with no date is not a record');
  eq(grantFromRow({ ...row, id: null }), null, 'nor one with no identity of its own');
  eq(grantFromRow({ ...row, role: 'owner' }), null,
    'and a row claiming an owner grant is refused here too — the function cannot write one, so a row holding one is not to be rendered as a grant');
  ...
  const erased = grantFromRow({ ...row, subject_id: null, actor_id: null });
  ...
  const dead = grantFromRow({ ...row, revoked_at: '2026-09-30T17:00:00Z', revoked_by: OWNER, revoked_by_name: 'Tim' });
  ok(!!dead && !isLiveGrant(dead), 'a revoked grant is not in force');
  ...
  ok(/join code/.test(grantNote(null, 'Tim')),
    'somebody with no recorded grant is said to have joined by join code, not left blank — a blank beside "who granted this" reads as nobody having granted it');
  ok(/Granted by Tim/.test(grantNote(g, 'Tim')), 'and a grant names who made it');
  ok(/Granted by Tim/.test(grantNote(g, 'Somebody Else')),
    'from the row itself rather than from whatever the caller could look up now — the stored name is the evidence');
  ok(/could not read/.test(grantNote(grantFromRow({ ...row, actor_name: null }), null)),
    'a name that could not be read says so rather than printing a dash as the subject of the sentence');
  ok(/has since been erased/.test(grantNote(grantFromRow({ ...row, actor_name: null, actor_id: null }), null)),
    'and an erased owner is a different sentence from an unreadable one');
  ok(/Access ended/.test(grantNote(dead, 'Tim')), 'and a revoked one says so first');
  ...
  ok(!/\d{1,2} \w+ 20\d\d/.test(grantNote(g, 'Tim')),
    'and no calendar date is formatted in here — the caller has the gym’s timezone and this does not');
