...
// this person at all.
import {
  paperworkFor, paperworkLine, paperworkItemLine, paperworkOutstanding, unsignedCount,
  type PaperworkDoc, type PaperworkItem,
} from './clientPaperwork';
...
const doc = (o: Partial<PaperworkDoc> & { id: string }): PaperworkDoc =>
  ({ title: o.id, required: true, retired: false, createdAt: '2026-01-01T00:00:00Z', ...o });
...

{
  ...
  eq(unsignedCount(items), 1, 'one outstanding');
  eq(paperworkLine('ready', items, 'Zoe'), 'Zoe has NOT accepted 1 of the 2 documents you require.',
    'and the sentence names the person and both numbers');
  eq(paperworkOutstanding('ready', items), true, 'which is worth flagging');
}
...

{
  ...
  eq(paperworkLine('ready', items, 'Zoe'), 'Zoe has accepted the one document you require.',
    'so this client is not permanently flagged for a waiver nobody uses any more');
  eq(paperworkOutstanding('ready', items), false, 'and nothing is flagged');
}
...

{
  ...
  ok(!paperworkLine('ready', sent, 'Zoe').includes('2 documents'),
    'and it is not counted against her');

  const his = paperworkFor({
    docs: [WAIVER, TERMS],
    recipients: [{ documentId: 'waiver', clientId: SAM }],
    acceptances: [],
    clientId: SAM,
...
const NONE: PaperworkItem[] = [];

{
  const loading = paperworkLine('loading', NONE, 'Zoe');
  ...
  const failed = paperworkLine('error', NONE, 'Zoe');
  ok(/could not be read/i.test(failed), 'a failed read says so');
  ok(/not a record of them having signed nothing/i.test(failed),
    'and refuses the reading a coach would otherwise take from an empty list');
  eq(paperworkOutstanding('error', NONE), false,
    'a failed read is not a warning either — a red flag over an unknown is the same lie as a green one');

  const part = paperworkLine('partial', NONE, 'Zoe');
  ...
  eq(paperworkOutstanding('partial', [{ id: 'a', title: 'a', acceptedAt: null }]), false,
    'nothing is flagged from a part of a set either');
}
...
ok(!paperworkLine('partial', [{ id: 'a', title: 'a', acceptedAt: null }], 'Zoe').includes('1 of the 1'),
  'a partial read never states a total');

/* ── a coach who requires nothing ───────────────────────────────────────── */

{
  const line = paperworkLine('ready', NONE, 'Zoe');
...
eq(paperworkItemLine({ id: 'a', title: 'a', acceptedAt: '2026-04-01T09:00:00Z' }, day), 'Accepted 2026-04-01',
  'a signed document carries the date');
eq(paperworkItemLine({ id: 'a', title: 'a', acceptedAt: null }, day), 'Not accepted',
  'and an unsigned one says so plainly');

/* ── nothing renders as a gap ───────────────────────────────────────────── */

for (const read of ['loading', 'ready', 'partial', 'error'] as const) {
  for (const items of [NONE, [{ id: 'a', title: 'a', acceptedAt: null }], [{ id: 'a', title: 'a', acceptedAt: '2026-04-01T09:00:00Z' }]]) {
    const line = paperworkLine(read, items, 'this client');
    ok(line.length > 0 && !line.includes('undefined') && !line.includes('null'),
      `${read} with ${items.length} item(s) is a real sentence`);
  }
}
