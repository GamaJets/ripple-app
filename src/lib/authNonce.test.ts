...
import { authNonce, NONCE_BYTES } from './authNonce';
...
{
  let called = 0;
  const n = authNonce((a) => { called++; a.forEach((_, i) => { a[i] = i + 1; }); });
  eq(called, 1, 'the platform generator is asked exactly once');
  ok(n.strong, 'and the answer is marked strong');
  eq(n.value.length, NONCE_BYTES * 2, 'the value is two hex characters per byte');
...
/* ── a generator that accepts the call and does nothing is not trusted ──── */
{
  ...
  const n = authNonce(() => { /* fills nothing */ });
...
{
  const n = authNonce(() => { throw new Error('no entropy source'); });
  ok(!n.strong, 'a throwing generator falls back, and says the answer is weak');
  eq(n.value.length, NONCE_BYTES * 2, 'and still returns a usable nonce');
...
/* ── the whole point: two nonces are not the same nonce ─────────────────── */
{
  ...
  for (let i = 0; i < 200; i++) seen.add(authNonce().value);
...
{
  const n = authNonce((a) => { a.fill(0); a[a.length - 1] = 1; });
  eq(n.value.length, NONCE_BYTES * 2, 'a leading zero byte is padded, not dropped');
