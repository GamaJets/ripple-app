...
import {
  ...
  lastClosedMonth, churnHeadline, isDay, MONTHS_SHOWN,
  type MembershipLike, type ChurnMonth,
} from './memberChurn';
...
const m = (
  ...
  endsOn: string | null,
  status: string,
): MembershipLike => ({ memberId, memberName: memberId, startedOn, endsOn, status });

/* ── isDay: what counts as a date ─────────────────────────────────────────── */

ok(isDay('2026-08-01'), 'a plain day is a day');
ok(isDay('2026-08-01T09:00:00Z'), 'a stamp that begins with a day is a day');
ok(!isDay(''), 'the empty string is not a day');
ok(!isDay(null), 'null is not a day');
ok(!isDay(undefined), 'undefined is not a day');
ok(!isDay('August 2026'), 'a month name is not a day');
...

{
  ...
  eq(undatedExitCount(spans), 0, 'a dated cancellation is not an undated exit');
  eq(onBooksCount(spans), 1, 'they are on the books');
}
...

{
  ...
  eq(spans[0].leftOn, null,
    'and it suppresses the leaving day entirely — the record does not know when they went, '
    + 'and the older row is not an answer to a question about the newer one');
  eq(undatedExitCount(spans), 1, 'counted, so a screen can say how many');
  eq(onBooksCount(spans), 0, 'a cancelled member with no end date is not on the books');
}
...

{
  ...
  const spans = memberSpans([{ memberId: 'eve', startedOn: '2026-01-01', endsOn: null, status: 'active' }]);
...
const twenty = (n: number, prefix = 'k'): MembershipLike[] =>
  Array.from({ length: n }, (_, i) => m(`${prefix}${i}`, '2025-01-15', null, 'active'));

function august(rows: MembershipLike[]): ChurnMonth {
  const spans = memberSpans(rows);
  const months = churnMonths(spans, undatedExitCount(spans), NOW);
  const aug = months.find((x) => x.key === '2026-08');
  if (!aug) throw new Error('August 2026 is not among the months drawn');
  return aug;
}

{
  ...
  eq(churnHeadline(aug).pct, 10, 'the headline rounds it to a whole percentage');
  eq(churnHeadline(aug).label, 'August 2026', 'and names the month it is about');
}
...

{
  ...
  const spans = memberSpans(rows);
  eq(undatedJoinCount(spans), 1, 'the member with no start date is counted and named');
  const aug = august(rows);
  ...
  eq(aug.churn, null,
    'so August has NO rate — not 15%, which counts them once, and not 10%, which quietly drops '
  ...
  eq(churnHeadline(aug).pct, null, 'the headline withholds the figure');
  eq(churnHeadline(aug).note, aug.churnNote, 'and gives the month’s own reason rather than a generic one');
}
...

{
  ...
  const spans = memberSpans(rows);
  eq(undatedExitCount(spans), 1, 'one membership ended without saying when');
  const months = churnMonths(spans, undatedExitCount(spans), NOW);
  ok(months.every((x) => x.churn == null),
    'no month carries a rate while a leaver could belong to any of them');
...

{
  ...
  const spans = memberSpans(rows);
  const months = churnMonths(spans, 0, NOW);
  ...
  const last = lastClosedMonth(months);
...
{
  const rows = twenty(MIN_COHORT_FOR_RATE - 1, 'small');
  rows[0] = m('small0', '2025-01-15', '2026-08-14', 'cancelled');
  const aug = august(rows);
  eq(aug.opening, MIN_COHORT_FOR_RATE - 1, 'the roster is one under the floor');
  eq(aug.left, 1, 'and the count is still shown — it is true and useful');
  eq(aug.churn, null, 'but no percentage is drawn over it');
  ok(/would move it/.test(aug.churnNote),
    `and the reason is stated in the gym’s own terms — got ${JSON.stringify(aug.churnNote)}`);
...
{
  const months = churnMonths([], 0, NOW);
  const aug = months.find((x) => x.key === '2026-08')!;
  ...
  eq(churnHeadline(null).pct, null, 'and with no month at all the headline is a dash');
  ok(/no finished month/.test(churnHeadline(null).note),
    'never 0%, which is the best figure on the scale offered to somebody who has no figure');
}
...
{
  const months = churnMonths([], 0, NOW);
  eq(months.length, MONTHS_SHOWN, 'thirteen months, so the same month last year is on screen');
  ...
  eq(churnMonths([], 0, NOW, 3).length, 3, 'and a caller with less room can ask for fewer');
}
