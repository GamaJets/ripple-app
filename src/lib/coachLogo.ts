 ...
 */
export function logoExtension(mime: string | null | undefined): 'png' | 'jpg' {
...
export function logoContentType(path: string | null | undefined): LogoMime | null {
 ...
 */
export function coachLogoPath(coachId: string, at: number, token: string, ext: 'png' | 'jpg'): string {
 ...
 * explain.
 */
export function isOwnLogoPath(coachId: string | null | undefined, path: string | null | undefined): boolean {
 ...
 */
export function logoRefusal(bytes: number): string | null {
...
export function logoDataUri(base64: string | null | undefined, mime: string | null | undefined): string | null {
  const b = String(base64 ?? '').trim();
  const m = String(mime ?? '').toLowerCase();
  if (!b) return null;
  if (m !== 'image/png' && m !== 'image/jpeg') return null;
 ...
 */
export const MAX_LOGO_DATA_URI = 4_000_000;
...
export function safeLogoDataUri(v: string | null | undefined): string | null {
  const s = String(v ?? '');
  if (!s || s.length > MAX_LOGO_DATA_URI) return null;
 ...
 * business name twice.
 */
export function logoImgHtml(dataUri: string | null | undefined): string {
...
export const LOGO_CSS =
  '.logo{max-height:52px;max-width:180px;object-fit:contain;display:block;margin-bottom:10px}';
 ...
 */
export const LOGO_SCOPE_NOTE =
  'Your logo goes on the invoices you issue, the client reports you prepare and the cards you share. It does not change what your clients see inside the app, where they see your name and your colour.';

/** What is said when the logo cannot be read. Never a broken image, and never
 *  an assertion that the coach has not set one. */
export const LOGO_UNREADABLE_NOTE =
  'Your logo could not be read just now, so what is set is not known. Anything you make in the meantime is prepared without it, exactly as it was before you added one.';
