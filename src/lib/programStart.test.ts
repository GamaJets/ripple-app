...
import {
  CLIENT_STARTS_NOW, blockPosition, blockPositionLine, daysBetween, isStartDate, laterStart,
} from './programStart';
...
eq(daysBetween('2026-09-01', '2026-09-08'), 7, 'a week is seven days');
eq(daysBetween('2026-09-08', '2026-09-01'), -7, 'and backwards is negative seven');
eq(daysBetween('2026-09-01', '2026-09-01'), 0, 'the same day is no days');
eq(daysBetween(null, '2026-09-01'), null, 'a missing end is not nought days');
eq(daysBetween('not a date', '2026-09-01'), null, 'and neither is an unreadable one');
...
eq(daysBetween('2026-03-25', '2026-04-01'), 7, 'a week across the spring clock change is still seven days');
eq(daysBetween('2026-10-21', '2026-10-28'), 7, 'and so is one across the autumn change');
eq(daysBetween('2026-03-01', '2026-05-01'), 61, 'a two-month span containing a change is still exact');

/* ── which week of the block, and the four times there is no answer ─────── */

const at = (start: string | null, today: string, weeks = 8) => blockPosition(start, today, weeks);

eq(at(null, '2026-09-01').phase, 'no-date',
  'an assignment with no start date is its own state — every assignment ever made is this');
eq(at(null, '2026-09-01').week, null, 'and has no week number, so nothing can print "Week 0"');
eq(at('the 3rd', '2026-09-01').phase, 'unreadable',
  'a stored date this build cannot parse is NOT the same as the coach not having said');
...
const before = blockPositionLine(at('2026-09-07', '2026-09-01'), '2026-09-07', 'Priya');
ok(/training week one of it already/i.test(before),
  'the "starts next week" line says plainly that the client is already on it');
ok(/does not wait/i.test(before), 'and that the Train tab does not wait for the date');

const during = blockPositionLine(at('2026-09-01', '2026-09-10'), '2026-09-01', 'Priya');
...
const none = blockPositionLine(at(null, '2026-09-01'), null, 'Priya');
ok(/began the moment it was sent/i.test(none),
  'no start date says what actually happened rather than implying something was scheduled');

const after = blockPositionLine(at('2026-06-01', '2026-09-01'), '2026-06-01', 'Priya');
ok(/Nothing here says whether Priya did it/i.test(after),
...
ok(isStartDate('2026-09-01'), 'a calendar date is a start date');
ok(!isStartDate(''), 'a blank is not one — that is the null case and the coach simply did not say');
ok(!isStartDate('01/09/2026'), 'nor is a spelling this app does not store, which would parse two ways in two countries');
ok(!isStartDate('2026-09-01T00:00:00Z'), 'nor an instant — a start date is a DATE, because no client timezone exists to place an instant in');
...
ok(!isStartDate('2026-02-30'), 'the 30th of February is refused rather than becoming the 2nd of March');
ok(isStartDate('2028-02-29'), 'and a real leap day is accepted');
ok(!isStartDate('2026-02-29'), 'while a leap day in a year that has none is not');
...
ok(isStartDate('2020-01-01'), 'a date in the past is allowed — a coach may be recording what already happened');
ok(isStartDate('2030-01-01'), 'and one well ahead is allowed — that is a season being planned');

ok(laterStart('2026-09-08', '2026-09-01'), 'one block starts after another');
ok(!laterStart('2026-09-01', '2026-09-08'), 'and not before it');
ok(!laterStart('2026-09-01', '2026-09-01'), 'the same day is not later than itself');
ok(!laterStart(null, '2026-09-01'), 'and an absent date is not later than anything');
