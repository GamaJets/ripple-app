...
import {
  WEEK_STARTS_ON, WEEK_START_NAME, WEEK_DAYS, WEEK_DAY_NAMES,
  dayIndexInWeek, jsDayForIndex, weekIndexOf, utcWeekIndexOf,
  startOfWeek, startOfWeekUTC, isoDay, weekStartIso,
} from './weekStart';
...
eq(WEEK_STARTS_ON, 0, 'the week opens on Sunday');
eq(WEEK_START_NAME, 'Sunday', 'and the copy that names it agrees with the constant');
eq(WEEK_DAYS.join(','), 'Sun,Mon,Tue,Wed,Thu,Fri,Sat', 'so the seven columns read Sunday first');
eq(WEEK_DAY_NAMES[0], 'Sunday', 'and so do the long names');

/* ── everything is derived, not restated ───────────────────────────────── */

eq(dayIndexInWeek(WEEK_STARTS_ON), 0, 'the start day is column 0, whatever it is');
eq(WEEK_DAYS.length, 7, 'seven columns');
eq(WEEK_DAY_NAMES.length, 7, 'seven names');
ok(WEEK_DAY_NAMES.every((n, i) => n.slice(0, 3) === WEEK_DAYS[i]),
  'the short labels and the long names are the same week in the same order');
...
// draw one twice.
for (let i = 0; i < 7; i++) {
  eq(dayIndexInWeek(jsDayForIndex(i)), i, `column ${i} survives the round trip`);
}
eq(new Set(Array.from({ length: 7 }, (_, i) => jsDayForIndex(i))).size, 7,
  'the seven columns name seven distinct weekdays');
...
eq(weekIndexOf(sun), 0, 'Sunday opens the week');
eq(weekIndexOf(wed), 3, 'Wednesday is the fourth column');
eq(weekIndexOf(sat), 6, 'Saturday closes it');

/* ── when the week opened ──────────────────────────────────────────────── */

eq(isoDay(startOfWeek(sun)), '2026-09-06', 'a Sunday is its own week start');
eq(isoDay(startOfWeek(wed)), '2026-09-06', 'midweek resolves back to that Sunday');
eq(isoDay(startOfWeek(sat)), '2026-09-06', 'and so does the Saturday that closes it');
eq(isoDay(startOfWeek(new Date(2026, 8, 13))), '2026-09-13', 'the next Sunday opens the next week');
eq(weekStartIso(sat), '2026-09-06', 'weekStartIso is the same answer as a string');
...
eq(isoDay(startOfWeek(new Date(2026, 8, 7))), '2026-09-06',
  'a Monday now belongs to the week that opened the day before it');
...
const started = startOfWeek(hh);
eq(started.getHours() + started.getMinutes() + started.getSeconds() + started.getMilliseconds(), 0,
  'the week opens at local midnight');
...
// seven distinct days. This is the property a grid depends on.
{
  ...
  for (let i = 0; i < 7; i++) {
    ...
    starts.add(weekStartIso(d));
    days.add(isoDay(d));
    eq(weekIndexOf(d), i, `day ${i} of the week is column ${i}`);
  }
...
eq(isoDay(startOfWeek(new Date(2026, 10, 1))), '2026-11-01', 'the 1st can open a week');
eq(isoDay(startOfWeek(new Date(2026, 9, 31))), '2026-10-25', 'and the day before it belongs to the previous month');
...
eq(isoDay(startOfWeek(new Date(2027, 0, 1))), '2026-12-27', 'a week can open in the previous year');
...
eq(utcWeekIndexOf(utcWed), 3, 'UTC Wednesday is the fourth column');
eq(startOfWeekUTC(utcWed).toISOString().slice(0, 10), '2026-09-06', 'and its week opened on the UTC Sunday');
eq(startOfWeekUTC(new Date(Date.UTC(2026, 8, 6, 12))).toISOString().slice(0, 10), '2026-09-06',
  'a UTC Sunday is its own week start');
...
eq(isoDay(new Date(2026, 8, 6)), '2026-09-06', 'a local midnight prints as its own day');
eq(isoDay(new Date(2026, 8, 6, 23, 59)), '2026-09-06', 'and so does the last minute of it');
