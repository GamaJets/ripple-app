...
/** The minimum a member has to look like to be sorted into a segment. Built by
 *  the screen out of what it already read, never re-queried. */
export interface SegmentMember {
  memberId: string;
  name: string | null;
  status: string | null;
  /** Days since their last door visit, or null when the log shows none. */
...
export interface Segment {
  id: SegmentId;
  label: string;
  /** What this set MEANS, in the owner's words. Rendered beside the count,
   *  because "unseen: 34" without the definition is a number people argue with. */
  note: string;
...
export const NOTICE_ROUTE = '/(client)/notifications';

/** What actually happened, in the two halves that can fail separately. */
export interface ReachResult {
  /** True when the notice board row was written. */
  posted: boolean;
  /** How many inbox rows the database says it created. Null when the fan-out
   *  was not attempted or its result could not be read — never 0, which would
   *  say it reached nobody. */
  delivered: number | null;
  /** Set when the inbox half failed while the notice itself posted. */
 ...
 */
export function reachBlocker(body: string, recipients: number): string | null {
  const b = (body ?? '').trim();
  if (!b) return 'Write the message first. An empty card on forty dashboards is worse than nothing at all.';
  if (b.length > MAX_BODY) {
    return `That is ${b.length} characters and the limit is ${MAX_BODY}. The database refuses the rest rather than trimming it.`;
  }
  if (recipients === 0) {
...
export function willTruncateInbox(body: string): boolean {
  return (body ?? '').trim().length > INBOX_BODY;
}
 ...
 */
export async function postToSegment(
  sb: Queryable,
  args: {
    tenantId: string;
    authorId: string;
    body: string;
    memberIds: string[];
    /** The gym's name, for the inbox line. Null when it could not be read, and
     *  the title then says "Your gym" rather than inventing one. */
    gymName: string | null;
  },
): Promise<ReachResult> {
  ...
  if (error || !data?.id) {
    throw new Error(
      `That notice was not posted, so nobody has seen it: ${(error as any)?.message ?? 'the write was refused'}. Your words are still here.`,
    );
  }

  const ids = [...new Set(args.memberIds.filter(Boolean))];
  if (!ids.length) return { posted: true, delivered: 0, deliveryError: null };
...
export function deliveryNote(r: ReachResult, intended: number): string {
  if (!r.posted) return 'Nothing was posted.';
  if (r.deliveryError) {
