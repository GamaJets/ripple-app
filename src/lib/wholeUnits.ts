 ...
 */
export function wholeFromMinor(
  minorUnits: number | null | undefined,
  currency: string | null | undefined,
): number | null {
  const dp = currencyDecimals(currency);
  if (dp == null || minorUnits == null || !Number.isFinite(minorUnits)) return null;
  if (dp === 0) return Math.trunc(minorUnits);
 ...
 */
export const NO_CURRENCY_CHECK_NOTE =
  'This gym has not set its currency, so the amounts in the register cannot be '
