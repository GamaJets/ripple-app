...
import {
  assessCadence, cadenceLine, worthRaising, byLateness, overdueNote,
  MIN_ACTIVE_DAYS, MIN_SPAN_DAYS, MAX_USUAL_GAP_DAYS, MIN_LATE_DAYS, LATE_FRACTION,
  type Cadence,
} from './cadence';
import type { ActivityEvent } from './clientDrift';
...
const ago = (n: number, kind: ActivityEvent['kind'] = 'workout'): ActivityEvent =>
  ({ at: new Date(midnight - n * DAY + 12 * 3600_000).toISOString(), kind });

/** Events every `gap` days going back `count` visits, the newest `lastAgo` days
 *  ago. The ordinary shape of a client's record. */
const every = (gap: number, count: number, lastAgo: number): ActivityEvent[] => {
...
ok(short.spanDays! < MIN_SPAN_DAYS, 'which is the constant it is measured against');
...
const spread = assessCadence(every(30, 5, 2), NOW, 200);
...
ok(30 > MAX_USUAL_GAP_DAYS, 'a thirty-day gap is past the cap');
...
const steady = assessCadence(every(3, 8, 1), NOW);
...
ok(worthRaising(late), 'this one reaches the coach');
...
// not reach a coach.
const due = assessCadence(every(7, 6, 8), NOW);
eq(due.usualGapDays, 7, 'a weekly client');
eq(due.state, 'due', 'one day past their gap is due, not overdue');
ok(!worthRaising(due), 'and being due is never surfaced — that is the nagging this avoids');
...
const daily = assessCadence(every(1, 20, 2), NOW);
eq(daily.usualGapDays, 1, 'a daily trainer');
eq(daily.state, 'due', 'two days of silence from a daily trainer is not yet a signal');
ok(MIN_LATE_DAYS === 3, 'because the floor is three days');
const dailyGone = assessCadence(every(1, 20, 5), NOW);
...
const fortnightly = assessCadence(every(14, 5, 17), NOW, 120);
eq(fortnightly.usualGapDays, 14, 'a fortnightly client');
eq(fortnightly.state, 'due', 'three days past a fortnight is nothing');
ok(LATE_FRACTION === 0.5, 'because the tolerance is half their gap again');
const fortnightlyGone = assessCadence(every(14, 5, 23), NOW, 120);
...
const busyEvening: ActivityEvent[] = [
  ...every(4, 6, 2),
  ago(2, 'check_in'), ago(2, 'session'), ago(2, 'visit'),
];
const busy = assessCadence(busyEvening, NOW);
...
for (const c of [empty, thin, short, spread, steady, late, due, daily, fortnightly]) {
  ...
  ok(!/\bmotivat|\bcommit|\blaz(y|iness)\b|\bgiven up\b|\bquit/i.test(line),
    `"${line}" states no verdict on the person`);
  ok(!/\binjur|\bholiday|\bsick\b|\bpayment/i.test(line),
    `"${line}" names no cause the record cannot see`);
}
ok(cadenceLine(late).includes('past their own usual gap'),
  'the overdue sentence measures against their own gap and not against a target');
ok(cadenceLine(steady).includes('not late'), 'and a client inside their gap is told to be left alone');

/* ── ordering, which is relative to each client’s own gap ───────────────── */

const row = (cadence: Cadence, id: string) => ({ id, cadence });
...
const bigGapSmallMiss = row(assessCadence(every(14, 5, 22), NOW, 120), 'fortnightly');
const smallGapBigMiss = row(assessCadence(every(3, 8, 9), NOW), 'frequent');
...
eq(byLateness([bigGapSmallMiss, smallGapBigMiss])[0].id, 'frequent',
  'but six days off a three-day rhythm outranks eight off a fortnightly one');

/* ── the note above the list ────────────────────────────────────────────── */

eq(overdueNote(null), 'Working out who was due…', 'null in, null out — no count before the read lands');
ok(overdueNote([row(steady, 'a'), row(due, 'b')]).includes('Nobody is past'),
  'a settled list with nobody late says so plainly');
ok(overdueNote([row(empty, 'a'), row(thin, 'b')]).includes('settled enough'),
  'and a list nobody could be paced from says THAT instead, not "nobody is late"');
ok(overdueNote([row(late, 'a')]).includes('1 client is'), 'one client, singular');
ok(overdueNote([row(late, 'a'), row(dailyGone, 'b')]).includes('2 clients are'), 'two, plural');
ok(overdueNote([row(late, 'a')]).includes('earlier than the quiet list'),
  'and it says what this signal is relative to the one next door');
