...
const names = <P extends { name: string }>(o: { plans: P[] }) => o.plans.map((p) => p.name);
...
const partial = planOffer(P, { Starter: 'price_123' });
...
const twoOfThree = planOffer(P, { Starter: 'price_1', Studio: 'price_3' });
...
eq(names(planOffer(P, { Studio: 'price_3', Starter: 'price_1' })), ['Starter', 'Studio'],
  'the surviving plans keep the order PLANS gave them, not the price table order');
...
eq(planOffer([], {}).buyable, false, 'no plans at all is not buyable either');
eq(planOffer([], {}).plans, [], 'and there is nothing to list');
...
const blank = planOffer(P, { Starter: '', Pro: undefined, Studio: 'price_3' });
...
const orphan = planOffer(P, { Legacy: 'price_old' });
...
eq(names(planOffer(PLANS, { Starter: 'p1', Pro: 'p2', Studio: 'p3' })), ['Starter', 'Pro', 'Studio'],
  'the three real plan names are exactly the three the price table is keyed on');
ok(PLANS.every((p) => p.name.trim().length > 0),
  'every real plan has a name to key a price id on');
