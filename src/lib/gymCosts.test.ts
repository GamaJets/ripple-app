...
const cost = (o: Partial<GymCost> = {}): GymCost => ({
...
const draft = (o: Partial<GymCostDraft> = {}): GymCostDraft => ({
...
{
  eq(gymCostBlockers(draft()).length, 0, 'a complete draft has nothing wrong with it');
  eq(gymCostBlockers(draft({ supplier: null })).length, 0, 'and a supplier is optional — a cash purchase is still a cost');

  const empty = gymCostBlockers({ description: '', amountText: '', currency: null, category: 'rent', paidOn: '' });
  ...
  const noCur = gymCostBlockers(draft({ currency: null }));
  ...
  eq(gymCostBlockers(draft({ amountText: '0' })).length, 1, 'a cost of nothing is refused');
  ok(gymCostBlockers(draft({ amountText: '0' }))[0].includes('was free'),
    'and says why a nought is a claim rather than a blank');
  eq(gymCostBlockers(draft({ amountText: '-40' })).length, 1, 'and so is a negative one');
  eq(gymCostBlockers(draft({ category: 'wages' as never })).length, 1, 'and a category nobody offers');

  // The column's own ceiling, stated beside the field rather than arriving as a
  // 23514 after the form has closed.
  eq(gymCostBlockers(draft({ amountText: '999999999.99' })).length, 0, 'a very large but recordable amount goes through');
  eq(gymCostBlockers(draft({ amountText: '9999999999.99' })).length, 1, 'and one past the column ceiling is refused here rather than by a 23514');
  eq(GYM_COST_MAX_MINOR, 100_000_000_000, 'and the ceiling is the one part 700 checks');
}
...
{
  eq(gymCostBlockers(draft({ amountText: '4500.50' })).length, 0, 'a two-place amount goes through');
  eq(gymCostBlockers(draft({ amountText: '4500,50' })).length, 0, 'and so does a comma decimal — half the world types it');
  eq(gymCostBlockers(draft({ currency: 'JPY', amountText: '450000' })).length, 0, 'a yen amount is a whole number');
  eq(gymCostBlockers(draft({ currency: 'JPY', amountText: '4500.50' })).length, 1, 'and a decimal in one is refused rather than rounded');
...
{
  const t = gymCostsTaken([cost(), cost({ id: 'c2', amountCents: 120000, category: 'utilities' })]);
  ...
  const mixed = gymCostsTaken([cost(), cost({ id: 'c2', currency: 'EUR', amountCents: 90000 })]);
  ...
  const holed = gymCostsTaken([cost(), cost({ id: 'c2', currency: null })]);
  ...
  eq(gymCostsTaken([cost({ amountCents: null })]).unpriced, 1, 'and one with no amount at all is counted too');
}

/* ── 4. where it went, per category AND per currency ──────────────────────*/

{
  const by = gymCostsByCategory([
  ...
  // "unknown".
  eq(gymCostCategoryLabel('something_added_later'), 'something_added_later', 'an unrecognised category prints as itself');
  eq(gymCostCategoryLabel(null), 'Not stated', 'and a missing one says so rather than being blank');
  eq(gymCostCategoryLabel('utilities'), 'Utilities', 'and a known one gets its label');
}
...
{
  ok(gymCostsEmptyLine('error').includes('could not be read'), 'a failed read says so');
  ok(gymCostsEmptyLine('error').includes('not a statement that none have been recorded'),
    'and says outright that it is not a claim about the record');
  ok(gymCostsEmptyLine('partial').includes('nothing here is a total'), 'a truncated read states no total');
  eq(gymCostsEmptyLine('loading'), 'Still reading.', 'and a read in flight says only that');
  ok(gymCostsEmptyLine('ready').includes('Nothing has been recorded'), 'a whole read with nothing in it is a real answer');
  ok(gymCostsEmptyLine('ready').includes('Rent'), 'and names what belongs here');
}
...
{
  const mod: Record<string, unknown> = require('./gymCosts');
  ...
  ok(GYM_COSTS_ARE_NEVER_NETTED.includes('no profit figure anywhere in this app'),
    'and the screen says so out loud, because an absent figure reads as an omission');
  ok(GYM_COSTS_ARE_NEVER_NETTED.includes('different currency'),
    'naming one of the reasons it could not be computed');
  ok(GYM_COSTS_ARE_NEVER_NETTED.includes('card processor'),
    'and another, which is the one nothing in this database holds at all');
}
...
{
  ok(GYM_COSTS_NOT_TWICE.includes('Payroll'), 'the double-count warning names payroll');
  ok(GYM_COSTS_NOT_TWICE.includes('refund'), 'and refunds, which are recorded against the original payment');
  ok(GYM_COSTS_NOT_TWICE.includes('reception'),
    'and says which wages DO belong here, because "no wages" would lose a gym its largest line');
  ...
  const ids: string[] = GYM_COST_CATEGORIES.map((c) => c.id);
  ok(!ids.includes('wages') && !ids.includes('payroll'),
  ...
  ok(GYM_COST_CATEGORIES.find((c) => c.id === 'staff')!.note.includes('NOT trainer session pay'),
    'whose note says which people it does not mean');
  ...
  // recorded at all.
  ok(GYM_COST_CATEGORIES.find((c) => c.id === 'software')!.note.includes('Repple'),
    'and the software category says the gym’s own Repple bill belongs in it');
}

/* ── 8. not a tax record ──────────────────────────────────────────────────*/

{
  ok(GYM_COSTS_ARE_NOT_TAX_ADVICE.includes('reclaimable'), 'no cost is marked reclaimable');
  ok(GYM_COSTS_ARE_NOT_TAX_ADVICE.includes('supplier’s own invoice'),
    'and the sentence names the evidence this product does not hold');
  ok(!GYM_COST_CATEGORIES.some((c) => /deduct|allowab|\btax\b|vat/i.test(c.id + c.label + c.note)),
    'and no category implies a tax treatment');

  ok(GYM_COSTS_ARE_YOUR_WORD.includes('has been checked against'), 'a recorded cost is the gym’s own word and says so');
  ok(GYM_COSTS_ARE_YOUR_WORD.includes('no receipt or invoice is stored'),
    'and says that nothing is held behind it');
}
