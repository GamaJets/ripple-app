...
import {
  ...
  moveAtRefusalLine, moveAtConfirmBody, MOVE_AT_NOT_MOVED, MOVE_STEP_MIN,
  MOVE_DAY_START_MIN, MOVE_DAY_END_MIN,
  type MoveAtRefusal, type MoveAtReport, type MoveBlocker, type MoveOpenSlot,
} from './moveTimes';
...
const merged = workWindows(quarters, 2);
...
eq(workWindows(quarters, 3).length, 0, 'another weekday has none of them');

const split = workWindows([
  { dow: 1, hour: 7, minute: 0, dur: 60 },
  { dow: 1, hour: 12, minute: 0, dur: 60 },
], 1);
eq(split.length, 2, 'a real break in the day stays two windows');

const overlapping = workWindows([
...
eq(workWindows([{ dow: 1, hour: 7, minute: 0, dur: 0 }], 1).length, 0,
  'a zero-length row is not a window');
...
const iso = (h: number, min = 0) => at(h, min).toISOString();
const NOW = at(1).getTime();
const WORK = workWindows([{ dow: at(7).getDay(), hour: 7, minute: 0, dur: 12 * 60 }], at(7).getDay());
eq(WORK.length, 1, 'the coach works a single stretch that day');

const SEVEN: MoveBlocker = { id: 'sess-7', startsAt: iso(7), durationMin: 60, kind: 'booked' };
...
const plain = moveTimes({ ...base, blockers: [SEVEN], open: [] });
...
ok(plain.some((t) => new Date(t.startMs).getHours() === 7 && new Date(t.startMs).getMinutes() === 30),
  'the hour being moved is not an obstacle to itself');
ok(!plain.some((t) => t.startMs === Date.parse(iso(7))),
  'but the hour it is already in is not offered — the server calls that same-time');
...
for (const t of plain) {
  ...
  ok(d.getMinutes() % MOVE_STEP_MIN === 0, 'and on a step boundary of the local clock');
}
...
// runner's timezone.
for (const [yy, mm, dd] of [[2027, 2, 14], [2027, 9, 3]] as const) {
  const dawn = new Date(yy, mm, dd, 0, 30, 0, 0).getTime();
  const dow = new Date(yy, mm, dd, 12, 0, 0, 0).getDay();
  const shifted = moveTimes({
    year: yy, monthIndex: mm, day: dd, durationMin: 60, movingId: 'none',
    work: workWindows([{ dow, hour: 6, minute: 0, dur: 14 * 60 }], dow),
    nowMs: dawn, blockers: [], open: [],
  });
  ok(shifted.length > 0, `${yy}-${mm + 1}-${dd} still offers times`);
  eq(new Set(shifted.map((t) => t.startMs)).size, shifted.length,
    'and an hour that the clock repeats or skips is never offered twice');
...
const kinds: MoveBlocker['kind'][] = ['booked', 'blocked', 'class'];
for (const kind of kinds) {
  ...
  ok(!times.some((t) => t.startMs === Date.parse(iso(7, 30))),
    `and a ${kind} at 8 removes 7:30, because an hour from 7:30 runs into it`);
...
const exact: MoveOpenSlot = { id: 'open-9', startsAt: iso(9), durationMin: 60 };
const withOpen = moveTimes({ ...base, blockers: [SEVEN], open: [exact] });
eq(withOpen.find((t) => t.startMs === Date.parse(iso(9)))?.slotId, 'open-9',
  'an exact open slot is offered as itself, so the proven one-write path is used');

const halfHour: MoveOpenSlot = { id: 'open-half', startsAt: iso(9), durationMin: 30 };
const withHalf = moveTimes({ ...base, blockers: [SEVEN], open: [halfHour] });
eq(withHalf.find((t) => t.startMs === Date.parse(iso(9)))?.slotId, null,
  'a 30-minute slot is not a place to put a 60-minute session');
ok(withHalf.some((t) => t.startMs === Date.parse(iso(9))),
  'though 9am is still offered — an open slot of the wrong length blocks nothing');

/* ── the past is not offered ────────────────────────────────────────────── */

const midday = moveTimes({ ...base, nowMs: at(12).getTime(), blockers: [SEVEN], open: [] });
...
const unstated = moveTimes({ ...base, work: [], blockers: [], open: [] });
ok(unstated.length > 0, 'a coach with no stated hours is still offered times');
ok(unstated.every((t) => t.inHours === false),
  'and none of them claims to be inside hours the coach never set');
const firstUnstated = new Date(unstated[0].startMs);
eq(firstUnstated.getHours() * 60 + firstUnstated.getMinutes(), MOVE_DAY_START_MIN,
  'the fallback day starts where MOVE_DAY_START_MIN says');
const lastUnstated = new Date(unstated[unstated.length - 1].startMs);
ok(lastUnstated.getHours() * 60 + lastUnstated.getMinutes() + 60 <= MOVE_DAY_END_MIN,
  'and no session runs past where it ends');

/* ── the two groups ─────────────────────────────────────────────────────── */

const early = moveTimes({ ...base, blockers: [SEVEN], open: [] });
const grouped = groupMoveTimes(early);
...
ok(grouped.outside.some((t) => new Date(t.startMs).getHours() < 7),
  'an early hour outside the stated day is still reachable, in the second group');

/* ── what the list cannot promise ───────────────────────────────────────── */

eq(moveTimesCaveat('ready', 'ready'), null, 'two whole reads need no caveat');
const partialCal = moveTimesCaveat('partial', 'ready') ?? '';
...
const noClasses = moveTimesCaveat('ready', 'error') ?? '';
...
ok(/class timetable/i.test(moveTimesCaveat('ready', 'partial') ?? ''), 'and a truncated one too');
ok((moveTimesCaveat('partial', 'error') ?? '').length > (partialCal.length),
  'and two problems say more than one');

/* ── the empty list, which means five things ────────────────────────────── */

const seen = new Set<string>();
for (const status of ['loading', 'error', 'partial'] as const) {
  const line = emptyMoveTimesLine(status, 'Tue 6 Apr', true);
  ok(line.length > 0, `'${status}' says something`);
  ok(!/nowhere to move|already has something of yours/i.test(line),
    `and '${status}' never claims the day is full`);
  seen.add(line);
}
const full = emptyMoveTimesLine('ready', 'Tue 6 Apr', true);
...
const fullNoHours = emptyMoveTimesLine('ready', 'Tue 6 Apr', false);
ok(/Weekly Availability/i.test(fullNoHours), 'while a coach who has not set hours is told where to');
seen.add(full); seen.add(fullNoHours);
eq(seen.size, 5, 'five states, five sentences');

/* ── the refusals ───────────────────────────────────────────────────────── */

const rep = (over: Partial<MoveAtReport> = {}): MoveAtReport => ({ ...MOVE_AT_NOT_MOVED, ...over });

const reasons: MoveAtRefusal[] = [
  'not-yours', 'already-started', 'past', 'clash-booked', 'clash-blocked',
  'clash-class', 'clash', 'same-time', 'bad-time', 'unreachable',
];
const lines = new Set<string>();
for (const reason of reasons) {
  const line = moveAtRefusalLine(rep({ reason }), 'Ana', '7am', '8am');
  ok(line.length > 0, `'${reason}' has a sentence`);
  lines.add(line);
  if (reason !== 'unreachable') {
    ok(/has not moved and is still booked/.test(line),
      `'${reason}' says where the session actually is`);
  }
}
eq(lines.size, reasons.length, 'and no two refusals read the same');

const lost = moveAtRefusalLine(rep({ reason: 'unreachable' }), 'Ana', '7am', '8am');
ok(!/has not moved/.test(lost), 'the unreachable one does not claim the move failed');
ok(/may or may not/.test(lost) && /Do not tell Ana/.test(lost),
  'it says nobody knows, and to tell the client nothing yet');

const taught = moveAtRefusalLine(rep({ reason: 'clash-class', className: 'Bootcamp' }), 'Ana', '7am', '8am');
ok(/Bootcamp/.test(taught), 'a named class is named');
ok(!/Bootcamp/.test(moveAtRefusalLine(rep({ reason: 'clash-class' }), 'Ana', '7am', '8am')),
  'and an unnamed one is not invented');

const bare = moveAtRefusalLine(rep({ reason: 'clash-booked' }), null, null, null);
ok(bare.length > 0 && !/null|undefined/.test(bare),
  'a refusal with no name and no times is still a sentence');

/* ── the confirm ────────────────────────────────────────────────────────── */

const confirm = moveAtConfirmBody('Ana', 'Tue 6 Apr at 7am', 'Tue 6 Apr at 8am', true);
...
ok(/Nothing is charged/.test(confirm) && /no session comes off their pack/.test(confirm),
  'the two money facts are stated rather than implied');
ok(!/outside the working hours/.test(confirm), 'an in-hours move says nothing about working hours');
ok(/outside the working hours/.test(moveAtConfirmBody('Ana', '7am', '5am', false)),
  'and an out-of-hours one does');
