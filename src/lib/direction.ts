 ...
 */
export function forwardArrow(rtl: boolean): string {
  return rtl ? '←' : '→';
}

/** Back, as an arrow. */
export function backArrow(rtl: boolean): string {
  return rtl ? '→' : '←';
}
 ...
 * apparent movement.
 */
export function mirrorTurn(rtl: boolean, degrees: number): string {
 ...
 */
export function endAlign(rtl: boolean): 'left' | 'right' {
 ...
 */
export const START_ALIGN = 'auto' as const;

/* ── the mirror table ─────────────────────────────────────────────────────── */

/**
 * The kinds of thing this app draws whose mirroring was actually decided,
 * rather than left to fall out of a layout.
 *
 * This is a table and not a set of `if`s so that the decisions are in one
 * readable place, can be asserted, and can be argued with by editing one line.
 * The reasons are in the header.
 */
export type Kind =
  | 'nav-chevron'
  | 'back-arrow'
...
const MIRRORS: Record<Kind, boolean> = {
...
export function mirrors(kind: Kind): boolean {
  return MIRRORS[kind];
}
 ...
 */
export const UNMIRRORED: readonly Kind[] = [
  'chart-time-axis',
  'svg-drawing',
  'media-transport',
  'clock-face',
