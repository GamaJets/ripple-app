...
import {
  payRunStops, payRunHeading, payRunBody, payRunYesLabel, PAY_RUN_NO_LABEL, type PayRun,
} from './payRunConfirm';
...
{
  const r = run();
  ok(payRunHeading(r).includes('£1,240.00'), 'the heading states the amount');
 ...
 * decimal rule to get wrong. */
{
  ...
  ok(payRunYesLabel(r).includes('¥184,000'), 'a zero-decimal currency passes straight through');
  ok(!payRunYesLabel(r).includes('.'), 'and acquires no decimal point on the way');
}
 ...
 */
{
  eq(payRunStops(run()), null, 'an ordinary run is offered');
  const noAmount = payRunStops(run({ amountText: null }));
  ok(noAmount !== null, 'a run whose total cannot be stated is refused outright');
  ok(!!noAmount && !noAmount.includes('0'),
    'and the refusal never states a figure — a run that cannot be totalled is not a run worth nothing');
  ok(!!noAmount && noAmount.includes('Owed'), 'it points at the column that says which part is unknown');

  const empty = payRunStops(run({ sessions: 0, classes: 0, adjustments: 0 }));
  ok(empty !== null, 'a run with nothing outstanding is refused rather than silently doing nothing');
  eq(payRunStops(run({ sessions: 0, classes: 0, adjustments: 1 })), null,
    'one adjustment on its own is a real run — a coach reimbursed for a course fee delivered no sessions');
  eq(payRunStops(run({ sessions: 0, classes: 2, adjustments: 0 })), null,
    'and so is a coach who only taught classes');
}
 ...
 */
{
  ok(payRunBody(run()).includes('31 sessions, 4 classes and 1 adjustment'),
    'all three kinds, in a sentence a person reads');
  ok(payRunBody(run({ classes: 0, adjustments: 0 })).includes('31 sessions as settled'),
    'a plain session run says only sessions');
  ok(!payRunBody(run({ classes: 0, adjustments: 0 })).includes('0 class'),
    'and never lists a kind that is not in the run');
  ok(payRunBody(run({ sessions: 1, classes: 1, adjustments: 1 }))
      .includes('1 session, 1 class and 1 adjustment'),
    'singulars, all three of them — "1 classes" is what a machine writes');
  ok(payRunBody(run({ sessions: 0, classes: 0, adjustments: 2 })).includes('2 adjustments as settled'),
    'adjustments alone');
  ok(payRunBody(run()).includes('will not appear in another one'),
    'and it says what being stamped means, which is that they are gone from every later run');
  ok(payRunBody(run()).includes('August 2026'), 'the period is named — a run is settled against one');
}
 ...
 */
{
  ...
  ok(payRunBody(run({ methodLabel: 'cash' })).includes('cash'),
    'the method is the one the owner chose, never a default');
}
 ...
 */
{
  const h = payRunHeading(run({ who: null }));
  ok(h.includes('this trainer'), 'an unread name becomes a phrase, not a hole');
  ok(!h.includes('null') && !h.includes('undefined') && !h.includes('—'),
    'and never leaks the absence as a value');
  ok(h.includes('£1,240.00'), 'the amount is still stated — that half is known');
  ok(payRunHeading(run({ who: null, amountText: null })).includes('this run'),
    'both unknown still reads as a sentence, though payRunStops refuses this case before it is shown');
}
