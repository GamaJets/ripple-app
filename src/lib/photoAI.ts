...
/** 'unknown' is a state, not a default: between first render and the stored
 *  answer landing, nobody in the process knows. Nothing is sent on it. */
export type PhotoConsent = 'yes' | 'no' | 'unasked' | 'unknown';

/**
 * Which photograph is being asked about.
 *
 * Not a label — it selects the storage key, so adding a member here without
 * adding it to `PHOTO_AI_KEYS` and the sentence maps below does not compile.
 * That is on purpose: a new kind of photograph reaching the model under an
 * existing subject's yes is the one mistake this type exists to prevent.
 */
export type PhotoSubject = 'machine' | 'meal';
 ...
 */
export const PHOTO_AI_KEYS: Record<PhotoSubject, string> = {
...
export const PHOTO_AI_KEY = PHOTO_AI_KEYS.machine;

/** Where one subject's answer lives. A function so callers cannot index the
 *  record with a string that is not a subject. */
export function photoAIKey(subject: PhotoSubject): string {
  return PHOTO_AI_KEYS[subject];
}
...
/** Why a photograph is not being sent. Each has its own sentence on screen —
 *  "the reader is off" and "you have not been asked yet" are different facts
 *  and a member acts on them differently. */
export type PhotoBlock = 'off' | 'unknown' | 'unasked' | 'refused';

export interface PhotoAsk {
  /** True only when the reader is on AND the member has said yes. */
  allowed: boolean;
  /** Null when allowed. */
  block: PhotoBlock | null;
}
 ...
 */
export function mayAnalyzePhoto(consent: PhotoConsent, available: boolean): PhotoAsk {
  if (!available) return { allowed: false, block: 'off' };
  if (consent === 'unknown') return { allowed: false, block: 'unknown' };
  if (consent === 'unasked') return { allowed: false, block: 'unasked' };
  if (consent === 'no') return { allowed: false, block: 'refused' };
...
export const PHOTO_NOT_SENT: string[] = [
...
export const PHOTO_DESTINATION =
  'The photo is sent to Anthropic, who run the model that reads it, and is used to answer this one question.';
...
export const MEAL_PHOTO_DESTINATION =
  'The photo is sent to Anthropic, who run the model that reads it, and is used to read this one meal. Your gym does not get it and your coach does not get it.';
...
export const PHOTO_SENT_BY_SUBJECT: Record<PhotoSubject, string[]> = {
  machine: PHOTO_SENT,
  meal: MEAL_PHOTO_SENT,
};

export const PHOTO_NOT_SENT_BY_SUBJECT: Record<PhotoSubject, string[]> = {
  machine: PHOTO_NOT_SENT,
  meal: MEAL_PHOTO_NOT_SENT,
};

export const PHOTO_DESTINATION_BY_SUBJECT: Record<PhotoSubject, string> = {
  machine: PHOTO_DESTINATION,
  meal: MEAL_PHOTO_DESTINATION,
};

export const PHOTO_ASK_KICKER: Record<PhotoSubject, string> = {
  machine: 'Before you photograph it',
  meal: 'Before you photograph your meal',
};

export const PHOTO_ASK_TITLE: Record<PhotoSubject, string> = {
...
export const PHOTO_SEND_A11Y: Record<PhotoSubject, string> = {
 ...
 * reason.
 */
export const PHOTO_REFUSED_TITLE: Record<PhotoSubject, string> = {
