...
import { monthKey, type Snapshots } from './monthlyHistory';
...
export interface YearOnYear {
  ...
  monthKey: string;
  /** This month's recorded figure. */
  now: number;
  /** The same calendar month, twelve months back. */
  then: number;
  /** now − then, in whatever unit the metric is. Never formatted here: the
   *  screen owns the currency and this module must never assume one. */
  delta: number;
  /**
   * The change as a percentage of last year, rounded — or null.
   *
   * Null when last year was zero. A change from 0 is not an infinite
   * improvement and it is not a 100% one; there is no percentage of nothing,
   * and printing one is how "+∞%" or a meaningless "+100%" reaches a screen.
   * The delta is still there and is the honest way to say it.
   */
  pct: number | null;
}
 ...
 */
export function yearOnYear(snapshots: Snapshots, now: Date): YearOnYear | null {
  const thisKey = monthKey(now);
  const backDate = new Date(now.getFullYear(), now.getMonth() - 12, 1);
  const backKey = monthKey(backDate);
 ...
 */
export function monthsUntilYearOnYear(snapshots: Snapshots, now: Date): number {
  const backKey = monthKey(new Date(now.getFullYear(), now.getMonth() - 12, 1));
  if (typeof snapshots[backKey] === 'number') return 0;
...
export function yearOnYearLine(snapshots: Snapshots, now: Date, status: LoadStatus): string {
  ...
  if (wait > 0) {
    return `A year-on-year read needs the same month last year, and that month is ${num(wait)} ${wait === 1 ? 'month' : 'months'} away. A snapshot is recorded each month, so it appears on its own.`;
  }
  const thisKey = monthKey(now);
...
/**
 * One coaching relationship, reduced to its two dates.
 *
 * `startedAt` is `coaching_relationships.created_at` and it carries a caveat
 * worth stating: `link_coaching()` is `on conflict … do update set status =
 * 'active'`, so a client who left and re-joined keeps their ORIGINAL row and
 * their original date. A cohort here is therefore "people who first joined in
 * this month", and somebody who took a six-month break reads as continuously
 * coached. That flatters retention, it cannot be fixed from this side of the
 * schema, and `COHORT_CAVEAT` says so on the screen rather than only here.
 */
export interface CoachingSpan {
  startedAt: string;
  /** Null while the coaching is still running. */
  endedAt: string | null;
}

export interface CohortRow {
  /** 'YYYY-MM' of the month this cohort started. */
  month: string;
  /** How many people joined that month. */
  size: number;
  /**
   * Share still coaching at 1, 3, 6 and 12 months, as a percentage — or null
   * where the answer is not yet knowable or the cohort is too small to state.
   *
   * Null at a milestone the cohort has not REACHED yet is the one that matters
   * most: a cohort that joined two months ago has no six-month figure, and a
   * zero there would draw as a collapse. `MILESTONES` is the index.
   */
  retained: (number | null)[];
  /**
   * The same thing as a COUNT — "4 of 6" — or null only where the milestone has
   * not been reached.
   *
   * This is the half most coaches will actually read, and it is here because
   * the floor above would otherwise leave them with nothing. `MIN_COHORT_FOR_RATE`
   * is 10, which is a sensible floor for a gym's monthly intake and is more
   * people than most self-employed coaches sign in a month — so a coach-side
   * screen that only had percentages would say "too small to say" against every
   * row it ever drew, which is a true sentence and a useless screen.
   *
   * A count needs no floor because it makes no claim beyond itself: "4 of 6
   * were still with you at three months" is exactly as true for six people as a
   * percentage is misleading for them.
   */
  held: (number | null)[];
  /** True when the cohort is under the floor and no percentage is stated. */
  tooSmall: boolean;
}
...
export const MILESTONES = [1, 3, 6, 12] as const;

const MONTH_MS = 30.436875 * 86_400_000;
...
const monthsBetween = (fromMs: number, toMs: number): number =>
  Math.floor((toMs - fromMs) / MONTH_MS);
 ...
 */
export function cohorts(spans: readonly CoachingSpan[], now: Date): CohortRow[] {
  const nowMs = now.getTime();
  const by = new Map<string, { started: number; ended: number | null }[]>();
  for (const s of spans) {
    const st = Date.parse(s.startedAt);
    if (!Number.isFinite(st)) continue;
    const key = monthKey(new Date(st));
  ...
  for (const [month, people] of [...by.entries()].sort((a, b) => a[0].localeCompare(b[0]))) {
    ...
    const tooSmall = size < MIN_COHORT_FOR_RATE;
    ...
    const held = MILESTONES.map((m) => {
      if (monthsBetween(latest, nowMs) < m) return null;
 ...
 */
export function cohortsBlocker(spanStatus: LoadStatus): string | null {
  if (spanStatus === 'loading') return 'Reading who has started and finished with you…';
...
export const COHORT_CAVEAT =
  'A cohort is everybody who first started with you in that month. Somebody who left and later came back keeps their original start, so a break does not show here and this reads slightly better than the truth. Percentages are only stated once every person in the cohort has had the full time to reach that mark.';
