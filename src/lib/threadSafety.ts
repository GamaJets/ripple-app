 ...
 *   'open'            read, and nobody has blocked anybody.
 *   'blocked-by-me'   I blocked them. I can lift it.
 *   'blocked-by-them' they blocked me. I cannot lift it, and I am not told who
 *                     to be angry with — there is only one other person on a
 *                     thread and they already know.
 */
export type BlockState = 'unknown' | 'open' | 'blocked-by-me' | 'blocked-by-them';

/** One row of `thread_blocks`, reduced to the only thing that decides anything. */
export interface BlockRow { blockerId: string }
 ...
 */
export function blockStateOf(status: LoadStatus, rows: BlockRow[] | null, myId: string | null): BlockState {
  ...
  if (status !== 'ready' || rows == null) return 'unknown';
  if (!myId) return 'unknown';
  if (rows.some((r) => r.blockerId === myId)) return 'blocked-by-me';
 ...
 *  nobody has blocked. */
export const canSendInto = (state: BlockState): boolean => state === 'unknown' || state === 'open';
 ...
 */
export function blockedComposerNote(state: BlockState, other: string): string | null {
  switch (state) {
    case 'blocked-by-me':
...
export function blockActionLabel(state: BlockState, other: string): string {
 ...
 *  sessions. */
export function blockConfirm(other: string): { title: string; body: string } {
  return {
...
export function unblockConfirm(other: string): { title: string; body: string } {
  return {
 ...
 */
export const SEND_REFUSED_NOTE =
  'The server would not accept that message, so it has not been sent. This conversation is closed: either it has been blocked, or you two are no longer connected.';
 ...
 */
export function looksLikeThreadRefusal(err: unknown): boolean {
...
/** The categories `abuse_reports.category` allows (supabase/parts/240). Same
 *  five, in the order they are offered, so the app and the constraint cannot
 *  come to disagree. */
export type ReportCategory = 'harassment' | 'sexual' | 'threat' | 'spam' | 'other';
...
export function reportCategoryLabel(id: ReportCategory): string {
  return REPORT_OPTIONS.find((o) => o.id === id)?.label ?? 'Something Else';
}
...
export function reportFiledLine(category: ReportCategory, blocked: BlockState): string {
