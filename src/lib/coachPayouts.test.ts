...
import {
  ...
  payoutSummary,
  payoutsEmptyLine,
  PAYOUT_IS_NOT_A_SALE,
  PAYOUT_STRIPE_IS_THE_RECORD,
  type CoachPayout,
} from './coachPayouts';
...
const pay = (over: Partial<CoachPayout> = {}): CoachPayout => ({
  id: 'po_1',
  amountCents: 428100,
  currency: 'aed',
  status: 'paid',
...
eq(payoutState('paid'), 'arrived', 'paid is arrived');
eq(payoutState('pending'), 'on-the-way', 'pending is on the way');
eq(payoutState('in_transit'), 'on-the-way', 'and so is in transit');
eq(payoutState('failed'), 'failed', 'failed is failed');
eq(payoutState('canceled'), 'failed', 'and so is cancelled, however it is spelled');
eq(payoutState('cancelled'), 'failed', 'either spelling');
eq(payoutState('PAID'), 'arrived', 'case is not a different status');
...
eq(payoutState('settled'), 'unknown', 'a status this app has never seen is unknown, never arrived');
eq(payoutState(''), 'unknown', 'and so is an empty one');
eq(payoutState(null), 'unknown', 'and a missing one');
eq(payoutStateLabel('settled'), 'Not Stated', 'and it says so on the row rather than guessing');
eq(payoutStateLabel('paid'), 'Arrived', 'a known status has its own label');
...
{
  const s = payoutSummary([pay({ id: 'a', currency: 'aed' }), pay({ id: 'b', currency: 'gbp', amountCents: 90000 })], 'ready');
...
{
  const s = payoutSummary([pay({ currency: null })], 'ready');
...
ok(/not a statement that nothing has been paid out/i.test(payoutSummary([], 'error').withheld!),
  'a failed read says outright that it is not a statement about the bank');

/* ── 4. a payout that bounced is the row to act on ──────────────────────── */

{
  const line = payoutFailureLine(pay({ status: 'failed', failureMessage: 'Account number is invalid.' }));
...
{
  const line = payoutFailureLine(pay({ status: 'failed', failureMessage: null }));
...
eq(payoutFailureLine(pay({ status: 'paid' })), null, 'an arrived payout has no failure line');
eq(payoutFailureLine(pay({ status: 'in_transit' })), null, 'and neither has one on the way');
...
{
  const line = payoutsEmptyLine('ready');
...
ok(/could not be read/i.test(payoutsEmptyLine('error')), 'a failed read says the read failed');
ok(!/not the same as not having been paid/i.test(payoutsEmptyLine('error')), 'and does not reuse the other sentence');
...
ok(/not the proceeds of one sale/i.test(PAYOUT_IS_NOT_A_SALE), 'the page says a payout is not a sale');
ok(/never subtracted from each other/i.test(PAYOUT_IS_NOT_A_SALE), 'and that the two are never netted');
ok(/less what Stripe and Repple took/i.test(PAYOUT_IS_NOT_A_SALE), 'and says what a balance is made of');
ok(/cannot see your bank/i.test(PAYOUT_STRIPE_IS_THE_RECORD), 'and that this app holds no bank details');
ok(/Stripe dashboard is the record/i.test(PAYOUT_STRIPE_IS_THE_RECORD), 'and that Stripe remains the authority');
