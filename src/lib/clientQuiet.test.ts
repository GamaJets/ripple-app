...
const server = (fromHour = 22, toHour = 7): QuietHours => ({ fromHour, toHour, tz: 'Europe/London' });

/* ── which window is shown ─────────────────────────────────────────────── */

const none = quietView(prefs(), null);
...
const deviceOnly = quietView(prefs({ quiet: true, quietFromHour: 23, quietToHour: 6 }), null);
eq(deviceOnly.fromHour, 23, 'a device-only window is shown');
eq(deviceOnly.source, 'device-only', 'and named as such');
ok(/do not stop anything sent to you/.test(deviceOnly.note ?? ''),
  'and the note says the half they have not got — which is the half their coach uses');

// A new handset on an account that already has a window.
const serverOnly = quietView(prefs(), server(21, 8));
eq(serverOnly.fromHour, 21, 'an account window is shown on a phone that has none');
eq(serverOnly.source, 'server-only', 'and named');
ok(/already stopping anything sent to you/.test(serverOnly.note ?? ''),
  'and says the half that is already working');
...
const both = quietView(prefs({ quiet: true, quietFromHour: 23, quietToHour: 6 }), server(22, 7));
...
const agree = quietView(prefs({ quiet: true, quietFromHour: 22, quietToHour: 7 }), server(22, 7));
...
const offButStored = quietView(prefs({ quiet: false, quietFromHour: 1, quietToHour: 5 }), null);
...
const w = quietView(prefs(), server(22, 7));
eq(quietNow(23, w), true, '23:00 is inside 22 to 7');
eq(quietNow(2, w), true, 'and so is 02:00, past midnight');
eq(quietNow(7, w), false, 'the end is exclusive, so 07:00 is not');
eq(quietNow(21, w), false, 'and neither is the hour before it starts');
eq(quietNow(23, none), false, 'a member with no window is never inside one');

const zero = quietView(prefs(), server(9, 9));
eq(quietNow(9, zero), false, 'a zero-length window is no quiet hours, not twenty-four of them');
...
ok(/wait and arrive/.test(QUIET_LOCAL_EFFECT), 'the local half says the reminder waits');
ok(/Nothing is dropped/.test(QUIET_LOCAL_EFFECT), 'and that nothing is lost');
ok(/does not buzz at all/.test(QUIET_REMOTE_EFFECT), 'the remote half says it does not arrive');
ok(/still written into your notifications/.test(QUIET_REMOTE_EFFECT),
  'and says where it is instead — which is what makes suppressing it safe to offer');
ok(!/wait/.test(QUIET_REMOTE_EFFECT),
  'and never promises the remote half waits, because nothing holds a push');
...
ok(/6am/.test(QUIET_COST) && /eleven/.test(QUIET_COST),
  'the cost names the case rather than warning in general');
ok(/still come through/.test(QUIET_COST),
  'and says which reminders are exempt, so the sentence is not simply frightening');

ok(/timezone this phone was in when you saved/.test(QUIET_ZONE_KEPT),
  'the zone note says the zone is the stored one and not the current one');
...
const bothOk = quietSaveNote({ device: true, server: true });
...
const halfA = quietSaveNote({ device: true, server: false });
...
const halfB = quietSaveNote({ device: false, server: true });
...
const neither = quietSaveNote({ device: false, server: false });
