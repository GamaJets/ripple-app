...
import {
  dayGaps, sellableGaps, gapsAreKnown, gapsUnknownNote, gapLengthLabel, gapNote,
  gapsHeading, MIN_SELLABLE_MIN, type GapBlocker,
} from './dayGaps';
import { workWindows } from './moveTimes';
...
const WORK = workWindows([{ dow: DOW, hour: 7, minute: 0, dur: 12 * 60 }], DOW);
const NOW = at(1).getTime();

const nine: GapBlocker = { startsAt: iso(9), durationMin: 60 };
const two: GapBlocker = { startsAt: iso(14), durationMin: 60 };

const base = { year: Y, monthIndex: M, day: D, work: WORK, nowMs: NOW };

/* ── the hole between two sessions ──────────────────────────────────────── */

const holes = dayGaps({ ...base, blockers: [nine, two], open: [] });
...
ok(!sellableGaps(wholeGapOpen).some((g) => g.startMs === at(10).getTime()),
  'so it is not reported — a coach knows about the hours they published');

const partlyOpen = dayGaps({
  ...base, blockers: [nine, two],
...
ok(sellableGaps(partlyOpen).some((g) => g.startMs === at(10).getTime()),
  'which is still worth saying');
...
const doubled = dayGaps({
  ...base, blockers: [nine, two],
  open: [{ startsAt: iso(10), durationMin: 60 }, { startsAt: iso(10, 30), durationMin: 60 }],
});
eq(doubled.find((g) => g.startMs === at(10).getTime())?.openMin, 90,
  'overlapping open slots are counted once');
...
ok(!tight.some((g) => g.startMs === at(10).getTime()),
  'fifteen minutes between two clients is a turnaround, not a hole');

const noHours = dayGaps({ ...base, work: [], blockers: [nine], open: [] });
eq(noHours.length, 0, 'a day the coach never said they work has no gaps to report');

const afternoon = dayGaps({ ...base, nowMs: at(12).getTime(), blockers: [nine, two], open: [] });
ok(!afternoon.some((g) => g.startMs < at(12).getTime()),
  'this morning is not something a coach can sell this afternoon');
ok(afternoon.some((g) => g.startMs === at(12).getTime()),
  'and the part of the hole still ahead is reported from now');

const done = dayGaps({ ...base, nowMs: at(23).getTime(), blockers: [], open: [] });
eq(done.length, 0, 'a day that is over has nothing left in it');

/* ── two windows, one day ───────────────────────────────────────────────── */

const splitDay = dayGaps({
  ...base,
  work: workWindows([
...
ok(!splitDay.some((g) => g.startMs < at(16).getTime() && g.endMs > at(10).getTime()),
  'and the hours between them are not claimed as free — the coach is not there');

/* ── the wall clock ─────────────────────────────────────────────────────── */

for (const g of dayGaps({ ...base, blockers: [nine, two], open: [] })) {
...
eq(gapsAreKnown('ready', 'ready', 'ready'), true, 'three whole reads support the claim');
for (const bad of ['loading', 'partial', 'error'] as const) {
  eq(gapsAreKnown(bad, 'ready', 'ready'), false, `an unread diary does not (${bad})`);
  eq(gapsAreKnown('ready', bad, 'ready'), false, `nor an unread timetable (${bad})`);
  eq(gapsAreKnown('ready', 'ready', bad), false, `nor unread working hours (${bad})`);
}
eq(gapsUnknownNote('ready', 'ready', 'ready'), null, 'and then there is nothing to say');
const notes = new Set<string>();
for (const bad of ['loading', 'partial', 'error'] as const) {
  const note = gapsUnknownNote(bad, 'ready', 'ready') ?? '';
  ok(note.length > 0, `'${bad}' says something`);
  ok(!/free|nothing booked/i.test(note) || /not known|not established/i.test(note),
    `and '${bad}' never states an hour is free`);
  notes.add(note);
}
eq(notes.size, 3, 'three causes, three sentences');
ok(/failed/i.test(gapsUnknownNote('error', 'ready', 'ready') ?? ''),
  'a failed read is named as a failure and not as an empty day');

/* ── the words ─────────────────────────────────────────────────────────── */

eq(gapLengthLabel(45), '45min', 'under an hour is minutes');
eq(gapLengthLabel(60), '1h', 'an exact hour has no minutes after it');
eq(gapLengthLabel(150), '2h 30min', 'and the rest is spelled out');
eq(gapLengthLabel(-5), '0min', 'a negative length is not printed as one');

ok(/Nobody can book this/.test(gapNote({
...
ok(/already open for booking/.test(gapNote({
  startsAt: iso(10), startMs: at(10).getTime(), endMs: at(14).getTime(),
  minutes: 240, openMin: 60, sellableMin: 180,
})), 'and a partly open one says how much of it is');

eq(gapsHeading(0), null, 'nothing to head means no heading');
ok(!/\d/.test(gapsHeading(1) ?? ''), 'one is counted in words, not as a figure');
ok(/2/.test(gapsHeading(2) ?? ''), 'and more than one carries the count');

eq(MIN_SELLABLE_MIN, 30, 'the shortest thing this product sells is half an hour');
