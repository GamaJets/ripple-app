...
import { previewCoachRoster, type CoachClientRow } from './csvImport';
import { screenInvites } from './memberInvites';
import {
...
const preview = previewCoachRoster(FILE);
...
ok(preview.rejected.some((r) => r.value?.name === 'Dup Person' && r.errors.some((e) => /duplicate of line/.test(e))),
  'the second use of one address names the line the first was on');

// Two people can genuinely share a name. Refusing the second would make a coach
// hand-add somebody the file already listed.
const twins = previewCoachRoster('Name\nSam Patel\nSam Patel');
eq(twins.ready.length, 2, 'two clients with the same name are two clients');

/* ── the file that cannot be imported at all ────────────────────────────── */

const noName = previewCoachRoster('Email,Goal\na@b.com,Strength');
const noNamePlan = rosterPlan(noName, screenInvites<CoachClientRow>(noName.ready));
ok(!!planBlocker(noName, noNamePlan), 'a file with no name column is blocked');
ok(/nothing has been imported/i.test(planBlocker(noName, noNamePlan)!),
  'and says nothing has happened, because an import that adds nobody must not report success');

const headerOnly = previewCoachRoster('Name,Email');
ok(!!planBlocker(headerOnly, rosterPlan(headerOnly, screenInvites<CoachClientRow>([]))),
  'a header with no rows under it is blocked');

const allBad = previewCoachRoster('Name,Delivery\nSam,quarterly');
ok(!!planBlocker(allBad, rosterPlan(allBad, screenInvites<CoachClientRow>(allBad.ready))),
  'a file every row of which was refused is blocked');

/* ── the plan, and the asymmetry that matters ───────────────────────────── */

const screened = screenInvites<CoachClientRow>(preview.ready, ['zoe@fit.com']);
const plan = rosterPlan(preview, screened);
...
const summary = planSummary(plan, preview.rows.length);
...
const noEmails = previewCoachRoster('Name\nSam\nBen');
const noEmailPlan = rosterPlan(noEmails, screenInvites<CoachClientRow>(noEmails.ready));
const noEmailSummary = planSummary(noEmailPlan, noEmails.rows.length);
...
const clean = resultSummary({ rows: [{ name: 'Sam', outcome: 'added' }] });
...
const nothing = resultSummary({ rows: [] });
...
const b64 = (s: string): string => {
  ...
  for (const ch of s) {
    ...
    else if (cp < 0x800) utf8.push(0xc0 | (cp >> 6), 0x80 | (cp & 0x3f));
    else if (cp < 0x10000) utf8.push(0xe0 | (cp >> 12), 0x80 | ((cp >> 6) & 0x3f), 0x80 | (cp & 0x3f));
...
eq(base64ToUtf8(b64('Name,Email')), 'Name,Email', 'plain ASCII survives');
...
eq(base64ToUtf8(b64('Zoë Adams')), 'Zoë Adams', 'an accented name is read as itself');
eq(base64ToUtf8(b64('Ünal Öz, Škoda, 北京')), 'Ünal Öz, Škoda, 北京', 'so are two- and three-byte characters');
eq(base64ToUtf8(b64('Sam 🏋️ Patel')), 'Sam 🏋️ Patel', 'and four-byte ones, as a surrogate pair');
eq(base64ToUtf8(''), '', 'an empty payload is an empty file, not a failure');
eq(base64ToUtf8(b64('a\nb\nc')), 'a\nb\nc', 'newlines survive, which is the entire file format');
...
const withBom = base64ToUtf8(b64('﻿Name,Email\nSam,sam@fit.com'))!;
ok(withBom.startsWith('Name'), 'a byte-order mark is stripped');
eq(previewCoachRoster(withBom).missingRequired.length, 0, 'so an Excel export finds its name column');
...
eq(base64ToUtf8('not base64!!'), null, 'a payload that is not base64 is refused');
eq(base64ToUtf8('%%%%'), null, 'and so is one made of characters outside the alphabet');
// Whitespace and padding are ordinary and must not be.
eq(base64ToUtf8('TmFtZQ=='), 'Name', 'padding is fine');
eq(base64ToUtf8('TmFt\nZQ=='), 'Name', 'and so are line breaks inside the payload');
