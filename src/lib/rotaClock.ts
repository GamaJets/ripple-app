...
import { gymDay, gymHour, instantAtGym, isZone, readerZone, NO_ZONE_NOTE } from './gymZone';
import { whoseClockNote } from './gymWhen';

export { NO_ZONE_NOTE, whoseClockNote };

/** Which clock a rota screen is actually drawing, and what it owes the reader. */
export interface RotaClock {
  /** The zone in force: the gym's when it has one, otherwise the reader's own
   *  IANA name where this runtime will give it up, and null when even that
   *  could not be read. */
  zone: string | null;
  /** True when this is the GYM's clock. False means the reader's was used. */
  atGym: boolean;
  /** The sentence to print beside the times when it is not the gym's. Null
   *  when it is, so a screen can render it unconditionally. */
  note: string | null;
}

/** What clock a rota is on, given whatever `tenants.timezone` came back as. */
export function rotaClock(zone: string | null | undefined): RotaClock {
  if (isZone(zone)) return { zone: String(zone).trim(), atGym: true, note: null };
  return { zone: readerZone(), atGym: false, note: whoseClockNote(zone) };
}
...
function readerDay(d: Date): string {
  return `${String(d.getFullYear()).padStart(4, '0')}-${pad2(d.getMonth() + 1)}-${pad2(d.getDate())}`;
}

function asDate(at: string | number | Date | null | undefined): Date | null {
 ...
 */
export function rotaDay(at: string | number | Date | null | undefined, zone: string | null | undefined): string | null {
  if (isZone(zone)) return gymDay(at, zone);
  const d = asDate(at);
 ...
 * laptop.
 */
export function rotaHour(at: string | number | Date | null | undefined, zone: string | null | undefined): number | null {
  if (isZone(zone)) return gymHour(at, zone);
  const d = asDate(at);
  return d ? d.getHours() : null;
}

/** One cell of the rota grid: the day and the hour, on the same clock. */
export interface RotaCell { date: string; hour: number }
...
export function rotaCell(at: string | number | Date | null | undefined, zone: string | null | undefined): RotaCell | null {
 ...
 */
export function rotaTimeLabel(at: string | number | Date | null | undefined, zone: string | null | undefined): string | null {
  const d = asDate(at);
  if (!d) return null;
  const hour = rotaHour(at, zone);
  if (hour == null) return null;
  const minute = isZone(zone) ? gymMinute(d, String(zone)) : d.getMinutes();
...
function gymMinute(d: Date, zone: string): number | null {
  try {
 ...
 */
export function rotaInstant(
  day: string | null | undefined, hour: number, zone: string | null | undefined,
): string | null {
  ...
  if (rolled.day == null) return null;

  if (isZone(zone)) return instantAtGym(`${rolled.day}T${pad2(rolled.hour)}:00`, zone);
 ...
 * about.
 */
export function addCalendarDays(day: string | null | undefined, n: number): string | null {
  const d = String(day ?? '').trim();
  if (!/^\d{4}-\d{2}-\d{2}$/.test(d)) return null;
  ...
  const x = new Date(t + n * 86_400_000);
 ...
 */
export function calendarWeekday(day: string | null | undefined): number | null {
  const d = String(day ?? '').trim();
  if (!/^\d{4}-\d{2}-\d{2}$/.test(d)) return null;
 ...
 */
export function rotaToday(zone: string | null | undefined, now: number | Date = Date.now()): string | null {
