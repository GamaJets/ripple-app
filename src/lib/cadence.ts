 ...
 */
export const MAX_USUAL_GAP_DAYS = 21;
...
export const MIN_LATE_DAYS = 3;

/** Where this client stands against their own interval. */
export type DueState =
  /** No settled interval, or nothing on record. Says NOTHING about the person —
   *  it says this module has no basis, which is not the same as "on time". */
  | 'unknown'
  /** Inside their usual gap. They are not late; there is nothing to raise. */
  | 'inside'
  /** Past the gap, but not past it far enough to be worth a coach's attention.
   *  Held apart from 'overdue' deliberately: everybody is 'due' every week, and
   *  a prompt at 'due' is the nagging src/lib/nudge.ts refuses. */
  | 'due'
  /** Past their own gap by more than the tolerance above. */
  | 'overdue';

export interface Cadence {
  state: DueState;
  /** Their usual interval between active days, in days, to one decimal. Null
   *  when they have no settled one. */
  usualGapDays: number | null;
  /** Days since the newest thing on record. Null when there is nothing on
   *  record at all — which is NOT zero and not infinity. */
  sinceLastDays: number | null;
  /** The local day their own pattern put them on, `YYYY-MM-DD`. Null without an
   *  interval or without a last visit. Arithmetic on their last active day, and
   *  never a prediction of what they will do. */
  expectedDay: string | null;
  /** How far past `expectedDay` they are, in whole days. Null when unknown,
   *  negative is not returned — a client who is early is 'inside' and this is
   *  null, because "minus three days late" is a number nobody reads correctly. */
  overdueDays: number | null;
  /** Active days the interval was computed from. */
  activeDays: number;
  /** Days of record between their first and last active day. */
  spanDays: number | null;
  /** Why there is no interval, when there is not. Null when there is one. */
  noPattern: 'no-events' | 'too-few' | 'too-short' | 'too-spread' | null;
}

/** Nothing to say, and the reason. Used for every early return so a caller
 *  cannot receive a half-filled object. */
function none(why: Cadence['noPattern'], partial: Partial<Cadence> = {}): Cadence {
  return {
    state: 'unknown',
    usualGapDays: null,
    sinceLastDays: null,
...
export function assessCadence(
  ...
  const days = activeDayLog([...events], now - windowDays * DAY, now + 1);
  if (days.length === 0) return none('no-events');
  ...
  if (days.length < MIN_ACTIVE_DAYS) return none('too-few', carry);
  if (spanDays < MIN_SPAN_DAYS) return none('too-short', carry);
  ...
  if (!(usual > 0)) return none('too-spread', carry);
  if (usual > MAX_USUAL_GAP_DAYS) return none('too-spread', carry);

  const expectedMs = last + usual * DAY;
  const expectedDay = localDayKey(expectedMs);
  const late = (todayMs - expectedMs) / DAY;
  const tolerance = Math.max(MIN_LATE_DAYS, usual * LATE_FRACTION);

  const state: DueState = late > tolerance ? 'overdue' : late > 0 ? 'due' : 'inside';
 ...
 *  what they did. */
export function cadenceLine(c: Cadence): string {
  if (c.state === 'unknown') {
    switch (c.noPattern) {
      case 'no-events':
 ...
 */
export function worthRaising(c: Cadence): boolean {
 ...
 */
export function byLateness<T extends { cadence: Cadence }>(rows: readonly T[]): T[] {
  const share = (c: Cadence) =>
    c.overdueDays != null && c.usualGapDays ? c.overdueDays / c.usualGapDays : -1;
 ...
 */
export function overdueNote(rows: readonly { cadence: Cadence }[] | null): string {
  if (rows == null) return 'Working out who was due…';
