...
import {
  bespokeNote, behindNote, memberVersions, programSignature, versionOf, versionSpread,
  type GroupVersion,
} from './groupProgram';
...
const prog = (movement: string): Program => ({
...
const versions: GroupVersion[] = [
  { version: 1, signature: programSignature(v1), createdAt: '2026-06-01T00:00:00Z' },
  { version: 2, signature: programSignature(v2), createdAt: '2026-07-01T00:00:00Z' },
  { version: 3, signature: programSignature(v3), createdAt: '2026-08-01T00:00:00Z' },
];
const sig3 = programSignature(v3);
...
const mv = memberVersions('ready', versions, 3, members, sig3);
...
eq(mv.map((m) => m.behind), [false, true, false, false],
  'only the member on an EARLIER stored version is behind — the bespoke one is not out of date, they are different on purpose');

const spread = versionSpread(mv, 'ready', 'ready', 3);
eq([spread.onCurrent, spread.behind, spread.bespoke, spread.none, spread.unknown], [1, 1, 1, 1, 0],
  'one on it, one behind, one bespoke, one on nothing — four rows a coach reads four different ways');
...
const unread = memberVersions('error', versions, 3, members, sig3);
...
const unreadSpread = versionSpread(unread, 'ready', 'error', 3);
...
const names = (id: string) => ({ ann: 'Ann', bob: 'Bob', cat: 'Cat', dan: 'Dan' } as Record<string, string>)[id] ?? id;

eq(behindNote(unread, names, unreadSpread), null,
  'no re-send is offered over an unread `assigned_programs` — the offer is an overwrite of somebody’s training');
eq(bespokeNote(unread, names, unreadSpread), null, 'and no warning is given about it either');

/* ── the two sentences, kept apart on purpose ───────────────────────────── */

const behind = behindNote(mv, names, spread) as string;
...
const bes = bespokeNote(mv, names, spread) as string;
...
eq(behindNote(memberVersions('ready', versions, 3, [{ clientId: 'ann', assigned: v3 }], sig3), names,
  versionSpread(memberVersions('ready', versions, 3, [{ clientId: 'ann', assigned: v3 }], sig3), 'ready', 'ready', 3)),
  null, 'and with nobody behind there is no cheerful "everybody is up to date" printed over anything');
