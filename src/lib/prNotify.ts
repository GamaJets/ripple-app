...
import { clip, NOTICE_BODY_MAX, NOTICE_TITLE_MAX } from './notifyCopy';
import { localDayKey } from './clientDrift';
import { liftLabel } from './units';

/** The set that took the record, in the units it is STORED in. `kg` is
 *  kilograms whatever the member typed — the runner converts at its own
...
export interface PrSet {
  ...
  kg: number;
  /** Reps completed on the set that took the record. */
  reps: number;
}

/** What this device has already told the coach about today. One record, not a
 *  list, because the rule is one message per local day: nothing older than the
 *  current day can change any answer, so nothing older is worth keeping. */
export interface PrAnnounced {
  /** The local day it was sent on, as `YYYY-MM-DD`. */
  day: string;
  /** The movement it was about, normalised by `movementKey`. Carried so the
   *  refusal can name it. */
  movement: string;
}
...
export const PR_TITLE = 'A client set a personal best';
...
export const PR_ROUTE_BASE = '/(trainer)/client-training';
 ...
 * training is not.
 */
export function prRoute(clientId: string | null | undefined): string | null {
  const id = (clientId ?? '').trim();
  if (!id) return null;
  return `${PR_ROUTE_BASE}?clientId=${encodeURIComponent(id)}`;
}
...
export function prDayKey(atMs: number): string {
  return localDayKey(atMs);
}

/** Why a personal best was, or was not, passed on. */
export interface PrDecision {
  announce: boolean;
  /** The sentence a reader of the log needs. Never shown to anybody. */
  why: string;
}
 ...
 * record again.
 */
export function prDecision(
  last: PrAnnounced | null | undefined,
  set: PrSet,
  atMs: number,
): PrDecision {
 ...
 *  message. */
export function announcedRecord(set: PrSet, atMs: number): PrAnnounced {
  return { day: prDayKey(atMs), movement: movementKey(set.movement) };
}

/** A title and a body, capped where `notify_users()` would cap them anyway. */
 ...
 */
export function bothUnits(kg: number): string {
  const metric = liftLabel(kg, 'kg');
  const imperial = liftLabel(kg, 'lb');
  if (!metric) return '';
 ...
 */
export function prNotification(set: PrSet, clientName: string | null | undefined): Notification | null {
  ...
  if (!movement || !load || !reps) return null;
  const who = (clientName ?? '').trim() || 'A client';
  const repWord = reps === 1 ? 'rep' : 'reps';
  const body =
    `${who} just logged ${movement} at ${load} for ${reps} ${repWord}, `
  ...
  return { title: clip(PR_TITLE, NOTICE_TITLE_MAX), body: clip(body, NOTICE_BODY_MAX) };
}
