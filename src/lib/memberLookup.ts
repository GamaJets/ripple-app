 ...
 */
export function memberNoBody(id: string | null | undefined): string | null {
  const key = String(id ?? '').trim();
  if (!key) return null;
  const n = memberNoFrom('', key, null);
 ...
 */
export function memberNoFor(id: string | null | undefined, brandLabel: string | null | undefined): string | null {
  const key = String(id ?? '').trim();
  return key ? memberNoFrom('', key, brandLabel ?? null) : null;
}
...
/**
 * What somebody typed or scanned, and how confident this module is about it.
 *
 * 'numbered' is a member number and nothing else could be: the shape carries a
 * prefix and a hyphen. 'bare' is nine characters that COULD be a body and could
 * equally be somebody's name — this is why a bare scan that matches nobody
 * falls silently back to the name search rather than announcing that no such
 * member exists. 'text' is a name.
 */
export type ScanShape =
  | { shape: 'blank' }
  | { shape: 'numbered'; body: string }
  | { shape: 'bare'; body: string }
  | { shape: 'text' };
 ...
 * of it.
 */
export function readScan(raw: string | null | undefined): ScanShape {
  const v = String(raw ?? '')
    .trim()
    .replace(/^\*+|\*+$/g, '')
    .toUpperCase()
    ...
    .trim();
  if (!v) return { shape: 'blank' };
  if (NUMBERED.test(v)) return { shape: 'numbered', body: v.slice(4) };
  if (BODY.test(v)) return { shape: 'bare', body: v };
  return { shape: 'text' };
}

/**
 * Anyone the desk can look up: the fields this module needs and no others.
 *
 * Every function below is generic over it rather than taking it exactly, so a
 * caller keeps whatever else its own rows carry — a membership status, a plan
 * name — through the lookup and out the other side. Narrowing to this shape and
 * handing the caller back two fields would make the screen re-find each person
 * in its own list by id, which is a second lookup that can disagree with this
 * one.
 */
...
/**
 * How the roster read came back.
 *
 * The same distinction every read in this codebase keeps, and it decides one
 * thing here: whether "no such member" may be said out loud. 'partial' is not
 * 'ready' — a prefix of a roster is exactly the set that makes a real member
 * look like a stranger.
 */
export type RosterState = 'loading' | 'ready' | 'partial' | 'failed';
 ...
 */
export function indexByMemberNo<T extends LookupPerson>(people: readonly T[]): Map<string, T[]> {
  const out = new Map<string, T[]>();
  for (const p of people) {
    const body = memberNoBody(p.id);
    if (!body) continue;
    const at = out.get(body);
    if (at) at.push(p); else out.set(body, [p]);
  }
  return out;
}

export type ScanResult<T extends LookupPerson = LookupPerson> =
  /** Nothing typed. */
  | { kind: 'blank' }
  /** Not a member number, or a bare body nobody on this roster has. Search by name. */
  | { kind: 'name' }
  /** Exactly one member has this number. */
  | { kind: 'one'; person: T }
  /** More than one does, and the desk has to choose. */
  | { kind: 'ambiguous'; people: T[] }
  /** A member number, and nobody on a WHOLE roster has it. */
  | { kind: 'unknown' }
  /** A member number, and the roster is not whole, so nothing may be concluded. */
  | { kind: 'unsure'; state: RosterState };
 ...
 * at the desk to do.
 */
export function findByMemberNo<T extends LookupPerson>(
  raw: string | null | undefined,
  people: readonly T[],
  state: RosterState,
): ScanResult<T> {
  const scan = readScan(raw);
  if (scan.shape === 'blank') return { kind: 'blank' };
  if (scan.shape === 'text') return { kind: 'name' };
  ...
  if (scan.shape === 'bare') return { kind: 'name' };
 ...
 */
export function scanNote(r: ScanResult<LookupPerson>): string | null {
  switch (r.kind) {
    case 'blank':
    case 'name':
    case 'one':
    ...
    case 'ambiguous':
      return (
        `${r.people.length} members share that number, so nobody has been chosen. ` +
        `Pick the right one below, or ask their name.`
      );
    case 'unknown':
      return (
        'Nobody on this roster has that number. It is a Repple member number from the ' +
