...
//     here.
import {
  missingRegisters, peopleWaiting, gapsHeading, gapsNote, gapLine,
} from './registerGaps';
...
function row(p: Partial<ClassSummaryRow> & { classId: string }): ClassSummaryRow {
  return {
...

{
  ...
  ok(!gaps.some((g) => g.classId === 'walkin-only'),
    'and so does one with a single walk-in ticked — that tick IS a coach at the door');
  ok(!gaps.some((g) => g.classId === 'nobody-booked'),
    'a class nobody booked had no register to take, so it is not paperwork anybody owes');
}
...

{
  ...
  // register gets taken.
  eq(missingRegisters([row({ classId: 'now', booked: 4, startsAt: NOW.toISOString() })], NOW).length, 1,
    'a class starting on this very instant is one you are standing in');
}
...

{
  ...
  eq(missingRegisters([row({ classId: 'only-unreadable', booked: 3, startsAt: '' })], NOW).length, 1,
    'and an empty start does not empty the list');
}
...

{
  ...
  eq(missingRegisters(tied, NOW).map((g) => g.classId).join(','), 'alpha,zeta', 'ties break on the id');
  eq(missingRegisters([...tied].reverse(), NOW).map((g) => g.classId).join(','), 'alpha,zeta',
    'and the answer does not depend on the order the server happened to return them in');
}
...

{
  ...
  eq(peopleWaiting(gaps), 18, 'the people booked across the open registers are summed');
  eq(peopleWaiting([]), 0, 'no open registers is nobody waiting, which is a real answer');
  eq(peopleWaiting([{ classId: 'z', title: 'T', branch: 'B', startsAt: '', booked: NaN }]), null,
    'a row whose booked count is not a number withholds the WHOLE total, never part of it');
  eq(peopleWaiting([{ classId: 'z', title: 'T', branch: 'B', startsAt: '', booked: -2 }]), null,
    'and so does a negative one, which is not a number of people');
}

/* ── 6. the words ─────────────────────────────────────────────────────────*/

{
  eq(gapsHeading([]), null, 'a coach who has taken every register is not congratulated on it every visit');
  eq(gapsNote([]), null, 'and there is no note over an empty list');

  const one = missingRegisters([row({ classId: 'x', booked: 1 })], NOW);
  eq(gapsHeading(one), '1 register still open', 'one reads as one');
  ok((gapsNote(one) as string).includes('The 1 person who booked'), 'and the note counts that one person');

  const many = missingRegisters([row({ classId: 'x', booked: 11 }), row({ classId: 'y', booked: 7 })], NOW);
  eq(gapsHeading(many), '2 registers still open', 'and two read as two');
  const note = gapsNote(many) as string;
  ok(note.includes('18 people'), 'the note names the headcount that is missing from the gym’s record');
  ok(note.includes('no closing time'),
    'and says the thing that makes the list worth showing: the register can still be taken');
  // The claim this must NOT make. Whether a gym has already run payroll for
  // that period is not a fact this screen has read.
  ok(!/paid|pay|payroll|money/i.test(note),
    'nothing here promises the coach will be paid for taking it — that is the gym’s run, not ours');

  const unreadable = { classId: 'z', title: 'T', branch: 'B', startsAt: '', booked: NaN };
  ok((gapsNote([unreadable]) as string).startsWith('The people who booked them'),
    'an unreadable headcount is said in words rather than printed as a number');
}

/* ── 7. the line under one class ──────────────────────────────────────────*/

{
  eq(gapLine({ classId: 'a', title: 'T', branch: 'B', startsAt: '', booked: 1 }),
    '1 person booked and nobody was marked.', 'one person is not "1 people"');
  eq(gapLine({ classId: 'a', title: 'T', branch: 'B', startsAt: '', booked: 12 }),
    '12 people booked and nobody was marked.', 'and twelve are');
  ok(gapLine({ classId: 'a', title: 'T', branch: 'B', startsAt: '', booked: NaN })
    .includes('could not be read'),
    'a headcount that cannot be read says so rather than printing a nought');
}
