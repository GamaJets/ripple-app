...
export function levelFor(ratio: number, mult: number[]): number {
  let lvl = -1;
  for (let i = 0; i < mult.length; i++) if (ratio >= mult[i]) lvl = i;
  return lvl;
}

export type LiftGrade =
  /** No best lift on the log for this movement. */
  | { kind: 'unlogged' }
  /** A best lift, and no bodyweight to grade it against. */
  | { kind: 'ungradable'; best: number }
  /** Both known. `level` is -1 below the first standard. */
  | { kind: 'graded'; best: number; ratio: number; level: number };
...
export function gradeLift(
  ...
  if (best == null || !Number.isFinite(best) || best <= 0) return { kind: 'unlogged' };
