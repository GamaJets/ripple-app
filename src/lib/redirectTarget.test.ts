...
import { checkRedirect, parseRedirectAllow, schemeOf, HOSTILE_SCHEMES } from './redirectTarget';
...
const allowed = (offered: unknown, fallback: string, allow: string[] = []): string | null => {
  const v = checkRedirect(offered, fallback, allow);
...
eq(parseRedirectAllow(undefined).length, 0, 'an unset secret is an empty list');
eq(parseRedirectAllow(null).length, 0, 'a null secret is an empty list');
eq(parseRedirectAllow('').length, 0, 'an empty secret is an empty list');
eq(parseRedirectAllow('   ').length, 0, 'a whitespace secret is an empty list');
eq(parseRedirectAllow('repple://').join('|'), 'repple://', 'one entry');
eq(parseRedirectAllow(' repple:// , https://A.example ').join('|'), 'repple://|https://a.example',
...
eq(parseRedirectAllow('repple://,,https://a.example,').length, 2, 'empty entries are dropped');
ok(!parseRedirectAllow('repple://,,').includes(''), 'no empty prefix survives');

/* ── the scheme ─────────────────────────────────────────────────────────── */

eq(schemeOf('https://a.example/x'), 'https', 'https');
eq(schemeOf('HTTPS://a.example/x'), 'https', 'a scheme is lowercased');
eq(schemeOf('repple://billing'), 'repple', 'a custom scheme');
...
eq(schemeOf('exp+repple://x'), 'exp+repple', 'a scheme may carry a plus');
eq(schemeOf('+bad://x'), null, 'a scheme may not begin with a symbol');
eq(schemeOf('exp://192.0.2.1:8081/--/billing'), 'exp', 'the Expo dev scheme reads');
eq(schemeOf('//a.example/x'), null, 'protocol-relative has no scheme');
eq(schemeOf('/billing'), null, 'a path has no scheme');
eq(schemeOf('a.example/x'), null, 'a bare host has no scheme');
...
eq(allowed(undefined, FALLBACK, ['https://only.example']), FALLBACK,
  'the fallback is returned even when the list would not admit it');
...
eq(allowed({ toString: () => 'https://a.example' }, FALLBACK), null,
  'an object that stringifies to a URL is refused');
...
for (const s of HOSTILE_SCHEMES) eq(allowed(`${s}:whatever`, FALLBACK), null, `${s}: is refused`);
...
eq(allowed('https://good.example\\@evil.example/', FALLBACK), null,
  'a backslash is refused — it is one host to a reader and another to a browser');
...
eq(allowed('https://repplefitness.com.evil.example/x', FALLBACK, LIST), null,
  'a longer hostname beginning with the allowed one is refused');
eq(allowed('https://repplefitness.commercial.example/', FALLBACK, LIST), null,
  'a hostname that merely starts with the allowed one is refused');
eq(allowed('https://repplefitness.com@evil.example/', FALLBACK, LIST), null,
  'userinfo before a different host is refused');
eq(allowed('https://repplefitness.com-evil.example/', FALLBACK, LIST), null,
  'a hyphenated continuation is refused');
...
eq(allowed('HTTPS://RepplefitnesS.com/Connect-Return', FALLBACK, LIST),
  'HTTPS://RepplefitnesS.com/Connect-Return',
  'the match ignores case and the URL passed on keeps it');
...
eq(allowed('javascript:alert(1)', FALLBACK, parseRedirectAllow('javascript:')), null,
  'listing javascript: does not admit it');
eq(allowed('data:text/html,x', FALLBACK, parseRedirectAllow('data:,')), null,
  'listing data: does not admit it');

/* ── refusals say something a person can read ───────────────────────────── */

const refusal = checkRedirect('https://evil.example', FALLBACK, LIST);
ok(refusal.ok === false, 'the refusal is a refusal');
if (refusal.ok === false) {
  ok(refusal.reason.length > 0, 'a refusal carries a reason');
  ok(/nothing has been charged/.test(refusal.reason),
    'a refusal on a payment path says nothing was charged');
