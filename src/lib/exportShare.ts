...
const page = (title: string, body: string) =>
  `<html><head><meta name="viewport" content="width=device-width, initial-scale=1"><style>
   ...
   </style></head><body><div class="h"><h1>${title}</h1><p>Repple</p></div>${body}<p class="foot">Generated by Repple</p></body></html>`;

export interface PlanMealRow { slot: string; name: string; K: number; P: number; C: number; F: number }

export function mealPlanDoc(name: string, targetKcal: number, meals: PlanMealRow[], avoid: string[] = []): { html: string; text: string } {
