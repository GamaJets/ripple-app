...
import { withWeeks } from './programBlock';
import { weekdayOfIso } from './dayPlan';
import type { NamedClient } from './slotName';
...
{
  const tap = clientTap('ana', roster, 'ready');
  eq(tap, { can: true, clientId: 'ana', name: 'Ana Ruiz', why: null, who: 'named' },
    'a named client on the roster opens, with their name to pass as a title');
  ok(clientTapLabel(tap).includes('Ana Ruiz'), 'and the spoken label names them');
  ok(/record/.test(clientTapLabel(tap)) && /planned/.test(clientTapLabel(tap)),
    'and says both things the tap does, because it does two');
}

{
  const tap = clientTap('blank', roster, 'ready');
  ...
  eq(tap.name, null,
    'and passes NO name rather than a blank one, which the record would print as a title');
  eq(tap.who, 'unnamed', 'the reading is carried so the screen can branch on the fact');
  ok(!/undefined|null/.test(clientTapLabel(tap)),
    'and the spoken label stands on its own without a name in it');
}

/* ── the two that do not ───────────────────────────────────────────────── */

{
  const tap = clientTap(null, roster, 'ready');
...

{
  ...
  const gone = clientTap('ghost', roster, 'ready');
  eq(gone.who, 'unread', 'a booked hour whose client is not on a complete roster is unread');
  eq(gone.can, false, 'and is not a button');
  ok(gone.why !== null && /no longer on your book/i.test(gone.why),
    'and says they have left, which is not something a refresh fixes');
  ok(gone.why !== null && !/refresh|load/i.test(gone.why),
    'so it does not send the coach round a loop that cannot end');
}
...
for (const status of STATUSES.filter((s) => s !== 'ready')) {
  ...
  const tap = clientTap('ana', [], status);
  ...
  ok(tap.why !== null && /until your roster loads/i.test(tap.why),
    `and the refusal is about the read, not about the client (${status})`);
}
...
for (const status of STATUSES) {
  for (const id of [null, undefined, '', 'ghost']) {
    const tap = clientTap(id, roster, status);
    if (!tap.can) {
  ...
  for (const id of ['ana', 'blank']) {
    ...
    if (tap.can) {
      ...
      ok(tap.name === null || tap.name === 'Ana Ruiz',
        `and its name is a name or nothing, never a noun phrase (${id}, ${status})`);
...
const ex = (name: string): ProgramExercise =>
  ({ key: name.toLowerCase(), name, group: 'full', sets: 3, reps: '8', alternatives: [] });
...
const block = (n: number): Program => withWeeks(one([day('Mon', 'Push 1', 3)]),
  Array.from({ length: n }, (_, i) => ({ days: [day('Mon', `Push ${i + 1}`, i + 1)] })));
...
{
  const d = trainingOnDay(one([day('Mon', 'Push', 3)]), null, 'the 7th', 'ready', 'Ana');
  ...
  eq(dayTrainingCaveat(d), null, 'and needs no caveat, because its own line already says it');
}
...

{
  ...
  const patchy = withWeeks(one([day('Mon', 'Push 1', 3)]), [
    { days: [day('Mon', 'Push 1', 3)] },
    { days: [] },
  ]);
  const d = trainingOnDay(patchy, W1, W2, 'ready', 'Ana');
...
{
  const d = trainingOnDay(one([day('Mon', 'Push', 3, '10 min bike')]), null, W1, 'ready', 'Ana');
  ...
  ok(d.line.includes('Push') && d.line.includes('3 exercises') && d.line.includes('10 min bike'),
    'the one line carries what it is, how much of it there is, and the cardio');
  eq(dayPlanHeading(d), 'Planned for this day',
    'and the heading names the day rather than implying today');
}

{
  // A coach who wrote six exercises and no heading has still written a session.
  const d = trainingOnDay(one([day('Mon', '   ', 2)]), null, W1, 'ready', 'Ana');
...
{
  const d = trainingOnDay(one([day('Mon', 'Push', 1)]), null, W1, 'ready', 'Ana');
...

{
  ...
  const d = trainingOnDay(one([day('Mon', 'Conditioning', 0, '20 min row')]), null, W1, 'ready', 'Ana');
  ...
  const bare = trainingOnDay(one([day('Mon', 'Push', 0)]), null, W1, 'ready', 'Ana');
...
{
  const p = block(8);
  const w1 = trainingOnDay(p, W1, W1, 'ready', 'Ana');
  const w2 = trainingOnDay(p, W1, W2, 'ready', 'Ana');
  const w5 = trainingOnDay(p, W1, W5, 'ready', 'Ana');
...

{
  ...
  const p = block(4);
  const d = trainingOnDay(p, null, W5, 'ready', 'Ana');
...

{
  ...
  const p = block(2);
  const d = trainingOnDay(p, '2026-01-05', W1, 'ready', 'Ana');
...

{
  ...
  const caveat = dayTrainingCaveat(stale);
  ok(caveat !== null && /last plan this phone had/i.test(caveat),
    'and the caveat is its own sentence, about the connection rather than the training');
  eq(dayTrainingCaveat(good), null, 'with nothing to add under a whole read');
  eq(dayPlanUnread(stale), true, 'an unconfirmed plan carries the mark');
  eq(dayPlanUnread(good), false, 'and a confirmed one does not');
}
...
{
  const all: DayTraining[] = [
    trainingOnDay(one([day('Mon', 'Push', 3)]), null, 'the 7th', 'ready', 'Ana'),
    trainingOnDay(null, null, W1, 'error', 'Ana'),
    trainingOnDay(null, null, W1, 'ready', 'Ana'),
    trainingOnDay(one([]), null, W1, 'ready', 'Ana'),
    trainingOnDay(one([day('Mon', 'Push', 3)]), null, '2026-09-08', 'ready', 'Ana'),
    trainingOnDay(one([day('Mon', 'Push', 3)]), null, W1, 'ready', 'Ana'),
  ];
  eq(all.map((d) => d.state),
    ['undated', 'unreadable', 'unassigned', 'unwritten', 'rest', 'session'],
    'all six states are reachable');
  for (const d of all) {
    ...
    ok(dayPlanHeading(d).trim().length > 0, `${d.state} has a heading`);
  }
  ...
  eq(new Set(nothing).size, nothing.length,
    'in particular the four ways of having no session are four different sentences');
}
