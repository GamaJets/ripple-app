    ...
    if (this.state.error) {
      return (
        <View style={{ flex: 1, backgroundColor: '#0d0d0d', alignItems: 'center', justifyContent: 'center', padding: 28 }}>
          <Text style={{ fontSize: 44, marginBottom: 12 }} accessibilityElementsHidden>🛠️</Text>
          ...
          {__DEV__ && this.state.error ? (
            <ScrollView style={{ maxHeight: 170, marginTop: 22, alignSelf: 'stretch' }}>
              <Text style={{ color: '#e66767', fontSize: 11, fontFamily: 'Courier' }}>{String(this.state.error.stack || this.state.error.message)}</Text>
            </ScrollView>
          ) : null}
        </View>
      );
