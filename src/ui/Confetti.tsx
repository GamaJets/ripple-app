  ...
  return (
    <View pointerEvents="none" style={{ position: 'absolute', left: 0, top: 0, right: 0, bottom: 0, zIndex: 999 }}>
      {pieces.map((p, i) => {
        ...
        return (
          // rtl-ok: a falling piece at a random x, drifting by translateX.
          // `start` would measure the x from whichever edge the reader starts
          // at while translateX kept pushing the same way regardless — RN does
          // not mirror a transform — so half the confetti would drift off the
          // screen it was thrown onto. There is no side for a random particle
          // to be on the wrong one of, so it stays physical and self-consistent.
          <Animated.View
            key={i}
            // rtl-ok: as above — physical `left`, paired with an unmirrored
            // translateX, is the self-consistent version.
