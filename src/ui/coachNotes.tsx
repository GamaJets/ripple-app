...
const seed: Record<string, Note[]> = {
  c1: [{ id: 'n0', at: new Date(Date.now() - 4 * 86400000).toISOString(), body: 'Right knee twinge on squats — keep an eye, regress to box squats if it flares.' }],
};
...
export function CoachNotesProvider({ children }: { children: ReactNode }) {
  const [map, setMap] = useState<Record<string, Note[]>>(() => JSON.parse(JSON.stringify(seed)));
