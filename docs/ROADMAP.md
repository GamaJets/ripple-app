...
| 4 | **Multi-site roll-up and drill-down** (Phase 6). **Half built, 11 Sep 2026 — the last line below was wrong when written.** | "No cross-site aggregate exists anywhere" was false: `src/lib/siteRollUp.ts` is 629 tested lines of exactly that arithmetic — what a total may be made of, currencies that cannot be added, zones whose "this month" are different months, and the four distinct reasons a gym contributed nothing. It had no caller because nothing could supply it figures. `supabase/parts/2614` now does: `owner_site_figures()` returns one row per owned gym — name, currency, timezone, active members, coaches, money taken — AGGREGATES ONLY, so it widens what an owner may count about their second gym and never what they may see in it. Verified on the live project against a temporary fixture: two sites for the recorded owner, one for a different owner, none for a member. **Still to do: the console screen, and drill-down** (switching which site the session is scoped to). |
...
open in the ordinary sense — **5 is struck: it was never open**, and **4 is half
built** as of 11 Sep 2026; **10** is a person in a Stripe dashboard; **11** is
