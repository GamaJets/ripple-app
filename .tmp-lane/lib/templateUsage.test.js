...
const templateUsage_1 = require("./templateUsage");
const errors = [];
...
const prog = (title, ...days) => ({ title, focus: [], note: '', days });
...
const tpl = (id, name, program) => ({ id, name, program });
/* ── nothing is counted over a read that did not come back whole ─────────── */
{
    ...
    for (const status of ['loading', 'partial', 'error']) {
        const u = (0, templateUsage_1.templateUsage)(library, book, status);
    ...
    const loading = String((0, templateUsage_1.templateUsage)(library, book, 'loading').withheld);
    const failed = String((0, templateUsage_1.templateUsage)(library, book, 'error').withheld);
    const part = String((0, templateUsage_1.templateUsage)(library, book, 'partial').withheld);
    ...
    eq((0, templateUsage_1.templateUsage)(library, book, 'ready').withheld, null, 'a whole read withholds nothing');
    eq((0, templateUsage_1.templateUsage)(library, book, 'ready').byId.t1.on.length, 3, 'a whole read counts everybody on it');
}
/* ── who is on it ───────────────────────────────────────────────────────── */
{
    const library = [tpl('t1', 'Push · Pull · Legs', PPL), tpl('t2', 'Fat-loss Circuit', OTHER)];
    const book = { ana: PPL, ben: PPL, cal: EDITED, dee: OTHER };
    const u = (0, templateUsage_1.templateUsage)(library, book, 'ready');
...
{
    const u = (0, templateUsage_1.templateUsage)([tpl('t1', 'Unused', prog('Unused', day('Fri', 'Deadlift')))], { ana: PPL }, 'ready');
...
/* ── the edge cases that would otherwise invent a match ─────────────────── */
{
    ...
    const u = (0, templateUsage_1.templateUsage)([tpl('t1', 'No name', blank)], { ana: blankOther }, 'ready');
    ...
    const withNull = (0, templateUsage_1.templateUsage)([tpl('t1', 'PPL', PPL)], { ana: null, ben: PPL }, 'ready');
    ...
    eq(Object.keys((0, templateUsage_1.templateUsage)([], {}, 'ready').byId).length, 0, 'an empty library has no rows');
    eq((0, templateUsage_1.templateUsage)([tpl('t1', 'PPL', PPL)], {}, 'ready').byId.t1.line, null, 'an empty book leaves every row silent');
    eq((0, templateUsage_1.templateUsage)(null, null, 'ready').withheld, null, 'a missing library is answered rather than thrown at');
    ...
    const all = (0, templateUsage_1.templateUsage)([tpl('t1', 'PPL', PPL), tpl('t2', 'Other', OTHER)], { ana: PPL }, 'error');
...
/* ── weeks two onward are part of the fingerprint ───────────────────────── */
{
    ...
    const sixWeek = { ...oneWeek, weeks: [{ days: oneWeek.days }, { days: oneWeek.days }] };
    const u = (0, templateUsage_1.templateUsage)([tpl('t1', 'Block', sixWeek)], { ana: oneWeek }, 'ready');
