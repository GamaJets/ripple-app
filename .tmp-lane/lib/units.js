...
exports.lengthUnitLabel = exports.weightUnitLabel = exports.inToCm = exports.cmToIn = exports.lbToKg = exports.kgToLb = exports.CM_PER_IN = exports.KG_PER_LB = void 0;
exports.plain = plain;
exports.readNumber = readNumber;
exports.weightIn = weightIn;
exports.weightLabel = weightLabel;
exports.weightDeltaIn = weightDeltaIn;
exports.heightIn = heightIn;
exports.heightParts = heightParts;
exports.heightLabel = heightLabel;
exports.lengthIn = lengthIn;
exports.lengthLabel = lengthLabel;
exports.lengthDeltaIn = lengthDeltaIn;
exports.liftIn = liftIn;
exports.liftLabel = liftLabel;
exports.liftDeltaIn = liftDeltaIn;
exports.est1RMIn = est1RMIn;
exports.volumeIn = volumeIn;
exports.volumeHeadline = volumeHeadline;
exports.liftToKg = liftToKg;
exports.readLift = readLift;
exports.readBodyWeight = readBodyWeight;
exports.weightToKg = weightToKg;
exports.lengthToCm = lengthToCm;
exports.heightToCm = heightToCm;
exports.convertedNote = convertedNote;
/** Exact by international definition since 1959 — not an approximation. */
exports.KG_PER_LB = 0.45359237;
/** Likewise exact: the inch is defined as 25.4 mm. */
exports.CM_PER_IN = 2.54;
const IN_PER_FT = 12;
...
const kgToLb = (kg) => kg / exports.KG_PER_LB;
exports.kgToLb = kgToLb;
const lbToKg = (lb) => lb * exports.KG_PER_LB;
exports.lbToKg = lbToKg;
const cmToIn = (cm) => cm / exports.CM_PER_IN;
exports.cmToIn = cmToIn;
const inToCm = (inches) => inches * exports.CM_PER_IN;
 ...
 */
function weightIn(kg, unit) {
    if (kg == null || !Number.isFinite(kg))
        return null;
    return unit === 'lb' ? Math.round((0, exports.kgToLb)(kg)) : roundTo(kg, 1);
}
/** The same figure with its unit attached, for a line of prose. */
function weightLabel(kg, unit) {
 ...
 */
function weightDeltaIn(deltaKg, unit) {
    if (deltaKg == null || !Number.isFinite(deltaKg))
        return null;
    return unit === 'lb' ? Math.round((0, exports.kgToLb)(deltaKg)) : roundTo(deltaKg, 1);
}
/**
 ...
 */
function heightIn(cm, unit) {
    if (cm == null || !Number.isFinite(cm))
        return null;
    return unit === 'in' ? Math.round((0, exports.cmToIn)(cm)) : Math.round(cm);
}
 ...
 */
function lengthIn(cm, unit) {
    if (cm == null || !Number.isFinite(cm))
        return null;
    return roundTo(unit === 'in' ? (0, exports.cmToIn)(cm) : cm, 1);
}
/** A tape measurement with its unit attached. */
function lengthLabel(cm, unit) {
 ...
 */
function lengthDeltaIn(deltaCm, unit) {
    if (deltaCm == null || !Number.isFinite(deltaCm))
        return null;
    return roundTo(unit === 'in' ? (0, exports.cmToIn)(deltaCm) : deltaCm, 1);
}
 ...
 */
const LIFT_MAX = { kg: 600, lb: 1300 };
const roundToStep = (n, step) => Math.round((n + Number.EPSILON) / step) * step;
/**
 ...
 */
function liftIn(kg, unit) {
    if (kg == null || !Number.isFinite(kg))
        return null;
    return unit === 'lb' ? roundToStep((0, exports.kgToLb)(kg), LB_LIFT_STEP) : roundTo(kg, LIFT_STORED_DP);
}
/** A lifted load with its unit attached: `60 kg`, `137.5 lb`. */
function liftLabel(kg, unit) {
 ...
 */
function liftDeltaIn(deltaKg, unit) {
    if (deltaKg == null || !Number.isFinite(deltaKg))
        return null;
    return unit === 'lb' ? roundToStep((0, exports.kgToLb)(deltaKg), LB_LIFT_STEP) : roundTo(deltaKg, LIFT_STORED_DP);
}
/**
 ...
 */
function est1RMIn(kg, unit) {
    if (kg == null || !Number.isFinite(kg))
        return null;
    return Math.round(unit === 'lb' ? (0, exports.kgToLb)(kg) : kg);
}
/**
 ...
 */
function volumeHeadline(kg, unit) {
    if (kg == null || !Number.isFinite(kg))
        return null;
    ...
    if (unit === 'kg')
        return { figure: Math.round(kg / 100) / 10, unit: 't' };
    return { figure: Math.round((0, exports.kgToLb)(kg)), unit: 'lb' };
}
 ...
 */
function liftToKg(text, unit) {
    const n = parse(text);
    if (n == null)
        return null;
    return roundTo(unit === 'lb' ? (0, exports.lbToKg)(n) : n, LIFT_STORED_DP);
}
/**
 ...
 */
function readLift(text, unit) {
    if (text == null || String(text).trim() === '')
        return { ok: true, kg: null };
    const n = parse(text);
    if (n == null)
        return { ok: false, reason: 'That load is not a number. Leave it empty for a bodyweight set.' };
    if (n < 0)
        return { ok: false, reason: 'A load cannot be negative.' };
    if (n > LIFT_MAX[unit]) {
        return { ok: false, reason: `${plain(n)} ${unit} is heavier than anyone has lifted — check that figure.` };
    }
    return { ok: true, kg: roundTo(unit === 'lb' ? (0, exports.lbToKg)(n) : n, LIFT_STORED_DP) };
}
/**
 ...
 */
const BODY_MIN = { kg: 20, lb: 44 };
const BODY_MAX = { kg: 400, lb: 880 };
/**
 ...
 */
function readBodyWeight(text, unit) {
    if (text == null || String(text).trim() === '')
        return { ok: true, kg: null };
    const n = parse(text);
    if (n == null)
        return { ok: false, reason: 'That weight is not a number.' };
    ...
    return { ok: true, kg: roundTo(unit === 'lb' ? (0, exports.lbToKg)(n) : n, STORED_DP) };
}
/**
 ...
 */
function weightToKg(text, unit) {
    const n = parse(text);
    if (n == null)
        return null;
    return roundTo(unit === 'lb' ? (0, exports.lbToKg)(n) : n, STORED_DP);
}
/** A tape measurement the client typed, as centimetres to store. */
function lengthToCm(text, unit) {
    const n = parse(text);
    if (n == null)
        return null;
    return roundTo(unit === 'in' ? (0, exports.inToCm)(n) : n, STORED_DP);
}
/**
 ...
 */
function heightToCm(primary, unit, inches) {
    if (unit === 'cm') {
    ...
    if (ft == null && inch == null)
        return null;
    return roundTo((0, exports.inToCm)((ft ?? 0) * IN_PER_FT + (inch ?? 0)), STORED_DP);
}
// ── the words for it ───────────────────────────────────────────────────────
/** Suffix for a bare figure elsewhere on screen (a <Hero>'s unit, say). */
const weightUnitLabel = (unit) => unit;
exports.weightUnitLabel = weightUnitLabel;
const lengthUnitLabel = (unit) => unit;
exports.lengthUnitLabel = lengthUnitLabel;
/**
