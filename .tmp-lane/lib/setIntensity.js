...
exports.PCT_MAX = exports.PCT_MIN = exports.RPE_MAX = exports.RPE_MIN = void 0;
exports.readRpe = readRpe;
exports.rpeLabel = rpeLabel;
exports.rpeMeaning = rpeMeaning;
exports.readPercent1RM = readPercent1RM;
exports.percentLabel = percentLabel;
exports.percentLoadKg = percentLoadKg;
exports.readTempo = readTempo;
exports.tempoPhases = tempoPhases;
exports.tempoMeaning = tempoMeaning;
exports.intensityOf = intensityOf;
exports.intensityLine = intensityLine;
exports.intensityMeaning = intensityMeaning;
/* ── RPE ──────────────────────────────────────────────────────────────────── */
/**
 ...
 */
function readRpe(text) {
    const s = String(text ?? '').trim().replace(',', '.');
    if (!s)
        return { ok: false, why: 'Nothing was typed, so there is no effort target to save.' };
    if (!/^\d{1,2}(\.\d)?$/.test(s)) {
        return { ok: false, why: 'An effort target is a number on the RPE scale, like 8 or 8.5.' };
    }
    const n = Number(s);
    if (!Number.isFinite(n))
        return { ok: false, why: 'That is not a number this app can read.' };
    if (n < exports.RPE_MIN || n > exports.RPE_MAX) {
        return { ok: false, why: `RPE runs from ${exports.RPE_MIN} to ${exports.RPE_MAX} on a written plan. Anything easier than ${exports.RPE_MIN} is a warm-up, and the set method already says that.` };
    }
 ...
 */
function rpeMeaning(rpe) {
    if (typeof rpe !== 'number' || !Number.isFinite(rpe))
        return null;
    const left = exports.RPE_MAX - rpe;
    if (left <= 0)
        return 'no further rep was possible';
    if (left < 1)
        return 'about half a rep left';
    if (left === 1)
        return 'one rep left';
 ...
 */
function readPercent1RM(text) {
    const s = String(text ?? '').trim().replace(/%\s*$/, '').trim();
    if (!s)
        return { ok: false, why: 'Nothing was typed, so there is no percentage to save.' };
    if (!/^\d{1,3}$/.test(s)) {
        return { ok: false, why: 'A share of a one-rep max is a whole percentage, like 75.' };
    }
    const n = Number(s);
    if (n < exports.PCT_MIN || n > exports.PCT_MAX) {
        return { ok: false, why: `Write a share between ${exports.PCT_MIN}% and ${exports.PCT_MAX}%. Lighter than that is a warm-up, and the set method already says so.` };
    }
 ...
 */
function percentLoadKg(pct, oneRepMaxKg) {
    if (typeof pct !== 'number' || !Number.isFinite(pct))
        return null;
    if (typeof oneRepMaxKg !== 'number' || !Number.isFinite(oneRepMaxKg) || oneRepMaxKg <= 0)
        return null;
 ...
 */
function readTempo(text) {
    const raw = String(text ?? '').trim().toUpperCase();
    if (!raw)
        return { ok: false, why: 'Nothing was typed, so there is no tempo to save.' };
 ...
 * on purpose.
 */
function intensityOf(ex, row) {
    const pick = (k) => (own(row, k) ? row[k] : (ex ? ex[k] : null)) ?? null;
    return {
        rpe: pick('rpe') ?? null,
        pct1rm: pick('pct1rm') ?? null,
        tempo: pick('tempo') ?? null,
    };
}
/**
 ...
 * carry these fields.
 */
function intensityLine(i) {
    const bits = [rpeLabel(i.rpe), percentLabel(i.pct1rm), i.tempo].filter((x) => !!x);
 ...
 */
function intensityMeaning(i) {
    const out = [];
    const rpe = rpeMeaning(i.rpe);
    if (i.rpe != null && rpe)
        out.push(`RPE ${i.rpe} means ${rpe}.`);
    if (i.pct1rm != null) {
        out.push(`${i.pct1rm}% is the share of a one rep max your coach wrote. It stays a percentage here: `
    ...
    if (tempo)
        out.push(`Tempo ${i.tempo} is ${tempo}.`);
    return out;
}
