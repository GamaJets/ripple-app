...
const blockPlan_1 = require("./blockPlan");
const programBlock_1 = require("./programBlock");
const errors = [];
const ok = (cond, msg) => { if (!cond)
    errors.push(msg); };
const eq = (a, b, msg) => ok(Object.is(a, b), `${msg} — got ${JSON.stringify(a)}, wanted ${JSON.stringify(b)}`);
const ex = (name, sets = 3, reps = '8-10') => ({ name, sets, reps });
const day = (d, ...names) => ({ day: d, focus: 'Upper', exercises: names.map((n) => ex(n)) });
const week = (...ds) => ({ days: ds });
/* ── the overview ───────────────────────────────────────────────────────── */
{
    ...
    const w3 = { days: [day('Mon', 'Bench Press')], deload: true, label: 'Easy week' };
    const rows = (0, blockPlan_1.blockOverview)([w1, w2, w3, { days: [] }]);
    ...
    const twoHoles = (0, blockPlan_1.blockOverview)([w1, { days: [] }, { days: [] }]);
    ...
    const named = (0, blockPlan_1.blockOverview)([w1, { ...w2, deload: true, label: 'Deload' }]);
    ...
    const sameSets = (0, blockPlan_1.blockOverview)([w1, { days: [{ ...w1.days[0], focus: 'Push' }, w1.days[1]] }]);
    eq(sameSets[1].sameAsPrevious, false, 'a changed focus is a changed day');
    eq((0, blockPlan_1.blockOverview)([]).length, 0, 'an empty block has no lines');
    eq((0, blockPlan_1.blockOverview)(null).length, 0, 'a missing block has no lines rather than throwing');
}
/* ── the one warning, and the silence around it ─────────────────────────── */
{
    const full = week(day('Mon', 'Bench Press'));
    eq((0, blockPlan_1.blockWarnings)([{ days: [] }]).length, 0, 'an unstarted one-week programme is not complained about');
    eq((0, blockPlan_1.blockWarnings)([full]).length, 0, 'a one-week programme is never warned about');
    eq((0, blockPlan_1.blockWarnings)([full, full]).length, 0, 'a block with training in every week is not warned about');
    const one = (0, blockPlan_1.blockWarnings)([full, { days: [] }]);
    ...
    const two = (0, blockPlan_1.blockWarnings)([full, { days: [] }, full, { days: [] }]);
...
{
    const ws = ['a', 'b', 'c', 'd'];
    const later = (0, blockPlan_1.moveWeek)(ws, 1, 2);
    ...
    const earlier = (0, blockPlan_1.moveWeek)(ws, 3, 0);
    ...
    const outOfOne = (0, blockPlan_1.moveWeek)(ws, 0, 2);
    ...
    eq((0, blockPlan_1.moveWeek)(ws, 1, 1), null, 'a move to the same position does nothing and says so');
    eq((0, blockPlan_1.moveWeek)(ws, -1, 2), null, 'a negative source is refused');
    eq((0, blockPlan_1.moveWeek)(ws, 1, 4), null, 'a destination past the end is refused rather than clamped');
    eq((0, blockPlan_1.moveWeek)(ws, 4, 1), null, 'a source past the end is refused');
    eq((0, blockPlan_1.moveWeek)(['a'], 0, 0), null, 'a one-week block cannot be reordered');
    eq((0, blockPlan_1.moveWeek)(ws, 1.5, 2), null, 'a fractional index is refused rather than floored');
}
/* ── duplicating a week ─────────────────────────────────────────────────── */
{
    const ws = [{ id: 'a' }, { id: 'b' }, { id: 'c' }];
    let cloned = 0;
    const clone = (w) => { cloned += 1; return { id: w.id + '-copy' }; };
    const dup = (0, blockPlan_1.duplicateWeek)(ws, 1, clone, programBlock_1.MAX_WEEKS);
    ...
    const first = (0, blockPlan_1.duplicateWeek)(ws, 0, clone, programBlock_1.MAX_WEEKS);
    ...
    const atCeiling = Array.from({ length: programBlock_1.MAX_WEEKS }, (_, i) => ({ id: String(i) }));
    eq((0, blockPlan_1.duplicateWeek)(atCeiling, 0, clone, programBlock_1.MAX_WEEKS), null, 'a block at the ceiling refuses rather than dropping a week');
    eq((0, blockPlan_1.duplicateWeek)(atCeiling, 0, clone, programBlock_1.MAX_WEEKS + 1)?.weeks.length, programBlock_1.MAX_WEEKS + 1, 'the ceiling is the caller’s, not a constant in here');
    eq((0, blockPlan_1.duplicateWeek)(ws, 3, clone, programBlock_1.MAX_WEEKS), null, 'duplicating a week that is not there is refused');
    eq((0, blockPlan_1.duplicateWeek)([], 0, clone, programBlock_1.MAX_WEEKS), null, 'there is nothing to duplicate in an empty block');
}
/* ── what the coach is told before it lands ─────────────────────────────── */
{
    eq((0, blockPlan_1.weekEditWarning)(null), null, 'no edit is no warning');
    eq((0, blockPlan_1.weekEditWarning)({ weeks: [], index: 0, movedWeekOne: false }), null, 'an edit that leaves week one alone is silent');
    const line = (0, blockPlan_1.weekEditWarning)({ weeks: [], index: 0, movedWeekOne: true });
