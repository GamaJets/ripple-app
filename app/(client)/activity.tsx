...
export default function Activity() {
  ...
  for (const e of log) {
    ...
    if (e.sets) {
      events.push({ at: e.t, icon: pr ? '🏆' : '🏋️', title: pr ? `New PR — ${e.exercise}` : `Logged ${e.exercise}`, sub: e.sets.map((s) => `${s[0]}×${s[1]}kg`).join(' · ') });
    } else if (e.cardio) {
      events.push({ at: e.t, icon: '🏃', title: `Logged ${e.exercise}`, sub: `${e.cardio.mins} min · ${e.cardio.dist} ${e.cardio.unit}` });
    }
  }
  // Streak milestone (as of now)
  const streak = currentStreak(log);
  const milestone = streakMilestone(streak);
  if (milestone) events.push({ at: new Date().toISOString(), icon: '🔥', title: 'Streak milestone', sub: milestone });
  // Check-ins
  for (const c of checkins) events.push({ at: c.at, icon: '📝', title: 'Weekly check-in sent', sub: `${c.weightKg} kg · energy ${c.energy}/5 · sleep ${c.sleep}/5` });
  // My sessions
  for (const s of sessions) {
    if (s.status === 'booked' && s.clientId === CLIENT_ID) events.push({ at: s.startsAt, icon: '📅', title: 'Session booked', sub: `${timeLabel(s.startsAt)} · ${s.durationMin} min` });
  }
  // Coach messages
  for (const m of MOCK_MESSAGES) if (m.sender === 'coach') events.push({ at: m.createdAt, icon: '💬', title: 'Message from your coach', sub: m.body });
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: t.bg }} edges={['top']}>
      <ScrollView contentContainerStyle={{ padding: 18, paddingBottom: 40 }}>
        ...
        <Text style={{ color: t.ink, fontSize: 24, fontWeight: '800' }}>Activity</Text>
        ...
        {feed.length === 0 ? (
          ...
          <View key={i} style={{ flexDirection: 'row', gap: 14, backgroundColor: t.surface, borderRadius: 14, borderWidth: 1, borderColor: t.ring, padding: 14, marginBottom: 9 }}>
            <View style={{ width: 42, height: 42, borderRadius: 12, backgroundColor: t.surface2, alignItems: 'center', justifyContent: 'center' }}><Text style={{ fontSize: 20 }}>{e.icon}</Text></View>
            <View style={{ flex: 1 }}>
