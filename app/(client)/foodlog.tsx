    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: t.bg }} edges={['top']}>
      <ScrollView contentContainerStyle={{ padding: 18, paddingBottom: 40 }}>
        ...
        <View style={{ flexDirection: 'row', gap: 10, marginBottom: 12 }}>
          ...
          <Pressable onPress={() => takeMealPhoto(true)} style={{ flex: 1, backgroundColor: t.brand, borderRadius: 12, paddingVertical: 14, alignItems: 'center', gap: 4 }}><Text style={{ fontSize: 20 }}>🍽️</Text><Text style={{ color: t.brandInk, fontWeight: '800', fontSize: 12 }}>Photo A Meal</Text></Pressable>
          <Pressable onPress={() => takeMealPhoto(false)} style={{ flex: 1, backgroundColor: t.surface2, borderColor: t.ring, borderWidth: 1, borderRadius: 12, paddingVertical: 14, alignItems: 'center', gap: 4 }}><Text style={{ fontSize: 20 }}>🖼️</Text><Text style={{ color: t.ink, fontWeight: '700', fontSize: 12 }}>Upload</Text></Pressable>
        </View>
        ...
        {log.length === 0 ? <Text style={{ color: t.ink3, fontSize: 13 }}>Nothing logged yet today.</Text> : log.map((f, i) => (
          <View key={i} style={{ backgroundColor: t.surface, borderColor: t.ring, borderWidth: 1, borderRadius: 12, padding: 13, marginBottom: 6, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
            <View style={{ flex: 1 }}>
              <Text style={{ color: t.ink, fontSize: 14, fontWeight: '600' }}>{f.via === 'photo' ? '🍽️ ' : f.via === 'barcode' ? '📷 ' : ''}{f.n}</Text>
