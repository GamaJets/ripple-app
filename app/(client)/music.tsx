...
const MODES: { id: GenParams['mode']; label: string; ico: string }[] = [
  { id: 'strength', label: 'Strength', ico: '🏋️' },
      ...
      <ScrollView contentContainerStyle={{ padding: 18, paddingBottom: 40 }}>
        ...
        {pl ? (
          <View style={{ backgroundColor: t.surface, borderRadius: 18, borderWidth: 1, borderColor: t.ring, padding: 18 }}>
            ...
            {pl.tracks.map((tr, i) => (
              <View key={i} style={{ flexDirection: 'row', alignItems: 'center', gap: 12, paddingVertical: 9, borderTopWidth: i === 0 ? 0 : 1, borderTopColor: t.ring }}>
            ...
            <View style={{ flexDirection: 'row', gap: 10, marginTop: 16 }}>
              <Pressable onPress={push} style={{ flex: 1, backgroundColor: t.brand, borderRadius: 12, paddingVertical: 13, alignItems: 'center' }}>
                <Text style={{ color: t.brandInk, fontWeight: '800', fontSize: 14 }}>{anyConnected ? 'Save to my music' : 'Connect to save'}</Text>
              </Pressable>
