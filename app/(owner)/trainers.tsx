      ...
      <ScrollView contentContainerStyle={{ padding: 18, paddingBottom: 40 }}>
        ...
        {TRAINERS.map((tr) => (
          <View key={tr.id} style={{ backgroundColor: t.surface, borderRadius: 16, borderWidth: 1, borderColor: t.ring, padding: 15, marginBottom: 10, flexDirection: 'row', alignItems: 'center', gap: 12 }}>
            ...
            <View style={{ flex: 1 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}><Text style={{ color: t.ink, fontWeight: '700', fontSize: 15, textTransform: 'capitalize' }}>{tr.name}</Text>{tr.status === 'trial' && <View style={{ backgroundColor: 'rgba(250,178,25,0.18)', borderRadius: 10, paddingHorizontal: 7, paddingVertical: 2 }}><Text style={{ color: t.warn, fontSize: 10, fontWeight: '800' }}>TRIAL</Text></View>}</View>
