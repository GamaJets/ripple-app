...
Deno.serve(async (req) => {
  ...
  if (action === 'cancel' || action === 'resume' || action === 'portal') {
    ...
    // Scoped to the caller's own row. RLS would refuse someone else's read, but
    // this runs as the service role, which RLS does not apply to — so the
    // ownership check that RLS would have made has to be made here, explicitly,
    // or any signed-in user could cancel any subscription id they can guess.
    const { data: row, error: readErr } = await service.from('client_subscriptions')
      .select('stripe_subscription_id, stripe_customer_id, client_id').eq('stripe_subscription_id', subId).eq('client_id', clientId).maybeSingle();
    ...
    const { error: wErr } = await service.from('client_subscriptions').update({
  ...
  if (interval) {
    ...
    try {
      const session = await stripe.checkout.sessions.create({
        ...
        metadata: { repple_kind: 'connect_subscription', package_id: packageId, trainer_id: pkg.trainer_id, client_id: clientId },
      });
