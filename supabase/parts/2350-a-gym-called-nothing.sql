-- ═══════════════════════════════════════════════════════════════════════════
-- APPLIED, and RENUMBERED from 153 on arrival. Read this before the rest.
--
-- This part was written and applied in a separate worktree that was 43 commits
-- behind main, where the highest part was 140. It was numbered 153 there. main
-- already has supabase/parts/153-a-coach-is-a-brand-too.sql, and in this repo
-- the number IS the apply order — two parts sharing a prefix is a dependency
-- ambiguity, not a filename inconvenience. It is 2350 here because that is what
-- was actually free; the author deliberately did not pick a number they could
-- not see, which was the right call.
--
-- Only the PART FILE was taken across. That worktree's commit also carried a
-- setup.sql rebuilt from its 140 parts; merging that would have replaced the
-- bundle built from main's 232 with a regression. setup.sql here is rebuilt by
-- db:build from the parts in this tree, as always.
--
-- Applied state verified independently against the live database before landing
-- this file, not taken on report: handle_new_user carries
-- nullif(trim(...->>'full_name'), ''), provision_profile carries
-- coalesce(nullif(trim(new.full_name), ''), 'My'), the old empty-string coalesce
-- is gone from both, and neither function is executable by anon or by
-- authenticated.
--
-- Ordering is safe at 2350 despite the change being live already: only parts 06
-- and 101 ever create-or-replace provision_profile, and only 01 and 07
-- handle_new_user. Nothing between 140 and 2340 redefines either, so this file
-- is the last word on both whether the bundle is applied from scratch or not.
-- ═══════════════════════════════════════════════════════════════════════════
