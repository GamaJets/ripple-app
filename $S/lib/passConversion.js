...
exports.MIN_COHORT_FOR_RATE = exports.summarisePasses = exports.passStatus = exports.isExpired = exports.remainingUses = exports.MONEY_NOTE = exports.CAUSAL_CAVEAT = exports.CONVERSION_COST = exports.CONVERSION_LABEL = exports.CONVERSION_PARTS = void 0;
exports.brokenConversionParts = brokenConversionParts;
exports.pendingConversionParts = pendingConversionParts;
exports.conversionWarning = conversionWarning;
exports.buildPassConversion = buildPassConversion;
exports.buildHolders = buildHolders;
exports.countHolders = countHolders;
exports.intervalOf = intervalOf;
exports.hostsOf = hostsOf;
exports.moneyOf = moneyOf;
exports.attributionSentence = attributionSentence;
exports.undecidedSentence = undecidedSentence;
exports.suppressionSentence = suppressionSentence;
exports.dateOf = dateOf;
exports.daysBetween = daysBetween;
exports.coversDate = coversDate;
const gymPasses_1 = require("./gymPasses");
Object.defineProperty(exports, "remainingUses", { enumerable: true, get: function () { return gymPasses_1.remainingUses; } });
Object.defineProperty(exports, "isExpired", { enumerable: true, get: function () { return gymPasses_1.isExpired; } });
Object.defineProperty(exports, "passStatus", { enumerable: true, get: function () { return gymPasses_1.passStatus; } });
Object.defineProperty(exports, "summarisePasses", { enumerable: true, get: function () { return gymPasses_1.summarisePasses; } });
...
Object.defineProperty(exports, "MIN_COHORT_FOR_RATE", { enumerable: true, get: function () { return gymRetention_1.MIN_COHORT_FOR_RATE; } });
const DAY = 86400000;
exports.CONVERSION_PARTS = ['passes', 'memberships', 'visits', 'plans'];
exports.CONVERSION_LABEL = {
    passes: 'passes',
...
function brokenConversionParts(rec) {
    const out = [];
    for (const part of exports.CONVERSION_PARTS) {
        const s = rec[part];
        if (s.state === 'failed') {
            out.push({ part, label: exports.CONVERSION_LABEL[part], cost: exports.CONVERSION_COST[part], reason: s.reason });
        }
    }
    return out;
}
function pendingConversionParts(rec) {
    return exports.CONVERSION_PARTS.filter((p) => rec[p].state === 'loading');
}
/** The sentence above a half-loaded page, or null when nothing failed. */
function conversionWarning(rec) {
 ...
 */
exports.CAUSAL_CAVEAT = 'This is a sequence, not a cause. It counts people who held a pass and later took out a membership, and how long there was between the two. Nothing in these rows says the pass is why they joined — some of them would have joined anyway. Read it as "used a pass, then joined", never as "the pass converted them".';
exports.MONEY_NOTE = 'These two figures are different kinds of money and must not be added. Pass income is cash already taken, once. The membership figure is what the memberships those holders now hold are worth per month, for as long as they last — it has not been taken and may never be. A single total would book a hypothetical year of subscription as revenue.';
function buildPassConversion(rec, opts = {}) {
    const today = opts.today ?? (0, weekStart_1.isoDay)(new Date());
    const minGroup = opts.minGroup ?? gymRetention_1.MIN_COHORT_FOR_RATE;
    const passRows = (0, memberView_1.rowsOf)(rec.passes);
    const memberRows = (0, memberView_1.rowsOf)(rec.memberships);
    const visitRows = (0, memberView_1.rowsOf)(rec.visits);
    const planRows = (0, memberView_1.rowsOf)(rec.plans);
    ...
    const passes = (0, gymPasses_1.summarisePasses)(passRows, today);
    ...
    const joinedAfterRate = counts == null ? null : (0, gymRetention_1.rateOf)(counts.joinedAfter, decided, minGroup);
    const suppressed = counts == null || joinedAfterRate != null ? null
 ...
 */
function buildHolders(passes, memberships, visits, today) {
    ...
    for (const m of memberships) {
        ...
        if (list)
            ...
            memberOf.set(m.memberId, [m]);
    }
    const out = [];
    for (const [holderId, theirs] of byHolder) {
        const dates = theirs.map((p) => dateOf(p.issuedOn)).filter((d) => d != null).sort();
        ...
        const hasLivePass = theirs.some((p) => (0, gymPasses_1.passStatus)(p, today) === 'live');
        const outcome = after ? 'joined-after'
        ...
        let firstUsedOn = null;
        for (const p of theirs) {
            const at = firstUse.get(p.id);
            if (at && (firstUsedOn == null || at < firstUsedOn))
                firstUsedOn = at;
        }
        const { cents } = (0, gymPasses_1.passRevenueCents)(theirs);
        out.push({
            holderId,
            ...
            lastPassOn,
            firstUsedOn,
            hostMemberIds: [...new Set(theirs.map((p) => p.hostMemberId).filter((h) => !!h))],
            hasLivePass,
            outcome,
            joinedOn,
...
function countHolders(holders) {
    let alreadyMember = 0, joinedAfter = 0, undecided = 0, noMembership = 0;
    for (const h of holders) {
        if (h.outcome === 'already-member')
            alreadyMember++;
        else if (h.outcome === 'joined-after')
            joinedAfter++;
        else if (h.outcome === 'undecided')
            undecided++;
        else
            noMembership++;
    }
 ...
 */
function hostsOf(passes, holders, memberships) {
    const outcomeOf = new Map(holders.map((h) => [h.holderId, h.outcome]));
    const nameOfMember = new Map();
    for (const m of memberships) {
        if (m.memberId && m.memberName?.trim())
            nameOfMember.set(m.memberId, m.memberName.trim());
    }
    ...
    return (0, gymPasses_1.guestsByHost)(passes).map(({ hostMemberId, guests }) => {
        ...
        const ids = [...new Set(theirs.map((p) => p.holderId).filter((h) => !!h))];
...
function moneyOf(passes, holders, memberships, plans) {
    const { cents, priced, total } = (0, gymPasses_1.passRevenueCents)(passes);
    ...
    if (holders && memberships && plans) {
        ...
        const s = (0, gymRecord_1.summarise)([], followed, plans);
...
/* ── sentences ─────────────────────────────────────────────────────────────── */
function floorSentence(minGroup) {
    const p = (0, gymRetention_1.pointsPerMember)(minGroup);
...
function attributionSentence(issued, anonymous, counts) {
    if (anonymous <= 0)
        return null;
...
function undecidedSentence(counts) {
    if (!counts || counts.undecided <= 0)
        return null;
...
function suppressionSentence(c, minGroup = gymRetention_1.MIN_COHORT_FOR_RATE) {
    ...
    if (c.suppressed === 'too-few') {
        ...
        const p = (0, gymRetention_1.pointsPerMember)(n);
...
function nameOf(passes, ms) {
    for (const m of ms)
        if (m.memberName?.trim())
            return m.memberName.trim();
    for (const p of passes)
        if (p.holderName?.trim())
            return p.holderName.trim();
    return null;
}
function fmt(n) {
