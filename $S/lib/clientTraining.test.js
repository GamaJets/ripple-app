...
const clientTraining_1 = require("./clientTraining");
...
const e = (over) => ({ ...over });
/* ── grouping ─────────────────────────────────────────────────────────────── */
const oneSession = (0, clientTraining_1.sessionsOf)([
...
const repeated = (0, clientTraining_1.sessionsOf)([
    e({ t: T1, exercise: 'Back Squat', sets: [[5, 100]] }),
    e({ t: T1, exercise: 'Back Squat', sets: [[5, 90]] }),
]);
eq(repeated[0].exercises, 1, 'one movement done twice in a session is one exercise, not two — the count is of movements');
eq(repeated[0].sets, 2, 'while both of its sets still count');
const ordered = (0, clientTraining_1.sessionsOf)([
    e({ t: T1, exercise: 'a' }), e({ t: T3, exercise: 'c' }), e({ t: T2, exercise: 'b' }),
]);
eq(ordered.map((x) => x.at).join(','), [T3, T2, T1].join(','), 'sessions come back newest first whatever order the rows arrived in — the screen must not '
    + 'depend on an ORDER BY in a file it does not own');
/* ── nothing is invented out of a blank row ───────────────────────────────── */
const blanks = (0, clientTraining_1.sessionsOf)([e({ t: T1, exercise: 'Deadlift', sets: [[0, 100], [5, 120]] })]);
...
const noDate = (0, clientTraining_1.sessionsOf)([e({ t: 'not-a-date', exercise: 'Plank' })]);
eq(noDate[0].day, null, 'a timestamp that cannot be read gets no day — filing it under today would invent a training '
    + 'day out of a parsing failure');
eq((0, clientTraining_1.sessionsOf)([{ t: '', exercise: 'x' }]).length, 0, 'an entry with no timestamp has no session to belong to and is dropped rather than bucketed under ""');
/* ── 1. a bodyweight session has no tonnage, not a tonnage of zero ────────── */
const bodyweight = (0, clientTraining_1.sessionsOf)([
    e({ t: T1, exercise: 'Pull-up', sets: [[8, 0], [8, 0], [6, 0]] }),
    e({ t: T1, exercise: 'Press-up', sets: [[20, 0]] }),
]);
eq(bodyweight[0].volumeKg, null, 'a session with no external load has NO volume — a 0 here reads as an hour of nothing');
eq(bodyweight[0].sets, 4, 'though every one of its sets is real and counted');
eq(bodyweight[0].bodyweightSets, 4, 'and the screen is told how many carried no load, so a small tonnage cannot imply an easy hour');
const mixed = (0, clientTraining_1.sessionsOf)([
...
const flagged = (0, clientTraining_1.sessionsOf)([
...
const weightedBw = (0, clientTraining_1.sessionsOf)([
...
// than guessed.
const unpriced = (0, clientTraining_1.sessionsOf)([
    e({ t: T1, exercise: 'Pull-up', sets: [[10, 0]], bw: [true] }),
]);
...
const holds = (0, clientTraining_1.sessionsOf)([
...
const both = (0, clientTraining_1.sessionsOf)([
    e({ t: T1, exercise: 'Squat', sets: [[5, 100]] }),
...
{
    const daysOut = (0, clientTraining_1.trainingDaysOf)((0, clientTraining_1.sessionsOf)([
        e({ t: T1, exercise: 'Plank', sets: [[45, 10]], timed: [true] }),
        e({ t: T1, exercise: 'Squat', sets: [[5, 100]] }),
    ], bwHistory));
...
eq((0, clientTraining_1.sessionsOf)([e({ t: T1, exercise: 'Squat', sets: [[5, 100]] })])[0].kcal, null, 'a strength session records reps and weight, not energy — an absent burn is a dash, not a 0');
eq((0, clientTraining_1.sessionsOf)([
    e({ t: T1, exercise: 'Row', kcal: 120 }), e({ t: T1, exercise: 'Bike', kcal: 80 }),
])[0].kcal, 200, 'where entries do carry a figure they are summed');
eq((0, clientTraining_1.sessionsOf)([e({ t: T1, exercise: 'Squat', sessionMins: 0 })])[0].mins, null, 'zero minutes is an unfinished form, not a session that lasted no time');
const disagree = (0, clientTraining_1.sessionsOf)([
...
eq(disagree[0].minsDisagree, true, 'and rows that disagree are reported rather than averaged into a number nobody wrote');
/* ── 3. attribution names the coach in the room, never a guess ────────────── */
const byClient = (0, clientTraining_1.sessionsOf)([e({ t: T1, exercise: 'Squat' })])[0];
const byMe = (0, clientTraining_1.sessionsOf)([e({ t: T1, exercise: 'Squat', loggedBy: COACH })])[0];
const byThem = (0, clientTraining_1.sessionsOf)([e({ t: T1, exercise: 'Squat', loggedBy: OTHER })])[0];
const byBoth = (0, clientTraining_1.sessionsOf)([
    e({ t: T1, exercise: 'Squat', loggedBy: COACH }), e({ t: T1, exercise: 'Plank' }),
])[0];
eq((0, clientTraining_1.attributionOf)(byClient, COACH), 'client', 'no logged_by is the client doing it themselves');
eq((0, clientTraining_1.attributionOf)(byMe, COACH), 'you', 'the coach reading the screen is told they logged it');
eq((0, clientTraining_1.attributionOf)(byThem, COACH), 'coach', 'a session another coach logged is never attributed to the coach reading it');
eq((0, clientTraining_1.attributionOf)(byMe, null), 'coach', 'and neither is one read while the viewer\'s own id is still unknown — "you logged this" is a '
    + 'claim about who stood in the room');
eq((0, clientTraining_1.attributionOf)(byBoth, COACH), 'mixed', 'a session part-logged by each is neither, and says so');
ok((0, clientTraining_1.attributionLabel)('client', WHO).includes(WHO), 'the client-logged label names the client');
ok(!(0, clientTraining_1.attributionLabel)('coach', WHO).toLowerCase().includes('you'), 'and the other-coach label does not say "you"');
...
const doubleTapped = (0, clientTraining_1.sessionsOf)([
...
const tappedDays = (0, clientTraining_1.trainingDaysOf)(doubleTapped);
eq(tappedDays.days.length, 1, 'but they are ONE training day, which is the unit a coach means when they ask what somebody '
...
const undatedDay = (0, clientTraining_1.trainingDaysOf)((0, clientTraining_1.sessionsOf)([e({ t: 'not-a-date', exercise: 'Plank', sets: [[1, 0]] })]));
eq(undatedDay.days.length, 0, 'a session with no readable date belongs to no day');
eq(undatedDay.undated.length, 1, 'and is kept apart rather than dropped or filed under today — the sets in it are real');
const twoDays = (0, clientTraining_1.trainingDaysOf)((0, clientTraining_1.sessionsOf)([
    e({ t: T1, exercise: 'a', sets: [[5, 100]] }),
    e({ t: T3, exercise: 'b', sets: [[5, 50]] }),
]));
eq(twoDays.days.map((d) => d.day).join(','), [(0, entryEdit_1.dayKeyOf)(T3), (0, entryEdit_1.dayKeyOf)(T1)].join(','), 'days come back newest first');
/* ── 2. no total the read cannot support ──────────────────────────────────── */
const three = (0, clientTraining_1.sessionsOf)([
    e({ t: T3, exercise: 'Squat', sets: [[5, 100]] }),
    e({ t: T2, exercise: 'Bench', sets: [[5, 60]] }),
    e({ t: T1, exercise: 'Pull-up', sets: [[8, 0]] }),
]);
const whole = (0, clientTraining_1.trainingBoard)(three, 'ready');
...
const tapped = (0, clientTraining_1.trainingBoard)(doubleTapped, 'ready');
eq(tapped.dayCount, 1, 'the headline figure a coach reads is days, which a double tap cannot inflate');
eq(tapped.entryCount, 4, 'while the logging events are still counted, so the screen can explain a day that looks like four workouts');
const part = (0, clientTraining_1.trainingBoard)(three, 'partial');
...
eq(part.newestDay, (0, entryEdit_1.dayKeyOf)(T3), 'the newest day survives truncation — the read is ordered newest first — so its date stands');
const broken = (0, clientTraining_1.trainingBoard)(null, 'error');
eq(broken.state, 'unreadable', 'a failed read is unreadable, never empty');
eq(broken.dayCount, null, 'and produces no count');
eq((0, clientTraining_1.trainingBoard)([], 'error').state, 'unreadable', 'an empty array under error is still unreadable — the status wins, so a caller that forgets to '
    + 'pass null cannot turn a refusal into "they have never trained"');
eq((0, clientTraining_1.trainingBoard)([], 'loading').state, 'unreadable', 'and a read still in flight has produced no rows, which is not an empty history either');
const empty = (0, clientTraining_1.trainingBoard)([], 'ready');
...
eq(empty.volumeKg, null, 'while its volume is still a dash: nobody lifted nothing, there is simply nothing to total');
const noLoad = (0, clientTraining_1.trainingBoard)(bodyweight, 'ready');
eq(noLoad.volumeKg, null, 'a whole read of bodyweight sessions totals to no volume rather than to zero');
...
const undatedOnly = (0, clientTraining_1.trainingBoard)((0, clientTraining_1.sessionsOf)([e({ t: 'nope', exercise: 'x', sets: [[5, 20]] })]), 'ready');
...
eq((0, clientTraining_1.trainingLine)('loading', (0, clientTraining_1.trainingBoard)(null, 'loading'), WHO), 'Reading the sessions they have logged…', 'a read in flight reads as in flight');
ok(/could not be read/.test((0, clientTraining_1.trainingLine)('error', broken, WHO))
    && /not the same as/.test((0, clientTraining_1.trainingLine)('error', broken, WHO)), 'a failed read says so, and says what it is not');
ok(!/Nothing logged yet/.test((0, clientTraining_1.trainingLine)('error', broken, WHO)), 'and never reaches for the empty-state sentence — a failed read stating "nothing logged yet" is '
...
ok(/Nothing logged yet/.test((0, clientTraining_1.trainingLine)('ready', empty, WHO))
    && /came back empty/.test((0, clientTraining_1.trainingLine)('ready', empty, WHO)), 'an empty read that actually landed may say so, and says why it is entitled to');
ok(/^3 days logged\./.test((0, clientTraining_1.trainingLine)('ready', whole, WHO)), 'a whole read is counted in days');
ok(/^1 day logged\./.test((0, clientTraining_1.trainingLine)('ready', tapped, WHO)), 'and the double-tapped workout is one day, not four sessions — this is the line the coach reads '
    + 'before deciding whether to open anything');
ok((0, clientTraining_1.trainingLine)('ready', whole, WHO).includes((0, adherence_1.dayLabel)((0, entryEdit_1.dayKeyOf)(T3))), 'and dated with the newest day');
ok(/row limit/.test((0, clientTraining_1.trainingLine)('partial', part, WHO)) && !/\b3 day/.test((0, clientTraining_1.trainingLine)('partial', part, WHO)), 'a truncated read is hedged and never counted flat');
ok((0, clientTraining_1.trainingLine)('partial', part, WHO).includes((0, adherence_1.dayLabel)((0, entryEdit_1.dayKeyOf)(T3))), 'while still dating the newest day, which truncation cannot remove');
eq((0, clientTraining_1.trainingLine)('ready', undatedOnly, WHO), '0 days logged.', 'a client whose only session carries an unreadable date has no dated day to name — a bare count '
    + 'and no invented date, rather than a day this build made up');
/* ── whose pounds ─────────────────────────────────────────────────────────── */
const agreed = (0, clientTraining_1.unitFor)('kg', 'kg', 'ready', WHO);
...
const differ = (0, clientTraining_1.unitFor)('lb', 'kg', 'ready', WHO);
eq(differ.unit, 'lb', 'the client\'s unit wins even against the coach\'s — this screen is a transcript of what the '
...
ok(differ.note != null && differ.note.includes('lb') && differ.note.includes('kg'), 'and both units are named, so neither figure is ever unlabelled');
const unchosen = (0, clientTraining_1.unitFor)(null, 'lb', 'ready', WHO);
...
ok(unchosen.note != null && /has not set a unit/.test(unchosen.note), 'with the reason, because their phone may still be showing them the other one');
eq((0, clientTraining_1.unitFor)('stones', 'kg', 'ready', WHO).source, 'you', 'a value the app does not recognise is treated as unset rather than printed as a unit');
const unread = (0, clientTraining_1.unitFor)(null, 'kg', 'error', WHO);
eq(unread.source, 'you', 'a failed read is not a client who chose kilograms');
ok(unread.note != null && /could not be read/.test(unread.note), 'and says so — the note is about the read, not about their preference');
ok(((0, clientTraining_1.unitFor)(null, 'kg', 'loading', WHO).note ?? '').includes('Reading'), 'a read still in flight says it is still asking rather than implying it has been told');
