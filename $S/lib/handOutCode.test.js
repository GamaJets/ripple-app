...
const handOutCode_1 = require("./handOutCode");
 ...
 *  not looked at. */
const wordsOf = (g) => g.give ? `${g.hand.code} ${g.hand.link} ${g.hand.message}` : `${g.head} ${g.note}`;
...
const failed = (0, handOutCode_1.codeToGive)({ status: 'error', reason: 'Your coaching code could not be read, so there is nothing to give out yet.' });
const empty = (0, handOutCode_1.codeToGive)({ status: 'ready', code: '' });
const waiting = (0, handOutCode_1.codeToGive)({ status: 'loading' });
for (const [name, g] of [['failed', failed], ['empty', empty], ['loading', waiting]]) {
...
ok(/still have one/i.test(handOutCode_1.UNREAD_NOTE), 'the note says the code still exists');
ok(/not changed|has not changed/i.test(handOutCode_1.UNREAD_NOTE), 'and that it has not changed');
ok(/already given out still works|already given out/i.test(handOutCode_1.UNREAD_NOTE), 'and that what is already out there still works');
ok(/stops the old one working/i.test(handOutCode_1.UNREAD_NOTE), 'and says outright what pressing New Code would cost');
ok(handOutCode_1.UNREAD_NOTE.includes(' '), 'and it is prose, not a code');
...
eq((0, handOutCode_1.keptReason)('Your coaching code could not be read, so there is nothing to give out yet.'), null, 'a reason that only claims absence is dropped whole');
eq((0, handOutCode_1.keptReason)('Your coaching code came back empty, so there is nothing to give out yet.'), null, 'and so is the empty one');
...
eq((0, handOutCode_1.keptReason)('Sign in to Repple to get your coaching code.'), 'Sign in to Repple to get your coaching code.', 'an actionable reason is kept as it was written');
...
eq((0, handOutCode_1.keptReason)('Sign in to Repple first. There is nothing to give out yet.'), 'Sign in to Repple first.', 'a mixed reason keeps the half a coach can act on');
eq((0, handOutCode_1.keptReason)(''), null, 'no reason is no sentence');
eq((0, handOutCode_1.keptReason)(null), null, 'and neither is nothing at all');
const signedOut = (0, handOutCode_1.codeToGive)({ status: 'error', reason: 'Sign in to Repple to get your coaching code.' });
ok(!signedOut.give && signedOut.note.startsWith('Sign in'), 'the kept reason leads, because it is the thing to do next');
ok(!signedOut.give && signedOut.note.includes(handOutCode_1.UNREAD_NOTE), 'and the correction still follows it');
ok(!failed.give && failed.note === handOutCode_1.UNREAD_NOTE, 'a dropped reason leaves the correction standing alone');
/* ── a code that did arrive ────────────────────────────────────────────── */
const good = (0, handOutCode_1.codeToGive)({ status: 'ready', code: 'k7m-2qx' });
...
if (good.give) {
    ...
    eq(good.hand.link, (0, joinCode_1.joinLink)('K7M2QX'), 'the bare link is the one joinCode.ts builds');
    eq(good.hand.message, (0, joinCode_1.inviteMessage)('K7M2QX'), 'and the message is the one every other share button sends');
...
eq((0, handOutCode_1.spokenCode)('K7M2QX'), 'K 7 M 2 Q X', 'the code is spoken one character at a time');
eq((0, handOutCode_1.spokenCode)('k7m-2qx'), 'K 7 M 2 Q X', 'however it arrived');
eq((0, handOutCode_1.spokenCode)(''), '', 'and nothing is spoken as nothing');
eq((0, handOutCode_1.handOut)('k7m-2qx').spoken, (0, handOutCode_1.spokenCode)('K7M2QX'), 'handOut spells it the same way');
eq((0, handOutCode_1.handOut)('K7M2QX').code, (0, joinCode_1.normaliseCode)('K7M2QX'), 'and normalises once, the same way as everywhere else');
...
const hand = (0, handOutCode_1.codesToHandOut)(rows);
...
eq((0, handOutCode_1.codesToHandOut)(null).length, 0, 'no rows hand out nothing');
eq((0, handOutCode_1.codesToHandOut)(undefined).length, 0, 'and neither does nothing at all');
// The order shapeJoinCodes established is preserved rather than re-imposed.
eq((0, handOutCode_1.codesToHandOut)([rows[3], rows[1]]).map((r) => r.id).join(','), 'c,a', 'the order the rows arrived in is kept');
/* ── four states, four different sentences ─────────────────────────────── */
const lines = new Map(['loading', 'error', 'partial', 'ready'].map((s) => [s, (0, handOutCode_1.namedCodesLine)(s, [])]));
...
const listed = (0, handOutCode_1.namedCodesLine)('ready', rows);
...
ok(/none yet/i.test((0, handOutCode_1.namedCodesLine)('ready', [row({ isLive: false })])), 'a book of turned-off codes has nothing to hand over');
...
eq((0, handOutCode_1.copyBlockedNote)(true), null, 'nothing is said when the clipboard works');
const blocked = (0, handOutCode_1.copyBlockedNote)(false) || '';
...
const failedCopy = (0, handOutCode_1.copyFailedNote)((0, joinCode_1.joinLink)('K7M2QX'));
ok(failedCopy.includes((0, joinCode_1.joinLink)('K7M2QX')), 'a failed copy still gives the coach the address');
...
const copied = (0, handOutCode_1.copiedNote)('your main code');
...
ok(/approve/i.test(handOutCode_1.HOW_THEY_USE_IT), 'the instructions say the coach still has to approve them');
ok(/find a trainer/i.test(handOutCode_1.HOW_THEY_USE_IT), 'and name the screen the client actually opens');
ok(/whatever address/i.test(handOutCode_1.HOW_THEY_USE_IT), 'and say the code does not care which address they signed up with');
if (errors.length) {
