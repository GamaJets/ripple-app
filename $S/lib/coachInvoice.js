...
exports.CHASE_FROM_IS_NOT_A_DUE_DATE = exports.INVOICE_SETTLEMENT_IS_YOUR_WORD = exports.BUCKET_TITLE = exports.AGEING_IS_YOUR_OWN_RECORD = exports.INVOICE_DUE_NOT_A_TERM = exports.kindLabel = exports.kindLine = exports.money = exports.INVOICE_VOID_NOTICE = exports.INVOICE_NOT_A_RECEIPT = exports.INVOICE_TAX_STATED = exports.INVOICE_TAX = exports.INVOICE_PROVENANCE = void 0;
exports.issuerCaveat = issuerCaveat;
exports.invoiceCaveats = invoiceCaveats;
exports.escapeHtml = escapeHtml;
exports.invoiceNumber = invoiceNumber;
exports.invoiceDayLabel = invoiceDayLabel;
exports.plusDays = plusDays;
exports.ageBucket = ageBucket;
exports.invoiceAge = invoiceAge;
exports.ageingBook = ageingBook;
exports.chaseBlocker = chaseBlocker;
exports.settleBlocker = settleBlocker;
exports.voidBlocker = voidBlocker;
exports.settleDayBlocker = settleDayBlocker;
exports.chaseFromBlocker = chaseFromBlocker;
exports.chaseFromDayBlocker = chaseFromDayBlocker;
exports.chaseHistoryLine = chaseHistoryLine;
exports.readTaxRate = readTaxRate;
exports.taxRateLabel = taxRateLabel;
exports.statesTax = statesTax;
exports.draftAmount = draftAmount;
exports.draftMinorUnits = draftMinorUnits;
exports.invoiceBlockers = invoiceBlockers;
exports.coachInvoiceDoc = coachInvoiceDoc;
exports.invoiceShareBlurb = invoiceShareBlurb;
exports.invoiceBook = invoiceBook;
 ...
 */
exports.INVOICE_TAX = 'No tax has been calculated, added or withheld. The amount shown is the amount charged, flat. This is not a tax invoice, no tax registration number is stated on it, and it should not be used as a tax document without your own accountant confirming what it needs to say.';
/**
 ...
 */
exports.INVOICE_NOT_A_RECEIPT = 'Whether this says the money was received or is being requested is the issuer’s own statement. It has not been checked against a bank or a card processor, and it is not a payment receipt from one.';
...
exports.INVOICE_VOID_NOTICE = 'THIS INVOICE HAS BEEN VOIDED BY THE ISSUER. It is not payable and it is not a record of a charge that stands. Its number has not been reused.';
/* ── read honesty ─────────────────────────────────────────────────────────── */
/**
...
function invoiceCaveats(input) {
    ...
    if ((0, exports.money)(input.invoice) === null) {
 ...
 */
exports.INVOICE_DUE_NOT_A_TERM = 'Any date shown as due is the issuer’s own statement of when they expect to be paid. It is not a payment term this app enforces, no interest or late fee is calculated anywhere on it, and this app is never told whether the money arrived.';
/**
 ...
 */
exports.AGEING_IS_YOUR_OWN_RECORD = 'This list is built from the due dates you typed and this phone’s clock. Nothing here has been checked against a bank or a card processor, and nobody tells this app when a client pays you — an invoice stays on this list until you say otherwise.';
/**
 ...
 */
function ageingBook(rows, status, today) {
    const overdue = [];
    const upcoming = [];
    const undated = [];
    for (const invoice of rows) {
        const age = invoiceAge(invoice, today);
        if (age.state === 'overdue')
            overdue.push({ invoice, age });
        else if (age.state === 'due-today' || age.state === 'not-due')
            upcoming.push({ invoice, age });
        else if (age.state === 'undated')
            undated.push({ invoice, age });
    ...
    const outstanding = status === 'ready'
        ? (0, coachMoney_1.sumTaken)([...overdue, ...upcoming].map(({ invoice }) => ({
 ...
 */
function chaseBlocker(inv) {
    if (inv.voidedAt)
        return 'This one is voided, so there is nothing to chase.';
    if (inv.settledOn)
        return `You recorded this one as settled on ${invoiceDayLabel(inv.settledOn)}, so there is nothing outstanding to chase.`;
    if (inv.kind === 'received')
        return 'You stated this one was received, so there is nothing outstanding to chase.';
    if (!inv.clientId)
        return 'This one is not tied to an account, so there is nobody here to notify. Send it to them the way you sent it the first time.';
    return null;
}
 ...
 */
exports.INVOICE_SETTLEMENT_IS_YOUR_WORD = 'The settlement above is the issuer’s own statement that this was paid, recorded by them after this document was issued. It has not been checked against a bank or a card processor and it is not a payment receipt from one. What the document said when it was issued has not been changed.';
/**
 ...
 */
function settleBlocker(inv) {
    if (inv.voidedAt)
        return 'This one is voided, so there is nothing to settle. A voided document is not a charge that stands.';
    if (inv.settledOn)
        return `You already recorded this one as settled on ${invoiceDayLabel(inv.settledOn)}. A settlement is written once — if the money went back out, that is a refund or a chargeback and it happened on its own day.`;
    if (inv.kind === 'received')
        return 'This one already states the money was received, so there is nothing to record. Settling it as well would put the same payment on one document twice, on two dates.';
    return null;
}
/**
 ...
 */
function voidBlocker(inv) {
    if (inv.voidedAt)
        return 'This one is already voided. A number is cancelled once and never uncancelled — somebody has been told it was.';
    if (inv.settledOn) {
 ...
 */
function settleDayBlocker(inv, settledOn, today) {
    const day = /^\d{4}-\d{2}-\d{2}$/;
    const on = String(settledOn ?? '').trim();
    if (!day.test(on))
        return 'Say which day the money arrived. Write it as a date.';
 ...
 */
function chaseFromBlocker(inv) {
    if (inv.voidedAt)
        return 'This one is voided, so there is nothing to chase.';
    if (inv.settledOn)
        return `You recorded this one as settled on ${invoiceDayLabel(inv.settledOn)}, so there is nothing to chase.`;
    if (inv.kind === 'received')
        return 'This one states the money was received, so there is nothing to chase.';
    if (inv.dueOn)
        return `This one carries a due date of ${invoiceDayLabel(inv.dueOn)}, which is what it is already chased against. That date is on the document and cannot be moved.`;
    return null;
}
...
function chaseFromDayBlocker(inv, from) {
    const day = /^\d{4}-\d{2}-\d{2}$/;
    const on = String(from ?? '').trim();
    if (!day.test(on))
        return 'Write the day to start chasing from as a date, or clear it and this one goes back on the undated list.';
    if (day.test(String(inv.issuedOn)) && on < String(inv.issuedOn)) {
 ...
 */
function draftAmount(amountText, currency) {
    const read = (0, coachMoney_1.readMinorAmount)(amountText, currency);
    if (!read.ok)
        return read;
...
function draftMinorUnits(amountText, currency) {
 ...
 */
function invoiceBlockers(d) {
    const out = [];
    if (!String(d.billTo ?? '').trim())
        out.push('Say who this invoice is for. The name you type is the name printed on it.');
    if (!String(d.description ?? '').trim())
        out.push('Say what it is for. A charge with no description is not something a client can check against anything.');
    ...
    if (!/^\d{4}-\d{2}-\d{2}$/.test(String(d.issuedOn ?? '')))
        out.push('The date this is issued on could not be read.');
 ...
 */
function coachInvoiceDoc(input) {
    ...
    const amount = (0, exports.money)(inv);
    ...
    H.push((0, coachLogo_1.logoImgHtml)(input.issuer.logoDataUri));
    ...
    if (voided) {
        H.push(`<div class="void"><h3>Voided</h3><p>${escapeHtml(exports.INVOICE_VOID_NOTICE)}</p>${inv.voidReason ? `<p>Reason given: ${escapeHtml(inv.voidReason)}</p>` : ''}</div>`);
        T.push('', '*** VOIDED ***', exports.INVOICE_VOID_NOTICE);
        if (inv.voidReason)
            T.push('Reason given: ' + inv.voidReason);
    }
    ...
    H.push(`<p>${escapeHtml((0, exports.kindLine)(inv.kind))}</p>`);
    T.push((0, exports.kindLine)(inv.kind));
    ...
    for (const line of exports.INVOICE_PROVENANCE) {
    ...
    {
        const taxLine = statesTax(inv) ? exports.INVOICE_TAX_STATED : exports.INVOICE_TAX;
        H.push(`<p class="lede">${escapeHtml(taxLine)}</p>`);
        T.push(taxLine);
    }
    H.push(`<p class="lede">${escapeHtml(exports.INVOICE_NOT_A_RECEIPT)}</p>`);
    T.push(exports.INVOICE_NOT_A_RECEIPT);
    ...
    if (inv.settledOn) {
        H.push(`<p class="lede">${escapeHtml(exports.INVOICE_SETTLEMENT_IS_YOUR_WORD)}</p>`);
        T.push(exports.INVOICE_SETTLEMENT_IS_YOUR_WORD);
    }
    ...
    H.push(`<p class="lede">${escapeHtml(exports.INVOICE_DUE_NOT_A_TERM)}</p>`);
    T.push(exports.INVOICE_DUE_NOT_A_TERM);
    /* ── foot ──────────────────────────────────────────────────────────────── */
    const foot = voided
 ...
 */
function invoiceShareBlurb(doc, inv) {
...
function invoiceBook(rows, status) {
    ...
    return { totals: (0, coachMoney_1.sumTaken)(taken), voided, live, reason: null };
}
