...
exports.RECURRING_CLASH_NOTE = exports.RECURRING_CREDIT_NOTE = exports.RECURRING_END_RULE = exports.SERIES_HORIZON_DAYS = exports.SERIES_MINUTES = exports.DOW_NAMES = void 0;
exports.shapeSeries = shapeSeries;
exports.clockLabel = clockLabel;
exports.seriesLabel = seriesLabel;
exports.memberSeriesLabel = memberSeriesLabel;
exports.seriesDates = seriesDates;
exports.zonedSlot = zonedSlot;
exports.seriesOccurrencesIn = seriesOccurrencesIn;
exports.cancelOptions = cancelOptions;
exports.occurrenceDetail = occurrenceDetail;
exports.seriesDetail = seriesDetail;
exports.clashLine = clashLine;
...
exports.SERIES_HORIZON_DAYS = 56;
...
function shapeSeries(rows) {
    ...
    return rows
        ...
        .sort((a, b) => Number(b.active) - Number(a.active) || a.dow - b.dow || a.hour - b.hour || a.minute - b.minute);
}
...
function seriesLabel(s) {
    const day = exports.DOW_NAMES[((s.dow % 7) + 7) % 7];
 ...
 */
function memberSeriesLabel(s) {
    return `Every ${(0, format_1.weekdayName)(s.dow)} at ${(0, format_1.fmtClock)(s.hour, s.minute)}`;
}
/**
 ...
 */
function seriesDates(dow, hour, minute = 0, weeks = 8, from = new Date()) {
    const out = [];
 ...
 */
function seriesOccurrencesIn(sessions, series, fromMs, untilMs) {
    const out = [];
    for (const x of sessions) {
        if (x.status !== 'booked')
            continue;
        if (x.clientId !== series.clientId)
            continue;
        if (x.trainerId !== series.trainerId)
            continue;
        const at = Date.parse(x.startsAt);
        if (!Number.isFinite(at) || at <= fromMs || at >= untilMs)
            continue;
        if (x.durationMin !== series.durationMin)
            continue;
        ...
        if (slot && !(slot.dow === (((series.dow % 7) + 7) % 7) && slot.hour === series.hour && slot.minute === series.minute))
            continue;
 ...
 */
function cancelOptions(o) {
    const now = o.now ?? Date.now();
    const notice = (0, booking_1.noticeHoursOf)(o.policy);
    ...
    const inside = (0, booking_1.insideNoticeWindow)(o.startsAt, notice, now);
    const verdict = (0, booking_1.lateCancelFee)(o.policy, inside);
...
function occurrenceDetail(v, noticeHours) {
    const w = (0, booking_1.noticeLabel)(noticeHours);
    switch (v.kind) {
        case 'in-time':
        ...
        case 'fee':
            return `Frees this one only — the rest of the standing appointment is untouched. This is inside ${w}, so a late-cancellation fee of ${(0, booking_1.feeAmountLine)(v.amount, v.currency)} is recorded. Repple doesn’t take this payment.${(0, booking_1.unstatedCurrency)(v.currency)}`;
    }
}
/**
 ...
 */
function seriesDetail(later, nextStartsAt) {
...
exports.RECURRING_END_RULE = 'Ending a standing appointment removes the sessions after the next one and never charges a cancellation fee, '
 ...
 */
exports.RECURRING_CREDIT_NOTE = 'A standing appointment books the time, not the sessions. Nothing comes off a session pack when the dates '
...
exports.RECURRING_CLASH_NOTE = 'A date you were already booked or blocked on is skipped rather than double-booked. The rest of the '
 ...
 */
function clashLine(skipped, dates) {
...
function createdLine(created, weeks = exports.SERIES_HORIZON_DAYS / 7) {
