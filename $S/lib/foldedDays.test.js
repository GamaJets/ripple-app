...
const foldedDays_1 = require("./foldedDays");
const errors = [];
const ok = (cond, msg) => { if (!cond)
    errors.push(msg); };
const eq = (a, b, msg) => ok(Object.is(a, b), `${msg} — got ${JSON.stringify(a)}, wanted ${JSON.stringify(b)}`);
/** The folded indices, ascending, so a whole map can be compared in one line. */
const folded = (f) => Object.keys(f).map(Number).filter((i) => f[i]).sort((a, b) => a - b);
const same = (f, want, msg) => eq(JSON.stringify(folded(f)), JSON.stringify(want), msg);
/* ── the report ────────────────────────────────────────────────────────────
...
same((0, foldedDays_1.foldsAfterRemoval)(week, 0), [0], 'a fold above the removal slides down one');
// Deleting Tuesday itself. Nothing is folded any more, and index 1 — now
// Wednesday — must NOT inherit it.
same((0, foldedDays_1.foldsAfterRemoval)(week, 1), [], 'the removed day takes its own fold with it');
// Deleting Wednesday or Thursday. Tuesday has not moved.
same((0, foldedDays_1.foldsAfterRemoval)(week, 2), [1], 'a fold below the removal is left where it is');
same((0, foldedDays_1.foldsAfterRemoval)(week, 3), [1], 'a removal past every fold changes nothing');
/* ── several folds at once ─────────────────────────────────────────────── */
const many = { 0: true, 2: true, 4: true };
same((0, foldedDays_1.foldsAfterRemoval)(many, 1), [0, 1, 3], 'every fold above the removal shifts, and only those');
same((0, foldedDays_1.foldsAfterRemoval)(many, 0), [1, 3], 'removing a folded first day drops it and shifts the rest');
same((0, foldedDays_1.foldsAfterRemoval)(many, 4), [0, 2], 'removing the folded last day leaves the others alone');
...
const withFalses = { 0: false, 1: true, 2: false };
eq(Object.keys((0, foldedDays_1.foldsAfterRemoval)(withFalses, 0)).length, 1, 'open days are not carried as keys');
same((0, foldedDays_1.foldsAfterRemoval)(withFalses, 0), [0], 'the one real fold still moves correctly past them');
...
const junk = { '-1': true, 1.5: true, 2: true };
same((0, foldedDays_1.foldsAfterRemoval)(junk, 0), [1], 'keys that are not day indices are dropped');
...
const before = { 1: true, 3: true };
(0, foldedDays_1.foldsAfterRemoval)(before, 0);
...
eq(Object.keys((0, foldedDays_1.foldsForNewProgramme)()).length, 0, 'a new programme starts with nothing folded');
ok((0, foldedDays_1.foldsForNewProgramme)() !== (0, foldedDays_1.foldsForNewProgramme)(), 'each caller gets its own map, not a shared one');
if (errors.length) {
