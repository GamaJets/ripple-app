...
exports.parseGymZone = parseGymZone;
exports.isZone = isZone;
exports.readerZone = readerZone;
exports.gymDay = gymDay;
exports.gymWeekday = gymWeekday;
exports.gymHour = gymHour;
exports.sameGymDay = sameGymDay;
exports.gymTimeLabel = gymTimeLabel;
exports.gymMinutesAhead = gymMinutesAhead;
exports.zoneGapNote = zoneGapNote;
exports.gymDayBounds = gymDayBounds;
exports.instantAtGym = instantAtGym;
exports.gymWallValue = gymWallValue;
exports.zoneOptions = zoneOptions;
exports.fetchGymZone = fetchGymZone;
/**
 ...
 */
exports.NO_ZONE_NOTE = 'this gym has not set its timezone, so days and hours here are your own device’s, not the gym’s';
function parseGymZone(input) {
    ...
    if (!isZone(raw)) {
        return {
            kind: 'bad',
            reason: 'A timezone is an IANA name — Europe/London, Asia/Dubai, America/Los_Angeles. ' +
...
function isZone(zone) {
    ...
    if (!z.includes('/') && z.toUpperCase() !== 'UTC')
        return false;
    try {
 ...
 */
function gymDay(at, zone) {
    if (at == null || !zone || !isZone(zone))
        return null;
 ...
 * gets wrong.
 */
function gymMinutesAhead(at, zone, reader = readerZone()) {
    if (!zone || !reader)
        return null;
 ...
 */
function zoneGapNote(zone, reader = readerZone()) {
    if (!zone || !reader || zone === reader)
        return null;
 ...
 */
function gymDayBounds(day, zone) {
    const d = String(day ?? '').trim();
    if (!/^\d{4}-\d{2}-\d{2}$/.test(d) || !zone || !isZone(zone))
        return null;
 ...
 */
function instantAtGym(wall, zone) {
...
function offsetMsAt(at, zone) {
    ...
    return asIfUtc - Math.floor(at / 60000) * 60000;
}
 ...
 */
async function fetchGymZone(sb, tenantId) {
    const { data, error } = await sb
        .from('tenants').select('timezone').eq('id', tenantId).single();
    if (error) {
        return { zone: null, error: error.message || 'The gym’s timezone could not be read.' };
    }
