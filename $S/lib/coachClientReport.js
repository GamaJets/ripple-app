...
exports.COACH_TRAINING_DAYS_SHOWN = exports.COACH_REPORT_NO_RATE = exports.COACH_REPORT_NO_PHOTOS = exports.COACH_REPORT_LIMITS = exports.COACH_REPORT_PROVENANCE = void 0;
exports.countableRows = countableRows;
exports.sessionTally = sessionTally;
exports.coachReportCaveats = coachReportCaveats;
exports.coachClientReportDoc = coachClientReportDoc;
exports.coachReportShareBlurb = coachReportShareBlurb;
...
const clientReport_1 = require("./clientReport");
 ...
 */
function countableRows(rows, status) {
    if (!(0, loadStatus_1.isWhole)(status) || rows == null)
        return null;
    return rows.length;
}
/**
 ...
 */
function sessionTally(rows, status) {
    const empty = {
 ...
 * can carry.
 */
exports.COACH_REPORT_PROVENANCE = [
 ...
 */
exports.COACH_REPORT_LIMITS = 'This document contains no assessment, no rating and no advice. It is a record of what was booked, logged and measured. Nothing in it is a clinical judgement, a score, or a statement about what this person should do next, and no figure in it has been reviewed by a clinician.';
 ...
 *  everybody. */
exports.COACH_REPORT_NO_PHOTOS = 'No photographs are included. Progress photographs are shared by the client with one coach at a time and are never attached to a document. Documents a client uploaded about an injury are likewise not included — only the injury they recorded from them.';
/**
 ...
 */
exports.COACH_REPORT_NO_RATE = 'No attendance rate or percentage is stated. Sessions whose outcome nobody recorded are counted separately below and are not treated as missed or as attended, so a percentage over them would not measure anything.';
/* ── read honesty ─────────────────────────────────────────────────────────── */
/** Every caveat this document has to carry, in the order the sections appear. */
function coachReportCaveats(input) {
    return [
        (0, clientReport_1.sectionCaveat)('Who prepared this', input.coachStatus),
        (0, clientReport_1.sectionCaveat)('Sessions', input.sessions.status),
        (0, clientReport_1.sectionCaveat)('Training logged', input.training.status),
        (0, clientReport_1.sectionCaveat)('Body composition', input.composition.status),
        (0, clientReport_1.sectionCaveat)('Tape measurements', input.measurements.status),
        (0, clientReport_1.sectionCaveat)('Injuries disclosed', input.injuries.status),
    ].filter((s) => s !== null);
}
/* ── plumbing ─────────────────────────────────────────────────────────────── */
const num = (v) => v == null || !Number.isFinite(v) ? '—' : v.toLocaleString();
const unitised = (text, unit) => (text === '—' ? '—' : `${text} ${unit}`);
const partialNote = (what) => `Not all of this person’s ${what} could be read in one request. What is listed is real; it is not all of it.`;
function oldestFirst(items, at) {
...
exports.COACH_TRAINING_DAYS_SHOWN = 30;
const STYLE = `
...
`;
const unreadableBlock = (what) => `<p class="none"><b>Not read.</b> ${(0, clientReport_1.escapeHtml)(what)} could not be read from the server when this document was made, so nothing is listed here. This is not a statement that there is none on record.</p>`;
const emptyBlock = (what) => `<p class="none">${(0, clientReport_1.escapeHtml)(what)}</p>`;
/* ── the document ─────────────────────────────────────────────────────────── */
/**
 ...
 */
function coachClientReportDoc(input) {
    ...
    const coachRead = (0, clientReport_1.sectionState)(input.coachStatus) !== 'unreadable';
    ...
    H.push((0, coachLogo_1.logoImgHtml)(input.logoDataUri));
    H.push(`<div class="h"><h1>Coaching record</h1><p>${(0, clientReport_1.escapeHtml)(who)} · prepared ${(0, clientReport_1.escapeHtml)((0, progressExport_1.dayLabel)(input.generatedOn))} · ${(0, clientReport_1.escapeHtml)(brand)}</p></div>`);
    T.push(`${who} — coaching record`);
    T.push(`Prepared ${(0, progressExport_1.dayLabel)(input.generatedOn)} · ${brand}`);
    /* ── who prepared it ───────────────────────────────────────────────────── */
    //
    ...
    if (!coachRead) {
        ...
        H.push(`<p>Prepared by <b>${(0, clientReport_1.escapeHtml)(coach)}</b>, who coaches ${(0, clientReport_1.escapeHtml)(who)} through ${(0, clientReport_1.escapeHtml)(brand)}.</p>`);
        T.push(`Prepared by ${coach}, who coaches ${who} through ${brand}.`);
    }
    /* ── what this is, and is not ──────────────────────────────────────────── */
    H.push('<h2>About this document</h2>');
    T.push('', 'ABOUT THIS DOCUMENT');
    for (const line of exports.COACH_REPORT_PROVENANCE) {
        H.push(`<p class="lede">${(0, clientReport_1.escapeHtml)(line)}</p>`);
        T.push(line);
    }
    H.push(`<p class="lede">${(0, clientReport_1.escapeHtml)(exports.COACH_REPORT_LIMITS)}</p>`);
    T.push(exports.COACH_REPORT_LIMITS);
    H.push(`<p class="lede">${(0, clientReport_1.escapeHtml)(exports.COACH_REPORT_NO_PHOTOS)}</p>`);
    T.push(exports.COACH_REPORT_NO_PHOTOS);
    if (input.unitNote) {
        H.push(`<p class="lede">${(0, clientReport_1.escapeHtml)(input.unitNote)}</p>`);
    ...
    if (!complete) {
        ...
        for (const c of caveats) {
            H.push(`<li>${(0, clientReport_1.escapeHtml)(c)}</li>`);
            T.push('- ' + c);
        }
        H.push('</ul></div>');
    }
    /* ── sessions ──────────────────────────────────────────────────────────── */
    {
        const st = (0, clientReport_1.sectionState)(input.sessions.status);
        ...
        if (st === 'unreadable' || tally.state === 'unreadable') {
            ...
            if (st === 'partial') {
                ...
                H.push(`<p class="lede"><b>${(0, clientReport_1.escapeHtml)(p)}</b></p>`);
                T.push(p);
            }
            if (tally.firstDay && tally.lastDay) {
                const span = tally.firstDay === tally.lastDay
                    ? `One day on record: ${(0, progressExport_1.dayLabel)(tally.firstDay)}.`
                    : `From ${(0, progressExport_1.dayLabel)(tally.firstDay)} to ${(0, progressExport_1.dayLabel)(tally.lastDay)}.`;
                H.push(`<p class="lede">${(0, clientReport_1.escapeHtml)(span)}</p>`);
                T.push(span);
            }
            const rows = [
            ...
            H.push(`<table>${rows.map(([k, v]) => `<tr><td>${(0, clientReport_1.escapeHtml)(k)}</td><td class="r">${(0, clientReport_1.escapeHtml)(v)}</td></tr>`).join('')}</table>`);
            ...
            H.push(`<p class="lede">${(0, clientReport_1.escapeHtml)(unrec)}</p>`);
            T.push(unrec);
            H.push(`<p class="lede">${(0, clientReport_1.escapeHtml)(exports.COACH_REPORT_NO_RATE)}</p>`);
            T.push(exports.COACH_REPORT_NO_RATE);
            if (st === 'partial') {
                ...
                H.push(`<p class="lede">${(0, clientReport_1.escapeHtml)(noCount)}</p>`);
                T.push(noCount);
            }
        }
    }
    ...
    {
        const st = (0, clientReport_1.sectionState)(input.training.status);
        ...
        if (st === 'unreadable' || b.state === 'unreadable') {
            ...
            const totals = [
                ...
                [`Total load moved (${wu})`, num((0, units_1.volumeIn)(b.volumeKg, wu))],
                ['Most recent day', b.newestDay ? (0, progressExport_1.dayLabel)(b.newestDay) : '—'],
            ];
            H.push(`<table>${totals.map(([k, v]) => `<tr><td>${(0, clientReport_1.escapeHtml)(k)}</td><td class="r">${(0, clientReport_1.escapeHtml)(v)}</td></tr>`).join('')}</table>`);
            ...
            H.push(`<p class="lede">${(0, clientReport_1.escapeHtml)(loadNote)}</p>`);
            ...
            if (b.undatedCount > 0) {
                ...
                H.push(`<p class="lede">${(0, clientReport_1.escapeHtml)(u)}</p>`);
                T.push(u);
            }
            const shown = b.days.slice(0, exports.COACH_TRAINING_DAYS_SHOWN);
            if (!shown.length) {
                const noneDated = 'No logged session carries a readable date, so there is no day-by-day table.';
                H.push(`<p class="none">${(0, clientReport_1.escapeHtml)(noneDated)}</p>`);
                T.push(noneDated);
            }
            else {
                ...
                H.push(`<p class="lede">${(0, clientReport_1.escapeHtml)(slice)}</p>`);
                T.push(slice);
                const body = shown.map((d) => `<tr><td>${(0, clientReport_1.escapeHtml)((0, progressExport_1.dayLabel)(d.day))}</td>`
                    + `<td class="r">${(0, clientReport_1.escapeHtml)(num(d.exercises))}</td>`
                    + `<td class="r">${(0, clientReport_1.escapeHtml)(num(d.sets))}</td>`
                    + `<td class="r">${(0, clientReport_1.escapeHtml)(num(d.bodyweightSets))}</td>`
                    + `<td class="r">${(0, clientReport_1.escapeHtml)(num((0, units_1.volumeIn)(d.volumeKg, wu)))}</td></tr>`).join('');
                H.push(`<table><tr><th>Date</th><th class="r">Exercises</th><th class="r">Sets</th><th class="r">Of those, bodyweight</th><th class="r">Load moved (${(0, clientReport_1.escapeHtml)(wu)})</th></tr>${body}</table>`);
                for (const d of shown)
                    T.push(`  ${(0, progressExport_1.dayLabel)(d.day)}  ${num(d.exercises)} exercises  ${num(d.sets)} sets (${num(d.bodyweightSets)} bodyweight)  ${unitised(num((0, units_1.volumeIn)(d.volumeKg, wu)), wu)}`);
            }
        }
    }
    /* ── body composition ──────────────────────────────────────────────────── */
    {
        const st = (0, clientReport_1.sectionState)(input.composition.status);
        const scans = oldestFirst(input.composition.items, (s) => s.takenAt);
        const rows = (0, clientReport_1.toProgressRows)(scans);
        ...
        if (st === 'unreadable') {
            ...
            const span = `${(0, progressExport_1.progressSpanLabel)(rows)}.`;
            H.push(`<p class="lede">${(0, clientReport_1.escapeHtml)(span)}</p>`);
            T.push(span);
            const conv = (0, units_1.convertedNote)(wu);
            if (conv) {
                H.push(`<p class="lede">${(0, clientReport_1.escapeHtml)(conv)}</p>`);
                T.push(conv);
            }
            const body = scans.map((s, i) => {
                const r = rows[i];
                return `<tr><td>${(0, clientReport_1.escapeHtml)((0, progressExport_1.dayLabel)(r.date))}</td>`
                    + `<td class="r">${(0, clientReport_1.escapeHtml)((0, progressExport_1.figure)((0, units_1.weightIn)(r.weightKg, wu)))}</td>`
                    + `<td class="r">${(0, clientReport_1.escapeHtml)((0, progressExport_1.figure)(r.bodyFatPct))}</td>`
                    + `<td class="r">${(0, clientReport_1.escapeHtml)((0, progressExport_1.figure)((0, units_1.weightIn)(r.muscleKg, wu)))}</td>`
                    + `<td>${(0, clientReport_1.escapeHtml)(s.source || '—')}</td></tr>`;
            }).join('');
            H.push(`<table><tr><th>Date</th><th class="r">Weight (${(0, clientReport_1.escapeHtml)(wu)})</th><th class="r">Body fat (%)</th><th class="r">Muscle (${(0, clientReport_1.escapeHtml)(wu)})</th><th>Recorded from</th></tr>${body}</table>`);
            for (const [i, r] of rows.entries()) {
                T.push(`  ${(0, progressExport_1.dayLabel)(r.date)}  ${(0, progressExport_1.figure)((0, units_1.weightIn)(r.weightKg, wu), ' ' + wu)}  ${(0, progressExport_1.figure)(r.bodyFatPct, '%')}  ${unitised((0, progressExport_1.figure)((0, units_1.weightIn)(r.muscleKg, wu), ' ' + wu), 'muscle')}  (${scans[i].source || 'source not recorded'})`);
            }
            ...
            if (st === 'whole') {
                const lines = (0, progressExport_1.progressChangeLines)(rows, wu);
                if (lines.length) {
                    H.push(`<p>First reading to latest: ${(0, clientReport_1.escapeHtml)(lines.join(' · '))}</p>`);
                ...
                H.push(`<p class="lede">${(0, clientReport_1.escapeHtml)(noChange)}</p>`);
                T.push(noChange);
            }
        }
    }
    /* ── tape measurements ─────────────────────────────────────────────────── */
    {
        const st = (0, clientReport_1.sectionState)(input.measurements.status);
        ...
        if (st === 'unreadable') {
            ...
            const conv = (0, units_1.convertedNote)(lu);
            if (conv) {
                H.push(`<p class="lede">${(0, clientReport_1.escapeHtml)(conv)}</p>`);
                T.push(conv);
            }
            const head = cols.map((c) => `<th class="r">${(0, clientReport_1.escapeHtml)(c.label)} (${(0, clientReport_1.escapeHtml)(lu)})</th>`).join('');
            const body = entries.map((e) => {
                const cells = cols.map((c) => `<td class="r">${(0, clientReport_1.escapeHtml)((0, progressExport_1.figure)((0, units_1.lengthIn)(e.values[c.key] ?? null, lu)))}</td>`).join('');
                return `<tr><td>${(0, clientReport_1.escapeHtml)((0, progressExport_1.dayLabel)(String(e.at).slice(0, 10)))}</td>${cells}</tr>`;
            }).join('');
            H.push(`<table><tr><th>Date</th>${head}</tr>${body}</table>`);
            for (const e of entries) {
                T.push(`  ${(0, progressExport_1.dayLabel)(String(e.at).slice(0, 10))}  ` + cols.map((c) => `${c.label} ${(0, progressExport_1.figure)((0, units_1.lengthIn)(e.values[c.key] ?? null, lu), ' ' + lu)}`).join('  '));
            }
        }
    }
    /* ── injuries disclosed ────────────────────────────────────────────────── */
    ...
    {
        const st = (0, clientReport_1.sectionState)(input.injuries.status);
        ...
        if (st === 'unreadable') {
            ...
            const body = items.map((i) => `<tr><td>${(0, clientReport_1.escapeHtml)(i.label)}</td>`
                + `<td>${(0, clientReport_1.escapeHtml)(i.severity)}</td>`
                + `<td>${(0, clientReport_1.escapeHtml)(i.status)}</td>`
                + `<td>${(0, clientReport_1.escapeHtml)((0, progressExport_1.dayLabel)(String(i.at).slice(0, 10)))}</td>`
                + `<td>${(0, clientReport_1.escapeHtml)(i.note || '—')}</td></tr>`).join('');
            H.push(`<table><tr><th>Area</th><th>Severity as recorded</th><th>State</th><th>Recorded on</th><th>Their note</th></tr>${body}</table>`);
            for (const i of items)
                T.push(`  ${i.label} — ${i.severity}, ${i.status}, recorded ${(0, progressExport_1.dayLabel)(String(i.at).slice(0, 10))}${i.note ? ' — "' + i.note + '"' : ''}`);
            ...
            H.push(`<p class="lede">${(0, clientReport_1.escapeHtml)(grading)}</p>`);
            T.push(grading);
        }
    }
    ...
    // block sits above the footer that says whether the document is complete.
    {
        ...
        if (!note) {
            ...
            H.push(`<p class="lede">${(0, clientReport_1.escapeHtml)(attrib)}</p>`);
            T.push(attrib);
            H.push(`<div class="quote">${(0, clientReport_1.escapeHtml)(note)}</div>`);
            T.push('  “' + note + '”');
        }
    }
    /* ── foot ──────────────────────────────────────────────────────────────── */
    const foot = complete
        ? `Every section of this document was read successfully on ${(0, progressExport_1.dayLabel)(input.generatedOn)}. Generated by ${brand}.`
        : `PARTS OF THIS DOCUMENT COULD NOT BE READ — see "This record is incomplete" above. Generated by ${brand}.`;
    H.push(`<p class="foot">${(0, clientReport_1.escapeHtml)(foot)}</p>`);
    T.push('', foot);
    const html = `<html><head><meta name="viewport" content="width=device-width, initial-scale=1"><style>${STYLE}${coachLogo_1.LOGO_CSS}</style></head><body>${H.join('')}</body></html>`;
 ...
 */
function coachReportShareBlurb(doc, clientName) {
