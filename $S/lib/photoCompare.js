...
exports.COMPARE_DISCLAIMER = void 0;
exports.selectionFromParams = selectionFromParams;
exports.sameDay = sameDay;
exports.readingOn = readingOn;
exports.compareDelta = compareDelta;
exports.compareRows = compareRows;
exports.readingText = readingText;
exports.deltaText = deltaText;
exports.compareBasis = compareBasis;
...
exports.COMPARE_DISCLAIMER = 'These are two photographs and the InBody figures recorded on the days they were taken. Nothing here is measured from the pictures themselves.';
/**
 ...
 * earlier one.
 */
function selectionFromParams(raw, known) {
    if (!raw || !known)
        return [];
...
function sameDay(a, b) {
    const x = (0, localDate_1.dateParts)(a);
    const y = (0, localDate_1.dateParts)(b);
    if (!x || !y)
        return false;
 ...
 */
function readingOn(iso, scans) {
    if (!iso || !scans)
        return null;
    let found = null;
    for (const s of scans)
        if (sameDay(s.takenAt, iso))
            found = s;
    return found;
}
 ...
 */
function compareRows(beforeISO, afterISO, scans, 
...
unit) {
     ...
     */
    const row = (key, label, metricUnit, pick, mass) => {
        ...
        return {
            ...
            before: mass ? (0, units_1.weightIn)(beforeKg, unit) : beforeKg,
            after: mass ? (0, units_1.weightIn)(afterKg, unit) : afterKg,
            delta: mass ? (0, units_1.weightDeltaIn)(deltaKg, unit) : deltaKg,
        };
    };
    return [
...
function deltaText(v, unit) {
    if (v === null)
        return '—';
    return `${v > 0 ? '+' : v < 0 ? deltaLabel_1.MINUS : ''}${Math.abs(v)} ${unit}`;
}
/**
 ...
 * dates at all.
 */
function compareSummary(beforeLabel, afterLabel, days, rows) {
    const head = `Progress comparison — ${beforeLabel} → ${afterLabel} (${spanLabel(days)})`;
    const lines = rows.map((r) => `${r.label}: ${readingText(r.before, r.unit)} → ${readingText(r.after, r.unit)} (${deltaText(r.delta, r.unit)})`);
    return [
        head,
        '',
        ...lines,
