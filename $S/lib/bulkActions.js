...
exports.NAMES_IN_BRIEF = void 0;
exports.namesWithRest = namesWithRest;
exports.overwriteBrief = overwriteBrief;
exports.unassignBrief = unassignBrief;
exports.bulkReport = bulkReport;
exports.selectAllOffer = selectAllOffer;
exports.guardRecipients = guardRecipients;
exports.bulkThreadNote = bulkThreadNote;
exports.endCoachingBrief = endCoachingBrief;
 ...
 */
function namesWithRest(names, limit = exports.NAMES_IN_BRIEF) {
    if (names.length <= limit)
        return (0, groupProgram_1.listNames)(names);
    const rest = names.length - limit;
    return `${names.slice(0, limit).join(', ')} and ${(0, format_1.num)(rest)} more`;
}
/**
 ...
 */
function overwriteBrief(targets, templateName) {
    ...
    if (r === 0) {
        return {
            title: n === 1 ? 'Assign This Programme?' : `Assign to ${(0, format_1.num)(n)} Clients?`,
            body: `${n === 1 ? 'This client is' : `None of these ${(0, format_1.num)(n)} are`} on a coach-assigned programme, so nothing is being replaced. `
                + `“${templateName}” will be waiting on ${n === 1 ? 'their Train tab' : 'each of their Train tabs'}.`,
            confirmLabel: n === 1 ? 'Assign' : `Assign to ${(0, format_1.num)(n)}`,
            replacing,
        };
    }
    const who = namesWithRest(replacing.map((x) => x.name));
    const heading = r === n
        ? (n === 1 ? 'Replace What They Are Training?' : `Replace What All ${(0, format_1.num)(n)} Are Training?`)
        : `Replace ${(0, format_1.num)(r)} Current Programmes?`;
    const lead = r === n
        ? (n === 1
            ? `${who} is on a programme now. Assigning “${templateName}” replaces it.`
            : `All ${(0, format_1.num)(n)} of these are on a programme now — ${who}. Assigning “${templateName}” replaces every one of them.`)
        : `${(0, format_1.num)(r)} of these ${(0, format_1.num)(n)} are on a programme now, including ${who}. Assigning “${templateName}” replaces what they are training.`;
    ...
    const rest = fresh.length
        ? `\n\nThe other ${(0, format_1.num)(fresh.length)} — ${namesWithRest(fresh.map((x) => x.name))} — ${fresh.length === 1 ? 'is' : 'are'} on no coach-assigned programme, so for them this is new work rather than a replacement.`
        : '';
    return {
        title: heading,
        body: `${lead} ${cost}${rest}`,
        ...
        confirmLabel: r === n
            ? (n === 1 ? 'Replace Their Programme' : `Replace All ${(0, format_1.num)(n)}`)
            : `Replace ${(0, format_1.num)(r)} and Assign ${(0, format_1.num)(n)}`,
        replacing,
    };
}
/**
 ...
 */
function unassignBrief(targets) {
    ...
    return {
        title: n === 1 ? 'Take Them Off This Programme?' : `Take ${(0, format_1.num)(n)} Clients Off Their Programmes?`,
        body: `${who} ${n === 1 ? 'goes' : 'go'} back to an auto-generated plan built from ${n === 1 ? 'their' : 'their own'} goal. `
        ...
        confirmLabel: n === 1 ? 'Take Them Off' : `Take ${(0, format_1.num)(n)} Off`,
        replacing: on,
    };
}
/**
 ...
 */
function bulkReport(kind, results) {
    const ok = results.filter((r) => r.ok);
    const bad = results.filter((r) => !r.ok);
    const n = results.length;
    const retry = bad.map((r) => r.clientId);
    const landed = kind === 'assign'
        ? (c) => `${c === 1 ? 'It is' : 'They are'} on ${c === 1 ? 'their' : `${(0, format_1.num)(c)} clients’`} Train tab${c === 1 ? '' : 's'} now.`
        ...
        : kind === 'unassign'
            ? (c) => `${c === 1 ? 'They are' : `All ${(0, format_1.num)(c)} are`} back on an auto-generated plan. Every session ${c === 1 ? 'they have' : 'they have'} logged is untouched.`
            ...
            : kind === 'end'
                ? (c) => `${c === 1 ? 'They are' : `All ${(0, format_1.num)(c)} are`} off your roster, and anyone with an account has been told the coaching ended. They keep everything they logged; anyone you had added by hand is deleted along with the name and goal you typed.`
                ...
                : kind === 'checklist'
                    ? (c) => `The lines are on ${c === 1 ? 'their' : `${(0, format_1.num)(c)} clients’`} daily list from tomorrow morning, marked as set by you. Nobody was notified, and only you can take them off again.`
                    : (c) => `Your message is in ${c === 1 ? 'their thread' : `${(0, format_1.num)(c)} threads`} now.`;
    if (n === 0) {
    ...
    if (!bad.length) {
        return {
            title: verbTitle,
            body: `${verbBody} ${(0, format_1.num)(n)} ${n === 1 ? 'client' : 'clients'} — ${namesWithRest(ok.map((r) => r.name))}. ${landed(n)}`,
            retry: [],
        };
    }
    const failureLines = bad.length <= REASONS_IN_REPORT
        ? bad.map((r) => `· ${r.name} — ${r.why ?? 'the server did not say why.'}`).join('\n')
        : `${(0, groupProgram_1.listNames)(bad.map((r) => r.name))}.\n\n${distinctReasons(bad)}`;
    if (!ok.length) {
        return {
            ...
            body: `${n === 1 ? 'The write' : `None of the ${(0, format_1.num)(n)} writes`} landed, so nothing has changed for ${n === 1 ? 'them' : 'any of them'}. `
    ...
    return {
        ...
        body: `${(0, format_1.num)(ok.length)} of ${(0, format_1.num)(n)} landed — ${namesWithRest(ok.map((r) => r.name))}. ${landed(ok.length)}\n\n`
            + `${(0, format_1.num)(bad.length)} did not, and ${bad.length === 1 ? 'is' : 'are'} still selected so you can try again:\n\n`
            + failureLines,
        retry,
    };
}
...
function distinctReasons(bad) {
    const seen = [];
    for (const r of bad) {
        const why = r.why ?? 'The server did not say why.';
        if (!seen.includes(why))
            seen.push(why);
    }
 ...
 */
function selectAllOffer(status, shown) {
    switch (status) {
        case 'ready':
        ...
        case 'partial':
            return {
                allowed: true,
                label: `Select the ${(0, format_1.num)(shown)} Shown`,
                note: `Your roster came back at its row limit, so this ticks the ${(0, format_1.num)(shown)} clients on this screen and not the ones past them. `
...
const RECIPIENTS_OK = { allowed: true, label: null, reason: null };
/**
 ...
 */
function guardRecipients(listStatus, segmentStatus, segment) {
    switch ((0, loadStatus_1.worstStatus)(listStatus, segmentStatus)) {
        case 'ready':
        ...
        case 'partial':
            return {
                ...
                reason: `Only part of ${segment} came back, so the number on this screen is the size of what loaded rather than the size of the segment. `
        ...
        case 'error':
            return {
                ...
                reason: `Who is in ${segment} could not be read. An empty list here means the read failed rather than that the segment is empty, `
 ...
 */
function bulkThreadNote(count) {
    if (count < 2)
        return null;
    return `Each of these ${(0, format_1.num)(count)} people gets this as an ordinary message from you, in their own thread. `
 ...
 */
function endCoachingBrief(targets) {
    const n = targets.length;
    if (n === 0) {
        return {
    ...
    const lead = n === 1
        ...
        : `All ${(0, format_1.num)(n)} of these come off your roster — ${who}.`;
    ...
    const linkedCost = linked.length
        ? `${linked.length === n ? (n === 1 ? 'They keep' : 'They all keep') : `${(0, format_1.num)(linked.length)} of them keep`} their account and everything logged in it, and ${linked.length === 1 ? 'they are' : 'they are each'} told the coaching has ended. `
    ...
    const handCost = hand.length
        ? `${hand.length === n ? (n === 1 ? 'This one is' : `All ${(0, format_1.num)(n)} are`) : `${(0, format_1.num)(hand.length)} of them — ${namesWithRest(hand.map((x) => x.name))} — are`} clients you added by hand, with no account behind them. `
    ...
    return {
        title: n === 1 ? 'Remove This Client?' : `Remove ${(0, format_1.num)(n)} Clients?`,
        body: parts.join('\n\n'),
        ...
        // forty people.
        confirmLabel: n === 1 ? 'Remove Them' : `Remove All ${(0, format_1.num)(n)}`,
