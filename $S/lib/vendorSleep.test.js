...
const vendorSleep_1 = require("./vendorSleep");
...
const ouraNight = (0, sleepMerge_1.nightKey)(OURA_WAKE);
const ouraOne = (0, vendorSleep_1.parseOuraSleep)([
    {
...
const ouraInBed = (0, vendorSleep_1.parseOuraSleep)([
...
const ouraRejected = (0, vendorSleep_1.parseOuraSleep)([
...
const ouraLateNap = (0, vendorSleep_1.parseOuraSleep)([
    { type: 'late_nap', day: '2026-08-26', bedtime_end: napEnd, total_sleep_duration: 3600 },
]);
ok(ouraLateNap[0]?.night === (0, sleepMerge_1.nightKey)(napEnd), `a late nap is filed on the evening it ended, not Oura's scoring day (got ${ouraLateNap[0]?.night})`);
// Two periods on one night are one total, summed in seconds and rounded once.
const ouraTwo = (0, vendorSleep_1.parseOuraSleep)([
...
const ouraSameDay = (0, sleepMerge_1.nightKey)('2026-08-29T14:00:00+03:00') === ouraNight;
if (ouraSameDay) {
    ok(ouraTwo.length === 1 && ouraTwo[0].minutesAsleep === Math.round(27722 / 60), `a nap adds to the night it ended on, got ${ouraTwo[0]?.minutesAsleep}`);
}
// Malformed durations contribute nothing at all.
for (const bad of [null, undefined, 0, -900, NaN, Infinity, '', '  ', 'seven hours', true, {}]) {
    const r = (0, vendorSleep_1.parseOuraSleep)([{ type: 'long_sleep', bedtime_end: OURA_WAKE, total_sleep_duration: bad, time_in_bed: bad }]);
    ok(r.length === 0, `total_sleep_duration ${JSON.stringify(bad)} yields no reading rather than a zero`);
}
ok((0, vendorSleep_1.parseOuraSleep)([{ type: 'long_sleep', bedtime_end: OURA_WAKE, total_sleep_duration: vendorSleep_1.MAX_RECORD_SECONDS + 1 }]).length === 0, 'a record claiming more than a day of sleep is refused rather than clamped');
ok((0, vendorSleep_1.parseOuraSleep)([{ type: 'long_sleep', bedtime_end: 'not a date', total_sleep_duration: 25920 }]).length === 0, 'a period whose end cannot be read belongs to no night');
ok((0, vendorSleep_1.parseOuraSleep)(null).length === 0 && (0, vendorSleep_1.parseOuraSleep)({}).length === 0 && (0, vendorSleep_1.parseOuraSleep)([null, 3]).length === 0, 'a response that is not a list of records reads as nothing');
// A numeric string is still a duration — none of these APIs is under our control.
ok((0, vendorSleep_1.parseOuraSleep)([{ bedtime_end: OURA_WAKE, total_sleep_duration: '25920' }])[0]?.minutesAsleep === 432, 'a numeric string is accepted as a duration');
// An unrecognised type is kept: refusing it would discard a real night.
ok((0, vendorSleep_1.parseOuraSleep)([{ type: 'something_new', bedtime_end: OURA_WAKE, total_sleep_duration: 25920 }]).length === 1, 'an unknown sleep type is still read, because discarding it would lose a real night');
// ── WHOOP ───────────────────────────────────────────────────────────────────
const WHOOP_END = '2026-08-29T13:25:44.774Z';
const whoopNight = (0, sleepMerge_1.nightKey)(WHOOP_END);
const stageSummary = {
...
const whoopOne = (0, vendorSleep_1.parseWhoopSleep)([
...
ok(whoopOne.length === 1 && whoopOne[0].minutesAsleep === expectWhoop, `WHOOP asleep is light + slow wave + REM = ${expectWhoop} min, got ${whoopOne[0]?.minutesAsleep}`);
...
ok(whoopOne[0]?.minutesAsleep !== Math.round(30272735 / 60000), 'time in bed is never reported as time asleep');
...
for (const state of ['PENDING_SCORE', 'UNSCORABLE']) {
    const r = (0, vendorSleep_1.parseWhoopSleep)([{ start: '2026-08-29T05:00:00Z', end: WHOOP_END, score_state: state }]);
...
const whoopPartial = (0, vendorSleep_1.parseWhoopSleep)([
...
ok(whoopPartial[0]?.minutesAsleep === Math.round((14905851 + 5879573) / 60000), `two readable stages still measure sleep, got ${whoopPartial[0]?.minutesAsleep}`);
// Scored, but no stage survived validation: in bed is then all WHOOP has said.
const whoopOnlyBed = (0, vendorSleep_1.parseWhoopSleep)([
...
ok(whoopOnlyBed.length === 1 && whoopOnlyBed[0].basis === 'in-bed', 'with no usable stage, the in-bed figure is reported and labelled in-bed');
ok((0, vendorSleep_1.parseWhoopSleep)([{ end: WHOOP_END, score_state: 'SCORED', score: { stage_summary: { total_light_sleep_time_milli: 0, total_slow_wave_sleep_time_milli: 0, total_rem_sleep_time_milli: 0, total_in_bed_time_milli: 0 } } }]).length === 0, 'an all-zero stage summary is no measurement at all');
ok((0, vendorSleep_1.parseWhoopSleep)([{ end: 'nonsense', score_state: 'SCORED', score: { stage_summary: stageSummary } }]).length === 0, 'a WHOOP sleep with an unreadable end belongs to no night');
// ── Fitbit ──────────────────────────────────────────────────────────────────
const fitbitOne = (0, vendorSleep_1.parseFitbitSleep)([
    {
...
ok(fitbitOne.length === 1 && fitbitOne[0].minutesAsleep === 432, `Fitbit reports minutes directly, got ${fitbitOne[0]?.minutesAsleep}`);
...
ok(fitbitOne[0]?.night === '2026-08-29', `a bare dateOfSleep is the calendar day it says, in every zone (got ${fitbitOne[0]?.night})`);
const fitbitClassic = (0, vendorSleep_1.parseFitbitSleep)([
...
const fitbitFallback = (0, vendorSleep_1.parseFitbitSleep)([
    { dateOfSleep: '2026-08-29', type: 'stages', minutesAsleep: null, timeInBed: 481 },
]);
ok(fitbitFallback.length === 1 && fitbitFallback[0].basis === 'in-bed' && fitbitFallback[0].minutesAsleep === 481, 'a log with no minutesAsleep falls back to timeInBed and says so');
// A main sleep and a nap on the same date are one night's total.
const fitbitNap = (0, vendorSleep_1.parseFitbitSleep)([
    { dateOfSleep: '2026-08-29', isMainSleep: true, minutesAsleep: 432 },
    { dateOfSleep: '2026-08-29', isMainSleep: false, minutesAsleep: 31 },
]);
ok(fitbitNap.length === 1 && fitbitNap[0].minutesAsleep === 463, `a nap adds to the same night, got ${fitbitNap[0]?.minutesAsleep}`);
ok((0, vendorSleep_1.parseFitbitSleep)([{ endTime: '2026-08-29T07:03:00.000', minutesAsleep: 432 }])[0]?.night === '2026-08-29', 'endTime is Fitbit local wall-clock time and is read as the day it names');
ok((0, vendorSleep_1.parseFitbitSleep)([{ dateOfSleep: '2026-08-29', minutesAsleep: 0, timeInBed: 0 }]).length === 0, 'a Fitbit log of zero minutes is no reading, not a night of no sleep');
// ── Dispatch, and the contract with the merge ───────────────────────────────
ok((0, vendorSleep_1.vendorReadsSleep)('oura') && (0, vendorSleep_1.vendorReadsSleep)('whoop') && (0, vendorSleep_1.vendorReadsSleep)('fitbit'), 'the three cloud sleep vendors are recognised');
ok(!(0, vendorSleep_1.vendorReadsSleep)('garmin') && !(0, vendorSleep_1.vendorReadsSleep)('apple') && !(0, vendorSleep_1.vendorReadsSleep)(''), 'a vendor with no sleep reader is not claimed to have one');
ok((0, vendorSleep_1.parseVendorSleep)('garmin', [{ dateOfSleep: '2026-08-29', minutesAsleep: 432 }]).length === 0, 'an unknown vendor reads nothing rather than guessing a parser');
ok((0, vendorSleep_1.parseVendorSleep)('fitbit', [{ dateOfSleep: '2026-08-29', minutesAsleep: 432 }], 'Tim’s Charge')[0]?.sourceName === 'Tim’s Charge', 'the display name passed in is what the screen will show');
...
const viaApi = (0, vendorSleep_1.parseOuraSleep)([{ bedtime_end: OURA_WAKE, total_sleep_duration: 25920 }])[0];
const viaHealth = {
    provider: 'apple', sourceId: 'com.ouraring.oura', sourceName: 'Oura',
    family: 'oura', basis: 'asleep', night: viaApi.night, minutesAsleep: 432,
};
const merged = (0, sleepMerge_1.mergeSleepNight)(viaApi.night, [viaApi, viaHealth]);
