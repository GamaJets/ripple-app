...
const close = (a, b, msg) => ok(a != null && Math.abs(a - b) < 0.05, `${msg} — got ${JSON.stringify(a)}, wanted about ${b}`);
/* ── 1. a metric sheet, which is what always worked ───────────────────────── */
const metric = (0, inbodySheet_1.parseInBodySheet)([
...
eq((0, inbodySheet_1.sheetMassKg)(metric.weight, metric.unit), 82.4, 'a metric mass is stored as it was printed');
/* ── 2. THE defect: a US sheet, in pounds ─────────────────────────────────── */
const imperial = (0, inbodySheet_1.parseInBodySheet)([
    'InBody 570 Body Composition Result Sheet',
    'Weight (lbs) 180.4',
    'SMM (lbs) 79.6',
    'PBF (%) 18.2',
...
close((0, inbodySheet_1.sheetMassKg)(imperial.weight, imperial.unit), 81.83, 'and it is converted once, on the way to the record');
close((0, inbodySheet_1.sheetMassKg)(imperial.muscle, imperial.unit), 36.11, 'the muscle mass carries the same unit as the weight, and converts with it');
...
ok(Math.round((0, inbodySheet_1.sheetMassKg)(180, 'lb') / 0.45359237) === 180, 'and reading the unit gives the member back their own 180');
/* ── 3. the third answer: a sheet that does not say ───────────────────────── */
const silent = (0, inbodySheet_1.parseInBodySheet)(['Weight 82.4', 'SMM 36.1', 'PBF 18.2'].join('\n'));
...
eq((0, inbodySheet_1.sheetMassKg)(silent.weight, silent.unit), 82.4, 'and are assumed metric, because the machine\'s default is');
ok(inbodySheet_1.ASSUMED_METRIC_NOTE.toLowerCase().includes('check'), 'and the screen is given a sentence that asks the member to check it');
ok(inbodySheet_1.CONVERTED_FROM_LB_NOTE.toLowerCase().includes('pounds'), 'and a different one for a sheet that was converted');
...
eq((0, inbodySheet_1.sheetUnit)('Height 178 cm\nWeight 82.4\nBasal Metabolic Rate 1720 kcal\nTarget 80 kg\nBone mass 3.2 lbs', ''), null, 'a document naming both units and neither on the weight line is ambiguous');
...
eq((0, inbodySheet_1.sheetUnit)('Height 5\' 10"\nWeight (kg) 82.4\nsomething about lbs', 'Weight (kg) 82.4'), 'kg', 'the weight line decides, even where the document mentions the other unit');
// And when the weight line lost its unit, the muscle line is the same
// statement about the same machine.
const smudged = (0, inbodySheet_1.parseInBodySheet)(['Weight 180.4', 'SMM (lbs) 79.6', 'PBF (%) 18.2'].join('\n'));
...
const withTargets = (0, inbodySheet_1.parseInBodySheet)([
    'Weight (kg) 82.4',
    'Target Weight 75.0',
    'Weight Control -7.4',
    'Ideal Weight 75.0',
...
const absurd = (0, inbodySheet_1.parseInBodySheet)(['Weight (kg) 500', 'PBF (%) 18.2'].join('\n'));
...
const okInLb = (0, inbodySheet_1.parseInBodySheet)(['Weight (lbs) 260', 'PBF (%) 18.2'].join('\n'));
eq(okInLb.weight, 260, 'while 260 IS a person in pounds, and is kept');
/* ── 7. nothing readable ──────────────────────────────────────────────────── */
const nothing = (0, inbodySheet_1.parseInBodySheet)('a photograph of the ceiling');
eq(nothing.ok, false, 'an unreadable sheet says so');
eq(nothing.weight, null, 'and hands back nothing rather than a zero');
eq((0, inbodySheet_1.sheetMassKg)(null, 'lb'), null, 'null in, null out');
eq((0, inbodySheet_1.sheetMassKg)(undefined, null), null, 'and so for an absent figure');
// "lb" inside a word — a member's name in a header, an ID — does not decide the
// unit of their scan.
eq((0, inbodySheet_1.sheetUnit)('Albert Halbrook\nWeight 82.4', 'Weight 82.4'), null, 'lb inside a word is not a unit');
if (errors.length) {
