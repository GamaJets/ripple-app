...
const reofferCopy_1 = require("./reofferCopy");
...
{
    const s = (0, reofferCopy_1.reofferConfirmation)({ offered: 12, recorded: 12 }, WHEN);
    ...
    ok(s.includes('Delivery depends on their notification settings.'), 'with the caveat that a recorded notification is still not a delivered one');
}
{
    const s = (0, reofferCopy_1.reofferConfirmation)({ offered: 1, recorded: 1 }, WHEN);
    eq(s.startsWith('All 1 of your client was sent a notification'), true, 'one client is singular on both the noun and the verb');
...
/* ── the substitution this module exists to stop ───────────────────────────── */
{
    ...
    const s = (0, reofferCopy_1.reofferConfirmation)({ offered: 12, recorded: 8 }, WHEN);
...
{
    const s = (0, reofferCopy_1.reofferConfirmation)({ offered: 2, recorded: 1 }, WHEN);
...
/* ── nobody ────────────────────────────────────────────────────────────────── */
{
    ...
    const s = (0, reofferCopy_1.reofferConfirmation)({ offered: 12, recorded: 0 }, WHEN);
...
{
    const s = (0, reofferCopy_1.reofferConfirmation)({ offered: 12, recorded: null }, WHEN);
...
/* ── numbers that are not numbers ──────────────────────────────────────────── */
{
    ...
    const s = (0, reofferCopy_1.reofferConfirmation)({ offered: 3, recorded: 5 }, WHEN);
...
{
    const s = (0, reofferCopy_1.reofferConfirmation)({ offered: Number.NaN, recorded: 0 }, WHEN);
    ok(!s.includes('NaN'), 'a count that is not a number never reaches the screen as the letters NaN');
}
{
    const s = (0, reofferCopy_1.reofferConfirmation)({ offered: 12, recorded: 8.7 }, WHEN);
 ...
 */
{
    const s = (0, reofferCopy_1.reofferConfirmation)({ offered: 12, recorded: 0, inboxKept: false }, WHEN);
    ok(!/recorded the notification for nobody/i.test(s), 'a row that was never going to be written is not reported as a row that failed');
    ...
    ok(/not kept in their notifications/i.test(s), 'and the consequence a coach can act on — nothing is waiting in the app — is said out loud');
    // The distinction is the whole point: a real zero still reads as a real zero.
    const failed = (0, reofferCopy_1.reofferConfirmation)({ offered: 12, recorded: 0, inboxKept: true }, WHEN);
    ok(/recorded the notification for nobody/i.test(failed), 'a kind the inbox DOES keep, recorded for nobody, still says so');
    ...
    ok((0, reofferCopy_1.reofferConfirmation)({ offered: 12, recorded: 0 }, WHEN) === failed, 'an absent flag changes nothing');
}
 ...
 */
{
    for (const out of [
        ...
        const plain = (0, reofferCopy_1.reofferConfirmation)(out, WHEN);
        const hedged = (0, reofferCopy_1.reofferConfirmation)({ ...out, partial: true }, WHEN);
        ok(hedged !== plain, 'every branch can say that the handset list was short');
        ok(hedged.includes(notifyCopy_1.PUSH_PARTIAL_NOTE), 'and says it in the one wording this product has for it');
        ok(!plain.includes(notifyCopy_1.PUSH_PARTIAL_NOTE), 'while a send with nothing wrong carries no warning');
    }
    // It is a fact about how far the push got, never about the slot or about how
    // many people were asked.
    const hedged = (0, reofferCopy_1.reofferConfirmation)({ offered: 12, recorded: 12, partial: true }, WHEN);
...
/* ── the whole point, stated once ──────────────────────────────────────────── */
{
    ...
    const short = (0, reofferCopy_1.reofferConfirmation)({ offered: 12, recorded: 8 }, WHEN);
    const whole = (0, reofferCopy_1.reofferConfirmation)({ offered: 12, recorded: 12 }, WHEN);
