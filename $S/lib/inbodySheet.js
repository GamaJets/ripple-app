...
exports.CONVERTED_FROM_LB_NOTE = exports.ASSUMED_METRIC_NOTE = void 0;
exports.sheetUnit = sheetUnit;
exports.parseInBodySheet = parseInBodySheet;
exports.sheetMassKg = sheetMassKg;
 ...
 */
const WEIGHT_BAND = { kg: [25, 300], lb: [55, 660] };
const MUSCLE_BAND = { kg: [10, 80], lb: [22, 176] };
...
const UNKNOWN_UNIT_ASSUMPTION = 'kg';
/**
 ...
 */
function sheetUnit(text, weightLine) {
    ...
    if (line) {
        ...
        if (lb && !kg)
            return 'lb';
        if (kg && !lb)
            return 'kg'; // unit-ok: as above — the printout's own word for its own unit.
    }
...
const numbersIn = (s) => (s.match(/\d{1,3}(?:\.\d)?/g) || []).map(Number);
/**
 ...
 */
function parseInBodySheet(text) {
    ...
    for (const ln of lines) {
        ...
        if (weight == null && low.includes('weight')
            ...
            if (cand.length) {
                weight = cand[cand.length - 1];
                weightLine = ln;
            }
        }
    }
    if (bodyFatPct == null) {
        const m = src.match(/PBF[^0-9]{0,12}(\d{1,2}(?:\.\d)?)/i);
        if (m)
            bodyFatPct = Number(m[1]);
    }
    if (muscle == null) {
        const m = src.match(/SMM[^0-9]{0,12}(\d{1,2}(?:\.\d)?)/i);
        if (m)
            muscle = Number(m[1]);
    }
    ...
    if (weight != null && (weight < band[0] || weight > band[1]))
        weight = null;
    const mband = MUSCLE_BAND[unit ?? UNKNOWN_UNIT_ASSUMPTION];
    if (muscle != null && (muscle < mband[0] || muscle > mband[1]))
        muscle = null;
 ...
 */
function sheetMassKg(value, unit) {
    if (value == null || !Number.isFinite(value))
        return null;
    return unit === 'lb' ? (0, units_1.lbToKg)(value) : value;
}
/** What the screen says when the printout did not name its unit. */
exports.ASSUMED_METRIC_NOTE = 'Your printout does not say whether it is in kilograms or pounds, so these were read as kilograms. Check the weight against the sheet before you save it.';
...
exports.CONVERTED_FROM_LB_NOTE = 'Your printout is in pounds — the figures were converted, so they may not read the same as the paper.';
