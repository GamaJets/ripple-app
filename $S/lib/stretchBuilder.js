...
exports.MAX_BUILD_MINUTES = exports.MIN_BUILD_MINUTES = exports.TOPUP_STEP_SEC = exports.MAX_BUILT_HOLD_SEC = exports.BUILD_HOLD_SEC = exports.BUILD_MINUTES = exports.STRETCH_FOCUS = exports.STRETCH_CATALOGUE = void 0;
exports.focusById = focusById;
exports.buildRoutine = buildRoutine;
...
const s = (id, name, area, muscle, sides, flow = false) => ({ id, name, area, muscle, sides, flow });
/**
...
function focusById(id) {
    ...
    if (!key)
        return null;
    return exports.STRETCH_FOCUS.find((f) => f.id === key) ?? null;
}
...
exports.BUILD_MINUTES = [5, 10, 15, 20];
 ...
 * more than 20.
 */
exports.MAX_BUILD_MINUTES = 60;
const fail = (problem) => ({ routine: null, problem, shortfall: null, canVary: false });
...
const stepOf = (c) => ({ id: c.id, name: c.name, holdSec: exports.BUILD_HOLD_SEC, sides: c.sides, flow: c.flow });
/**
 ...
 */
function buildRoutine(req) {
    const focusId = req.focus ?? 'whole-body';
    const focus = focusById(focusId);
    if (!focus)
        return fail('We do not have that part of the body as a stretch focus.');
    const catalogue = req.catalogue ?? exports.STRETCH_CATALOGUE;
    if (!catalogue.length)
        return fail('We have no stretches to build a routine from.');
    ...
    if (!Number.isFinite(minutes))
        return fail('That is not a length of time we can build a routine from.');
    if (minutes < exports.MIN_BUILD_MINUTES) {
        return fail(`Under ${exports.MIN_BUILD_MINUTES} minutes there is room for one position, which is a stretch rather than a routine.`);
    }
    if (minutes > exports.MAX_BUILD_MINUTES) {
        return fail(`We build routines up to ${exports.MAX_BUILD_MINUTES} minutes long.`);
    }
    const seed = Number.isFinite(req.seed) ? Math.trunc(req.seed) : 0;
    ...
    if (!pool)
        return fail(`We have no equipment-free stretches for your ${focus.phrase} yet.`);
    ...
    const picked = [];
    const steps = [];
    const asRoutine = (ss) => ({ id: 'fit', title: 'Fit', note: 'measured, never shown.', steps: ss });
    for (const cand of order) {
        const trial = [...steps, stepOf(cand.c)];
        if ((0, stretchRoutine_1.routineTotalSec)(asRoutine(trial)) > budget)
            continue;
    ...
    const passes = Math.ceil((exports.MAX_BUILT_HOLD_SEC - exports.BUILD_HOLD_SEC) / exports.TOPUP_STEP_SEC);
    for (let pass = 0; pass < passes; pass++) {
        for (const step of ordered) {
            if (step.holdSec + exports.TOPUP_STEP_SEC > exports.MAX_BUILT_HOLD_SEC)
                continue;
            const trial = ordered.map((x) => (x === step ? { ...x, holdSec: x.holdSec + exports.TOPUP_STEP_SEC } : x));
            if ((0, stretchRoutine_1.routineTotalSec)(asRoutine(trial)) > budget)
                continue;
            step.holdSec += exports.TOPUP_STEP_SEC;
        }
    }
    const routine = {
        ...
        title: `${(0, stretchRoutine_1.routineMinutes)(asRoutine(ordered))}-Minute ${focus.label}`,
        note: `Built for you from ${ordered.length} equipment-free ${ordered.length === 1 ? 'stretch' : 'stretches'}, spread across your ${focus.phrase}.`,
        steps: ordered,
    };
    const built = (0, stretchRoutine_1.routineMinutes)(routine);
    return {
        routine,
        problem: null,
 ...
 */
function buildProblems(built, minutes) {
    const problems = [];
    const r = built.routine;
    if (!r)
        return built.problem ? [] : ['there is no routine and no reason given for there not being one'];
    if (built.problem)
        problems.push('there is a routine and a problem sentence, and only one of those can be true');
    const total = (0, stretchRoutine_1.routineTotalSec)(r);
    if (total > minutes * 60) {
    ...
    if ((0, stretchRoutine_1.routineMinutes)(r) < minutes && !built.shortfall) {
        problems.push(`the routine is ${(0, stretchRoutine_1.routineMinutes)(r)} minutes against ${minutes} asked for, and nothing says so`);
    }
    if ((0, stretchRoutine_1.routineMinutes)(r) >= minutes && built.shortfall) {
    ...
    for (const step of r.steps) {
        if (seen.has(step.id))
            problems.push(`${step.id} is in the routine twice`);
        seen.add(step.id);
        if (step.holdSec < exports.BUILD_HOLD_SEC)
            problems.push(`${step.id} is held for ${step.holdSec}s, under the ${exports.BUILD_HOLD_SEC}s a built stretch starts at`);
        if (step.holdSec > exports.MAX_BUILT_HOLD_SEC)
            problems.push(`${step.id} is held for ${step.holdSec}s, over the ${exports.MAX_BUILT_HOLD_SEC}s ceiling the top-up works to`);
        if (step.holdSec % exports.TOPUP_STEP_SEC !== 0)
            problems.push(`${step.id} is held for ${step.holdSec}s, which is not a round number on a screen`);
    }
    return problems;
}
