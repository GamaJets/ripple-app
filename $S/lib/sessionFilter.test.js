...
const sessionFilter_1 = require("./sessionFilter");
const sessionHistory_1 = require("./sessionHistory");
const errors = [];
const ok = (cond, msg) => { if (!cond)
    errors.push(msg); };
const eq = (a, b, msg) => ok(Object.is(a, b), `${msg} — got ${JSON.stringify(a)}, wanted ${JSON.stringify(b)}`);
const row = (clientId, clientName) => ({ clientId, clientName });
const F = (over = {}) => ({ ...sessionFilter_1.NO_FILTER, ...over });
/* ── is anything narrowed ──────────────────────────────────────────────── */
ok(!(0, sessionFilter_1.filterActive)(sessionFilter_1.NO_FILTER), 'an untouched filter narrows nothing');
ok((0, sessionFilter_1.filterActive)(F({ clientId: 'a' })), 'a picked client narrows');
ok((0, sessionFilter_1.filterActive)(F({ state: 'delivered' })), 'so does a picked outcome');
ok((0, sessionFilter_1.filterActive)(F({ text: 'ana' })), 'and typed text');
// A box holding a space is not a search. Reporting it as one sends a coach
// hunting for a control they never touched.
ok(!(0, sessionFilter_1.filterActive)(F({ text: '   ' })), 'whitespace on its own is not a search');
/* ── text ──────────────────────────────────────────────────────────────── */
ok((0, sessionFilter_1.matchesText)('Ana Ruiz', 'ana'), 'the match is case-folded');
ok((0, sessionFilter_1.matchesText)('Ana Ruiz', ' RUIZ '), 'and the query is trimmed');
ok((0, sessionFilter_1.matchesText)('Ana Ruiz', ''), 'an empty query matches everybody rather than nobody');
ok(!(0, sessionFilter_1.matchesText)(null, 'ana'), 'a row with no name matches a real search');
ok((0, sessionFilter_1.matchesText)(null, '  '), 'but is not excluded by an empty one');
/* ── who is in the window ──────────────────────────────────────────────── */
const rows = [
    row('c2', 'Ben ODwyer'), row('c1', 'Ana Ruiz'), row('c1', 'Ana Ruiz'),
    row(null, null), row('c3', ''),
];
const opts = (0, sessionFilter_1.clientOptions)(rows);
...
/* ── the filter itself ─────────────────────────────────────────────────── */
eq((0, sessionFilter_1.filterSessions)(rows, sessionFilter_1.NO_FILTER).length, 5, 'no filter drops nothing');
eq((0, sessionFilter_1.filterSessions)(rows, F({ clientId: 'c1' })).length, 2, 'a client filter keeps only theirs');
...
eq((0, sessionFilter_1.filterSessions)(sams, F({ clientId: 's1' })).length, 1, 'the filter is on the id, so two clients named Sam stay two clients');
eq((0, sessionFilter_1.filterSessions)(rows, F({ clientId: 'c1', text: 'ben' })).length, 0, 'the clauses are ANDed, not ORed');
// An unattributed row cannot be reached by a client filter, and that is stated
// behaviour rather than an accident.
eq((0, sessionFilter_1.filterSessions)(rows, F({ clientId: 'c1' })).some((r) => r.clientId === null), false, 'a row with no client id is never matched by a client filter');
const stated = [
    { ...row('a', 'A'), s: 'delivered' },
    { ...row('b', 'B'), s: 'unmarked' },
    { ...row('c', 'C'), s: 'delivered' },
...
const stateOf = (r) => r.s;
eq((0, sessionFilter_1.filterSessions)(stated, F({ state: 'delivered' }), stateOf).length, 2, 'the state clause uses the caller’s verdict');
...
eq((0, sessionFilter_1.filterSessions)(stated, F({ state: 'delivered' })).length, 3, 'with no verdict function the state clause does not apply at all, rather than matching nothing');
/* ── the counts ────────────────────────────────────────────────────────── */
const counts = (0, sessionFilter_1.stateCounts)(stated, stateOf, sessionHistory_1.PAST_STATES);
...
for (const s of sessionHistory_1.PAST_STATES)
    ok(s in counts, `${s} has a count even when it is zero`);
eq(counts.cancelled, 0, 'and a state nothing is in is zero rather than absent');
/* ── what is said about what is shown ──────────────────────────────────── */
eq((0, sessionFilter_1.filterLine)(4, 4, sessionFilter_1.NO_FILTER), null, 'an unfiltered list gets no line, because a permanent "4 of 4" is furniture');
const line = (0, sessionFilter_1.filterLine)(2, 88, F({ clientId: 'c1' }), 'Ana Ruiz');
...
// and a blank is not.
const anon = (0, sessionFilter_1.filterLine)(2, 88, F({ clientId: 'gone' }));
ok(anon !== null && /one client/.test(anon), 'a pick whose name is no longer in the rows still reads as one client');
const both = (0, sessionFilter_1.filterLine)(1, 9, F({ clientId: 'c', text: 'an', state: 'delivered' }), 'Ana');
ok(both !== null && /and/.test(both), 'three clauses are joined into one sentence rather than three');
/* ── AND THE ONE THAT MATTERS ──────────────────────────────────────────── */
const empty = (0, sessionFilter_1.emptyFilterLine)(88, F({ text: 'zzz' }));
...
ok(!/you have none|nothing outstanding|all caught up|no sessions of yours/i.test(empty), 'and never says the coach has none — that is the sentence a settlement gets cleared by');
if (errors.length) {
