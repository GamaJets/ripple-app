...
const dayClasses_1 = require("./dayClasses");
const errors = [];
...
const mine = cls({ id: 'a' });
eq((0, dayClasses_1.classesOnDay)([mine], THURSDAY, ME).length, 1, 'a coach sees the class they are teaching');
eq((0, dayClasses_1.classesOnDay)([mine], THURSDAY, ME)[0].mine, true, 'and it is marked as theirs');
/* ── the local-day rule ─────────────────────────────────────────────────── */
...
eq((0, dayClasses_1.classesOnDay)([evening], THURSDAY, ME).length, 1, 'an evening class is on the evening it is taught');
// And the day before does not swallow it.
eq((0, dayClasses_1.classesOnDay)([evening], new Date(2026, 8, 2, 9), ME).length, 0, 'and not on the day before');
eq((0, dayClasses_1.classesOnDay)([evening], new Date(2026, 8, 4, 9), ME).length, 0, 'nor the day after');
// The early one is the mirror image: 6am on the 3rd is the 2nd in UTC terms
// east of Greenwich.
const early = cls({ id: 'early', startsAt: at(2026, 9, 3, 6) });
eq((0, dayClasses_1.classesOnDay)([early], THURSDAY, ME).length, 1, 'and a morning class is on its own morning');
/* ── whose classes are drawn ────────────────────────────────────────────── */
const theirs = cls({ id: 'theirs', trainerId: 'coach-2' });
const nobodys = cls({ id: 'nobodys', trainerId: null });
const sheet = (0, dayClasses_1.classesOnDay)([mine, theirs, nobodys], THURSDAY, ME);
...
const noUid = (0, dayClasses_1.classesOnDay)([mine, theirs, nobodys], THURSDAY, null);
...
eq((0, dayClasses_1.classesOnDay)([off], THURSDAY, ME).length, 0, 'a cancelled class frees the hour and is not drawn');
...
eq((0, dayClasses_1.classesOnDay)([cls({ id: 'old', status: undefined })], THURSDAY, ME).length, 1, 'a class with no status is a class that is on');
/* ── order ──────────────────────────────────────────────────────────────── */
const three = (0, dayClasses_1.classesOnDay)([
...
eq((0, dayClasses_1.classesOnDay)([cls({ id: 'broken', startsAt: 'not a date' })], THURSDAY, ME).length, 0, 'an unreadable date lands on no day at all');
/* ── what the rows and the sheet say ────────────────────────────────────── */
ok(/Your class/.test((0, dayClasses_1.classDayNote)({ ...mine, mine: true })), 'the coach’s own class says so');
ok(/no coach recorded/.test((0, dayClasses_1.classDayNote)({ ...nobodys, mine: false })), 'and an unattributed one says what is actually unknown about it');
/* ── the caveat, which is the whole empty-list rule ─────────────────────── */
eq((0, dayClasses_1.classDayCaveat)('ready'), null, 'a whole read needs no caveat');
for (const status of ['loading', 'partial', 'error']) {
    const note = (0, dayClasses_1.classDayCaveat)(status) ?? '';
    ok(note.length > 0, `'${status}' says something`);
    ok(!/no classes that day|nothing scheduled/i.test(note), `and '${status}' never states the day is clear`);
}
ok(/not a free evening/.test((0, dayClasses_1.classDayCaveat)('error') ?? ''), 'a failed read is named as a failed read and not as an empty day');
ok(/row limit/.test((0, dayClasses_1.classDayCaveat)('partial') ?? ''), 'and a truncated one as truncated');
/* ── the heading ────────────────────────────────────────────────────────── */
eq((0, dayClasses_1.classDayHeading)(0), null, 'nothing to head means no heading');
eq((0, dayClasses_1.classDayHeading)(1), 'Class that day', 'one is counted in the singular');
ok(/2/.test((0, dayClasses_1.classDayHeading)(2) ?? ''), 'and more than one carries the count');
if (errors.length) {
