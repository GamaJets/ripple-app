...
exports.DEVICE_AVATAR_NOTE_COACH = exports.AVATAR_NO_SERVER_NOTE = exports.AVATAR_UPLOAD_FAILED_NOTE = exports.DEVICE_AVATAR_NOTE = exports.AVATAR_MAX_PX = exports.MAX_AVATAR_BYTES = exports.AVATAR_BUCKET = void 0;
exports.avatarObjectKey = avatarObjectKey;
exports.avatarRefusal = avatarRefusal;
exports.isDeviceAvatar = isDeviceAvatar;
exports.avatarSource = avatarSource;
/** The bucket from supabase/parts/961. */
exports.AVATAR_BUCKET = 'avatars';
/**
 ...
 */
exports.MAX_AVATAR_BYTES = 2 * 1024 * 1024;
 ...
 */
function avatarObjectKey(uid, token) {
    const id = (uid ?? '').trim();
    const tok = (token ?? '').trim().toLowerCase();
    if (!id)
        throw new Error('An avatar key needs the signed-in id.');
    if (!/^[a-z0-9]{8,}$/.test(tok))
        throw new Error('An avatar key needs a random token.');
    return `${id}/${tok}.jpg`;
}
/** Why these bytes cannot be stored, or null when they can. */
function avatarRefusal(byteLength) {
    if (!Number.isFinite(byteLength) || byteLength <= 0) {
        return 'That image could not be read off your phone, so nothing was uploaded.';
    }
    if (byteLength > exports.MAX_AVATAR_BYTES) {
 ...
 */
exports.DEVICE_AVATAR_NOTE = 'Your photo was saved as a file on the phone that chose it, so nobody else could ever see it — your coach has been looking at a blank circle. Choose it again and it will be uploaded properly this time.';
/** What to say when the upload did not happen. Never "saved". */
exports.AVATAR_UPLOAD_FAILED_NOTE = 'Your photo was not uploaded, so it has not been changed. Nothing was saved anywhere and you can try again in a moment.';
/** What to say when the build has no server behind it. */
exports.AVATAR_NO_SERVER_NOTE = 'This build has no server, so a photo has nowhere to go and would not reach your coach.';
/**
 ...
 */
exports.DEVICE_AVATAR_NOTE_COACH = 'Your photo was saved as a file on the phone that chose it, so nobody else has ever been able to see it — '
