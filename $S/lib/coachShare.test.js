...
/* ── the stored answer ─────────────────────────────────────────────────── */
eq((0, coachShare_1.consentFromStored)((0, coachShare_1.storedConsent)('yes')), 'yes', 'an answer of yes is written and read back as yes');
eq((0, coachShare_1.consentFromStored)((0, coachShare_1.storedConsent)('no')), 'no', 'and an answer of no survives the round trip too');
...
eq((0, coachShare_1.consentFromStored)(null), 'unasked', 'a device that has never been asked has not answered');
eq((0, coachShare_1.consentFromStored)(undefined), 'unasked', 'however the read came back empty');
...
eq((0, coachShare_1.consentFromStored)('{not json'), 'unasked', 'a blob that will not parse is not an answer');
eq((0, coachShare_1.consentFromStored)('null'), 'unasked', 'and neither is a parsed null');
eq((0, coachShare_1.consentFromStored)('[]'), 'unasked', 'nor an array');
eq((0, coachShare_1.consentFromStored)('"yes"'), 'unasked', 'nor the word yes on its own');
eq((0, coachShare_1.consentFromStored)('{"shareHealth":"true"}'), 'unasked', 'the string "true" is damage, not consent — only a real boolean counts');
eq((0, coachShare_1.consentFromStored)('{"somethingElse":true}'), 'unasked', 'a blob from a version that stored a different key has not answered this question');
...
eq((0, coachShare_1.shareableContext)(FULL, 'unknown'), null, 'while the stored answer is still being read, nothing may be sent — the read is asynchronous and this window is real');
eq((0, coachShare_1.shareableContext)(FULL, 'unasked'), null, 'and a member who has never been asked has not agreed to anything');
/* ── what goes on each answer ──────────────────────────────────────────── */
const no = (0, coachShare_1.shareableContext)(FULL, 'no');
const yes = (0, coachShare_1.shareableContext)(FULL, 'yes');
ok(no != null && yes != null, 'an answered consent produces a payload');
for (const k of coachShare_1.FITNESS_KEYS) {
    ok(no != null && k in no, `no: ${k} still goes — declining health details must not break the coach`);
    ok(yes != null && k in yes, `yes: ${k} goes`);
}
for (const k of coachShare_1.HEALTH_KEYS) {
...
for (const [label, payload] of [['no', no], ['yes', yes]]) {
...
const withNewField = (0, coachShare_1.shareableContext)({ ...FULL, homeAddress: '12 Example Street' }, 'yes');
ok(withNewField != null && !('homeAddress' in withNewField), 'a field added to the screen and not declared here is not transmitted — the default for anything new is "not sent"');
// Undefined is dropped rather than sent as a null the prompt would print.
const sparse = (0, coachShare_1.shareableContext)({ goal: 'lose fat', streak: undefined }, 'no');
...
for (const k of coachShare_1.HEALTH_KEYS) {
    ok(!coachShare_1.FITNESS_KEYS.includes(k), `${k} is health-only and is not also in the always-sent list`);
}
...
const line = (0, coachShare_1.sharedInjuries)(injuries);
...
eq((0, coachShare_1.sharedInjuries)([]), '', 'nothing active produces nothing, and the caller decides what to say about that');
eq((0, coachShare_1.sharedInjuries)(), '', 'and an undefined list is the same');
...
// unlocks the note.
const withInjuries = (0, coachShare_1.shareableContext)({ ...FULL, injuries: (0, coachShare_1.sharedInjuries)(injuries) }, 'yes');
ok(withInjuries != null && !JSON.stringify(withInjuries).includes('meniscus'), 'even the member who agreed to share their injuries does not send the report’s words');
/* ── the member is told the truth, and it is the same truth ────────────── */
ok(coachShare_1.ALWAYS_SENT.length > 0 && coachShare_1.SENT_WITH_PERMISSION.length > 0 && coachShare_1.NEVER_SENT.length > 0, 'all three lists say something');
ok(coachShare_1.NEVER_SENT.some((s) => /name/i.test(s)), 'the never-sent list names the name, because that is the field that was there and is now gone');
ok(coachShare_1.NEVER_SENT.some((s) => /note|document/i.test(s)), 'and the injury note, because that is the rule this was breaking');
ok(coachShare_1.ALWAYS_SENT.some((s) => /what you type/i.test(s)), 'and the always-sent list starts with the obvious one — a chat sends what you typed, and a disclosure that omits it looks like it is hiding something');
ok(/Anthropic|Claude/.test(coachShare_1.WHERE_IT_GOES), 'where it goes is named rather than described as "our systems"');
...
ok(/injur/i.test(coachShare_1.WITHHELD_NOTE), 'turning it off says what happens to injuries, which is the consequence that can hurt somebody');
ok(!/personalis|personaliz/i.test(coachShare_1.WITHHELD_NOTE), 'and does not hide behind "less personalised"');
ok(/doctor|physio/i.test(coachShare_1.NOT_MEDICAL_ADVICE), 'the disclaimer sends people to a professional');
...
const both = ['yes', 'no'];
for (const c of both)
    ok((0, coachShare_1.shareableContext)(FULL, c) != null, `${c} is a usable answer, not just the one we wanted`);
...
eq((0, coachShare_1.weeklyFacts)(FITNESS, HEALTH, 'unknown'), null, 'nothing is written from a week before the stored answer has been read');
eq((0, coachShare_1.weeklyFacts)(FITNESS, HEALTH, 'unasked'), null, 'and nothing before the question has been put at all');
const noWeek = (0, coachShare_1.weeklyFacts)(FITNESS, HEALTH, 'no');
const yesWeek = (0, coachShare_1.weeklyFacts)(FITNESS, HEALTH, 'yes');
...
const withBlanks = (0, coachShare_1.weeklyFacts)(['a fact.', '', '   '], ['', 'another.'], 'yes');
...
const named = (0, coachShare_1.weeklyFacts)(['Sarah trained 4 times.'], [], 'yes');
eq(named.length, 1, 'this function does not invent or strip prose — the screen owns what it writes');
ok(!JSON.stringify((0, coachShare_1.weeklyFacts)(FITNESS, HEALTH, 'yes')).includes('name'), 'and nothing keyed "name" is added on the way through');
ok(/training only/i.test(coachShare_1.REPORT_WITHHELD_NOTE), 'declining says the summary is still written, from the training half');
ok(/yours to read/i.test(coachShare_1.REPORT_WITHHELD_NOTE), 'and that the figures on the screen are unaffected');
ok(!/personalis|personaliz/i.test(coachShare_1.REPORT_WITHHELD_NOTE), 'and does not hide behind "less personalised"');
ok(String(coachShare_1.REPORT_CONSENT_TITLE).length > 10 && String(coachShare_1.REPORT_CONSENT_BODY).length > 80, 'the question put on the report is a real explanation rather than a label');
ok(/language model/i.test(coachShare_1.REPORT_CONSENT_BODY), 'and it names what the paragraph actually is before anybody agrees to it');
if (errors.length) {
