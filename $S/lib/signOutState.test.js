...
/* ── 1 · the four that go ──────────────────────────────────────────────── */
{
    ok((0, signOutState_1.isPersonalDeviceKey)('repple.notifyPrefs'), 'the notification categories and quiet hours are cleared');
    ok((0, signOutState_1.isPersonalDeviceKey)('repple.appLock.enabled'), 'so is the biometric lock, which was left armed by somebody who had gone');
    ok((0, signOutState_1.isPersonalDeviceKey)('repple.reminders'), 'and so are the reminders, which were scheduled on the phone itself');
    eq(signOutState_1.PERSONAL_DEVICE_KEYS.length, 3, 'and nothing has joined the list without a line in this file about it');
}
/* ── 2 · what must not go ──────────────────────────────────────────────── */
{
    for (const k of signOutState_1.KEPT_ON_SIGN_OUT) {
        ok(!(0, signOutState_1.isPersonalDeviceKey)(k), `${k} survives a sign-out — it is this device's, not this person's`);
    }
    ...
    ok(!signOutState_1.PERSONAL_DEVICE_KEYS.includes('repple.appName'), 'the gym name cached for the signed-out screens is never cleared');
}
/* ── 3 · nothing account-scoped ────────────────────────────────────────── */
{
    for (const k of signOutState_1.PERSONAL_DEVICE_KEYS) {
        for (const p of signOutState_1.ACCOUNT_SCOPED_PREFIXES) {
    ...
    ok(new Set(signOutState_1.PERSONAL_DEVICE_KEYS).size === signOutState_1.PERSONAL_DEVICE_KEYS.length, 'no key is listed twice');
    ok(!(0, signOutState_1.isPersonalDeviceKey)('outbox:v1:abc'), 'an outbox is a person’s unsent work and survives their sign-out');
}
if (errors.length) {
