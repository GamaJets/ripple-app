...
eq((0, wroteRows_1.writeFailure)('That membership', { error: null, count: 1 }), null, 'one row changed, no error — the only outcome that may be reported as saved');
eq((0, wroteRows_1.writeFailure)('That membership', { error: null, count: 12 }), null, 'several rows changed is equally a pass; the rule is "at least one", not "exactly one"');
// ── zero rows is the bug, and it must not be silent ───────────────────────
ok((0, wroteRows_1.writeFailure)('That membership', { error: null, count: 0 }) !== null, 'ZERO ROWS WITH NO ERROR MUST FAIL — this is the entire point of the module, and the shape a refused RLS write arrives in');
...
{
    const why = (0, wroteRows_1.writeFailure)('That membership', { error: null, count: 0 }) ?? '';
    ok(why.includes('That membership'), 'the message names the thing in the owner’s words, so the alert reads as a sentence rather than a field name');
    ok(/not changed|no rows|matched no/i.test(why), 'and it states that nothing changed — an owner who reads it must not be left thinking the write half-landed');
}
// ── a missing count is not a pass ─────────────────────────────────────────
//
// The regression guard for every call site that has not been converted yet.
ok((0, wroteRows_1.writeFailure)('That shift', { error: null, count: null }) !== null, 'a null count is NOT confirmation — it means the caller forgot { count: \'exact\' }, and treating it as success re-admits the whole bug');
ok((0, wroteRows_1.writeFailure)('That shift', { error: null }) !== null, 'an absent count field is the same omission as a null one');
ok((0, wroteRows_1.writeFailure)('That shift', {}) !== null, 'and an empty result — which is what a stubbed or mocked client hands back — is not a pass either');
...
{
    const missing = (0, wroteRows_1.writeFailure)('That shift', { error: null, count: null }) ?? '';
    const zero = (0, wroteRows_1.writeFailure)('That shift', { error: null, count: 0 }) ?? '';
    ok(missing !== zero, 'a missing count and a zero count are different diagnoses and must not produce the same sentence');
    ok(/did not say|whether/i.test(missing), 'the missing-count message names the omission rather than blaming the row');
}
...
ok((0, wroteRows_1.writeFailure)('That plan', { error: new Error('permission denied'), count: 0 }) !== null, 'an error with zero rows fails');
ok((0, wroteRows_1.writeFailure)('That plan', { error: new Error('permission denied'), count: 1 }) !== null, 'AN ERROR WITH ONE ROW STILL FAILS — the count must never be able to argue a refusal away');
{
    const withError = (0, wroteRows_1.writeFailure)('That plan', { error: { message: 'nope' }, count: 1 }) ?? '';
    ok(/could not be saved/i.test(withError), 'and it says the save failed rather than describing a row count the caller should not be reading');
}
...
const bareErr = (0, wroteRows_1.writeFailure)('That plan', { error: { code: '42501', message: 'row-level security' }, count: 1 });
ok(bareErr !== null, 'a bare PostgREST error object is an error; it does not have to be an Error instance');
ok(/could not be saved/i.test(bareErr ?? ''), 'and it is diagnosed as a refusal, not as a row count — those send a reader to two different places');
...
eq((0, wroteRows_1.writeFailure)('That equipment', { error: undefined, count: 1 }), null, 'an undefined error with a row touched is a pass, same as an explicit null');
...
const threw = (r) => {
    try {
        (0, wroteRows_1.assertWrote)('That thing', r);
        return null;
    }
    catch (e) {
        return e.message;
...
eq(threw({ error: null, count: 0 }), (0, wroteRows_1.writeFailure)('That thing', { error: null, count: 0 }), 'and it throws THE SAME SENTENCE, so the alert an owner reads is the one asserted above');
if (errors.length) {
