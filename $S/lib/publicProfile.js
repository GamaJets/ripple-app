...
exports.PUBLIC_REVIEW_NOTE = exports.PUBLIC_CLAIM_NOTE = exports.WITHHELD_FIELDS = exports.PUBLISHED_FIELDS = exports.RESERVED_HANDLES = exports.HANDLE_MAX = exports.HANDLE_MIN = void 0;
exports.normaliseHandle = normaliseHandle;
exports.handleProblem = handleProblem;
exports.handleProblemText = handleProblemText;
exports.publicPageUrl = publicPageUrl;
exports.publicJoinUrl = publicJoinUrl;
exports.publicPageState = publicPageState;
exports.publicPageStateNote = publicPageStateNote;
exports.asPublishResult = asPublishResult;
exports.publishOutcome = publishOutcome;
exports.publishableCredentials = publishableCredentials;
exports.publicFee = publicFee;
exports.pageState = pageState;
exports.pageStateEyebrow = pageStateEyebrow;
exports.pageStateHeading = pageStateHeading;
exports.pageStateNote = pageStateNote;
const coachCredentials_1 = require("./coachCredentials");
/* ── the address ──────────────────────────────────────────────────────────── */
/** Shorter than this is not an address, it is a collision waiting to happen. */
exports.HANDLE_MIN = 3;
/** Longer than this is not something anybody types off a phone screen. */
exports.HANDLE_MAX = 30;
/**
 ...
 * itself.
 */
exports.RESERVED_HANDLES = Object.freeze([
...
function handleProblem(handle) {
    ...
    if (!h)
        return 'empty';
    if (h.length < exports.HANDLE_MIN)
        return 'too-short';
    if (h.length > exports.HANDLE_MAX)
        return 'too-long';
    if (h.startsWith('-') || h.endsWith('-'))
        return 'edge-hyphen';
    if (exports.RESERVED_HANDLES.includes(h))
        return 'reserved';
    return 'ok';
}
function handleProblemText(p) {
    switch (p) {
        case 'ok': return '';
        case 'empty': return 'Pick a short address for your page, like your name or your business.';
        case 'too-short': return `An address is at least ${exports.HANDLE_MIN} characters.`;
        case 'too-long': return `Keep it to ${exports.HANDLE_MAX} characters or fewer.`;
 ...
 */
function publicPageUrl(origin, handle) {
    const h = normaliseHandle(handle);
    if (handleProblem(h) !== 'ok')
        return null;
 ...
 */
function publicJoinUrl(origin, joinCode) {
 ...
 */
function publicPageStateNote(s) {
    switch (s) {
        case 'off-directory':
 ...
 * returns.
 */
exports.PUBLISHED_FIELDS = Object.freeze([
 ...
 */
exports.WITHHELD_FIELDS = Object.freeze([
 ...
 */
function publishOutcome(r, handle) {
    const h = normaliseHandle(handle);
    switch (r) {
        case 'published':
        ...
        case 'invalid':
            return { title: 'Address not saved', body: `Use ${exports.HANDLE_MIN} to ${exports.HANDLE_MAX} letters, numbers and hyphens, starting and ending with a letter or a number.`, changed: false };
        case 'reserved':
 ...
 * is kind then title.
 */
function publishableCredentials(list, today) {
    if (list === null)
        return null;
    return list.filter((c) => c.verification === 'self_declared' && (0, coachCredentials_1.credentialState)(c, today) !== 'expired');
}
/**
 ...
 */
exports.PUBLIC_CLAIM_NOTE = 'These are what the coach says about themselves. Repple has not seen the certificates and has not checked them with the awarding bodies, so ask to see them or look the registration number up yourself.';
/**
 ...
 */
exports.PUBLIC_REVIEW_NOTE = 'Reviews are written by this coach’s own clients inside the app. Repple does not publish what they wrote or who they are, so the number is all that appears here.';
/**
 ...
 * for nothing.
 */
function publicFee(fee, currency) {
 ...
 */
function pageState(status, handle, found) {
    if (handleProblem(normaliseHandle(handle)) !== 'ok')
        return 'no-address';
    if (status === 'loading')
        return 'loading';
    if (status !== 'ready')
