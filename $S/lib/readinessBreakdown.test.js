...
const readinessBreakdown_1 = require("./readinessBreakdown");
const readiness_1 = require("./readiness");
const errors = [];
const ok = (cond, msg) => { if (!cond)
    errors.push(msg); };
const eq = (a, b, msg) => ok(Object.is(a, b), `${msg} — got ${JSON.stringify(a)}, wanted ${JSON.stringify(b)}`);
const night = (n, from, hours) => ({ night: `2026-08-${String(30 - n).padStart(2, '0')}`, hours, from });
...
const scored = (0, readiness_1.readinessScore)({ avgSleepHours: 7.5, hydrationPct: 0.5, recoveryPct: null, workoutsLast2Days: 1 });
...
const br = (over = {}) => (0, readinessBreakdown_1.readinessBreakdown)({ ...whole, ...over });
const lineFor = (b, key) => b.lines.find((l) => l.key === key);
...
{
    const good = { name: 'Oura Ring', status: 'ready', nights: 3 };
    const bad = { name: 'WHOOP', status: 'error', nights: 0 };
    const cannot = { name: 'Health Connect', status: 'unsupported', nights: 0 };
    eq((0, readinessBreakdown_1.deviceSleepTrust)('ready', []), 'ready', 'no device connected is not a broken device — nothing was asked and nothing is missing');
    eq((0, readinessBreakdown_1.deviceSleepTrust)('ready', [good]), 'ready', 'one provider that answered is a whole read');
    ...
    eq((0, readinessBreakdown_1.deviceSleepTrust)('ready', [good, bad]), 'partial', 'one of two providers failing is a real but SHORT read — not a ready one');
    eq((0, readinessBreakdown_1.deviceSleepTrust)('ready', [bad]), 'error', 'every provider failing means we know nothing about their nights');
    eq((0, readinessBreakdown_1.deviceSleepTrust)('ready', [bad, bad]), 'error', 'and that holds however many of them there are');
    // 'unsupported' is a settled fact about this build, not a gap in tonight.
    eq((0, readinessBreakdown_1.deviceSleepTrust)('ready', [cannot]), 'ready', 'Health Connect not reporting sleep is not a failed read');
    eq((0, readinessBreakdown_1.deviceSleepTrust)('ready', [good, cannot]), 'ready', 'and it does not drag a working provider down with it');
    eq((0, readinessBreakdown_1.deviceSleepTrust)('ready', [bad, cannot]), 'error', 'nor does it stand in for one — an unsupported provider cannot rescue a failed walk');
    eq((0, readinessBreakdown_1.deviceSleepTrust)('error', [good]), 'error', 'a walk that never completed is an error whatever the rows say');
    eq((0, readinessBreakdown_1.deviceSleepTrust)('loading', [bad]), 'loading', 'and a read still in flight has not failed yet');
    eq((0, readinessBreakdown_1.deviceSleepTrust)('partial', [good]), 'partial', 'a truncated walk stays truncated');
}
// ── a score over a short read is 'partial', and says which device ─────────
{
    ...
    eq(b.caveats[0], 'WHOOP could not be read, so a night it measured may be missing from this.', 'and it names the device, because "a device" sends nobody anywhere');
}
{
    const b = br({ sources: [
    ...
    eq(b.caveats[0], 'Oura Ring and WHOOP could not be read, so a night they measured may be missing from this.', 'two devices are listed and the verb agrees with them');
}
{
    ...
    eq(b.caveats[0], 'We could not reach your devices just now, so a night one of them measured may be missing from this.', 'and it says the read failed rather than naming a device that never answered');
...
// ── the whole read: nothing unread, nothing to caveat ─────────────────────
{
    ...
    eq(b.lines.map((l) => l.title).join(','), 'Sleep,Device Recovery,Hydration,Recent Sessions', 'titles are Title Case, per the house rule for a label beside a value');
...
// teaches them to ignore the word on the day it means something.
{
    ...
    eq(lineFor(b, 'sleep').detail, '7h 30m a night over 2 of the last 3 nights, all measured by a device', 'but the line says how many nights it ran over, because the average cannot');
}
{
    eq(lineFor(br(), 'sleep').detail, '7h 30m a night over the last 3 nights, all measured by a device', 'a full window says so plainly rather than "3 of the last 3"');
    eq(lineFor(br({ sleep: sleepOf(1, 0) }), 'sleep').detail, '7h 30m a night over 1 of the last 3 nights, measured by a device', 'one device night is singular');
    eq(lineFor(br({ sleep: sleepOf(0, 1) }), 'sleep').detail, '7h 30m a night over 1 of the last 3 nights, from a night you logged', 'a typed night says it was typed — a memory and a measurement are not the same fact');
    eq(lineFor(br({ sleep: sleepOf(0, 3) }), 'sleep').detail, '7h 30m a night over the last 3 nights, all from the nights you logged', 'and three of them likewise');
    eq(lineFor(br({ sleep: sleepOf(2, 1) }), 'sleep').detail, '7h 30m a night over the last 3 nights, 2 measured by a device, 1 from the nights you logged', 'a mixed window splits the two, because they are different kinds of evidence');
}
// ── hydration: untracked and unread are one null and two sentences ────────
{
    const untracked = br({ hydrationGoal: false, hydrationPct: null, readiness: (0, readiness_1.readinessScore)({ avgSleepHours: 7.5, hydrationPct: null, recoveryPct: null, workoutsLast2Days: 1 }) });
    const h = lineFor(untracked, 'hydration');
    eq(h.state, 'not-tracked', 'no goal set is not a failure');
    eq(h.detail, 'not in the scale — you have not set a daily water goal', 'and it says the signal LEFT the scale, because a member reading a lower number will assume they were docked for it');
...
{
    const unread = br({ hydrationStatus: 'error', hydrationPct: null, readiness: (0, readiness_1.readinessScore)({ avgSleepHours: 7.5, hydrationPct: null, recoveryPct: null, workoutsLast2Days: 1 }) });
    ...
    eq(unread.caveats[0], "Today's water count could not be read, so hydration is not in the scale.", 'with a sentence, because 0 cups over a goal they DID set is thirty points for a network blip');
}
{
    // Still in flight is not a failure, and a hero that flashes a warning on
    // every launch is a hero nobody reads.
    const loading = br({ hydrationStatus: 'loading', hydrationPct: null, readiness: (0, readiness_1.readinessScore)({ avgSleepHours: 7.5, hydrationPct: null, recoveryPct: null, workoutsLast2Days: 1 }) });
...
}
{
    ...
    eq(lineFor(br({ hydrationPct: 0 }), 'hydration').detail, "0% of today's goal", 'and nought per cent is a figure, not an absence — this is the `|| 0` trap from the other side');
    eq(lineFor(br({ hydrationPct: 0 }), 'hydration').state, 'scored', 'so its state is scored');
}
// ── recent sessions ───────────────────────────────────────────────────────
{
    eq(lineFor(br({ workoutsLast2Days: 0 }), 'load').detail, 'no sessions in the last two days', 'zero sessions is a fact and reads as one');
    ...
    eq(lineFor(br({ workoutsLast2Days: 0 }), 'load').state, 'scored', 'and nought is scored, never mistaken for unread — the two are opposite ends of the scale');
}
...
{
    const noScore = { readiness: null, sleep: sleepOf(0, 0) };
    ...
    eq(log.absence, 'We could not read your training log, so there is no readiness to show — it does not mean you are rested.', 'an unread log outranks every other reason, even with three good nights on file');
    ...
    eq(devErr.absence, 'We could not read your devices just now, so there is no readiness to show — it does not mean you slept badly.', 'a device that did not answer is not a bad night');
    ...
    eq(typedErr.absence, 'We could not read your sleep log just now, so there is no readiness to show — it does not mean you have not logged a night.', 'an unread sleep log must never be reported as an unlogged one');
    ...
    eq(empty.absence, 'No sleep on record for the last 3 nights yet.', 'a device that answered and had nothing is a true empty answer');
    ...
    eq(none.absence, 'Log a night of sleep, or connect a watch, to see your readiness.', 'the only member this sentence is honest for is the one with no device at all');
    ...
    eq(unsupported.absence, 'Log a night of sleep, or connect a watch, to see your readiness.', 'Health Connect cannot report sleep, so it does not count as a device that looked');
}
...
{
    const oneNight = { night: '2026-08-30', hours: 7.5, from: 'device' };
    const zero = br({ sleep: { avgHours: 0, nights: [oneNight], fromDevice: 1, fromTyped: 0 }, readiness: null });
    eq(lineFor(zero, 'sleep').state, 'no-record', 'an average of nought hours is the absence of a night, not a night of no sleep');
    ...
    eq(lineFor(half, 'sleep').detail, '0h 30m a night over 1 of the last 3 nights, measured by a device', 'shown as what the device reported');
    const noNights = br({ sleep: { avgHours: 7.5, nights: [], fromDevice: 0, fromTyped: 0 }, readiness: null });
    eq(lineFor(noNights, 'sleep').state, 'no-record', 'an average with no nights under it cannot say how many nights it ran over, so it does not claim to');
}
...
// print "NaN sessions in the last two days" underneath the dash.
{
    ...
    eq(nan.absence, 'We could not read your training log, so there is no readiness to show — it does not mean you are rested.', 'the absence names it as the read it is');
...
// the scale, and only one of them is something the member can go and fix.
{
    ...
    ok(/readiness/i.test(oura.detail), 'and in OURA’S own word for it — the two vendors name the same 0-100 figure differently and a member cross-checking needs ours to match theirs');
    const noStrap = lineFor(br({ recoveryPct: null, recoveryDeviceConnected: false }), 'recovery');
    eq(noStrap.state, 'not-tracked', 'NO DEVICE IS NOT A FAILED READ — telling somebody with no strap that we could not read their recovery invents a fault');
    ...
    eq(silent.state, 'unread', 'a connected strap with no figure today IS unread — that is the one the member can act on');
    ...
    for (const pct of [null, 0, 0.5, 1]) {
        for (const load of [null, 0, 1, 3]) {
            for (const rec of [null, 0, 62, 100]) {
                const r = (0, readiness_1.readinessScore)({ avgSleepHours: 7.5, hydrationPct: pct, recoveryPct: rec, workoutsLast2Days: load });
                ...
                if (r) {
                    eq(scoredSignals.join(','), r.from.join(','), `the rows marked scored are exactly readinessScore's own \`from\` (hydration ${pct}, recovery ${rec}, load ${load})`);
                }
                else {
