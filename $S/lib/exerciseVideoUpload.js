...
exports.VIDEO_BUCKET = void 0;
exports.exerciseVideoPath = exerciseVideoPath;
exports.isOwnVideoPath = isOwnVideoPath;
exports.videoUploadRefusal = videoUploadRefusal;
exports.videoUploadFailureLine = videoUploadFailureLine;
/** The private bucket. Named once so the three call sites in
 *  src/ui/exerciseVideos.ts cannot drift apart from each other. */
exports.VIDEO_BUCKET = 'exercise-videos';
/**
 ...
 * from the key.
 */
function exerciseVideoPath(uid, at, token) {
 ...
 */
function isOwnVideoPath(uid, path) {
 ...
 */
function videoUploadRefusal(error) {
    const e = error;
    if (!e)
        return 'refused';
    ...
    if (status === 401 || status === 403 || text.includes('jwt') || text.includes('unauthor'))
        return 'refused';
 ...
 */
function videoUploadFailureLine(reason) {
    switch (reason) {
        case 'signed-out':
