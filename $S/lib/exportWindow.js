...
exports.PRESET_IDS = exports.PRESET_LABEL = exports.NO_WINDOW = void 0;
exports.isBounded = isBounded;
exports.instantOf = instantOf;
exports.placeInWindow = placeInWindow;
exports.dayStart = dayStart;
exports.dayEnd = dayEnd;
exports.windowFromDays = windowFromDays;
exports.windowBlocker = windowBlocker;
exports.dayOf = dayOf;
exports.windowSlug = windowSlug;
exports.describeWindow = describeWindow;
exports.presetDays = presetDays;
/** The whole record. Not a wide window — the absence of one. */
exports.NO_WINDOW = { from: null, to: null };
/** True when this export covers a period rather than everything. */
function isBounded(w) {
...
function dayEnd(day) {
    ...
    if (!DAY.test(s))
        return null;
    const ms = instantOf(s);
    return ms === null ? null : new Date(ms + 86400000 - 1).toISOString();
}
/** The window two date inputs describe. Either may be blank for open-ended. */
function windowFromDays(fromDay, toDay) {
 ...
 */
function windowBlocker(fromDay, toDay) {
    const f = (fromDay ?? '').trim();
    const t = (toDay ?? '').trim();
    if (f && !DAY.test(f))
        return 'The start of the period is not a date. It wants a day, as 2026-01-01.';
    if (t && !DAY.test(t))
        return 'The end of the period is not a date. It wants a day, as 2026-03-31.';
    if (f && t) {
        const a = instantOf(f);
        const b = instantOf(t);
        if (a === null || b === null)
            return 'One of those days does not exist.';
        if (a > b) {
...
function describeWindow(w) {
    ...
    const b = dayOf(w.to);
    if (a && b)
        return `${a} to ${b} inclusive`;
    if (a)
        return `${a} onwards`;
    if (b)
        return `everything up to and including ${b}`;
...
exports.PRESET_IDS = ['all', 'thisMonth', 'lastMonth', 'last90', 'thisYear', 'lastYear'];
/**
...
function presetDays(id, today) {
    ...
    switch (id) {
        ...
        case 'last90':
            return { from: utcIso(t - 89 * 86400000), to: todayDay };
        case 'thisYear':
