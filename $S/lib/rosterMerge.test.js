...
{
    const merged = (0, rosterMerge_1.mergeRoster)([linkedRow('a', { goal: 'Fat loss' })], [manualRow('a', { goal: 'Build muscle' })]);
    eq(merged[0].coachGoal, 'Build muscle', "the coach's recorded goal is carried onto the surviving row");
    ok((0, rosterMerge_1.goalsDisagree)(merged[0].goal, merged[0].coachGoal), 'a client working to lose fat under a coach who wrote down muscle is a disagreement worth showing');
}
{
    ...
    const merged = (0, rosterMerge_1.mergeRoster)([linkedRow('a')], []);
    eq(merged[0].coachGoal, undefined, 'a coach who wrote nothing down has no recorded goal');
    ok(!(0, rosterMerge_1.goalsDisagree)(merged[0].goal, merged[0].coachGoal), 'silence from the coach is not a disagreement');
}
/* ── a manual-only entry is not touched and not dropped ────────────────── */
{
    const only = manualRow('m1');
    const merged = (0, rosterMerge_1.mergeRoster)([linkedRow('a')], [only]);
...
}
{
    ...
    const merged = (0, rosterMerge_1.mergeRoster)([], [manualRow('m1'), manualRow('m2')]);
...
}
{
    ...
    eq((0, rosterMerge_1.mergeRoster)([], []).length, 0, 'two empty lists merge to an empty roster');
}
/* ── ids are unique in the output ──────────────────────────────────────── */
{
    const merged = (0, rosterMerge_1.mergeRoster)([linkedRow('a'), linkedRow('b')], [manualRow('a'), manualRow('b'), manualRow('m1')]);
...
}
{
    ...
    const merged = (0, rosterMerge_1.mergeRoster)([linkedRow('a'), linkedRow('a', { name: 'Second a' })], [manualRow('m1'), manualRow('m1')]);
...
/* ── ordering is deterministic ─────────────────────────────────────────── */
{
    ...
    const once = (0, rosterMerge_1.mergeRoster)(linked, manual);
    eq(once.map((c) => c.id).join(','), 'a,b,m1,m2', 'linked clients in the order read, then the manual-only ones in the order read');
    ...
    eq(JSON.stringify((0, rosterMerge_1.mergeRoster)(linked, manual)), JSON.stringify(once), 'merging the same reads twice gives the same list');
}
/* ── two vocabularies, three goals ─────────────────────────────────────── */
eq((0, rosterMerge_1.goalToEnum)('fatloss'), 'fatloss', "the client's enum resolves");
eq((0, rosterMerge_1.goalToEnum)('tone'), 'tone', "the client's enum resolves");
eq((0, rosterMerge_1.goalToEnum)('muscle'), 'muscle', "the client's enum resolves");
eq((0, rosterMerge_1.goalToEnum)('Fat loss'), 'fatloss', "the coach's label resolves");
eq((0, rosterMerge_1.goalToEnum)('Tone'), 'tone', "the coach's label resolves");
eq((0, rosterMerge_1.goalToEnum)('Build muscle'), 'muscle', "the coach's label resolves");
...
ok(!(0, rosterMerge_1.goalsDisagree)('fatloss', 'Fat loss'), "'Fat loss' and 'fatloss' are one goal, not two");
ok(!(0, rosterMerge_1.goalsDisagree)('muscle', 'Build muscle'), "'Build muscle' and 'muscle' are one goal, not two");
ok(!(0, rosterMerge_1.goalsDisagree)('tone', 'Tone'), "'Tone' and 'tone' are one goal, not two");
ok((0, rosterMerge_1.goalsDisagree)('fatloss', 'Build muscle'), 'lose fat against build muscle is a real disagreement');
ok((0, rosterMerge_1.goalsDisagree)('Tone', 'fatloss'), 'the comparison works whichever side the label is on');
...
eq((0, rosterMerge_1.goalToEnum)('General'), null, "'General' is a placeholder for unknown, not a goal");
eq((0, rosterMerge_1.goalToEnum)(''), null, 'an empty goal is unknown');
eq((0, rosterMerge_1.goalToEnum)(null), null, 'a null goal is unknown');
eq((0, rosterMerge_1.goalToEnum)(undefined), null, 'a missing goal is unknown');
eq((0, rosterMerge_1.goalToEnum)('—'), null, 'a dash is unknown');
...
eq((0, rosterMerge_1.goalToEnum)('muscle tone'), null, 'a phrase naming two goals resolves to neither');
eq((0, rosterMerge_1.goalToEnum)('get strong for skiing'), null, 'free text a coach typed is not silently a goal');
...
eq((0, rosterMerge_1.goalToEnum)('toString'), null, 'toString is not a goal');
eq((0, rosterMerge_1.goalToEnum)('constructor'), null, 'constructor is not a goal');
eq((0, rosterMerge_1.goalToEnum)('__proto__'), null, '__proto__ is not a goal');
// An unknown on either side is not a disagreement. A failed read must not
// manufacture a coaching conversation.
ok(!(0, rosterMerge_1.goalsDisagree)('General', 'Build muscle'), 'a goal we could not read does not disagree with anything');
ok(!(0, rosterMerge_1.goalsDisagree)('fatloss', 'General'), 'a coach note we could not read does not disagree with anything');
ok(!(0, rosterMerge_1.goalsDisagree)(null, null), 'two unknowns are not a disagreement');
ok(!(0, rosterMerge_1.goalsDisagree)('fatloss', undefined), 'nothing recorded is not a disagreement');
if (errors.length) {
