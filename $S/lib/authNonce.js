 ...
 */
function authNonce(fill) {
    const bytes = new Uint8Array(exports.NONCE_BYTES);
    const platform = fill
        ?? globalThis
            .crypto?.getRandomValues?.bind(globalThis.crypto);
    if (platform) {
        try {
            platform(bytes);
            ...
            if (bytes.some((b) => b !== 0))
                return { value: hex(bytes), strong: true };
        }
