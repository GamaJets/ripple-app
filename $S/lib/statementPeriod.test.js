...
{
    const a = (0, coachStatement_1.fiscalYear)(2026);
    const b = (0, coachStatement_1.calendarYear)(2026);
...
for (const q of [1, 2, 3, 4]) {
    const a = (0, coachStatement_1.fiscalQuarter)(2026, q);
    const b = (0, coachStatement_1.calendarQuarter)(2026, q);
    eq(a.from, b.from, `Q${q} of a default year opens where the calendar quarter did`);
    eq(a.to, b.to, `and closes where it did`);
    eq(a.label, b.label, `and is labelled the same`);
}
ok((0, coachStatement_1.isCalendarStart)(coachStatement_1.CALENDAR_YEAR_START), 'the default start is the calendar one');
ok(!(0, coachStatement_1.isCalendarStart)({ month: 4, day: 6 }), 'and 6 April is not');
...
{
    const uk = (0, coachStatement_1.fiscalYear)(2026, { month: 4, day: 6 });
...
{
    const au = (0, coachStatement_1.fiscalYear)(2026, { month: 7, day: 1 });
...
eq((0, coachStatement_1.fiscalYear)(2027, { month: 3, day: 1 }).to, '2028-02-29', 'a leap day is the last day of the year that contains it');
...
eq((0, coachStatement_1.fiscalYear)(2028, { month: 2, day: 29 }).from, '2028-02-29', 'a year may open on a leap day');
eq((0, coachStatement_1.fiscalYear)(2028, { month: 2, day: 29 }).to, '2029-02-28', 'and closes on the last day of February in a year with no 29th');
...
eq((0, coachStatement_1.fiscalYear)(2026, { month: 2, day: 31 }).from, '2026-02-28', 'a start day past the end of its month is clamped, not rolled over');
eq((0, coachStatement_1.fiscalYear)(2026, { month: 4, day: 31 }).from, '2026-04-30', 'and April has thirty days');
eq((0, coachStatement_1.fiscalYear)(2026, { month: 13, day: 1 }).from, '2026-12-01', 'a month past December is clamped rather than rolled into next year');
eq((0, coachStatement_1.fiscalYear)(2026, { month: 0, day: 0 }).from, '2026-01-01', 'and a zero month and day are the first of January');
...
for (const start of [{ month: 1, day: 1 }, { month: 4, day: 6 }, { month: 7, day: 1 }, { month: 10, day: 15 }]) {
    const a = (0, coachStatement_1.fiscalYear)(2026, start);
    const b = (0, coachStatement_1.fiscalYear)(2027, start);
    const ra = (0, coachStatement_1.periodRange)(a);
    const rb = (0, coachStatement_1.periodRange)(b);
...
for (const start of [{ month: 1, day: 1 }, { month: 4, day: 6 }, { month: 7, day: 1 }]) {
    const y = (0, coachStatement_1.fiscalYear)(2026, start);
    eq((0, coachStatement_1.fiscalQuarter)(2026, 1, start).from, y.from, `Q1 opens with the year (start ${start.month}/${start.day})`);
    eq((0, coachStatement_1.fiscalQuarter)(2026, 4, start).to, y.to, `Q4 closes with it (start ${start.month}/${start.day})`);
    for (const q of [1, 2, 3]) {
        const here = (0, coachStatement_1.periodRange)((0, coachStatement_1.fiscalQuarter)(2026, q, start));
        const next = (0, coachStatement_1.periodRange)((0, coachStatement_1.fiscalQuarter)(2026, q + 1, start));
...
{
    const q1 = (0, coachStatement_1.fiscalQuarter)(2026, 1, { month: 4, day: 6 });
    ...
    const q4 = (0, coachStatement_1.fiscalQuarter)(2026, 4, { month: 4, day: 6 });
    eq(q4.from, '2027-01-06', 'Q4 of an April year opens in the next calendar year');
    eq(q4.to, '2027-04-05', 'and closes with the year');
}
eq((0, coachStatement_1.fiscalQuarter)(2026, 9, { month: 4, day: 6 }).label, 'Q4 2026/27', 'a quarter number past four is clamped rather than wrapped');
eq((0, coachStatement_1.fiscalQuarter)(2026, 0, { month: 4, day: 6 }).label, 'Q1 2026/27', 'and one below one is too');
/* ── 6. any two dates, or nothing at all ────────────────────────────────── */
{
    const p = (0, coachStatement_1.customRange)('2026-03-01', '2026-05-31');
    ...
    ok(p.label.includes((0, format_1.fmtPointDay)(2026, 2, 1)) && p.label.includes((0, format_1.fmtPointDay)(2026, 4, 31)), 'and the label spells both out rather than saying "custom"');
    ok(!/custom/i.test(p.label), 'and never falls back to the word itself');
}
ok(!!(0, coachStatement_1.customRange)('2026-03-01', '2026-03-01'), 'a single day is a period');
...
eq((0, coachStatement_1.customRange)('2026-05-31', '2026-03-01'), null, 'a backwards range is refused rather than swapped');
eq((0, coachStatement_1.customRange)('', ''), null, 'and so is an empty one');
eq((0, coachStatement_1.customRange)('2026-3-1', '2026-05-31'), null, 'and one that is not zero-padded');
eq((0, coachStatement_1.customRange)('last march', '2026-05-31'), null, 'and one that is words');
...
eq((0, coachStatement_1.customRange)('2026-02-30', '2026-05-31'), null, 'and one naming a day that does not exist');
/* ── 7. the sentence that keeps this out of tax advice ──────────────────── */
ok(/does not know which tax year/i.test(coachStatement_1.YEAR_START_IS_YOURS), 'the page says this app does not know the coach’s tax year');
ok(/has not inferred/i.test(coachStatement_1.YEAR_START_IS_YOURS), 'and that it has not guessed one from anything');
ok(/does not check/i.test(coachStatement_1.YEAR_START_IS_YOURS), 'and that it has not checked the one they chose');
