...
exports.clampGlasses = exports.WATER_CAP = exports.byNewest = exports.localId = exports.isPending = exports.LOCAL_PREFIX = void 0;
exports.mergeLog = mergeLog;
exports.adoptServerId = adoptServerId;
exports.mergeCount = mergeCount;
/**
 ...
 */
exports.LOCAL_PREFIX = 'local:';
/** True for an entry that has never been accepted by the server. */
const isPending = (id) => id.startsWith(exports.LOCAL_PREFIX);
exports.isPending = isPending;
let SEQ = 0;
/** A new local id. Unique within a session; never collides with a uuid. */
const localId = () => `${exports.LOCAL_PREFIX}${Date.now().toString(36)}.${SEQ++}`;
 ...
 * to differ.
 */
const byNewest = (a, b) => (a.at < b.at ? 1 : a.at > b.at ? -1 : 0) || (a.id < b.id ? 1 : a.id > b.id ? -1 : 0);
 ...
 */
function mergeLog(server, local) {
    const out = new Map();
    if (server) {
        for (const e of server)
            out.set(e.id, e);
        for (const e of local)
            if ((0, exports.isPending)(e.id) && !out.has(e.id))
                out.set(e.id, e);
    }
    else {
        for (const e of local)
    ...
    const entries = [...out.values()].sort(exports.byNewest);
    return { entries, pending: entries.filter((e) => (0, exports.isPending)(e.id)) };
}
/**
 ...
 */
function adoptServerId(list, local, server) {
 ...
 */
function mergeCount(server, local) {
    if (!server && !local)
        return { count: 0, push: false };
    if (!server)
        return { count: local.count, push: true };
    if (!local)
        return { count: server.count, push: false };
 ...
 *  parse failure. */
const clampGlasses = (n) => (Number.isFinite(n) ? Math.max(0, Math.min(exports.WATER_CAP, Math.round(n))) : 0);
exports.clampGlasses = clampGlasses;
