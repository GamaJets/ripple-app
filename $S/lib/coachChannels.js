...
exports.channelSum = channelSum;
exports.spendAgainstReturn = spendAgainstReturn;
exports.againstLine = againstLine;
exports.channelEmptyLine = channelEmptyLine;
exports.channelReachLine = channelReachLine;
...
function pot(by, currency, minorUnits) {
    const held = by.get(currency);
    if (held) {
        held.minorUnits += minorUnits;
        held.count += 1;
    }
    else
        by.set(currency, { currency, minorUnits, count: 1 });
}
/** Biggest first, then by code, so the order does not shuffle between reads. */
const ranked = (by) => [...by.values()].sort((a, b) => (b.minorUnits - a.minorUnits) || a.currency.localeCompare(b.currency));
function channelSum(rows) {
    ...
    for (const r of rows) {
        ...
        stayed += r.activeNow;
        if (r.spend)
            pot(spentBy, r.spend.currency, r.spend.cents);
        else
            costUnknown += 1;
        if (r.revenue)
            pot(earnedBy, r.revenue.currency, r.revenue.cents);
        else
            paidUnknown += 1;
        if (r.spend && r.revenue && r.spend.currency !== r.revenue.currency)
            crossCurrency += 1;
    }
    return {
...
function spendAgainstReturn(status, sum) {
    ...
    if (sum.costUnknown > 0) {
        return {
            statable: false,
            why: 'cost-unknown',
            note: `${(0, format_1.num)(sum.costUnknown)} of your ${(0, format_1.num)(sum.codes)} ${sum.codes === 1 ? 'code has' : 'codes have'} no cost recorded, so no figure is stated. That is not a cost of nothing — counting it as nothing would make your advertising look cheaper than it was, which is the mistake that costs you money. Record what each one cost and this becomes a real number.`,
        };
    }
    if (sum.paidUnknown > 0) {
        return {
            statable: false,
            why: 'paid-unknown',
            note: `What the clients off ${(0, format_1.num)(sum.paidUnknown)} of your ${(0, format_1.num)(sum.codes)} ${sum.codes === 1 ? 'code has' : 'codes have'} paid could not be put into a single currency, so there is no total to set against your spend. Nothing here says those clients paid nothing.`,
        };
    }
    if (sum.crossCurrency > 0) {
        return {
            statable: false,
            why: 'cross',
            note: `${(0, format_1.num)(sum.crossCurrency)} of your ${(0, format_1.num)(sum.codes)} ${sum.codes === 1 ? 'code was' : 'codes were'} paid for in one currency and brought in clients who paid in another. Subtracting one from the other would give a number that is not an amount of anything.`,
        };
    }
 ...
 */
function written(minorUnits, currency) {
    if (!Number.isFinite(minorUnits))
        return null;
    return (0, gymRecord_1.money)(minorUnits, currency);
}
function againstLine(a) {
    if (!a.statable)
        return a.note;
    ...
    if (!spent || !earned)
        return 'These amounts could not be written out, so no comparison is stated.';
    const codes = `${(0, format_1.num)(a.codes)} ${a.codes === 1 ? 'code' : 'codes'}`;
    ...
    if (diff === 0)
        return `${opening} That is exactly what they cost.`;
 ...
 */
function channelEmptyLine(status) {
    if (status === 'error')
        return 'Nothing is shown because your codes could not be read, not because none of them worked. Anything they already brought in still stands.';
    if (status === 'partial')
        return 'There are more codes on record than could be read in one request, so nothing here is a count.';
    if (status === 'loading')
 ...
 */
function channelReachLine(status, sum) {
    if (status !== 'ready')
        return '';
    if (sum.codes === 0)
        return '';
    ...
    const people = `${(0, format_1.num)(sum.clients)} ${sum.clients === 1 ? 'person' : 'people'}`;
    const still = sum.stayed === sum.clients
        ? `and all of them are still with you`
        : `and ${(0, format_1.num)(sum.stayed)} of them ${sum.stayed === 1 ? 'is' : 'are'} still with you`;
