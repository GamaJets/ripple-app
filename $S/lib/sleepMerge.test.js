...
for (const [h, m] of [[0, 5], [6, 30], [13, 0], [23, 55]]) {
    ok((0, sleepMerge_1.nightKey)(at(2026, 8, 3, h, m)) === '2026-08-03', `[${TZ}] a wake-up at ${pad(h)}:${pad(m)} local on 3 Aug belongs to 2026-08-03, got ${(0, sleepMerge_1.nightKey)(at(2026, 8, 3, h, m))}`);
}
...
ok((0, sleepMerge_1.nightKey)('2026-08-03') === '2026-08-03', `[${TZ}] a date-only night must not shift, got ${(0, sleepMerge_1.nightKey)('2026-08-03')}`);
ok((0, sleepMerge_1.nightKey)(null) === null, 'no timestamp is no night, not today');
ok((0, sleepMerge_1.nightKey)('not a date') === null, 'an unparseable timestamp is no night');
// ── the window of nights we ask about ───────────────────────────────────────
const nights = (0, sleepMerge_1.recentNights)(7, new Date(2026, 2, 1, 12));
...
ok((0, sleepMerge_1.recentNights)(3)[0] === localYmd(new Date()), `[${TZ}] the first night defaults to today in local time`);
...
const folded = (0, sleepMerge_1.foldSleepIntervals)([
...
ok(Date.parse(folded[0].end) - Date.parse(folded[0].start) === 7 * 3600000, 'the folded span is the seven hours in bed, not the sum of the samples');
ok((0, sleepMerge_1.foldSleepIntervals)([{ start: at(2026, 8, 3, 1, 0), end: at(2026, 8, 3, 1, 0) }]).length === 0, 'a zero-length sample is a malformed row, not sleep');
// ── a night that straddles midnight is one night ────────────────────────────
const straddle = (0, sleepMerge_1.nightsFromIntervals)([
...
const twoNights = (0, sleepMerge_1.nightsFromIntervals)([
...
const disagree = (0, sleepMerge_1.mergeSleepNight)('2026-08-03', [watch, ring]);
...
ok(disagree.others.length === 1 && disagree.others[0].sourceName === 'Apple Watch', 'the losing reading stays visible rather than being dropped');
// ── both present and close enough to corroborate ────────────────────────────
const close = (0, sleepMerge_1.mergeSleepNight)('2026-08-03', [ring, { ...watch, minutesAsleep: 424 - sleepMerge_1.AGREEMENT_TOLERANCE_MIN }]);
...
const justOver = (0, sleepMerge_1.mergeSleepNight)('2026-08-03', [ring, { ...watch, minutesAsleep: 424 - sleepMerge_1.AGREEMENT_TOLERANCE_MIN - 1 }]);
...
const echoed = (0, sleepMerge_1.mergeSleepNight)('2026-08-03', [ring, ouraViaHealth]);
...
const mixed = (0, sleepMerge_1.mergeSleepNight)('2026-08-03', [inBed, watch]);
...
const onlyInBed = (0, sleepMerge_1.mergeSleepNight)('2026-08-03', [inBed]);
ok(onlyInBed.outcome === 'measured' && onlyInBed.source?.basis === 'in-bed', 'a source that only records time in bed is not reported as a night nobody recorded');
// ── only one has a reading ──────────────────────────────────────────────────
const alone = (0, sleepMerge_1.mergeSleepNight)('2026-08-03', [watch]);
...
const nobody = (0, sleepMerge_1.mergeSleepNight)('2026-08-03', []);
...
ok((0, sleepMerge_1.formatSleepHours)(nobody.minutesAsleep) === '—', 'which renders as a dash');
// ── a read that FAILED is unknown, not zero ─────────────────────────────────
const brokeAll = (0, sleepMerge_1.mergeSleepNight)('2026-08-03', [], ['whoop']);
...
const partial = (0, sleepMerge_1.mergeSleepNight)('2026-08-03', [watch], ['whoop']);
...
const zeroed = (0, sleepMerge_1.mergeSleepNight)('2026-08-03', [reading({ sourceId: 'buggy', minutesAsleep: 0 })]);
ok(zeroed.outcome === 'no-record' && zeroed.minutesAsleep === null, 'a zero-minute row is a mapping that had nothing to say, not a sleepless night');
// ── the whole window ────────────────────────────────────────────────────────
const reads = [
...
const window3 = (0, sleepMerge_1.mergeSleepNights)(reads, ['2026-08-03', '2026-08-02', '2026-08-01']);
...
const windowBroken = (0, sleepMerge_1.mergeSleepNights)([{ provider: 'whoop', status: 'error', readings: [], reason: 'server did not answer' }], ['2026-08-03']);
...
const walkFailed = (0, sleepMerge_1.mergeSleepNights)([], ['2026-08-03', '2026-08-02']);
ok(walkFailed.every((n) => n.outcome === 'no-record'), 'a merge given nothing cannot know a read failed — which is why the caller has to say so');
const unread = (0, sleepMerge_1.markNightsUnread)(walkFailed, ['whoop', 'oura']);
ok(unread.length === 2, `every night survives the marking, got ${unread.length}`);
ok(unread.every((n) => n.outcome === 'unknown'), `a night from a failed walk is unknown, not empty, got ${unread.map((n) => n.outcome).join()}`);
...
// a measurement.
const measured = (0, sleepMerge_1.mergeSleepNights)([{ provider: 'apple', status: 'ready', readings: [{ ...watch, night: '2026-08-03' }] }], ['2026-08-03', '2026-08-02']);
const markedMixed = (0, sleepMerge_1.markNightsUnread)(measured, ['apple']);
ok(markedMixed[0].outcome === 'measured' && markedMixed[0].minutesAsleep === 392, `a night a device actually measured keeps its figure, got ${markedMixed[0].outcome}/${markedMixed[0].minutesAsleep}`);
...
ok((0, sleepMerge_1.markNightsUnread)(walkFailed)[0].outcome === 'unknown', 'a failed walk with no providers named is still a failed walk');
// Idempotent, because a screen re-rendering must not accumulate duplicates in
// the list of devices it names to the client.
ok((0, sleepMerge_1.markNightsUnread)(unread, ['whoop', 'oura'])[0].failed.join() === 'whoop,oura', 'marking twice names each device once');
// ── formatting ──────────────────────────────────────────────────────────────
ok((0, sleepMerge_1.formatSleepHours)(432) === '7h 12m', `432 minutes is 7h 12m, got ${(0, sleepMerge_1.formatSleepHours)(432)}`);
ok((0, sleepMerge_1.formatSleepHours)(420) === '7h', `a whole number of hours drops the minutes, got ${(0, sleepMerge_1.formatSleepHours)(420)}`);
ok((0, sleepMerge_1.formatSleepHours)(null) === '—' && (0, sleepMerge_1.formatSleepHours)(0) === '—' && (0, sleepMerge_1.formatSleepHours)(NaN) === '—', 'nothing knowable renders as a dash');
if (errors.length) {
