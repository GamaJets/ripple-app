...
const at = (day, time = '12:00:00') => `${day}T${time}`;
const entry = (day, exercise, sets, time) => ({ t: at(day, time), exercise, sets });
...
const tapped = (0, exerciseHistory_1.exerciseOutings)(doubleTapped, 'Back Squat');
...
eq(tapped[0].entryCount, 4, 'and the day says how many entries it was folded from, because folding is not deduplicating — '
...
const split = (0, exerciseHistory_1.exerciseOutings)(splitDay, 'Bench Press');
...
const chins = [entry('2026-03-04', 'Pull-up', [[10, 0], [8, 0]])];
const chinOut = (0, exerciseHistory_1.exerciseOutings)(chins, 'Pull-up')[0];
...
const blanks = (0, exerciseHistory_1.exerciseOutings)([entry('2026-03-05', 'Row', [[0, 40], [10, 40]])], 'Row')[0];
...
eq((0, exerciseHistory_1.exerciseOutings)([{ t: at('2026-03-06'), exercise: 'Plank' }], 'Plank').length, 0, 'a movement logged with nothing done to it produces no outing rather than an empty one');
/* ── 3 · the arithmetic is the app's own, in the app's own unit ───────────── */
const bench = (0, exerciseHistory_1.exerciseOutings)([entry('2026-03-07', 'Bench Press', [[8, 60], [5, 80], [3, 85]])], 'Bench Press')[0];
...
eq(bench.best1RMKg, Math.max((0, streaks_1.est1RM)(60, 8), (0, streaks_1.est1RM)(80, 5), (0, streaks_1.est1RM)(85, 3)), 'and the estimated max is `est1RM` from src/lib/streaks.ts — the app has one 1RM formula and this is not a second');
...
eq((0, exerciseHistory_1.exerciseOutings)([entry('2026-03-08', 'Squat', [[3, 100], [5, 100]])], 'Squat')[0].topReps, 5, 'the reps at the top load are the most achieved there');
eq((0, exerciseHistory_1.exerciseOutings)([entry('2026-03-08', 'Squat', [[5, 100], [3, 100]])], 'Squat')[0].topReps, 5, 'and the best of them, not the last of them');
eq((0, exerciseHistory_1.exerciseOutings)([entry('2026-03-08', 'Squat', [[3, 100], [8, 60]])], 'Squat')[0].topReps, 3, 'reps done at a LIGHTER load are not reps at the top load, however many of them there were');
// The two figures a mis-set threshold would quietly discard.
const single = (0, exerciseHistory_1.exerciseOutings)([entry('2026-03-09', 'Deadlift', [[1, 180]])], 'Deadlift')[0];
eq(single.setCount, 1, 'a single is a set — the only thing not counted is a row with no reps at all');
eq(single.best1RMKg, (0, streaks_1.est1RM)(180, 1), 'and it estimates a max like any other');
const light = (0, exerciseHistory_1.exerciseOutings)([entry('2026-03-09', 'Curl', [[10, 1]])], 'Curl')[0];
...
const squatOnly = (0, exerciseHistory_1.exerciseOutings)(twoLifts, 'Squat')[0];
eq((0, exerciseHistory_1.exerciseOutings)(twoLifts, 'Squat').length, 1, 'asking for one movement returns one movement');
...
eq((0, exerciseHistory_1.exerciseOutings)([{ t: '', exercise: 'Squat', sets: [[5, 100]] }], 'Squat').length, 0, 'an entry with no timestamp produces no outing rather than an undated one');
...
const spelled = (0, exerciseHistory_1.exerciseOutings)(spellings, 'Bench Press');
...
eq((0, exerciseHistory_1.exerciseOutings)(spellings, 'bench-press').length, 3, 'and the query is slugged the same way the record is');
eq((0, exerciseHistory_1.exerciseOutings)(spellings, '   ').length, 0, 'a query that slugs to nothing matches nothing rather than everything');
const index = (0, exerciseHistory_1.exerciseIndex)([
    ...spellings,
...
eq(index[1].best1RMKg, (0, streaks_1.est1RM)(60, 6), 'the index carries the best estimated max across every outing of a movement');
...
const sameDay = (0, exerciseHistory_1.exerciseIndex)([entry('2026-04-01', 'Squat', [[5, 100]]), entry('2026-04-01', 'Bench Press', [[5, 60]])]);
...
const cardio = (0, exerciseHistory_1.exerciseIndex)([
...
eq((0, exerciseHistory_1.matchExercises)(cardio, 'cycling').length, 1, 'and is still findable by name');
eq((0, exerciseHistory_1.matchExercises)(cardio, 'cycling')[0].days, 3, 'with the days it was actually done');
eq((0, exerciseHistory_1.matchExercises)(cardio, 'cycling')[0].daysWithSets, 0, 'and a separate figure saying none of them carried sets, so no screen has to guess');
eq((0, exerciseHistory_1.matchExercises)(cardio, 'cycling')[0].best1RMKg, null, 'an hour on a bike estimates no 1RM');
eq((0, exerciseHistory_1.exerciseOutings)([{ t: at('2026-08-25'), exercise: 'Cycling' }], 'Cycling').length, 0, 'while the reps-and-loads trail behind it is honestly empty rather than padded with nothing');
eq((0, exerciseHistory_1.matchExercises)(cardio, 'squats')[0].daysWithSets, 1, 'a lift that did carry sets says so on the same field');
eq((0, exerciseHistory_1.matchExercises)(index, 'bench').length, 1, 'a search narrows to the movement asked for');
eq((0, exerciseHistory_1.matchExercises)(index, 'press bench')[0]?.name, 'BENCH PRESS', 'word order is not part of the question');
eq((0, exerciseHistory_1.matchExercises)(index, 'bench press').length, 1, 'and every word must appear — "bench press" does not return every leg press in the book');
eq((0, exerciseHistory_1.matchExercises)(index, '').length, 3, 'an empty box is not a filter');
eq((0, exerciseHistory_1.matchExercises)(index, 'deadlift').length, 0, 'a movement nobody has done returns nothing rather than everything');
/* ── 5 · what a failed and a truncated read may say ───────────────────────── */
const trail = (0, exerciseHistory_1.exerciseOutings)([
    entry('2026-03-01', 'Bench Press', [[10, 50]]),
    entry('2026-03-08', 'Bench Press', [[8, 55]]),
    entry('2026-03-15', 'Bench Press', [[8, 60]]),
], 'Bench Press');
eq((0, exerciseHistory_1.exerciseTrend)(null, 'ready').state, 'unreadable', 'a null read is unreadable, whatever the status says');
eq((0, exerciseHistory_1.exerciseTrend)(trail, 'error').state, 'unreadable', 'and an error is unreadable even holding rows');
eq((0, exerciseHistory_1.exerciseTrend)([], 'loading').state, 'unreadable', 'a question nobody has finished asking is not an answer of "never done"');
eq((0, exerciseHistory_1.exerciseTrend)([], 'ready').state, 'none', 'an empty read that landed IS "never done", and says so');
eq((0, exerciseHistory_1.exerciseTrend)([], 'ready').outingCount, 0, 'which is a real count of nought, not a dash');
const whole = (0, exerciseHistory_1.exerciseTrend)(trail, 'ready');
...
const tied = (0, exerciseHistory_1.exerciseTrend)((0, exerciseHistory_1.exerciseOutings)([
...
const partial = (0, exerciseHistory_1.exerciseTrend)(trail, 'partial');
...
eq(partial.latest?.day, '2026-03-15', 'the newest is still safe to state — the read is ordered newest first, so truncation cannot remove it');
...
eq(whole.sinceFirst.est1RMKg, (0, streaks_1.est1RM)(60, 8) - (0, streaks_1.est1RM)(50, 10), 'estimated maxes move by their own difference');
eq(whole.sinceLast.reps, 0, 'a count that has not moved is 0 here — the module states the fact and no more');
const alone = (0, exerciseHistory_1.exerciseTrend)((0, exerciseHistory_1.exerciseOutings)([entry('2026-03-01', 'Row', [[10, 40]])], 'Row'), 'ready');
...
const mixed = (0, exerciseHistory_1.exerciseTrend)((0, exerciseHistory_1.exerciseOutings)([
...
ok((0, deltaLabel_1.deltaLabel)(mixed.sinceLast.topLoadKg, { since: '1 Mar', unit: 'kg' }) === 'No earlier reading', 'and null reads as "no earlier reading" rather than as a flat line');
...
const flat = (0, exerciseHistory_1.exerciseTrend)((0, exerciseHistory_1.exerciseOutings)([
    entry('2026-03-01', 'Squat', [[5, 100]]),
    entry('2026-03-08', 'Squat', [[5, 100]]),
], 'Squat'), 'ready');
eq(flat.sinceLast.topLoadKg, 0, 'a lift that held is a movement of exactly nothing');
eq((0, deltaLabel_1.deltaSign)(flat.sinceLast.topLoadKg), '', 'which carries no sign');
eq((0, deltaLabel_1.deltaMoved)(flat.sinceLast.topLoadKg), false, 'and did not move');
eq((0, deltaLabel_1.deltaLabel)((0, units_1.liftDeltaIn)(flat.sinceLast.topLoadKg, 'lb'), { since: '1 Mar', unit: 'lb' }), 'No change since 1 Mar', 'so a member on a fat-loss block whose bench held is told it held, in their own unit, with no arrow on it');
// The span is converted once, at the boundary, never the two ends separately.
eq((0, units_1.liftDeltaIn)(whole.sinceFirst.topLoadKg, 'lb'), 22, '10 kg reads as 22 lb as a span, at the half-pound the plates justify');
eq((0, units_1.est1RMIn)(whole.sinceFirst.est1RMKg, 'kg'), (0, streaks_1.est1RM)(60, 8) - (0, streaks_1.est1RM)(50, 10), 'and an estimated max keeps the whole-kilogram grain it was derived at');
/* ── 7 · a date that will not parse invents no day ────────────────────────── */
const broken = (0, exerciseHistory_1.exerciseOutings)([
...
const brokenIndex = (0, exerciseHistory_1.exerciseIndex)([
...
const listed = whole.outings;
ok(listed.every((o, i) => i === 0 || (listed[i - 1].day ?? '') >= (o.day ?? '')), 'outings come back newest first, stated here rather than inherited from a query in a file this does not own');
 ...
 */
{
    ...
    const chin = { t: at('2026-05-10'), exercise: 'Pull-up', sets: [[10, 0], [8, 20]], bw: [true, true] };
    const unweighed = (0, exerciseHistory_1.exerciseOutings)([chin], 'Pull-up')[0];
    ...
    const priced = (0, exerciseHistory_1.exerciseOutings)([chin], 'Pull-up', HIST)[0];
    ...
    // steps on a scale.
    const january = (0, exerciseHistory_1.exerciseOutings)([{ t: at('2026-02-01'), exercise: 'Pull-up', sets: [[10, 0]], bw: [true] }], 'Pull-up', HIST)[0];
