...
exports.MEAL_DB = exports.DIET_LABEL = exports.DEFICIT_PROTEIN_PER_KG_LBM = exports.GOAL_ADJ = void 0;
exports.maintenanceFor = maintenanceFor;
exports.macrosFor = macrosFor;
exports.applyCoachAdjust = applyCoachAdjust;
exports.seriesFromScans = seriesFromScans;
exports.buildMealPlan = buildMealPlan;
exports.caloriesLeft = caloriesLeft;
exports.caloriesNote = caloriesNote;
exports.budgetedActiveKcal = budgetedActiveKcal;
exports.dayBurn = dayBurn;
 ...
 * dieting.
 */
exports.DEFICIT_PROTEIN_PER_KG_LBM = 2.2;
/**
 ...
 */
function macrosFor(s) {
    const { lbm, bmr, tdee } = maintenanceFor(s);
    const plan = s.energyPlan && s.energyPlan.kind === 'derived' ? s.energyPlan : null;
    const kcal = plan
        ? (0, goalEnergy_1.planKcal)(tdee, plan.plannedRateKg).kcal
        : Math.round((tdee * (1 + exports.GOAL_ADJ[s.goal])) / 10) * 10;
    ...
    const proteinPerKg = plan && plan.kcalDelta < 0 ? Math.max(enumPerKg, exports.DEFICIT_PROTEIN_PER_KG_LBM) : enumPerKg;
...
function applyCoachAdjust(m, a) {
    if (!a || (!a.kcalDelta && !a.proteinDelta && !a.carbDelta && !a.fatDelta))
        return m;
...
function seriesFromScans(scans) {
...
function buildMealPlan(stats, mealsPerDay, seed = 0) {
    const targets = macrosFor(stats);
    const db = exports.MEAL_DB[stats.diet];
...
function caloriesLeft(targetKcal, eatenKcal, burnedKcal = 0, budgetedActiveKcal = 0, burnKind = 'active') {
...
function caloriesNote(cal) {
    ...
    const head = `${(0, format_1.num)(cal.eaten)} of ${(0, format_1.num)(cal.target)} kcal eaten`;
    if (!cal.burned)
        return head;
    ...
    const what = cal.burnKind === 'total'
        ? `${(0, format_1.num)(cal.burned)} kcal burned all day, rest included`
        : `${(0, format_1.num)(cal.burned)} active kcal burned`;
