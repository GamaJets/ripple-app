...
exports.est1RM = void 0;
exports.activeDays = activeDays;
exports.currentStreak = currentStreak;
exports.freezeBudget = freezeBudget;
exports.currentStreakFrozen = currentStreakFrozen;
exports.shownStreak = shownStreak;
exports.streakRisk = streakRisk;
exports.longestStreak = longestStreak;
exports.personalRecords = personalRecords;
exports.isNewPR = isNewPR;
exports.statsSince = statsSince;
exports.weekStats = weekStats;
exports.thisWeekStats = thisWeekStats;
exports.streakMilestone = streakMilestone;
 ...
 */
const dayCursor = (now) => { const d = new Date(now); d.setHours(12, 0, 0, 0); return d; };
const stepBack = (d) => { d.setDate(d.getDate() - 1); return d; };
/** Unique calendar days (YYYY-MM-DD) that have at least one workout, newest first. */
function activeDays(log) {
 ...
 */
function currentStreak(log, now = Date.now()) {
 ...
 */
function currentStreakFrozen(log, freezes = 0, now = Date.now()) {
    const daysArr = activeDays(log);
    if (daysArr.length === 0)
        return { streak: 0, freezesUsed: 0, frozen: [] };
    ...
    while (true) {
        ...
        if (streak === 0)
            break; // no active day anchoring the chain yet
        if (budget <= 0)
            break; // out of freezes — chain ends here
        if (keyOf(cursor) <= minActiveKey)
            break; // nothing older to reach — don't waste a freeze
        budget--;
        used++;
        frozen.push(keyOf(cursor));
        stepBack(cursor);
    }
 ...
 */
function shownStreak(log, now = Date.now()) {
 ...
 */
function streakRisk(log, now = Date.now()) {
    const days = new Set(activeDays(log));
    const cursor = dayCursor(now);
...
const est1RM = (weight, reps) => Math.round(weight * (1 + reps / 30));
exports.est1RM = est1RM;
/**
 ...
 */
function personalRecords(log, history = []) {
    const best = new Map();
    for (const e of log) {
        if (!e.sets)
            continue;
        for (let i = 0; i < e.sets.length; i++) {
            ...
            if (!reps)
                continue;
            const weight = (0, bodyweightSets_1.setLoadKg)(e, i, e.sets[i], history, e.t);
            if (weight == null || weight <= 0)
                continue;
            const e1 = (0, exports.est1RM)(weight, reps);
            const cur = best.get(e.exercise);
            if (!cur || e1 > cur.est1RM)
                best.set(e.exercise, {
                    exercise: e.exercise, weight, reps, est1RM: e1, at: e.t,
                    bodyweight: (0, bodyweightSets_1.isBodyweightSet)(e, i),
                    addedKg: (0, bodyweightSets_1.isBodyweightSet)(e, i) ? Math.max(0, e.sets[i][1] || 0) : 0,
                });
        }
    }
 ...
 */
function isNewPR(log, entry, history = []) {
    if (!entry.sets)
        return false;
    ...
    const top = (e) => Math.max(0, ...(e.sets ?? []).map((s, i) => {
        if ((0, timedSets_1.isTimedSet)(e, i))
            return 0;
        const w = (0, bodyweightSets_1.setLoadKg)(e, i, s, history, e.t);
        return w != null && s[0] ? (0, exports.est1RM)(w, s[0]) : 0;
    }));
 ...
 */
function statsSince(log, sinceMs, history = []) {
 ...
 */
function weekStats(log, now = Date.now(), history = []) {
 ...
 */
function thisWeekStats(log, now = Date.now(), history = []) {
    return statsSince(log, (0, weekStart_1.startOfWeek)(now).getTime(), history);
}
...
function streakMilestone(streak) {
    ...
    if (streak >= 14)
        return `${streak}-day streak — two weeks strong! 🔥`;
    if (streak >= 7)
        return `${streak}-day streak — a full week! 🔥`;
    if (streak >= 3)
