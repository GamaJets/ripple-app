...
exports.MAX_END_NOTE = exports.CLIENT_END_CONFIRM_TITLE = exports.CLIENT_END_EXPLAINER = exports.CLIENT_END_REASON_NOTE = exports.CLIENT_END_REASON_LABEL = exports.CLIENT_END_REASONS = exports.END_REASON_NOTE = exports.END_REASON_LABEL = exports.END_REASONS = void 0;
exports.coachLabel = coachLabel;
exports.replaceCoachNote = replaceCoachNote;
exports.leaveCoachPrompt = leaveCoachPrompt;
exports.leaveOutcome = leaveOutcome;
exports.endCoachingErrorMessage = endCoachingErrorMessage;
exports.endCoaching = endCoaching;
exports.clientEndConfirmBody = clientEndConfirmBody;
exports.clientEndOutcomeLine = clientEndOutcomeLine;
exports.isEndReason = isEndReason;
exports.reasonAttribution = reasonAttribution;
exports.endReasonPrompt = endReasonPrompt;
exports.endCoachingWithReason = endCoachingWithReason;
exports.recordEndReason = recordEndReason;
exports.fetchEndRecord = fetchEndRecord;
exports.departureTally = departureTally;
exports.departureLine = departureLine;
/* ── pure ─────────────────────────────────────────────────────────────────
 ...
 */
function replaceCoachNote(currentName, newName) {
    const now = coachLabel(currentName);
    const next = coachLabel(newName);
    return (`${now} coaches you now, and this app gives one person one coach. If ${next} accepts, `
 ...
 */
function leaveCoachPrompt(coachName) {
    const who = coachLabel(coachName);
    return {
        title: `Leave ${who}?`,
        body: `${who} stops being able to see your workouts, measurements, check-ins, habits, scans, food logs, goals and daily targets, and your message thread with them closes.\n\n` +
 ...
 */
function leaveOutcome(result, coachName) {
    const who = coachLabel(coachName);
    if (!result.ok) {
        return {
            title: 'Still Linked',
 ...
 */
function endCoachingErrorMessage(raw) {
    const m = (raw || '').toLowerCase();
    if (m.includes('not signed in'))
        return 'You are not signed in, so nothing could be changed.';
    if (m.includes('with yourself'))
        return 'That is your own account, so there is no coaching relationship to end.';
    if (m.includes('no one to end coaching with'))
        return 'We could not tell which coach you meant.';
    const t = (raw || '').trim();
 ...
 */
async function endCoaching(otherId) {
    const id = (otherId || '').trim();
    if (!id)
        return { ok: false, reason: endCoachingErrorMessage('no one to end coaching with') };
    try {
...
exports.CLIENT_END_REASON_NOTE = {
...
exports.CLIENT_END_EXPLAINER = 'Your coach sees this. It is the only thing that will ever tell them why people leave, and it is filed as your own words rather than as their guess. You can skip it and still leave.';
/** The heading over the confirmation, and what it does and does not do. */
exports.CLIENT_END_CONFIRM_TITLE = 'Leave this coach?';
function clientEndConfirmBody(name) {
...
function clientEndOutcomeLine(ended, reasonGiven, reasonStored) {
    if (!ended) {
    ...
    if (!reasonGiven)
        return `${head} Nothing was recorded about why.`;
    return reasonStored
...
function isEndReason(v) {
    return typeof v === 'string' && exports.END_REASONS.includes(v);
}
/**
 ...
 */
function reasonAttribution(rec) {
    if (rec.reason == null)
        return null;
...
function endReasonPrompt(rec) {
    ...
    return exports.END_REASON_NOTE[rec.reason];
}
 ...
 */
async function endCoachingWithReason(otherId, reason, note) {
    const id = (otherId || '').trim();
    if (!id)
        return { ok: false, reason: endCoachingErrorMessage('no one to end coaching with') };
    const body = (note || '').trim().slice(0, exports.MAX_END_NOTE) || null;
    try {
 ...
 */
async function recordEndReason(otherId, reason, note) {
    const id = (otherId || '').trim();
    if (!id)
        return { ok: false, reason: endCoachingErrorMessage('no one to end coaching with') };
    const body = (note || '').trim().slice(0, exports.MAX_END_NOTE) || null;
 ...
 */
async function fetchEndRecord(otherId, meId) {
    const id = (otherId || '').trim();
    if (!id || !meId)
        return null;
    try {
 ...
 */
function departureTally(rows) {
    if (rows == null)
        return null;
    const by = new Map();
    let unrecorded = 0;
    for (const r of rows) {
        if (isEndReason(r.reason))
            by.set(r.reason, (by.get(r.reason) ?? 0) + 1);
        else
            unrecorded++;
    }
    const counts = [...by.entries()]
        .map(([reason, n]) => ({ reason, n }))
        .sort((a, b) => b.n - a.n || exports.END_REASONS.indexOf(a.reason) - exports.END_REASONS.indexOf(b.reason));
 ...
 */
function departureLine(tally, windowDays) {
    ...
    const lead = `${people}. The commonest reason recorded is ${exports.END_REASON_LABEL[top.reason].toLowerCase()}, against ${top.n} of them.`;
    if (tally.unrecorded === 0)
        return lead;
    return `${lead} ${tally.unrecorded} ${tally.unrecorded === 1 ? 'has' : 'have'} nothing recorded at all, which is not the same as ${exports.END_REASON_LABEL.unsaid.toLowerCase()}.`;
}
