...
const ago = (days, kind = 'workout') => ({ at: new Date(NOW - days * DAY).toISOString(), kind });
...
const pattern = (from, to, kind = 'workout') => {
    const out = [];
...
const D = (events, since = agoIso(200)) => (0, clientDrift_1.assessDrift)({ clientId: 'c', events, since }, NOW, clientDrift_1.DEFAULT_WINDOWS);
...
for (const [who, d] of [['quiet', dQuiet], ['nothing', dNothing], ['slipping', dSlipping], ['steady', dSteady]]) {
    const draft = (0, nudge_1.draftMessage)('Sam Okafor', d);
    eqJson((0, nudge_1.refusalsIn)(draft), [], `the draft for a ${who} client claims nothing the record cannot support`);
    eqJson((0, nudge_1.refusalsIn)((0, nudge_1.observedLine)(d)), [], `and neither does the observed line for a ${who} client`);
...
ok((0, nudge_1.draftMessage)('Sam', dQuiet).includes('14 days')
    || (0, nudge_1.draftMessage)('Sam', dQuiet).includes(`${dQuiet.quietDays} days`), 'the draft says how long the app has had nothing, because that is the observation');
ok(/\bapp\b/.test((0, nudge_1.draftMessage)('Sam', dQuiet)), 'and attributes it to the app, so a client who trained without logging can say so');
ok(!/you have not|you haven't|didn't train|did not train/i.test((0, nudge_1.draftMessage)('Sam', dQuiet)), 'and never states that they did not train, which is the thing the record does not know');
...
for (const word of ['njur', 'away', 'gym', 'payment']) {
    ok(nudge_1.WHAT_IT_CANNOT_SEE.toLowerCase().includes(word.toLowerCase()), `the caveat names what the signal cannot see: ${word}`);
}
ok((0, nudge_1.refusalsIn)(nudge_1.WHAT_IT_CANNOT_SEE).length > 0, 'and it would be refused inside a draft — which is the point: it is written to the COACH');
...
ok(nudge_1.NEVER_SAYS.length >= 15, 'the refusal list is not empty');
eqJson((0, nudge_1.refusalsIn)('You seem to have lost your motivation lately.'), ['a state of mind'], 'a draft that diagnoses a state of mind is refused');
eqJson((0, nudge_1.refusalsIn)('I know you have been injured, so no rush.'), ['a cause the record cannot see'], 'and one that asserts a cause the app cannot see is refused');
eqJson((0, nudge_1.refusalsIn)('You need to get back in this week.'), ['an instruction, which is the nagging this exists to avoid'], 'and an instruction is refused');
eqJson((0, nudge_1.refusalsIn)('I will be at the gym on Thursday.'), [], 'but "I will" is not "ill" — the boundaries are word boundaries, not substrings');
eqJson((0, nudge_1.refusalsIn)('Still training on Fridays?'), [], 'and "still" is not "ill" either');
eq((0, nudge_1.greetingName)('Sam Okafor'), 'Sam', 'a greeting uses the given name');
eq((0, nudge_1.greetingName)('  '), null, 'an empty name is no greeting, not an empty one');
eq((0, nudge_1.greetingName)('7f3a9c21-0000-4000-8000-000000000000'), null, 'and a uuid is never greeted as a person');
eq((0, nudge_1.greetingName)('sam@example.com'), null, 'nor is an email address');
ok(!(0, nudge_1.draftMessage)(null, dQuiet).startsWith('Hi'), 'with no name the draft opens on the observation rather than on "Hi ,"');
/* ── NEVER-NAG · one approach, then silence for a stated window ───────────── */
const rec = (over) => ({
    id: 'r1', clientId: 'c1', at: agoIso(1), mutedDays: 14, observed: null, ...over,
});
eq((0, nudge_1.mutedBy)([rec({ action: 'sent', at: agoIso(3), mutedDays: 14 })], 'c1', NOW)?.daysLeft, 11, 'a client messaged three days ago is muted for the eleven that are left');
eq((0, nudge_1.mutedBy)([rec({ action: 'sent', at: agoIso(15), mutedDays: 14 })], 'c1', NOW), null, 'and comes back the day after the window closes');
eq((0, nudge_1.mutedBy)([rec({ action: 'dismissed', at: agoIso(20), mutedDays: 30 })], 'c1', NOW)?.daysLeft, 10, 'a client set aside twenty days ago is still set aside');
eq((0, nudge_1.mutedBy)([rec({ action: 'sent' })], 'someone-else', NOW), null, 'a record about one client does not mute another');
...
eq((0, nudge_1.mutedBy)([rec({ action: 'dismissed', at: agoIso(40), mutedDays: 60 })], 'c1', NOW)?.daysLeft, 20, 'the window is the one that was written down, not the one today’s constants would give');
...
eq((0, nudge_1.mutedBy)([rec({ action: 'sent', at: 'not a date' })], 'c1', NOW), null, 'a record nobody can date mutes nothing');
eq((0, nudge_1.mutedBy)([rec({ action: 'sent', at: agoIso(1), mutedDays: 0 })], 'c1', NOW), null, 'and a zero-day window is not a mute');
eq((0, nudge_1.mutedBy)([rec({ action: 'sent', at: agoIso(1), mutedDays: -5 })], 'c1', NOW), null, 'nor is a negative one');
// The windows themselves come from the client's own rhythm, not from a fixed
// number of days for everybody.
const sentQuiet = (0, nudge_1.mutedDaysFor)('sent', dQuiet);
ok(sentQuiet >= 7 && sentQuiet <= 28, 'a send mutes for a span paced off their own baseline');
eq((0, nudge_1.mutedDaysFor)('dismissed', dQuiet), Math.max(nudge_1.DISMISS_FLOOR_DAYS, sentQuiet), 'and setting aside is never shorter than a month');
ok((0, nudge_1.mutedDaysFor)('dismissed', dNothing) >= nudge_1.DISMISS_FLOOR_DAYS, 'including for a client with no pattern at all to pace against');
/* ── the board refuses to nag ─────────────────────────────────────────────── */
const cand = (id, events, name = 'Sam ' + id) => ({ clientId: id, name, activity: { read: true, events }, since: agoIso(200) });
const book = [
    cand('a', wentQuiet), cand('b', steady), cand('c', slipping), cand('d', []),
];
const plain = (0, nudge_1.buildNudgeBoard)(book, [], { now: NOW });
eqJson(plain.nudges.map((n) => n.clientId), ['a', 'd'], 'only the client who broke their pattern and the one with no record at all are suggested');
...
ok((0, nudge_1.earnsNudge)(dSlipping) === false, 'the watch band earns no suggestion — a busy fortnight is not a call to make');
const afterSend = (0, nudge_1.buildNudgeBoard)(book, [
    { id: 'r', clientId: 'a', action: 'sent', at: agoIso(2), mutedDays: 14, observed: 'Nothing for 15 days.' },
], { now: NOW });
eqJson(afterSend.nudges.map((n) => n.clientId), ['d'], 'a client messaged two days ago is not suggested again');
...
const afterDismiss = (0, nudge_1.buildNudgeBoard)(book, [
    { id: 'r', clientId: 'd', action: 'dismissed', at: agoIso(5), mutedDays: 30, observed: null },
], { now: NOW });
eqJson(afterDismiss.nudges.map((n) => n.clientId), ['a'], 'a client the coach set aside is not offered again');
const longAfter = (0, nudge_1.buildNudgeBoard)(book, [
    { id: 'r', clientId: 'd', action: 'dismissed', at: agoIso(40), mutedDays: 30, observed: null },
], { now: NOW });
eqJson(longAfter.nudges.map((n) => n.clientId), ['a', 'd'], 'but they do come back once the window has passed — a set-aside is not a delete');
...
const hr = (0, nudge_1.buildNudgeBoard)(halfRead, [], { now: NOW });
eqJson(hr.nudges.map((n) => n.clientId), ['a'], 'a client whose record did not come back is NEVER suggested, however empty it looks');
...
for (const w of hr.withheld) {
    eq(w.note, nudge_1.WITHHELD_NOTE[w.why], 'each withheld client carries the reason in words');
...
const emptyRead = (0, nudge_1.buildNudgeBoard)([cand('d', [])], [], { now: NOW });
const failedRead = (0, nudge_1.buildNudgeBoard)([{ clientId: 'd', name: 'Sam d', activity: { read: false, why: 'read-failed' } }], [], { now: NOW });
...
// calm week.
ok((0, nudge_1.boardNote)(hr).includes('not the whole book'), 'the note says the list is incomplete when anybody could not be assessed');
eq((0, nudge_1.boardNote)(null), 'Reading who has gone quiet…', 'and before the read lands it claims no number at all');
ok((0, nudge_1.boardNote)((0, nudge_1.buildNudgeBoard)([cand('b', steady)], [], { now: NOW })).startsWith('Nobody to chase'), 'an assessed book with nobody to contact says so plainly');
ok((0, nudge_1.boardNote)((0, nudge_1.buildNudgeBoard)([], [], { now: NOW })).includes('Nobody could be assessed'), 'an empty book is not a clear week');
/* ── ordering ─────────────────────────────────────────────────────────────── */
const ordered = (0, nudge_1.buildNudgeBoard)([cand('d', []), cand('a', wentQuiet)], [], { now: NOW });
eqJson(ordered.nudges.map((n) => n.clientId), ['a', 'd'], 'the measurable break leads, and the client nothing is known about is second — never last');
/* ── EVIDENCE · the dates behind the figure ───────────────────────────────── */
const ev = (0, nudge_1.explainDrift)({ drift: dQuiet, events: wentQuiet }, NOW, clientDrift_1.DEFAULT_WINDOWS);
eq(ev.recentDays.length, 0, 'nothing in the near window');
eq(ev.lastSeen?.day, (0, clientDrift_1.localDayKey)(NOW - 15 * DAY), 'and the last thing on record is the day it actually happened');
eq(ev.baselineDays.length, 21, 'twenty-one active days sit behind the baseline rate');
eqJson(ev.baselineDays.map((d) => d.day).slice(0, 2), [(0, clientDrift_1.localDayKey)(NOW - 55 * DAY), (0, clientDrift_1.localDayKey)(NOW - 53 * DAY)], 'listed oldest first, as the actual days');
eq(ev.window.today, (0, clientDrift_1.localDayKey)(NOW), 'the window ends today');
eq(ev.window.recentFrom, (0, clientDrift_1.localDayKey)(NOW - clientDrift_1.DEFAULT_WINDOWS.recentDays * DAY), 'and the near window starts where assessDrift starts it');
...
const withDoor = (0, nudge_1.explainDrift)({ drift: dQuiet, events: wentQuiet, doorLogRead: true }, NOW);
...
ok(ev.lines.some((l) => l.includes((0, nudge_1.readableDay)((0, clientDrift_1.localDayKey)(NOW - 15 * DAY)))), 'the printed lines carry the real date, not only the count of days');
ok(ev.lines[ev.lines.length - 1] === nudge_1.WHAT_IT_CANNOT_SEE, 'and the last thing a coach reads under the working is what it cannot see');
const evNothing = (0, nudge_1.explainDrift)({ drift: dNothing, events: [] }, NOW);
eq(evNothing.lastSeen, null, 'a client with nothing on record has no last day');
ok(evNothing.lines[0].includes('Nothing on record at all'), 'and the working says so rather than printing a blank date');
if (errors.length) {
