...
const has = (s, needle, msg) => {
    if (typeof s !== 'string' || !s.includes(needle))
        errors.push(`${msg} — got ${JSON.stringify(s)}`);
};
const eq = (a, b, msg) => {
...
const UNSOUND = ['loading', 'partial', 'error'];
const shoulder = { id: 'i1', area: 'shoulder', severity: 'severe', status: 'active', note: '', at: '' };
const knee = { id: 'i2', area: 'knee', severity: 'mild', status: 'active', note: '', at: '' };
/** A member with nothing to acknowledge and both reads whole — the one shape
 *  that is allowed straight through. */
const clear = (id, name) => ({ clientId: id, name, disclosures: 'ready', ackStatus: 'ready', injuries: [], acknowledged: null });
/** A member who has disclosed something nobody has confirmed reading. */
const undisclosed = (id, name) => ({ clientId: id, name, disclosures: 'ready', ackStatus: 'ready', injuries: [shoulder], acknowledged: null });
/** A member who has disclosed something and whose coach HAS read it. */
const acknowledged = (id, name) => ({ clientId: id, name, disclosures: 'ready', ackStatus: 'ready', injuries: [shoulder], acknowledged: [(0, injuryGate_1.injuryKey)(shoulder)] });
const P = (title, name, sets, reps) => ({
...
// say who is off-plan.
{
    ...
    eq((0, groupProgram_1.programSignature)(a), (0, groupProgram_1.programSignature)(b), 'focus, note, key and alternatives must not change a programme fingerprint');
    ok((0, groupProgram_1.programSignature)(a) !== (0, groupProgram_1.programSignature)(P('Bootcamp', 'Back Squat', 5, '8-10')), 'a different set count is a different programme');
    ok((0, groupProgram_1.programSignature)(a) !== (0, groupProgram_1.programSignature)(P('Bootcamp', 'Back Squat', 4, '5-6')), 'a different rep range is a different programme');
    ok((0, groupProgram_1.programSignature)(a) !== (0, groupProgram_1.programSignature)(P('Bootcamp', 'Front Squat', 4, '8-10')), 'a different movement is a different programme');
    ok((0, groupProgram_1.programSignature)(a) !== (0, groupProgram_1.programSignature)(P('Bootcamp 2', 'Back Squat', 4, '8-10')), 'a different title is a different programme');
    eq((0, groupProgram_1.programSignature)(null), null, 'a group with no programme has no fingerprint, not the fingerprint of an empty one');
    eq((0, groupProgram_1.programSignature)(undefined), null, 'an absent programme has no fingerprint');
}
// ══ memberState ════════════════════════════════════════════════════════════
//
...
// assign over a programme nobody saw.
{
    ...
    const sig = (0, groupProgram_1.programSignature)(group);
    eq((0, groupProgram_1.memberState)('ready', sig, group), 'on', 'a client on the group programme is on it');
    eq((0, groupProgram_1.memberState)('ready', sig, null), 'none', 'a whole read with no row is genuinely nobody assigned');
    eq((0, groupProgram_1.memberState)('ready', sig, P('Shoulder-safe', 'Leg Press', 4, '8-10')), 'diverged', 'a client on something else has diverged');
    for (const s of UNSOUND) {
        eq((0, groupProgram_1.memberState)(s, sig, null), 'unknown', `'${s}' must not be reported as "not assigned yet"`);
        eq((0, groupProgram_1.memberState)(s, sig, group), 'unknown', `'${s}' must not be reported as on the programme either`);
    }
    ...
    eq((0, groupProgram_1.memberState)('ready', null, group), 'diverged', 'with no group programme, a client on one is on something else');
    eq((0, groupProgram_1.memberState)('ready', null, null), 'none', 'with no group programme and no row, nobody is on anything');
}
// ══ groupCoverage ══════════════════════════════════════════════════════════
//
...
{
    const states = ['on', 'on', 'diverged', 'none', 'unknown'];
    const c = (0, groupProgram_1.groupCoverage)(states, 'ready', 'ready');
    ...
    for (const s of UNSOUND) {
        ok(!(0, groupProgram_1.groupCoverage)(states, s, 'ready').countable, `a '${s}' membership read must not license a headline count`);
        ok(!(0, groupProgram_1.groupCoverage)(states, 'ready', s).countable, `a '${s}' programme read must not license a headline count`);
    }
    eq((0, groupProgram_1.groupCoverage)([], 'ready', 'ready').total, 0, 'an empty group under a whole read is genuinely empty');
}
...
// four people on last month's programme with nothing anywhere saying so.
{
    ...
    for (const s of UNSOUND) {
        const p = (0, groupProgram_1.planFanOut)(s, 'ready', members, true, SUBJECT);
    ...
    const reasons = UNSOUND.map((s) => (0, groupProgram_1.planFanOut)(s, 'ready', members, true, SUBJECT).reason);
...
{
    const members = [clear('a', 'Priya'), clear('b', 'Sam')];
    ok((0, groupProgram_1.planFanOut)('ready', 'ready', members, true, SUBJECT).allowed, 'two whole reads and nothing disclosed must let the assign run');
    for (const s of UNSOUND) {
        const p = (0, groupProgram_1.planFanOut)('ready', s, members, true, SUBJECT);
    ...
    for (const s of ['partial', 'error']) {
        const r = (0, groupProgram_1.planFanOut)('ready', s, members, true, SUBJECT).reason;
...
{
    const p = (0, groupProgram_1.planFanOut)('ready', 'ready', [clear('a', 'Priya')], false, SUBJECT);
    ok(!p.allowed, 'a group with no programme must not assign one');
    eq(p.send, [], 'a group with no programme must write to nobody');
    const e = (0, groupProgram_1.planFanOut)('ready', 'ready', [], true, SUBJECT);
...
{
    const noProgGroup = (0, groupProgram_1.planFanOut)('ready', 'ready', [clear('a', 'Priya')], false, SUBJECT);
    const noProgBuild = (0, groupProgram_1.planFanOut)('ready', 'ready', [clear('a', 'Priya')], false, SUBJECT, 'written');
    ...
    ok(/group/i.test(noProgGroup.reason), 'the group screen still talks about a group');
    ok(!/group/i.test(noProgBuild.reason) && !/group/i.test(noProgBuild.label), 'and the builder never mentions a group, because there is not one on that screen');
    ok(!/library/i.test(noProgBuild.reason), 'nor sends a coach who is writing a programme off to pick one from their library');
    ok(/exercise/i.test(noProgBuild.reason), 'it names what is actually missing, which is exercises');
    const noneGroup = (0, groupProgram_1.planFanOut)('ready', 'ready', [], true, SUBJECT);
    const noneBuild = (0, groupProgram_1.planFanOut)('ready', 'ready', [], true, SUBJECT, 'written');
    eq(noneGroup.code, 'nobody', 'an empty recipient list is its own refusal');
    eq(noneBuild.code, 'nobody', 'on both screens');
    ok(/group/i.test(noneGroup.label), 'which for a group is about who is in the group');
    ok(!/group/i.test(noneBuild.reason) && !/group/i.test(noneBuild.label), 'and in the builder is about who the coach has ticked, because nobody has been added to anything');
    ok(/tick/i.test(noneBuild.reason), 'and it names the gesture that fixes it');
}
// The default is the group wording, so no existing caller changed behaviour by
// this parameter being added.
{
    const a = (0, groupProgram_1.planFanOut)('ready', 'ready', [clear('a', 'Priya')], false, SUBJECT);
    const b = (0, groupProgram_1.planFanOut)('ready', 'ready', [clear('a', 'Priya')], false, SUBJECT, 'chosen');
...
{
    const holds = [
        (0, groupProgram_1.planFanOut)('loading', 'ready', [clear('a', 'Priya')], true, SUBJECT),
        (0, groupProgram_1.planFanOut)('partial', 'ready', [clear('a', 'Priya')], true, SUBJECT),
        (0, groupProgram_1.planFanOut)('error', 'ready', [clear('a', 'Priya')], true, SUBJECT),
        (0, groupProgram_1.planFanOut)('ready', 'error', [clear('a', 'Priya')], true, SUBJECT),
        (0, groupProgram_1.planFanOut)('ready', 'ready', [clear('a', 'Priya')], false, SUBJECT),
        (0, groupProgram_1.planFanOut)('ready', 'ready', [], true, SUBJECT),
        (0, groupProgram_1.planFanOut)('ready', 'ready', [undisclosed('a', 'Priya')], true, SUBJECT),
    ];
    ...
    eq((0, groupProgram_1.planFanOut)('ready', 'ready', [clear('a', 'Priya')], true, SUBJECT).code, null, 'an assign that may go ahead is refusing nothing and says so with a null');
}
...
{
    const blockedFirst = (0, groupProgram_1.planFanOut)('ready', 'ready', [undisclosed('a', 'Priya'), clear('b', 'Sam')], true, SUBJECT);
    ...
    const blockedLast = (0, groupProgram_1.planFanOut)('ready', 'ready', [clear('a', 'Priya'), undisclosed('b', 'Sam')], true, SUBJECT);
    ...
    const read = (0, groupProgram_1.planFanOut)('ready', 'ready', [acknowledged('a', 'Priya'), clear('b', 'Sam')], true, SUBJECT);
    ...
    const stale = { clientId: 'a', name: 'Priya', disclosures: 'ready', ackStatus: 'ready', injuries: [shoulder, knee], acknowledged: [(0, injuryGate_1.injuryKey)(shoulder)] };
    eq((0, groupProgram_1.planFanOut)('ready', 'ready', [stale, clear('b', 'Sam')], true, SUBJECT).send, ['b'], 'a disclosure made since the acknowledgement must hold the client again');
    ...
    for (const s of UNSOUND) {
        const unreadDisclosures = { clientId: 'a', name: 'Priya', disclosures: s, ackStatus: 'ready', injuries: [], acknowledged: null };
        eq((0, groupProgram_1.planFanOut)('ready', 'ready', [unreadDisclosures, clear('b', 'Sam')], true, SUBJECT).send, ['b'], `a '${s}' read of a client's own disclosures must hold that client, empty list or not`);
        const unreadAcks = { clientId: 'a', name: 'Priya', disclosures: 'ready', ackStatus: s, injuries: [shoulder], acknowledged: null };
        eq((0, groupProgram_1.planFanOut)('ready', 'ready', [unreadAcks, clear('b', 'Sam')], true, SUBJECT).send, ['b'], `a '${s}' read of the acknowledgements must hold a client who has disclosed something`);
    }
}
...
// reported. `send` and `blocked` must partition the membership exactly.
{
    ...
    const p = (0, groupProgram_1.planFanOut)('ready', 'ready', members, true, SUBJECT);
...
{
    const p = (0, groupProgram_1.planFanOut)('ready', 'ready', [undisclosed('a', 'Priya'), undisclosed('b', 'Sam')], true, SUBJECT);
    ...
    const one = (0, groupProgram_1.planFanOut)('ready', 'ready', [undisclosed('a', 'Priya')], true, SUBJECT);
...
// ══ the sentences ══════════════════════════════════════════════════════════
{
    eq((0, groupProgram_1.listNames)([]), '', 'no names is no sentence');
    eq((0, groupProgram_1.listNames)(['Priya']), 'Priya', 'one name is the name');
    eq((0, groupProgram_1.listNames)(['Priya', 'Sam']), 'Priya and Sam', 'two names are joined with "and", not a comma');
    eq((0, groupProgram_1.listNames)(['Priya', 'Sam', 'Alex']), 'Priya, Sam and Alex', 'three names read as a list a person would say aloud');
    ok((0, groupProgram_1.fanOutSubject)(1) !== (0, groupProgram_1.fanOutSubject)(4), 'one client and several must not be described with the same sentence');
    ok(/programme this client/.test((0, groupProgram_1.fanOutSubject)(1)), 'a single client must be spoken about in the singular');
}
if (errors.length) {
