...
const night = (id, at, hours = 7) => ({ id, at, hours });
/* ── ids: local and server must be distinguishable ──────────────────────── */
ok((0, wellnessSync_1.isPending)((0, wellnessSync_1.localId)()), 'a freshly minted id is pending');
ok(!(0, wellnessSync_1.isPending)('9f1c3a2e-0b4d-4c8a-9f21-6b0e1d2c3a4b'), 'a uuid is not pending');
// The specific trap availability.ts's `id.includes('-')` walks into. A uuid has
// hyphens; so, one day, will something else.
ok(!(0, wellnessSync_1.isPending)('a-b-c'), 'a hyphenated non-uuid is still not pending — the prefix decides, not the shape');
ok((0, wellnessSync_1.localId)() !== (0, wellnessSync_1.localId)(), 'two ids minted in the same millisecond differ');
ok((0, wellnessSync_1.localId)().startsWith(wellnessSync_1.LOCAL_PREFIX), 'local ids carry the prefix');
/* ── mergeLog: null is not empty ────────────────────────────────────────── */
const offline = [night(wellnessSync_1.LOCAL_PREFIX + '1', '2026-08-30T22:00:00Z')];
{
    // The read failed. Everything the device holds stays, and stays pending.
    const m = (0, wellnessSync_1.mergeLog)(null, offline);
...
}
{
    ...
    const m = (0, wellnessSync_1.mergeLog)([], offline);
...
}
{
    ...
    const m = (0, wellnessSync_1.mergeLog)([], stale);
    eq(m.entries.length, 0, 'a confirmed-gone server row is dropped on a successful read');
    const m2 = (0, wellnessSync_1.mergeLog)(null, stale);
...
}
{
    ...
    const m = (0, wellnessSync_1.mergeLog)(server, offline);
    eq(m.entries.length, 3, 'server rows plus the unsent one');
    eq(m.entries[0].id, wellnessSync_1.LOCAL_PREFIX + '1', 'newest first, and the offline night is the newest');
...
}
{
    ...
    const dup = [night(wellnessSync_1.LOCAL_PREFIX + '1', '2026-08-30T22:00:00Z')];
    const m = (0, wellnessSync_1.mergeLog)(dup, offline);
...
/* ── ordering is total and stable ───────────────────────────────────────── */
{
    ...
    const sorted = [...same].sort(wellnessSync_1.byNewest);
    eq(sorted[0].id, 'b', 'a shared timestamp is broken by id, so the order cannot wobble between renders');
    eq([...sorted].sort(wellnessSync_1.byNewest)[0].id, 'b', 'and sorting an already-sorted list does not change it');
}
/* ── adopting the id the server assigned ────────────────────────────────── */
{
    const list = [night(wellnessSync_1.LOCAL_PREFIX + '1', '2026-08-30T22:00:00Z'), night('srv', '2026-08-29T22:00:00Z')];
    const after = (0, wellnessSync_1.adoptServerId)(list, wellnessSync_1.LOCAL_PREFIX + '1', 'srv-new');
    eq(after[0].id, 'srv-new', 'the local id is replaced');
    eq((0, wellnessSync_1.isPending)(after[0].id), false, 'so the entry stops being pending');
    eq(after[1].id, 'srv', 'and nothing else moves');
    eq(list[0].id, wellnessSync_1.LOCAL_PREFIX + '1', 'the input list is not mutated');
    ...
    eq((0, wellnessSync_1.adoptServerId)([night('srv', '2026-08-29T22:00:00Z')], wellnessSync_1.LOCAL_PREFIX + '9', 'x').length, 1, 'adopting an id that is no longer in the list adds nothing');
}
/* ── mergeCount: the correction that must not undo itself ───────────────── */
{
    eq((0, wellnessSync_1.mergeCount)(null, null).count, 0, 'nothing anywhere is zero glasses');
    eq((0, wellnessSync_1.mergeCount)(null, null).push, false, 'and there is nothing to send');
    eq((0, wellnessSync_1.mergeCount)(null, { count: 4, at: '2026-08-31T09:00:00Z' }).count, 4, 'device only: the device wins');
    eq((0, wellnessSync_1.mergeCount)(null, { count: 4, at: '2026-08-31T09:00:00Z' }).push, true, 'and the server has to be told');
    eq((0, wellnessSync_1.mergeCount)({ count: 6, at: '2026-08-31T09:00:00Z' }, null).count, 6, 'server only: the server wins');
    eq((0, wellnessSync_1.mergeCount)({ count: 6, at: '2026-08-31T09:00:00Z' }, null).push, false, 'and nothing needs sending');
    // The bug. Local 5 is a correction made after the server saw 6.
    const corrected = (0, wellnessSync_1.mergeCount)({ count: 6, at: '2026-08-31T09:00:00Z' }, { count: 5, at: '2026-08-31T10:00:00Z' });
    ...
    const elsewhere = (0, wellnessSync_1.mergeCount)({ count: 7, at: '2026-08-31T11:00:00Z' }, { count: 5, at: '2026-08-31T10:00:00Z' });
    ...
    const tie = (0, wellnessSync_1.mergeCount)({ count: 7, at: '2026-08-31T11:00:00Z' }, { count: 5, at: '2026-08-31T11:00:00Z' });
    ...
    const zeroed = (0, wellnessSync_1.mergeCount)({ count: 3, at: '2026-08-31T09:00:00Z' }, { count: 0, at: '2026-08-31T10:00:00Z' });
...
/* ── the clamp tracks the column, and the column tracks the goal ────────── */
{
    ...
    // nobody can reach.
    eq(wellnessSync_1.WATER_CAP, 30, 'the cap matches clients_water_goal_glasses_check and hydration_logs_glasses_check');
    eq((0, wellnessSync_1.clampGlasses)(31), 30, 'above the cap clamps down');
    eq((0, wellnessSync_1.clampGlasses)(-1), 0, 'below zero clamps up');
    eq((0, wellnessSync_1.clampGlasses)(3.4), 3, 'a fraction of a glass rounds');
    eq((0, wellnessSync_1.clampGlasses)(3.6), 4, 'and rounds the other way too');
    eq((0, wellnessSync_1.clampGlasses)(0), 0, 'zero survives');
    ...
    eq((0, wellnessSync_1.clampGlasses)(NaN), 0, 'NaN is not a count');
    eq((0, wellnessSync_1.clampGlasses)(Infinity), 0, 'and neither is infinity — it is the absence of one, not a big one');
    eq((0, wellnessSync_1.clampGlasses)(-Infinity), 0, 'from either end');
}
if (errors.length) {
