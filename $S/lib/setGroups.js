...
exports.canJoinNext = canJoinNext;
exports.isGrouped = isGrouped;
exports.joinNext = joinNext;
...
function groupRuns(items) {
    ...
    while (i < items.length) {
        ...
        let j = i + 1;
        while (j < items.length && items[j]?.setGroupId === id)
            j += 1;
        if (j - i >= 2)
            runs.push({ start: i, size: j - i, id });
        i = j;
    }
    return runs;
}
 ...
 */
function joinNext(items, at, mint) {
    const ids = items.map((x) => x.setGroupId ?? null);
    if (!canJoinNext(items, at))
        return ids;
