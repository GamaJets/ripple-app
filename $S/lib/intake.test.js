...
function fullIntake() {
    const i = (0, intake_1.emptyIntake)(NOW);
    for (const q of intake_1.READINESS_QUESTIONS)
        i.readiness[q.id] = { answer: 'no' };
...
/* ── the seven questions ────────────────────────────────────────────────── */
eq(intake_1.INTAKE_VERSION, 1, 'the document format on disk is version 1');
eq((0, intake_1.emptyIntake)(NOW).version, intake_1.INTAKE_VERSION, 'and a fresh document is stamped with it');
eq(intake_1.READINESS_QUESTIONS.length, 7, 'seven readiness questions');
eq(new Set(intake_1.READINESS_QUESTIONS.map((q) => q.id)).size, 7, 'and each has its own id');
ok(intake_1.READINESS_QUESTIONS.every((q) => q.prompt.trim().endsWith('?')), 'every one of them is asked as a question');
...
// about the person answering it.
{
    ...
    const disclosed = (0, intake_1.readinessDisclosed)(i);
    ...
    const order = (0, intake_1.readinessDisclosed)(j).map((d) => d.id);
    ...
    const note = (0, intake_1.readinessNote)(disclosed, (0, intake_1.readinessUnanswered)(i), 'Priya');
    ...
    ok(!/\b(high|low|moderate|elevated|severe)[- ]?risk\b/i.test(note)
        ...
        && !/\b\d+\s*(\/|out of)\s*\d+\b/.test(note), `the coach's sentence grades nobody — got: ${note}`);
    ok(/does not score it/i.test(note), 'and says outright that nothing scored it');
    ok(note.includes(intake_1.READINESS_NOT_ADVICE), 'and says plainly what this is and is not');
    ok(note.includes(intake_1.READINESS_SEE_A_DOCTOR), 'and carries the referral where something was answered yes');
    ok(note.startsWith('Priya answered yes to the following'), `two yeses and nothing unanswered opens by saying so — got: ${note}`);
}
...
// and still pass everything above it.
{
    ...
    const clean = (0, intake_1.readinessNote)([], (0, intake_1.readinessUnanswered)(allNo), 'Priya');
    ok(clean.startsWith('Priya answered no to every readiness question'), `all seven answered no says exactly that — got: ${clean}`);
    ok(!/still unanswered/.test(clean), 'and does not mention unanswered questions when there are none');
    const halfWay = (0, intake_1.emptyIntake)(NOW);
    halfWay.readiness.heart = { answer: 'no' };
    const partial = (0, intake_1.readinessNote)([], (0, intake_1.readinessUnanswered)(halfWay), 'Priya');
    ok(partial.startsWith('Priya answered no to the readiness questions they have answered so far'), `one answered no out of seven is hedged, not a clean sheet — got: ${partial}`);
    const oneYes = (0, intake_1.emptyIntake)(NOW);
    oneYes.readiness.joints = { answer: 'yes' };
    const single = (0, intake_1.readinessNote)((0, intake_1.readinessDisclosed)(oneYes), (0, intake_1.readinessUnanswered)(oneYes), 'Priya');
    ok(single.startsWith('Priya answered yes to the following'), `a single yes among unanswered questions still opens with the yes — got: ${single}`);
    ok(/6 of the 7 questions are still unanswered/.test(single), 'and says how many are outstanding');
    ...
    ok(single.includes(intake_1.READINESS_SEE_A_DOCTOR), 'a single yes carries the referral, exactly as two do');
    // One question left is still a question left.
    const nearlyDone = fullIntake();
    delete nearlyDone.readiness[intake_1.READINESS_QUESTIONS[6].id];
    const nearly = (0, intake_1.readinessNote)([], (0, intake_1.readinessUnanswered)(nearlyDone), 'Priya');
    ok(/1 of the 7 questions is still unanswered/.test(nearly), `one outstanding question is still reported — got: ${nearly}`);
}
...
// fails.
{
    const heart = (0, intake_1.emptyIntake)(NOW);
    heart.readiness.heart = { answer: 'yes', note: 'the same note' };
    const joints = (0, intake_1.emptyIntake)(NOW);
    joints.readiness.joints = { answer: 'yes', note: 'the same note' };
    const a = (0, intake_1.readinessDisclosed)(heart)[0];
    const b = (0, intake_1.readinessDisclosed)(joints)[0];
    eq(Object.keys(a).sort().join(','), Object.keys(b).sort().join(','), 'a cardiac yes is not a different kind of object from a joint yes');
...
{
    const half = (0, intake_1.emptyIntake)(NOW);
    half.readiness.heart = { answer: 'no' };
    eq((0, intake_1.readinessDisclosed)(half).length, 0, 'no yeses is no yeses');
    eq((0, intake_1.readinessUnanswered)(half).length, 6, 'and the other six are unanswered, not "no"');
    const note = (0, intake_1.readinessNote)([], (0, intake_1.readinessUnanswered)(half), 'Priya');
    ok(note.includes('not a no'), 'the sentence says an unanswered question is not a no');
    ok(!note.includes(intake_1.READINESS_SEE_A_DOCTOR), 'and does not issue a referral where nothing was answered yes');
}
eq((0, intake_1.readinessUnanswered)(null).length, 7, 'with no document at all, all seven are unanswered');
eq((0, intake_1.readinessDisclosed)(null).length, 0, 'and nothing is disclosed');
...
for (const s of ['loading', 'error', 'partial']) {
    eq((0, intake_1.intakeState)(s, null), 'unknown', `a ${s} read says nothing about whether they filled it in`);
    eq((0, intake_1.intakeState)(s, fullIntake()), 'unknown', `and a ${s} read does not confirm a complete one either`);
}
eq((0, intake_1.intakeState)('ready', null), 'none', 'a finished read of nothing is genuinely nothing');
eq((0, intake_1.intakeState)('ready', (0, intake_1.emptyIntake)(NOW)), 'started', 'a document with no answers in it is started, not absent');
eq((0, intake_1.intakeState)('ready', fullIntake()), 'complete', 'and a full one is complete');
// The sentence follows the state, and the unknown one blames nobody.
{
    const p = (0, intake_1.intakeProgress)(null);
    const unknown = (0, intake_1.intakeLine)('unknown', p, 'Priya');
    ok(/could not be read/i.test(unknown), 'the unknown line says the read failed');
    ok(!/has not (filled|started|answered)/i.test(unknown.replace('This is not a statement that they have not.', '')), `the unknown line does not accuse them — got: ${unknown}`);
    ok((0, intake_1.intakeLine)('none', p, 'Priya').includes('Priya'), 'the others are addressed to them by name');
    eq((0, intake_1.intakePrompt)('unknown', p, 'Priya'), null, 'and a failed read produces no instruction to chase anybody');
    eq((0, intake_1.intakePrompt)('complete', (0, intake_1.intakeProgress)(fullIntake()), 'Priya'), null, 'nor does a finished one');
    ok((0, intake_1.intakePrompt)('none', p, 'Priya') !== null, 'an unstarted intake does prompt');
    ok((0, intake_1.intakePrompt)('none', p, 'Priya').includes('cannot fill it in for them'), 'and says the coach cannot do it for them');
}
/* ── progress ───────────────────────────────────────────────────────────── */
{
    const empty = (0, intake_1.intakeProgress)((0, intake_1.emptyIntake)(NOW));
    eq(empty.done, 0, 'an empty document has nothing done');
    eq(empty.of, intake_1.INTAKE_SECTIONS.length, 'and counts every section');
    ok(!empty.complete, 'and is not complete');
    eq(empty.nextId, intake_1.INTAKE_SECTIONS[0].id, 'the next thing to do is the first section');
    const full = (0, intake_1.intakeProgress)(fullIntake());
    ...
    eq((0, intake_1.intakeProgress)(null).done, 0, 'no document is no progress');
    ok(!(0, intake_1.intakeProgress)(null).complete, 'and is never complete');
}
...
{
    const cases = [
        { id: 'readiness', break_: (i) => { delete i.readiness[intake_1.READINESS_QUESTIONS[3].id]; } },
    ...
    eq(cases.length, intake_1.INTAKE_SECTIONS.length, 'every section has a case here — a new one added without a case is not covered');
    for (const c of cases) {
        const i = fullIntake();
        c.break_(i);
        const p = (0, intake_1.intakeProgress)(i);
...
// Whitespace is not an answer.
{
    const i = fullIntake();
    i.emergency.name = '   ';
    ok(!(0, intake_1.intakeProgress)(i).complete, 'a name of spaces is not a name');
}
...
// name really is one character.
{
    ...
    i.emergency.name = '李';
    i.want.headline = '5';
    ok((0, intake_1.intakeProgress)(i).complete, 'a one-character name is a name, and a one-character goal is a goal');
}
...
{
    for (const field of ['worked', 'didnt', 'wont']) {
        const i = fullIntake();
        i.tried = { worked: '', didnt: '', wont: '' };
        i.tried[field] = 'something';
        ok((0, intake_1.intakeProgress)(i).complete, `${field} on its own finishes what you have tried`);
    }
}
/* ── reading a document back ────────────────────────────────────────────── */
eq((0, intake_1.parseIntake)(null), null, 'no column is no document');
eq((0, intake_1.parseIntake)(undefined), null, 'and neither is undefined');
eq((0, intake_1.parseIntake)('{}'), null, 'a string is not a document');
eq((0, intake_1.parseIntake)([1, 2, 3]), null, 'nor is an array');
ok((0, intake_1.parseIntake)({}) !== null, 'but {} IS a document — somebody who started and saved nothing');
eq((0, intake_1.intakeState)('ready', (0, intake_1.parseIntake)({})), 'started', 'and that is "started", never "they have not filled it in"');
{
    // A round trip keeps every answer.
    const before = fullIntake();
    const after = (0, intake_1.parseIntake)(JSON.parse(JSON.stringify(before)));
    ...
    eq((0, intake_1.intakeProgress)(after).complete, true, 'and it is still complete afterwards');
}
{
    // Rubbish in the column does not throw and does not invent answers.
    const junk = (0, intake_1.parseIntake)({
    ...
    eq(junk.version, intake_1.INTAKE_VERSION, 'a non-numeric version falls back to this build’s');
    ...
    ok(!(0, intake_1.intakeProgress)(junk).complete, 'and none of that reads as a completed intake');
}
{
    // An answer that is not yes/no is not an answer.
    const i = (0, intake_1.parseIntake)({ readiness: { heart: { answer: 'maybe' }, joints: { answer: 'yes' } } });
    eq(Object.keys(i.readiness).join(','), 'joints', 'only yes and no are answers');
    eq((0, intake_1.readinessUnanswered)(i).length, 6, 'and the rejected one counts as unanswered');
}
{
    // An empty note is no note, so the coach's screen never renders empty quotes.
    const i = (0, intake_1.parseIntake)({ readiness: { heart: { answer: 'yes', note: '   ' } } });
    eq((0, intake_1.readinessDisclosed)(i)[0].note, null, 'a whitespace note is no note');
}
{
    ...
    let parsed = null;
    try {
        parsed = (0, intake_1.parseIntake)({ readiness: null, history: null, emergency: null });
    }
    catch {
        threw = true;
    }
    ...
    threw = false;
    try {
        parsed = (0, intake_1.parseIntake)({ readiness: { heart: null, joints: { answer: 'yes' } } });
    }
    catch {
        threw = true;
    }
...
{
    eq((0, intake_1.intakeOwnership)('u1', 'u1').mayEdit, true, 'the person it is about may edit it');
    eq((0, intake_1.intakeOwnership)('u1', 'u1').reason, null, 'with no reason to give');
    const coach = (0, intake_1.intakeOwnership)('coach', 'u1');
    ...
    eq((0, intake_1.intakeOwnership)(null, null).mayEdit, false, 'two nulls are not the same person');
    eq((0, intake_1.intakeOwnership)(null, 'u1').mayEdit, false, 'an unknown viewer may not edit');
    eq((0, intake_1.intakeOwnership)('u1', null).mayEdit, false, 'nor may anyone edit an unknown subject');
    eq((0, intake_1.intakeOwnership)('', '').mayEdit, false, 'and two empty strings are not a person either');
    ok((0, intake_1.intakeOwnership)(null, null).reason !== null, 'and an unknown says so rather than going quiet');
    ...
    for (const pair of [[null, 'u1'], ['u1', null], [null, null]]) {
        const g = (0, intake_1.intakeOwnership)(pair[0], pair[1]);
        ok(/could not tell whose intake this is/i.test(g.reason ?? ''), `an unread id says we do not know whose it is — got: ${g.reason}`);
    }
    ok(!/could not tell whose/i.test(coach.reason), 'and a coach is told whose it is, not that we could not tell');
    // Case and whitespace are not the same id. A uuid comparison that had been
    // loosened would let a near-miss through.
    eq((0, intake_1.intakeOwnership)('U1', 'u1').mayEdit, false, 'ids are compared exactly, not case-insensitively');
    eq((0, intake_1.intakeOwnership)(' u1', 'u1').mayEdit, false, 'and not after trimming');
}
/* ── what the client is asked ───────────────────────────────────────────── */
{
    const fresh = (0, intake_1.askIntakeMessage)('none', (0, intake_1.intakeProgress)(null));
    ...
    const p = (0, intake_1.intakeProgress)(partial);
    const nudge = (0, intake_1.askIntakeMessage)('started', p);
