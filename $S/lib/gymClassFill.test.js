...
const row = (status, attended = false) => ({ class_id: 'c1', status, attended_at: attended ? '2026-09-01T06:05:00.000Z' : null });
/* ── the bug itself ───────────────────────────────────────────────────────── */
...
{
    const t = (0, gymSchedule_1.tallyBookings)([
...
{
    const t = (0, gymSchedule_1.tallyBookings)([row('booked'), row('cancelled'), row('no_show'), row('')]);
...
// is not a rate.
{
    ...
    ok((t.attended.get('c1') ?? 0) <= (t.booked.get('c1') ?? 0), 'attended can never exceed booked — the property that makes show rate a rate');
}
...
{
    const s = (0, gymSchedule_1.summariseAttendance)([cls()]);
...
{
    const s = (0, gymSchedule_1.summariseAttendance)([
    ...
    ok(s.fillRate !== null && s.fillRate <= 1, 'with waitlisters out of the numerator, a fill rate over 100% now means a genuine over-sell');
}
...
// capacity recorded is not a 0% fill.
{
    ...
    const unsized = (0, gymSchedule_1.summariseAttendance)([cls({ capacity: 0, booked: 3, attended: 3, waitlisted: 0 })]);
