...
const withInv = (over) => base({ invoice: { ...INV, ...over } });
...
{
    const d = (0, coachInvoice_1.coachInvoiceDoc)(base());
    ...
    let prose = (d.html + '\n' + d.text).toLowerCase();
    for (const stated of [coachInvoice_1.INVOICE_TAX, coachInvoice_1.INVOICE_NOT_A_RECEIPT, ...coachInvoice_1.INVOICE_PROVENANCE]) {
    ...
    ok(d.html.includes((0, coachInvoice_1.escapeHtml)(coachInvoice_1.INVOICE_TAX).slice(0, 60)), 'and it says outright that no tax has been calculated');
    ok(d.text.includes(coachInvoice_1.INVOICE_TAX), 'in the text fallback as well as the HTML');
    ok(d.text.includes(coachInvoice_1.INVOICE_NOT_A_RECEIPT), 'and that it is not a payment receipt');
}
{
    ...
    ok(d.text.includes(coachInvoice_1.INVOICE_TAX), 'a voided invoice with an unreadable issuer still carries the tax statement');
    ok(d.text.includes(coachInvoice_1.INVOICE_NOT_A_RECEIPT), 'and still says it is not a receipt');
}
   ...
   Repple worked out: no tax amount, no net figure, no subtotal. */
{
    ...
    const r = (s) => (0, coachInvoice_1.readTaxRate)(s);
    eq(r('').ok && r('').ok === true ? r('').pct : 'x', null, 'an empty box means the coach stated no rate');
    eq(r('20').pct, 20, 'a whole rate is read as one');
    eq(r('12.5').pct, 12.5, 'and a fractional one keeps its half');
    eq(r('12,5').pct, 12.5, 'a comma decimal is accepted — half the world types it');
    eq(r('20%').pct, 20, 'and a typed per-cent sign is not a refusal');
    // A stated zero is NOT the same as an empty box. A registered business
    // stating a zero rate has said something deliberate.
    eq(r('0').pct, 0, 'a stated zero is a statement, not an absence');
    ...
    eq((0, coachInvoice_1.statesTax)(INV), false, 'an invoice with neither field states nothing about tax');
    eq((0, coachInvoice_1.statesTax)({ ...INV, taxRatePct: 0 }), true, 'a stated zero rate IS a statement');
    eq((0, coachInvoice_1.statesTax)({ ...INV, taxRatePct: null, taxRegistration: 'GB123456789' }), true, 'and so is a number on its own');
    eq((0, coachInvoice_1.statesTax)({ ...INV, taxRegistration: '   ' }), false, 'a blank registration number is not one');
    // The document. Both fields print verbatim and NOTHING is derived from them.
    const d = (0, coachInvoice_1.coachInvoiceDoc)(withInv({ taxRatePct: 20, taxRegistration: 'GB123456789', amountCents: 48000, currency: 'GBP' }));
    ...
    ok(d.text.includes(coachInvoice_1.INVOICE_TAX_STATED), 'and the tax sentence is the one for a document that states something');
    ok(!d.text.includes(coachInvoice_1.INVOICE_TAX), 'never both: the old sentence denies that a registration number is stated, and this document states one');
    ok(coachInvoice_1.INVOICE_TAX_STATED.includes('no tax amount anywhere on this document'), 'and it still says Repple calculated nothing, because Repple calculated nothing');
    ...
    for (const stated of [coachInvoice_1.INVOICE_TAX_STATED, coachInvoice_1.INVOICE_NOT_A_RECEIPT, ...coachInvoice_1.INVOICE_PROVENANCE]) {
        derivedProse = derivedProse.split(stated).join(' ').split((0, coachInvoice_1.escapeHtml)(stated)).join(' ');
    }
    for (const derived of ['96.00', '400.00', '384.00', 'Subtotal', 'subtotal', 'Net', 'VAT']) {
        ok(!derivedProse.includes(derived), `nothing is derived from the stated rate — found "${derived}"`);
    }
    ...
    ok((0, coachInvoice_1.coachInvoiceDoc)(withInv({ taxRatePct: 0 })).text.includes('0%'), 'a stated zero rate is printed');
    ok((0, coachInvoice_1.coachInvoiceDoc)(withInv({ taxRatePct: 0 })).text.includes(coachInvoice_1.INVOICE_TAX_STATED), 'and carries the stated sentence');
    ok(!(0, coachInvoice_1.coachInvoiceDoc)(withInv({})).text.includes('Rate stated by the issuer'), 'an invoice stating no rate prints no rate line rather than "none" or "0%"');
    ok(!(0, coachInvoice_1.coachInvoiceDoc)(withInv({})).text.includes('registration number:'), 'and no registration line either');
    ...
    const nasty = (0, coachInvoice_1.coachInvoiceDoc)(withInv({ taxRegistration: 'GB<script>1</script>' }));
    ...
    const good = { billTo: 'Dana', description: '8 sessions', amountText: '480', currency: 'GBP', kind: 'requested', issuedOn: '2026-08-31' };
    eq((0, coachInvoice_1.invoiceBlockers)({ ...good, taxRateText: '20' }).length, 0, 'a stated rate does not block an invoice');
    eq((0, coachInvoice_1.invoiceBlockers)({ ...good, taxRateText: '' }).length, 0, 'nor does an empty one — both fields are optional');
    eq((0, coachInvoice_1.invoiceBlockers)({ ...good, taxRateText: '120' }).length, 1, 'a rate above a hundred is refused');
    eq((0, coachInvoice_1.invoiceBlockers)({ ...good, taxRegistration: 'X'.repeat(61) }).length, 1, 'and a registration number longer than the column');
    eq((0, coachInvoice_1.invoiceBlockers)({ ...good, taxRegistration: 'X'.repeat(60) }).length, 0, 'but not one that fits');
}
...
}
{
    ...
    const d = (0, coachInvoice_1.coachInvoiceDoc)(withInv({ currency: null }));
    eq((0, coachInvoice_1.money)({ ...INV, currency: null }), null, 'no currency means no printable amount');
    ok(!d.complete, 'and the document does not call itself complete');
    ok(d.caveats.some((c) => c.includes('not print')) || d.caveats.some((c) => c.toLowerCase().includes('currency')), 'the missing currency is named in the caveats');
    ok(d.text.includes('Do not read the missing figure as nothing being charged'), 'and the page says so where the total would be — a blank total reads as nothing owed');
}
{
    // A currency with no minor unit is not divided by a hundred. Getting this
    // backwards bills ¥500 for a ¥50,000 session.
    eq((0, coachInvoice_1.money)({ ...INV, amountCents: 50000, currency: 'JPY' }), 'JPY 50,000', 'a zero-decimal currency is not divided by 100');
    eq((0, coachInvoice_1.money)({ ...INV, amountCents: 50000, currency: 'GBP' }), 'GBP 500.00', 'and a two-decimal one is');
}
/* ── 3. the typed amount, in minor units ──────────────────────────────────
   Where a hundredfold error would enter. */
{
    eq((0, coachInvoice_1.draftMinorUnits)('45.50', 'GBP'), 4550, 'a decimal amount becomes minor units');
    eq((0, coachInvoice_1.draftMinorUnits)('45,50', 'GBP'), 4550, 'a comma decimal is accepted — half the world types it that way');
    eq((0, coachInvoice_1.draftMinorUnits)('45', 'GBP'), 4500, 'a whole amount too');
    eq((0, coachInvoice_1.draftMinorUnits)('50000', 'JPY'), 50000, 'a zero-decimal currency is not multiplied by 100');
    eq((0, coachInvoice_1.draftMinorUnits)('500.50', 'JPY'), null, 'and a decimal in one is refused rather than silently rounded');
    eq((0, coachInvoice_1.draftMinorUnits)('0', 'GBP'), null, 'zero is not an amount to invoice');
    eq((0, coachInvoice_1.draftMinorUnits)('-40', 'GBP'), null, 'nor is a negative one');
    eq((0, coachInvoice_1.draftMinorUnits)('45.505', 'GBP'), null, 'three decimal places is not money');
    eq((0, coachInvoice_1.draftMinorUnits)('1,234', 'GBP'), null, 'a thousands separator is refused, not guessed at — "1,234" is two different amounts in two countries');
    eq((0, coachInvoice_1.draftMinorUnits)('45.50', null), null, 'and nothing at all is computed without a currency');
    eq((0, coachInvoice_1.draftMinorUnits)('', 'GBP'), null, 'an empty box is not zero');
    ...
    eq((0, coachInvoice_1.draftMinorUnits)('12.500', 'KWD'), 12500, 'a three-decimal currency is multiplied by a THOUSAND, not a hundred');
    eq((0, coachInvoice_1.draftMinorUnits)('12.500', 'kwd'), 12500, 'and the code is read case-insensitively');
    eq((0, coachInvoice_1.draftMinorUnits)('12.5', 'KWD'), 12500, 'a short fraction is padded to the currency’s own places, not read as hundredths');
    eq((0, coachInvoice_1.draftMinorUnits)('12', 'BHD'), 12000, 'and a whole dinar is a thousand fils');
    eq((0, coachInvoice_1.draftMinorUnits)('12.345', 'KWD'), null, 'Stripe charges thousandths in tens, so a fils in the last place is refused rather than rounded');
    eq((0, coachInvoice_1.draftMinorUnits)('12.5000', 'KWD'), null, 'four places is not an amount in a three-place currency');
    eq((0, coachInvoice_1.draftMinorUnits)('40.00', 'OMR'), 40000, 'and the same holds for every one of the five');
    eq((0, coachInvoice_1.draftMinorUnits)('0.000', 'KWD'), null, 'a nought is still not an amount to invoice, whatever the currency');
}
...
{
    const kw = { billTo: 'Nasser', description: 'Ten pack', amountText: '12.345', currency: 'KWD', kind: 'requested', issuedOn: '2026-08-31' };
    const b = (0, coachInvoice_1.invoiceBlockers)(kw);
    ...
    eq((0, coachInvoice_1.invoiceBlockers)({ ...kw, amountText: '12.500' }).length, 0, 'a real dinar amount is not blocked');
    const jp = (0, coachInvoice_1.invoiceBlockers)({ ...kw, currency: 'JPY', amountText: '500.50' });
...
{
    const d = { billTo: '', description: '', amountText: '', currency: null, kind: 'requested', issuedOn: '2026-08-31' };
    const b = (0, coachInvoice_1.invoiceBlockers)(d);
...
{
    const good = { billTo: 'Dana', description: '8 sessions', amountText: '480', currency: 'GBP', kind: 'requested', issuedOn: '2026-08-31' };
    eq((0, coachInvoice_1.invoiceBlockers)(good).length, 0, 'a complete draft has nothing stopping it');
    eq((0, coachInvoice_1.invoiceBlockers)({ ...good, currency: 'GB' }).length, 1, 'a currency that is not three letters cannot be printed');
    eq((0, coachInvoice_1.invoiceBlockers)({ ...good, issuedOn: '31/08/2026' }).length, 1, 'a date that will not read is a blocker, not a silent today');
    ok((0, coachInvoice_1.invoiceBlockers)({ ...good, currency: 'JPY', amountText: '500.50' })[0].includes('whole number'), 'a decimal in a zero-decimal currency is explained in terms of that currency');
}
...
{
    eq((0, coachInvoice_1.invoiceNumber)(7), '0007', 'zero-padded so a book reads as a sequence');
    eq((0, coachInvoice_1.invoiceNumber)(1), '0001', 'from one');
    eq((0, coachInvoice_1.invoiceNumber)(12345), '12345', 'and nothing is truncated past four digits');
    eq((0, coachInvoice_1.invoiceNumber)(0), '—', 'a number below one is not a number this issued');
    eq((0, coachInvoice_1.invoiceNumber)(Number.NaN), '—', 'nor is one that will not read');
    const d = (0, coachInvoice_1.coachInvoiceDoc)(base());
...
{
    const d = (0, coachInvoice_1.coachInvoiceDoc)(withInv({ voidedAt: '2026-08-31T09:00:00Z', voidReason: 'issued twice by mistake' }));
    ok(d.html.includes((0, coachInvoice_1.escapeHtml)(coachInvoice_1.INVOICE_VOID_NOTICE).slice(0, 40)), 'a voided invoice says so on its face');
    ...
    ok((0, coachInvoice_1.invoiceShareBlurb)(d, { ...INV, voidedAt: '2026-08-31T09:00:00Z' }).includes('VOIDED'), 'the share sheet warns before a voided document leaves the phone');
}
...
{
    eq((0, coachInvoice_1.voidBlocker)(INV), null, 'an ordinary open invoice can be voided');
    eq((0, coachInvoice_1.voidBlocker)({ ...INV, kind: 'received' }), null, 'and so can one stating the money was received — the document is still a document, and voiding is how a wrong one is withdrawn');
    const settled = (0, coachInvoice_1.voidBlocker)({ ...INV, settledOn: '2026-09-02', settledAt: '2026-09-02T10:00:00Z' });
    ok(settled !== null, 'an invoice recorded as settled cannot also be voided');
    ok(!/constraint|relation|coach_invoices/.test(settled ?? ''), 'and the refusal is a sentence about the invoice, never a database one');
    ok((settled ?? '').includes((0, coachInvoice_1.invoiceDayLabel)('2026-09-02')), 'it names the day the coach said the money arrived, so they can see which claim is in the way');
    ok(/new document/i.test(settled ?? ''), 'and names what is left to do, because the number still stands');
    const already = (0, coachInvoice_1.voidBlocker)({ ...INV, voidedAt: '2026-08-31T09:00:00Z', voidReason: 'issued twice' });
    ...
    ok((0, coachInvoice_1.voidBlocker)({ ...INV, voidedAt: '2026-08-31T09:00:00Z', settledOn: '2026-09-02' }) === already, 'a row carrying both states reads as voided, which is the sentence with nothing left to do');
}
...
{
    eq((0, coachInvoice_1.issuerCaveat)('ready'), null, 'a landed read needs no caveat');
    ok((0, coachInvoice_1.issuerCaveat)('error') !== null, 'a refused one does');
    ok((0, coachInvoice_1.issuerCaveat)('loading') !== null, 'and so does one still in flight — it is not treated as read');
    ok((0, coachInvoice_1.issuerCaveat)('partial') !== null, 'and a truncated one');
    eq((0, coachInvoice_1.invoiceCaveats)(base()).length, 0, 'a whole read produces no caveats');
}
{
    const d = (0, coachInvoice_1.coachInvoiceDoc)(base({ issuer: { status: 'error', name: null, brand: 'Ironhaus Strength' } }));
    ...
    ok(d.text.includes('This is not a statement that the record has no name in it'), 'and refuses to let a failed read be read as an absent name');
...
}
{
    ...
    const d = (0, coachInvoice_1.coachInvoiceDoc)(base({ issuer: { status: 'ready', name: null, brand: 'Ironhaus Strength' } }));
   ...
   the amount off the page with it — on a document about money. */
{
    ...
    eq((0, coachInvoice_1.escapeHtml)(`a&b<c>d"e'f`), 'a&amp;b&lt;c&gt;d&quot;e&#39;f', 'all five replacements');
    eq((0, coachInvoice_1.escapeHtml)(null), '', 'and null is an empty string, never the word null');
}
/* ── 9. the kind is the coach's claim, never a verified state ─────────────*/
{
    eq((0, coachInvoice_1.kindLabel)('received'), 'Stated received', 'the list hedges it');
    eq((0, coachInvoice_1.kindLabel)('requested'), 'Stated requested', 'both ways');
    const d = (0, coachInvoice_1.coachInvoiceDoc)(withInv({ kind: 'received' }));
   ...
   assertion that would fail in one of them. */
{
    ...
    eq((0, coachInvoice_1.invoiceDayLabel)('2026-01-01'), (0, format_1.fmtPointDay)(2026, 0, 1), 'the first of January stays the first of January');
    eq((0, coachInvoice_1.invoiceDayLabel)('2026-12-31'), (0, format_1.fmtPointDay)(2026, 11, 31), 'and the last of December');
    ...
    eq((0, coachInvoice_1.invoiceDayLabel)('2026-08-31T22:30:00Z'), (0, coachInvoice_1.invoiceDayLabel)('2026-08-31'), 'a timestamp is cut to its calendar day, not shifted by one');
    eq((0, coachInvoice_1.invoiceDayLabel)('2026-08-31T22:30:00Z'), (0, format_1.fmtPointDay)(2026, 7, 31), 'and that day is the one the string names');
    eq((0, coachInvoice_1.invoiceDayLabel)(''), '—', 'an empty date is a dash');
    eq((0, coachInvoice_1.invoiceDayLabel)('31/08/2026'), '—', 'and an unparseable one is a dash rather than a guess');
    eq((0, coachInvoice_1.invoiceDayLabel)('2026-13-01'), '—', 'a month that does not exist is a dash');
}
/* ── 11. the book: currencies are never added, voids are never hidden ─────*/
{
    ...
    const b = (0, coachInvoice_1.invoiceBook)(rows, 'ready');
...
{
    const rows = [{ ...INV }];
    eq((0, coachInvoice_1.invoiceBook)(rows, 'partial').totals, null, 'a truncated read states no total — a sum over a prefix is not a smaller total, it is a wrong one');
    ok(((0, coachInvoice_1.invoiceBook)(rows, 'partial').reason ?? '').includes('not all of them'), 'and says why');
    eq((0, coachInvoice_1.invoiceBook)(rows, 'error').totals, null, 'nor does a failed one');
    ok(((0, coachInvoice_1.invoiceBook)(rows, 'error').reason ?? '').includes('does not mean you have issued none'), 'and an empty list under error is not "you have issued none"');
    eq((0, coachInvoice_1.invoiceBook)([], 'ready').totals?.pots.length, 0, 'a coach who has issued nothing gets an honest empty book');
    eq((0, coachInvoice_1.invoiceBook)([], 'ready').reason, null, 'which is a real answer, not a withheld one');
}
...
/* ── 13. the white-label brand, and the platform name that is not it ──────*/
{
    ...
    const noBrand = (0, coachInvoice_1.coachInvoiceDoc)(base({ issuer: { status: 'ready', name: 'Sam Whitfield', brand: null } }));
   ...
   later by flipping `kind` to 'received' instead. */
{
    ...
    const d = (0, coachInvoice_1.coachInvoiceDoc)(paid);
    ok(d.text.includes('The issuer states this amount is being requested.'), 'the claim the document was issued with survives a settlement, word for word');
    ...
    ok(d.text.includes(`the issuer states this was paid on ${(0, format_1.fmtPointDay)(2026, 7, 20)}`), 'and the settlement is printed beside it, dated');
    ok(d.text.includes('How the issuer says it arrived: Bank transfer'), 'with the coach’s own words about how, where they gave any');
    ok(d.text.includes(coachInvoice_1.INVOICE_SETTLEMENT_IS_YOUR_WORD), 'and the hedge that says nothing checked it, exactly as `kind` carries one');
    ...
    const unsettled = (0, coachInvoice_1.coachInvoiceDoc)(withInv({ kind: 'requested' }));
    ok(!unsettled.text.includes(coachInvoice_1.INVOICE_SETTLEMENT_IS_YOUR_WORD), 'and appears on no document that has not been settled');
    ...
    const orphan = (0, coachInvoice_1.coachInvoiceDoc)(withInv({ kind: 'requested', settleNote: 'Bank transfer' }));
    ...
    const nasty = (0, coachInvoice_1.coachInvoiceDoc)(withInv({
    ...
    const planned = (0, coachInvoice_1.coachInvoiceDoc)(withInv({ kind: 'requested', dueOn: null, chaseFrom: '2026-09-15' }));
