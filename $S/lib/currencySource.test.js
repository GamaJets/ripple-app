...
const currencySource_1 = require("./currencySource");
const errors = [];
const ok = (cond, msg) => { if (!cond)
    errors.push(msg); };
const eq = (a, b, msg) => ok(Object.is(a, b), `${msg} — got ${JSON.stringify(a)}, wanted ${JSON.stringify(b)}`);
const gym = (over = {}) => ({ hasGym: false, currency: null, failed: false, ...over });
const own = (over = {}) => ({ hasRow: true, currency: null, failed: false, unavailable: false, ...over });
/* ── the gym is the authority wherever there is one ───────────────────────── */
{
    const r = (0, currencySource_1.resolveMyCurrency)(gym({ hasGym: true, currency: 'AED' }), own({ currency: 'GBP' }));
...
// fallback.
eq((0, currencySource_1.resolveMyCurrency)(gym({ hasGym: true, currency: 'GBP' }), own({ currency: 'JPY' })).currency, 'GBP', 'the coach’s own column never outranks their gym');
...
{
    const r = (0, currencySource_1.resolveMyCurrency)(gym({ hasGym: true, currency: null }), own({ currency: 'JPY' }));
...
{
    const r = (0, currencySource_1.resolveMyCurrency)(gym(), own({ currency: 'jpy' }));
...
eq((0, currencySource_1.resolveMyCurrency)(gym(), own({ currency: '  gbp  ' })).currency, 'GBP', 'trimmed and upper-cased on the way out');
eq((0, currencySource_1.resolveMyCurrency)(gym(), own({ currency: '   ' })).gap, 'own-unset', 'whitespace is not a currency');
{
    const r = (0, currencySource_1.resolveMyCurrency)(gym(), own());
...
{
    const r = (0, currencySource_1.resolveMyCurrency)(gym({ failed: true }), own({ currency: 'JPY' }));
...
eq((0, currencySource_1.resolveMyCurrency)(gym({ failed: true, hasGym: true, currency: 'AED' }), own()).gap, 'unreadable', 'a failed read is failed even when something came back with it');
{
    const r = (0, currencySource_1.resolveMyCurrency)(gym(), own({ failed: true }));
...
{
    const r = (0, currencySource_1.resolveMyCurrency)(gym(), own({ unavailable: true }));
...
{
    const r = (0, currencySource_1.resolveMyCurrency)(gym(), own({ hasRow: false }));
...
for (const g of [gym(), gym({ failed: true }), gym({ hasGym: true, currency: 'AED' })]) {
    eq((0, currencySource_1.resolveMyCurrency)(g, own({ currency: 'GBP' }), true).gap, 'reading', 'nothing is known while a read is in flight, whatever half-answers are lying around');
}
eq((0, currencySource_1.resolveMyCurrency)(gym({ hasGym: true, currency: 'AED' }), own(), true).currency, null, 'and no code is printed from a read that has not finished');
...
for (const s of STATES) {
    ...
    eq((0, currencySource_1.resolveMyCurrency)(s.g, s.o).canSetOwn, want, `canSetOwn for "${s.label}"`);
}
eq((0, currencySource_1.resolveMyCurrency)(gym(), own(), true).canSetOwn, false, 'and never while the read is in flight');
/* ── six causes, six different sentences ──────────────────────────────────── */
const GAPS = ['reading', 'unreadable', 'unavailable', 'nowhere', 'gym-unset', 'own-unset'];
const CONSEQUENCE = 'there is no unit to price these sessions in';
const lines = GAPS.map((g) => [g, (0, currencySource_1.myCurrencyLine)(g, CONSEQUENCE)]);
...
ok(/yours to choose/.test((0, currencySource_1.myCurrencyLine)('own-unset', CONSEQUENCE)), 'the independent coach is told the choice is theirs, which is the whole fix');
ok(/waiting to be applied/.test((0, currencySource_1.myCurrencyLine)('unavailable', CONSEQUENCE)), 'an unapplied migration reads as a deploy step, not as the coach’s fault');
ok(/try again/.test((0, currencySource_1.myCurrencyLine)('unreadable', CONSEQUENCE)), 'and only the failed read tells them to try again, because only that one is fixed by trying');
// The consequence is a fragment. A caller that passes a full stop must not
// produce two.
ok(!/\.\./.test((0, currencySource_1.myCurrencyLine)('own-unset', 'nothing can be priced.')), 'a trailing stop on the fragment is absorbed');
/* ── where a figure came from ─────────────────────────────────────────────── */
eq((0, currencySource_1.currencyFromNote)(null, 'AED'), null, 'no source is nothing to say');
eq((0, currencySource_1.currencyFromNote)('gym', null), null, 'and no code is nothing to say either');
ok(((0, currencySource_1.currencyFromNote)('gym', 'aed') || '').includes('AED'), 'the note carries the code, upper-cased');
ok(/gym/.test((0, currencySource_1.currencyFromNote)('gym', 'AED') || ''), 'a gym figure says so');
ok(/you set/.test((0, currencySource_1.currencyFromNote)('own', 'JPY') || ''), 'and the coach’s own says that instead');
ok((0, currencySource_1.currencyFromNote)('gym', 'AED') !== (0, currencySource_1.currencyFromNote)('own', 'AED'), 'the two sources never read identically — they are acted on differently');
if (errors.length) {
