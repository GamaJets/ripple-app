...
exports.ratePots = ratePots;
exports.runCurrency = runCurrency;
exports.potNames = potNames;
exports.settleCurrencyBlocker = settleCurrencyBlocker;
exports.runLabel = runLabel;
exports.totalNote = totalNote;
const code = (v) => {
 ...
 */
function ratePots(sessions) {
    const pots = new Map();
    for (const s of sessions) {
        if (s.rateCents == null || !Number.isFinite(s.rateCents))
            continue;
...
function runCurrency(sessions) {
    const pots = ratePots(sessions);
    if (pots.length === 0)
        return { kind: 'none' };
    if (pots.length > 1)
        return { kind: 'mixed', pots };
    const only = pots[0];
 ...
 */
function settleCurrencyBlocker(sessions, gymCurrency) {
