...
exports.expiryFor = exports.PASS_COVERS_LABEL = exports.PASS_COVERS = void 0;
exports.remainingUses = remainingUses;
exports.isExpired = isExpired;
exports.isRedeemable = isRedeemable;
exports.passBlocker = passBlocker;
exports.spendable = spendable;
exports.passRevenueCents = passRevenueCents;
exports.summarisePasses = summarisePasses;
exports.guestsByHost = guestsByHost;
exports.passStatus = passStatus;
exports.fetchPassTypes = fetchPassTypes;
exports.passTypeBlocker = passTypeBlocker;
exports.createPassType = createPassType;
exports.setPassTypeActive = setPassTypeActive;
exports.fetchPasses = fetchPasses;
exports.issuePass = issuePass;
exports.redeemPass = redeemPass;
exports.fetchRedemptions = fetchRedemptions;
exports.redemptionUndoBlocker = redemptionUndoBlocker;
exports.undoRedemption = undoRedemption;
 ...
 */
function isExpired(p, today) {
    if (!p.expiresOn)
        return false; // no expiry set is a decision, not a gap
...
Object.defineProperty(exports, "expiryFor", { enumerable: true, get: function () { return termDates_1.expiryFor; } });
/**
 ...
 */
function passRevenueCents(passes) {
    let cents = 0;
    let priced = 0;
...
function summarisePasses(passes, today) {
    let live = 0;
    let expired = 0;
    let usedUp = 0;
 ...
 */
function guestsByHost(passes) {
    const byHost = new Map();
    for (const p of passes) {
        if (p.kind !== 'guest' || !p.hostMemberId)
            continue;
 ...
 * half a price list.
 */
async function fetchPassTypes(sb, tenantId) {
    ...
    if (error)
        throw error;
    return (0, rowCap_1.assertWhole)(data, "this gym's pass price book").map(rowToPassType);
}
function rowToPassType(r) {
    return {
        id: r.id,
        name: r.name,
        kind: r.kind,
 ...
 */
function passTypeBlocker(t) {
    if (!(t.name ?? '').trim())
        return 'Give the pass a name — it is what the desk picks from.';
...
async function createPassType(sb, tenantId, t) {
 ...
 * selling it.
 */
async function setPassTypeActive(sb, passTypeId, active) {
    const r = await sb
        .from('gym_pass_types')
    ...
    if (r.error)
        throw r.error;
    (0, wroteRows_1.assertWrote)(active ? 'Putting that pass back on sale' : 'Retiring that pass', r);
}
/* ── issued passes ─────────────────────────────────────────────────────────── */
/**
 ...
 * round.
 */
async function fetchPasses(sb, tenantId) {
    const rows = await (0, rowCap_1.readAll)((from, to) => sb
        .from('gym_passes')
        .select('id, pass_type_id, holder_id, holder_name, host_member_id, issued_on, expires_on, ' +
        ...
        .range(from, to), "this gym's passes");
...
async function issuePass(sb, tenantId, p) {
    ...
    const { error } = await sb.from('gym_passes').insert({
        ...
        issued_on: issuedOn,
        expires_on: (0, termDates_1.expiryFor)(issuedOn, p.passType.validDays),
 ...
 */
async function redeemPass(sb, pass, opts) {
    ...
    const today = opts.today ?? (0, memberRecord_1.todayIso)(new Date());
    ...
    if (vErr) {
        throw new Error(`The visit was taken off the pass, but the door log did not record it: ${vErr.message ?? 'the write was refused'}. `
            + 'Do not take the pass again — that would spend a second visit. Add the arrival by hand from Check someone in.');
    }
}
async function fetchRedemptions(sb, passId) {
    const { data, error } = await sb
    ...
    return (0, rowCap_1.assertWhole)(data, 'the visits taken off this pass').map((r) => ({
        id: r.id,
        passId: r.pass_id,
 ...
 */
function redemptionUndoBlocker(r) {
    if (r.sessionId) {
 ...
 */
async function undoRedemption(sb, r) {
    ...
    if (r0.error)
        throw r0.error;
    (0, wroteRows_1.assertWrote)('That redemption', r0);
}
