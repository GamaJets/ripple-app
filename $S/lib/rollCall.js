...
exports.emergencyLine = emergencyLine;
exports.buildRollCall = buildRollCall;
exports.escapeHtml = escapeHtml;
exports.rollCallHtml = rollCallHtml;
/** "Ada Kowalski — 050 111 2222", or whichever half exists, or null. */
function emergencyLine(r) {
    if (!r)
        return null;
 ...
 */
function buildRollCall(input) {
    const printedAtIso = input.nowIso ?? new Date().toISOString();
    const people = [...input.inside]
    ...
    if (input.records === null) {
        caveats.push('The gym’s own notes did not load, so no next-of-kin number and no medical note could be printed. Those columns are UNKNOWN on this sheet, not empty — do not read a blank as “nobody to ring”.');
    }
    if (unnamed > 0) {
        caveats.push(`${unnamed} of these ${unnamed === 1 ? 'is a visit' : 'are visits'} the desk could not put a name to. ${unnamed === 1 ? 'It is' : 'They are'} still ${unnamed === 1 ? 'a person' : 'people'} in the building and ${unnamed === 1 ? 'is' : 'are'} counted here.`);
    }
    if (input.openFromEarlierDays > 0) {
        caveats.push(`${input.openFromEarlierDays} check-${input.openFromEarlierDays === 1 ? 'in is' : 'ins are'} still open from an earlier day and ${input.openFromEarlierDays === 1 ? 'is' : 'are'} deliberately NOT on this list — ${input.openFromEarlierDays === 1 ? 'it is a row' : 'they are rows'} nobody closed rather than ${input.openFromEarlierDays === 1 ? 'a person' : 'people'} standing in the gym.`);
    }
    caveats.push('It carries phone numbers and, where the desk recorded one, a medical note. Take it with you and destroy it afterwards rather than leaving it on the desk.');
