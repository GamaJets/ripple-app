...
exports.SERVER_WRITTEN = exports.KNOWN_PUSHES = void 0;
exports.inboxIcon = inboxIcon;
exports.inboxHeading = inboxHeading;
exports.inboxDecision = inboxDecision;
exports.safeRoute = safeRoute;
exports.inboxAge = inboxAge;
exports.unreadBadge = unreadBadge;
exports.inboxControls = inboxControls;
exports.clearReadPrompt = clearReadPrompt;
exports.deletedNote = deletedNote;
exports.clearedNote = clearedNote;
 ...
 */
const SERVER_WROTE_THE_ROW = [
    'New coaching request',
    notifyCopy_1.CLASS_OFF_TITLE,
    notifyCopy_1.CLASS_OFF_TITLE_MANY,
];
...
const startsWithAny = (route, prefixes) => prefixes.some((p) => route === p || route.startsWith(p + '?'));
/** The icon a route implies, or the generic bell. */
function inboxIcon(route) {
    const r = (route ?? '').trim();
    if (!r)
        return 'bell';
    for (const [prefix, icon] of ICON_BY_ROUTE) {
        if (r === prefix || r.startsWith(prefix + '?'))
            return icon;
    }
    return 'bell';
}
 ...
 */
function inboxHeading(title, icon) {
    const t = (title ?? '').trim();
    if (t)
        return t;
    if (icon === 'message')
 ...
 */
function inboxDecision(title, body, route) {
    const t = (title ?? '').trim();
    const b = (body ?? '').trim();
    const r = (route ?? '').trim();
    const icon = inboxIcon(r);
    if (!b)
        return { record: false, icon, why: 'no body — an inbox row with only a heading says nothing' };
 ...
 */
const MIN = 60000, HOUR = 60 * MIN, DAY = 24 * HOUR, WEEK = 7 * DAY, YEAR = 365 * DAY;
/** "Just now", "12m", "3h", "2d", "5w", "1y". */
function inboxAge(iso, now = Date.now()) {
 ...
 */
function unreadBadge(unread, status) {
    ...
    if (n === 0)
        return { kind: 'none' };
    return { kind: 'count', label: (0, format_1.num)(n), a11y: `Notifications. ${(0, format_1.num)(n)} unread.` };
}
function inboxControls(status) {
    if (status === 'error') {
        return {
 ...
 */
function clearReadPrompt(readCount, status) {
    if (!inboxControls(status).clearRead)
        return null;
    ...
    return {
        ...
        title: one ? 'Delete the read notification?' : `Delete ${(0, format_1.num)(n)} read notifications?`,
 ...
 */
function clearedNote(ok, changed) {
    if (!ok)
        return 'Nothing was deleted — the server did not answer. Your inbox is unchanged.';
    const n = Number.isFinite(changed) && changed > 0 ? Math.floor(changed) : 0;
    if (n === 0)
        return 'Nothing was deleted. There was nothing marked read to remove.';
    return n === 1 ? 'One read notification deleted.' : `${(0, format_1.num)(n)} read notifications deleted.`;
}
