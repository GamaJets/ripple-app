...
exports.todayIndex = todayIndex;
const E = (key, name, group, sets, reps, alternatives) => ({ key, name, group, sets, reps, alternatives });
/** Focus areas inferred from body composition + goal (stands in for the scan/photo analysis). */
function focusAreas(bodyFatPct, goal) {
    const f = [];
    if (goal === 'fatloss')
        f.push('Full-body strength', 'Conditioning');
    else if (goal === 'muscle')
        f.push('Hypertrophy', 'Progressive overload');
    else
        f.push('Muscle tone', 'Definition');
    if (bodyFatPct != null && bodyFatPct >= 25)
        f.push('Glutes & core');
    else
        f.push('Shoulders & back');
    return f;
}
...
function buildProgram(goal, bodyFatPct) {
...
function todayIndex(days, weekday) {
    ...
    days.forEach((d, i) => { const diff = Math.abs(map[d.day] - weekday); if (diff < bestDiff) {
        bestDiff = diff;
        best = i;
    } });
    return best;
