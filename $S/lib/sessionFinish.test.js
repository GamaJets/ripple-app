...
// Midway.
const sessionFinish_1 = require("./sessionFinish");
const errors = [];
const ok = (cond, msg) => { if (!cond)
    errors.push(msg); };
const eq = (a, b, msg) => ok(Object.is(a, b), `${msg} — got ${JSON.stringify(a)}, wanted ${JSON.stringify(b)}`);
const NOW = new Date(2026, 8, 2, 12, 0, 0, 0).getTime(); // 2 Sep 2026, local noon
const at = (h) => new Date(2026, 8, 2, h, 0, 0, 0).toISOString();
const ANSWERS = ['stored', 'refused', 'unsent'];
const OUTCOMES = ['stored', 'refused', 'unsent', 'not-asked', 'not-attempted'];
const report = (over) => (0, sessionFinish_1.finishReport)({
 ...
 * words, and the words must state that nothing came off the client. */
{
    ...
    ok(/no session credit has been taken off/i.test(all), `a refused log must say no credit was spent — got ${JSON.stringify(r.lines)}`);
    ...
    ok(!/marked as delivered\.?$/i.test(all.replace(/NOT been marked as delivered/i, '')), `a refused log must not claim the session was marked — got ${JSON.stringify(r.lines)}`);
}
 ...
 * never doubles as the sentence about the session. */
{
    for (const entries of ANSWERS) {
        for (const outcome of OUTCOMES) {
            ...
            if (outcome === 'not-attempted' && entries !== 'refused')
                continue;
 ...
 * the marking queue and must not be told it is finished. */
{
    ...
    ok(/Mark Sessions/.test(r.lines.join(' ')), `a refused outcome must point at the queue — got ${JSON.stringify(r.lines)}`);
...
/* ── 3 · a queued write is never a saved one ───────────────────────────────── */
{
    ...
    const all = r.lines.join(' ');
    ok(/still waiting on an outcome/i.test(all), `a queued outcome must say the session still looks unmarked — got ${JSON.stringify(r.lines)}`);
    ok(!/\bis now marked as delivered\b/i.test(all), `a queued outcome must never claim the mark landed — got ${JSON.stringify(r.lines)}`);
 ...
 * and at nothing they can do about it. */
{
    ...
    const all = r.lines.join(' ');
    ok(/not waiting to send/i.test(all), `A REFUSED KEEP MUST NOT PROMISE A SEND — got ${JSON.stringify(r.lines)}`);
    ok(!/server/i.test(r.lines[0]), `and must not blame a server that never saw it — got ${JSON.stringify(r.lines[0])}`);
    ok(!/4 exercises/.test(r.lines[0]), 'the count is not evidence here either');
    eq(r.mayLeave, false, 'THE SETS EXIST ONLY ON THIS SCREEN — the coach may not walk away from them');
    ok(/no session credit/i.test(all), 'and no credit is spent for an hour whose record was not kept');
}
 ...
 * receipt for it. */
{
    ...
    ok(/1 exercise\b/.test(one.lines[0]) && !/1 exercises/.test(one.lines[0]), `one exercise is singular — got ${JSON.stringify(one.lines[0])}`);
    const queued = report({ entries: 'unsent', entryCount: 4, outcome: 'not-asked' });
    ok(!/4 exercises/.test(queued.lines[0]), `a queued log must not count what nobody answered — got ${JSON.stringify(queued.lines[0])}`);
}
 ...
 * be silence. */
{
    ...
    ok((0, sessionFinish_1.canFinish)(base, NOW), 'a booked, finished, unmarked session with a client can be finished');
    eq((0, sessionFinish_1.finishBlockedNote)(base, NOW), null, 'nothing to explain when it is offered');
    const running = { ...base, startsAt: at(11), durationMin: 90 };
    ok(!(0, sessionFinish_1.canFinish)(running, NOW), 'an hour still running is not offered');
    ok(/has not finished yet/i.test((0, sessionFinish_1.finishBlockedNote)(running, NOW) ?? ''), `a running session says why — got ${JSON.stringify((0, sessionFinish_1.finishBlockedNote)(running, NOW))}`);
    const marked = { ...base, outcome: 'completed' };
    ok(!(0, sessionFinish_1.canFinish)(marked, NOW), 'a session with an outcome is not re-finished');
    ok(/already has an outcome/i.test((0, sessionFinish_1.finishBlockedNote)(marked, NOW) ?? ''), 'a marked session says why');
    ...
    for (const o of ['completed', 'no_show', 'cancelled', 'late_cancelled']) {
        ok(!(0, sessionFinish_1.canFinish)({ ...base, outcome: o }, NOW), `${o} is already decided`);
    }
    const nobody = { ...base, clientId: null };
    ok(!(0, sessionFinish_1.canFinish)(nobody, NOW), 'an hour with nobody in it is not a session');
    ok(/Nobody is booked/i.test((0, sessionFinish_1.finishBlockedNote)(nobody, NOW) ?? ''), 'an empty hour says why');
    const open = { ...base, status: 'available' };
    ok(!(0, sessionFinish_1.canFinish)(open, NOW), 'an open slot that went by is not a delivered session');
    ok(((0, sessionFinish_1.finishBlockedNote)(open, NOW) ?? '').length > 20, 'an open slot still gets a sentence');
    const blocked = { ...base, status: 'blocked' };
    ok(!(0, sessionFinish_1.canFinish)(blocked, NOW), 'blocked time is not a session');
    ...
    ok(!(0, sessionFinish_1.canFinish)({ ...base, startsAt: 'not a date' }, NOW), 'an unreadable start is not finishable');
}
...
{
    ok(/pack|pass/i.test(sessionFinish_1.DELIVERED_MEANS), 'the warning names what a credit comes off');
    ok(/comes off/i.test(sessionFinish_1.DELIVERED_MEANS), 'the warning says something is spent');
    ok(/Mark Sessions/.test(sessionFinish_1.DELIVERED_MEANS), 'the warning says where the session goes');
    ok(/nothing comes off/i.test(sessionFinish_1.NOT_DELIVERED_MEANS), 'turning it off says nothing is spent');
    ok(/still saved/i.test(sessionFinish_1.NOT_DELIVERED_MEANS), 'turning it off must not read as losing the log');
    ok(sessionFinish_1.DELIVERED_MEANS !== sessionFinish_1.NOT_DELIVERED_MEANS, 'two states, two sentences');
    eq((0, sessionFinish_1.finishCta)(true, true), 'Save and Finish Session', 'the button says what it does');
    eq((0, sessionFinish_1.finishCta)(false, true), 'Save Without Finishing', 'and says what it does not do');
    eq((0, sessionFinish_1.finishCta)(true, false), 'Save Session', 'with no session in hand it is the plain save');
    eq((0, sessionFinish_1.finishCta)(false, false), 'Save Session', 'and the switch cannot change that');
}
/* ── 7 · an unread count is not a count of zero ─────────────────────────── */
{
    ...
    const seen = new Set();
    for (const s of ALL) {
        const line = (0, sessionFinish_1.loggedAgainstLine)(s, s === 'ready' ? 0 : 2);
        ok(line.trim().length > 20, `${s} must be a sentence`);
        ok(!seen.has(line), `${s} repeated another status's sentence`);
        seen.add(line);
    }
    ok(/could not be read/i.test((0, sessionFinish_1.loggedAgainstLine)('error', null)), 'a failed read says so');
    ok(!/Nothing is filed/i.test((0, sessionFinish_1.loggedAgainstLine)('error', null)), 'a failed read must never state the session is empty');
    ok(/not a session with nothing in it/i.test((0, sessionFinish_1.loggedAgainstLine)('error', null)), 'and says so in as many words');
    ok(/Nothing is filed/i.test((0, sessionFinish_1.loggedAgainstLine)('ready', 0)), 'only a whole read may state the session is empty');
    ok(/^2 exercises logged/.test((0, sessionFinish_1.loggedAgainstLine)('ready', 2)), 'a whole read counts');
    ok(/^1 exercise logged/.test((0, sessionFinish_1.loggedAgainstLine)('ready', 1)), 'and counts one singularly');
    ok(/not necessarily all/i.test((0, sessionFinish_1.loggedAgainstLine)('partial', 5)), 'a truncated read says it is a floor and not a total');
    ok(!/^0 /.test((0, sessionFinish_1.loggedAgainstLine)('loading', null)), 'loading never renders a number');
}
...
{
    eq((0, sessionFinish_1.loggedExercisesLine)([]), null, 'nothing to name gives no phrase');
    eq((0, sessionFinish_1.loggedExercisesLine)(['  ', '']), null, 'blank names are not names');
    eq((0, sessionFinish_1.loggedExercisesLine)(['Back Squat']), 'Back Squat.', 'one movement');
    eq((0, sessionFinish_1.loggedExercisesLine)(['Back Squat', 'Bench Press']), 'Back Squat and Bench Press.', 'two');
    eq((0, sessionFinish_1.loggedExercisesLine)(['A', 'B', 'C']), 'A, B and C.', 'three, the cap');
    eq((0, sessionFinish_1.loggedExercisesLine)(['A', 'B', 'C', 'D']), 'A, B, C and 1 more.', 'four says how many are missing');
    eq((0, sessionFinish_1.loggedExercisesLine)(['A', 'B', 'C', 'D', 'E']), 'A, B, C and 2 more.', 'and counts them');
    ok(!/…|\.\.\./.test((0, sessionFinish_1.loggedExercisesLine)(['A', 'B', 'C', 'D']) ?? ''), 'the tail is a count, never an ellipsis');
}
...
function fakeSb(rows, error = null) {
    const asked = { table: '', in: null, limit: null, chunks: [] };
    const from = (table) => {
        asked.table = table;
        let mine = [];
        const chain = {
            select: () => chain,
            in: (_c, ids) => { mine = ids; asked.in = ids; asked.chunks.push(ids); return chain; },
            order: () => chain,
            limit: (n) => { asked.limit = n; return chain; },
            then: (res) => res(typeof rows === 'function' ? rows(mine, asked.chunks.length - 1) : { data: rows, error }),
        };
        return chain;
    };
    return { sb: { from }, asked };
}
const log = (session, exercise = 'Back Squat') => ({ session_id: session, exercise, performed_at: '2026-09-01T09:00:00.000Z' });
async function readAssertions() {
    /* ── counting ─────────────────────────────────────────────────────────── */
    {
        const { sb } = fakeSb([log('s1'), log('s1', 'Bench Press'), log('s1', 'Deadlift'), log('s2')]);
        const out = await (0, sessionFinish_1.fetchSessionLogCounts)(sb, ['s1', 's2']);
    ...
    }
    {
        ...
        const out = await (0, sessionFinish_1.fetchSessionLogCounts)(sb, ['s1']);
    ...
    // told the figure is not established.
    {
        ...
        const out = await (0, sessionFinish_1.fetchSessionLogCounts)(sb, ['s1'], CAP);
    ...
    }
    {
        ...
        ok(/not necessarily all of them/.test((0, sessionFinish_1.loggedAgainstLine)(out.status, out.bySession.get('s1') ?? null)), 'and the sentence a coach reads says the figure is a floor rather than a total');
    }
    /* ── a read that did not happen ───────────────────────────────────────── */
    {
        ...
        ok(/could not be read/.test((0, sessionFinish_1.loggedAgainstLine)(out.status, null)), 'so the screen says the read failed rather than that the session is empty');
    }
    {
        const { sb, asked } = fakeSb([log('s1')]);
        const out = await (0, sessionFinish_1.fetchSessionLogCounts)(sb, []);
     ...
     * as far as the app could tell. */
    {
        ...
        await (0, sessionFinish_1.fetchSessionLogCounts)(sb, ids);
        ...
        ok(asked.chunks.every((c) => c.length <= 150), `no chunk may exceed the 150 ids a ~5.9KB request line allows — got ${asked.chunks.map((c) => c.length).join(', ')}`);
    ...
    }
    {
        ...
        const out = await (0, sessionFinish_1.fetchSessionLogCounts)(sb, ids);
    ...
    }
    {
        ...
        const ids = Array.from({ length: 300 }, (_, i) => `s${i}`);
        const { sb, asked } = fakeSb((chunk, nth) => (nth === 0 ? { data: chunk.map((id) => log(id)), error: null } : { data: null, error: { message: 'refused' } }));
        const out = await (0, sessionFinish_1.fetchSessionLogCounts)(sb, ids);
    ...
    }
    {
        ...
        // for no extra rows.
        const { sb, asked } = fakeSb([log('s1')]);
        await (0, sessionFinish_1.fetchSessionLogCounts)(sb, ['s1', 's1', '', 's2']);
