...
exports.gymTodayWindow = gymTodayWindow;
 ...
 */
function gymTodayWindow(zone, now = Date.now()) {
    const at = now instanceof Date ? now : new Date(now);
    const day = (0, gymZone_1.gymDay)(at, zone);
    if (day) {
        const bounds = (0, gymZone_1.gymDayBounds)(day, zone);
        if (bounds) {
            return {
                day,
