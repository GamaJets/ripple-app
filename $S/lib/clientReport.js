...
exports.TRAINING_DAYS_SHOWN = exports.countable = exports.REPORT_NO_PHOTOS = exports.REPORT_LIMITS = exports.REPORT_PROVENANCE = void 0;
exports.sectionState = sectionState;
exports.sectionCaveat = sectionCaveat;
exports.reportCaveats = reportCaveats;
exports.escapeHtml = escapeHtml;
exports.toProgressRows = toProgressRows;
exports.clientReportDoc = clientReportDoc;
exports.reportShareBlurb = reportShareBlurb;
 ...
 */
exports.REPORT_LIMITS = 'This document contains no assessment, no interpretation and no advice. It is a record of measurements and logged activity only. Nothing in it has been reviewed by a clinician, and no figure in it should be read as a diagnosis or as a substitute for professional judgement.';
...
exports.REPORT_NO_PHOTOS = 'No photographs are included. Progress photographs are stored privately to this account and are never attached to a document; they can only be shared one at a time, deliberately, from the app. Documents uploaded about an injury are likewise not included — only the injury the person recorded from them.';
/**
...
const countable = (status) => sectionState(status) === 'whole';
 ...
 * kind.
 */
function sectionCaveat(what, status) {
...
function toProgressRows(scans) {
    return oldestFirst(scans, (s) => s.takenAt).map((s) => ({
        date: String(s.takenAt).slice(0, 10),
        weightKg: Number.isFinite(s.weightKg) ? s.weightKg : null,
        bodyFatPct: Number.isFinite(s.bodyFatPct) ? s.bodyFatPct : null,
        muscleKg: Number.isFinite(s.muscleKg) ? s.muscleKg : null,
    }));
}
/**
 ...
 */
exports.TRAINING_DAYS_SHOWN = 30;
/* ── the document ─────────────────────────────────────────────────────────── */
const STYLE = `
...
const unreadableBlock = (what) => `<p class="none"><b>Not read.</b> ${escapeHtml(what)} could not be read from the server when this document was made, so nothing is listed here. This is not a statement that there is none on record.</p>`;
const emptyBlock = (what) => `<p class="none">${escapeHtml(what)}</p>`;
/**
 ...
 */
function clientReportDoc(input) {
    ...
    const T = [];
    /* ── heading ───────────────────────────────────────────────────────────── */
    H.push(`<div class="h"><h1>Health &amp; training summary</h1><p>${escapeHtml(who)} · prepared ${escapeHtml((0, progressExport_1.dayLabel)(input.generatedOn))} · ${escapeHtml(brand)}</p></div>`);
    ...
    for (const line of exports.REPORT_PROVENANCE) {
        H.push(`<p class="lede">${escapeHtml(line)}</p>`);
        T.push(line);
    }
    H.push(`<p class="lede">${escapeHtml(exports.REPORT_LIMITS)}</p>`);
    T.push(exports.REPORT_LIMITS);
    H.push(`<p class="lede">${escapeHtml(exports.REPORT_NO_PHOTOS)}</p>`);
    T.push(exports.REPORT_NO_PHOTOS);
    ...
    // two readings that disagree rather than one said twice.
    {
        ...
        if (st === 'unreadable') {
            ...
            if (conv) {
                H.push(`<p class="lede">${escapeHtml(conv)}</p>`);
                T.push(conv);
            }
            const body = scans.map((s, i) => {
                const r = rows[i];
                return `<tr><td>${escapeHtml((0, progressExport_1.dayLabel)(r.date))}</td>`
                    + `<td class="r">${escapeHtml((0, progressExport_1.figure)((0, units_1.weightIn)(r.weightKg, wu)))}</td>`
                    + `<td class="r">${escapeHtml((0, progressExport_1.figure)(r.bodyFatPct))}</td>`
                    + `<td class="r">${escapeHtml((0, progressExport_1.figure)((0, units_1.weightIn)(r.muscleKg, wu)))}</td>`
    ...
    /* ── tape measurements ─────────────────────────────────────────────────── */
    {
        ...
        if (st === 'unreadable') {
            ...
            const body = entries.map((e) => {
                const cells = cols.map((c) => `<td class="r">${escapeHtml((0, progressExport_1.figure)((0, units_1.lengthIn)(e.values[c.key] ?? null, lu)))}</td>`).join('');
                return `<tr><td>${escapeHtml((0, progressExport_1.dayLabel)(String(e.at).slice(0, 10)))}</td>${cells}</tr>`;
            }).join('');
    ...
    // beside it so the reader can see which of the two they are looking at.
    {
        ...
        if (st === 'unreadable' || b.state === 'unreadable') {
            ...
            const tonnage = (0, units_1.volumeIn)(b.volumeKg, wu);
            const totals = [
            ...
            const shown = b.days.slice(0, exports.TRAINING_DAYS_SHOWN);
            if (!shown.length) {
                ...
                T.push(slice);
                const body = shown.map((d) => `<tr><td>${escapeHtml((0, progressExport_1.dayLabel)(d.day))}</td>`
                    + `<td class="r">${escapeHtml(num(d.exercises))}</td>`
                    + `<td class="r">${escapeHtml(num(d.sets))}</td>`
                    + `<td class="r">${escapeHtml(num(d.bodyweightSets))}</td>`
                    + `<td class="r">${escapeHtml(num((0, units_1.volumeIn)(d.volumeKg, wu)))}</td></tr>`).join('');
    ...
    // the unreadable branch above it and the caveat repeated at the top.
    {
        ...
        if (st === 'unreadable') {
            ...
            const body = items.map((i) => `<tr><td>${escapeHtml(i.label)}</td>`
                ...
                + `<td>${escapeHtml((0, progressExport_1.dayLabel)(String(i.at).slice(0, 10)))}</td>`
