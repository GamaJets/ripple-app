...
const trialGate_1 = require("./trialGate");
...
/* ── 1. the arithmetic ──────────────────────────────────────────────────── */
eq((0, trialGate_1.trialFrom)('2026-09-01T10:00:00.000Z', START).daysLeft, trialGate_1.TRIAL_DAYS, 'a trial that has just started has all of it left');
eq((0, trialGate_1.trialFrom)('2026-09-01T10:00:00.000Z', START + DAY).daysLeft, trialGate_1.TRIAL_DAYS - 1, 'a day in, a day fewer');
eq((0, trialGate_1.trialFrom)('2026-09-01T10:00:00.000Z', START + 13 * DAY).daysLeft, 1, 'the last day has one left');
eq((0, trialGate_1.trialFrom)('2026-09-01T10:00:00.000Z', START + 14 * DAY).daysLeft, 0, 'and then none');
eq((0, trialGate_1.trialFrom)('2026-09-01T10:00:00.000Z', START + 14 * DAY).expired, true, 'which is expired');
eq((0, trialGate_1.trialFrom)('2026-09-01T10:00:00.000Z', START + 13 * DAY).expired, false, 'and the day before is not');
...
eq((0, trialGate_1.trialFrom)('2026-09-01T23:00:00.000Z', Date.parse('2026-09-02T09:00:00.000Z')).daysLeft, trialGate_1.TRIAL_DAYS, 'ten hours after an 11pm signup is still day one');
// Never negative, however long ago it was.
eq((0, trialGate_1.trialFrom)('2020-01-01T00:00:00.000Z', START).daysLeft, 0, 'a trial from six years ago is nought days left, never a negative');
...
eq((0, trialGate_1.trialFrom)('2026-09-10T10:00:00.000Z', START).daysLeft, trialGate_1.TRIAL_DAYS, 'a future start is the whole trial and no more');
eq((0, trialGate_1.trialFrom)(null, START), null, 'no start date is no answer');
eq((0, trialGate_1.trialFrom)('', START), null, 'and neither is an empty one');
eq((0, trialGate_1.trialFrom)('whenever', START), null, 'nor one that will not parse');
eq((0, trialGate_1.trialFrom)('2026-09-01T10:00:00.000Z', NaN), null, 'and a clock that will not read produces no figure either');
...
// two of the six runs.
const startedState = (0, trialGate_1.trialFrom)('2026-09-01T10:00:00.000Z', START);
const startInstant = new Date(Date.parse('2026-09-01T10:00:00.000Z'));
const pad2 = (n) => String(n).padStart(2, '0');
eq(startedState.startedOn, `${startInstant.getFullYear()}-${pad2(startInstant.getMonth() + 1)}-${pad2(startInstant.getDate())}`, 'the start day is the coach’s own calendar day, not UTC’s');
ok(/^\d{4}-\d{2}-\d{2}$/.test(startedState.startedOn), 'and it is still a plain day, because the sentence prints it verbatim');
...
eq(startedState.daysLeft, trialGate_1.TRIAL_DAYS, 'and the countdown beside it is zone-independent, as it has to be');
...
for (const bad of ['error', 'partial', 'loading']) {
    const r = (0, trialGate_1.readTrial)('2020-01-01T00:00:00.000Z', bad, START);
...
{
    const r = (0, trialGate_1.readTrial)('2020-01-01T00:00:00.000Z', 'loading', START);
...
{
    const r = (0, trialGate_1.readTrial)(null, 'ready', START);
...
{
    const r = (0, trialGate_1.readTrial)('2026-09-01T10:00:00.000Z', 'ready', START + 3 * DAY);
...
{
    const r = (0, trialGate_1.readTrial)('2026-01-01T10:00:00.000Z', 'ready', START);
...
const ended = (0, trialGate_1.readTrial)('2026-01-01T10:00:00.000Z', 'ready', START);
...
{
    const line = (0, trialGate_1.trialDisagreement)(ended, 11);
...
eq((0, trialGate_1.trialDisagreement)(ended, 0), null, 'agreement says nothing');
eq((0, trialGate_1.trialDisagreement)(ended, null), null, 'and nothing is said when the phone has no figure');
...
{
    const none = (0, trialGate_1.readTrial)(null, 'ready', START);
    eq(none.source, 'none', 'the fixture is an account that answered with no start date');
    const line = (0, trialGate_1.trialDisagreement)(none, 12);
    ...
    ok((0, trialGate_1.trialDisagreement)(none, 0) != null, 'and it is said even when the phone has run out too');
}
...
for (const bad of ['error', 'partial', 'loading']) {
    eq((0, trialGate_1.trialDisagreement)((0, trialGate_1.readTrial)(null, bad, START), 11), null, `nothing is claimed against an account that did not answer under '${bad}'`);
}
...
// same number twice.
{
    const mid = (0, trialGate_1.readTrial)('2026-09-01T10:00:00.000Z', 'ready', START + 3 * DAY);
    const left = mid.state.daysLeft;
    eq(left, trialGate_1.TRIAL_DAYS - 3, 'the fixture is mid-trial rather than run out');
    eq((0, trialGate_1.trialDisagreement)(mid, left), null, 'a phone that agrees with an account still in its trial says nothing');
    ok((0, trialGate_1.trialDisagreement)(mid, left + 4) != null, 'and one that is four days out does');
    ...
    eq((0, trialGate_1.trialDisagreement)(mid, left + 0.5), null, 'half a day apart is not a disagreement worth a sentence');
    ok((0, trialGate_1.trialDisagreement)(mid, left + 1) != null, 'a whole day apart is');
    // Said in words, and the words have to agree with themselves.
    const one = (0, trialGate_1.trialDisagreement)(mid, 1);
    ok(/has 1 day recorded/.test(String(one)), 'one day is a day, not "1 days"');
    const many = (0, trialGate_1.trialDisagreement)(mid, 3);
    ok(/has 3 days recorded/.test(String(many)), 'and three are days');
    ok(String(many).includes(String(left)), 'and the account’s own figure is the one quoted back, not the phone’s repeated twice');
}
...
{
    const mid = (0, trialGate_1.readTrial)('2026-09-01T10:00:00.000Z', 'ready', START + 3 * DAY);
    const card = (0, trialGate_1.trialCard)(mid, false);
    ok(!!card, 'an account that answered with a start date gets a card');
    ok(card.title.includes(String(mid.state.daysLeft)), 'and the figure on it is the account’s, which is the only figure anybody may print');
...
for (const bad of ['error', 'partial', 'loading']) {
    eq((0, trialGate_1.trialCard)((0, trialGate_1.readTrial)('2026-09-01T10:00:00.000Z', bad, START), true), null, `no card is drawn from an account that did not answer under '${bad}'`);
}
eq((0, trialGate_1.trialCard)((0, trialGate_1.readTrial)(null, 'ready', START), true), null, 'and none is drawn for an account that holds no start date');
// An expired trial IS printed, because that one is a fact the account stated.
{
    const card = (0, trialGate_1.trialCard)(ended, false);
...
for (const open of [true, false]) {
    for (const r of [(0, trialGate_1.readTrial)('2026-09-01T10:00:00.000Z', 'ready', START + 3 * DAY), ended]) {
        const card = (0, trialGate_1.trialCard)(r, open);
        ok(!/lock|unlock|keep coaching your|stop|lose|disabled/i.test(card.note), `the card claims no consequence (billingOpen=${open}, expired=${card.expired})`);
    }
}
...
{
    const mid = (0, trialGate_1.readTrial)('2026-09-01T10:00:00.000Z', 'ready', START + 3 * DAY);
    ok(/not open yet/i.test((0, trialGate_1.trialCard)(mid, false).note), 'with billing off, the card says subscriptions are not open');
    ok(/Subscribe/i.test((0, trialGate_1.trialCard)(mid, true).note), 'and with billing on it offers the plans');
}
...
ok(/Nothing is switched off/i.test(trialGate_1.TRIAL_NOT_YET_ENFORCED), 'the screen says nothing is gated on this yet');
ok(/survives a reinstall/i.test(trialGate_1.TRIAL_NOT_YET_ENFORCED), 'and says what it IS for');
