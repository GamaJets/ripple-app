...
exports.seedDecision = seedDecision;
exports.stillListed = stillListed;
exports.pruneSelection = pruneSelection;
exports.assignCtaLabel = assignCtaLabel;
...
const NOTHING_TO_SAY = (action) => ({ action, note: null, replaceLabel: null });
/**
...
function seedDecision(o) {
    ...
    if (o.hasDraft) {
        ...
        if (o.seededFor === o.clientId)
            return NOTHING_TO_SAY('hold');
        if (o.programStatus === 'ready') {
            return {
                action: 'hold',
                note: `What is in the builder is your own work, so picking ${o.firstName} has not replaced it. `
        ...
        return {
            action: 'hold',
            note: `What is in the builder is your own work, and it has not been replaced. `
 ...
 * selection.
 */
function stillListed(status, available, id) {
    if (!id)
        return false;
    if (status !== 'ready')
        return true;
 ...
 * `stillListed`.
 */
function pruneSelection(status, available, selected) {
    if (status !== 'ready')
        return [...selected];
 ...
 * groups in it.
 */
function assignCtaLabel(o) {
    if (o.busy)
        return 'Assigning…';
    if (o.exercises === 0)
        return 'Add an Exercise First';
    if (o.picked === 0)
        return 'Pick Who Gets This';
    if (o.planLabel)
        return o.planLabel;
    const size = ` · ${(0, format_1.num)(o.exercises)} exercise${s(o.exercises)}`;
    if (o.picked === 1 && o.soleName)
        return `Assign to ${o.soleName}${size}`;
    return `Assign to ${(0, format_1.num)(o.picked)} Clients${size}`;
}
