...
exports.INSTAGRAM_NOT_AVAILABLE = exports.WHY_NO_PHOTOGRAPHS = exports.PHOTO_KEEPS_THE_SHARE_SHEET = exports.MAX_RATIO = exports.MIN_RATIO = exports.MAX_CARD_BYTES = exports.CARD_OBJECT_TTL_MIN = exports.CARD_BUCKET = void 0;
exports.checkPublishable = checkPublishable;
exports.aspectRatio = aspectRatio;
exports.ratioAccepted = ratioAccepted;
exports.ratioLabel = ratioLabel;
exports.jpegFilename = jpegFilename;
exports.isJpegBytes = isJpegBytes;
exports.tooLarge = tooLarge;
exports.cardObjectKey = cardObjectKey;
exports.keyIsUnguessable = keyIsUnguessable;
exports.cardPublicUrl = cardPublicUrl;
exports.cardObjectExpired = cardObjectExpired;
exports.cardObjectRemoved = cardObjectRemoved;
exports.cardObjectAbsent = cardObjectAbsent;
exports.postOutcome = postOutcome;
exports.outcomeNote = outcomeNote;
exports.connectionState = connectionState;
exports.connectionNote = connectionNote;
exports.reviewRefusalNote = reviewRefusalNote;
/* ── what the server needs from a human ───────────────────────────────────── */
/** The bucket the card is written to. Public — the only one in this product. */
exports.CARD_BUCKET = 'share-cards';
/**
 ...
 */
exports.CARD_OBJECT_TTL_MIN = 15;
/** Meta's own limit for a feed image. 8 MiB. */
exports.MAX_CARD_BYTES = 8 * 1024 * 1024;
/**
 ...
 */
exports.MIN_RATIO = 0.8;
exports.MAX_RATIO = 1.91;
/**
 ...
 */
function checkPublishable(card) {
    if (!card) {
        return {
    ...
    if (card.photo) {
        return {
            ...
            why: exports.PHOTO_KEEPS_THE_SHARE_SHEET,
        };
    }
 ...
 */
exports.PHOTO_KEEPS_THE_SHARE_SHEET = 'This card has your client’s photo on it, so it goes through your phone’s share sheet rather than through Instagram directly. '
...
exports.WHY_NO_PHOTOGRAPHS = 'Instagram fetches the image from a web address rather than accepting it from the app, so a card posted this way is briefly public. Cards with a client’s photo on them are never posted this way — they go through your share sheet instead.';
/* ── the shape ────────────────────────────────────────────────────────────── */
 ...
 */
function ratioAccepted(w, h) {
    const r = aspectRatio(w, h);
    if (!r)
        return false;
    return r >= exports.MIN_RATIO - 0.001 && r <= exports.MAX_RATIO + 0.001;
}
/** "9:16", "4:5" — the shape as a coach would say it, for the refusal. */
function ratioLabel(w, h) {
    const r = aspectRatio(w, h);
    if (!r)
        return 'not a shape Repple could measure';
    ...
    if (Math.abs(r - 0.5625) < 0.002)
        return '9:16';
    if (Math.abs(r - 0.8) < 0.002)
        return '4:5';
    if (Math.abs(r - 1) < 0.002)
        return 'square';
...
function tooLarge(byteLength) {
    return !Number.isFinite(byteLength) || byteLength <= 0 || byteLength > exports.MAX_CARD_BYTES;
}
/* ── the public object ────────────────────────────────────────────────────── */
/**
...
function cardPublicUrl(supabaseUrl, key) {
    const base = String(supabaseUrl ?? '').replace(/\/+$/, '');
    if (!base || !keyIsUnguessable(key))
        return '';
    return `${base}/storage/v1/object/public/${exports.CARD_BUCKET}/${key}`;
}
/** Past its ceiling, and therefore a sweep's business. */
function cardObjectExpired(createdAtMs, nowMs) {
    if (!Number.isFinite(createdAtMs) || !Number.isFinite(nowMs))
        return true;
    return nowMs - createdAtMs >= exports.CARD_OBJECT_TTL_MIN * 60000;
}
/**
 ...
 */
function cardObjectRemoved(key, data) {
    if (!Array.isArray(data))
        return false;
    const base = key.slice(key.lastIndexOf('/') + 1);
    return data.some((o) => o?.name === key || o?.name === base);
}
 ...
 *  unnecessary. */
function cardObjectAbsent(key, data) {
    if (!Array.isArray(data))
        return false;
    const base = key.slice(key.lastIndexOf('/') + 1);
    return !data.some((o) => o?.name === base || o?.name === key);
}
function postOutcome(containerId, mediaId) {
    if (String(mediaId ?? '').trim())
        return 'published';
 ...
 * neither.
 */
function outcomeNote(outcome, detail) {
...
function connectionState(configured, status, hasAccount) {
    if (!configured)
        return 'unconfigured';
...
function connectionNote(state) {
    switch (state) {
        ...
        case 'unconfigured':
        default:
            return exports.INSTAGRAM_NOT_AVAILABLE;
    }
}
/**
 ...
 */
exports.INSTAGRAM_NOT_AVAILABLE = 'Posting straight to Instagram is not switched on in this build. It needs a Meta app whose Instagram publishing permission has been through Meta’s App Review, which is theirs to grant rather than ours. '
...
function reviewRefusalNote(detail) {
    const said = String(detail ?? '').trim();
