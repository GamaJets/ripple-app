...
exports.PAGE_CEILING = exports.TruncatedRead = exports.ROW_CAP = void 0;
exports.capLimit = capLimit;
exports.assertWhole = assertWhole;
...
exports.TruncatedRead = TruncatedRead;
/**
 ...
 */
function assertWhole(rows, what, cap = exports.ROW_CAP) {
    const r = rows ?? [];
    if (r.length > cap)
        throw new TruncatedRead(what, cap);
    return r;
}
/** True when this looks like a set that was cut off rather than one that ended. */
function isTruncated(rows, cap = exports.ROW_CAP) {
 ...
 */
function capped(rows, cap = exports.ROW_CAP) {
    const r = rows ?? [];
 ...
 */
async function readAll(page, what, opts = {}) {
    const size = Math.max(1, Math.floor(opts.pageSize ?? exports.ROW_CAP));
    const ceiling = Math.max(size, Math.floor(opts.ceiling ?? exports.PAGE_CEILING));
    const out = [];
    for (let from = 0;; from += size) {
        ...
        if (out.length > ceiling)
            throw new TruncatedRead(what, ceiling);
