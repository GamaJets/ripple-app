...
const gymImports_1 = require("./gymImports");
const errors = [];
...
const lines = (rows) => rows.map((payment, i) => ({ line: i + 2, payment }));
/* ── stable ───────────────────────────────────────────────────────────────── */
eq((0, gymImports_1.paymentKeyContent)(pay()), (0, gymImports_1.paymentKeyContent)(pay()), 'the same line produces the same key — without this a re-run recognises nothing and writes everything again');
...
eq((0, gymImports_1.paymentKeyContent)(pay({ memberName: '  SARA   AHMED ', note: '  renewal  ' })), (0, gymImports_1.paymentKeyContent)(pay()), 'whitespace and case are normalised, so a re-export of the same sheet still matches what was imported');
/* ── different payments must not collide ──────────────────────────────────── */
{
    const base = (0, gymImports_1.paymentKeyContent)(pay());
    ok((0, gymImports_1.paymentKeyContent)(pay({ amountCents: 45001 })) !== base, 'a different amount is a different payment');
    ok((0, gymImports_1.paymentKeyContent)(pay({ takenOn: '2026-08-03' })) !== base, 'a different date is a different payment');
    ok((0, gymImports_1.paymentKeyContent)(pay({ method: 'cash' })) !== base, 'a different method is a different payment');
    ok((0, gymImports_1.paymentKeyContent)(pay({ memberName: 'Sara Ahmad' })) !== base, 'a different person is a different payment');
    ok((0, gymImports_1.paymentKeyContent)(pay({ note: 'Joining fee' })) !== base, 'a different note is a different payment');
}
...
ok((0, gymImports_1.paymentKeyContent)(pay({ memberName: 'ab', note: 'c' }))
    !== (0, gymImports_1.paymentKeyContent)(pay({ memberName: 'a', note: 'bc' })), 'the fields are separated, so shifting a character between two of them changes the key');
...
// from the other end.
{
    ...
    const keyed = (0, gymImports_1.keyPaymentRows)(lines([twin, twin]));
    eq(keyed.length, 2, 'both identical lines are kept');
    ok(keyed[0].key !== keyed[1].key, 'AND THEY GET DIFFERENT KEYS — two genuinely identical payments both land; dropping the second loses money the gym took');
    eq(keyed[0].key, (0, gymImports_1.paymentImportKey)(twin, 0), 'the first is occurrence 0');
    eq(keyed[1].key, (0, gymImports_1.paymentImportKey)(twin, 1), 'the second is occurrence 1');
}
...
// unique index recognises every one of them and the upsert writes nothing.
{
    ...
    const first = (0, gymImports_1.keyPaymentRows)(lines(file)).map((k) => k.key);
    const second = (0, gymImports_1.keyPaymentRows)(lines(file)).map((k) => k.key);
...
// evaporate without anything failing.
{
    ...
    const forward = (0, gymImports_1.keyPaymentRows)(lines([a, b, a]));
    ...
    eq(forward[0].key, (0, gymImports_1.paymentImportKey)(a, 0), 'the first occurrence of a line is #0, by the same rule anything else computing a key would use');
    eq(forward[2].key, (0, gymImports_1.paymentImportKey)(a, 1), 'and its repeat later in the file is #1');
    ok(forward[0].key !== forward[2].key, 'the repeat of the first line is a second occurrence, not the same key');
    eq(forward[1].key, (0, gymImports_1.paymentImportKey)(b, 0), 'an unrelated line between two repeats does not disturb their numbering');
}
...
// the lines that already landed are still keyed the way they were written.
{
    ...
    const whole = (0, gymImports_1.keyPaymentRows)(lines(file)).map((k) => k.key);
    const prefix = (0, gymImports_1.keyPaymentRows)(lines(file.slice(0, 2))).map((k) => k.key);
...
{
    const keyed = (0, gymImports_1.keyPaymentRows)([
...
/* ── the version marker ───────────────────────────────────────────────────── */
ok((0, gymImports_1.paymentKeyContent)(pay()).startsWith('p1|'), 'the key carries a format version, so this rule can be changed later without silently making every stored key unmatchable');
if (errors.length) {
