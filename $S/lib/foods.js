...
exports.COMMON_FOODS = void 0;
exports.searchCommonFoods = searchCommonFoods;
/** One table, grouped only for reading. Values are per the portion named. */
exports.COMMON_FOODS = [
    // ── Meat, fish, eggs ──────────────────────────────────────────────────
 ...
 * unanswerable one.
 */
function searchCommonFoods(query, limit = 12) {
    const terms = (query || '').trim().toLowerCase().split(/[^a-z0-9]+/).filter(Boolean);
    if (!terms.length)
        return [];
    const scored = [];
    for (const f of exports.COMMON_FOODS) {
