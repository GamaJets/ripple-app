 ...
 */
{
    const guessed = { t: at('2026-06-01'), exercise: 'Bench Press', sets: [[8, 0]] };
    eq((0, bodyweightSets_1.isBodyweightSet)(guessed, 0), false, 'a set with a zero load and no flag is NOT bodyweight — it is a set nobody described');
    eq((0, bodyweightSets_1.setLoadKg)(guessed, 0, guessed.sets[0], HISTORY, guessed.t), null, 'and it has no load, rather than being priced at the member’s body');
    const said = { t: at('2026-06-01'), exercise: 'Pull-up', sets: [[8, 0]], bw: [true] };
    eq((0, bodyweightSets_1.isBodyweightSet)(said, 0), true, 'a flagged set is bodyweight');
    const mixed = {
        t: at('2026-06-01'), exercise: 'Dip', sets: [[10, 0], [8, 20], [6, 60]], bw: [true, true, false],
    };
    eq((0, bodyweightSets_1.isBodyweightSet)(mixed, 1), true, 'the flags are per set, aligned to sets, exactly as feel is');
    eq((0, bodyweightSets_1.isBodyweightSet)(mixed, 2), false, 'so one movement can hold both kinds of set in one entry');
    eq((0, bodyweightSets_1.isBodyweightSet)(mixed, 9), false, 'and an index past the end is not bodyweight, it is nothing');
}
/* ── PRICING A SET ────────────────────────────────────────────────────────
 *
 ...
 */
{
    eq((0, bodyweightSets_1.bodyweightAtKg)(HISTORY, at('2026-02-14')), 96, 'January’s weigh-in prices a February set');
    eq((0, bodyweightSets_1.bodyweightAtKg)(HISTORY, at('2026-06-20')), 92, 'and May’s prices a June one — the NEAREST at or before, not the first');
    eq((0, bodyweightSets_1.bodyweightAtKg)(HISTORY, at('2026-09-01')), 88, 'a weigh-in prices a set done the same day');
    eq((0, bodyweightSets_1.bodyweightAtKg)(HISTORY, at('2025-12-31')), null, 'a set done before the first weigh-in has no weight — the future is not evidence about the past');
    eq((0, bodyweightSets_1.bodyweightAtKg)([], at('2026-06-01')), null, 'and an empty history prices nothing');
    eq((0, bodyweightSets_1.bodyweightAtKg)([{ t: at('2026-01-01'), v: 0 }], at('2026-06-01')), null, 'a zero reading is not a body, it is a missing one');
    eq((0, bodyweightSets_1.bodyweightAtKg)([{ t: 'not a date', v: 90 }], at('2026-06-01')), null, 'and an unparseable reading is discarded rather than thrown');
    const e = { t: at('2026-06-01'), exercise: 'Pull-up', sets: [[8, 0], [6, 20]], bw: [true, true] };
    eq((0, bodyweightSets_1.setLoadKg)(e, 0, e.sets[0], HISTORY, e.t), 92, 'a plain pull-up moved the person');
    eq((0, bodyweightSets_1.setLoadKg)(e, 1, e.sets[1], HISTORY, e.t), 112, 'and a belted one moved the person plus the belt');
    const bar = { t: at('2026-06-01'), exercise: 'Back Squat', sets: [[5, 140]] };
    eq((0, bodyweightSets_1.setLoadKg)(bar, 0, bar.sets[0], HISTORY, bar.t), 140, 'an ordinary set is its own load and the body has nothing to do with it');
}
/* ── NO INVENTED BODY ─────────────────────────────────────────────────────
 *
 ...
 */
{
    const e = { t: at('2026-06-01'), exercise: 'Press-up', sets: [[20, 0]], bw: [true] };
    eq((0, bodyweightSets_1.setLoadKg)(e, 0, e.sets[0], [], e.t), null, 'an unweighed member’s set has NO load');
    const t = (0, bodyweightSets_1.entryTonnage)(e, []);
    eq(t.kg, 0, 'so it adds nothing to the tonnage');
    eq(t.unknownSets, 1, 'and the tonnage says how many sets it could not price');
    ok((0, bodyweightSets_1.tonnageNote)(t) != null, 'and a screen is given a sentence to print under the figure');
    ok(!/70/.test((0, bodyweightSets_1.tonnageNote)(t) ?? ''), 'which names no weight, because none was given');
}
/* ── TONNAGE ──────────────────────────────────────────────────────────────
 *
 ...
 */
{
    ...
    const whole = (0, bodyweightSets_1.tonnage)(log, HISTORY);
    ...
    const blind = (0, bodyweightSets_1.tonnage)(log, []);
    ...
    ok(((0, bodyweightSets_1.tonnageNote)(blind) ?? '').includes('2 bodyweight sets'), 'the sentence counts them, so the member knows how much is not in the figure');
    eq((0, bodyweightSets_1.tonnageNote)(whole), null, 'a whole tonnage carries no caveat');
    eq((0, bodyweightSets_1.entryTonnage)({ t: day, exercise: 'Row', cardio: { mins: 20, dist: 5, unit: 'km' } }, HISTORY).kg, bodyweightSets_1.NO_TONNAGE.kg, 'an entry with no sets has no tonnage and no complaint');
    ...
    const wk = (0, streaks_1.weekStats)(log, Date.parse(day) + 1000, HISTORY);
    ...
    eq((0, streaks_1.weekStats)(log, Date.parse(day) + 1000, []).unpricedSets, 2, 'and reports both sets missing when they are not');
}
/* ── RECORDS ──────────────────────────────────────────────────────────────
 *
 ...
 */
{
    ...
    const blind = (0, streaks_1.personalRecords)(log, []);
    ...
    const known = (0, streaks_1.personalRecords)(log, HISTORY);
    ...
    eq(pu.est1RM, (0, streaks_1.est1RM)(102, 12), 'estimated through the app’s one 1RM formula');
    const reps = (0, bodyweightSets_1.repRecords)(log);
    ...
    eq((0, bodyweightSets_1.repRecords)([{ t: at('2026-06-01'), exercise: 'Back Squat', sets: [[5, 120]] }]).length, 0, 'a barbell lift is not a bodyweight record');
    eq((0, bodyweightSets_1.bodyweightSetLabel)(12, 0, null), '12 reps at bodyweight', 'a plain set reads plainly');
    eq((0, bodyweightSets_1.bodyweightSetLabel)(12, 10, '10 kg'), '12 reps at bodyweight +10 kg', 'and a belted one names the belt');
    ...
    const second = log[1];
    eq((0, streaks_1.isNewPR)(log.slice(0, 2), second, HISTORY), true, 'twelve beats eight');
    eq((0, streaks_1.isNewPR)(log, log[0], HISTORY), false, 'and eight does not beat twelve — the prior best is read with the same arithmetic as the new one');
    eq((0, streaks_1.isNewPR)(log.slice(0, 2), second, []), false, 'with no weigh-in there is no load, so no claim is made either way');
}
/* ── ROUND TRIP ───────────────────────────────────────────────────────────
 *
 ...
 */
{
    ...
    const row = (0, workoutRow_1.entryToRow)('user-1', e);
    ok('bw' in row, 'the row carries a bw column');
    eq(JSON.stringify((0, workoutRow_1.rowToEntry)((0, workoutRow_1.entryToRow)('user-1', e)).bw), JSON.stringify(e.bw), 'and the flags survive the trip out and back');
    const plain = { t: at('2026-06-01'), exercise: 'Walk' };
    eq((0, workoutRow_1.entryToRow)('user-1', plain).bw, null, 'an entry with no flags sends null rather than omitting the column, so an edit can clear it');
    eq((0, workoutRow_1.rowToEntry)({ performed_at: e.t, exercise: 'Dip', sets: [[10, 0]] }).bw, undefined, 'and a row written before the column existed reads back as "nobody was asked"');
    eq((0, bodyweightSets_1.isBodyweightSet)((0, workoutRow_1.rowToEntry)({ performed_at: e.t, exercise: 'Dip', sets: [[10, 0]] }), 0), false, 'which is read as an ordinary set — the same answer the app gave before, so no old row changes meaning');
}
if (errors.length) {
