...
const win = (label, firstDay, lastDay) => ({
...
{
    const at = (0, gymWindow_1.cutAtGym)(win('August 2026', '2026-08-01', '2026-08-31'), 'Asia/Dubai');
    eq(at.basis, 'gym', 'a gym with a zone gets its own clock');
    eq(at.window.fromIso, '2026-07-31T20:00:00.000Z', 'August in Dubai starts at 20:00 UTC on 31 July, not at midnight UTC and not at midnight in London');
    eq(at.window.toIso, '2026-08-31T20:00:00.000Z', 'and ends when the gym’s 1 September starts — exclusive, so a payment at that instant is September’s');
    ok(at.note.includes('Asia/Dubai'), 'the caption names the clock it used');
    ok(!at.note.includes(gymZone_1.NO_ZONE_NOTE), 'and carries no caveat, because there is nothing to caveat');
...
/* ── a quarter containing a clock change ───────────────────────────────────── */
{
    ...
    const at = (0, gymWindow_1.cutAtGym)(win('Q1 2026', '2026-01-01', '2026-03-31'), 'Europe/London');
    ...
    eq(at.window.toIso, '2026-03-31T23:00:00.000Z', 'and closes on BST midnight — 23:00 UTC, which is what makes the last day 23 hours long rather than 24');
}
/* ── a gym that has not said, and one nobody could ask ─────────────────────── */
{
    for (const zone of [null, undefined, '', '   ', 'Mars/Olympus_Mons', 'GMT+4']) {
        ...
        ok(at.note.includes(gymZone_1.NO_ZONE_NOTE), 'and the caption says whose clock it is, in the one wording the rest of the console uses');
        ok(!/in the gym.s own timezone|cut on the gym/.test(at.note), 'and never claims the gym’s — that claim over the device’s clock IS the defect');
    }
}
/* ── the window is copied, never edited in place ───────────────────────────── */
{
    const base = win('August 2026', '2026-08-01', '2026-08-31');
    const at = (0, gymWindow_1.cutAtGym)(base, 'Asia/Dubai');
    eq(base.fromIso, '1999-01-01T00:00:00.000Z', 'the caller’s own window is untouched — a memoised `monthWindow` is shared, and mutating it would move a month under a second reader');
