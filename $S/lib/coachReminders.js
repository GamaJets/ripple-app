...
exports.BACKLOG_FLOOR = exports.BACKLOG_PROMPT_KEY = exports.MAX_ARMED = exports.ARM_AHEAD_DAYS = exports.COACH_LEAD_MINUTES = void 0;
exports.remindAt = remindAt;
exports.toArm = toArm;
exports.staleReminders = staleReminders;
exports.expiredReminders = expiredReminders;
exports.reminderBody = reminderBody;
exports.backlogDue = backlogDue;
exports.backlogBody = backlogBody;
exports.backlogNote = backlogNote;
...
function remindAt(startsAt, now = Date.now()) {
    const t = Date.parse(startsAt);
    if (!Number.isFinite(t))
        return null;
    const at = t - exports.COACH_LEAD_MINUTES * MIN;
 ...
 */
function toArm(sessions, armed, now = Date.now()) {
    const horizon = now + exports.ARM_AHEAD_DAYS * 86400000;
    const out = sessions.filter((s) => {
        ...
        if (!Number.isFinite(t) || t > horizon)
            return false;
        if (remindAt(s.startsAt, now) == null)
            return false;
    ...
    return out.slice(0, exports.MAX_ARMED);
}
/**
 ...
 */
function staleReminders(armed, sessions, windowFrom, windowTo) {
    ...
    for (const [id, a] of Object.entries(armed)) {
        ...
        if (!Number.isFinite(t) || t < windowFrom || t > windowTo)
            continue;
        const s = live.get(id);
 ...
 */
function expiredReminders(armed, now = Date.now()) {
 ...
 */
exports.BACKLOG_PROMPT_KEY = 'repple.coachBacklog.week';
 ...
 */
function backlogDue(n, seenWeek, now = Date.now()) {
    if (n == null || n < exports.BACKLOG_FLOOR)
        return false;
    ...
    return !seenWeek || seenWeek !== (0, nudge_1.weekKey)(now);
}
/**
