...
exports.RECURRING_MIN_MONTHS = exports.COST_LOOKBACK_MONTHS = void 0;
exports.costMonthOf = costMonthOf;
exports.isMonthKey = isMonthKey;
exports.monthsBefore = monthsBefore;
exports.costLineKey = costLineKey;
exports.standingCosts = standingCosts;
exports.missingStandingCosts = missingStandingCosts;
exports.costVerdict = costVerdict;
exports.costNoteForClose = costNoteForClose;
exports.joinCloseBlockers = joinCloseBlockers;
 ...
 */
exports.COST_LOOKBACK_MONTHS = 6;
/**
 ...
 */
exports.RECURRING_MIN_MONTHS = 3;
/* ── months, as text ──────────────────────────────────────────────────────── */
/**
 ...
 */
function monthsBefore(key, count) {
    if (!isMonthKey(key) || count <= 0)
        return [];
 ...
 */
function costLineKey(row) {
    const supplier = (0, consoleSearch_1.normalise)(row.supplier);
    const category = (0, consoleSearch_1.normalise)(row.category);
    if (!supplier)
        return null;
 ...
 */
function standingCosts(past, months, minMonths = exports.RECURRING_MIN_MONTHS) {
    const window = new Set(months.filter(isMonthKey));
    if (!window.size)
        return [];
    const acc = new Map();
    const monthsOnRecord = new Set();
    for (const row of past) {
        const month = costMonthOf(row.paidOn);
        if (!month || !window.has(month))
            continue;
        ...
        at.months.add(month);
        if (row.currency)
            at.currencies.add(row.currency);
    }
    ...
    for (const [key, at] of acc) {
        if (at.months.size < minMonths)
            continue;
        if (!at.months.has(newestOnRecord))
            continue;
        out.push({
            key,
            ...
            usual: at.currencies.size === 1 && at.latestCents != null && at.latestCurrency
    ...
    return out.sort((a, b) => b.monthsSeen - a.monthsSeen || (a.key < b.key ? -1 : a.key > b.key ? 1 : 0));
}
/**
 ...
 */
function missingStandingCosts(month, standing) {
 ...
 * an opinion at all.
 */
function costVerdict(input) {
    const lookback = input.lookback ?? exports.COST_LOOKBACK_MONTHS;
    const minMonths = input.minMonths ?? exports.RECURRING_MIN_MONTHS;
    const none = { missing: [], standing: [], entered: null, unnamed: null };
    if (input.monthState === 'failed' || input.pastState === 'failed') {
        return {
            ...none,
            kind: 'unread',
            text: `This gym’s costs could not be read, so nothing here says whether ${input.monthLabel} ` +
    ...
    if (input.monthState === 'partial' || input.pastState === 'partial') {
        return {
            ...none,
            kind: 'truncated',
            text: `Only the first rows of this gym’s costs were read and there are more, so no line can be ` +
    ...
    const standing = standingCosts(input.pastRows, monthsBefore(input.monthKey, lookback), minMonths);
    ...
    if (!missing.length) {
        return {
            missing: [], standing, entered, unnamed,
            text: `Every one of the ${standing.length} suppliers this gym pays most months is in ` +
    ...
    return {
        missing, standing, entered, unnamed,
        kind: 'gaps',
        text: `${missing.length} of the ${standing.length} suppliers this gym pays most months ` +
 ...
 */
function joinCloseBlockers(...parts) {
