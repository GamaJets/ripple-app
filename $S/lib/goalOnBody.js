...
exports.goalOnBody = goalOnBody;
exports.goalOfKind = goalOfKind;
 ...
 * through untouched.
 */
function goalOnBody(goal, series, opts) {
    if (!goal)
        return null;
    const p = (0, goalTargets_1.progressOf)(goal, series);
    ...
    if (!p)
        return null;
    const conv = (v) => (opts.weight ? (0, units_1.weightIn)(v, opts.wu) : Math.round(v * 10) / 10);
 ...
 */
function goalOfKind(goals, kind) {
    if (!goals?.length)
        return null;
