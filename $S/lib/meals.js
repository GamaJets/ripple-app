...
exports.PLAN_WEEK_DAYS = exports.SNACK_SHARE = exports.ALLERGENS = exports.DEPT_ICO = exports.DEPTS = void 0;
exports.poolGaps = poolGaps;
exports.planGaps = planGaps;
exports.mealAllergens = mealAllergens;
exports.allergenLabel = allergenLabel;
exports.allergenGapNote = allergenGapNote;
exports.catalogSize = catalogSize;
exports.mealAt = mealAt;
exports.slotsFor = slotsFor;
exports.buildPlan = buildPlan;
exports.snackIdeas = snackIdeas;
exports.swapIndex = swapIndex;
exports.searchMeals = searchMeals;
exports.planWeek = planWeek;
exports.groceryFromWeek = groceryFromWeek;
exports.groceryData = groceryData;
...
const forDiet = (arr, diet) => arr.filter((x) => x.d.includes(diet));
exports.ALLERGENS = [
...
function componentAllergens(comp) {
    ...
    if (/almond|walnut|cashew|pecan|macadamia|peanut|hazelnut|pistachio|\bnut|trail mix/.test(text))
        out.push('nuts');
    if (/prawn|shrimp|crab|lobster|scallop|mussel|oyster|shellfish/.test(text))
        out.push('shellfish');
    if (/\begg/.test(text))
        out.push('egg');
    if (/tofu|tempeh|edamame|\bsoy|miso/.test(text))
        out.push('soy');
    return out;
}
/**
 ...
 * wrong.
 */
function poolFilter(pool, avoid) {
    if (!avoid.length)
        return pool;
 ...
 */
function poolGaps(diet, slot, avoid = []) {
    if (!avoid.length)
        return [];
    ...
    for (const a of avoid) {
        const empties = pools.some((pool) => {
            ...
            if (!forThisDiet.length)
                return false; // the diet already empties it; not this allergen's doing
...
function planGaps(diet, slots, avoid = []) {
    const seen = new Set();
    for (const slot of slots)
        for (const a of poolGaps(diet, slot, avoid))
            seen.add(a);
    return [...seen];
}
/**
 ...
 * it became.
 */
function mealAllergens(meal, avoid = []) {
    if (!avoid.length)
        return [];
...
function allergenLabel(a) {
    return (exports.ALLERGENS.find((x) => x.id === a)?.label ?? a).toLowerCase();
}
/**
...
].map((v) => ({ ...v, d: ['meat', 'vegetarian', 'vegan', 'paleo', 'keto'] }));
const FLAVORS = [
...
].map((x) => ({ ...x, ing: [], k: 0, p: 0, c: 0, f: 0, d: ['meat', 'vegetarian', 'vegan', 'paleo', 'keto'] }));
const SNACK_PREP = [
...
function dims(diet, slot, avoid = []) {
    const f = (arr) => poolFilter(forDiet(arr, diet), avoid);
    if (slot === 'Breakfast')
        return [f(BREK_BASE), f(BREK_TOP), f(BREK_BOOST), f(BREK_STYLE)];
    if (slot === 'Snack')
        return [f(SNACK_A), f(SNACK_B), f(SNACK_PREP)];
    return [f(PROTEINS), f(CARBS), f(VEGS), f(FLAVORS)];
}
/** Number of distinct meals available for a diet + slot. */
function catalogSize(diet, slot, avoid = []) {
...
function mealAt(diet, slot, idx, avoid = []) {
    ...
    const EMPTY = { n: '', k: 0, p: 0, c: 0, f: 0, ing: [], d: [] };
    const at = (i) => parts[i] ?? EMPTY;
    let n, ico, steps;
    if (slot === 'Breakfast') {
...
function buildPlan(c) {
    const target = (0, nutrition_1.applyCoachAdjust)((0, nutrition_1.macrosFor)(c), c.coachAdjust);
    const slots = slotsFor(c.mealsPerDay);
    const override = c.mealOverride ?? {};
    const avoid = c.avoid ?? [];
    let plan = slots.map((slot, i) => {
 ...
 * a fourth meal.
 */
function snackIdeas(c, count = 3) {
    const avoid = c.avoid ?? [];
    const size = catalogSize(c.diet, 'Snack', avoid);
    if (!size)
        return [];
    const target = (0, nutrition_1.applyCoachAdjust)((0, nutrition_1.macrosFor)(c), c.coachAdjust);
    const want = target.kcal * exports.SNACK_SHARE;
...
function searchMeals(diet, slot, query, limit = 40, avoid = []) {
    ...
    for (let i = 0; i < scan && rows.length < limit; i++) {
        ...
        if (!q || m.n.toLowerCase().includes(q))
            rows.push(m);
    }
    return rows;
}
...
function planWeek(c, coachDay) {
    ...
    const out = [];
    for (let d = 0; d < exports.PLAN_WEEK_DAYS; d++) {
        const written = coachDay ? coachDay(d) : null;
        if (written) {
            out.push(buildPlan({ ...c, mealOverride: written }).plan);
            continue;
        }
        const ov = {};
...
function groceryFromWeek(week) {
    ...
    Object.entries(agg).forEach(([key, q]) => {
        const [dept, item, unit] = key.split('||');
        let qty;
        if (unit === 'g' || unit === 'ml')
            qty = Math.round(q / 10) * 10;
        else if (['', 'piece', 'slice', 'clove', 'rasher', 'scoop', 'cup', 'pinch'].includes(unit))
            qty = Math.ceil(q);
        else
            qty = Math.round(q * 10) / 10;
        (byDept[dept] = byDept[dept] || []).push({ item, qty, unit });
    });
    Object.values(byDept).forEach((list) => list.sort((a, b) => a.item.localeCompare(b.item)));
    return { byDept, mealCount: meals.size };
}
/** The shopping list for the week this client is shown. */
function groceryData(c, coachDay) {
