...
exports.FLOOR_CAP = exports.floorQueueKey = void 0;
exports.supersedeKey = supersedeKey;
exports.enqueueAct = enqueueAct;
exports.dropSent = dropSent;
exports.readFloorQueue = readFloorQueue;
exports.actLine = actLine;
exports.floorPendingNote = floorPendingNote;
exports.keptOfflineLine = keptOfflineLine;
exports.registerVisibilityLine = registerVisibilityLine;
exports.floorFullLine = floorFullLine;
exports.refusedLine = refusedLine;
exports.flushResultLine = flushResultLine;
 ...
 */
const floorQueueKey = (uid) => `repple.floorQueue:${uid}`;
exports.floorQueueKey = floorQueueKey;
/**
 ...
 */
function enqueueAct(queue, entry) {
    const key = supersedeKey(entry.act);
    if (key !== null) {
        let replaced = false;
        const next = queue.map((q) => {
            if (replaced || supersedeKey(q.act) !== key)
                return q;
            replaced = true;
            return entry;
        });
    ...
    if (queue.length >= exports.FLOOR_CAP)
...
function actLine(a) {
    switch (a.kind) {
        ...
        case 'session-outcome': {
            ...
            // is still carrying.
            if (a.outcome == null)
                return `${whose} — outcome taken back`;
 ...
 */
function floorPendingNote(n) {
    return (0, offlineQueue_1.unsentNote)(n, 'change', 'changes');
}
/**
 ...
 */
function registerVisibilityLine(unsent, queueRead) {
    if (!queueRead) {
 ...
 */
function flushResultLine(r) {
    const parts = [];
    if (r.sent > 0)
        parts.push(`${r.sent} ${r.sent === 1 ? 'change' : 'changes'} went up.`);
    if (r.refused > 0) {
