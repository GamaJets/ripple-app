...
eq((0, gymRecord_1.sharedCurrency)([]), null, 'an empty set states no currency');
eq((0, gymRecord_1.sharedCurrency)([{ currency: 'GBP' }]), 'GBP', 'one row states its own');
eq((0, gymRecord_1.sharedCurrency)([{ currency: 'GBP' }, { currency: 'GBP' }]), 'GBP', 'rows that agree state it');
eq((0, gymRecord_1.sharedCurrency)([{ currency: 'GBP' }, { currency: 'AED' }]), null, 'rows that disagree state nothing — this is the legacy-dirham case');
...
eq((0, gymRecord_1.sharedCurrency)([{ currency: 'GBP' }, { currency: null }]), null, 'a row stating no currency does not agree with one that does');
eq((0, gymRecord_1.sharedCurrency)([{ currency: null }, { currency: null }]), null, 'rows that all state nothing state nothing, not a shared null to print with');
eq((0, gymRecord_1.sharedCurrency)([{}, {}]), null, 'an absent currency field is the same silence as a null one');
...
eq((0, gymRecord_1.sharedCurrency)([{ currency: 'AED' }, { currency: 'GBP' }]), (0, gymRecord_1.sharedCurrency)([{ currency: 'GBP' }, { currency: 'AED' }]), 'the answer does not depend on which row sorted first');
/* ── the two sums, and what they claim to be in ────────────────────────── */
const pay = (cents, currency) => ({
...
const plan = (id, priceCents, currency, interval = 'month') => ({
    id, name: id, priceCents, currency, interval, active: true,
});
const member = (id, planId) => ({
...
});
{
    ...
    const s = (0, gymRecord_1.summarise)([pay(5000, 'GBP'), pay(2500, 'GBP')], [member('a', 'p1'), member('b', 'p1')], [plan('p1', 4000, 'GBP')]);
    ...
    eq((0, gymRecord_1.money)(s.takenCents, s.takenCurrency), 'GBP 75.00', 'and renders');
}
{
    ...
    const s = (0, gymRecord_1.summarise)([pay(5000, 'GBP'), pay(2500, 'AED')], [member('a', 'p1')], [plan('p1', 4000, 'GBP')]);
    ...
    eq((0, gymRecord_1.money)(s.takenCents, s.takenCurrency), null, 'so money() withholds it rather than labelling dirhams-plus-pounds as pounds');
...
}
{
    ...
    const s = (0, gymRecord_1.summarise)([pay(5000, 'GBP')], [member('a', 'p1'), member('b', 'p2')], [plan('p1', 4000, 'GBP'), plan('p2', 3000, 'EUR')]);
...
}
{
    ...
    const s = (0, gymRecord_1.summarise)([], [], []);
...
}
{
    ...
    // plan handed in.
    const s = (0, gymRecord_1.summarise)([], [member('a', 'p1'), member('b', 'p2')], [plan('p1', 4000, 'GBP'), plan('p2', 9900, 'AED', 'once')]);
...
}
{
    ...
    const s = (0, gymRecord_1.summarise)([], [member('a', 'p1'), member('b', 'p2')], [plan('p1', 1200, 'GBP'), plan('p2', 12000, 'AED', 'year')]);
...
}
{
    ...
    const s = (0, gymRecord_1.summarise)([], [member('a', 'p1'), member('b', 'p2')], 
