...
const versionsFor = (seen, current, aud) => (0, releaseNotes_1.unseenReleases)(seen, current, aud, FIXTURE).map((r) => r.version).join(',');
...
eq((0, releaseNotes_1.unseenReleases)('2.3.0', '2.3.0', 'owner', FIXTURE).length, 0, 'and the screen therefore has nothing to render');
...
eq(versionsFor('2.0.0', '2.3.0', 'client'), '2.2.0,2.1.0', 'a client who skipped three releases is shown every release that had something for them, newest first');
...
eq(versionsFor('2.2.0', '2.3.0', 'client'), '', 'a client one release behind, where that release was owner-only, is shown nothing rather than an empty sheet');
/* ── a release with nothing for this app is skipped, not shown empty ───── */
const ownerRun = (0, releaseNotes_1.unseenReleases)('2.0.0', '2.3.0', 'owner', FIXTURE);
...
for (const aud of ALL) {
    for (const r of (0, releaseNotes_1.unseenReleases)('2.0.0', '2.3.0', aud, FIXTURE)) {
...
// in Settings uses.
ok((0, releaseNotes_1.releasesFor)('owner', FIXTURE).every((r) => r.entries.length > 0), 'the full history is filtered the same way');
eq((0, releaseNotes_1.releasesFor)('owner', FIXTURE).length, 2, 'an owner has two releases in the whole fixture history');
...
for (const junk of ['corrupt', '{"v":1}', 'undefined', 'null', 'v2.1.0', '   ', '💥', 'NaN']) {
    ...
    eq((0, releaseNotes_1.isVersion)(junk), false, `${JSON.stringify(junk)} is not accepted as a version`);
}
ok((0, releaseNotes_1.isVersion)('2.3.0') && (0, releaseNotes_1.isVersion)('1.1') && (0, releaseNotes_1.isVersion)('10'), 'ordinary versions are accepted');
eq((0, releaseNotes_1.isVersion)(null), false, 'and nothing at all is not a version');
eq((0, releaseNotes_1.isVersion)(2.3), false, 'nor is a number that merely looks like one');
...
ok((0, releaseNotes_1.compareVersions)('2.10.0', '2.9.0') > 0, 'versions compare as numbers, not as strings');
ok((0, releaseNotes_1.compareVersions)('1.2.0', '1.1.0') > 0, 'a newer minor is newer');
eq((0, releaseNotes_1.compareVersions)('1.1', '1.1.0'), 0, 'a missing segment counts as zero');
eq((0, releaseNotes_1.compareVersions)('1.1.0', '1.1.0'), 0, 'a version equals itself');
/* ── the real list ─────────────────────────────────────────────────────── */
eq(releaseNotes_1.CURRENT_RELEASE, releaseNotes_1.RELEASES[0].version, 'the build is the newest release it carries — not what app.json says');
ok(releaseNotes_1.RELEASES.every((r, i) => i === 0 || (0, releaseNotes_1.compareVersions)(releaseNotes_1.RELEASES[i - 1].version, r.version) > 0), 'the list is newest first, which is the order every reader of it assumes');
eq(new Set(releaseNotes_1.RELEASES.map((r) => r.version)).size, releaseNotes_1.RELEASES.length, 'no version is listed twice');
for (const r of releaseNotes_1.RELEASES) {
    for (const e of r.entries) {
        ...
        ok(!/\bRLS\b|supabase|postgrest|policy|constraint|webhook|trigger|\bRPC\b|column|migration|row.level|\bindex\b|\btable\b|endpoint/i.test(`${e.title} ${e.note ?? ''}`), `"${e.title}" names a mechanism the reader cannot act on`);
        ok(!/exciting|amazing|revolutionary|delighted|!/i.test(`${e.title} ${e.note ?? ''}`), `"${e.title}" is written at a volume the rest of the app does not use`);
    }
    ...
    for (const [aud, line] of Object.entries(r.headlines ?? {})) {
        ok((0, releaseNotes_1.releasesFor)(aud, [r]).length > 0, `${r.version}: ${aud} has a headline but no entries — the sheet would never open to read it`);
...
const release = (v) => {
    const found = releaseNotes_1.RELEASES.find((r) => r.version === v);
    ok(!!found, `release ${v} is still listed — these assertions describe it`);
    return found ?? releaseNotes_1.RELEASES[0];
};
...
for (const aud of ALL) {
    ok((0, releaseNotes_1.releasesFor)(aud, [releaseNotes_1.RELEASES[0]]).length === 1, `${aud} got something out of ${releaseNotes_1.RELEASES[0].version}, the newest release`);
}
const r120 = release('1.2.0');
const titlesFor = (aud) => (0, releaseNotes_1.releasesFor)(aud, [r120]).flatMap((r) => r.entries.map((e) => e.title));
...
const t130 = (aud) => (0, releaseNotes_1.releasesFor)(aud, [r130]).flatMap((r) => r.entries.map((e) => e.title));
...
for (const aud of ALL) {
    const text = (0, releaseNotes_1.storeNotes)(aud, releaseNotes_1.RELEASES[0].version);
...
// INTRODUCED the changelog is the one release nobody is ever shown.
{
    ...
    eq((0, releaseNotes_1.firstRunReleases)('2026-01-15T09:00:00Z', '2.0.0', 'client', REL).length, 1, 'an existing account is shown the release that introduced the sheet');
    // Only the current one — not everything back to the day they joined.
    eq((0, releaseNotes_1.firstRunReleases)('2025-06-01T09:00:00Z', '2.0.0', 'client', REL)[0].version, '2.0.0', 'and only the current release, not the whole history');
    // Somebody who signed up after it shipped learns nothing from being told.
    eq((0, releaseNotes_1.firstRunReleases)('2026-09-02T09:00:00Z', '2.0.0', 'client', REL).length, 0, 'an account created after the release is not shown it');
    eq((0, releaseNotes_1.firstRunReleases)('2026-08-31T00:00:00Z', '2.0.0', 'client', REL).length, 0, 'nor one created the day it shipped');
    // An age we do not know is not evidence somebody is owed a changelog.
    eq((0, releaseNotes_1.firstRunReleases)(null, '2.0.0', 'client', REL).length, 0, 'unknown account age shows nothing');
    eq((0, releaseNotes_1.firstRunReleases)(undefined, '2.0.0', 'client', REL).length, 0, 'undefined account age shows nothing');
    eq((0, releaseNotes_1.firstRunReleases)('not a date', '2.0.0', 'client', REL).length, 0, 'an unparseable date shows nothing');
    // A release with nothing for this reader's app is not an empty sheet.
    eq((0, releaseNotes_1.firstRunReleases)('2026-01-15T09:00:00Z', '2.0.0', 'owner', REL).length, 0, 'a release with no entries for this app is not shown');
    // A current version that is not in the list cannot be shown.
    eq((0, releaseNotes_1.firstRunReleases)('2026-01-15T09:00:00Z', '3.0.0', 'client', REL).length, 0, 'a version the list does not carry shows nothing');
    eq((0, releaseNotes_1.firstRunReleases)('2026-01-15T09:00:00Z', 'nonsense', 'client', REL).length, 0, 'an unparseable current version shows nothing');
}
if (errors.length) {
    console.error(errors.join('\n'));
    process.exit(1);
}
console.log(`releaseNotes: ok (${releaseNotes_1.RELEASES.length} releases, current ${releaseNotes_1.CURRENT_RELEASE})`);
