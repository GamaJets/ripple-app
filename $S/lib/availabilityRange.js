...
exports.MAX_DURATION_MIN = exports.LARGEST_POSSIBLE_WEEK = exports.MAX_WEEK_SLOTS = void 0;
exports.rangeSlotCount = rangeSlotCount;
exports.rangeBlocker = rangeBlocker;
exports.expandRange = expandRange;
exports.remainderNote = remainderNote;
exports.splitAgainstExisting = splitAgainstExisting;
exports.addButtonLabel = addButtonLabel;
exports.rangeSummary = rangeSummary;
 ...
 */
function rangeSlotCount(r) {
    const step = r.durationMin + Math.max(0, r.gapMin ?? 0);
    if (step <= 0 || r.durationMin <= 0)
        return 0;
 ...
 */
function rangeBlocker(r, alreadyHeld = 0) {
    const days = new Set(r.days);
    if (days.size === 0)
        return 'Pick at least one day.';
    for (const d of days) {
        if (!Number.isInteger(d) || d < 0 || d > 6)
            return 'One of those days is not a day of the week.';
    }
    if (!Number.isFinite(r.fromMin) || !Number.isFinite(r.toMin))
        return 'Set a start and an end time.';
    if (r.fromMin < 0 || r.toMin > 24 * 60)
        return 'Times have to fall inside one day.';
    ...
    if (!Number.isFinite(r.durationMin) || r.durationMin <= 0)
        return 'Set how long one session is.';
    if (r.durationMin > exports.MAX_DURATION_MIN) {
    ...
    if ((r.gapMin ?? 0) < 0)
        return 'A gap cannot be negative.';
    const span = r.toMin - r.fromMin;
    if (span < r.durationMin) {
    ...
    if (n === 0)
        return 'That range produces no slots.';
    ...
    if (alreadyHeld + n > exports.MAX_WEEK_SLOTS) {
        return `That would take your weekly hours to ${alreadyHeld + n} slots, and Repple cannot read a week longer than ${exports.MAX_WEEK_SLOTS} in one go. `
 ...
 */
function expandRange(r, alreadyHeld = 0) {
    if (rangeBlocker(r, alreadyHeld))
        return [];
    const step = r.durationMin + Math.max(0, r.gapMin ?? 0);
    const out = [];
 ...
 */
function remainderNote(r) {
    if (rangeBlocker(r))
        return null;
    const step = r.durationMin + Math.max(0, r.gapMin ?? 0);
    let last = r.fromMin;
    for (let at = r.fromMin; at + r.durationMin <= r.toMin; at += step)
        last = at + r.durationMin;
 ...
 */
function splitAgainstExisting(slots, existing) {
    const have = new Set(existing.map((e) => `${e.dow}:${e.hour}:${e.minute}`));
    const fresh = [];
    let duplicates = 0;
    for (const s of slots) {
        if (have.has(`${s.dow}:${s.hour}:${s.minute}`)) {
            duplicates++;
            continue;
        }
        fresh.push(s);
...
function addButtonLabel(fresh, duplicates) {
    if (fresh === 0 && duplicates > 0)
        return 'You already offer all of these';
    if (fresh === 0)
        return 'Nothing to add';
    return `Add ${fresh} slot${fresh === 1 ? '' : 's'}`;
}
/** A plain summary of what is about to happen, for the line under the button. */
function rangeSummary(r, fresh, duplicates) {
    if (rangeBlocker(r))
        return null;
    const dayNames = [...new Set(r.days)].sort((a, b) => a - b).map((d) => weekStart_1.WEEK_DAYS[d]).join(', ');
 ...
 */
function addOutcome(saved, attempted, duplicates) {
