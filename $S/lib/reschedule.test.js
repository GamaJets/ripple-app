...
const charges = { applies: true, noticeHours: 24, fee: 30, currency: 'GBP' };
const free = { applies: false, noticeHours: 24, fee: null, currency: 'GBP' };
/* ── when Move is offered ──────────────────────────────────────────────── */
ok((0, reschedule_1.canOfferMove)(inHours(72), charges, NOW), 'a session three days out may be moved');
ok(!(0, reschedule_1.canOfferMove)(inHours(3), charges, NOW), 'one three hours out may not, under a 24-hour policy');
ok(!(0, reschedule_1.canOfferMove)(inHours(-1), charges, NOW), 'and neither may one already in progress — the window has no lower bound, exactly as insideNoticeWindow has none');
...
ok((0, reschedule_1.canOfferMove)(inHours(1), free, NOW), 'a coach with no policy has no notice window to be inside');
...
ok((0, reschedule_1.canOfferMove)(inHours(72), null, NOW), 'an unread policy still offers the move well in advance');
ok(!(0, reschedule_1.canOfferMove)(inHours(2), null, NOW), 'and still holds it to the default notice period up close');
/* ── the refusals ──────────────────────────────────────────────────────── */
const refusal = (reason, over = {}) => ({ ...reschedule_1.NOT_MOVED, reason, ...over });
const REASONS = [
    'same_slot', 'not_yours', 'taken', 'other_coach', 'already_started', 'inside_notice', 'clash', 'unreachable',
];
for (const r of REASONS) {
    const line = (0, reschedule_1.rescheduleRefusalLine)(refusal(r), '7:00 am');
...
ok(/cannot say whether anything moved/.test((0, reschedule_1.rescheduleRefusalLine)(refusal('unreachable'), null)), 'an unreachable server says it does not know, and says to check');
...
const late = (0, reschedule_1.rescheduleRefusalLine)(refusal('inside_notice', { noticeHours: 48, fee: 30, currency: 'GBP' }), '7:00 am');
...
const noCcy = (0, reschedule_1.rescheduleRefusalLine)(refusal('inside_notice', { noticeHours: 24, fee: 30, currency: null }), '7:00 am');
...
ok(/ask them what that amount is in/.test(noCcy), 'and a bare figure in a sentence carries the clause saying nobody set a currency');
ok(!/ask them what that amount is in/.test(late), 'while a stated currency adds no such clause');
// A policy that applies with no amount behind it quotes nothing at all.
const unpriced = (0, reschedule_1.rescheduleRefusalLine)(refusal('inside_notice', { noticeHours: 24, fee: null, currency: 'GBP' }), '7:00 am');
...
ok(/1 hour of notice/.test((0, reschedule_1.rescheduleRefusalLine)(refusal('inside_notice', { noticeHours: 1 }), null)), 'one hour is singular');
/* ── the move that worked ──────────────────────────────────────────────── */
const moved = {
    moved: true, reason: null, noticeHours: 24, fee: null, currency: null, promoted: false, waiting: 0,
};
const lines = (0, reschedule_1.rescheduleLines)(moved, 'Tue 7:00 am', 'Thu 6:00 pm');
...
ok(/back on your coach/.test((0, reschedule_1.rescheduleLines)(moved, 'a', 'b').join(' ')), 'an unwanted slot with nobody waiting goes back on the calendar');
ok(/gone straight to them/.test((0, reschedule_1.rescheduleLines)({ ...moved, promoted: true }, 'a', 'b').join(' ')), 'and one somebody was waiting for is reported as gone, from the server’s own answer');
const mc = (0, reschedule_1.moveConfirm)('Tue 7:00 am', 'Thu 6:00 pm');
...
eq((0, reschedule_1.noSlotsLine)('loading').endsWith('…'), true, 'a read in flight says so');
ok(/could not read/.test((0, reschedule_1.noSlotsLine)('error')), 'a failed read is not "no times"');
ok(/unchanged/.test((0, reschedule_1.noSlotsLine)('error')), 'and says the session is untouched');
ok(/not all of them/.test((0, reschedule_1.noSlotsLine)('partial')), 'a truncated read says it is short');
ok(/no other open times/.test((0, reschedule_1.noSlotsLine)('ready')), 'and only a completed one claims there are none');
/* ── pausing: the preview ──────────────────────────────────────────────── */
ok(/do not expect anything to be cancelled/.test((0, reschedule_1.pausePreviewLine)(0, 0, charges)), 'an empty range promises nothing');
ok(/authority/.test((0, reschedule_1.pausePreviewLine)(0, 0, charges)), 'and does not claim the device has the last word on the coach’s calendar');
...
ok(/of this arrangement/.test((0, reschedule_1.pausePreviewLine)(4, 0, charges)), 'the preview names the arrangement it is about rather than the whole diary');
ok(/we expect/.test((0, reschedule_1.pausePreviewLine)(4, 0, charges)), 'and states an expectation rather than a fact about somebody’s coach’s calendar');
ok(/costs nothing/.test((0, reschedule_1.pausePreviewLine)(4, 0, charges)), 'four sessions all outside the window cost nothing, and it says so');
ok(/does not charge/.test((0, reschedule_1.pausePreviewLine)(4, 2, free)), 'a coach with no policy costs nothing however late it is');
ok(/could not read your coach/.test((0, reschedule_1.pausePreviewLine)(4, 2, null)), 'and an unread policy is never softened into "no fee"');
const preview = (0, reschedule_1.pausePreviewLine)(4, 2, charges);
...
ok(/One of them is inside/.test((0, reschedule_1.pausePreviewLine)(3, 1, charges)), 'one of them is singular');
...
const pauseNoCcy = { applies: true, noticeHours: 24, fee: 25, currency: null };
const previewNoCcy = (0, reschedule_1.pausePreviewLine)(4, 2, pauseNoCcy);
...
ok(!/hasn’t set a currency/.test((0, reschedule_1.pausePreviewLine)(4, 2, { ...pauseNoCcy, fee: null })), 'nor when there is no figure to be in doubt about');
...
const nothing = (0, reschedule_1.pauseOutcomeLines)(base).join(' ');
...
const priced = (0, reschedule_1.pauseOutcomeLines)({ ...base, freed: 2, charged: 1, fees: 30, currency: 'GBP' }).join(' ');
...
const outcomeNoCcy = (0, reschedule_1.pauseOutcomeLines)({ ...base, freed: 2, charged: 1, fees: 25, currency: null }).join(' ');
...
const mixed = (0, reschedule_1.pauseOutcomeLines)({ ...base, freed: 3, charged: 2, fees: null, currency: null, mixedCurrencies: true }).join(' ');
...
const partly = (0, reschedule_1.pauseOutcomeLines)({ ...base, freed: 1, notFreed: 2 }).join(' ');
...
eq((0, reschedule_1.pausedRangeLine)('7 Sep', '7 Sep', null), 'Paused on 7 Sep.', 'a single day reads as one day');
eq((0, reschedule_1.pausedRangeLine)('7 Sep', '21 Sep', null), 'Paused from 7 Sep to 21 Sep.', 'and a range as a range');
ok(/\(away\)/.test((0, reschedule_1.pausedRangeLine)('7 Sep', '21 Sep', 'away')), 'a reason is carried when there is one');
const rc = (0, reschedule_1.resumeConfirm)('7 Sep', '21 Sep');
ok(/already passed do not come back/.test(rc.body), 'resuming says what cannot happen, which is the thing people expect it to do');
ok(/still to come/.test(rc.body), 'and what can');
ok(/no sessions were booked back in/.test((0, reschedule_1.resumedLine)(0)), 'lifting a pause that had nothing left in it says so rather than implying a failure');
ok(/one session has been booked back in/.test((0, reschedule_1.resumedLine)(1)), 'one is singular');
ok(/3 sessions have been booked back in/.test((0, reschedule_1.resumedLine)(3)), 'and more than one is not');
/* ── the coach moving a client's hour ───────────────────────────────────── */
const moveRep = (over = {}) => ({ moved: false, reason: null, clientId: null, promoted: false, waiting: 0, ...over });
...
for (const reason of ['taken', 'clash', 'already_started', 'not_yours', 'same_slot']) {
    const line = (0, reschedule_1.coachMoveRefusalLine)(moveRep({ reason }), 'Ana', '7am');
...
const lost = (0, reschedule_1.coachMoveRefusalLine)(reschedule_1.COACH_NOT_MOVED, 'Ana', '7am');
...
const bare = (0, reschedule_1.coachMoveRefusalLine)(moveRep({ reason: 'taken' }), null, null);
...
const promoted = (0, reschedule_1.coachMovedLine)(moveRep({ moved: true, promoted: true, waiting: 1 }), 'Ana', '7am', '8am', true);
...
const untold = (0, reschedule_1.coachMovedLine)(moveRep({ moved: true }), 'Ana', '7am', '8am', false);
...
const openAgain = (0, reschedule_1.coachMovedLine)(moveRep({ moved: true, waiting: 2 }), 'Ana', '7am', '8am', true);
...
const notWhole = (0, reschedule_1.pausePreviewLine)(0, 0, charges, false);
ok(/could not read your own calendar/.test(notWhole), 'an uncountable calendar says so rather than reporting a count');
ok(!/do not expect anything to be cancelled/.test(notWhole), 'and never tells a member with four sessions booked that nothing will be cancelled');
...
const shortAndFree = (0, reschedule_1.pausePreviewLine)(4, 0, charges, false);
ok(!/costs nothing/.test(shortAndFree), 'a truncated read never produces "this costs nothing" — the late ones may be the rows that did not come back');
...
ok(!/would carry their late fee|would each carry their late fee|inside your coach’s notice period/.test(shortAndFree), 'and it does not assert how many are inside the notice period: a floor supports neither claim, in either direction');
...
eq((0, reschedule_1.pausePreviewLine)(4, 2, charges), (0, reschedule_1.pausePreviewLine)(4, 2, charges, true), 'omitting the flag is the same as saying the read was whole');
if (errors.length) {
