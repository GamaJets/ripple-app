...
exports.PLAN_ALIASES = exports.PAYMENT_ALIASES = exports.COACH_ROSTER_ALIASES = exports.MEMBER_ALIASES = void 0;
exports.parseMoneyCents = parseMoneyCents;
exports.parseDate = parseDate;
exports.detectDateOrder = detectDateOrder;
exports.parseEmail = parseEmail;
exports.previewMembers = previewMembers;
exports.previewCoachRoster = previewCoachRoster;
exports.previewPayments = previewPayments;
exports.previewPlans = previewPlans;
exports.describePreview = describePreview;
 ...
 * spreadsheet.
 */
const NO_CURRENCY = 'the currency is not known, so this figure cannot be read into minor units — '
 ...
 */
function parseMoneyCents(raw, currency) {
    ...
    if (s === '')
        return { ok: false, reason: `"${raw}" has no digits` };
...
function ymdToIso(y, m, d) {
    if (m < 1 || m > 12)
        return { ok: false, reason: `month ${m} is out of range` };
    if (d < 1 || d > 31)
        return { ok: false, reason: `day ${d} is out of range` };
...
function parseDate(raw, order) {
    ...
    const bIsDay = b > 12;
    if (aIsDay && bIsDay)
        return { ok: false, reason: `"${raw}" has two components above 12` };
    if (aIsDay)
        return ymdToIso(c, b, a); // day first, settled by the value
    if (bIsDay)
        return ymdToIso(c, a, b); // month first, settled by the value
...
function parseEmail(raw) {
    ...
    if (t === '')
        return { ok: false, reason: 'empty' };
    if (!EMAIL.test(t))
        return { ok: false, reason: `"${raw}" is not an email address` };
    return { ok: true, value: t };
}
/* ── member import ─────────────────────────────────────────────────────────── */
exports.MEMBER_ALIASES = {
 ...
 */
function previewMembers(text, order) {
    const sheet = (0, csv_1.parseSheet)(text);
    const { index, unmatched } = (0, csv_1.mapColumns)(sheet.header, exports.MEMBER_ALIASES);
    const missingRequired = index.name === undefined ? ['name'] : [];
    const at = (r, f) => index[f] === undefined ? '' : (r[index[f]] ?? '');
    ...
    const effective = order ?? (detected === 'ambiguous' || detected === 'unknown' ? undefined : detected);
    const rows = sheet.rows.map((r, i) => {
        ...
        if (rawStart) {
            ...
            if (d.ok)
                ...
                errors.push(`start date: ${d.reason}`);
        }
        let endsOn = null;
        ...
        if (rawStatus) {
            ...
            if (!s)
                errors.push(`status "${at(r, 'status').trim()}" is not one this recognises`);
            else
                status = s;
        }
        const plan = at(r, 'plan').trim() || null;
        const value = { name, email, plan, startedOn, endsOn, status };
    ...
    for (const r of rows) {
        ...
        if (first !== undefined)
            r.errors.push(`duplicate of line ${first} (same email)`);
        else
            seen.set(e, r.line);
    }
 ...
 */
function previewCoachRoster(text) {
    const sheet = (0, csv_1.parseSheet)(text);
    const { index, unmatched } = (0, csv_1.mapColumns)(sheet.header, exports.COACH_ROSTER_ALIASES);
    const missingRequired = index.name === undefined ? ['name'] : [];
    const at = (r, f) => index[f] === undefined ? '' : (r[index[f]] ?? '');
    const rows = sheet.rows.map((r, i) => {
        const line = i + 2; // +1 for zero-index, +1 for the header
        ...
        if (rawMode) {
            ...
            if (!m)
                errors.push(`delivery "${at(r, 'mode').trim()}" is not one this recognises`);
            else
                mode = m;
        }
        const goal = at(r, 'goal').trim() || null;
        const value = { name, email, goal, mode };
        return { line, value, errors };
    });
 ...
 */
function previewPayments(text, order, currency) {
    const sheet = (0, csv_1.parseSheet)(text);
    const { index, unmatched } = (0, csv_1.mapColumns)(sheet.header, exports.PAYMENT_ALIASES);
    const missingRequired = [];
    if (index.amount === undefined)
    ...
    const detected = detectDateOrder(index.date !== undefined ? sheet.rows.map((r) => at(r, 'date')) : []);
    const effective = order ?? (detected === 'ambiguous' || detected === 'unknown' ? undefined : detected);
    const rows = sheet.rows.map((r, i) => {
        const line = i + 2;
        const errors = [];
        ...
        // one is a refund, which is not what this importer is for.
            ...
            errors.push('amount is negative — refunds are not imported here');
        const d = parseDate(at(r, 'date'), effective);
        if (!d.ok)
            errors.push(`date: ${d.reason}`);
        let email = null;
        const rawEmail = at(r, 'email').trim();
        if (rawEmail) {
            const e = parseEmail(rawEmail);
        ...
        if (stated && importIn && stated !== importIn) {
            errors.push(`this row is in ${stated} and the import is writing ${importIn} — `
                + 'the figures are not the same money and are not converted here');
        }
        const rawMethod = at(r, 'method').trim().toLowerCase().replace(/[^a-z]/g, '');
        const method = rawMethod ? (METHODS[rawMethod] ?? 'other') : 'other';
        const value = {
            memberName,
            email,
 ...
 */
function previewPlans(text, currency) {
    const sheet = (0, csv_1.parseSheet)(text);
    const { index, unmatched } = (0, csv_1.mapColumns)(sheet.header, exports.PLAN_ALIASES);
    const missingRequired = [];
    ...
    const rows = sheet.rows.map((r, i) => {
        ...
        if (name) {
            ...
            if (first !== undefined)
                errors.push(`duplicate of line ${first}`);
            else
                seen.set(key, line);
        }
        ...
        if (rawCurrency) {
            if (/^[A-Z]{3}$/.test(rawCurrency))
                ...
                errors.push(`currency "${at(r, 'currency').trim()}" is not a three-letter code`);
        }
        ...
        if (rawInterval) {
            ...
            if (hit)
                ...
                errors.push(`billing period "${at(r, 'interval').trim()}" is not month, year or one-off`);
        }
        ...
        if (rawActive) {
            if (ACTIVE_WORDS.has(rawActive))
                active = true;
            else if (INACTIVE_WORDS.has(rawActive))
                active = false;
            // Same reasoning as member status: a retired plan imported as active
            // goes back on sale.
            else
                errors.push(`"${at(r, 'active').trim()}" is not a yes/no I can read`);
        }
        const value = { name, priceCents, interval, currency, active };
        return errors.length ? { line, errors } : { line, value, errors };
    });
    const ready = rows.filter((r) => r.value !== undefined).map((r) => r.value);
...
function describePreview(p) {
    ...
    if (p.rows.length === 0)
        return 'That file has a header but no rows.';
    const parts = [`${p.ready.length} of ${p.rows.length} rows ready`];
    if (p.rejected.length)
        parts.push(`${p.rejected.length} need attention`);
    if (p.dateOrder === 'ambiguous')
        parts.push('date order unclear — say which the file uses');
    if (p.unmatchedColumns.length)
        parts.push(`ignoring ${p.unmatchedColumns.length} unrecognised column(s)`);
    return parts.join(' · ');
}
