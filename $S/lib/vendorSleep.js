...
exports.MAX_RECORD_SECONDS = void 0;
exports.parseOuraSleep = parseOuraSleep;
exports.parseWhoopSleep = parseWhoopSleep;
exports.parseFitbitSleep = parseFitbitSleep;
exports.parseVendorSleep = parseVendorSleep;
exports.vendorReadsSleep = vendorReadsSleep;
const sleepMerge_1 = require("./sleepMerge");
/** Fallback display names, used when the caller does not pass the registry's. */
const VENDOR_NAME = { oura: 'Oura Ring', whoop: 'WHOOP', fitbit: 'Fitbit' };
/**
 ...
 */
exports.MAX_RECORD_SECONDS = 24 * 3600;
/**
...
function seconds(v, perUnit) {
    const n = duration(v);
    if (n == null)
        return null;
    const s = n * perUnit;
    return s > 0 && s <= exports.MAX_RECORD_SECONDS ? s : null;
}
function bucket(totals, night) {
    let t = totals.get(night);
    if (!t) {
        t = { asleepSec: 0, inBedSec: 0 };
        totals.set(night, t);
    }
    return t;
}
 ...
 * genuinely recorded.
 */
function toReadings(totals, provider, vendor, sourceName) {
    const out = [];
    for (const [night, t] of totals) {
        ...
        if (minutesAsleep <= 0)
            continue;
        const basis = staged ? 'asleep' : 'in-bed';
        out.push({
            provider,
...
const rows = (records) => (Array.isArray(records) ? records : []);
/**
 * Oura sleep periods.
 *
 ...
 */
function parseOuraSleep(records, sourceName = VENDOR_NAME.oura) {
    const totals = new Map();
    for (const r of rows(records)) {
        const type = typeof r?.type === 'string' ? r.type.toLowerCase() : '';
        if (type === 'deleted' || type === 'rest')
            continue;
        const night = (0, sleepMerge_1.nightKey)(r?.bedtime_end) ?? (0, sleepMerge_1.nightKey)(r?.day);
        if (!night)
            continue;
        const asleep = seconds(r?.total_sleep_duration, 1);
        if (asleep != null) {
            bucket(totals, night).asleepSec += asleep;
            continue;
        }
        const inBed = seconds(r?.time_in_bed, 1);
        if (inBed != null)
            bucket(totals, night).inBedSec += inBed;
    }
 ...
 * reader here follows.
 */
function parseWhoopSleep(records, sourceName = VENDOR_NAME.whoop) {
    const totals = new Map();
    for (const r of rows(records)) {
        const night = (0, sleepMerge_1.nightKey)(r?.end);
        if (!night)
            continue;
        ...
        if (asleep > 0 && asleep <= exports.MAX_RECORD_SECONDS) {
            bucket(totals, night).asleepSec += asleep;
            continue;
        }
 ...
 */
function parseFitbitSleep(records, sourceName = VENDOR_NAME.fitbit) {
    const totals = new Map();
    for (const r of rows(records)) {
        const night = (0, sleepMerge_1.nightKey)(r?.dateOfSleep) ?? (0, sleepMerge_1.nightKey)(r?.endTime);
        if (!night)
            continue;
...
function parseVendorSleep(vendor, records, sourceName) {
    if (vendor === 'oura')
        return parseOuraSleep(records, sourceName ?? VENDOR_NAME.oura);
    if (vendor === 'whoop')
        return parseWhoopSleep(records, sourceName ?? VENDOR_NAME.whoop);
    if (vendor === 'fitbit')
        return parseFitbitSleep(records, sourceName ?? VENDOR_NAME.fitbit);
    return [];
}
/** Whether Repple has a sleep reader for this provider at all. */
function vendorReadsSleep(id) {
