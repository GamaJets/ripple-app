...
const assignPicker_1 = require("./assignPicker");
const errors = [];
...
for (const status of ALL) {
    const d = (0, assignPicker_1.seedDecision)({
        programStatus: status, hasDraft: true, seededFor: null, clientId: 'c1', firstName: 'Priya',
    });
    eq(d.action, 'hold', `A DRAFT IS NEVER REPLACED BY PICKING A CLIENT (${status}) — this is the twenty minutes of work that disappeared at the exact gesture meant to send it`);
    ok(!!d.note && d.note.length > 20, `and the disagreement is SAID (${status}) — a builder showing one thing while another client's name is selected is the trap the screen's own header describes`);
    ok((d.note ?? '').includes('Priya'), `and the sentence names the client (${status}), because "this client" is not who the coach is looking at`);
}
...
for (const status of ['loading', 'partial', 'error']) {
    ...
    eq(d.replaceLabel, null, `NO REPLACE CONTROL WHEN THERE IS NOTHING TO REPLACE IT WITH (${status}) — a button that loads an unread programme would load silence over real work`);
}
...
{
    const ready = (0, assignPicker_1.seedDecision)({
...
for (const status of ['loading', 'partial', 'error']) {
    eq((0, assignPicker_1.seedDecision)({
        programStatus: status, hasDraft: false, seededFor: null, clientId: 'c1', firstName: 'Priya',
    }).action, 'clear', `AN UNREAD PROGRAMME LEAVES THE BUILDER EMPTY (${status}) — filling it with a generated plan and calling it theirs is what the overwrite guard exists to stop`);
}
...
eq((0, assignPicker_1.stillListed)('ready', ['a', 'b'], 'a'), true, 'a client on a whole read is on the book');
eq((0, assignPicker_1.stillListed)('ready', ['a', 'b'], 'z'), false, 'and one who is NOT on a whole read has left it — which is what stops the screen reporting a departed client’s injuries as unreadable');
eq((0, assignPicker_1.stillListed)('ready', [], ''), false, 'nobody is not somebody');
for (const status of ['loading', 'partial', 'error']) {
    eq((0, assignPicker_1.stillListed)(status, [], 'z'), true, `ABSENT UNDER ${status.toUpperCase()} IS UNKNOWN, NOT GONE — the read did not finish or stopped at the row cap, and dropping the tick would silently unselect somebody the coach chose`);
}
eq((0, assignPicker_1.pruneSelection)('ready', ['a', 'b', 'c'], ['a', 'z', 'c']).join(','), 'a,c', 'a whole read drops the ticks that name nobody, in the order they were given');
eq((0, assignPicker_1.pruneSelection)('error', ['a'], ['a', 'z']).join(','), 'a,z', 'a failed read changes nothing — an empty roster under error means the read failed');
eq((0, assignPicker_1.pruneSelection)('partial', [], ['a', 'z']).join(','), 'a,z', 'and neither does a truncated one');
{
    const src = ['a'];
    const out = (0, assignPicker_1.pruneSelection)('error', [], src);
...
eq((0, assignPicker_1.assignCtaLabel)({ busy: true, picked: 3, exercises: 9, planLabel: null, soleName: null }), 'Assigning…', 'a write in flight says so before anything else');
{
    const l = (0, assignPicker_1.assignCtaLabel)({ busy: false, picked: 3, exercises: 0, planLabel: null, soleName: null });
    ok(/exercise/i.test(l), 'AN EMPTY PROGRAMME IS SAID FIRST — there is nothing to assign to anybody, so that is the coach’s next move whoever is ticked');
}
{
    ...
    const l = (0, assignPicker_1.assignCtaLabel)({ busy: false, picked: 0, exercises: 0, planLabel: null, soleName: null });
    ok(/exercise/i.test(l) && !/Pick Who/i.test(l), 'THE EMPTY PROGRAMME OUTRANKS THE EMPTY SELECTION — asking who should receive nothing sends the coach to the wrong end of the screen');
}
eq((0, assignPicker_1.assignCtaLabel)({ busy: false, picked: 0, exercises: 9, planLabel: null, soleName: null }), 'Pick Who Gets This', 'with a programme built and nobody ticked, the button asks for the recipients');
eq((0, assignPicker_1.assignCtaLabel)({ busy: false, picked: 0, exercises: 9, planLabel: 'Nobody In This Group Yet', soleName: null }), 'Pick Who Gets This', 'AND ASKS BEFORE THE FAN-OUT DOES — planFanOut answers an empty list in the Groups screen’s vocabulary, and this screen has no groups in it');
eq((0, assignPicker_1.assignCtaLabel)({ busy: false, picked: 2, exercises: 9, planLabel: 'Read Their Injuries First', soleName: null }), 'Read Their Injuries First', 'a refusal from the fan-out is what the button carries, so the control names its own reason');
{
    const l = (0, assignPicker_1.assignCtaLabel)({ busy: false, picked: 1, exercises: 1, planLabel: null, soleName: 'Priya' });
    ok(l.includes('Priya'), 'one recipient is named, because that is who the tap writes over');
    ok(l.includes('1 exercise') && !l.includes('1 exercises'), 'and the count is written in English — four counts on this screen once read "1 exercises"');
}
{
    const l = (0, assignPicker_1.assignCtaLabel)({ busy: false, picked: 4, exercises: 1200, planLabel: null, soleName: null });
