...
const clipOwner_1 = require("./clipOwner");
const errors = [];
...
const mine = { id: 'db-1111', trainerId: ME };
const theirs = { id: 'db-2222', trainerId: THEM };
const platform = { id: 'db-3333', trainerId: null };
const local = { id: 'vx-4444', trainerId: null };
/* ── the reported bug ──────────────────────────────────────────────────── */
// All four of these start with 'db' or were treated as manageable by the old
// predicate. Only one of them is this coach's.
{
    eq((0, clipOwner_1.clipOwner)(mine, ME), 'mine', "the coach's own row is theirs");
    eq((0, clipOwner_1.clipOwner)(theirs, ME), 'other', "another coach's published clip is not");
    eq((0, clipOwner_1.clipOwner)(platform, ME), 'platform', 'a clip that ships with Repple is not');
    eq((0, clipOwner_1.clipOwner)(local, ME), 'local', 'and an entry that never reached the server is not');
}
...
{
    ok((0, clipOwner_1.canManageClip)(mine, ME), 'a coach manages their own clip');
    ok(!(0, clipOwner_1.canManageClip)(theirs, ME), 'a coach does not manage another coach’s clip');
    ok(!(0, clipOwner_1.canManageClip)(platform, ME), 'a coach does not manage a platform clip');
    ok(!(0, clipOwner_1.canManageClip)(local, ME), 'there is no row behind a local entry to manage');
}
...
{
    eq((0, clipOwner_1.clipOwner)(mine, null), 'other', 'an unknown reader owns nothing');
    ok(!(0, clipOwner_1.canManageClip)(mine, null), 'and is offered nothing to manage');
    ...
    eq((0, clipOwner_1.clipOwner)(platform, null), 'platform', 'a platform clip is one regardless of who is looking');
}
...
{
    eq((0, clipOwner_1.clipOwner)({ id: 'db-5', trainerId: '' }, ''), 'other', 'an empty id matches nobody');
}
...
{
    ok((0, clipOwner_1.canRemoveClip)(mine, ME), 'a coach removes their own clip');
    ok((0, clipOwner_1.canRemoveClip)(local, ME), 'and can forget a local-only entry');
    ok(!(0, clipOwner_1.canRemoveClip)(theirs, ME), 'but cannot remove another coach’s clip');
    ok(!(0, clipOwner_1.canRemoveClip)(platform, ME), 'nor one that ships with Repple');
    // The two lists differ in exactly one place, which is the whole reason there
    // are two functions.
    const all = [mine, theirs, platform, local];
    eq(all.filter((c) => (0, clipOwner_1.canRemoveClip)(c, ME)).length
        - all.filter((c) => (0, clipOwner_1.canManageClip)(c, ME)).length, 1, 'remove is wider than manage by one case, and only one');
}
...
{
    eq((0, clipOwner_1.clipOwner)({ id: 'zz-9', trainerId: ME }, ME), 'local', 'an unrecognised prefix has no row behind it');
    ok(!(0, clipOwner_1.canManageClip)({ id: 'zz-9', trainerId: ME }, ME), 'so it is not managed');
}
...
{
    eq((0, clipOwner_1.clipOwner)({ id: 'db-abc-def', trainerId: ME }, ME), 'mine', 'a real uuid row still resolves to mine');
    ok((0, clipOwner_1.canManageClip)({ id: 'db-abc-def', trainerId: ME }, ME), 'and is fully manageable');
}
if (errors.length) {
