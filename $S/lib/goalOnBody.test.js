...
const goalOnBody_1 = require("./goalOnBody");
const errors = [];
...
const series = (...vals) => vals.map((v, i) => ({ t: new Date(Date.UTC(2026, 0, 2 + i * 7)).toISOString(), v }));
// ── a plain distance ──────────────────────────────────────────────────────
{
    const r = (0, goalOnBody_1.goalOnBody)(goal({ targetValue: 80 }), series(90, 85), { weight: true, unit: 'kg', wu: 'kg' });
...
{
    const down = (0, goalOnBody_1.goalOnBody)(goal({ targetValue: 80 }), series(90, 83), { weight: true, unit: 'kg', wu: 'kg' });
    const up = (0, goalOnBody_1.goalOnBody)(goal({ kind: 'muscle', targetValue: 40 }), series(34, 37), { weight: true, unit: 'kg', wu: 'kg' });
...
// stone.
{
    const r = (0, goalOnBody_1.goalOnBody)(goal({ targetValue: 80 }), series(90), { weight: true, unit: 'kg', wu: 'lb' });
...
{
    const r = (0, goalOnBody_1.goalOnBody)(goal({ kind: 'bodyfat', targetValue: 15 }), series(22, 18), { weight: false, unit: '%', wu: 'lb' });
...
// ── reaching it ──────────────────────────────────────────────────────────
{
    const exactly = (0, goalOnBody_1.goalOnBody)(goal({ targetValue: 80 }), series(90, 80), { weight: true, unit: 'kg', wu: 'kg' });
    const past = (0, goalOnBody_1.goalOnBody)(goal({ targetValue: 80 }), series(90, 78), { weight: true, unit: 'kg', wu: 'kg' });
...
{
    const r = (0, goalOnBody_1.goalOnBody)(goal({ targetValue: 80 }), series(90, 80.02), { weight: true, unit: 'kg', wu: 'kg' });
...
// ── the four refusals ────────────────────────────────────────────────────
{
    eq((0, goalOnBody_1.goalOnBody)(null, series(90), { weight: true, unit: 'kg', wu: 'kg' }), null, 'no goal, no line — the caller owns the sentence about why, because only the caller can tell "none set" from "not read"');
    eq((0, goalOnBody_1.goalOnBody)(goal({ kind: 'custom', targetValue: null, title: 'Run a 10k' }), series(90), { weight: true, unit: 'kg', wu: 'kg' }), null, 'A CUSTOM GOAL IS A SENTENCE, NOT A NUMBER — percentages of sentences are how progress stops meaning anything');
    eq((0, goalOnBody_1.goalOnBody)(goal({ targetValue: null }), series(90), { weight: true, unit: 'kg', wu: 'kg' }), null, 'a goal with no target value has no distance to be from');
    eq((0, goalOnBody_1.goalOnBody)(goal(), [], { weight: true, unit: 'kg', wu: 'kg' }), null, 'and no readings means nothing to measure the distance from — not a distance of the whole target');
}
// ── picking the goal for a metric ────────────────────────────────────────
{
    const goals = [goal({ id: 'a', kind: 'weight' }), goal({ id: 'b', kind: 'bodyfat', targetValue: 15 })];
    eq((0, goalOnBody_1.goalOfKind)(goals, 'bodyfat').id, 'b', 'the goal of the asked-for kind is the one returned');
    eq((0, goalOnBody_1.goalOfKind)(goals, 'muscle'), null, 'and a kind with no goal is null rather than the first goal in the list');
    eq((0, goalOnBody_1.goalOfKind)(null, 'weight'), null, 'an unread goal list is null, and the caller decides what to say about that');
    eq((0, goalOnBody_1.goalOfKind)([], 'weight'), null, 'as is an empty one');
    ...
    eq((0, goalOnBody_1.goalOfKind)(done, 'weight').id, 'c', 'an achieved goal is still returned');
}
if (errors.length) {
