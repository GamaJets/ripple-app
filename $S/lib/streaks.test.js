...
const middayOn = (y, mIdx, d) => new Date(y, mIdx, d, 12, 0, 0, 0);
const entryAt = (at, exercise = 'Squat') => ({ t: at.toISOString(), exercise, sets: [[5, 100]] });
...
/* ── the sweep: a whole year, one assertion per day ──────────────────────── */
{
    ...
    for (let i = 0; i < 365; i++) {
        ...
        const got = (0, streaks_1.currentStreak)(log, today.getTime());
        if (got !== 5) {
            failedDays++;
            if (!firstFailure)
                firstFailure = `${today.toDateString()} gave ${got}`;
        }
    }
    ok(start.getTime() > 0, 'the year starts somewhere');
    eq(failedDays, 0, `five consecutive local days is a five-day streak on every day of the year (first break: ${firstFailure || 'none'})`);
}
...
// day the same way and broke the same way.
{
    ...
    for (let i = 0; i < 365; i++) {
        const today = new Date(2026, 0, 1 + i, 12, 0, 0, 0);
        const log = runEndingAt(today, 5);
        if ((0, streaks_1.currentStreakFrozen)(log, 2, today.getTime()).streak !== 5)
            frozenFailures++;
        ...
        const r = (0, streaks_1.streakRisk)(upToYesterday, today.getTime());
        if (!r.atRisk || r.trainedToday)
            riskFailures++;
    }
...
/* ── a freeze still bridges exactly one missing day ──────────────────────── */
{
    ...
    eq((0, streaks_1.currentStreak)(log, today.getTime()), 1, 'without a freeze the chain ends at the gap');
    const f = (0, streaks_1.currentStreakFrozen)(log, 1, today.getTime());
    ...
    const none = (0, streaks_1.currentStreakFrozen)(log, 0, today.getTime());
...
// A trailing gap must never spend a freeze: there is nothing older to reach.
{
    ...
    const f = (0, streaks_1.currentStreakFrozen)(log, 2, today.getTime());
...
// just trained must not be told their streak is at risk.
{
    ...
    eq((0, streaks_1.currentStreak)(log, today.getTime()), 1, 'a 22:30 session counts as today');
    eq((0, streaks_1.streakRisk)(log, today.getTime()).trainedToday, true, 'and the risk flag knows it');
    eq((0, streaks_1.activeDays)(log).length, 1, 'one day, whatever UTC says about it');
}
/* ── nothing to count ────────────────────────────────────────────────────── */
{
    const today = middayOn(2026, 5, 15);
    eq((0, streaks_1.currentStreak)([], today.getTime()), 0, 'an empty log is a streak of zero, not a crash');
    eq((0, streaks_1.currentStreakFrozen)([], 2, today.getTime()).streak, 0, 'and so is the frozen walk');
    eq((0, streaks_1.longestStreak)([]), 0, 'and the longest run of nothing is nothing');
    // A gap of two days with a single freeze cannot be bridged.
    const stale = [entryAt(new Date(2026, 5, 10, 12, 0, 0, 0))];
    eq((0, streaks_1.currentStreak)(stale, today.getTime()), 0, 'a chain that ended last week is over');
}
/* ── longestStreak agrees with the walk ──────────────────────────────────── */
{
    const today = middayOn(2026, 2, 30);
    const log = runEndingAt(today, 9);
    eq((0, streaks_1.longestStreak)(log), 9, 'nine consecutive days is a longest run of nine');
    eq((0, streaks_1.currentStreak)(log, today.getTime()), 9, 'and the current streak agrees with it');
}
...
// the model. `shownStreak` is the single answer every one of those now asks for.
{
    ...
    const raw = (0, streaks_1.currentStreak)(log, today.getTime());
    const shown = (0, streaks_1.shownStreak)(log, today.getTime());
    ok(shown > raw, 'the freeze the app granted is reflected in the figure the member is shown');
    eq(shown, (0, streaks_1.currentStreakFrozen)(log, (0, streaks_1.freezeBudget)(log), today.getTime()).streak, 'and it is exactly the frozen walk over the budget the log earned — no second opinion about either');
}
{
    ...
    eq((0, streaks_1.shownStreak)(log, today.getTime()), (0, streaks_1.currentStreak)(log, today.getTime()), 'with nothing to bridge the shown streak is the plain chain');
    eq((0, streaks_1.shownStreak)([], today.getTime()), 0, 'and an empty log is zero, not a crash');
}
...
// instant is built from `startOfWeek` of a chosen `now`.
{
    ...
    const weekOpened = (0, weekStart_1.startOfWeek)(now).getTime();
    const HOUR = 3600000;
    const entry = (t) => ({ t: new Date(t).toISOString(), exercise: 'Bench', sets: [[8, 60]] });
    ...
    eq((0, streaks_1.thisWeekStats)(log, now).workouts, 1, 'the calendar week counts only what was done since the week opened');
    eq((0, streaks_1.weekStats)(log, now).workouts, 2, 'and the rolling window still counts both, which is what it is for');
    ok((0, streaks_1.thisWeekStats)(log, now).workouts !== (0, streaks_1.weekStats)(log, now).workouts, 'the two windows are genuinely different answers, which is why one screen may not print the other’s figure under the other’s caption');
    ...
    eq((0, streaks_1.thisWeekStats)(lastWeekOnly, now).workouts, 0, 'a week with nothing done in it counts nought, however busy the seven days before it were');
    eq((0, streaks_1.thisWeekStats)(lastWeekOnly, now).days, 0, 'and no active days either');
    // The boundary itself: an entry AT the opening instant is in the week.
    eq((0, streaks_1.thisWeekStats)([entry(weekOpened)], now).workouts, 1, 'the instant the week opened belongs to the week it opened');
    eq((0, streaks_1.thisWeekStats)([entry(weekOpened - 1)], now).workouts, 0, 'and the millisecond before it does not');
    // The engine both windows share. The report screen states a span and now
    // counts exactly that span, which is what this is for.
    eq((0, streaks_1.statsSince)(log, weekOpened).workouts, (0, streaks_1.thisWeekStats)(log, now).workouts, 'the calendar week is statsSince from the moment the week opened, and nothing else');
    eq((0, streaks_1.statsSince)(log, 0).workouts, 2, 'and an open window counts everything');
    // Nothing is invented from an empty log.
    eq((0, streaks_1.thisWeekStats)([], now).workouts, 0, 'an empty log is nought sessions');
    eq((0, streaks_1.thisWeekStats)([], now).volumeKg, 0, 'and nought volume');
    eq((0, streaks_1.thisWeekStats)([], now).unpricedSets, 0, 'with no unpriced sets to declare');
}
if (errors.length) {
    errors.forEach((e) => console.error(e));
    console.error(`streaks: ${errors.length} failure(s)`);
    process.exit(1);
}
