...
const trainerProfileAccess_1 = require("./trainerProfileAccess");
const errors = [];
...
const read = (o = {}) => (0, trainerProfileAccess_1.resolveTrainerAccess)({ variant: 'trainer', settled: true, signedIn: true, trainerRow: 'present', ...o });
/* ── the five outcomes ──────────────────────────────────────────────────── */
...
ok(read({ trainerRow: 'absent' }) === 'not-a-trainer', 'a signed-in user whose trainers row came back absent is not a trainer');
...
ok(read({ trainerRow: 'unknown' }) === 'ok', 'an unreadable trainers row does not demote a coach on the coach build');
...
ok(read({ variant: 'client', settled: false }) === 'wrong-app', 'an unsettled read on the client build is still the wrong app, not loading');
ok(read({ variant: 'client', signedIn: false }) === 'wrong-app', 'a signed-out client build is still the wrong app, not signed out');
ok(read({ variant: 'client', settled: false, signedIn: false, trainerRow: 'absent' }) === 'wrong-app', 'no combination of the other three facts turns the wrong app into a real answer');
...
ok(read({ variant: 'client', trainerRow: 'unknown' }) === 'wrong-app', 'the client build is refused even when the row read is inconclusive');
/* ── only 'ok' lets the loaded values out ───────────────────────────────── */
const ALL = ['wrong-app', 'loading', 'signed-out', 'not-a-trainer', 'ok'];
const BLOCKED = ALL.filter((a) => a !== 'ok');
ok(ALL.filter(trainerProfileAccess_1.mayReadTrainerProfile).length === 1, 'exactly one access state is readable');
ok((0, trainerProfileAccess_1.mayReadTrainerProfile)('ok'), 'and it is the one that means the profile is really yours');
ok((0, trainerProfileAccess_1.guardTrainerProfile)('ok', LOADED) === LOADED, 'a real trainer sees the row that was loaded');
...
ok(BLOCKED.every((a) => (0, trainerProfileAccess_1.guardTrainerProfile)(a, LOADED).name === ''), 'no blocked state hands out a name');
ok(BLOCKED.every((a) => (0, trainerProfileAccess_1.guardTrainerProfile)(a, LOADED).photo === null), 'no blocked state hands out a photo');
...
ok(BLOCKED.every((a) => {
    const g = (0, trainerProfileAccess_1.guardTrainerProfile)(a, LOADED);
...
ok(BLOCKED.every((a) => (0, trainerProfileAccess_1.guardTrainerProfile)(a, LOADED).sessionFee === null), 'an unknown session fee is null in every blocked state');
ok(BLOCKED.every((a) => (0, trainerProfileAccess_1.guardTrainerProfile)(a, LOADED).sessionFee !== 0), 'and it is never 0, which is a rate somebody could actually charge');
ok(trainerProfileAccess_1.NO_TRAINER_PROFILE.sessionFee === null, 'the blank profile carries no fee at all');
...
ok(BLOCKED.every((a) => (0, trainerProfileAccess_1.guardTrainerProfile)(a, LOADED).publicPage === false), 'a build that cannot read this profile is never told the reader has a public page');
ok(BLOCKED.every((a) => (0, trainerProfileAccess_1.guardTrainerProfile)(a, LOADED).publicHandle === null), 'and never handed the address of one');
ok(trainerProfileAccess_1.NO_TRAINER_PROFILE.publicPage === false && trainerProfileAccess_1.NO_TRAINER_PROFILE.publicHandle === null, 'the blank profile publishes nothing and has nowhere to publish it');
...
ok((0, trainerProfileAccess_1.guardTrainerProfile)('ok', { ...LOADED, sessionFee: 0 }).sessionFee === 0, 'a real rate of 0 passes through as 0');
...
const blank = (0, trainerProfileAccess_1.guardTrainerProfile)('wrong-app', LOADED);
try {
    blank.offers.push('sneaked in');
}
catch { /* frozen, as intended */ }
ok((0, trainerProfileAccess_1.guardTrainerProfile)('wrong-app', LOADED).offers.length === 0, 'the blank profile cannot be added to');
/* ── the notes ──────────────────────────────────────────────────────────── */
ok((0, trainerProfileAccess_1.trainerAccessNote)('ok') === null, 'a real profile needs no explanation beside it');
ok(!!(0, trainerProfileAccess_1.trainerAccessNote)('loading'), 'a coach waiting on the read is told so');
ok(!!(0, trainerProfileAccess_1.trainerAccessNote)('signed-out'), 'a signed-out coach is told to sign in');
ok(!!(0, trainerProfileAccess_1.trainerAccessNote)('not-a-trainer'), 'an account with no trainer row is told why it is empty');
...
ok((0, trainerProfileAccess_1.trainerAccessNote)('wrong-app') === null, 'the wrong app is given no reassuring sentence');
ok(new Set(BLOCKED.map(trainerProfileAccess_1.trainerAccessNote)).size === BLOCKED.length, 'each blocked state says something different, so the reason is never guessed at');
