...
// ── the bug: the sentence is in the member's unit ──────────────────────────
{
    ...
    const lb = (0, progression_1.suggestNextWeight)([[8, 60]], { low: 6, high: 8 }, 2.5, 'lb');
    ...
    const kg = (0, progression_1.suggestNextWeight)([[8, 60]], { low: 6, high: 8 }, 2.5, 'kg');
...
{
    const hold = (0, progression_1.suggestNextWeight)([[6, 50]], { low: 6, high: 8 }, 2.5, 'lb');
...
// ── no range: the "match last" wording ─────────────────────────────────────
{
    ...
    eq((0, progression_1.parseRepRange)('45 sec'), null, 'a duration is not a rep range');
    const none = (0, progression_1.suggestNextWeight)([[10, 40]], null, 2.5, 'lb');
...
// ── omitting the unit no longer means kilograms ────────────────────────────
{
    ...
    const bare = (0, progression_1.suggestNextWeight)([[8, 60]], { low: 6, high: 8 });
...
{
    const at = (t) => `2026-08-3${t}T10:00:00.000Z`;
    const one = (sets, feel) => (0, progression_1.suggestProgression)([{ t: at('0'), exercise: 'Back Squat', sets, feel }], 'lb')[0];
    ...
    eq(new Set(branches.map(({ tip }) => tip.rationale)).size, branches.length, `each fixture reaches a branch of its own — got ${new Set(branches.map(({ tip }) => tip.rationale)).size} distinct cues from ${branches.length} fixtures`);
    ...
    const kg = (0, progression_1.suggestProgression)([{ t: at('0'), exercise: 'Back Squat', sets: [[6, 100], [6, 100]] }], 'kg')[0];
    ...
    const none = (0, progression_1.suggestProgression)([{ t: at('0'), exercise: 'Back Squat', sets: [[6, 100], [6, 100]] }])[0];
...
// ── through the convenience wrapper the Train tab actually calls ───────────
{
    ...
    const lb = (0, progression_1.suggestForExercise)(log, 'Back Squat', '6-8', 2.5, 'lb');
    ...
    ok((0, progression_1.priorBest1RM)(log, 'Back Squat') > 60, 'a prior best exists for the PR check');
    eq((0, progression_1.priorBest1RM)(log, 'Front Squat'), 0, 'and is per movement, not shared');
}
...
// Every session.
{
    ...
    const tips = (0, progression_1.suggestProgression)(log, 'kg');
    eq(tips.length, 0, 'a session of holds produces no load target at all');
    eq((0, progression_1.priorBest1RM)(log, 'Plank'), 0, 'and no estimated 1RM — Epley over seconds is arithmetic on a stopwatch, and it silently outranked every real set on that movement for ever');
    eq((0, progression_1.suggestForExercise)(log, 'Plank', '8-12', 2.5, 'kg'), null, 'and nothing to suggest for next time');
}
{
    ...
    const tip = (0, progression_1.suggestProgression)(log, 'kg').find((x) => x.exercise === 'Farmer Carry');
    ...
    ok((0, progression_1.priorBest1RM)(log, 'Farmer Carry') === Math.max(0, Math.round(20 * (1 + 12 / 30))), 'the record is computed from the repeated sets only');
}
if (errors.length) {
