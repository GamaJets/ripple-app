...
eq((0, gymZone_1.gymDay)(MONTH_EDGE, 'Asia/Dubai'), '2026-09-01', 'the instant is September at the Dubai gym');
eq((0, gymZone_1.gymDay)(MONTH_EDGE, 'Europe/London'), '2026-08-31', 'and August in London');
{
    ...
    const dubai = (0, gymWhen_1.gymDateText)(MONTH_EDGE, 'Asia/Dubai') ?? '';
    const london = (0, gymWhen_1.gymDateText)(MONTH_EDGE, 'Europe/London') ?? '';
...
eq((0, gymWhen_1.gymDateText)(MONTH_EDGE, 'Asia/Dubai'), (0, gymWhen_1.gymDateText)(MONTH_EDGE, 'Asia/Dubai'), 'the same question twice gives the same answer');
{
    ...
    for (const z of ['Asia/Dubai', 'Europe/London', 'America/Los_Angeles', 'Pacific/Auckland', 'UTC']) {
        const day = (0, gymZone_1.gymDay)(MONTH_EDGE, z) ?? '';
        const [y, m, d] = day.split('-');
        const text = (0, gymWhen_1.gymDateText)(MONTH_EDGE, z) ?? '';
...
/* ── the time half, which is where the four hours actually go ──────────── */
{
    for (const z of ['Asia/Dubai', 'Europe/London', 'America/Los_Angeles']) {
        const label = (0, gymZone_1.gymTimeLabel)(MONTH_EDGE, z) ?? '';
        const [hh, mm] = label.split(':');
        const text = (0, gymWhen_1.gymTimeText)(MONTH_EDGE, z, { hour: '2-digit', minute: '2-digit', hourCycle: 'h23' }) ?? '';
...
ok((0, gymWhen_1.drawnAtGym)('Asia/Dubai'), 'a real zone is a zone');
ok(!(0, gymWhen_1.drawnAtGym)(null), 'a missing one is not');
ok(!(0, gymWhen_1.drawnAtGym)(''), 'nor is an empty string');
...
ok(!(0, gymWhen_1.drawnAtGym)('+04:00'), 'an offset is not a place');
ok(!(0, gymWhen_1.drawnAtGym)('EST'), 'and an abbreviation is a fixed offset pretending to be one');
eq((0, gymWhen_1.whoseClockNote)('Asia/Dubai'), null, 'a gym with a zone needs no caveat');
eq((0, gymWhen_1.whoseClockNote)(null), gymWhen_1.NO_ZONE_NOTE, 'a gym without one gets the shared sentence');
eq((0, gymWhen_1.whoseClockNote)('EST'), gymWhen_1.NO_ZONE_NOTE, 'and so does a gym whose stored value cannot be drawn in');
{
    const w = (0, gymWhen_1.gymWhenDate)(MONTH_EDGE, 'Asia/Dubai');
    ok(w != null && w.atGym, 'a rendered date knows it is the gym’s own');
    const r = (0, gymWhen_1.gymWhenDate)(MONTH_EDGE, null);
...
eq((0, gymWhen_1.gymWhenDate)(null, 'Asia/Dubai'), null, 'null is not a date');
eq((0, gymWhen_1.gymWhenDate)(undefined, 'Asia/Dubai'), null, 'nor is undefined');
eq((0, gymWhen_1.gymWhenDate)('', 'Asia/Dubai'), null, 'nor is an empty string');
eq((0, gymWhen_1.gymWhenDate)('not a date', 'Asia/Dubai'), null, 'and an unparseable string renders nothing');
eq((0, gymWhen_1.gymDateText)('not a date', 'Asia/Dubai'), null, 'never the words "Invalid Date", which is what toLocaleDateString would have put in the cell');
/* ── the three shapes carry what they say they carry ───────────────────── */
{
    ...
    const dt = (0, gymWhen_1.gymDateTimeText)(MONTH_EDGE, 'Asia/Dubai') ?? '';
    const d = (0, gymWhen_1.gymDateText)(MONTH_EDGE, 'Asia/Dubai') ?? '';
    const t = (0, gymWhen_1.gymTimeText)(MONTH_EDGE, 'Asia/Dubai') ?? '';
...
}
{
    ...
    const only = (0, gymWhen_1.gymDateText)(MONTH_EDGE, 'Asia/Dubai', { year: 'numeric' }) ?? '';
...
}
{
    ...
    const forced = (0, gymWhen_1.gymDateText)(MONTH_EDGE, 'Asia/Dubai', { timeZone: 'America/Los_Angeles' }) ?? '';
    eq(forced, (0, gymWhen_1.gymDateText)(MONTH_EDGE, 'Asia/Dubai'), 'a per-call timeZone is ignored — the gym’s zone wins');
}
/* ── a calendar date is not an instant ─────────────────────────────────── */
{
    ...
    const t = (0, gymWhen_1.calendarDateText)('2026-09-06') ?? '';
    ok(/\b0?9\b/.test(t), `month 9 survives — got ${t}`);
    ok(/\b0?6\b/.test(t), `day 6 survives — got ${t}`);
    ok(/2026/.test(t), `and the year — got ${t}`);
    eq((0, gymWhen_1.calendarDateText)('2026-09-06', { month: 'long', year: 'numeric' })?.includes('2026'), true, 'and a month-and-year label carries its year');
    ...
    const jan = (0, gymWhen_1.calendarDateText)('2026-01-01', { month: 'numeric', year: 'numeric' }) ?? '';
    ok(/\b0?1\b/.test(jan) && /2026/.test(jan), `1 January stays January 2026 — got ${jan}`);
    const dec = (0, gymWhen_1.calendarDateText)('2026-12-31', { month: 'numeric', day: 'numeric' }) ?? '';
    ...
    const heading = (0, gymWhen_1.calendarDateText)('2026-03-04', { month: 'short' }) ?? '';
    ...
    // default date.
    const whole = (0, gymWhen_1.calendarDateText)('2026-03-04') ?? '';
    ok(/2026/.test(whole) && /\b0?3\b/.test(whole) && /\b0?4\b/.test(whole), `a caller asking for nothing gets the whole date — got ${whole}`);
}
eq((0, gymWhen_1.calendarDateText)(null), null, 'nothing is not a calendar date');
eq((0, gymWhen_1.calendarDateText)('2026-09'), null, 'and neither is half of one');
eq((0, gymWhen_1.calendarDateText)('2026-09-06T10:00:00Z'), null, 'and neither is an instant — that is what gymWhenDate is for, and it needs the gym’s zone');
if (errors.length) {
