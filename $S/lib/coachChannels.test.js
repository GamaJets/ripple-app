...
const coachChannels_1 = require("./coachChannels");
const errors = [];
...
const whyAgainst = (a) => (a.statable ? 'statable' : a.why);
...
const withDefault = (0, coachChannels_1.channelSum)([
...
eq((0, coachChannels_1.channelSum)([row()]).codes, 1, 'a named code is counted');
eq((0, coachChannels_1.channelSum)([row()]).unnamed, 0, 'and is not filed as unnamed');
/* ── unknown is not zero, on either side ────────────────────────────────── */
const noCost = (0, coachChannels_1.channelSum)([row({ revenue: { cents: 125000, currency: 'GBP' } })]);
...
const zeroCost = (0, coachChannels_1.channelSum)([row({ spend: { cents: 0, currency: 'GBP' }, revenue: { cents: 125000, currency: 'GBP' } })]);
...
const noRevenue = (0, coachChannels_1.channelSum)([row({ spend: { cents: 40000, currency: 'GBP' } })]);
...
const twoCurrencies = (0, coachChannels_1.channelSum)([
...
const tied = (0, coachChannels_1.channelSum)([
...
const sameCurrency = (0, coachChannels_1.channelSum)([
    row({ id: 'a', spend: { cents: 5000, currency: 'GBP' } }),
...
const crossed = (0, coachChannels_1.channelSum)([row({ spend: { cents: 40000, currency: 'GBP' }, revenue: { cents: 125000, currency: 'AED' } })]);
eq(crossed.crossCurrency, 1, 'a code paid for in one currency whose clients paid in another is flagged');
eq((0, coachChannels_1.channelSum)([row({ spend: { cents: 1, currency: 'GBP' }, revenue: { cents: 1, currency: 'GBP' } })]).crossCurrency, 0, 'and a code whose two sides agree is not');
...
const STATUSES = ['loading', 'partial', 'error'];
for (const s of STATUSES) {
    eq(whyAgainst((0, coachChannels_1.spendAgainstReturn)(s, sum())), 'unread', `a ${s} read states no comparison`);
}
...
ok(!(0, coachChannels_1.spendAgainstReturn)('partial', sum()).statable, 'a truncated set of codes is an unknown set, not a smaller one');
eq(whyAgainst((0, coachChannels_1.spendAgainstReturn)('ready', sum())), 'statable', 'and a completed read with no holes does state one');
eq(whyAgainst((0, coachChannels_1.spendAgainstReturn)('ready', sum({ codes: 0, spent: [], earned: [] }))), 'no-channels', 'no named codes is its own answer, and not a comparison of nothing');
eq(whyAgainst((0, coachChannels_1.spendAgainstReturn)('ready', sum({ codes: 4, costUnknown: 3 }))), 'cost-unknown', 'three uncosted codes out of four withholds the figure');
eq(whyAgainst((0, coachChannels_1.spendAgainstReturn)('ready', sum({ codes: 4, costUnknown: 1 }))), 'cost-unknown', 'and so does one — a single hole is enough to move the difference');
eq(whyAgainst((0, coachChannels_1.spendAgainstReturn)('ready', sum({ codes: 2, paidUnknown: 1 }))), 'paid-unknown', 'a code whose revenue could not be totalled withholds the figure too');
eq(whyAgainst((0, coachChannels_1.spendAgainstReturn)('ready', sum({ codes: 2, crossCurrency: 1 }))), 'cross', 'and so does a code whose two sides are in different currencies');
const split = (0, coachChannels_1.spendAgainstReturn)('ready', sum({
    codes: 2,
...
eq(whyAgainst((0, coachChannels_1.spendAgainstReturn)('ready', sum({
    spent: [{ currency: 'AED', minorUnits: 90000, count: 1 }, { currency: 'GBP', minorUnits: 40000, count: 1 }],
...
eq(whyAgainst((0, coachChannels_1.spendAgainstReturn)('ready', sum())), 'statable', 'one currency on each side is comparable');
...
eq(whyAgainst((0, coachChannels_1.spendAgainstReturn)('ready', sum({ earned: [] }))), 'cross', 'a missing pot refuses rather than reading off nothing');
eq(whyAgainst((0, coachChannels_1.spendAgainstReturn)('ready', sum({ spent: [] }))), 'cross', 'from either side');
eq(whyAgainst((0, coachChannels_1.spendAgainstReturn)('ready', sum({
    spent: [{ currency: 'GBP', minorUnits: 40000, count: 1 }],
...
const good = (0, coachChannels_1.spendAgainstReturn)('ready', sum({ codes: 3 }));
...
const up = (0, coachChannels_1.againstLine)((0, coachChannels_1.spendAgainstReturn)('ready', sum({ codes: 3 })));
...
const down = (0, coachChannels_1.againstLine)((0, coachChannels_1.spendAgainstReturn)('ready', sum({
...
const level = (0, coachChannels_1.againstLine)((0, coachChannels_1.spendAgainstReturn)('ready', sum({
    spent: [{ currency: 'GBP', minorUnits: 40000, count: 1 }],
    earned: [{ currency: 'GBP', minorUnits: 40000, count: 1 }],
})));
...
const barelyUp = (0, coachChannels_1.againstLine)((0, coachChannels_1.spendAgainstReturn)('ready', sum({
    spent: [{ currency: 'GBP', minorUnits: 40000, count: 1 }],
...
const barelyDown = (0, coachChannels_1.againstLine)((0, coachChannels_1.spendAgainstReturn)('ready', sum({
...
ok(/1 code,/.test((0, coachChannels_1.againstLine)((0, coachChannels_1.spendAgainstReturn)('ready', sum({ codes: 1 })))), 'one code is singular');
eq((0, coachChannels_1.againstLine)({ statable: false, why: 'unread', note: 'the read failed.' }), 'the read failed.', 'a refusal renders its own reason rather than a sentence about money');
...
// missing data.
ok(/could not be written out/.test((0, coachChannels_1.againstLine)({ statable: true, currency: '', spentMinor: 1, earnedMinor: 2, codes: 1 })), 'an amount with no currency on it is withheld, not printed bare');
...
ok(/could not be written out/.test((0, coachChannels_1.againstLine)({ statable: true, currency: 'GBP', spentMinor: NaN, earnedMinor: 200, codes: 1 })), 'an unwritable spend withholds the whole sentence');
ok(/could not be written out/.test((0, coachChannels_1.againstLine)({ statable: true, currency: 'GBP', spentMinor: 200, earnedMinor: NaN, codes: 1 })), 'and so does an unwritable revenue, on its own');
/* ── an empty list means four different things ──────────────────────────── */
ok(/could not be read/.test((0, coachChannels_1.channelEmptyLine)('error')), 'a failed read says so');
ok(!/no named codes/.test((0, coachChannels_1.channelEmptyLine)('error')), 'and never that none of the coach’s codes worked');
ok(/more codes on record/.test((0, coachChannels_1.channelEmptyLine)('partial')), 'a truncated read says nothing here is a count');
eq((0, coachChannels_1.channelEmptyLine)('loading'), 'Still reading.', 'a read in flight claims nothing at all');
ok(/no named codes yet/.test((0, coachChannels_1.channelEmptyLine)('ready')), 'and only a completed read says the coach has none');
/* ── the headcount, which is a count and is gated like one ──────────────── */
for (const s of STATUSES) {
    eq((0, coachChannels_1.channelReachLine)(s, sum({ codes: 2, clients: 9, stayed: 7 })), '', `a ${s} read states no headcount`);
}
eq((0, coachChannels_1.channelReachLine)('ready', sum({ codes: 0, clients: 0, stayed: 0 })), '', 'and nor does a coach with no channels');
const none = (0, coachChannels_1.channelReachLine)('ready', sum({ codes: 2, clients: 0, stayed: 0 }));
...
const some = (0, coachChannels_1.channelReachLine)('ready', sum({ codes: 2, clients: 9, stayed: 7 }));
...
const all = (0, coachChannels_1.channelReachLine)('ready', sum({ codes: 1, clients: 4, stayed: 4 }));
...
const one = (0, coachChannels_1.channelReachLine)('ready', sum({ codes: 1, clients: 1, stayed: 0 }));
...
ok(/1 of them is still with you/.test((0, coachChannels_1.channelReachLine)('ready', sum({ codes: 1, clients: 3, stayed: 1 }))), 'and a single client who stayed takes a singular verb');
if (errors.length) {
