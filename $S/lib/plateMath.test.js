...
const plateMath_1 = require("./plateMath");
...
eqJson([...plateMath_1.PLATES.kg], [25, 20, 15, 10, 5, 2.5, 1.25], 'the metric rack is the IWF/IPF competition set');
eqJson([...plateMath_1.PLATES.lb], [45, 35, 25, 10, 5, 2.5, 1.25], 'the imperial rack is the American commercial set');
eqJson([...plateMath_1.BARS.kg], [20, 15], "the metric bars are the men's and women's Olympic bars");
eqJson([...plateMath_1.BARS.lb], [45, 35], 'the imperial bars are the American 45 and 35 lb bars');
...
for (const unit of UNITS) {
    for (const p of plateMath_1.PLATES[unit]) {
        ok(p > 0, `${p} ${unit} is not a plate`);
        ok(Number.isInteger(p * 4), `${p} ${unit} is not a denomination anyone sells — a converted figure`);
    }
}
for (const kgPlate of plateMath_1.PLATES.kg) {
    const converted = Math.round((0, units_1.kgToLb)(kgPlate) * 100) / 100;
    ok(!plateMath_1.PLATES.lb.includes(converted), `${converted} lb is ${kgPlate} kg converted, not a plate on a rack`);
}
ok(!plateMath_1.BARS.lb.includes(Math.round((0, units_1.kgToLb)(20) * 100) / 100), 'the 45 lb bar is its own bar, not the 20 kg bar converted to 44.09 lb');
for (const unit of UNITS) {
    for (let i = 1; i < plateMath_1.PLATES[unit].length; i++) {
        ok(plateMath_1.PLATES[unit][i] < plateMath_1.PLATES[unit][i - 1], `${unit} plates must run heaviest first for the greedy fill`);
    }
}
...
{
    const l = (0, plateMath_1.loadBar)(225, 45, 'lb');
...
eqJson((0, plateMath_1.loadBar)(135, 45, 'lb').plates, [45], '135 lb is one 45 a side');
eqJson((0, plateMath_1.loadBar)(315, 45, 'lb').plates, [45, 45, 45], '315 lb is three 45s a side');
eqJson((0, plateMath_1.loadBar)(185, 45, 'lb').plates, [45, 25], '185 lb is a 45 and a 25 a side');
eqJson((0, plateMath_1.loadBar)(95, 45, 'lb').plates, [25], '95 lb is a 25 a side');
eqJson((0, plateMath_1.loadBar)(155, 45, 'lb').plates, [45, 10], '155 lb is a 45 and a 10 a side');
eqJson((0, plateMath_1.loadBar)(65, 35, 'lb').plates, [10, 5], '65 lb on the 35 lb bar is a 10 and a 5 a side');
eqJson((0, plateMath_1.loadBar)(100, 20, 'kg').plates, [25, 15], '100 kg is a 25 and a 15 a side');
eqJson((0, plateMath_1.loadBar)(60, 20, 'kg').plates, [20], '60 kg is a 20 a side');
eqJson((0, plateMath_1.loadBar)(140, 20, 'kg').plates, [25, 25, 10], '140 kg is two 25s and a 10 a side');
eqJson((0, plateMath_1.loadBar)(60, 15, 'kg').plates, [20, 2.5], '60 kg on the 15 kg bar is a 20 and a 2.5 a side');
...
{
    const l = (0, plateMath_1.loadBar)(102.5, 20, 'kg');
...
{
    const l = (0, plateMath_1.loadBar)(47.5, 45, 'lb');
...
// ── nothing is invented ───────────────────────────────────────────────────
eq((0, plateMath_1.loadBar)(null, 20, 'kg'), null, 'no target is no answer');
eq((0, plateMath_1.loadBar)(undefined, 20, 'kg'), null, 'an absent target is no answer');
eq((0, plateMath_1.loadBar)(NaN, 20, 'kg'), null, 'text that did not parse is no answer');
// A target under the bar has no answer, and must not come back as an empty bar
// dressed up as the load that was asked for.
eq((0, plateMath_1.loadBar)(15, 20, 'kg'), null, '15 kg on a 20 kg bar cannot be loaded');
eq((0, plateMath_1.loadBar)(40, 45, 'lb'), null, '40 lb on a 45 lb bar cannot be loaded');
{
    const bare = (0, plateMath_1.loadBar)(20, 20, 'kg');
    eqJson(bare.plates, [], 'the bar alone carries no plates');
    eq(bare.exact, true, 'and it is exactly what was asked for');
}
eq((0, plateMath_1.loadBar)(1e9, 20, 'kg'), null, 'an absurd target is refused rather than filled one plate at a time');
eq((0, plateMath_1.loadBar)(100, 0, 'kg'), null, 'there is no bar weighing nothing');
...
for (const unit of UNITS) {
    const set = new Set(plateMath_1.PLATES[unit]);
    for (const bar of plateMath_1.BARS[unit]) {
        for (let q = 0; q <= 1600; q++) {
            const target = Math.round((bar + q * 0.25) * 100) / 100;
            const l = (0, plateMath_1.loadBar)(target, bar, unit);
            ok(l != null, `${target} ${unit} on a ${bar} ${unit} bar must have an answer`);
            if (!l)
                continue;
            for (const p of l.plates)
                ok(set.has(p), `${target} ${unit} used a ${p} ${unit} plate, which is not on the ${unit} rack`);
            ...
            ok(Math.round((bar + l.perSide * 2) * 100) / 100 === l.total, `${target} ${unit}: bar + both sides is not the ${l.total} claimed`);
...
for (const unit of UNITS) {
    const smallest = plateMath_1.PLATES[unit][plateMath_1.PLATES[unit].length - 1];
    for (const bar of plateMath_1.BARS[unit]) {
        for (let pairs = 0; pairs <= 400; pairs++) {
            const target = Math.round((bar + pairs * smallest * 2) * 100) / 100;
            const l = (0, plateMath_1.loadBar)(target, bar, unit);
            ok(l != null && l.exact, `${target} ${unit} is ${pairs} pairs of ${smallest} ${unit} on a ${bar} ${unit} bar and must load exactly`);
        }
    }
}
...
// again.
{
    const asKg = (0, plateMath_1.loadBar)(100, 20, 'kg');
    const asLb = (0, plateMath_1.loadBar)(100, 45, 'lb');
    ok(JSON.stringify(asKg.plates) !== JSON.stringify(asLb.plates), 'a target of 100 must not produce the same plates in both units');
}
if (errors.length) {
