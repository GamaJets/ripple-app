...
const W = (0, adherence_1.recentWindow)(NOW);
// ── the window ─────────────────────────────────────────────────────────────
ok(W.days === adherence_1.WINDOW_DAYS, `the window is ${adherence_1.WINDOW_DAYS} days, got ${W.days}`);
...
// to a month" cannot pass quietly.
{
    ...
    for (let i = 0; i < W.days; i++)
        counts[new Date(2026, 7, 2 + i).getDay()]++;
...
{
    const dst = (0, adherence_1.recentWindow)(new Date(2026, 2, 20, 12, 0, 0));
...
// ── THE ONE: an item added on Thursday is not judged on the days before it ──
{
    ...
    ok(!(0, adherence_1.setItemLine)(a).includes('28'), `a line added two days ago must never read as anything out of 28: "${(0, adherence_1.setItemLine)(a)}"`);
}
...
// ── a retired item: a count, never a rate ──────────────────────────────────
{
    ...
    ok(!/\d+ of \d+/.test((0, adherence_1.setItemLine)(a)), `a retired item must not read as a fraction: "${(0, adherence_1.setItemLine)(a)}"`);
}
...
// ── silence and skipping are separated, and never merged ───────────────────
{
    ...
    const line = (0, adherence_1.setItemLine)(a);
...
{
    const s = (0, adherence_1.summariseAdherence)({ window: W, items: [item({ id: 'a' })], ticks: [] });
    const a = find(s.set, 'a');
...
// ── derived items: a count, and never a rate ───────────────────────────────
{
    ...
    ok(!Object.prototype.hasOwnProperty.call(s.derived[0], 'eligibleDays'), 'a derived item carries no denominator: the coach cannot know which days it was on the list');
    ok(s.derived[0].label === 'Water', `known ids are named, got ${s.derived[0].label}`);
    ok(s.derived.some((d) => d.id === 'somethingNew' && d.label === 'somethingNew'), 'an id this module has never heard of is shown under its own id rather than dropped');
...
ok((0, adherence_1.dayOf)('2026-08-27') === '2026-08-27', 'a bare date is itself');
ok((0, adherence_1.dayOf)(new Date(2026, 7, 27, 23, 30).toISOString()) === '2026-08-27', 'a late-evening timestamp belongs to the day it was local evening on, not to tomorrow in UTC');
ok((0, adherence_1.dayOf)(null) === null && (0, adherence_1.dayOf)('nonsense') === null, 'an unreadable date is null, not today');
...
// test:zones runs and the words stay the runner's own.
{
    ...
    ok((0, adherence_1.dayLabel)('2026-08-29') === AUG29, `a window's end reads as its own date, got ${(0, adherence_1.dayLabel)('2026-08-29')}`);
    ok((0, adherence_1.dayLabel)('2026-08-01') === AUG1, `the first of the month reads as the first, got ${(0, adherence_1.dayLabel)('2026-08-01')}`);
    ok((0, adherence_1.dayLabel)('2026-08-01') !== JUL31, 'and never as the 31st of July, which is what UTC midnight would make it');
}
ok((0, adherence_1.dayLabel)('') === '—', 'and an unreadable one is a dash rather than an invented day');
if (errors.length) {
