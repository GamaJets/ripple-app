...
const coachStatement_1 = require("./coachStatement");
...
const at = (y, m, d, h = 12) => new Date(y, m - 1, d, h, 0, 0, 0).toISOString();
const Y26 = (0, coachStatement_1.calendarYear)(2026);
const packRow = (o = {}) => ({ amount_cents: 48000, currency: 'GBP', created_at: at(2026, 3, 4), ...o });
const invoice = (o = {}) => ({
...
const fee = (o = {}) => ({ amount: 25, currency: 'GBP', createdAt: at(2026, 6, 11), waivedAt: null, ...o });
...
const payout = (o = {}) => ({ amountCents: 42810, currency: 'GBP', status: 'paid', arrivalOn: '2026-07-09', ...o });
...
const refund = (o = {}) => ({ refundedCents: 12000, currency: 'GBP', refundedAt: at(2026, 8, 3), on: 'sale', ...o });
/** One chargeback, counted on the day the issuer raised it — the day the money
 *  left, whatever was decided afterwards. */
const dispute = (o = {}) => ({ amountCents: 9000, currency: 'GBP', status: 'needs_response', reason: 'fraudulent', openedAt: at(2026, 9, 14), closedAt: null, ...o });
...
const cost = (o = {}) => ({ description: 'Gym floor rent, October', category: 'rent', amountCents: 40000, currency: 'GBP', paidOn: '2026-10-01', ...o });
function input(over = {}) {
    return {
        period: Y26,
...
const sec = (s, key) => s.sections.find((x) => x.key === key);
...
const items = (o = {}) => ({ invoices: [], fees: [], refunds: [], disputes: [], costs: [], ...o });
...
eq((0, coachStatement_1.calendarYear)(2026).from, '2026-01-01', 'a year opens on 1 January');
eq((0, coachStatement_1.calendarYear)(2026).to, '2026-12-31', 'and closes on 31 December');
eq((0, coachStatement_1.calendarYear)(2026).label, '2026', 'and is labelled with the year alone');
eq((0, coachStatement_1.calendarQuarter)(2026, 1).from, '2026-01-01', 'Q1 opens the year');
eq((0, coachStatement_1.calendarQuarter)(2026, 1).to, '2026-03-31', 'and ends on the last day of March');
eq((0, coachStatement_1.calendarQuarter)(2026, 4).to, '2026-12-31', 'Q4 ends on the last day of December');
eq((0, coachStatement_1.calendarQuarter)(2026, 2).label, 'Q2 2026', 'a quarter is labelled by its number and year');
eq((0, coachStatement_1.calendarMonth)(2026, 2).to, '2026-02-28', 'February in a common year ends on the 28th');
eq((0, coachStatement_1.calendarMonth)(2024, 2).to, '2024-02-29', 'and on the 29th in a leap year');
eq((0, coachStatement_1.calendarMonth)(2026, 11).to, '2026-11-30', 'a thirty-day month ends on the 30th');
eq((0, coachStatement_1.calendarMonth)(2026, 1).label, 'Jan 2026', 'a month is labelled by its short name and year');
eq((0, coachStatement_1.calendarMonth)(2026, 3).from, '2026-03-01', 'a month opens on its first day, never on a day zero');
eq((0, coachStatement_1.calendarQuarter)(2026, 0).from, '2026-01-01', 'a quarter number below one is clamped to Q1 rather than building a negative month');
eq((0, coachStatement_1.calendarQuarter)(2026, 9).to, '2026-12-31', 'and one above four is clamped to Q4');
eq((0, coachStatement_1.calendarMonth)(2026, 0).from, '2026-01-01', 'a month number below one is clamped to January');
eq((0, coachStatement_1.calendarMonth)(2026, 13).to, '2026-12-31', 'and one above twelve to December');
// No label anywhere in this module offers a split year. A "2025/26" would be a
// jurisdiction chosen on the coach's behalf.
for (const p of [(0, coachStatement_1.calendarYear)(2026), (0, coachStatement_1.calendarQuarter)(2026, 3), (0, coachStatement_1.calendarMonth)(2026, 7)]) {
...
{
    const r = (0, coachStatement_1.periodRange)(Y26);
    ...
    eq((0, coachStatement_1.periodRange)({ from: 'not a date', to: '2026-12-31', label: 'x' }), null, 'an unreadable start is no range at all');
    eq((0, coachStatement_1.periodRange)({ from: '2026-12-31', to: '2026-01-01', label: 'x' }), null, 'and a period that ends before it starts is refused rather than inverted');
    ...
    eq((0, coachStatement_1.periodRange)({ from: '2026-01-01', to: '2025-12-31', label: 'x' }), null, 'and one that collapses to no length at all is refused too');
    ...
    const split = (0, coachStatement_1.splitByPeriod)(rows, (x) => x.created_at, r);
...
{
    const b = (0, coachStatement_1.periodBoundsIso)(Y26);
    ...
    eq((0, coachStatement_1.periodBoundsIso)({ from: 'x', to: 'y', label: 'x' }), null, 'and there are no bounds for a period that cannot be read');
}
...
eq((0, coachStatement_1.dayLabel)('2026-08-01'), (0, format_1.fmtPointDay)(2026, 7, 1), 'a date-only value reads as its own day, west of Greenwich included');
eq((0, coachStatement_1.dayLabel)('not a date'), '—', 'and an unreadable one is a dash');
eq((0, coachStatement_1.dayLabel)('2026-13-01'), '—', 'a month number past December is a dash, not an undefined month name');
eq((0, coachStatement_1.dayLabel)('2026-00-01'), '—', 'and so is a month number below January');
eq((0, coachStatement_1.periodSentence)(Y26), `${(0, format_1.fmtPointDay)(2026, 0, 1)} to ${(0, format_1.fmtPointDay)(2026, 11, 31)} inclusive`, 'the period is spelled out at both ends and says inclusive');
/* ── 3. an undated row is in NO period, and is counted ────────────────────
   ...
   see. */
{
    const s = (0, coachStatement_1.splitByPeriod)([packRow(), packRow({ created_at: 'nonsense' }), packRow({ created_at: at(2025, 6, 1) })], (r) => r.created_at, (0, coachStatement_1.periodRange)(Y26));
...
eq((0, coachStatement_1.splitByPeriod)([packRow()], (r) => r.created_at, null).inside.length, 0, 'with no readable period nothing is inside it — never everything');
...
{
    const s = (0, coachStatement_1.splitByDay)([invoice({ issuedOn: '2026-01-01' }), invoice({ issuedOn: '2026-12-31' }),
        invoice({ issuedOn: '2025-12-31' }), invoice({ issuedOn: '2027-01-01' }),
        invoice({ issuedOn: 'not a date' })], (r) => r.issuedOn, Y26);
...
}
{
    ...
    const s = (0, coachStatement_1.coachStatement)(input({
        invoices: {
            status: 'ready',
            rows: [
...
eq((0, coachStatement_1.minorToPlain)(48000, 'GBP'), '480.00', 'a two-decimal currency is divided by a hundred, exactly');
eq((0, coachStatement_1.minorToPlain)(5, 'GBP'), '0.05', 'and a sub-unit amount keeps its leading zero');
eq((0, coachStatement_1.minorToPlain)(0, 'GBP'), '0.00', 'a genuine zero is a zero');
eq((0, coachStatement_1.minorToPlain)(50000, 'JPY'), '50000', 'a zero-decimal currency is NOT divided');
eq((0, coachStatement_1.minorToPlain)(50000, 'jpy'), '50000', 'whatever case the code arrives in');
eq((0, coachStatement_1.minorToPlain)(-2500, 'GBP'), '-25.00', 'a negative keeps its sign');
eq((0, coachStatement_1.minorToPlain)(48000, null), null, 'an amount with no currency is not an amount of money');
eq((0, coachStatement_1.minorToPlain)(48000, '  '), null, 'nor is one whose currency is blank');
eq((0, coachStatement_1.minorToPlain)(null, 'GBP'), null, 'and a missing amount is null, never "0.00"');
eq((0, coachStatement_1.minorToPlain)(1.5, 'GBP'), null, 'a fractional minor unit is not a stored amount and is refused');
eq((0, coachStatement_1.majorToPlain)(25, 'GBP'), '25.00', 'a whole-unit fee is not divided by anything');
eq((0, coachStatement_1.majorToPlain)(25.5, 'GBP'), '25.50', 'and keeps its own decimals');
eq((0, coachStatement_1.majorToPlain)(5000, 'JPY'), '5000', 'a zero-decimal fee has no decimals at all');
eq((0, coachStatement_1.majorToPlain)(25, null), null, 'a fee with no currency states no amount');
eq((0, coachStatement_1.majorToPlain)(null, 'GBP'), null, 'and a missing fee amount is null, never zero');
/* ── 5. fees: waived rows out of the figure, and said out loud ────────────*/
{
    const t = (0, coachStatement_1.sumCharges)([fee(), fee({ amount: 40 }), fee({ waivedAt: '2026-06-12T00:00:00Z' }), fee({ currency: null }), fee({ amount: null })]);
...
{
    const t = (0, coachStatement_1.sumCharges)([fee(), fee({ currency: 'AED', amount: 100 })]);
...
}
{
    ...
    const t = (0, coachStatement_1.sumCharges)([fee({ currency: 'GBP', amount: 25 }), fee({ currency: 'AED', amount: 25 }), fee({ currency: 'USD', amount: 100 })]);
...
}
{
    ...
    const two = (0, coachStatement_1.coachStatement)(input({ lateCancellations: { status: 'ready', rows: [fee(), fee({ currency: 'AED', amount: 100 })] } }));
    ok(sec(two, 'lateCancellations').notes.some((n) => n.includes('deliberately not added together')), 'two currencies of fees carry the warning');
    const one = (0, coachStatement_1.coachStatement)(input({ lateCancellations: { status: 'ready', rows: [fee(), fee({ amount: 40 })] } }));
    ok(!sec(one, 'lateCancellations').notes.some((n) => n.includes('deliberately not added together')), 'and one currency does not, so the warning still means something when it appears');
}
...
const STATED = [
    coachStatement_1.STATEMENT_NOT, coachStatement_1.STATEMENT_IS, coachStatement_1.STATEMENT_NOT_THE_WHOLE_BOOK,
    coachStatement_1.STATEMENT_STRIPE_IS_THE_RECORD, coachStatement_1.PERIOD_IS_YOURS,
    coachStatement_1.SESSIONS_NOT_MONEY, coachStatement_1.INVOICES_NOT_ADDED, coachStatement_1.LATE_FEES_NOT_TAKINGS,
    ...
    coachStatement_1.REFUNDS_ARE_NOT_NETTED, coachStatement_1.REFUNDS_ARE_A_RUNNING_TOTAL, coachStatement_1.DISPUTES_ARE_NOT_NETTED,
    coachCosts_1.COSTS_ARE_NEVER_NETTED, coachCosts_1.COSTS_ARE_NOT_TAX_ADVICE,
];
...
function scannable(text) {
    ...
    for (const stated of STATED) {
        prose = prose.split(stated.toLowerCase()).join(' ').split((0, coachInvoice_1.escapeHtml)(stated).toLowerCase()).join(' ');
    }
    return prose;
}
const FORBIDDEN = [
...
const forbiddenRe = (f) => new RegExp(`\\b${f.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`);
for (const shape of [
    input(),
    ...
    const s = (0, coachStatement_1.coachStatement)(shape);
    const d = (0, coachStatement_1.statementDoc)(s);
    const prose = scannable(d.html + '\n' + d.text + '\n' + (0, coachStatement_1.statementCsv)(s) + '\n' + (0, coachStatement_1.statementItemsCsv)(s, { invoices: shape.invoices.rows, fees: shape.lateCancellations.rows, refunds: shape.refunds.rows, disputes: shape.disputes.rows, costs: shape.costs.rows }));
...
/* ── 7. and it says outright what it is not ───────────────────────────────*/
{
    const s = (0, coachStatement_1.coachStatement)(input());
    const d = (0, coachStatement_1.statementDoc)(s);
    ok(d.text.includes(coachStatement_1.STATEMENT_NOT), 'the text says outright that it is not a tax document');
    ok(d.html.includes((0, coachInvoice_1.escapeHtml)(coachStatement_1.STATEMENT_NOT).slice(0, 60)), 'and so does the HTML');
    ok(d.text.includes(coachStatement_1.STATEMENT_NOT_THE_WHOLE_BOOK), 'it says money paid outside this app is not on it');
    ok(d.text.includes(coachStatement_1.STATEMENT_STRIPE_IS_THE_RECORD), 'and that Stripe holds the record of what moved');
    ok(d.text.includes(coachStatement_1.PERIOD_IS_YOURS), 'and that the period was the coach’s own choice');
    ok((0, coachStatement_1.statementCsv)(s).includes(coachStatement_1.STATEMENT_NOT), 'the CSV carries the same sentence on its own face');
    ok((0, coachStatement_1.statementItemsCsv)(s, items({ invoices: [invoice()], fees: [fee()] })).includes(coachStatement_1.STATEMENT_NOT), 'and so does the line-item file');
    ok(d.text.includes((0, coachStatement_1.periodSentence)(Y26)), 'and the exact period is on it, both ends');
    ok((0, coachStatement_1.statementCsv)(s).includes((0, coachStatement_1.periodSentence)(Y26)), 'and on the CSV too');
}
{
    ...
    const d = (0, coachStatement_1.statementDoc)(s);
    ok(d.text.includes(coachStatement_1.STATEMENT_NOT), 'an empty statement with an unreadable name still carries the whole disclaimer');
    ok(d.text.includes(coachStatement_1.STATEMENT_NOT_THE_WHOLE_BOOK), 'and still says what is missing from it');
   ...
   they took nothing, over a read that was refused. */
{
    ...
    ok((sec(s, 'packs').withheld ?? '').includes('does NOT mean there were none'), 'and says in words that the empty section is not a statement about the record');
    eq(sec(s, 'subscriptions').count, null, 'a truncated read reports no count either');
    ok((sec(s, 'subscriptions').withheld ?? '').includes('not all of it'), 'and says the rows shown are real but not all of them');
    ok((sec(s, 'invoices').withheld ?? '').includes('had not finished loading'), 'a read still in flight is a read that did not answer, and says which');
    const d = (0, coachStatement_1.statementDoc)(s);
    ...
    ok((0, coachStatement_1.statementFileStem)(s).endsWith('-INCOMPLETE'), 'the filename carries the warning to whoever opens it later');
    ok((0, coachStatement_1.statementShareBlurb)(s).includes('BEFORE YOU SEND IT'), 'and the share sheet says so before it leaves the phone');
}
eq((0, coachStatement_1.withheldReason)('ready', 'sales'), null, 'a whole read withholds nothing');
ok(((0, coachStatement_1.withheldReason)('error', 'sales') ?? '').includes('could not be read'), 'a failed one says so');
ok(((0, coachStatement_1.withheldReason)('partial', 'sales') ?? '').includes('not all of it'), 'a truncated one says the rows shown are not all of them');
ok(((0, coachStatement_1.withheldReason)('loading', 'sales') ?? '').includes('had not finished loading'), 'and a read still in flight says that, and not that it failed');
ok(!((0, coachStatement_1.withheldReason)('error', 'sales') ?? '').includes('had not finished loading'), 'the four are four different sentences and are not interchangeable');
/* ── 8b. a section with nothing to admit to says nothing extra ────────────
   ...
   real ones with it. */
{
    const clean = (0, coachStatement_1.coachStatement)(input({ packs: { status: 'ready', rows: [packRow()] } }));
    eq(sec(clean, 'packs').notes.length, 0, 'one currency, nothing missing, nothing said');
}
{
    const one = (0, coachStatement_1.coachStatement)(input({ packs: { status: 'ready', rows: [packRow(), packRow({ amount_cents: null })] } }));
...
{
    const s = (0, coachStatement_1.coachStatement)(input({ subscriptions: { status: 'partial', rows: [packRow()] } }));
    eq(s.salesTotal, null, 'a truncated half means NO combined figure');
    ok((s.salesWithheld ?? '').includes('wrong rather than small'), 'and the reason names what a partial sum actually is');
}
{
    ...
    ok(sec(s, 'packs').notes.some((n) => n.includes('deliberately not added together')), 'and the document says they are not added');
    ok(!(0, coachStatement_1.statementDoc)(s).text.includes('1080.00'), 'no bare sum across currencies appears anywhere');
}
/* ── 10. an amount with no currency is a hole, and the size of it is said ──*/
{
    const s = (0, coachStatement_1.coachStatement)(input({ packs: { status: 'ready', rows: [packRow(), packRow({ currency: null })] } }));
    ...
    ok(sec(s, 'packs').notes.some((n) => n.includes('Do not read the shorter total as the whole of it')), 'with the instruction not to read the short total as the whole');
}
{
    // The empty-cell rule in the file an accountant opens.
    const csv = (0, coachStatement_1.statementItemsCsv)((0, coachStatement_1.coachStatement)(input()), items({ invoices: [invoice({ currency: null, amountCents: 48000 })] }));
...
/* ── 11. sessions are counted and never priced ────────────────────────────*/
{
    ...
    ok(sec(s, 'sessions').notes.some((n) => n === coachStatement_1.SESSIONS_NOT_MONEY), 'the reason is printed rather than left to a comment');
    ...
    // their clients.
    ok(sec(s, 'sessions').notes.some((n) => n === 'Marked completed: 1. No-show: 1. Late-cancelled: 0. Cancelled: 0.'), 'the four outcomes are counted separately and each one is its own number');
    ok(sec(s, 'sessions').notes.some((n) => n.includes('1 session in this period has no outcome recorded')), 'an unmarked session is named, not folded into a category');
}
{
    const one = (0, coachStatement_1.coachStatement)(input({ sessions: { status: 'ready', rows: [{ startsAt: at(2026, 6, 10), outcome: 'completed' }] } }));
...
{
    const s = (0, coachStatement_1.coachStatement)(input());
    ok(sec(s, 'invoices').notes.some((n) => n === coachStatement_1.INVOICES_NOT_ADDED), 'the statement says why invoices stand apart');
    ok(!s.salesTotal.lines.some((l) => l.amount === 'GBP 1,560.00'), 'and the invoice is not in the combined figure');
    ok(sec(s, 'lateCancellations').notes.some((n) => n === coachStatement_1.LATE_FEES_NOT_TAKINGS), 'and so do the fees');
}
{
    const s = (0, coachStatement_1.coachStatement)(input({ invoices: { status: 'ready', rows: [invoice(), invoice({ seq: 2, voidedAt: '2026-05-03T00:00:00Z' })] } }));
...
{
    const p = (0, coachStatement_1.payoutFacts)({ status: 'ready', hasAccount: true, chargesEnabled: true, detailsSubmitted: true });
    ...
    ok(!/\bnext payout will (arrive|be sent) on\b|\barriv(es|ing) on\b|\bevery (monday|week|month)\b|\bin \d+ days\b/.test(all), 'and no date, cadence or arrival is stated anywhere');
    ...
    ok(all.includes('nothing here subtracts one from the other'), 'and the statement says it never nets a payout against the charges above it');
    ...
    ok((0, coachStatement_1.payoutFacts)({ status: 'ready', hasAccount: false, chargesEnabled: false, detailsSubmitted: false })
        .lines[0].includes('not connected a payout account'), 'a coach with no account is told that plainly');
    ok((0, coachStatement_1.payoutFacts)({ status: 'error', hasAccount: false, chargesEnabled: false, detailsSubmitted: false })
   ...
   document carried only the first. What must NOT happen is the subtraction. */
{
    ...
    ok(p.notes.includes(coachStatement_1.PAYOUTS_ARE_NOT_NETTED), 'the section says a payout is not the proceeds of a sale');
    ok(p.notes.includes(coachStatement_1.PAYOUTS_ONLY_ARRIVED), 'and that only arrived payouts are in the figure');
    ...
    const doc = (0, coachStatement_1.statementDoc)(s);
    ...
    // different transactions over different spans.
    ok(!doc.text.includes('651.90') && !doc.html.includes('651.90'), 'and neither nets the payout against the sales above it');
    ok(doc.text.includes(coachStatement_1.PAYOUTS_ARE_NOT_NETTED), 'the document says so on its own face');
    ...
    const mixed = (0, coachStatement_1.coachStatement)(input({
        payoutsPaid: {
            status: 'ready',
            rows: [
                payout(),
    ...
    ok(pm.notes.some((n) => n.includes('3 payouts dated in this period have a status other than paid')), 'the other three are counted and named rather than dropped or added');
    // An undated payout is in no period at all, including this one.
    const undated = (0, coachStatement_1.coachStatement)(input({
    ...
    const failed = (0, coachStatement_1.coachStatement)(input({ payoutsPaid: { status: 'error', rows: [] } }));
    ...
    ok(failed.caveats.some((c) => c.includes('Payouts that reached your bank')), 'and it reaches the caveat list, so the file names itself INCOMPLETE');
    ok((0, coachStatement_1.statementFileStem)(failed).includes('INCOMPLETE'), 'which it does');
    ...
    const edge = (0, coachStatement_1.coachStatement)(input({
...
}
{
    ...
    const csv = (0, coachStatement_1.statementItemsCsv)((0, coachStatement_1.coachStatement)(input()), items({ invoices: [invoice({ billTo: 'Smith, Jr.', description: 'paid cash; owes 20' })] }));
...
{
    const whole = (0, coachStatement_1.statementCsv)((0, coachStatement_1.coachStatement)(input()));
    ...
    const short = (0, coachStatement_1.statementCsv)((0, coachStatement_1.coachStatement)(input({ packs: { status: 'error', rows: [] } })));
...
{
    const c = (0, coachStatement_1.statementCaveats)(input({ lateCancellations: { status: 'error', rows: [] } }));
    eq(c.length, 1, 'one failed read, one caveat');
    ok(c[0].includes('Late-cancellation fees'), 'the caveat names the part');
    ok(c[0].includes('MISSING from this statement rather than absent from your record'), 'and says which of the two an empty section means');
    eq((0, coachStatement_1.statementCaveats)(input()).length, 0, 'a whole set of reads leaves nothing to say');
    eq((0, coachStatement_1.coachStatement)(input()).complete, true, 'and the statement is complete');
    ...
    ok((0, coachStatement_1.statementCaveats)(input({ packs: { status: 'partial', rows: [] } }))[0].includes('more rows than one request returns'), 'a truncated read is named as truncated');
    ok((0, coachStatement_1.statementCaveats)(input({ packs: { status: 'loading', rows: [] } }))[0].includes('had not finished loading'), 'a read still in flight is named as that');
    ok((0, coachStatement_1.statementCaveats)(input({ packs: { status: 'error', rows: [] } }))[0].includes('could not be read'), 'and a refused one as refused');
}
...
{
    const s = (0, coachStatement_1.coachStatement)(input());
    ok(sec(s, 'lateCancellations').notes.some((n) => n === coachStatement_1.LATE_FEES_ONLY_CURRENT_CLIENTS), 'the fees section says which fees it cannot see');
    ok((0, coachStatement_1.statementDoc)(s).text.includes(coachStatement_1.LATE_FEES_ONLY_CURRENT_CLIENTS), 'and the document carries it');
    ok((0, coachStatement_1.statementItemsCsv)(s, items()).includes(coachStatement_1.LATE_FEES_ONLY_CURRENT_CLIENTS), 'and so does the line-item file');
}
...
{
    const s = (0, coachStatement_1.coachStatement)(input());
    const csv = (0, coachStatement_1.statementItemsCsv)(s, items({
...
{
    ok((0, coachStatement_1.payoutFacts)({ status: 'ready', hasAccount: true, chargesEnabled: false, detailsSubmitted: true })
        .lines[0].includes('has not finished verifying'), 'a submitted but unverified account says so');
    ok((0, coachStatement_1.payoutFacts)({ status: 'ready', hasAccount: true, chargesEnabled: false, detailsSubmitted: false })
        .lines[0].includes('setup was never finished'), 'an abandoned setup says that instead');
    ok((0, coachStatement_1.payoutFacts)({ status: 'ready', hasAccount: true, chargesEnabled: true, detailsSubmitted: true })
...
{
    const s = (0, coachStatement_1.coachStatement)(input());
    eq((0, coachStatement_1.statementFileStem)(s), 'statement-of-record-ironhaus-strength-2026-01-01-to-2026-12-31', 'the filename carries the brand and both ends of the period');
    ok(!(0, coachStatement_1.statementFileStem)(s).includes('INCOMPLETE'), 'and says nothing about incompleteness when there is none');
}
/* ── 18. THE THOUSANDTH ───────────────────────────────────────────────────
...
{
    eq((0, coachStatement_1.minorToPlain)(12340, 'KWD'), '12.340', 'a Kuwaiti dinar has three decimal places, not two');
    eq((0, coachStatement_1.minorToPlain)(12340, 'kwd'), '12.340', 'and the code is read case-insensitively, as everywhere else');
    eq((0, coachStatement_1.minorToPlain)(5, 'BHD'), '0.005', 'five fils keeps its leading nought rather than starting with a point');
    eq((0, coachStatement_1.minorToPlain)(-12340, 'JOD'), '-12.340', 'and a negative one keeps its sign in front of the whole figure');
    eq((0, coachStatement_1.minorToPlain)(50000, 'JPY'), '50000', 'a yen still has none, which is what this function already got right');
    eq((0, coachStatement_1.minorToPlain)(48000, 'GBP'), '480.00', 'and two places is still the answer for most of the world');
    eq((0, coachStatement_1.minorToPlain)(4800, null), null, 'no currency is still no figure, never a bare number');
    ...
    eq((0, coachStatement_1.majorToPlain)(12.345, 'KWD'), '12.345', 'a whole-unit dinar keeps all three of its places');
    eq((0, coachStatement_1.majorToPlain)(25, 'GBP'), '25.00', 'and a pound still gets two');
    eq((0, coachStatement_1.majorToPlain)(50000, 'JPY'), '50000', 'and a yen still gets none');
    // And the file itself carries the right digits, which is the thing that
    // actually reaches an accountant.
    const kw = (0, coachStatement_1.coachStatement)(input({ period: Y26 }));
    const csv = (0, coachStatement_1.statementItemsCsv)(kw, items({ invoices: [invoice({ amountCents: 12340, currency: 'KWD' })] }));
...
{
    const broken = { from: 'the first', to: 'the last', label: 'nonsense' };
    eq((0, coachStatement_1.periodReads)(broken), false, 'a period whose ends are not days does not read');
    eq((0, coachStatement_1.periodReads)(Y26), true, 'and a real one does');
    eq((0, coachStatement_1.periodReads)({ from: '2026-12-31', to: '2026-01-01', label: 'backwards' }), false, 'nor does one that closes before it opens');
    // Nothing vanishes between the two counts.
    const split = (0, coachStatement_1.splitByPeriod)([{ at: at(2026, 3, 4) }, { at: 'not a date' }], (r) => r.at, null);
    ...
    const day = (0, coachStatement_1.splitByDay)([{ on: '2026-05-02' }], (r) => r.on, broken);
    ...
    const s = (0, coachStatement_1.coachStatement)(input({ period: broken }));
    ok(!s.complete, 'a statement over an unreadable period is not complete');
    ok(s.caveats.some((c) => c.includes(coachStatement_1.PERIOD_UNREADABLE)), 'and names the period as the thing that failed');
    for (const key of ['sessions', 'packs', 'subscriptions', 'receipts', 'invoices', 'lateCancellations', 'payoutsPaid', 'refunds', 'disputes', 'costs']) {
        ...
        eq(sec(s, key).withheld, coachStatement_1.PERIOD_UNREADABLE, `and ${key} says which of the two went wrong`);
    ...
    const doc = (0, coachStatement_1.statementDoc)(s);
    ok(doc.text.includes(coachStatement_1.PERIOD_UNREADABLE), 'the document carries the reason where it cannot be scrolled past');
    ...
    const csv = (0, coachStatement_1.statementItemsCsv)(s, items({ invoices: [invoice()], costs: [cost()] }));
    ok(csv.includes(coachStatement_1.PERIOD_UNREADABLE), 'the line-item file says why it lists nothing');
   ...
   sections. */
{
    ...
    ok(sec(s, 'refunds').notes.includes(coachStatement_1.REFUNDS_ARE_NOT_NETTED), 'and says it is not subtracted from the sales');
    ok(sec(s, 'refunds').notes.includes(coachStatement_1.REFUNDS_ARE_A_RUNNING_TOTAL), 'and that one line is every refund on one charge');
    eq(sec(s, 'disputes').count, 1, 'a chargeback raised inside the period is counted');
    ok(sec(s, 'disputes').notes.includes(coachStatement_1.DISPUTES_ARE_NOT_NETTED), 'and says it is not subtracted either');
    ...
    ok(sec(s, 'costs').notes.includes(coachCosts_1.COSTS_ARE_NEVER_NETTED), 'and carries the standing rule from the costs screen itself');
    ...
    const out = (0, coachStatement_1.coachStatement)(input({
    ...
    for (const key of ['refunds', 'disputes', 'costs']) {
        const bad = (0, coachStatement_1.coachStatement)(input({ [key]: { status: 'error', rows: [] } }));
    ...
    const two = (0, coachStatement_1.coachStatement)(input({
    ...
    ok(sec(two, 'refunds').notes.some((n) => n.includes('deliberately not added together')), 'and carry the warning that they are not added');
}
   ...
   sale on the same document untrue. */
{
    ...
    ok(csv.includes('Gym floor rent, October'.replace(',', ',')) || csv.includes('"Gym floor rent, October"'), 'a description with a comma in it is quoted rather than becoming two columns');
