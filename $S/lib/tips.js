...
exports.EMPTY_TIP_STATE = void 0;
exports.tipsFor = tipsFor;
exports.isDue = isDue;
exports.nextTip = nextTip;
exports.markShown = markShown;
exports.tipToShow = tipToShow;
const guide_1 = require("./guide");
exports.EMPTY_TIP_STATE = { seen: [], lastShownAt: null };
/** Every tip available to a build, derived from its own guide. */
function tipsFor(v) {
    const out = [];
    for (const section of (0, guide_1.guideFor)(v)) {
        section.points.forEach((text, i) => {
 ...
 * stops existing.
 */
function isDue(state, now = Date.now(), minHours = 20) {
    if (!state.lastShownAt)
        return true;
    const t = Date.parse(state.lastShownAt);
    if (!Number.isFinite(t))
        return true;
    return now - t >= minHours * 3600000;
}
/**
 ...
 */
function markShown(state, tip, total, nowIso) {
 ...
 */
function tipToShow(v, state, now = Date.now(), minHours = 20) {
    if (!isDue(state, now, minHours))
        return null;
