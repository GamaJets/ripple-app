...
exports.PROMO_WITHDRAW_IS_FORWARD_ONLY = exports.PROMO_LIVES_AT_STRIPE = exports.PROMO_IS_TYPED_AT_CHECKOUT = exports.PROMO_IS_A_PERCENTAGE = exports.MAX_PERCENT_OFF = exports.CODE_IS_FOR_ANOTHER_PACKAGE = exports.CODE_CANNOT_COME_OFF_THIS_ONE = void 0;
exports.exactPercentOff = exactPercentOff;
exports.oneOffDiscount = oneOffDiscount;
exports.normaliseCode = normaliseCode;
exports.promoBlocker = promoBlocker;
exports.wholePercentsFor = wholePercentsFor;
exports.checkoutCodeBlocker = checkoutCodeBlocker;
exports.codeAppliesTo = codeAppliesTo;
exports.promoState = promoState;
exports.promoStateLabel = promoStateLabel;
exports.promoUseLine = promoUseLine;
 ...
 * integer.
 */
function exactPercentOff(priceCents, percentOff) {
 ...
 * can act on first.
 */
function oneOffDiscount(priceCents, currency, coupon, restrictions) {
    const cur = String(currency ?? '').trim().toLowerCase();
    if (!cur)
        return { ok: false, why: 'the package has no currency, so there is no money for a discount to be in' };
    if (!Number.isInteger(priceCents) || priceCents <= 0) {
        return { ok: false, why: `the package price ${priceCents} is not a whole number of minor units above nought` };
    }
    if (!coupon.valid)
        return { ok: false, why: 'Stripe reports the coupon behind that code as no longer valid' };
    ...
    if (hasAmt) {
        ...
        if (!offCur)
            return { ok: false, why: 'the coupon takes an amount off and names no currency for it' };
        if (offCur !== cur) {
 ...
 */
exports.CODE_CANNOT_COME_OFF_THIS_ONE = 'That code does not come off this one. Nothing has been charged — buy it at the price shown, or check with your coach which of the things they sell it is for.';
/* ── the code itself ──────────────────────────────────────────────────────── */
/**
 ...
 */
function promoBlocker(code, percentOff, target) {
    const out = [];
    ...
    if (!Number.isFinite(percentOff) || !Number.isInteger(percentOff) || percentOff < 1) {
    ...
    else if (percentOff > exports.MAX_PERCENT_OFF) {
        ...
        out.push(`The most this can take off is ${exports.MAX_PERCENT_OFF}%. A bigger discount than that leaves too little on the charge for the payment to go through at all, and your client would meet a checkout that simply fails.`);
    }
    ...
    if (!target) {
        ...
        else if (Number.isInteger(percentOff) && percentOff >= 1 && percentOff <= exports.MAX_PERCENT_OFF && exactPercentOff(target.priceCents, percentOff) == null) {
 ...
 */
function wholePercentsFor(priceCents) {
    const works = [];
    for (let p = 1; p <= exports.MAX_PERCENT_OFF; p += 1) {
        if (exactPercentOff(priceCents, p) != null)
            works.push(p);
    }
    if (!works.length) {
    ...
    if (works.length === exports.MAX_PERCENT_OFF) {
 ...
 */
function checkoutCodeBlocker(typed, target) {
    const c = normaliseCode(typed);
    if (!c)
        return 'Type the code your coach gave you. Letters and numbers only.';
    if (c.length < 3)
        return 'That code is too short. Check what your coach gave you.';
    if (!target)
        return 'That package could not be read, so a code cannot be checked against it.';
    return null;
}
/**
 ...
 */
function codeAppliesTo(codePackageId, packageId) {
...
exports.CODE_IS_FOR_ANOTHER_PACKAGE = 'That code is for something else your coach sells, so it does not come off this one. Check with them which it is for.';
/**
...
function promoState(p, today) {
    ...
    if (/^\d{4}-\d{2}-\d{2}$/.test(exp) && /^\d{4}-\d{2}-\d{2}$/.test(today) && today > exp)
        return 'expired';
    return 'live';
}
/** The state's own label. Title Case: it heads a value on a row. */
function promoStateLabel(state) {
    if (state === 'live')
        return 'Live';
    if (state === 'expired')
        return 'Expired';
 ...
 */
exports.PROMO_IS_A_PERCENTAGE = 'A code takes a percentage off, never a fixed amount. Repple is white-labelled, so a fixed amount would need a currency and would do nothing at all for a client paying in a different one — a percentage is the same offer whatever you charge in.';
/**
 ...
 * "does not work".
 */
exports.PROMO_IS_TYPED_AT_CHECKOUT = 'Your client types the code in the app, on the package itself, before the payment page opens. Give them the code and tell them to tap Have A Code when they subscribe.';
/**
 ...
 */
exports.PROMO_LIVES_AT_STRIPE = 'These live in your own Stripe account rather than in Repple, which is why the number of times each has been used is exact — there is one copy of it and Stripe keeps it. Withdrawing one here archives it at Stripe.';
/**
 ...
 */
exports.PROMO_WITHDRAW_IS_FORWARD_ONLY = 'Withdrawing a code stops anybody new using it. It does not change what somebody already subscribed on it pays — they keep the price they signed up at until their subscription ends, which is what they agreed to.';
