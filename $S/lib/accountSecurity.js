...
exports.MIN_PASSWORD = void 0;
exports.passwordProblem = passwordProblem;
exports.looksLikeEmail = looksLikeEmail;
exports.emailProblem = emailProblem;
exports.classifyEmailChange = classifyEmailChange;
exports.pendingEmail = pendingEmail;
exports.authErrorNote = authErrorNote;
exports.changePassword = changePassword;
exports.endOtherSessions = endOtherSessions;
 ...
 */
function passwordProblem(next, confirm, current) {
    if (!current)
        return 'Enter your current password.';
    if (!next)
        return 'Enter a new password.';
    if (next.length < exports.MIN_PASSWORD)
        return `A new password needs at least ${exports.MIN_PASSWORD} characters.`;
    ...
    if (!next.trim())
        return 'A password of only spaces is not a password.';
    if (next === current)
...
function emailProblem(next, current) {
    const s = next.trim();
    if (!s)
        return 'Enter the new email address.';
    if (!looksLikeEmail(s))
        return "That does not look like an email address.";
    if (s.toLowerCase() === (current || '').trim().toLowerCase())
        return 'That is the address already on your account.';
    return null;
}
...
function classifyEmailChange(user, requested) {
    if (!user)
        return 'unknown';
    if (sameAddress(user.email, requested))
        return 'changed';
    if (sameAddress(user.new_email, requested))
        return 'pending';
    return 'unknown';
}
/**
 ...
 */
function authErrorNote(message, what) {
    const m = (message || '').toLowerCase();
    if (!m)
        return what === 'password' ? 'Your password was not changed.' : 'Your email address was not changed.';
 ...
 */
async function changePassword(auth, email, current, next) {
    try {
        ...
        if (error)
            return { ok: false, field: 'new', note: authErrorNote(error.message, 'password') };
        return { ok: true };
    }
    catch (e) {
        ...
        return { ok: false, field: 'other', note: `Your password was not changed. (${e.message})` };
    }
}
/**
 ...
 */
async function endOtherSessions(auth) {
    ...
    try {
        ...
        if (error)
            return { ok: false, note: `Your other sessions were not ended. (${error.message})` };
        return { ok: true };
    }
    catch (e) {
        return { ok: false, note: `Your other sessions were not ended. (${e.message})` };
    }
}
/**
 ...
 */
async function changeEmail(auth, next) {
    const requested = next.trim();
    try {
        const { error } = await auth.updateUser({ email: requested });
        if (error)
            return { ok: false, note: authErrorNote(error.message, 'email') };
        ...
        if (readErr)
            return { ok: true, outcome: 'unknown', requested };
        return { ok: true, outcome: classifyEmailChange(data?.user ?? null, requested), requested };
    }
    catch (e) {
        return { ok: false, note: `Your email address was not changed. (${e.message})` };
    }
}
