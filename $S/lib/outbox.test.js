...
/* ── ids and keys ──────────────────────────────────────────────────────── */
{
    const a = (0, outbox_1.newItem)('message', { body: 'hi' });
    const b = (0, outbox_1.newItem)('message', { body: 'hi' });
    ok(a.id !== b.id, 'two intents made in the same millisecond are still two intents');
    ok((0, wellnessSync_1.isPending)(a.id), 'an outbox id is a local id: nothing may mistake it for a server key');
    eq(a.tries, 0, 'a new intent has not been tried');
    eq(a.expiresAt, null, 'and does not expire unless somebody says so');
    ok((0, outbox_1.outboxKey)(UID).includes(UID), 'the outbox is per account, so two people sharing a phone do not inherit each other');
}
/* ── 1 · reading the device ────────────────────────────────────────────── */
{
    eq((0, outbox_1.readOutbox)(null).read, true, 'a key that was never written is a real empty queue');
    eq((0, outbox_1.readOutbox)(null).items.length, 0, 'and it holds nothing');
    eq((0, outbox_1.readOutbox)('{ not json').read, false, 'bytes nobody could parse are NOT an empty queue');
    eq((0, outbox_1.readOutbox)('{"kind":"message"}').read, false, 'and neither is something that is not a list');
    const good = (0, outbox_1.newItem)('measurement', { waist: 80 });
    const raw = JSON.stringify([good, { id: 'x', at: '2026-01-01T00:00:00.000Z', kind: 'not-a-kind' }, { kind: 'message' }]);
    const back = (0, outbox_1.readOutbox)(raw);
    ...
    eq(back.items[0].payload.waist, 80, 'and its payload');
    ok((0, outbox_1.isOutboxKind)('message'), 'the union is what the handler registry is keyed on');
    ok(!(0, outbox_1.isOutboxKind)('booking'), 'and a seat somebody else can take is not in it');
    ok(!(0, outbox_1.isOutboxKind)('injury'), 'nor a profile write, which src/ui/clientData.tsx re-sends from state');
}
{
    // Round trip: what is written is what comes back, including tries.
    const one = { ...(0, outbox_1.newItem)('pt-approval', { id: 's9' }), tries: 3 };
    const back = (0, outbox_1.readOutbox)(JSON.stringify([one]));
...
}
{
    ...
    const good = (0, outbox_1.newItem)('goal', { kind: 'weight' });
    const withHoles = (0, outbox_1.readOutbox)(`[null, ${JSON.stringify(good)}, 7, "text"]`);
...
}
{
    ...
    const row = (o) => (0, outbox_1.readOutbox)(JSON.stringify([{ id: 'i1', kind: 'message', at, payload: null, ...o }])).items[0];
    eq(row({ tries: 'seven' }).tries, 0, 'a tries count that is not a number reads as none tried — a phone that has never sent must not report an attempt it never made');
    ...
    eq(row({ expiresAt: 1759000000 }).expiresAt, null, 'and an expiry of the wrong shape is no expiry rather than a value nothing can parse');
    eq(row({}).expiresAt, null, 'an intent with no expiry keeps none');
}
/* ── 2 · the cap refuses rather than evicts ────────────────────────────── */
{
    let list = [];
    const first = (0, outbox_1.newItem)('measurement', { waist: 1 });
    list = (0, outbox_1.addItem)(list, first).list;
    for (let i = 1; i < outbox_1.OUTBOX_CAP; i++)
        list = (0, outbox_1.addItem)(list, (0, outbox_1.newItem)('measurement', { waist: i + 1 })).list;
    eq(list.length, outbox_1.OUTBOX_CAP, 'the cap is reached exactly');
    const res = (0, outbox_1.addItem)(list, (0, outbox_1.newItem)('measurement', { waist: 999 }));
    eq(res.added, false, 'and the next one is refused, not swallowed');
    eq(res.list.length, outbox_1.OUTBOX_CAP, 'the list is unchanged');
    ok(res.list[0].id === first.id, 'and specifically the OLDEST intent was not evicted to make room');
}
/* ── drop and bump ─────────────────────────────────────────────────────── */
{
    const a = (0, outbox_1.newItem)('message', { body: 'one' });
    const b = (0, outbox_1.newItem)('message', { body: 'two' });
    const list = [a, b];
    eq((0, outbox_1.dropItem)(list, a.id).length, 1, 'a stored intent comes out');
    eq((0, outbox_1.dropItem)(list, a.id)[0].id, b.id, 'and takes nothing with it');
    eq((0, outbox_1.dropItem)(list, 'nobody').length, 2, 'dropping an id that is not there changes nothing');
    const bumped = (0, outbox_1.bumpTry)(list, b.id);
...
/* ── 3 · lapsing ───────────────────────────────────────────────────────── */
{
    ...
    const live = (0, outbox_1.newItem)('measurement', { waist: 80 });
    const gone = (0, outbox_1.newItem)('pt-approval', { id: 's1' }, { expiresAt: '2026-09-02T11:59:59.000Z' });
    const stillGood = (0, outbox_1.newItem)('pt-approval', { id: 's2' }, { expiresAt: '2026-09-03T00:00:00.000Z' });
    const junk = (0, outbox_1.newItem)('pt-approval', { id: 's3' }, { expiresAt: 'not a date' });
    const p = (0, outbox_1.partitionLapsed)([live, gone, stillGood, junk], now);
    ...
    const onTheDot = (0, outbox_1.newItem)('pt-approval', { id: 's4' }, { expiresAt: new Date(now).toISOString() });
    const edge = (0, outbox_1.partitionLapsed)([onTheDot], now);
    ...
    const aMsLeft = (0, outbox_1.partitionLapsed)([(0, outbox_1.newItem)('pt-approval', { id: 's5' }, { expiresAt: new Date(now + 1).toISOString() })], now);
    eq(aMsLeft.lapsed.length, 0, 'and one millisecond before it, it has not');
    ok((0, outbox_1.lapsedNote)('pt-approval').includes('not sent'), 'and the member is told plainly that it did not happen');
    ok(!/will be sent|we will/i.test((0, outbox_1.lapsedNote)('message')), 'a lapsed intent must not promise anything further');
}
...
{
    const id = (0, outbox_1.newRowId)();
    ok(/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/.test(id), `A ROW ID MUST BE A UUID THE COLUMN WILL TAKE — got "${id}"`);
    const seen = new Set();
    for (let i = 0; i < 5000; i++)
        seen.add((0, outbox_1.newRowId)());
...
// actually gets told.
{
    const a = (0, outbox_1.newItem)('day-plan', { d: 1 }, { at: '2026-09-01T09:00:00.000Z' });
    const b = (0, outbox_1.newItem)('day-plan', { d: 2 }, { at: '2026-09-02T09:00:00.000Z' });
    const g = (0, outbox_1.newItem)('goal', { kind: 'weight' }, { at: '2026-09-01T08:00:00.000Z' });
    ...
    const folded = (0, outbox_1.mergeLapsed)([a], [b, g]);
    ...
    const held = (0, outbox_1.mergeLapsed)([], [a]);
    const afterLaunch = (0, outbox_1.mergeLapsed)(held, [g]);
    ...
    const older = (0, outbox_1.newItem)('day-plan', { d: 3 }, { at: sameMoment });
    const newer = (0, outbox_1.newItem)('day-plan', { d: 4 }, { at: sameMoment });
    eq((0, outbox_1.mergeLapsed)([older], [newer]).find((i) => i.kind === 'day-plan').id, newer.id, 'on an exact tie the later arrival wins');
    eq((0, outbox_1.mergeLapsed)([newer], [older]).find((i) => i.kind === 'day-plan').id, older.id, 'which is a fact about arrival order and not about the two ids');
    ...
    let piled = [];
    for (let i = 0; i < 500; i++) {
        piled = (0, outbox_1.mergeLapsed)(piled, [(0, outbox_1.newItem)(outbox_1.OUTBOX_KINDS[i % outbox_1.OUTBOX_KINDS.length], { i })]);
    }
    eq(piled.length, outbox_1.OUTBOX_KINDS.length, 'and it can never hold more than one of each');
    eq(piled.map((i) => i.kind).join(','), outbox_1.OUTBOX_KINDS.join(','), 'drawn in the union order, so the home screen puts the same line in the same place every launch');
    ...
    ok((0, outbox_1.outboxLapsedKey)('u1') !== (0, outbox_1.outboxLapsedKey)('u2'), 'the notices are keyed per account');
    ok((0, outbox_1.outboxLapsedKey)('u1') !== (0, outbox_1.outboxKey)('u1'), 'and they are not the outbox — an unread outbox must not lose them');
}
/* ── 4 · order ─────────────────────────────────────────────────────────── */
{
    const older = (0, outbox_1.newItem)('message', { body: 'first' }, { at: '2026-09-01T10:00:00.000Z' });
    const newer = (0, outbox_1.newItem)('message', { body: 'second' }, { at: '2026-09-01T10:05:00.000Z' });
    const other = (0, outbox_1.newItem)('measurement', { waist: 80 }, { at: '2026-09-01T09:00:00.000Z' });
    const msgs = (0, outbox_1.ofKind)([newer, other, older], 'message');
    eq(msgs.length, 2, 'one kind at a time');
    eq(msgs[0].payload.body, 'first', 'oldest first: a thread replayed backwards is a different conversation');
    eq((0, outbox_1.inOrder)([newer, other, older])[0].id, other.id, 'and across kinds it is still the order they happened in');
}
/* ── 5 · the copy ──────────────────────────────────────────────────────── */
{
    eq((0, outbox_1.outboxNote)(0, 'message'), null, 'nothing waiting draws no banner');
    const one = (0, outbox_1.outboxNote)(1, 'measurement') ?? '';
    ok(one.includes('measurement') && !one.includes('measurements'), 'one is singular');
    ok(((0, outbox_1.outboxNote)(2, 'measurement') ?? '').includes('measurements'), 'two is plural');
    ...
    eq((0, outbox_1.kindNoun)('pt-approval').many, 'session approvals', 'every kind has a noun, so no sentence can be assembled without one');
    eq((0, outbox_1.kindNoun)('day-plan').one, 'planned day', 'and the noun is the member’s word for the thing, not the table’s');
}
...
{
    eq(outbox_1.OUTBOX_KINDS.length, 9, 'the list has one entry per kind in the union');
    for (const k of outbox_1.OUTBOX_KINDS) {
        ok((0, outbox_1.isOutboxKind)(k), `${k} is recognised coming back off the disk`);
        const n = (0, outbox_1.kindNoun)(k);
        ok(!!n.one && !!n.many, `${k} has a singular and a plural to be named by`);
        ok(((0, outbox_1.outboxNote)(1, k) ?? '').includes(n.one), `${k} names itself in the waiting line`);
        ok((0, outbox_1.lapsedNote)(k).includes(n.one), `${k} names itself in the lapsed line`);
    }
    eq((0, outbox_1.outboxNote)(0, 'message'), null, 'nothing waiting draws no line at all');
    ok(!(0, outbox_1.isOutboxKind)('booking'), 'and a booking is still not a kind — see the header for why');
    ...
    ok((0, outbox_1.isOutboxKind)('coach-doc-accept'), 'a coach document acceptance survives the round trip through storage');
    eq((0, outbox_1.kindNoun)('coach-doc-accept').one, 'signed document', 'and it is a signed document to the member, not an acceptance row');
    ok(((0, outbox_1.outboxNote)(1, 'coach-doc-accept') ?? '').includes('not sent yet'), 'the waiting line does not claim the coach has it');
    ...
    ok((0, outbox_1.isOutboxKind)('scan'), 'a body scan survives the round trip through storage');
    eq((0, outbox_1.kindNoun)('scan').one, 'body scan', 'and it is a body scan to the member, not a scans row');
    ok(((0, outbox_1.outboxNote)(1, 'scan') ?? '').includes('not sent yet'), 'the waiting line does not claim it is on their record');
    ...
    // they typed.
    ok((0, outbox_1.isOutboxKind)('session-request'), 'a session request survives the round trip through storage');
    ok(!(0, outbox_1.isOutboxKind)('booking'), 'while a booking is still not a kind, which is the distinction the header draws');
    eq((0, outbox_1.kindNoun)('session-request').one, 'session request', 'and it is a request to the member, never a booking');
    ok(!/book/i.test((0, outbox_1.outboxNote)(1, 'session-request') ?? ''), 'the waiting line never uses the word book');
    ok(!/book/i.test((0, outbox_1.lapsedNote)('session-request')), 'and neither does the one that says it did not go');
}
if (errors.length) {
