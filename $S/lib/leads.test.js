...
/* ── REACH ──────────────────────────────────────────────────────────────── */
eq((0, leads_1.contactKind)('joe@example.com'), 'email', 'a plain address is an email');
eq((0, leads_1.contactKind)('  Joe.Bloggs@sub.example.co.uk '), 'email', 'so is one with dots, a subdomain and stray spacing');
...
eq((0, leads_1.contactKind)('joe+ads@x.com'), 'email', 'a plus-addressed inbox is an email, not a phone number');
eq((0, leads_1.contactKind)('joe@example'), 'unknown', 'an address with no dot after the @ is not something a mail app can take');
eq((0, leads_1.contactKind)('a@b.c d@e.f'), 'unknown', 'and two addresses with a space between them are not one address');
eq((0, leads_1.contactKind)('+44 7700 900123'), 'phone', 'an international number with spaces is a phone number');
eq((0, leads_1.contactKind)('(020) 7946 0958'), 'phone', 'so is one written with brackets and a dash');
eq((0, leads_1.contactKind)('07700-900123'), 'phone', 'and a local one with a dash');
...
eq((0, leads_1.contactKind)('123456'), 'unknown', 'six digits is not a phone number');
eq((0, leads_1.contactKind)('1234567'), 'phone', 'seven is');
eq((0, leads_1.contactKind)('123456789012345'), 'phone', 'fifteen still is');
eq((0, leads_1.contactKind)('1234567890123456'), 'unknown', 'sixteen is not — that is an account number');
eq((0, leads_1.contactKind)('@joebloggs'), 'unknown', 'an Instagram handle is neither, and is not made into one');
eq((0, leads_1.contactKind)('ask for joe'), 'unknown', 'nor is a sentence');
eq((0, leads_1.contactKind)(''), 'unknown', 'nor is nothing');
eq((0, leads_1.contactKind)(null), 'unknown', 'nor is a null the database says cannot happen');
/* ── ATTRIBUTION ────────────────────────────────────────────────────────── */
const one = (over) => ({
...
{
    const rows = (0, leads_1.shapeLeads)([one({ via_code: 'flyer7' })], CODES);
...
{
    const rows = (0, leads_1.shapeLeads)([one({ via_code: 'K7M2QX' })], CODES);
...
}
{
    ...
    const rows = (0, leads_1.shapeLeads)([one({ via_code: 'OLDGONE' })], CODES);
...
}
{
    ...
    const rows = (0, leads_1.shapeLeads)([one({}), one({ id: 'l2', via_code: 'IGBIO9' })], []);
    eqJson(rows.map((r) => r.campaign), [null, null], 'with no codes read, no enquiry claims a campaign');
}
/* ── ORDER ──────────────────────────────────────────────────────────────── */
{
    const rows = (0, leads_1.shapeLeads)([
    ...
    eqJson(rows.map((r) => r.id), ['new-today', 'new-old', 'contacted-new', 'closed-new', 'closed-old'], 'what still needs doing comes first, newest first inside each state');
}
{
    ...
    eqJson(rows.map((r) => r.id), ['dated', 'a-undated', 'b-undated'], 'an unreadable date sorts last within its state, and ties break on the id so the list does not reshuffle');
}
{
    const first = (0, leads_1.shapeLeads)([
        one({ id: 'zzz', at: ago(3) }),
        one({ id: 'aaa', at: ago(3) }),
    ], CODES);
    eqJson(first.map((r) => r.id), ['aaa', 'zzz'], 'two enquiries in the same instant read in the same order every time');
}
/* ── The rows that are not rows ─────────────────────────────────────────── */
{
    ...
    eqJson(rows.map((r) => r.id), ['l4'], 'a row with no id, no name or no contact is dropped rather than drawn blank');
}
{
    const rows = (0, leads_1.shapeLeads)([one({ name: '  Sam   Vimes  ', note: '   ' })], CODES);
...
{
    const long = 'x'.repeat(leads_1.MAX_LEAD_NAME + 40);
    const rows = (0, leads_1.shapeLeads)([one({
            name: long,
            contact: `${'c'.repeat(leads_1.MAX_LEAD_CONTACT + 40)}@x.com`,
            note: 'y'.repeat(leads_1.MAX_LEAD_NOTE + 40),
        })], CODES);
    eq(rows[0].name.length, leads_1.MAX_LEAD_NAME, 'a name past the server’s limit is cut to it');
    eq(rows[0].contact.length, leads_1.MAX_LEAD_CONTACT, 'and so is a contact string');
    eq(rows[0].note?.length, leads_1.MAX_LEAD_NOTE, 'and so is a message — at the server’s length and not at some other one');
}
{
    ...
    const rows = (0, leads_1.shapeLeads)([{ id: 'l9', name: 'Sam', contact: 'sam@example.com', note: null, via_code: 'FLYER7', state: 'new' }], CODES);
    eq(rows[0].at, null, 'a row with no `at` key at all carries null, never undefined');
}
{
    const rows = (0, leads_1.shapeLeads)([one({ state: 'joined' })], CODES);
    eq(rows[0].state, 'new', 'a state this app does not have falls back to new, never to a fourth state');
    ok(!leads_1.LEAD_STATES.includes('joined'), 'and there is no ‘joined’ state to fall into');
    eqJson(leads_1.LEAD_STATES.map((s) => leads_1.LEAD_STATE_LABEL[s]), ['New', 'Contacted', 'Closed'], 'the three states are labelled in Title Case, as buttons and filters');
}
eqJson((0, leads_1.shapeLeads)(null, CODES), [], 'a null read shapes to nothing rather than throwing');
/* ── COUNT ──────────────────────────────────────────────────────────────── */
const ready = (0, leads_1.shapeLeads)([
    one({ id: 'a', state: 'new' }),
    one({ id: 'b', state: 'new' }),
    one({ id: 'c', state: 'closed' }),
], CODES);
{
    const line = (0, leads_1.leadCountLine)('error', []);
...
}
{
    ...
    const line = (0, leads_1.leadCountLine)('partial', ready);
...
{
    const line = (0, leads_1.leadCountLine)('loading', ready);
    ok(!/\b3\b/.test(line), 'nothing is counted while the read is still in flight');
}
{
    const line = (0, leads_1.leadCountLine)('ready', []);
    ok(/Nobody has left their details yet/.test(line), 'an empty list under ready may say so');
}
{
    const line = (0, leads_1.leadCountLine)('ready', ready);
    eq(line, '3 enquiries · 2 waiting on you.', 'a whole read states the count and who is waiting');
}
{
    const done = (0, leads_1.shapeLeads)([one({ id: 'a', state: 'closed' })], CODES);
    eq((0, leads_1.leadCountLine)('ready', done), '1 enquiry, all of them dealt with.', 'one enquiry is singular, and none waiting is said rather than left off');
}
{
    const big = [];
    for (let i = 0; i < 1200; i++)
        big.push(...(0, leads_1.shapeLeads)([one({ id: `n${i}`, state: 'closed' })], CODES));
    ok((0, leads_1.leadCountLine)('ready', big).includes('1,200'), 'a four-figure count carries its thousands separator');
}
/* ── What a person is told before it is too late to fix ─────────────────── */
eq((0, leads_1.leadProblem)('', 'joe@example.com'), 'Put in a name, so the coach knows who is asking.', 'a blank name is refused HERE, because the server answers every input identically');
ok(((0, leads_1.leadProblem)('x'.repeat(leads_1.MAX_LEAD_NAME + 1), 'joe@example.com') || '').includes(String(leads_1.MAX_LEAD_NAME)), 'a name past the limit names the limit');
ok(((0, leads_1.leadProblem)('Sam', '') || '').includes('no way to answer'), 'no contact is refused, and the reason is the coach cannot reply');
ok(((0, leads_1.leadProblem)('Sam', '@joebloggs') || '').includes('reach you on'), 'something that is neither an email nor a number is refused before it is sent');
eq((0, leads_1.leadProblem)('Sam', '+44 7700 900123'), null, 'a name and a number is enough');
eq((0, leads_1.leadProblem)('  Sam  ', ' joe@example.com '), null, 'and stray spacing is not a reason to refuse anybody');
...
eq((0, leads_1.leadProblem)('n'.repeat(leads_1.MAX_LEAD_NAME), 'joe@example.com'), null, 'a name of exactly the limit is accepted, not refused one character early');
eq((0, leads_1.leadProblem)('Sam', `${'a'.repeat(leads_1.MAX_LEAD_CONTACT - '@example.io'.length)}@example.io`), null, 'and so is a contact string of exactly the limit');
eq((0, leads_1.followUpProblem)('   '), 'Write what you did, so the next time you open this you know where it got to.', 'an empty follow-up is refused rather than filed as a note nobody wrote');
ok(((0, leads_1.followUpProblem)('x'.repeat(leads_1.MAX_FOLLOW_UP + 1)) || '').includes(String(leads_1.MAX_FOLLOW_UP)), 'and one past the limit names the limit');
eq((0, leads_1.followUpProblem)('x'.repeat(leads_1.MAX_FOLLOW_UP)), null, 'one of exactly the limit saves — the bound is inclusive, as the CHECK is');
eq((0, leads_1.followUpProblem)('Rang, left a voicemail.'), null, 'a real note saves');
...
eq(leads_1.MAX_LEAD_NAME, 80, 'a name is 80 characters, as `char_length(name) between 1 and 80` allows');
eq(leads_1.MAX_LEAD_CONTACT, 120, 'a contact string is 120, as the CHECK allows');
eq(leads_1.MAX_LEAD_NOTE, 500, 'a message is 500 — long enough for a real enquiry, and what the CHECK allows');
eq(leads_1.MAX_FOLLOW_UP, 1000, 'a follow-up note is 1,000, as coach_lead_notes_sane allows');
/* ── Follow-ups ─────────────────────────────────────────────────────────── */
{
    const notes = (0, leads_1.shapeFollowUps)([
    ...
    eqJson(notes.map((n) => n.id), ['n3', 'n1', 'n0'], 'follow-ups read newest first, an undated one comes last, and an empty one or one with no id is not a record of anything');
    eq(notes[0].body, 'Texted them the address.', 'and the words come through whole — a note missing its first character is a note somebody wrote and cannot read back');
}
{
    ...
    const notes = (0, leads_1.shapeFollowUps)([{ id: 'n1', body: 'Rang them.' }]);
...
ok(/does not contact these people/i.test(leads_1.FOLLOW_UP_IS_MANUAL), 'the screen says plainly that Repple contacts nobody');
ok(/nothing will be/i.test(leads_1.FOLLOW_UP_IS_MANUAL), 'and that it is not going to start — there is no email channel to start it with');
ok(!/(sequence|automat|schedul|drip)/i.test(leads_1.FOLLOW_UP_IS_MANUAL), 'and it does not use a word that implies one');
...
ok(String(leads_1.ENQUIRY_IS_ANNOUNCED) !== String(leads_1.FOLLOW_UP_IS_MANUAL), 'the notification sentence is its own, and does not replace the one saying nothing is sent');
ok(!/(sent to them|reply|replied|acknowledg|respond)/i.test(leads_1.ENQUIRY_IS_ANNOUNCED), 'and it never implies the enquirer has heard anything');
ok(/name/i.test(leads_1.ENQUIRY_IS_ANNOUNCED) && /lock screen/i.test(leads_1.ENQUIRY_IS_ANNOUNCED), 'it says what the notification carries and why it carries no more: a push is drawn on a lock screen and the contact string is a stranger\u2019s typing');
ok(!/(sequence|automat|schedul|drip)/i.test(leads_1.ENQUIRY_IS_ANNOUNCED), 'and it promises no sequence either');
ok(/no coach to give them to/i.test(leads_1.MISTYPED_CODE_NOTE), 'the cost of dropping an unresolvable code is stated rather than hidden');
...
const KINDS = ['first', 'second', 'last'];
const lead = { name: 'Sarah Ahmed', note: 'I want to get back into lifting.', contact: 'sarah@example.com', contactKind: 'email' };
for (const k of KINDS) {
    const d = (0, leads_1.followUpDraft)(k, lead, 'Tim Rodgers', 'Northside Strength');
    ...
    ok(!/guarantee|transform|results in \d|lose \d|\bfree trial\b/i.test(d.body), `the ${k} draft promises nothing`);
    ok(!/[£$€]|\bAED\b|\bper (month|session)\b/i.test(d.body), `and states no price — this product has no default currency and a draft is the wrong place to invent one`);
}
// The business name is the COACH's trading name, never the app's, and it is
// simply absent when they have not set one.
ok((0, leads_1.followUpDraft)('first', lead, 'Tim', 'Northside Strength').body.includes('Northside Strength'), 'a coach with a trading name writes from it');
ok(!/ at \b/.test((0, leads_1.followUpDraft)('first', lead, 'Tim', null).body.split('\n')[2] ?? ''), 'and a coach without one simply does not name a business');
ok(!/undefined|null/.test((0, leads_1.followUpDraft)('first', { name: '', note: null }, null, null).body), 'a nameless lead and a nameless coach still produce a readable message');
...
ok((0, leads_1.followUpDraft)('first', lead, 'Tim', null).body.includes('back into lifting'), 'a note they left is quoted back');
ok(!/You mentioned/.test((0, leads_1.followUpDraft)('first', { name: 'Sarah', note: null }, 'Tim', null).body), 'and nothing is quoted when they left nothing');
// The sender's name, on the same rule `greetingName` keeps.
eq((0, leads_1.senderName)('Tim Rodgers'), 'Tim', 'first word only');
eq((0, leads_1.senderName)('  '), null, 'whitespace is nobody');
eq((0, leads_1.senderName)('tim@example.com'), null, 'an email address is not a name');
eq((0, leads_1.senderName)('7f3a9c21-0000'), null, 'and neither is a uuid');
/* ── the link that opens their own mail app ─────────────────────────────── */
const draft = (0, leads_1.followUpDraft)('first', lead, 'Tim', null);
const mail = (0, leads_1.followUpLink)(lead, draft);
...
const phoneLead = { ...lead, contact: '+44 (0)7700 900-123', contactKind: 'phone' };
const sms = (0, leads_1.followUpLink)(phoneLead, draft);
...
ok(((0, leads_1.followUpLink)({ contact: '(0161) 496 0000', contactKind: 'phone' }, draft) ?? '')
    .startsWith('sms:01614960000?'), 'a domestic number keeps its area code');
eq((0, leads_1.followUpLink)({ contact: '+++', contactKind: 'phone' }, draft), null, 'and a number with no digits opens nothing');
...
eq((0, leads_1.followUpLink)({ contact: '@sarahlifts', contactKind: 'unknown' }, draft), null, 'there is nothing to open an Instagram handle with, and nothing is offered');
eq((0, leads_1.followUpLink)({ contact: '', contactKind: 'email' }, draft), null, 'and nothing for an empty contact');
...
ok(/^Opened the first reply/.test((0, leads_1.followUpRecord)('first', 'email')), 'the note names which draft');
ok((0, leads_1.followUpRecord)('second', 'text').includes('a text'), 'and which channel');
ok(!/sent/i.test((0, leads_1.followUpRecord)('last', 'email')), 'and never says "sent" — nothing here observed a send');
// The buttons and their captions.
for (const k of KINDS) {
    ok(/^[A-Z]/.test(leads_1.FOLLOW_UP_LABEL[k]), `${k}'s label is Title Case — it is a button`);
    ok(leads_1.FOLLOW_UP_WHEN[k][0] === leads_1.FOLLOW_UP_WHEN[k][0].toLowerCase(), `${k}'s caption is sentence case`);
    ok(leads_1.FOLLOW_UP_WHEN[k].endsWith('.'), `${k}'s caption is a sentence`);
}
eq(new Set(Object.values(leads_1.FOLLOW_UP_LABEL)).size, KINDS.length, 'no two buttons read the same');
if (errors.length) {
