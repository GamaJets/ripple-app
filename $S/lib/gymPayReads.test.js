...
function fakeSb(answer) {
    const asked = [];
    const from = (table) => {
        const q = { table, orders: [], range: null, in: null };
        asked.push(q);
        const chain = {
            select: () => chain,
            eq: () => chain,
...
async function oneLineDated(startsAt, zone) {
    const { sb } = fakeSb((q) => {
    ...
    return (await (0, gymPay_1.fetchClassPay)(sb, 'gym', zone))[0];
}
async function main() {
    /* ── fetchClassPay pages to the end ────────────────────────────────────────── */
    {
        const N = rowCap_1.ROW_CAP * 2 + 7;
        ...
        const { sb, asked } = fakeSb((q) => {
            if (q.table === 'gym_class_pay') {
                const [a, b] = q.range ?? [0, rowCap_1.ROW_CAP - 1];
        ...
        const out = await (0, gymPay_1.fetchClassPay)(sb, 'gym', null);
        ...
        eq(pay[0].orders.join(','), 'created_at:desc,id:desc', 'the read is ordered totally — `created_at` alone ties, and tied pages drop and repeat rows silently');
        eq(pay[1].range?.[0], rowCap_1.ROW_CAP, 'the second page starts where the first ended');
        const dates = asked.filter((q) => q.table === 'gym_classes');
        eq(dates.length, Math.ceil(N / idLookup_1.ID_CHUNK), 'the class dates are looked up in chunks — one `.in()` of 2007 ids truncates and 414s, both in silence');
        ok(dates.every((q) => (q.in ?? []).length <= idLookup_1.ID_CHUNK), 'and no chunk is bigger than the chunk size');
        eq(new Set(dates.flatMap((q) => q.in ?? [])).size, N, 'every class id is asked about exactly once');
        ok(out.every((r) => r.taughtOn === readerDay(CLASS_AT)), 'so every line is dated — an undated line is dropped from the run by `scopedToRun`, which is a coach not paid');
        ok(out.every((r) => r.taughtOnBasis === 'reader'), 'and with no gym zone passed, every line says out loud whose calendar it was cut on');
    }
    ...
    // supabase/parts/710, was never consulted at all.
    {
        ...
        ok([london, dubai, la].every((r) => r.taughtOnBasis === 'gym'), 'none of the three fell back to the machine the console is open on');
        ...
        eq((0, gymPay_1.scopedToRun)([london], (r) => r.taughtOn, AUG).length, 1, 'so August’s run pays the London class');
        eq((0, gymPay_1.scopedToRun)([dubai], (r) => r.taughtOn, AUG).length, 0, 'and does not pay the Dubai one, which belongs to September and has September’s run coming');
        ...
        eq((0, gymPay_1.scopedToRun)([kiritimati], (r) => r.taughtOn, AUG).length, 0, 'and is genuinely not on August’s run');
        ...
        eq(junk.taughtOnBasis, 'reader', 'a stored value this runtime cannot resolve is a gym with no zone, not a crash and not a guess');
    }
     ...
     * screen, and a wrong reading here is not caught anywhere downstream. */
    {
        const readOne = async (row) => {
            const { sb } = fakeSb((q) => {
                ...
                if (q.table === 'gym_classes')
                    return { data: (q.in ?? []).map((id) => ({ id, starts_at: CLASS_AT })), error: null };
                return { data: (q.in ?? []).map((id) => ({ id, full_name: 'Ana' })), error: null };
            });
            return (await (0, gymPay_1.fetchClassPay)(sb, 'gym', 'Europe/London'))[0];
        };
        ...
        eq((await readOne({ pay_kind: 'per_head' })).payKind, 'per_class', 'a kind this build does not know is read as flat rather than multiplied by a register');
    ...
    /* ── fetchAdjustments pages to the end ─────────────────────────────────────── */
    {
        ...
        const out = await (0, gymPay_1.fetchAdjustments)(sb, 'gym');
        ...
        eq(adj[0].orders.join(','), 'applies_on:desc,id:desc', '`applies_on` is a DATE, so a batch filed on the first of the month is entirely tied without `id`');
    ...
    /* ── a page that errors is not the end of the set ──────────────────────────── */
    {
        const { sb } = fakeSb((q) => {
            ...
            return {
                data: Array.from({ length: rowCap_1.ROW_CAP }, (_, i) => ({
        ...
        let threw = false;
        try {
            await (0, gymPay_1.fetchClassPay)(sb, 'gym', null);
        }
        catch {
            threw = true;
        }
     ...
     */
    {
        ...
        await (0, gymPay_1.stampRunExtras)(sb, 'run-1', lineIds, adjIds);
        const writes = asked.filter((q) => q.update);
        ok(writes.every((q) => q.counted), 'every stamp asks PostgREST to COUNT the rows it changed — RLS filters an update rather than refusing it, so an uncounted stamp reports a settlement over rows it never touched');
        ok(writes.every((q) => (q.in ?? []).length <= idLookup_1.ID_CHUNK), 'and no single PATCH carries more ids than fit in a request line — one `.in()` of 461 uuids is an 18KB query string and a 414 the owner sees as a run that half worked');
        const lines = writes.filter((q) => q.table === 'gym_class_pay');
        eq(lines.length, Math.ceil(N / idLookup_1.ID_CHUNK), 'the class lines are stamped in chunks, the last one short');
        ...
        eq(writes.filter((q) => q.table === 'payroll_adjustments').length, 1, 'four adjustments are one request, because chunking is a ceiling and not a fixed batch size');
    }
    /* ── a chunk that goes missing is reported, never swallowed ────────────── */
    {
        const lineIds = ids(idLookup_1.ID_CHUNK + 5, 'c1a55');
        ...
        let said = null;
        try {
            await (0, gymPay_1.stampRunExtras)(sb, 'run-1', lineIds, []);
        }
        catch (e) {
            said = e?.message ?? '';
        }
        ok(said != null, 'a stamp that changed fewer rows than it was given throws rather than returning');
        ok(!!said && said.includes(`${idLookup_1.ID_CHUNK} of ${idLookup_1.ID_CHUNK + 5}`), `the sentence names how many of how many actually stamped — got ${JSON.stringify(said)}`);
        ok(!!said && /settled twice/.test(said), 'and says what it costs, because the unstamped remainder is payable again');
    }
    /* ── an id listed twice is one row and not a shortfall ─────────────────── */
    {
        ...
        let threw = false;
        try {
            await (0, gymPay_1.stampRunExtras)(sb, 'run-1', dup, []);
        }
        catch {
            threw = true;
        }
