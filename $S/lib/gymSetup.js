...
exports.SETUP_ORDER = void 0;
exports.looksProvisioned = looksProvisioned;
exports.assessGymSetup = assessGymSetup;
exports.setupTally = setupTally;
 ...
 */
exports.SETUP_ORDER = ['currency', 'timezone', 'name', 'plan', 'member', 'fee'];
/**
 ...
 */
function looksProvisioned(name) {
    const n = (name ?? '').trim();
    if (!n)
        return true;
...
const UNREAD_GYM = 'The gym record could not be read, so this could not be checked — it is not known to be either way.';
/**
 ...
 */
function assessGymSetup(f) {
    const t = f.tenant;
    const gymItem = (key, isDone, title, breaks, onlyIf, found = () => null) => (t === null
    ...
    const countItem = (key, n, title, breaks, onlyIf, unknownWhy) => (n === null
    ...
    const by = {
        currency: gymItem('currency', (s) => stated(s.currency) !== null, 'Set what this gym charges in', 'Until a currency is set this gym cannot price a plan, record a payment, import a price '
            + 'book or settle payroll, and every money figure in the console is withheld rather than '
            + 'shown in a currency nobody chose.', null),
        timezone: gymItem('timezone', (s) => stated(s.timezone) !== null, 'Say where this gym is', 'This is the only setting here that is wrong rather than empty while it is unset: with no '
            ...
            + 'by accident; read by a bookkeeper in another country they are not.', null),
        name: gymItem('name', (s) => !looksProvisioned(s.name), 'Name the gym', 'Sign-up named this gym after the account that created it. That name is the heading on the '
            + 'month-close sheet, the tax pack and every export that leaves the building, and it is what '
            + 'members see in the app.', null, (s) => stated(s.name)),
        plan: countItem('plan', f.plans, 'Price at least one membership', 'A membership opens against a plan, so until the price book has one, a membership carries '
            + 'no price and the recurring revenue figure on the overview is a dash.', 'Only if this gym sells memberships rather than drop-ins alone.', 'The price book did not come back whole, so this could not be checked.'),
        member: countItem('member', f.members, 'Get the members in', 'Every screen in this console is a view of the roster, so with nothing on it they are all '
            ...
            + 'one go.', null, 'The roster did not come back whole, so this could not be checked.'),
        fee: gymItem('fee', (s) => s.sessionFee !== null, 'Say what a session is worth', 'Payroll multiplies delivered one-to-ones by this, so while it is unset the payroll screen '
            + 'shows what was delivered and refuses to price it.', 'Only if this gym delivers one-to-one sessions.'),
    };
    return exports.SETUP_ORDER.map((k) => by[k]);
}
/** How many of each. Three counts, never two: see the header. */
function setupTally(items) {
    let done = 0, todo = 0, unknown = 0;
    for (const i of items) {
        if (i.state === 'done')
            done++;
        else if (i.state === 'todo')
            todo++;
        else
            unknown++;
    }
 ...
 */
function setupLine(items) {
    const t = setupTally(items);
    if (t.todo === 0 && t.unknown === 0)
        return null;
