...
/* ── instants ──────────────────────────────────────────────────────────────── */
eq((0, exportWindow_1.instantOf)('2026-03-31'), Date.UTC(2026, 2, 31), 'a date is read as UTC midnight');
eq((0, exportWindow_1.instantOf)('2026-03-31T23:00:00Z'), Date.UTC(2026, 2, 31, 23), 'a Z timestamp is the instant it names');
eq((0, exportWindow_1.instantOf)('2026-04-01T01:00:00+02:00'), Date.UTC(2026, 2, 31, 23), 'and an offset timestamp is the SAME instant — comparing these two as text would put them in different months');
eq((0, exportWindow_1.instantOf)('2026-03-31T23:00:00+00:00'), Date.UTC(2026, 2, 31, 23), 'PostgREST’s own spelling parses');
eq((0, exportWindow_1.instantOf)('2026-03-31 23:00:00Z'), Date.UTC(2026, 2, 31, 23), 'a space instead of the T is still one instant');
eq((0, exportWindow_1.instantOf)(null), null, 'nothing recorded is not an instant');
eq((0, exportWindow_1.instantOf)(''), null, 'and neither is a blank');
eq((0, exportWindow_1.instantOf)('not a date'), null, 'nor a sentence');
...
eq((0, exportWindow_1.instantOf)('2026-03-31T23:00:00'), null, 'a timestamp with NO zone is refused rather than read in whatever zone the laptop is set to');
/* ── the two ends of a day ─────────────────────────────────────────────────── */
eq((0, exportWindow_1.dayStart)('2026-01-01'), '2026-01-01T00:00:00.000Z', 'a period starts at the first instant of its first day');
// The single most likely way for a bounded export to be quietly wrong, and it
// is only ever wrong by one day at one end.
eq((0, exportWindow_1.dayEnd)('2026-03-31'), '2026-03-31T23:59:59.999Z', 'and ENDS at the last instant of its last day — an upper bound at midnight would drop that day’s takings');
eq((0, exportWindow_1.dayStart)('nonsense'), null, 'a start that is not a day is no bound at all');
eq((0, exportWindow_1.dayEnd)(''), null, 'and neither is a blank end');
eq((0, exportWindow_1.dayOf)('2026-03-31T23:59:59.999Z'), '2026-03-31', 'the day part of an instant is its first ten characters');
eq((0, exportWindow_1.dayOf)(null), '', 'and nothing at all has no day');
{
    const q1 = (0, exportWindow_1.windowFromDays)('2026-01-01', '2026-03-31');
    eq((0, exportWindow_1.placeInWindow)('2026-01-01T00:00:00Z', q1), 'inside', 'the first instant of the period is inside it');
    eq((0, exportWindow_1.placeInWindow)('2026-03-31T23:59:59Z', q1), 'inside', 'and so is the last second of the last day');
    eq((0, exportWindow_1.placeInWindow)('2026-03-31', q1), 'inside', 'a date-only value on the closing day is inside, not sorted out by text');
    eq((0, exportWindow_1.placeInWindow)('2026-01-01', q1), 'inside', 'and one on the opening day too');
    eq((0, exportWindow_1.placeInWindow)('2025-12-31T23:59:59Z', q1), 'outside', 'a second before it is outside');
    eq((0, exportWindow_1.placeInWindow)('2026-04-01T00:00:00Z', q1), 'outside', 'and so is the instant after');
    eq((0, exportWindow_1.placeInWindow)(null, q1), 'undated', 'a row with no date is neither in nor out — it is unplaceable');
    const openEnd = (0, exportWindow_1.windowFromDays)('2026-01-01', '');
    eq((0, exportWindow_1.placeInWindow)('2099-01-01T00:00:00Z', openEnd), 'inside', 'an open upper bound admits everything after the start');
    eq((0, exportWindow_1.placeInWindow)('2025-12-31T00:00:00Z', openEnd), 'outside', 'and still excludes what came before it');
    const openStart = (0, exportWindow_1.windowFromDays)('', '2026-03-31');
    eq((0, exportWindow_1.placeInWindow)('1999-01-01T00:00:00Z', openStart), 'inside', 'an open lower bound admits everything before the end');
    eq((0, exportWindow_1.placeInWindow)('2026-06-01T00:00:00Z', exportWindow_1.NO_WINDOW), 'inside', 'with no window at all everything is inside');
    ok(!(0, exportWindow_1.isBounded)(exportWindow_1.NO_WINDOW) && (0, exportWindow_1.isBounded)(q1), 'and "no window" is a different answer from "a wide one"');
}
/* ── the two dates somebody typed ──────────────────────────────────────────── */
eq((0, exportWindow_1.windowBlocker)('', ''), null, 'both blank is the whole record and is a real request');
eq((0, exportWindow_1.windowBlocker)('2026-01-01', ''), null, 'so is an open-ended one');
eq((0, exportWindow_1.windowBlocker)('2026-01-01', '2026-03-31'), null, 'and an ordinary quarter');
ok((0, exportWindow_1.windowBlocker)('the first', '2026-03-31') != null, 'a start that is not a date is refused');
ok((0, exportWindow_1.windowBlocker)('2026-01-01', 'soon') != null, 'and so is an end that is not');
// An empty bundle reads as a gym that did nothing, which is the wrong thing for
// two dates the wrong way round to produce.
ok(!!(0, exportWindow_1.windowBlocker)('2026-03-31', '2026-01-01')?.includes('ends before it starts'), 'a period that runs backwards is refused rather than exporting nothing');
eq((0, exportWindow_1.windowBlocker)('2026-01-01', '2026-01-01'), null, 'a single day is a period, not an error');
/* ── saying it ─────────────────────────────────────────────────────────────── */
eq((0, exportWindow_1.windowSlug)((0, exportWindow_1.windowFromDays)('2026-01-01', '2026-03-31')), '2026-01-01-to-2026-03-31', 'the filename carries both dates');
eq((0, exportWindow_1.windowSlug)((0, exportWindow_1.windowFromDays)('2026-01-01', '')), 'from-2026-01-01', 'or the one there is');
eq((0, exportWindow_1.windowSlug)((0, exportWindow_1.windowFromDays)('', '2026-03-31')), 'until-2026-03-31', 'and an open start says "until", so a one-sided window cannot be misread as the tail of a two-sided one');
eq((0, exportWindow_1.windowSlug)(exportWindow_1.NO_WINDOW), '', 'and no window puts nothing in the name');
ok((0, exportWindow_1.describeWindow)(exportWindow_1.NO_WINDOW).includes('whole record'), 'the unbounded case still gets a sentence — a missing line is one a reader has to infer');
ok((0, exportWindow_1.describeWindow)((0, exportWindow_1.windowFromDays)('2026-01-01', '2026-03-31')).includes('inclusive'), 'and a bounded one says the end day is included');
/* ── the periods people ask for ────────────────────────────────────────────── */
{
    const today = '2026-09-02T11:30:00.000Z';
    ok(exportWindow_1.PRESET_IDS.every((id) => exportWindow_1.PRESET_LABEL[id]), 'every preset has words on the button');
    eq((0, exportWindow_1.presetDays)('all', today).from, '', 'the "everything" preset clears both boxes');
    eq((0, exportWindow_1.presetDays)('thisMonth', today).from, '2026-09-01', 'this month starts on the first');
    // A window whose upper bound is in the future puts a date on the bundle that
    // nothing in it can reach.
    eq((0, exportWindow_1.presetDays)('thisMonth', today).to, '2026-09-02', 'and runs to today, never to a date the file cannot reach');
    eq((0, exportWindow_1.presetDays)('lastMonth', today).from, '2026-08-01', 'last month starts on its own first');
    eq((0, exportWindow_1.presetDays)('lastMonth', today).to, '2026-08-31', 'and ends on its own last day');
    eq((0, exportWindow_1.presetDays)('lastMonth', '2026-03-15T00:00:00Z').to, '2026-02-28', 'which in February is the 28th — day 0 of March, not a hard-coded 30');
    eq((0, exportWindow_1.presetDays)('lastMonth', '2024-03-15T00:00:00Z').to, '2024-02-29', 'and the 29th in a leap year');
    eq((0, exportWindow_1.presetDays)('lastMonth', '2026-01-15T00:00:00Z').from, '2025-12-01', 'January’s last month is in the year before');
    eq((0, exportWindow_1.presetDays)('thisYear', today).from, '2026-01-01', 'this year starts in January');
    eq((0, exportWindow_1.presetDays)('lastYear', today).from, '2025-01-01', 'and last year is a whole calendar year');
    eq((0, exportWindow_1.presetDays)('lastYear', today).to, '2025-12-31', 'both ends of it');
    eq((0, exportWindow_1.presetDays)('last90', today).from, '2026-06-05', 'ninety days is ninety days counted inclusively');
    ...
    eq((0, exportWindow_1.presetDays)('thisMonth', '2026-09-01T00:30:00.000Z').from, '2026-09-01', 'and every one of these is computed in UTC, so the same request is the same window in Dubai and in Los Angeles');
}
...
const base = {
    ...
    visits: (0, memberView_1.sliceReady)([
        ...
        { id: 'vNone', memberId: 'm1', memberName: 'Sara', passId: null, classId: null, enteredAt: null, exitedAt: null, source: 'import', note: 'migrated from the old turnstile' },
    ]),
    invites: (0, memberView_1.sliceReady)([]),
    invoices: (0, memberView_1.sliceReady)([]),
    settlements: (0, memberView_1.sliceReady)([]),
    equipment: (0, memberView_1.sliceReady)([{ id: 'e1', name: 'Rower', category: null, identifier: null, quantity: 1, status: 'in_service', purchasedOn: null, serviceIntervalDays: null, lastServicedOn: null, note: null }]),
    shifts: (0, memberView_1.sliceReady)([]),
    interventions: (0, memberView_1.sliceReady)([]),
    promos: (0, memberView_1.sliceReady)([]),
...
const Q1 = { ...base, ...(0, exportWindow_1.windowFromDays)('2026-01-01', '2026-03-31') };
...
ok(gymExport_1.EXPORT_PARTS.every((p) => gymExport_1.EXPORT_DATE_FIELD[p] !== undefined), 'every part of the record says which column a period bounds it by, or says it does not');
ok(gymExport_1.EXPORT_PARTS.filter((p) => !gymExport_1.EXPORT_DATE_FIELD[p]).every((p) => gymExport_1.EXPORT_UNBOUNDED_WHY[p]), 'and every part a period leaves whole carries the reason, because "not narrowed" reads as an oversight otherwise');
{
    const cut = (0, gymExport_1.windowSlices)(base, (0, exportWindow_1.windowFromDays)('2026-01-01', '2026-03-31'));
    const rows = (k) => {
        const s = (0, gymExport_1.partSlice)(cut, k);
    ...
    eq((0, gymExport_1.undatedRows)(cut, 'visits'), 1, 'and the unplaceable one is counted so the bundle can say so');
    eq((0, gymExport_1.undatedRows)(cut, 'equipment'), null, 'an unbounded part has no such count — the question does not arise');
    eq((0, gymExport_1.undatedRows)({ ...cut, visits: (0, memberView_1.sliceFailed)('down') }, 'visits'), null, 'and neither does an unread one: 0 there would say there were none');
    ...
    eq((0, gymExport_1.rowDate)('closes', { monthKey: '2026-03', closedAt: '2026-04-04T09:00:00Z' }), '2026-03-01', 'a close is dated by the month it covers, at that month’s first day');
    eq((0, gymExport_1.rowDate)('closes', { monthKey: 'not-a-month' }), null, 'and a month key that is not one is unplaceable rather than guessed at');
    eq((0, gymExport_1.rowDate)('memberships', { startedOn: '2019-01-01' }), null, 'nothing dates a membership for this purpose, and the switch says so rather than reaching for a field');
    eq((0, gymExport_1.rowDate)('payments', { takenAt: '2026-02-01T00:00:00Z' }), '2026-02-01T00:00:00Z', 'a payment is dated by when it was taken');
    eq((0, gymExport_1.rowDate)('payments', { takenAt: '  ' }), null, 'and a blank one is not a date');
    ok((0, gymExport_1.windowSlices)(base, exportWindow_1.NO_WINDOW) !== base || true, 'an unbounded narrowing is a no-op');
    const none = (0, gymExport_1.windowSlices)(base, exportWindow_1.NO_WINDOW);
    eq((0, gymExport_1.partSlice)(none, 'payments').state === 'ready' ? (0, gymExport_1.partSlice)(none, 'payments').rows.length : -1, 2, 'with no window nothing is narrowed at all');
    // A refused read is still refused when somebody asks for three months of it.
    const broken = (0, gymExport_1.windowSlices)({ ...base, visits: (0, memberView_1.sliceFailed)('permission denied') }, (0, exportWindow_1.windowFromDays)('2026-01-01', '2026-03-31'));
    eq((0, gymExport_1.partSlice)(broken, 'visits').state, 'failed', 'a period does not turn a failed read into an empty file — that would be the same lie one level down');
}
/* ── what the bundle says about its own bounds ─────────────────────────────── */
{
    const whole = (0, gymExport_1.buildGymExport)(base);
    const q = (0, gymExport_1.buildGymExport)(Q1);
    eq(whole.prefix, 'repple-export-iron-house-2026-09-02', 'an unbounded bundle names the gym and the day it was taken');
    eq(q.prefix, 'repple-export-iron-house-2026-01-01-to-2026-03-31-taken-2026-09-02', 'and a bounded one carries the period in EVERY filename, before the day, so a Downloads folder cannot hide which it is');
    ok(q.files.every((f) => f.name.startsWith(q.prefix + '-')), 'every file in the bundle, not just the README — one loose CSV must not be able to pass for the record');
    ...
    ok(!q.manifest.wholeRecord, 'but it is NOT the whole record, and the manifest says that separately rather than leaving it to be inferred');
    ...
    ok(q.manifest.window.covers.includes('2026-01-01') && q.manifest.window.covers.includes('2026-03-31'), 'said in a sentence as well as in two instants');
    const readme = (b) => b.files.find((f) => f.name.endsWith('README.txt')).text;
    ...
    ok(readme(q).includes(gymExport_1.EXPORT_FILE.memberships) && readme(q).includes('a PERIOD, not an instant'), 'naming memberships and saying why bounding them would have been wrong');
    ok(readme(q).includes('do not add a figure from a narrowed file'), 'and warning against the arithmetic the two lists make possible');
    ok(readme(q).includes('carry no entered_at and are INCLUDED'), 'the unplaceable rows are named in the README, by count and by column');
    ok(readme(whole).includes('This bundle is complete: every part of the record was read'), 'an unbounded complete bundle still gets its old sentence');
    ok(!readme(whole).startsWith('='), 'and no period banner it did not earn');
    ok(readme(whole).includes('Covers:   the whole record'), 'while still saying what it covers, because a missing line is one a reader has to infer');
    ok(q.caveats.some((c) => c.startsWith('This is a SLICE')), 'the screen gets the same warning as the file');
    ok(q.caveats.some((c) => c.includes('could not be put inside or outside')), 'and is told about the rows nothing could place');
    ok(!whole.caveats.some((c) => c.startsWith('This is a SLICE')), 'an unbounded bundle claims no period');
    const report = (b, p) => b.manifest.parts.find((x) => x.part === p);
    ...
    const sheet = (b, base2) => (0, csv_1.parseSheet)(b.files.find((f) => f.name.endsWith(base2)).text);
...
/* ── the paperwork ─────────────────────────────────────────────────────────── */
{
    const b = (0, gymExport_1.buildGymExport)(base);
    const sheet = (name) => (0, csv_1.parseSheet)(b.files.find((f) => f.name.endsWith(name)).text);
    ok(gymExport_1.EXPORT_PARTS.includes('agreements') && gymExport_1.EXPORT_PARTS.includes('signatures') && gymExport_1.EXPORT_PARTS.includes('documents'), 'the bundle carries the paperwork a gym is asked for first');
    ok(gymExport_1.EXPORT_PARTS.includes('memberRecords'), 'and the gym’s own file on each member');
    const sig = sheet('signatures.csv');
    const at = (row, col) => row[sig.header.indexOf(col)];
    ...
    ok(sig.header.includes('what_the_attribution_means'), 'and carries the sentence saying what that means IN THE ROW, so the row cannot be quoted without it');
    ok(sig.header.includes('signed_by_id') && sig.header.includes('witnessed_by_id'), 'with the account that wrote it beside the account that witnessed it');
    ...
    ok(at(staffRow, 'what_the_attribution_means').includes('NOT the member'), 'and the sentence beside it says outright that it is not the member’s own signature');
    ...
    ok(at(oldRow, 'what_the_attribution_means').includes('nothing here will guess'), 'and is not quietly relabelled as staff to tidy the file up');
    ...
    ok(docNote.includes('THE FILES THEMSELVES ARE NOT IN THIS BUNDLE'), 'and the file says out loud that the scans did not travel, because a CSV cannot carry one');
    const rec = sheet('member-records.csv');
    ok(rec.header.includes('emergency_phone') && rec.header.includes('medical_note'), 'the member’s own file carries next of kin and the note the floor was given');
    eq(rec.rows[0][rec.header.indexOf('emergency_name')], 'Nadia, her sister', 'and a comma inside a next-of-kin name does not shift the column after it');
...
{
    ok(gymExport_1.MEMBER_PARTS.includes('memberRecords'), 'a subject-access bundle carries the member’s own record — the thing the request is usually FOR');
    ok(gymExport_1.MEMBER_PARTS.includes('signatures') && gymExport_1.MEMBER_PARTS.includes('agreements'), 'and every waiver they signed, with the wording they signed it against');
    const mine = (0, gymExport_1.memberSlices)(base, 'm1');
    const n = (k) => {
        const s = (0, gymExport_1.partSlice)(mine, k);
        return s.state === 'ready' ? s.rows.length : -1;
    };
    ...
    const all = (0, gymExport_1.memberRowCount)(base, 'm1');
    const q = (0, gymExport_1.memberRowCount)(Q1, 'm1');
    ...
    const bundle = (0, gymExport_1.buildGymExport)({ ...Q1, ...(0, gymExport_1.memberSlices)(Q1, 'm1'), subject: { memberId: 'm1', memberName: 'Sara' } });
    const counted = bundle.manifest.parts
        .filter((p) => gymExport_1.MEMBER_PARTS.includes(p.part))
        .reduce((a, p) => a + (p.rows ?? 0), 0);
    eq(counted, q, 'and the number on the screen is the number of rows in the file');
    ok(bundle.prefix.includes('2026-01-01-to-2026-03-31'), 'a member bundle taken over a period says so in its name too — it is the file that answers a legal request');
}
if (errors.length) {
