...
const screenHelp_1 = require("./screenHelp");
const errors = [];
...
const ALL = Object.keys(screenHelp_1.SCREEN_HELP);
...
same(screenHelp_1.CLIENT_HELP_KEYS, ['home', 'train', 'meals', 'progress', 'me'], 'the client five, in tab order');
eq(screenHelp_1.CLIENT_HELP_KEYS.length, 5, 'there is one card per client tab');
...
eq(screenHelp_1.COACH_HELP_KEYS.length, 5, 'and five for the coach');
for (const k of screenHelp_1.COACH_HELP_KEYS) {
...
eq(ALL.length, screenHelp_1.CLIENT_HELP_KEYS.length + screenHelp_1.COACH_HELP_KEYS.length, 'the record holds the two lists and nothing else');
for (const k of ALL) {
    const inClient = screenHelp_1.CLIENT_HELP_KEYS.includes(k);
    const inCoach = screenHelp_1.COACH_HELP_KEYS.includes(k);
    ok(inClient !== inCoach, `${k} is listed in exactly one of the two populations`);
}
for (const k of ALL) {
    const h = screenHelp_1.SCREEN_HELP[k];
...
/* ── keys ───────────────────────────────────────────────────────────────── */
for (const k of ALL)
    ok((0, screenHelp_1.isHelpKey)(k), `${k} is a key`);
ok(!(0, screenHelp_1.isHelpKey)('dashboard'), 'the route file name is not the key');
ok(!(0, screenHelp_1.isHelpKey)(''), 'the empty string is not a key');
ok(!(0, screenHelp_1.isHelpKey)(null), 'null is not a key');
ok(!(0, screenHelp_1.isHelpKey)(0), 'an index is not a key');
// Object.keys on a plain object, checked with includes: a prototype member must
// not read back as a dismissed screen.
ok(!(0, screenHelp_1.isHelpKey)('toString'), 'toString is not a key');
ok(!(0, screenHelp_1.isHelpKey)('hasOwnProperty'), 'hasOwnProperty is not a key');
/* ── what was dismissed, read out of storage ────────────────────────────── */
same((0, screenHelp_1.dismissedFrom)(null), [], 'nothing stored means nothing dismissed');
same((0, screenHelp_1.dismissedFrom)(undefined), [], 'undefined means nothing dismissed');
same((0, screenHelp_1.dismissedFrom)('home'), [], 'a bare string is not a list');
same((0, screenHelp_1.dismissedFrom)({ home: true }), [], 'an object is not a list');
same((0, screenHelp_1.dismissedFrom)([]), [], 'an empty list is empty');
same((0, screenHelp_1.dismissedFrom)(['home', 'meals']), ['home', 'meals'], 'a stored list reads back');
...
same((0, screenHelp_1.dismissedFrom)(['home', 42, null, 'meals', { k: 'me' }]), ['home', 'meals'], 'the recognisable keys survive a corrupt neighbour');
same((0, screenHelp_1.dismissedFrom)(['home', 'nutrition']), ['home'], 'a key for a screen that no longer exists is dropped');
// Storage round-trips, so a duplicate written once would come back forever.
same((0, screenHelp_1.dismissedFrom)(['home', 'home', 'home']), ['home'], 'a duplicate in storage reads back once');
/* ── dismissing, and staying dismissed ──────────────────────────────────── */
ok(!(0, screenHelp_1.isDismissed)([], 'home'), 'an untouched card shows');
ok((0, screenHelp_1.isDismissed)(['home'], 'home'), 'a dismissed card does not');
ok(!(0, screenHelp_1.isDismissed)(['home'], 'meals'), 'dismissing one card does not dismiss the others');
same((0, screenHelp_1.withDismissed)([], 'home'), ['home'], 'dismissing adds the key');
same((0, screenHelp_1.withDismissed)(['home'], 'meals'), ['home', 'meals'], 'dismissing a second keeps the first');
...
same((0, screenHelp_1.withDismissed)(['home'], 'home'), ['home'], 'dismissing twice writes one key');
eq((0, screenHelp_1.withDismissed)((0, screenHelp_1.withDismissed)(['home'], 'home'), 'home').length, 1, 'and a third time');
...
const before = ['home'];
const after = (0, screenHelp_1.withDismissed)(before, 'meals');
...
ok((0, screenHelp_1.withDismissed)(before, 'home') !== before, 'including when the key was already there');
// The whole round trip, which is what actually runs: dismiss, serialise,
// reload, and the card must still be gone.
let stored = [];
for (const k of ALL)
    stored = (0, screenHelp_1.withDismissed)(stored, k);
const reread = (0, screenHelp_1.dismissedFrom)(JSON.parse(JSON.stringify(stored)));
for (const k of ALL)
    ok((0, screenHelp_1.isDismissed)(reread, k), `${k} survives the round trip through storage`);
