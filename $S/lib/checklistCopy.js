...
exports.planChecklistCopy = planChecklistCopy;
exports.guardChecklistCopy = guardChecklistCopy;
exports.copyBrief = copyBrief;
exports.copyPreview = copyPreview;
 ...
 */
function planChecklistCopy(lines, targets) {
    const wanted = [];
    const seen = new Set();
...
const COPY_OK = { allowed: true, label: null, reason: null };
/**
 ...
 */
function guardChecklistCopy(rosterStatus, existingStatus) {
    switch ((0, loadStatus_1.worstStatus)(rosterStatus, existingStatus)) {
        case 'ready':
            return COPY_OK;
        case 'loading':
            return {
        ...
        case 'partial':
            return {
                ...
                reason: 'What these clients already have came back at its row limit, so a line past the point it stopped would '
        ...
        case 'error':
            return {
                ...
                reason: 'What these clients already have could not be read. An empty list here means the read failed rather '
 ...
 */
function copyBrief(plan, sourceName) {
    ...
    if (plan.writes === 0) {
        return {
            ...
            body: people === 1
                ...
                : `All ${(0, format_1.num)(people)} of them already have every one of these lines, so there is nothing to add.`,
            confirmLabel: 'OK',
            actionable: false,
        };
    }
    ...
    const skipNote = plan.unchanged.length
        ? `\n\n${(0, groupProgram_1.listNames)(plan.unchanged.map((p) => p.name))} `
    ...
    const partialNote = plan.changed.some((p) => p.alreadyThere.length)
        ...
        : '';
    return {
        title: to === 1 ? 'Copy To One Client?' : `Copy To ${(0, format_1.num)(to)} Clients?`,
        body: `${(0, format_1.num)(lines)} ${lines === 1 ? 'line' : 'lines'} from ${sourceName}’s list ${lines === 1 ? 'goes' : 'go'} onto `
            + `${to === 1 ? 'one client’s' : `${(0, format_1.num)(to)} clients’`} daily list, marked as set by you.`
            + partialNote
            + skipNote
        ...
        confirmLabel: to === 1 ? 'Copy To Them' : `Copy To ${(0, format_1.num)(to)}`,
        actionable: true,
    };
}
/**
 ...
 */
function copyPreview(plan) {
    if (!plan.targets.length)
        return null;
    if (plan.writes === 0) {
        return plan.targets.length === 1
            ? `${plan.targets[0].name} already has every one of these lines.`
            : `All ${(0, format_1.num)(plan.targets.length)} of them already have every one of these lines.`;
    }
    const to = plan.changed.length;
    const skipped = plan.unchanged.length;
    return `${(0, format_1.num)(plan.writes)} ${plan.writes === 1 ? 'line' : 'lines'} onto ${(0, format_1.num)(to)} ${to === 1 ? 'client' : 'clients'}`
        + (skipped ? `, and nothing onto ${(0, format_1.num)(skipped)} who already ${skipped === 1 ? 'has' : 'have'} them all.` : '.');
}
