...
exports.heatmapDayLabel = heatmapDayLabel;
exports.heatmapColumnLabel = heatmapColumnLabel;
exports.heatmapSummary = heatmapSummary;
 ...
 */
function heatmapDayLabel(d, count, today) {
    const when = d.toLocaleDateString((0, locale_1.appLocale)(), { weekday: 'short', day: 'numeric', month: 'short' });
    if (d.getTime() > today.getTime())
        return `${when}, still to come`;
    if (count == null)
        return `${when}, not read`;
    if (count === 0)
        return `${when}, no sessions`;
 ...
 */
function heatmapColumnLabel(col, prev) {
    const first = col[0];
    if (!first)
        return null;
    const month = (x) => x.toLocaleDateString((0, locale_1.appLocale)(), { month: 'short' });
    if (!prev || !prev[0])
        return month(first);
...
function heatmapSummary(weeks, known) {
    if (!known)
        return `Training days over the last ${weeks} weeks. Your log has not been read, so none of these squares is an answer yet.`;
