...
const endCoaching_1 = require("./endCoaching");
const errors = [];
...
// sentence "Leave ___?".
ok((0, endCoaching_1.coachLabel)('Sam Rivera') === 'Sam Rivera', 'a real name must survive unchanged');
ok((0, endCoaching_1.coachLabel)('  Sam Rivera  ') === 'Sam Rivera', 'a padded name must be trimmed, not shown padded');
ok((0, endCoaching_1.coachLabel)(null) === 'your coach', 'no name must fall back to the role');
ok((0, endCoaching_1.coachLabel)(undefined) === 'your coach', 'an absent name must fall back to the role');
ok((0, endCoaching_1.coachLabel)('') === 'your coach', 'an empty name must fall back to the role');
ok((0, endCoaching_1.coachLabel)('   ') === 'your coach', 'a whitespace-only name is not a name');
for (const bad of [null, undefined, '', '   ']) {
    const p = (0, endCoaching_1.leaveCoachPrompt)(bad);
...
const named = (0, endCoaching_1.leaveCoachPrompt)('Sam Rivera');
...
ok(/cannot be undone|can’t be undone|can not be undone/i.test(named.body), 'the photo consequence is the one irreversible part and must be named as irreversible');
...
ok(/nothing of yours is deleted|still yours|stays exactly as it is/i.test(named.body), 'the prompt must say the client keeps their own history, or leaving reads as erasing it');
...
const LEFT = { ok: true, ended: true };
const NOTHING = { ok: true, ended: false };
const FAILED = { ok: false, reason: 'The change could not be saved.' };
const left = (0, endCoaching_1.leaveOutcome)(LEFT, 'Sam Rivera');
const nothing = (0, endCoaching_1.leaveOutcome)(NOTHING, 'Sam Rivera');
const failed = (0, endCoaching_1.leaveOutcome)(FAILED, 'Sam Rivera');
ok(left.title !== nothing.title && nothing.title !== failed.title && left.title !== failed.title, 'the three outcomes must not share a title — they are three different states');
...
ok(!/you have left|you.?ve left|no longer see|is closed/i.test(`${failed.title} ${failed.body}`), `a failed call must not read as having succeeded: ${failed.title} — ${failed.body}`);
...
ok(!/you have left|you.?ve left/i.test(`${nothing.title} ${nothing.body}`), 'a call that found no link must not claim the client left anybody');
ok(!/could not|failed|try again/i.test(`${nothing.title} ${nothing.body}`), 'a call that found no link must not be dressed up as a failure — nothing went wrong');
...
for (const r of [LEFT, NOTHING, FAILED]) {
    const o = (0, endCoaching_1.leaveOutcome)(r, null);
...
ok((0, endCoaching_1.endCoachingErrorMessage)('not signed in').includes('not signed in'), 'a signed-out call must say so');
ok(/own account/i.test((0, endCoaching_1.endCoachingErrorMessage)('you cannot end a coaching relationship with yourself')), 'ending with yourself must be explained, not echoed as SQL');
ok(/which coach/i.test((0, endCoaching_1.endCoachingErrorMessage)('no one to end coaching with')), 'a missing id must be explained as not knowing who was meant');
// The unknown case is the one that has cost this codebase debugging time when
// it was flattened. The server's words survive.
const odd = (0, endCoaching_1.endCoachingErrorMessage)('permission denied for table clients');
...
ok((0, endCoaching_1.endCoachingErrorMessage)(null) === 'The change could not be saved.', 'a null reason still produces a sentence');
ok((0, endCoaching_1.endCoachingErrorMessage)('   ') === 'The change could not be saved.', 'a whitespace reason still produces a sentence');
 ...
 */
const dep = (reason) => ({ reason, endedAt: '2026-08-01T00:00:00Z' });
ok((0, endCoaching_1.departureTally)(null) === null, 'an unread list of endings produces no tally at all');
ok((0, endCoaching_1.departureLine)(null, 90) === null, 'and no sentence over it');
ok((0, endCoaching_1.departureLine)((0, endCoaching_1.departureTally)([]), 90) === null, 'nor does an empty book: "0 people have left you" reads as a compliment on a book nobody has been on');
const tally = (0, endCoaching_1.departureTally)([
...
ok(tally.unrecorded === 2, 'a null and a reason this build does not know both land in unrecorded, rather than being dropped or guessed at');
ok(!tally.counts.some((c) => c.reason === 'a-reason-a-later-build-invented'), 'and an unrecognised reason never becomes a row nothing can label');
...
const unsaidOnly = (0, endCoaching_1.departureTally)([dep('unsaid'), dep(null)]);
ok(unsaidOnly.counts.length === 1 && unsaidOnly.counts[0].n === 1 && unsaidOnly.unrecorded === 1, 'an unrecorded ending is never folded in with They Did Not Say');
const line = (0, endCoaching_1.departureLine)(tally, 90);
ok(!/%/.test(line), 'there is no percentage in the line: a share over a small book is noise reading as a trend');
ok(line.includes(endCoaching_1.END_REASON_LABEL.cost.toLowerCase()), 'the commonest reason is named in the coach\u2019s own vocabulary');
...
const allBlank = (0, endCoaching_1.departureLine)((0, endCoaching_1.departureTally)([dep(null), dep(null)]), 90);
ok(/nothing is recorded about why/.test(allBlank), 'a book where nobody has been asked says so, rather than showing an empty list under a heading');
...
for (const r of endCoaching_1.CLIENT_END_REASONS) {
    ok(endCoaching_1.END_REASONS.includes(r), `every reason a client can pick is a reason the coach's list knows — ${r}`);
    ok(!!endCoaching_1.CLIENT_END_REASON_LABEL[r], `and carries a label — ${r}`);
    ok(!!endCoaching_1.CLIENT_END_REASON_NOTE[r], `and a line under it — ${r}`);
}
// The two a client cannot honestly answer.
ok(!endCoaching_1.CLIENT_END_REASONS.includes('coach-ended'), '"My Decision" is a coach speaking about themselves and is not offered to a client');
ok(!endCoaching_1.CLIENT_END_REASONS.includes('unsaid'), '"They Did Not Say" is a fact about the coach\'s asking — a client who will not say has the Skip button');
// First person, and not the coach's wording carried over.
for (const r of endCoaching_1.CLIENT_END_REASONS) {
    ok(!/\bthey\b/i.test(endCoaching_1.CLIENT_END_REASON_NOTE[r]), `the member reads about themselves, not about a third party — ${r}: ${endCoaching_1.CLIENT_END_REASON_NOTE[r]}`);
}
ok(/coach sees this/i.test(endCoaching_1.CLIENT_END_EXPLAINER), 'the member is told who reads it before they write it');
ok(/your own words/i.test(endCoaching_1.CLIENT_END_EXPLAINER), 'and that it is filed as theirs rather than as a guess');
ok(/skip/i.test(endCoaching_1.CLIENT_END_EXPLAINER), 'and that they can leave without answering');
const body = (0, endCoaching_1.clientEndConfirmBody)('Sam');
...
ok(/not refunded here/i.test(body) || /settled with them directly/i.test(body), 'and that money is not settled by this button');
ok(/this coach/.test((0, endCoaching_1.clientEndConfirmBody)(null)), 'an unnamed coach still gets a grammatical sentence');
ok(/this coach/.test((0, endCoaching_1.clientEndConfirmBody)('   ')), 'and so does a blank name');
...
ok(/nothing was changed/i.test((0, endCoaching_1.clientEndOutcomeLine)(false, true, false)), 'a server that found no relationship says so rather than claiming an ending');
ok(/Nothing was recorded about why/i.test((0, endCoaching_1.clientEndOutcomeLine)(true, false, false)), 'skipping is reported as having recorded nothing');
ok(/passed on to them/i.test((0, endCoaching_1.clientEndOutcomeLine)(true, true, true)), 'a stored reason says it reached the coach');
const lost = (0, endCoaching_1.clientEndOutcomeLine)(true, true, false);
...
{
    const note = (0, endCoaching_1.replaceCoachNote)('Dana Ruiz', 'Sam Okafor');
    ...
    const anon = (0, endCoaching_1.replaceCoachNote)(null, '   ');
