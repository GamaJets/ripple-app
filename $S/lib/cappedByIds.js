...
exports.readCappedByIds = readCappedByIds;
 ...
 */
async function readCappedByIds(ids, page, opts = {}) {
    const unique = (0, idLookup_1.uniqueIds)(ids);
    if (!unique.length)
        return { rows: [], truncated: false, error: null };
    const cap = Math.max(1, Math.floor(opts.cap ?? rowCap_1.ROW_CAP));
    const out = [];
    let truncated = false;
    for (const chunk of (0, idLookup_1.chunkIds)(unique, opts.chunk ?? idLookup_1.ID_CHUNK)) {
        ...
        if (error)
            return { rows: out, truncated, error };
        const page1 = (0, rowCap_1.capped)(data, cap);
        if (page1.truncated)
            truncated = true;
