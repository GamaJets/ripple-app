...
const gymOrders_1 = require("./gymOrders");
const errors = [];
 ...
 */
{
    const rows = [
        ...
        order({ id: 'stuck', status: 'pending', membershipId: null, createdAt: iso(gymOrders_1.PENDING_STALE_MS) }),
        order({ id: 'gone', status: 'abandoned', membershipId: null }),
    ];
    const tr = (0, gymOrders_1.orderTrouble)(rows, NOW);
    eq(tr.failed.map((o) => o.id).join(','), 'bad', 'a failed order is money taken with nothing granted');
    eq(tr.paidWithNothing.map((o) => o.id).join(','), 'quiet', 'and so is a "paid" row with neither a membership nor a pass behind it — the same harm, recorded by a row that does not say so');
    eq(tr.stuck.map((o) => o.id).join(','), 'stuck', 'a pending order older than a Stripe session can be is a webhook that never arrived');
    ok(!tr.stuck.some((o) => o.id === 'fresh'), 'an hour-old pending order is a customer deciding, not a fault');
    ok(!tr.failed.some((o) => o.id === 'gone') && !tr.stuck.some((o) => o.id === 'gone'), 'abandoned is nobody charged and nothing to do');
    eq((0, gymOrders_1.orderTrouble)(rows, NOW).failed.length + (0, gymOrders_1.orderTrouble)(rows, NOW).stuck.length + (0, gymOrders_1.orderTrouble)(rows, NOW).paidWithNothing.length, 3, 'three rows need a person, out of six');
    const unparseable = (0, gymOrders_1.orderTrouble)([order({ id: 'x', status: 'pending', membershipId: null, createdAt: 'not a date' })], NOW);
    eq(unparseable.stuck.length, 0, 'an unreadable timestamp is not evidence of age — it must not become a permanent false alarm');
    eq(gymOrders_1.ORDER_STATUS_LABEL.failed, 'Paid, not granted', 'the label says what happened, not "failed", which reads as a declined card');
}
/* ── CURRENCY ─────────────────────────────────────────────────────────────
 * A figure blended across two currencies is not a bigger number.
 */
{
    const pots = (0, gymOrders_1.paidPots)([
    ...
    ok(!pots.some((p) => p.currency.trim() === ''), 'a row with no currency is not money that can be added to money');
    ok(gbp.cents === 7500, 'a NaN amount does not poison the pot it landed in');
}
/* ── COUNTS ───────────────────────────────────────────────────────────────
 ...
 */
{
    ...
    ok(by.some(([k]) => k === 'refunded'), 'an unrecognised status shows up rather than being silently excluded');
    eq(by.reduce((a, [, n]) => a + n, 0), 4, 'and the counts add up to the rows they came from');
}
/* ── LINE ─────────────────────────────────────────────────────────────────── */
{
    eq((0, gymOrders_1.orderLine)({ kind: 'membership', intent: 'new' }), 'Membership · new', 'a first membership');
    eq((0, gymOrders_1.orderLine)({ kind: 'membership', intent: 'renew' }), 'Membership · renewal', 'an extension of the plan they are on');
    eq((0, gymOrders_1.orderLine)({ kind: 'membership', intent: 'upgrade' }), 'Membership · upgrade', 'a move to a different plan');
    eq((0, gymOrders_1.orderLine)({ kind: 'pass', intent: 'new' }), 'Pass', 'a pass supersedes nothing, so its intent is never worth printing');
}
if (errors.length) {
