...
const foodSearch_1 = require("./foodSearch");
...
const common = (n, k = 100) => ({ n, k, p: 1, c: 1, f: 1 });
const dish = (id, name, cuisine) => ({ id, name, cuisine, kcal: 500, protein: 20, carbs: 50, fat: 20 });
const branded = (name, serving = '100 g') => ({ name, kcal: 200, protein: 10, carbs: 20, fat: 5, serving });
...
const labelled = (0, foodSearch_1.mergeFoodResults)('chicken', {
...
eq(labelled.find((r) => r.source === 'restaurant')?.label, 'Restaurant · American', 'a restaurant dish carries its cuisine in the label');
...
eq((0, foodSearch_1.mergeFoodResults)('chicken', {
    common: [common('Chicken Breast, Grilled (150g)')],
    restaurant: [dish('caesar-chx', 'Chicken Caesar salad', 'American')],
...
eq((0, foodSearch_1.mergeFoodResults)('latte', {
    common: [common('Latte, Regular (350ml)')],
    restaurant: [dish('latte', 'Latte deluxe', 'Cafe')],
    branded: [branded('Latte Macchiato')],
}).map((r) => r.source).join(','), 'common,restaurant,branded', 'inside one tier a one-word query orders common, restaurant, branded');
...
const tesco = (0, foodSearch_1.mergeFoodResults)('tesco greek yogurt', {
...
eq((0, foodSearch_1.mergeFoodResults)('protein bar', {
    common: [common('Protein Bar')],
    restaurant: [],
    branded: [branded('Protein Bar Chocolate', '60 g')],
}).map((r) => r.source).join(','), 'branded,common', 'a multi-word query orders branded ahead of common inside a tier');
...
eq((0, foodSearch_1.mergeFoodResults)('greek yogurt', {
    common: [common('Greek Yogurt, 0% (200g)')],
    restaurant: [],
...
const dupe = (0, foodSearch_1.mergeFoodResults)('banana', {
...
eq((0, foodSearch_1.mergeFoodResults)('croissant', {
...
eq((0, foodSearch_1.mergeFoodResults)('yogurt', {
...
const withRemote = (0, foodSearch_1.mergeFoodResults)('chicken', {
    common: (0, foods_1.searchCommonFoods)('chicken'),
    restaurant: (0, restaurant_1.searchDishes)('chicken'),
    branded: [branded('Tesco Chicken Breast')],
});
const withoutRemote = (0, foodSearch_1.mergeFoodResults)('chicken', {
    common: (0, foods_1.searchCommonFoods)('chicken'),
    restaurant: (0, restaurant_1.searchDishes)('chicken'),
    branded: [],
});
...
eq(JSON.stringify(names(withoutRemote)), JSON.stringify(names(withRemote).filter((n) => n !== 'Tesco Chicken Breast (100 g)')), 'the local rows keep their order when the branded search fails');
...
eq((0, foodSearch_1.mergeFoodResults)('oreo', { ...empty, branded: [branded('Oreo Original', '3 biscuits')] }).length, 1, 'a branded row stands on its own when nothing local matched');
eq((0, foodSearch_1.mergeFoodResults)('anything', empty).length, 0, 'three empty sources merge to an empty list');
/* ── a dish logged from search is the standard serving ─────────────────── */
const ramen = restaurant_1.DISHES.find((d) => d.id === 'ramen');
const fromSearch = (0, foodSearch_1.mergeFoodResults)('ramen', { ...empty, restaurant: [ramen] })[0];
...
const runA = (0, foodSearch_1.mergeFoodResults)('chicken', {
    common: (0, foods_1.searchCommonFoods)('chicken'),
    restaurant: (0, restaurant_1.searchDishes)('chicken'),
    branded: [branded('Tesco Chicken Breast'), branded('Chicken Soup')],
});
const runB = (0, foodSearch_1.mergeFoodResults)('chicken', {
    common: (0, foods_1.searchCommonFoods)('chicken'),
    restaurant: (0, restaurant_1.searchDishes)('chicken'),
    branded: [branded('Tesco Chicken Breast'), branded('Chicken Soup')],
});
...
ok((0, foods_1.searchCommonFoods)('chicken breast').length > 0, '"chicken breast" matches the common table');
ok((0, foods_1.searchCommonFoods)('greek yogurt').length > 0, '"greek yogurt" matches the common table');
eq((0, foods_1.searchCommonFoods)('chicken breast')[0].n, 'Chicken Breast, Grilled (150g)', 'a phrase query leads with the food it named');
// A word typed that lands nowhere still means no.
eq((0, foods_1.searchCommonFoods)('chicken zzz').length, 0, 'every word typed has to land somewhere');
// And the single-word behaviour it had before is unchanged.
eq((0, foods_1.searchCommonFoods)('rice')[0].n, 'Brown Rice, Cooked (1 cup, 160g)', 'a one-word query still ranks as it did');
if (errors.length) {
    console.error(errors.join('\n'));
    process.exit(1);
}
console.log(`foodSearch: ok (${restaurant_1.DISHES.length} dishes merged with the common table)`);
