...
const deviceSleepStore_1 = require("./deviceSleepStore");
const errors = [];
const ok = (cond, msg) => { if (!cond)
    errors.push(msg); };
const eq = (a, b, msg) => ok(Object.is(a, b), `${msg} — got ${JSON.stringify(a)}, wanted ${JSON.stringify(b)}`);
/** A fully-known input, so each case below can vary exactly one thing. */
const known = { avgSleepHours: 8, hydrationPct: 1, recoveryPct: 100, workoutsLast2Days: 0 };
const score = (over) => (0, readiness_1.readinessScore)({ ...known, ...over });
...
eq(score({ avgSleepHours: null }), null, 'no night on record yields NO score — not the 20 that used to read as under-recovered');
eq(score({ avgSleepHours: 0 }), null, 'zero hours is the absence of a night, not a catastrophic one');
eq(score({ avgSleepHours: NaN }), null, 'and an unparseable figure is an absence too — NaN must never reach the arithmetic');
eq((0, readiness_1.readinessScore)({ avgSleepHours: null, hydrationPct: 0.9, recoveryPct: 90, workoutsLast2Days: 0 }), null, 'the other three signals cannot carry a score between them');
...
eq(score({ workoutsLast2Days: null }), null, 'AN UNREAD TRAINING LOG YIELDS NO SCORE — it must not be scored as a member who has not trained');
eq(score({ workoutsLast2Days: NaN }), null, 'and neither does a load that arrived as NaN');
{
    ...
    ok(restedForReal != null && restedForReal.score === 100, 'a KNOWN zero sessions still scores full marks — the null channel narrows nothing for somebody who genuinely has not trained');
    ok(restedForReal != null && restedForReal.tip.length > 0 && /push|PR/i.test(restedForReal.tip), 'and it carries the "go and push" tip — which is exactly why an unread log must not be able to reach it');
}
...
// and "this person drank nothing" has been lost.
{
    ...
    ok(untracked != null && untracked.score === 100, 'no hydration figure is not dehydration — the remaining signals are rescaled, not docked 30');
    ok(drankNothing != null && drankNothing.score < 100, 'but a hydration figure that says zero genuinely counts against the score');
    ok(untracked.score > drankNothing.score, 'THE UNTRACKED CASE MUST OUTSCORE THE TRACKED-ZERO CASE — collapsing them is how a missing input becomes a bad one');
...
// or deflate readiness for most of the app rather than for a minority.
{
    ...
    ok(noDevice != null && noDevice.score === 100, 'NO CONNECTED DEVICE IS NOT A BAD RECOVERY — the remaining signals are rescaled, not docked 40');
    ok(wrecked != null && wrecked.score < 100, 'but a device reporting zero recovery genuinely counts against the score');
    ok(noDevice.score > wrecked.score, 'the two must never converge — collapsing them turns "no strap" into "wrecked"');
    ok(!noDevice.from.includes('recovery'), 'and a signal nobody supplied is never listed as though it had been');
    eq(score({ recoveryPct: 140 }).score, score({ recoveryPct: 100 }).score, 'a vendor figure above 100 is clamped rather than extrapolated — the field is theirs, not ours');
    eq(score({ recoveryPct: -5 }).score, score({ recoveryPct: 0 }).score, 'and one below zero is clamped the same way rather than subtracting from the other signals');
    const nan = score({ recoveryPct: NaN });
    eq(nan.score, noDevice.score, 'NaN is an absence, exactly as it is for sleep — it must never reach the arithmetic');
...
// cannot supply it.
eq((0, readiness_1.readinessScore)({ avgSleepHours: 7, hydrationPct: null, recoveryPct: null, workoutsLast2Days: 1 }).score, Math.round(((7 / 8) * 50 + 20) / 70 * 100), 'a member with neither water goal nor strap scores over sleep and load alone, exactly as before');
// ── the score says what it is made of ─────────────────────────────────────
{
    ...
    ok(!twoOf.from.includes('hydration'), 'a signal that was not scored is never listed as though it had been');
    ...
    const said = (0, readiness_1.readinessMadeOf)(twoOf);
    ok(/sleep/i.test(said) && /session/i.test(said), 'the sentence names the signals the number is built from');
    ok(!/hydration|water/i.test(said), 'and it does NOT mention the missing one — "no hydration figure" reads as a deduction, and nothing was deducted');
    ok(!said.endsWith('.'), 'no trailing full stop: every caller appends it to a tip that already has one');
    ok((0, readiness_1.readinessMadeOf)(all).includes('hydration'), 'a full score lists hydration among its parts');
}
    ...
    for (const w of [null, 0, 0.5, 1, 3]) {
        for (const l of [0, 1, 2, 5]) {
            for (const rec of [null, 0, 55, 100]) {
                const r = (0, readiness_1.readinessScore)({ avgSleepHours: h, hydrationPct: w, recoveryPct: rec, workoutsLast2Days: l });
                ...
                ok(r != null && r.from.includes('sleep') && r.from.includes('load'), `${h}h / ${w} / ${rec} / ${l} always names sleep and load, which are never optional`);
            }
        }
    }
}
// A hydration figure over 1 is a client past their goal, not 130% recovered.
eq(score({ hydrationPct: 3 }).score, score({ hydrationPct: 1 }).score, 'drinking three times the goal scores the same as meeting it — the signal is clamped, not extrapolated');
// ── the tone thresholds, which are what the screen colours and words on ───
{
    const great = (0, readiness_1.readinessScore)({ avgSleepHours: 8, hydrationPct: 1, recoveryPct: null, workoutsLast2Days: 0 });
    const middling = (0, readiness_1.readinessScore)({ avgSleepHours: 5.5, hydrationPct: 0.5, recoveryPct: null, workoutsLast2Days: 1 });
    const bad = (0, readiness_1.readinessScore)({ avgSleepHours: 4, hydrationPct: 0.2, recoveryPct: null, workoutsLast2Days: 3 });
...
const measured = (night, minutes) => ({ night, outcome: 'measured', minutesAsleep: minutes });
const unknownNight = (night) => ({ night, outcome: 'unknown', minutesAsleep: null });
{
    const s = (0, readiness_1.readinessSleep)([measured('2026-09-01', 480), measured('2026-08-31', 420)], [], 3);
...
{
    const s = (0, readiness_1.readinessSleep)([], [], 3);
...
}
{
    ...
    const s = (0, readiness_1.readinessSleep)([unknownNight('2026-09-01'), measured('2026-08-31', 480)], [], 3);
...
}
{
    ...
    const s = (0, readiness_1.readinessSleep)([measured('2026-09-01', 300)], [{ at, hours: 9 }], 3);
...
}
{
    ...
    const s = (0, readiness_1.readinessSleep)([measured('2026-09-01', 480)], [{ at, hours: 6 }], 3);
...
{
    const s = (0, readiness_1.readinessSleep)([measured('2026-09-01', 480), measured('2026-08-31', 480), measured('2026-08-30', 480), measured('2026-08-29', 60)], [], 3);
...
{
    const keep = (0, deviceSleepStore_1.storableNights)([
        night(),
...
}
{
    ...
    const keep = (0, deviceSleepStore_1.storableNights)([night({ source: null })]);
    ...
    eq((0, deviceSleepStore_1.storableNights)([night({ minutesAsleep: 0 })]).length, 0, 'and zero minutes is not a measurement of a sleepless night');
}
{
    const stored = [{
    ...
    const out = (0, deviceSleepStore_1.withStored)(fresh, stored);
...
}
{
    ...
    const out = (0, deviceSleepStore_1.withStored)([night({ minutesAsleep: 470, source: reading({ minutesAsleep: 470 }) })], stored);
...
}
{
    ...
    const out = (0, deviceSleepStore_1.withStored)([night()], stored);
    eq(out.length, 1, 'a stored night outside the window is not pushed onto a list whose length other screens count');
    eq((0, deviceSleepStore_1.withStored)([], stored).length, 0, 'and an empty window stays empty');
}
{
    ...
    const allFailed = ['2026-09-01', '2026-08-31', '2026-08-30'].map((n) => night({ night: n, outcome: 'unknown', minutesAsleep: null, source: null, failed: ['whoop'] }));
    const stored = ['2026-09-01', '2026-08-31', '2026-08-30'].map((n) => ({
        night: n, minutesAsleep: 480, provider: 'whoop', sourceId: 'whoop',
        sourceName: 'WHOOP', family: 'whoop', basis: 'asleep',
    }));
    eq((0, readiness_1.readinessSleep)(allFailed, [], 3).avgHours, null, 'without the kept nights a total read failure leaves readiness nothing to score — which is correct, and is why it used to disappear');
    const backed = (0, readiness_1.readinessSleep)((0, deviceSleepStore_1.withStored)(allFailed, stored), [], 3);
    eq(backed.avgHours, 8, 'WITH THEM, THE SCORE SURVIVES AN OFFLINE MORNING — from real nights real devices really measured');
    const r = (0, readiness_1.readinessScore)({ avgSleepHours: backed.avgHours, hydrationPct: null, recoveryPct: null, workoutsLast2Days: 0 });
...
// ── the row mapping, which is where a string becomes a NaN ────────────────
{
    ...
    const n = (0, deviceSleepStore_1.rowToStored)(row);
    ...
    eq((0, deviceSleepStore_1.rowToStored)({ ...row, basis: 'in-bed' })?.basis, 'in-bed', 'including "in bed", which runs longer than asleep and is labelled differently on screen');
    eq((0, deviceSleepStore_1.rowToStored)({ ...row, source_name: '' }), null, 'a row with no source name is dropped rather than shown as an unattributed figure');
    eq((0, deviceSleepStore_1.rowToStored)({ ...row, minutes_asleep: 0 }), null, 'and zero minutes is not a night');
    eq((0, deviceSleepStore_1.rowToStored)({ ...row, minutes_asleep: null }), null, 'nor is a null one');
    eq((0, deviceSleepStore_1.rowToStored)({ ...row, night: 'last tuesday' }), null, 'a night that is not a date is not a night');
    eq((0, deviceSleepStore_1.rowToStored)(null), null, 'and nothing at all is nothing at all');
}
{
    const r = (0, deviceSleepStore_1.storedToRow)('u-1', {
    ...
    const back = (0, deviceSleepStore_1.rowToStored)(r);
