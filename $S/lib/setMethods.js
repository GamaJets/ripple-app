...
exports.DEFAULT_METHOD = exports.SET_METHODS = void 0;
exports.methodFor = methodFor;
exports.otherMethodsHint = otherMethodsHint;
exports.countsToVolume = countsToVolume;
...
exports.DEFAULT_METHOD = 'normal';
const BY_ID = new Map(exports.SET_METHODS.map((m) => [m.id, m]));
/**
...
function methodFor(id) {
    ...
    if (found)
        return { method: found, known: true };
    return { method: BY_ID.get(exports.DEFAULT_METHOD), known: false };
}
/**
 ...
 * listing twelve.
 */
function otherMethodsHint(currentId, show = 3) {
    const current = methodFor(currentId).method.id;
    const others = exports.SET_METHODS.filter((m) => m.id !== current);
    if (others.length === 0)
        return '';
 ...
 */
function restAfter(id, exerciseRestSec) {
    const { method } = methodFor(id);
    if (method.restsAfter === false)
        return 0;
    if (method.restsAfter === true)
        return Math.max(0, Math.round(exerciseRestSec));
...
function badgeFor(id) {
    ...
    const { method } = methodFor(id);
    if (method.id === exports.DEFAULT_METHOD)
        return null;
