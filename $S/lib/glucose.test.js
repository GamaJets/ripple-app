...
const near = (a, b, msg) => ok(Math.abs(a - b) < 0.05, `${msg} — got ${a}, wanted ~${b}`);
// ── units ───────────────────────────────────────────────────────────────────
near((0, glucose_1.mmolToMgdl)(5.5), 99.1, '5.5 mmol/L is about 99 mg/dL');
near((0, glucose_1.mgdlToMmol)(99), 5.49, '99 mg/dL is about 5.5 mmol/L');
near((0, glucose_1.mgdlToMmol)((0, glucose_1.mmolToMgdl)(7.3)), 7.3, 'the conversion round-trips');
// ── what counts as a reading at all ─────────────────────────────────────────
eq((0, glucose_1.plausible)(5.5), true, 'an ordinary reading is plausible');
eq((0, glucose_1.plausible)(0.2), false, 'below the floor is a broken import');
eq((0, glucose_1.plausible)(99), false, 'a mg/dL number under mmol/L is not plausible');
eq((0, glucose_1.plausible)(NaN), false, 'NaN is not a reading');
eq((0, glucose_1.plausible)(null), false, 'null is not a reading');
eq((0, glucose_1.plausible)('5.5'), false, 'a string is not a reading');
// ── an absent reading renders as a dash, never as a number ──────────────────
eq((0, glucose_1.formatGlucose)(null, 'mmol/L'), '—', 'no reading is a dash, not 0.0');
eq((0, glucose_1.formatGlucose)(undefined, 'mg/dL'), '—', 'undefined is a dash, not 0');
eq((0, glucose_1.formatGlucose)(NaN, 'mmol/L'), '—', 'NaN is a dash');
eq((0, glucose_1.formatGlucose)(5.5, 'mmol/L'), '5.5', 'mmol/L carries one decimal');
eq((0, glucose_1.formatGlucose)(5.5, 'mg/dL'), '99', 'mg/dL is whole — no invented precision');
// ── bands are descriptive, and "unknown" is a real answer ───────────────────
eq((0, glucose_1.band)(null), 'unknown', 'nothing to band is not "typical"');
eq((0, glucose_1.band)(glucose_1.TYPICAL_LOW_MMOL - 0.1), 'below', 'under the quoted floor');
eq((0, glucose_1.band)(glucose_1.TYPICAL_LOW_MMOL), 'typical', 'the floor itself is inside');
eq((0, glucose_1.band)(glucose_1.TYPICAL_HIGH_MMOL), 'typical', 'the ceiling itself is inside');
eq((0, glucose_1.band)(glucose_1.TYPICAL_HIGH_MMOL + 0.1), 'above', 'over the quoted ceiling');
...
const parsed = (0, glucose_1.parseHealthSamples)(raw);
...
eq((0, glucose_1.parseHealthSamples)(null).length, 0, 'a failed read is an empty list, not a throw');
eq((0, glucose_1.parseHealthSamples)({}).length, 0, 'a non-array is an empty list');
// A reading with no id is kept — somebody may have typed two identical values.
eq((0, glucose_1.parseHealthSamples)([
...
near((0, glucose_1.mmolFromLevel)({ inMillimolesPerLiter: 5.5, inMilligramsPerDeciliter: 99.1 }) ?? 0, 5.5, 'mmol/L is taken as-is when the store supplies it');
near((0, glucose_1.mmolFromLevel)({ inMilligramsPerDeciliter: 99.1 }) ?? 0, 5.5, 'mg/dL alone is converted, and to the mmol/L figure — not left at 99');
near((0, glucose_1.mmolFromLevel)({ inMillimolesPerLiter: 5.5 }) ?? 0, 5.5, 'mmol/L alone needs no conversion');
...
near(((0, glucose_1.mmolFromLevel)({ inMilligramsPerDeciliter: 7.3 * glucose_1.MGDL_PER_MMOL }) ?? 0)
    - ((0, glucose_1.mmolFromLevel)({ inMillimolesPerLiter: 7.3 }) ?? 0), 0, 'the two units describe the same reading, whichever one survives the bridge');
eq((0, glucose_1.mmolFromLevel)(null), null, 'no level is null, never 0');
eq((0, glucose_1.mmolFromLevel)({}), null, 'an empty level is null, never 0');
eq((0, glucose_1.mmolFromLevel)({ inMillimolesPerLiter: 'five' }), null, 'a non-number level is null');
...
near((0, glucose_1.mmolFromLevel)({ inMillimolesPerLiter: NaN, inMilligramsPerDeciliter: 99.1 }) ?? 0, 5.5, 'a NaN mmol/L falls back to mg/dL and converts it — it does not read 99.1 as mmol/L');
eq((0, glucose_1.mmolFromLevel)({ inMillimolesPerLiter: NaN, inMilligramsPerDeciliter: NaN }), null, 'both fields broken is null, never 0');
const hcRecords = [
...
const hc = (0, glucose_1.parseHealthConnectRecords)(hcRecords);
...
eq((0, glucose_1.parseHealthConnectRecords)(null).length, 0, 'a failed read is an empty list, not a throw');
eq((0, glucose_1.parseHealthConnectRecords)({ records: [] }).length, 0, 'the response object is not the array');
...
const idless = (0, glucose_1.parseHealthConnectRecords)([
...
eq((0, glucose_1.unsaved)(idless, []).length, 0, 'and is never sent for import');
...
eq(JSON.stringify((0, glucose_1.parseHealthConnectRecords)([
...
])), JSON.stringify((0, glucose_1.parseHealthSamples)([
...
const r = (at, mmol) => ({ id: null, at, mmol, externalId: at, sourceName: null });
const meals = [
...
const paired = (0, glucose_1.pairMeals)(meals, readings);
...
const noBase = (0, glucose_1.pairMeals)([meals[0]], [r('2026-08-31T08:45:00Z', 8.2)]);
...
const stale = (0, glucose_1.pairMeals)([meals[0]], [r('2026-08-31T05:30:00Z', 5.0), r('2026-08-31T08:45:00Z', 8.2)]);
eq(stale[0].before, null, 'a stale baseline is not used');
// A meal with no readings at all is still listed, with nothing claimed.
const bare = (0, glucose_1.pairMeals)([meals[0]], []);
...
const few = (0, glucose_1.summarise)(readings);
...
const many = [];
for (let i = 0; i < glucose_1.MIN_FOR_PERCENT; i++) {
    many.push(r(`2026-08-31T${String(i).padStart(2, '0')}:00:00Z`, i < 6 ? 5.5 : 9.0));
}
eq((0, glucose_1.summarise)(many).inTypicalPct, 50, 'half in the quoted range');
// A garbage value that reached the list is excluded from the arithmetic.
eq((0, glucose_1.summarise)([r('2026-08-31T08:00:00Z', 5.0), { id: null, at: '2026-08-31T09:00:00Z', mmol: 0, externalId: null, sourceName: null }]).count, 1, 'an implausible value does not drag the average down');
// ── dedupe against what is already stored ───────────────────────────────────
eq((0, glucose_1.unsaved)([r('2026-08-31T08:00:00Z', 5.0)], ['2026-08-31T08:00:00Z']).length, 0, 'an already-stored sample is not sent again');
eq((0, glucose_1.unsaved)([r('2026-08-31T08:00:00Z', 5.0)], []).length, 1, 'a new sample is sent');
eq((0, glucose_1.unsaved)([{ id: null, at: '2026-08-31T08:00:00Z', mmol: 5.0, externalId: null, sourceName: null }], []).length, 0, 'a hand-typed reading is never re-imported from Health');
// ── typed input ─────────────────────────────────────────────────────────────
eq((0, glucose_1.parseTyped)('5.5', 'mmol/L'), 5.5, 'an ordinary mmol/L reading');
eq((0, glucose_1.parseTyped)('99', 'mg/dL'), 5.5, 'mg/dL is converted on the way in');
eq((0, glucose_1.parseTyped)('99', 'mmol/L'), null, 'a mg/dL number typed under mmol/L is refused, not stored');
eq((0, glucose_1.parseTyped)('', 'mmol/L'), null, 'nothing typed is nothing stored');
eq((0, glucose_1.parseTyped)('0', 'mmol/L'), null, 'zero is not a reading');
eq((0, glucose_1.parseTyped)('-5', 'mmol/L'), null, 'negative is not a reading');
eq((0, glucose_1.parseTyped)('abc', 'mmol/L'), null, 'words are not a reading');
eq((0, glucose_1.parseTyped)(' 6.2 ', 'mmol/L'), 6.2, 'surrounding whitespace is tolerated');
...
eq((0, glucose_1.parseTyped)('5,5', 'mmol/L'), 5.5, 'a decimal comma is a decimal point');
eq((0, glucose_1.parseTyped)('12,7', 'mmol/L'), 12.7, 'including on a high reading');
eq((0, glucose_1.parseTyped)('5,5,5', 'mmol/L'), null, 'two commas are a slip or a separator, and are not guessed at');
if (errors.length) {
