...
const reminderPlan_1 = require("./reminderPlan");
const errors = [];
const ok = (cond, msg) => { if (!cond)
    errors.push(msg); };
const eq = (a, b, msg) => ok(Object.is(a, b), `${msg} — got ${JSON.stringify(a)}, wanted ${JSON.stringify(b)}`);
const saved = (over = {}) => ({ ...reminderPlan_1.EMPTY_SAVED, ...over });
...
{
    eq(reminderPlan_1.EVERY_DAY.length, 7, 'a week is seven days');
    eq(reminderPlan_1.EVERY_DAY[0], 1, 'AND IT STARTS AT 1 = SUNDAY, which is expo-notifications’ convention and not JavaScript’s');
    eq(reminderPlan_1.EVERY_DAY[6], 7, 'ending at 7 = Saturday');
    eq(reminderPlan_1.DAY_LABEL[1], 'Sun', 'the labels are indexed by the same number, so DAY_LABEL[w] is always right');
    eq(reminderPlan_1.DAY_LABEL[2], 'Mon', 'Monday is 2');
    eq(reminderPlan_1.WEEKDAYS_ONLY.join(','), '2,3,4,5,6', 'and Monday to Friday is 2 through 6');
}
...
{
    const old = (0, reminderPlan_1.savedFromStored)(JSON.stringify({
    ...
    const emptied = (0, reminderPlan_1.savedFromStored)(JSON.stringify({ hydrationDays: [], supps: [{ id: 'r1', name: 'X', hour: 8, minute: 0, days: [] }] }));
...
{
    eq((0, reminderPlan_1.savedFromStored)(null).supps.length, 0, 'nothing stored means the defaults');
    eq((0, reminderPlan_1.savedFromStored)('not json').every, reminderPlan_1.EMPTY_SAVED.every, 'and so does a corrupt blob, rather than a throw');
    const bad = (0, reminderPlan_1.savedFromStored)(JSON.stringify({ every: 99, startH: -4, endH: 40, supps: [{ name: '   ' }, { name: 'Ok', hour: 61 }] }));
    eq(bad.every, reminderPlan_1.EMPTY_SAVED.every, 'an out-of-range interval falls back');
    eq(bad.startH, reminderPlan_1.EMPTY_SAVED.startH, 'as does an impossible hour');
    ...
    const days = (0, reminderPlan_1.savedFromStored)(JSON.stringify({ hydrationDays: [1, 9, 'Mon', 3] })).hydrationDays;
    eq(days.join(','), '1,3', 'rubbish inside a day list is dropped and the real days kept');
}
// ── nothing is scheduled on no days ──────────────────────────────────────
{
    eq((0, reminderPlan_1.plannedReminders)(saved({ hydrationDays: [] })).length, 0, 'hydration on no days schedules nothing');
    eq((0, reminderPlan_1.plannedReminders)(saved({ hydration: false })).length, 0, 'and neither does hydration switched off');
    const off = (0, reminderPlan_1.plannedReminders)(saved({ hydration: false, fixed: { training: { on: true, hour: 18, minute: 0, days: [] } } }));
    eq(off.length, 0, 'a training reminder on no days schedules nothing either');
    const offSwitch = (0, reminderPlan_1.plannedReminders)(saved({ hydration: false, fixed: { training: { on: false, hour: 18, minute: 0, days: [...reminderPlan_1.EVERY_DAY] } } }));
    eq(offSwitch.length, 0, 'nor does one that is switched off, whatever days it remembers');
}
// ── the hydration window ─────────────────────────────────────────────────
{
    const p = (0, reminderPlan_1.plannedReminders)(saved({ hydration: true, every: 3, startH: 9, endH: 21, hydrationDays: [...reminderPlan_1.EVERY_DAY] }));
    ...
    eq((0, reminderPlan_1.plannedReminders)(saved({ startH: 21, endH: 9 })).length, 0, 'A BACKWARDS WINDOW SCHEDULES NOTHING and is not quietly swapped round');
}
...
{
    const mwf = [2, 4, 6];
    const p = (0, reminderPlan_1.plannedReminders)(saved({ hydration: false, fixed: { training: { on: true, hour: 18, minute: 30, days: mwf } } }));
    eq(p.length, 1, 'a training reminder on three days is ONE reminder');
    eq((0, reminderPlan_1.plannedNotificationCount)(p), 3, 'and THREE scheduled notifications');
...
{
    const all = (0, reminderPlan_1.plannedReminders)(saved({
        hydration: false,
        fixed: {
            training: { on: true, hour: 18, minute: 0, days: [...reminderPlan_1.EVERY_DAY] },
...
{
    eq((0, reminderPlan_1.daysLabel)([]), 'Off', 'no days is off, and says so');
    eq((0, reminderPlan_1.daysLabel)([...reminderPlan_1.EVERY_DAY]), 'Every day', 'all seven is not a list of seven abbreviations');
    eq((0, reminderPlan_1.daysLabel)([...reminderPlan_1.WEEKDAYS_ONLY]), 'Weekdays', 'Monday to Friday has a name');
    eq((0, reminderPlan_1.daysLabel)([1, 7]), 'Weekends', 'and so does Saturday and Sunday');
    ...
    eq((0, reminderPlan_1.daysLabel)([1, 2, 4]), 'Sun, Mon, Wed', 'anything else lists the days in the order the week is drawn');
    eq((0, reminderPlan_1.daysLabel)([4, 1, 2]), 'Sun, Mon, Wed', 'whatever order they were toggled on in');
}
if (errors.length) {
