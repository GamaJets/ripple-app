 ...
 *  allowed to. */
function fakeSb(error = null, count = 1) {
    const deleted = [];
    const opts = [];
    return {
        deleted,
...
async function main() {
    ...
    {
        eq((0, gymPasses_1.redemptionUndoBlocker)(red()), null, 'a visit taken at the door can be put back');
        eq((0, gymPasses_1.redemptionUndoBlocker)(red({ classId: 'c1' })), null, 'and so can one taken against a class — a class is still the door as far as the pass is concerned');
        const f = fakeSb();
        await (0, gymPasses_1.undoRedemption)(f.sb, red());
        ...
        eq(JSON.stringify(f.opts[0]), JSON.stringify({ count: 'exact' }), 'and the server is asked HOW MANY rows it deleted — without that there is no answer to read');
    }
    /* ── a credit that paid for a one-to-one is refused ─────────────────────── */
    {
        const r = red({ sessionId: 's9' });
        const why = (0, gymPasses_1.redemptionUndoBlocker)(r);
        ...
        let threw = null;
        try {
            await (0, gymPasses_1.undoRedemption)(f.sb, r);
        }
        catch (e) {
            threw = e?.message ?? '';
        }
    ...
    /* ── a refused delete is not reported as an undo ────────────────────────── */
    {
        ...
        ok(threw, 'supabase-js RESOLVES on a database error, so a refused delete has to be read off the result — otherwise the desk is told a visit went back that is still spent');
    }
     ...
     * on their card by a trigger that never ran, with the credit still spent. */
    {
        ...
        ok(threw !== null, 'A DELETE THAT MATCHED NO ROW IS NOT AN UNDO — 204 with no error is what a refusal looks like here');
        ok(/matched no rows/.test(threw ?? ''), `and the reason says the server matched nothing rather than blaming the network — got ${JSON.stringify(threw)}`);
    }
    {
