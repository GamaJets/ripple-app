...
const lastTime_1 = require("./lastTime");
...
const daysAgo = (d, hour = 18) => new Date(2026, 8, 3 - d, hour, 30, 0).toISOString();
const base = { log: [], exercise: 'Back Squat', today: TODAY, unit: 'kg' };
/* ── the four silences that used to be one blank box ───────────────────── */
eq((0, lastTime_1.lastTime)({ ...base, status: 'loading' }).kind, 'loading', 'a read in flight says so rather than implying an empty history');
{
    const r = (0, lastTime_1.lastTime)({ ...base, status: 'error' });
    ...
    ok(/read that failed rather than a movement you have never done/.test(note), 'and refuses the sentence that would accuse them of never having done it');
}
{
    const r = (0, lastTime_1.lastTime)({ ...base, status: 'ready' });
...
}
{
    ...
    const r = (0, lastTime_1.lastTime)({ ...base, status: 'partial' });
...
{
    const r = (0, lastTime_1.lastTime)({ ...base, log: squats, status: 'ready' });
    if (r.kind !== 'outing') {
        errors.push('the newest session for the movement is an outing');
    }
    else {
        ...
        eq(r.sets[0].label, `${(0, units_1.liftLabel)(60, 'kg')} × 8`, 'phrased as the Records board phrases it');
        eq(r.sets[2].label, `${(0, units_1.liftLabel)(60, 'kg')} × 7`, 'including the set that dropped a rep');
...
}
{
    ...
    const r = (0, lastTime_1.lastTime)({ ...base, log: squats, status: 'partial' });
...
}
{
    ...
    const r = (0, lastTime_1.lastTime)({ ...base, exercise: 'back squat', log: squats, status: 'ready' });
    eq(r.kind, 'outing', 'the movement is matched by slug, not by exact spelling');
    if (r.kind === 'outing')
        eq(r.name, 'Back Squat', 'and it is shown as it was actually written that day');
}
{
    const r = (0, lastTime_1.lastTime)({ ...base, exercise: 'Front Squat', log: squats, status: 'ready' });
    eq(r.kind, 'never', 'a different movement is a different movement');
}
/* ── the number in the box, beside the number above it ─────────────────── */
{
    const r = (0, lastTime_1.lastTime)({ ...base, log: squats, status: 'ready', boxKg: 62.5 });
    if (r.kind === 'outing')
        ok(/2\.5 kg more/.test(r.boxNote ?? ''), 'a step up is stated as arithmetic');
}
{
    const r = (0, lastTime_1.lastTime)({ ...base, log: squats, status: 'ready', boxKg: 60 });
    if (r.kind === 'outing')
        ok(/same as/.test(r.boxNote ?? ''), 'a repeat says it is a repeat');
}
{
    const r = (0, lastTime_1.lastTime)({ ...base, log: squats, status: 'ready', boxKg: 57.5 });
    if (r.kind === 'outing')
        ok(/2\.5 kg less/.test(r.boxNote ?? ''), 'and a step down is not hidden');
}
{
    ...
    const r = (0, lastTime_1.lastTime)({ ...base, log: squats, status: 'ready', boxKg: 60.01 });
    if (r.kind === 'outing')
        ok(/same as/.test(r.boxNote ?? ''), 'a difference that rounds away is not a difference');
}
{
    const r = (0, lastTime_1.lastTime)({ ...base, log: squats, status: 'ready', unit: 'lb', boxKg: 62.5 });
    if (r.kind === 'outing') {
...
}
{
    ...
    const dbl = [{ t: daysAgo(3), exercise: 'Back Squat', sets: [[5, 40.48]] }];
    const r = (0, lastTime_1.lastTime)({ ...base, log: dbl, status: 'ready', unit: 'lb', boxKg: 42.98 });
    if (r.kind === 'outing')
        ok(/5\.5 lb more/.test(r.boxNote ?? ''), 'the difference is converted once, not taken between two rounded readings');
}
{
    ...
    // is standing at.
    const r = (0, lastTime_1.lastTime)({ ...base, log: squats, status: 'ready', boxKg: 0 });
    if (r.kind === 'outing')
        eq(r.boxNote, null, 'an empty box is compared to nothing');
}
/* ── a body is not a bar ───────────────────────────────────────────────── */
const pullups = [
...
{
    const r = (0, lastTime_1.lastTime)({ ...base, exercise: 'Pull-up', log: pullups, status: 'ready', boxKg: 20 });
    if (r.kind !== 'outing') {
        errors.push('a bodyweight movement has a history like any other');
    }
    else {
        eq(r.sets[0].label, '8 reps at bodyweight', 'a plain pull-up is not a lift of 0 kg');
        eq(r.sets[1].label, `8 reps at bodyweight +${(0, units_1.liftLabel)(20, 'kg')}`, 'and a belted one names what was added');
        ok(!/× 8/.test(r.sets[0].label), 'a body is never printed as a bar figure');
        eq(r.boxNote, null, 'and nothing compares the box to a figure that is partly a weigh-in');
    }
}
...
{
    const r = (0, lastTime_1.lastTime)({ ...base, exercise: 'Plank', log: planks, status: 'ready', boxKg: 10 });
    if (r.kind !== 'outing') {
        errors.push('a hold is a session too');
    }
    else {
...
/* ── mixed, and the odd row a queue can produce ────────────────────────── */
{
    ...
    const r = (0, lastTime_1.lastTime)({ ...base, exercise: 'Circuit', log: mixed, status: 'ready', boxKg: 45 });
    if (r.kind === 'outing')
        ok(/5 kg more/.test(r.boxNote ?? ''), 'the hold contributes nothing to the top load');
}
{
    ...
    const junk = [{ t: daysAgo(9), exercise: 'Back Squat', sets: [[0, 0]] }];
    const r = (0, lastTime_1.lastTime)({ ...base, log: junk, status: 'ready' });
...
}
{
    ...
    const undated = [{ t: 'not a timestamp', exercise: 'Back Squat', sets: [[5, 100]] }];
    const r = (0, lastTime_1.lastTime)({ ...base, log: undated, status: 'ready' });
...
}
{
    ...
    const r = (0, lastTime_1.lastTime)({ ...base, log: shuffled, status: 'ready' });
    if (r.kind === 'outing')
        eq(r.daysAgo, 6, 'the outing is chosen by date, not by position');
}
{
    const r = (0, lastTime_1.lastTime)({ ...base, log: [{ t: daysAgo(0, 9), exercise: 'Back Squat', sets: [[5, 70]] }], status: 'ready' });
    if (r.kind === 'outing')
        eq(r.when, 'Today', 'a second session on the same day reads as today');
}
/* ── truncation is stated, not silently trimmed ────────────────────────── */
{
    const many = [{
            t: daysAgo(1), exercise: 'Back Squat',
            sets: Array.from({ length: lastTime_1.MAX_CHIPS + 4 }, () => [5, 50]),
        }];
    const r = (0, lastTime_1.lastTime)({ ...base, log: many, status: 'ready' });
    if (r.kind === 'outing') {
        eq(r.sets.length, lastTime_1.MAX_CHIPS, 'the strip stops at the cap');
        eq(r.more, 4, 'and the rest are counted rather than vanishing');
    }
}
/* ── nothing is claimed about a movement with no name ──────────────────── */
eq((0, lastTime_1.lastTime)({ ...base, exercise: '   ', log: squats, status: 'ready' }).kind, 'never', 'a nameless movement matches nothing rather than matching everything');
if (errors.length) {
