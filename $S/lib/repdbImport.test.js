...
const repdbImport_1 = require("./repdbImport");
const catalogueLocale_1 = require("./catalogueLocale");
const errors = [];
const ok = (cond, msg) => { if (!cond)
    errors.push(msg); };
const eq = (a, b, msg) => {
    if (JSON.stringify(a) !== JSON.stringify(b))
        errors.push(`${msg} — got ${JSON.stringify(a)}, want ${JSON.stringify(b)}`);
};
// ── the licence read off the pack, not off a flag ──────────────────────────
{
    ok((0, repdbImport_1.packTier)('# RepDB — Standard License (v1.2)') === 'standard', 'the Standard bundle is recognised');
    ok((0, repdbImport_1.packTier)('# RepDB — Free Tier License (v1.0)') === 'free', 'the free tier is recognised');
    ok((0, repdbImport_1.packTier)('Creative Commons Attribution-NonCommercial 4.0') === 'preview-nc', 'the evaluation preview is recognised');
    ...
    ok((0, repdbImport_1.packTier)('Preview Pack License (CC BY-NC 4.0)\nFor commercial use see the Standard License at https://repdb.co.') === 'preview-nc', 'a preview that MENTIONS the Standard tier is still the preview');
    for (const nothing of ['', '   ', null, undefined]) {
        ok((0, repdbImport_1.packTier)(nothing) === 'unknown', `${JSON.stringify(nothing)} is unknown, never a tier`);
    }
    ok((0, repdbImport_1.packTier)('some other vendor licence') === 'unknown', 'an unrecognised licence is unknown');
}
// ── an unreadable licence is refused, not assumed ──────────────────────────
{
    ok((0, repdbImport_1.tierMayShip)('standard'), 'the Standard bundle may ship');
    ok((0, repdbImport_1.tierMayShip)('free'), 'so may the free tier — it permits commercial in-app use');
    ok(!(0, repdbImport_1.tierMayShip)('preview-nc'), 'the CC BY-NC preview may NOT ship in a product that sells memberships');
    ...
    ok(!(0, repdbImport_1.tierMayShip)('unknown'), 'and neither may a pack whose licence we could not read');
    eq((0, repdbImport_1.demoLicenceFor)('standard'), 'commercial', 'Standard media is stamped commercial');
    eq((0, repdbImport_1.demoLicenceFor)('free'), 'commercial', 'free-tier media is stamped commercial too');
    eq((0, repdbImport_1.demoLicenceFor)('preview-nc'), 'evaluation', 'preview media is stamped evaluation');
    eq((0, repdbImport_1.demoLicenceFor)('unknown'), null, 'an unknown tier stamps nothing, which renders nowhere');
}
// ── the row id is the slug of the NAME ─────────────────────────────────────
{
    ...
    eq((0, repdbImport_1.catalogueId)(rec), 'bent-over-barbell-row', 'the row is keyed by the displayed name');
    ok((0, repdbImport_1.catalogueId)(rec) !== rec.id, 'and explicitly NOT by the vendor id');
    ...
    eq((0, repdbImport_1.catalogueId)({ id: 'childs-pose', name_en: "Child's Pose" }), 'child-s-pose', 'an apostrophe separates exactly as exerciseSlug does it, matching the seeded row');
    eq((0, repdbImport_1.catalogueId)({ id: 'x', name_en: 'Behind-the-Neck Pull-Up' }), 'behind-the-neck-pull-up', 'hyphens and mixed case resolve to the one form the screens ask for');
    ...
    // saying so.
    eq((0, repdbImport_1.catalogueId)({ id: 'only-an-id', name_en: null }), 'only-an-id', 'a nameless record falls back to the vendor id');
    eq((0, repdbImport_1.catalogueId)({ id: null, name_en: null }), '', 'and a record with neither resolves to nothing');
}
// ── the media key is the VENDOR id, which is a different string ────────────
{
    const rec = { id: 'barbell-row', name_en: 'Bent-Over Barbell Row' };
    eq((0, repdbImport_1.mediaKey)(rec), 'barbell-row', 'media files are named by the vendor id');
    ...
    ok((0, repdbImport_1.mediaKey)(rec) !== (0, repdbImport_1.catalogueId)(rec), 'the media key and the row id are NOT the same string');
    ...
    eq((0, repdbImport_1.mediaKey)({ id: 'paused-ohp', name_en: 'Paused Overhead Press', image_alias: 'ohp' }), 'ohp', 'a record that borrows artwork is keyed by the alias, not by its own id');
    eq((0, repdbImport_1.catalogueId)({ id: 'paused-ohp', name_en: 'Paused Overhead Press', image_alias: 'ohp' }), 'paused-overhead-press', 'while its ROW is still keyed by its own displayed name — the alias is about files only');
    eq((0, repdbImport_1.mediaKey)({ id: 'pause-deadlift', name_en: 'Pause Deadlift', image_alias: null }), 'pause-deadlift', 'a null alias falls through to the vendor id rather than blanking the key');
    eq((0, repdbImport_1.mediaKey)({ id: 'pause-deadlift', name_en: 'Pause Deadlift', image_alias: '' }), 'pause-deadlift', 'and so does an empty one');
}
// ── stills come back in the order the screen cross-fades them ──────────────
{
    ...
    eq((0, repdbImport_1.stillFiles)(rec, 'classic'), ['images/classic/barbell-row-start.webp', 'images/classic/barbell-row-peak.webp'], 'start precedes peak even when the vendor lists them the other way round');
    eq((0, repdbImport_1.stillFiles)(rec, 'flat'), ['images/flat/barbell-row-start.webp', 'images/flat/barbell-row-peak.webp'], 'and the style selects the folder');
    ...
    eq((0, repdbImport_1.stillFiles)({ id: 'plank', name_en: 'Plank', images: { classic: ['main'] } }, 'classic'), ['images/classic/plank-main.webp'], 'a single-position movement yields its one illustration');
    eq((0, repdbImport_1.stillFiles)({ id: 'x', name_en: 'X', images: { classic: [] } }, 'classic'), [], 'no variants is an empty answer, not a path to nothing');
    eq((0, repdbImport_1.stillFiles)({ id: 'x', name_en: 'X' }, 'classic'), [], 'and a record with no images block at all');
}
// ── an animation path is a claim the caller still has to confirm ───────────
{
    eq((0, repdbImport_1.animationFile)({ id: 'barbell-row', name_en: 'Bent-Over Barbell Row', animation: true }), 'images/animations/barbell-row.webp', 'the animation is named by the vendor id too');
    eq((0, repdbImport_1.animationFile)({ id: 'x', name_en: 'X', animation: false }), null, 'a record claiming none yields none');
    eq((0, repdbImport_1.animationFile)({ id: 'x', name_en: 'X' }), null, 'and so does one that says nothing about it');
    ...
    eq((0, repdbImport_1.animationFile)({ id: 'paused-ohp', name_en: 'Paused Overhead Press', image_alias: 'ohp', animation: true }), 'images/animations/ohp.webp', 'an aliased record finds its clip under the alias');
    eq((0, repdbImport_1.stillFiles)({ id: 'paused-ohp', name_en: 'Paused Overhead Press', image_alias: 'ohp', images: { classic: ['start', 'peak'] } }, 'classic'), ['images/classic/ohp-start.webp', 'images/classic/ohp-peak.webp'], 'and its stills too — the alias governs every filename, not just the animation');
}
// ── the whole plan for one record ──────────────────────────────────────────
{
    const p = (0, repdbImport_1.planRow)({ id: 'barbell-row', name_en: 'Bent-Over Barbell Row', images: { classic: ['start', 'peak'] }, animation: true }, 'classic');
    ...
    const same = (0, repdbImport_1.planRow)({ id: 'arnold-press', name_en: 'Arnold Press', images: { classic: ['start', 'peak'] } }, 'classic');
    ...
    ok((0, repdbImport_1.planRow)({ id: null, name_en: null }, 'classic') === null, 'a record with no id at all is refused, not invented');
}
// ── the overlap number, which decides backfill vs replacement ──────────────
{
    ...
    const r = (0, repdbImport_1.overlap)(recs, existing);
    ...
    ok(r.matched.includes('bent-over-barbell-row'), 'a record whose vendor id differs from its name still counts as MATCHED, not as an addition');
}
// ── the slug rule itself, since everything above rests on it ───────────────
{
    eq((0, repdbImport_1.slug)('Bent-Over Barbell Row'), 'bent-over-barbell-row', 'spaces and hyphens collapse to one hyphen');
    eq((0, repdbImport_1.slug)("Child's Pose"), 'child-s-pose', 'an apostrophe separates rather than vanishing');
    eq((0, repdbImport_1.slug)('  Leading and trailing  '), 'leading-and-trailing', 'edges are trimmed rather than left as hyphens');
    eq((0, repdbImport_1.slug)('90/90 Hip Stretch'), '90-90-hip-stretch', 'digits survive and the slash separates');
    for (const nothing of ['', null, undefined])
        eq((0, repdbImport_1.slug)(nothing), '', `${JSON.stringify(nothing)} slugs to nothing`);
}
// ── the translation set, where the vendor id trap appears a third time ─────
{
    ...
    const { rows, skipped } = (0, repdbImport_1.planTranslations)(records, de, 'de');
    ...
    ok(!rows.some((r) => r.exerciseId === 'barbell-row'), 'and specifically NOT by the vendor id, which is the failure this join exists to prevent');
    eq(rows[1].description, null, 'a name with no description is a row with a null description, not a blank one');
    ok(skipped.some((s) => s.vendorId === 'ghost-movement' && s.reason === 'no-english-record'), 'a localised entry with no English counterpart is skipped and COUNTED, not guessed at');
    ok(skipped.some((s) => s.vendorId === 'plank' && s.reason === 'nothing-to-translate'), 'an entry that translates nothing is skipped rather than written blank');
    ...
    const again = (0, repdbImport_1.planTranslations)(records, de, 'de');
    eq(again.rows, rows, 'planning the same set twice produces the same rows');
    const corrected = (0, repdbImport_1.planTranslations)(records, [{ id: 'back-squat', name: 'Langhantel-Kniebeuge' }], 'de');
    ...
    const collide = (0, repdbImport_1.planTranslations)([{ id: 'row-a', name_en: 'Bent-Over Row' }, { id: 'row-b', name_en: 'Bent-over Row' }], [{ id: 'row-a', name: 'Vorgebeugtes Rudern' }, { id: 'row-b', name: 'Rudern vorgebeugt' }], 'de');
    ...
    eq((0, repdbImport_1.planTranslations)(records, de, 'en').rows, [], "'en' is not a translation locale — English is exercises.name, and the planner refuses it outright");
}
// ── coverage, which is the number a run should be judged on ────────────────
{
    const rows = [
        { exerciseId: 'back-squat', locale: 'de', name: 'Kniebeuge', description: null },
        ...
        { exerciseId: 'plank', locale: 'de', name: null, description: 'Eine Rumpfübung.' },
    ];
    const cov = (0, repdbImport_1.translationCoverage)(rows, new Set(['back-squat', 'plank', 'deadlift']));
    eq(cov.translated, ['back-squat'], 'only a movement with a NAME counts as translated');
    eq(cov.untranslated, ['deadlift', 'plank'], 'and the rest are named, because that list is what somebody works through next');
}
// ── the duplicated locale set has not drifted ──────────────────────────────
{
    ...
    eq([...repdbImport_1.IMPORT_TRANSLATION_LOCALES], [...catalogueLocale_1.TRANSLATION_LOCALES], 'the importer and the app agree on which languages exist');
}
if (errors.length) {
