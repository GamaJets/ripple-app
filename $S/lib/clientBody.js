...
exports.storedUnit = exports.DIRECTION_CAVEAT = exports.SCAN_STALE_DAYS = exports.metricDelta = exports.metricValue = exports.metricUnit = exports.BODY_METRICS = void 0;
exports.readBodyHistory = readBodyHistory;
exports.seriesOf = seriesOf;
exports.bodyBoard = bodyBoard;
exports.movementOf = movementOf;
exports.readingLine = readingLine;
exports.seriesAgeDays = seriesAgeDays;
exports.isSeriesStale = isSeriesStale;
exports.seriesAgeLine = seriesAgeLine;
exports.readManual = readManual;
exports.manualIsCurrent = manualIsCurrent;
exports.manualFigures = manualFigures;
exports.manualLine = manualLine;
exports.bodyLine = bodyLine;
...
const emptySeries = (key) => ({
    key,
    label: exports.BODY_METRICS.find((m) => m.key === key).label,
    readings: [],
});
/**
...
function readBodyHistory(rows) {
    ...
    for (const { r, day } of ordered) {
        ...
        scans++;
        if (w != null && w > 0)
            weight.push({ atISO: r.taken_at, v: w, source: src });
        if (f != null && f > 0)
            bodyfat.push({ atISO: r.taken_at, v: f, source: src });
        if (m != null && m > 0)
            muscle.push({ atISO: r.taken_at, v: m, source: src });
        if (!latest || day >= latest.day)
            latest = { iso: r.taken_at, day };
    }
...
function movementOf(s) {
    if (s.readings.length < 2)
        return null;
    ...
    return {
        ...
        days: (0, coachWeek_1.daysBetweenIso)(first.atISO, last.atISO),
    };
}
...
const metricUnit = (key, wu) => (0, clientGoals_1.goalUnit)(key, wu);
exports.metricUnit = metricUnit;
/** A stored reading in the coach's unit. */
const metricValue = (v, key, wu) => (0, clientGoals_1.goalValue)(v, key, wu);
exports.metricValue = metricValue;
/** A stored CHANGE in the coach's unit, converted as a span. */
const metricDelta = (v, key, wu) => (0, clientGoals_1.goalDelta)(v, key, wu);
exports.metricDelta = metricDelta;
/**
...
function readingLine(s, wu) {
    ...
    const unit = (0, exports.metricUnit)(s.key, wu);
    const d = (0, exports.metricDelta)(mv.deltaStored, s.key, wu);
    const since = (0, coachWeek_1.dayHeading)(mv.first.atISO);
    ...
    return `${n} readings · ${(0, deltaLabel_1.deltaLabel)(d, { since, unit, noChange: 'unchanged' })}.`;
}
/**
 ...
 */
exports.SCAN_STALE_DAYS = 42;
/** How many days ago this metric was last measured, or null with no readings. */
function seriesAgeDays(s, todayISO) {
    const last = s.readings[s.readings.length - 1];
    return last ? (0, coachWeek_1.daysBetweenIso)(last.atISO, todayISO) : null;
}
function isSeriesStale(s, todayISO) {
    const d = seriesAgeDays(s, todayISO);
    return d != null && d >= exports.SCAN_STALE_DAYS;
}
/**
 ...
 */
function seriesAgeLine(s, todayISO) {
    const last = s.readings[s.readings.length - 1];
    if (!last)
        return 'Never measured.';
    const on = (0, coachWeek_1.dayHeading)(last.atISO);
    const when = (0, coachWeek_1.whenLabel)(last.atISO, todayISO);
 ...
 * any one metric.
 */
exports.DIRECTION_CAVEAT = 'Each change is shown with its sign and nothing else. Whether a figure moving ' +
 ...
 */
function readManual(row) {
    if (!row || !row.manual_at)
        return null;
 ...
 */
function manualIsCurrent(m, latestScanISO) {
 ...
 * the answer.
 */
function manualFigures(m, wu, who) {
    const parts = [];
    if (m.weightKg != null) {
        parts.push(`${(0, units_1.plain)((0, exports.metricValue)(m.weightKg, 'weight', wu))} ${(0, exports.metricUnit)('weight', wu)}`);
    }
    if (m.bodyFatPct != null)
        parts.push(`${(0, units_1.plain)(m.bodyFatPct)}% body fat`);
    return `${who} typed ${parts.join(' and ')} on ${(0, coachWeek_1.dayHeading)(m.atISO)}.`;
}
function manualLine(m, latestScanISO, wu, who) {
 ...
 */
function bodyLine(failed, loading, newestISO, hasEarlier, todayISO, who) {
    if (failed) {
    ...
    const when = (0, coachWeek_1.whenLabel)(newestISO, todayISO).toLowerCase();
    const on = (0, coachWeek_1.dayHeading)(newestISO);
    return hasEarlier
...
const storedUnit = (key) => goalTargets_1.GOAL_METRIC[key].unit;
exports.storedUnit = storedUnit;
