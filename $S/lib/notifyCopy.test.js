...
/* ── clipping ──────────────────────────────────────────────────────────── */
eq((0, notifyCopy_1.clip)('short', 20), 'short', 'text inside the cap is untouched');
eq((0, notifyCopy_1.clip)('  padded  ', 20), 'padded', 'the text is trimmed before it is measured');
ok((0, notifyCopy_1.clip)('x'.repeat(40), 10).length <= 10, 'a clipped string honours the cap it was given');
...
eq((0, notifyCopy_1.clip)('abcdefghij', 10), 'abcdefghij', 'text exactly the length of the cap is untouched');
eq((0, notifyCopy_1.clip)('abcdefghijk', 10).length, 10, 'and one character over is cut to the cap');
ok((0, notifyCopy_1.clip)('x'.repeat(40), 10).endsWith('…'), 'a clipped string says it was clipped');
...
eq((0, notifyCopy_1.clip)('we are closed on monday morning', 20), 'we are closed on…', 'the cut prefers a word boundary');
ok(!(0, notifyCopy_1.clip)('supercalifragilisticexpialidocious and more', 20).startsWith('…'), 'a body whose first word is longer than the cap is still cut to something readable');
/* ── a notice is the author's words, with the app's heading ────────────── */
const BODY = 'No 6pm class this Thursday — the room is being re-floored.';
const coach = (0, notifyCopy_1.noticeNotification)('coach', BODY);
...
const gym = (0, notifyCopy_1.noticeNotification)('gym', BODY, 'Iron & Oak');
...
eq((0, notifyCopy_1.noticeNotification)('gym', BODY, null)?.title, 'A notice from your gym', 'an unread gym name gives a true generic heading rather than an invented one');
eq((0, notifyCopy_1.noticeNotification)('gym', BODY, '   ')?.title, 'A notice from your gym', 'a blank gym name is the same as no gym name');
...
eq((0, notifyCopy_1.noticeNotification)('coach', ''), null, 'a blank notice is not a notification');
eq((0, notifyCopy_1.noticeNotification)('gym', '   \n '), null, 'a whitespace notice is not a notification');
...
const long = (0, notifyCopy_1.noticeNotification)('coach', 'a'.repeat(4000));
ok((long?.body.length ?? 0) <= notifyCopy_1.NOTICE_BODY_MAX, 'a long notice is cut to what the column will store');
ok(long?.body.endsWith('…') === true, 'a cut notice says it was cut');
const longName = (0, notifyCopy_1.noticeNotification)('gym', BODY, 'g'.repeat(400));
ok((longName?.title.length ?? 0) <= notifyCopy_1.NOTICE_TITLE_MAX, 'a long gym name cannot overflow the title column');
...
eq((0, notifyInbox_1.safeRoute)(notifyCopy_1.NOTICE_ROUTE, 'client'), notifyCopy_1.NOTICE_ROUTE, 'a notice sends a client to the notices screen');
eq((0, notifyInbox_1.safeRoute)(notifyCopy_1.NOTICE_ROUTE, 'trainer'), null, 'and cannot send a coach build into the client group');
...
const asked = (0, notifyCopy_1.invoiceNotification)(base);
...
ok(asked.body.includes('Ten personal training sessions'), 'the coach’s own description of the work reaches the body intact');
// And it is capped on the way in, because `description` is free text and the
// whole body has to fit `left(body, 500)`.
const wordy = (0, notifyCopy_1.invoiceNotification)({ ...base, description: 'session '.repeat(60).trim() });
ok(wordy.body.length <= notifyCopy_1.NOTICE_BODY_MAX, 'a description nobody stopped typing cannot overflow the column');
ok(wordy.body.includes('…'), 'and the reader can tell it was cut');
const paid = (0, notifyCopy_1.invoiceNotification)({ ...base, kind: 'received' });
eq(paid.title, 'Your coach recorded a payment', 'a received invoice says who recorded what');
ok(paid.body.includes('states this amount has been received'), 'it reports the coach’s statement rather than asserting the payment');
...
for (const inv of [asked, paid]) {
    ...
    ok(inv.body.length <= notifyCopy_1.NOTICE_BODY_MAX, 'the body fits the column that stores it');
}
...
const noCcy = (0, notifyCopy_1.invoiceNotification)({ ...base, currency: null });
...
ok((0, notifyCopy_1.invoiceNotification)({ ...base, description: 'd'.repeat(2000) }).body.length <= notifyCopy_1.NOTICE_BODY_MAX, 'a long description cannot push the body past the column');
...
const unknown = (0, notifyCopy_1.deliverySummary)({ recipients: null, recorded: null, push: 'off' });
...
const none = (0, notifyCopy_1.deliverySummary)({ recipients: 0, recorded: 0, push: 'off' });
ok(/nobody to notify/i.test(none), 'a gym with no members is told there is nobody, not that a send failed');
const some = (0, notifyCopy_1.deliverySummary)({ recipients: 40, recorded: 38, push: 'off' });
ok(some.includes('38 of 40'), 'both figures are stated when they differ — skipped recipients are visible');
const all = (0, notifyCopy_1.deliverySummary)({ recipients: 12, recorded: 12, push: 'off' });
ok(all.includes('12 people have'), 'a whole fan-out states one figure');
ok((0, notifyCopy_1.deliverySummary)({ recipients: 1, recorded: 1, push: 'off' }).includes('1 person has'), 'one recipient is a person, not 1 people');
...
// for.
ok((0, notifyCopy_1.deliverySummary)({ recipients: 1204, recorded: 1204, push: 'off' }).includes('1,204'), 'a four-figure count carries its separator');
...
ok(/queued/i.test((0, notifyCopy_1.deliverySummary)({ recipients: 3, recorded: 3, push: 'queued' })), 'a push that went out is described as queued rather than received');
ok((0, notifyCopy_1.deliverySummary)({ recipients: 3, recorded: 3, push: 'failed', pushError: 'Not connected to the server.' })
    .includes('Not connected to the server.'), 'a failed push reports the server’s own reason, which is the part an author can act on');
ok(/did not go out/i.test((0, notifyCopy_1.deliverySummary)({ recipients: 3, recorded: 3, push: 'failed', pushError: null })), 'a failed push with no reason still says it failed');
ok(/no push/i.test((0, notifyCopy_1.deliverySummary)({ recipients: 3, recorded: 3, push: 'off' })), 'not pushing is stated too — silence about it would read as a push');
 ...
 */
const partial = (0, notifyCopy_1.deliverySummary)({ recipients: 900, recorded: 900, push: 'queued', pushPartial: true });
ok(/not all of the recipient list could be read/i.test(partial), 'a partly-read recipient list is said out loud rather than reported as a send that went out');
ok(/notifications/i.test(partial), 'and the inbox rows, which DID all land, are still credited — the two halves are different facts');
...
ok(!/only people on a push-enabled build/i.test(partial), 'the partial sentence REPLACES the ordinary one rather than being appended to it');
ok(!/not all of the recipient list/i.test((0, notifyCopy_1.deliverySummary)({ recipients: 3, recorded: 3, push: 'queued' })), 'and a send with nothing wrong carries no warning — an absent flag is not a truncation');
ok(!/not all of the recipient list/i.test((0, notifyCopy_1.deliverySummary)({ recipients: 3, recorded: 3, push: 'queued', pushPartial: false })), 'nor does an explicit false');
...
const atCap = (0, notifyCopy_1.deliverySummary)({ recipients: 2400, recorded: 2000, push: 'off', recordedAtCap: true });
...
ok(!/send it again/i.test(atCap), 'no remedy is offered — the ceiling has no ORDER BY, so a second send would address an arbitrary two thousand');
ok(!/\bdelivered\b/i.test(atCap), 'and still nothing claims delivery');
ok(!/at least/i.test((0, notifyCopy_1.deliverySummary)({ recipients: 40, recorded: 38, push: 'off' })), 'an ordinary fan-out under the ceiling says nothing about one');
...
const cappedList = (0, notifyCopy_1.deliverySummary)({ recipients: 1001, recorded: 998, push: 'off', recipientsTruncated: true });
ok(/more people to address than this app could read/i.test(cappedList), 'a capped roster admits that the list is not the whole roster');
...
const warn = (0, notifyCopy_1.pushConsequence)('gym', 240);
ok(warn.includes('240 members'), 'the author is told how many phones this is');
ok(/straight away/i.test(warn) && /what ?ever time it is where they are/i.test(warn.replace('whatever', 'what ever')), 'and that it happens now, wherever they are — there is no scheduler and no timezone on record');
eq((0, notifyCopy_1.pushConsequence)('coach', 1), 'Sends a push to 1 client straight away, at whatever time it is where they are. Without it the notice still reaches their notices and their notifications — quietly.', 'one client is a client');
ok(/every member/.test((0, notifyCopy_1.pushConsequence)('gym', null)), 'an uncounted audience is "every member", never a figure nobody counted');
...
eq((0, notifyCopy_1.classStartsIn)(T0 - 1, T0), 'It has already started.', 'a class one millisecond into the past has started');
eq((0, notifyCopy_1.classStartsIn)(T0, T0), 'It has already started.', 'and so has one starting exactly now');
eq((0, notifyCopy_1.classStartsIn)(T0 - 3 * HR, T0), 'It has already started.', 'a promotion into a class three hours gone says so rather than counting down');
eq((0, notifyCopy_1.classStartsIn)(T0 + 1, T0), 'It starts in under an hour.', 'a millisecond away is under an hour');
eq((0, notifyCopy_1.classStartsIn)(T0 + 59 * MIN, T0), 'It starts in under an hour.', 'fifty-nine minutes is still under an hour');
...
eq((0, notifyCopy_1.classStartsIn)(T0 + 30 * MIN, T0), 'It starts in under an hour.', 'half an hour is under an hour and not "about an hour"');
eq((0, notifyCopy_1.classStartsIn)(T0 + HR, T0), 'It starts in about an hour.', 'exactly an hour is an hour');
eq((0, notifyCopy_1.classStartsIn)(T0 + 89 * MIN, T0), 'It starts in about an hour.', 'an hour and a half rounds down to one — singular, never "1 hours"');
eq((0, notifyCopy_1.classStartsIn)(T0 + 91 * MIN, T0), 'It starts in about 2 hours.', 'and just over rounds up to two');
eq((0, notifyCopy_1.classStartsIn)(T0 + 6 * HR, T0), 'It starts in about 6 hours.', 'six hours');
eq((0, notifyCopy_1.classStartsIn)(T0 + 23 * HR, T0), 'It starts in about 23 hours.', 'twenty-three hours is still counted in hours');
...
eq((0, notifyCopy_1.classStartsIn)(T0 + 23 * HR + 40 * MIN, T0), 'It starts in about a day.', 'twenty-four hours is a day, not "24 hours"');
eq((0, notifyCopy_1.classStartsIn)(T0 + DAY, T0), 'It starts in about a day.', 'exactly a day');
eq((0, notifyCopy_1.classStartsIn)(T0 + 30 * HR, T0), 'It starts in about a day.', 'thirty hours rounds down to one day — singular, never "1 days"');
// And the far side of that boundary, so the day band is pinned at both ends.
eq((0, notifyCopy_1.classStartsIn)(T0 + 36 * HR, T0), 'It starts in about 2 days.', 'a day and a half rounds up to two');
eq((0, notifyCopy_1.classStartsIn)(T0 + 2 * DAY, T0), 'It starts in about 2 days.', 'two days');
eq((0, notifyCopy_1.classStartsIn)(T0 + 13 * DAY, T0), 'It starts in about 13 days.', 'a fortnight away is still answerable');
...
for (const ms of [-DAY, -1, 0, 1, 30 * MIN, HR, 5 * HR, 23 * HR, DAY, 9 * DAY]) {
    const said = (0, notifyCopy_1.classStartsIn)(T0 + ms, T0);
    ok(!/\d{1,2}[:.]\d{2}/.test(said), `“${said}” states no clock time`);
    ok(!/(Mon|Tue|Wed|Thu|Fri|Sat|Sun|Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)/.test(said), `“${said}” names no day and no month`);
...
eq((0, notifyCopy_1.classStartsIn)(NaN, T0), '', 'an unreadable start time produces nothing at all');
eq((0, notifyCopy_1.classStartsIn)(T0, NaN), '', 'and so does an unreadable now');
eq((0, notifyCopy_1.classStartsIn)(Infinity, T0), '', 'infinity is not a start time');
 ...
 */
{
    const yes = (0, notifyCopy_1.coachAnswerNotification)(true, 'Alex Rivera');
    const no = (0, notifyCopy_1.coachAnswerNotification)(false, 'Alex Rivera');
    ...
    eq(yes.route, notifyCopy_1.COACH_ACCEPTED_ROUTE, 'an accepted request opens the screen that just changed');
    eq(no.route, notifyCopy_1.COACH_DECLINED_ROUTE, 'a declined one opens the only useful next step');
    ok(yes.route !== no.route, 'and they are not the same screen');
    eq((0, notifyInbox_1.safeRoute)(yes.route, 'client'), yes.route, 'the accepted route is one the client app will open');
    eq((0, notifyInbox_1.safeRoute)(no.route, 'client'), no.route, 'and so is the declined one');
    // A name that could not be read is a subject that is still there.
    for (const missing of [null, undefined, '', '   ']) {
        const n = (0, notifyCopy_1.coachAnswerNotification)(false, missing);
    ...
    ok(!/not taking|right now|at the moment/i.test(no.body), 'the decline states what happened and invents no reason on the coach\u2019s behalf');
    // Recorded, both. Nothing else in the product would ever tell them.
    ok((0, notifyInbox_1.inboxDecision)(yes.title, yes.body, yes.route).record, 'an accepted request is worth an inbox row');
    ok((0, notifyInbox_1.inboxDecision)(no.title, no.body, no.route).record, 'and a declined one is the row that matters most');
}
/* what the coach is told, which is never more than happened */
{
    const sent = (0, notifyCopy_1.coachAnswerConfirmation)(true, 'Sam', { ok: true, recorded: 1 });
    const rowOnly = (0, notifyCopy_1.coachAnswerConfirmation)(true, 'Sam', { ok: false, recorded: 1 });
    const neither = (0, notifyCopy_1.coachAnswerConfirmation)(true, 'Sam', { ok: false, recorded: 0 });
    ok(sent !== rowOnly && rowOnly !== neither && sent !== neither, 'three outcomes, three sentences');
    ...
    const declined = (0, notifyCopy_1.coachAnswerConfirmation)(false, 'Sam', { ok: false, recorded: 0 });
    ok(declined.includes('still waiting on you'), 'a decline nobody could deliver says the consequence out loud: their app still shows it pending');
    ok(!(0, notifyCopy_1.coachAnswerConfirmation)(false, 'Sam', { ok: true, recorded: 1 }).includes('roster'), 'a decline never says roster');
    ok((0, notifyCopy_1.coachAnswerConfirmation)(true, '   ', { ok: true, recorded: 1 }).startsWith('That client is now on your roster.'), 'a name that could not be read still leaves a sentence with a subject, and a grammatical one');
}
 ...
 */
{
    ...
    eq(notifyCopy_1.CLASS_OFF_TITLE, 'A class you booked is not running', 'the singular title is part 493\u2019s own');
    const one = (0, notifyCopy_1.classOffNotification)('Spin', 1, 'the instructor is off sick');
    eq(one.title, notifyCopy_1.CLASS_OFF_TITLE, 'one class off gets the singular heading');
    ...
    eq(one.route, notifyCopy_1.CLASS_OFF_ROUTE, 'it opens the timetable');
    eq((0, notifyInbox_1.safeRoute)(one.route, 'client'), one.route, 'which is a screen the client app has');
    const many = (0, notifyCopy_1.classOffNotification)('Spin', 3, 'the room is being re-floored');
    eq(many.title, notifyCopy_1.CLASS_OFF_TITLE_MANY, 'more than one gets the plural heading');
    ok(many.body.includes('3'), 'and says how many of THEIR bookings went');
    const noReason = (0, notifyCopy_1.classOffNotification)('Spin', 1, '   ');
    ok(!noReason.body.includes(':'), 'a blank reason leaves no dangling colon');
    const noName = (0, notifyCopy_1.classOffNotification)(null, 1, null);
    ...
    ok(!(0, notifyInbox_1.inboxDecision)(one.title, one.body, one.route).record, 'the singular class-off push leaves the row to the trigger');
    ok(!(0, notifyInbox_1.inboxDecision)(many.title, many.body, many.route).record, 'and so does the plural one');
}
/* who gets which sentence */
{
    eq((0, notifyCopy_1.classOffBuckets)([]).length, 0, 'nothing cancelled reaches nobody');
    const one = (0, notifyCopy_1.classOffBuckets)([{ userId: 'a', classId: 'c1' }, { userId: 'b', classId: 'c1' }]);
    ...
    const dup = (0, notifyCopy_1.classOffBuckets)([{ userId: 'a', classId: 'c1' }, { userId: 'a', classId: 'c1' }]);
    ...
    const series = (0, notifyCopy_1.classOffBuckets)([
    ...
    eq((0, notifyCopy_1.classOffBuckets)([{ userId: '', classId: 'c1' }, { userId: 'a', classId: '  ' }]).length, 0, 'a row with no person or no class is not somebody to notify');
}
/* what the coach is told about the fan-out */
{
    const unread = (0, notifyCopy_1.classOffConfirmation)(1, null, null);
    ok(unread.includes('couldn\u2019t read who had booked'), 'a roster that could not be read is never reported as nobody');
    ok(!/\bnobody had booked\b/i.test(unread), 'and is not collapsed into the empty case');
    ok((0, notifyCopy_1.classOffConfirmation)(1, 0, 0).includes('Nobody had booked'), 'an empty class had nobody to tell');
    ok((0, notifyCopy_1.classOffConfirmation)(1, 4, 0).includes('tell them yourself'), 'four people and no push reached is four people the coach has to tell');
    ok((0, notifyCopy_1.classOffConfirmation)(1, 4, 2).includes('2 of them'), 'a partial fan-out says which part');
    ok((0, notifyCopy_1.classOffConfirmation)(1, 4, 4).includes('all of them'), 'and a whole one says so');
    for (const line of [unread, (0, notifyCopy_1.classOffConfirmation)(9, 4, 4), (0, notifyCopy_1.classOffConfirmation)(1, 4, 2)]) {
        ...
        ok(!/in their notifications/i.test(line), 'nothing claims a row this app did not write and cannot see');
    }
    ok((0, notifyCopy_1.classOffConfirmation)(1, 1, 1).includes('1 class was'), 'one is singular');
    ok((0, notifyCopy_1.classOffConfirmation)(9, 1, 1).includes('9 classes were'), 'nine is not');
}
 ...
 */
{
    const whole = (0, notifyCopy_1.classOffConfirmation)(1, 12, 12, true);
    ok(!/all of them/i.test(whole), 'a partly-read handset list withdraws exactly the claim that everybody got one');
    ok(whole.includes(notifyCopy_1.PUSH_PARTIAL_NOTE), 'and says why in the one wording this product has for it, not a fifth');
    ...
    const some = (0, notifyCopy_1.classOffConfirmation)(1, 12, 8, true);
    ok(some.includes('8 of them'), 'the number that WAS handed over is still stated');
    ok(some.includes(notifyCopy_1.PUSH_PARTIAL_NOTE), 'with the same clause after it');
    ok(!/couldn\u2019t reach the rest/i.test(some), 'and not the confident "we could not reach the rest", which names a set nobody counted');
    // A partly-read list is not a failed send: some of the room was woken up.
    ok(!/couldn\u2019t reach any of their phones/i.test((0, notifyCopy_1.classOffConfirmation)(1, 12, 12, true)), 'a partial send is never collapsed into the nobody-was-told branch');
    // And an absent flag changes nothing, so every existing caller keeps its
    // sentence.
    ok((0, notifyCopy_1.classOffConfirmation)(1, 12, 12) === (0, notifyCopy_1.classOffConfirmation)(1, 12, 12, false), 'an explicit false is the same as saying nothing');
    ok(!(0, notifyCopy_1.classOffConfirmation)(1, 12, 12).includes(notifyCopy_1.PUSH_PARTIAL_NOTE), 'and a send with nothing wrong carries no warning');
}
/* ── one wording, and its one-person form ────────────────────────────────── */
{
    ...
    ok((0, notifyCopy_1.deliverySummary)({ recipients: 900, recorded: 900, push: 'queued', pushPartial: true })
        .includes(notifyCopy_1.PUSH_PARTIAL_NOTE), 'the summary that owned this sentence now shares the constant rather than a copy of it');
    ok((0, notifyCopy_1.pushPartialNote)(4) === notifyCopy_1.PUSH_PARTIAL_NOTE, 'a crowd gets the crowd wording');
    ok((0, notifyCopy_1.pushPartialNote)() === notifyCopy_1.PUSH_PARTIAL_NOTE, 'and so does an unstated number');
    const one = (0, notifyCopy_1.pushPartialNote)(1);
    ok(one !== notifyCopy_1.PUSH_PARTIAL_NOTE, 'one recipient is not "more people"');
    ok(!/more people/i.test(one), 'which is the half that would have been false');
    ok(one.startsWith('Not all of the recipient list could be read'), 'and the CAUSE — the half a reader can act on — is word for word the same');
}
if (errors.length) {
