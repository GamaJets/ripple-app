...
ok((0, overwriteGuard_1.guardOverwrite)('ready', SUBJECT).allowed, 'a whole read must let the coach save');
ok((0, overwriteGuard_1.guardOverwrite)('ready', SUBJECT).reason === null, 'an allowed write must not carry a refusal to render');
ok((0, overwriteGuard_1.guardOverwrite)('ready', SUBJECT).label === null, 'an allowed write must leave the control label alone');
for (const s of UNSOUND) {
    const g = (0, overwriteGuard_1.guardOverwrite)(s, SUBJECT);
    ...
    ok(g.reason.includes(SUBJECT), `'${s}' must name what would have been overwritten`);
...
// noticing.
const reasons = UNSOUND.map((s) => (0, overwriteGuard_1.guardOverwrite)(s, SUBJECT).reason);
...
for (const s of ['partial', 'error']) {
    const r = (0, overwriteGuard_1.guardOverwrite)(s, SUBJECT).reason;
...
const loadingReason = (0, overwriteGuard_1.guardOverwrite)('loading', SUBJECT).reason;
ok(!/could not be read|failed|no signal/i.test(loadingReason), 'a read still in flight must not be reported to the coach as a failure');
if (errors.length) {
