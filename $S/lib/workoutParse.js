...
exports.parseWorkoutText = parseWorkoutText;
...
const halves = (n) => Math.round(n * 2) / 2;
...
unit) {
    ...
    const toKg = (n) => (unit === 'lb' ? halves(n * units_1.KG_PER_LB) : n);
    ...
    for (const cl of clauses) {
        ...
        if (mKg) {
            written = parseFloat(mKg[1]);
            kg = written;
        }
        else if (mLb) {
            written = parseFloat(mLb[1]);
            kg = halves(written * units_1.KG_PER_LB);
        }
        else if (mAt) {
            written = parseFloat(mAt[1]);
            kg = toKg(written);
        }
        let sets = [];
