...
exports.WEEKLY_SUMMARY_PROMPT = void 0;
exports.coachAvailable = coachAvailable;
exports.askCoachForMember = askCoachForMember;
exports.askAboutMyWeek = askAboutMyWeek;
exports.askAboutMyBusiness = askAboutMyBusiness;
exports.askAboutClient = askAboutClient;
...
function coachAvailable() {
    return (0, vision_1.visionAvailable)();
}
 ...
 *  reply looks like. */
async function invoke(messages, context) {
    try {
        const { data, error } = await supabase_1.supabase.functions.invoke('coach-chat', { body: { messages, context } });
 ...
 */
async function askCoachForMember(messages, context, consent) {
    const body = (0, coachShare_1.shareableContext)(context, consent);
...
exports.WEEKLY_SUMMARY_PROMPT = 'Write a warm, concise 2-3 sentence weekly summary for this person from the facts below. '
 ...
 */
async function askAboutMyWeek(facts, consent) {
    const lines = (0, coachShare_1.weeklyFacts)(facts.fitness, facts.health, consent);
    ...
    const reply = await invoke([{ role: 'user', content: `${exports.WEEKLY_SUMMARY_PROMPT}\n\n${lines.join('\n')}` }], {});
    return reply == null ? { ok: false, reason: 'failed' } : { ok: true, reply };
}
 ...
 */
async function askAboutMyBusiness(messages, context) {
    if (!coachAvailable())
        return { ok: false, reason: 'unavailable' };
    const reply = await invoke(messages, (0, coachShare_1.businessAskContext)(context));
    return reply == null ? { ok: false, reason: 'failed' } : { ok: true, reply };
}
/**
 ...
 */
async function askAboutClient(messages, context) {
    if (!coachAvailable())
        return { ok: false, reason: 'unavailable' };
    const reply = await invoke(messages, (0, coachShare_1.clientAskContext)(context));
    return reply == null ? { ok: false, reason: 'failed' } : { ok: true, reply };
}
