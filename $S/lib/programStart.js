...
exports.CLIENT_STARTS_NOW = void 0;
exports.daysBetween = daysBetween;
exports.blockPosition = blockPosition;
exports.blockPositionNow = blockPositionNow;
exports.blockPositionLine = blockPositionLine;
exports.isStartDate = isStartDate;
exports.laterStart = laterStart;
/**
 ...
 */
function blockPosition(startsOn, todayISO, weeks) {
    ...
    if (offset == null)
        return { phase: 'unreadable', week: null, weeks: n, dayOffset: null };
    if (offset < 0)
        return { phase: 'before', week: null, weeks: n, dayOffset: offset };
    const week = Math.floor(offset / 7) + 1;
    if (week > n)
        return { phase: 'after', week: null, weeks: n, dayOffset: offset };
...
function blockPositionNow(startsOn, weeks, now = new Date()) {
    return blockPosition(startsOn, (0, dayPlan_1.isoToday)(now), weeks);
}
/**
 ...
 */
function blockPositionLine(pos, startsOn, who) {
    switch (pos.phase) {
        case 'no-date':
 ...
 */
exports.CLIENT_STARTS_NOW = 'A start date is your own record of when the block begins, and once it has passed it is what counts the week '
 ...
 */
function isStartDate(v) {
    const s = String(v ?? '').trim();
    if (!/^\d{4}-\d{2}-\d{2}$/.test(s))
        return false;
    const p = (0, localDate_1.dateParts)(s);
    if (!p)
        return false;
 ...
 */
function laterStart(a, b) {
    const c = (0, dayPlan_1.compareIsoDays)(String(a ?? ''), String(b ?? ''));
    return c != null && c > 0;
}
