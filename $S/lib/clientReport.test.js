...
eq((0, clientReport_1.sectionState)('ready'), 'whole', 'a landed read is whole');
eq((0, clientReport_1.sectionState)('partial'), 'partial', 'a truncated read is partial, not whole');
eq((0, clientReport_1.sectionState)('error'), 'unreadable', 'a refused read is unreadable');
eq((0, clientReport_1.sectionState)('loading'), 'unreadable', 'a read still in flight is unreadable, NOT whole — this is the one that would print an empty injuries table');
ok((0, clientReport_1.countable)('ready'), 'only a whole read may be totalled');
ok(!(0, clientReport_1.countable)('partial'), 'a truncated read may not be totalled');
ok(!(0, clientReport_1.countable)('loading'), 'and neither may one that has not landed');
ok(!(0, clientReport_1.countable)('error'), 'nor a failed one');
eq((0, clientReport_1.sectionCaveat)('Injuries disclosed', 'ready'), null, 'a whole read carries no caveat');
{
    const c = (0, clientReport_1.sectionCaveat)('Injuries disclosed', 'error') ?? '';
...
{
    const d = (0, clientReport_1.clientReportDoc)(base({ injuries: { status: 'error', items: [] } }));
...
}
{
    ...
    const d = (0, clientReport_1.clientReportDoc)(base({ injuries: { status: 'ready', items: [] } }));
...
}
{
    ...
    const d = (0, clientReport_1.clientReportDoc)(base({
...
{
    const whole = (0, clientReport_1.clientReportDoc)(base());
    ...
    const part = (0, clientReport_1.clientReportDoc)(base({ composition: { status: 'partial', items: SCANS } }));
...
}
{
    ...
    const capped = { ...TRAINING, dayCount: null, entryCount: null, sets: null, volumeKg: null };
    const d = (0, clientReport_1.clientReportDoc)(base({ training: { status: 'partial', items: capped } }));
   ...
   sentence, and this is the test that fails when they do. */
{
    ...
    let prose = (d.html + '\n' + d.text).toLowerCase();
    for (const stated of [clientReport_1.REPORT_LIMITS, clientReport_1.REPORT_NO_PHOTOS, ...clientReport_1.REPORT_PROVENANCE]) {
    ...
    ok(d.html.includes((0, clientReport_1.escapeHtml)(clientReport_1.REPORT_LIMITS).slice(0, 60)), 'and it says outright that it contains no assessment or advice');
    ok(d.text.includes(clientReport_1.REPORT_LIMITS), 'in the text fallback as well as the HTML');
}
{
    ...
    ok(d.text.includes(clientReport_1.REPORT_LIMITS), 'and still carries the limits statement');
    ok(d.text.includes(clientReport_1.REPORT_NO_PHOTOS), 'and still says no photographs are included');
}
...
{
    const d = (0, clientReport_1.clientReportDoc)(base({ weightUnit: 'lb', lengthUnit: 'in' }));
...
eq((0, clientReport_1.escapeHtml)('pain < 4/10'), 'pain &lt; 4/10', 'a less-than is escaped');
eq((0, clientReport_1.escapeHtml)('S&C coach'), 'S&amp;C coach', 'an ampersand is escaped — it is in ordinary names');
eq((0, clientReport_1.escapeHtml)('a "quote"'), 'a &quot;quote&quot;', 'and a double quote');
eq((0, clientReport_1.escapeHtml)(null), '', 'a missing value escapes to nothing rather than to "null"');
{
    const nasty = [
        { label: 'Shoulder', severity: 'moderate', status: 'active', note: '<script>alert(1)</script> pain < 4/10', at: '2026-05-04' },
    ];
    const d = (0, clientReport_1.clientReportDoc)(base({ name: 'Ann & Bob <b>', injuries: { status: 'ready', items: nasty } }));
...
{
    const many = [];
    for (let i = 0; i < clientReport_1.TRAINING_DAYS_SHOWN + 12; i++) {
        many.push({ day: `2026-0${1 + Math.floor(i / 28)}-${String((i % 28) + 1).padStart(2, '0')}`, exercises: 3, sets: 9, bodyweightSets: 0, volumeKg: 5000 });
    }
    const d = (0, clientReport_1.clientReportDoc)(base({ training: { status: 'ready', items: { ...TRAINING, days: many } } }));
    ...
    eq(printed, clientReport_1.TRAINING_DAYS_SHOWN, 'and prints exactly that many day rows, not all of them');
}
{
    const d = (0, clientReport_1.clientReportDoc)(base());
...
{
    ok(!(0, clientReport_1.clientReportDoc)(base()).html.includes('timestamp that could not be read'), 'a record with nothing undated says nothing about undated events');
    const d = (0, clientReport_1.clientReportDoc)(base({ training: { status: 'ready', items: { ...TRAINING, undatedCount: 2 } } }));
...
{
    const allUndated = { ...TRAINING, days: [], newestDay: null, undatedCount: 3 };
    const d = (0, clientReport_1.clientReportDoc)(base({ training: { status: 'ready', items: allUndated } }));
...
}
{
    ...
    const single = { ...TRAINING, days: [], newestDay: null, undatedCount: 1 };
    const d = (0, clientReport_1.clientReportDoc)(base({ training: { status: 'ready', items: single } }));
...
}
{
    ...
    // have swallowed it.
    const none = { state: 'none', dayCount: 0, entryCount: 0, sets: 0, volumeKg: null, newestDay: null, days: [], undatedCount: 0 };
    const d = (0, clientReport_1.clientReportDoc)(base({ training: { status: 'ready', items: none } }));
   ...
   than trusting either. */
{
    ...
    const d = (0, clientReport_1.clientReportDoc)(base({ composition: { status: 'ready', items: reversed } }));
    ...
    const rows = (0, clientReport_1.toProgressRows)(reversed);
...
{
    const good = (0, clientReport_1.reportShareBlurb)((0, clientReport_1.clientReportDoc)(base()));
    ...
    const bad = (0, clientReport_1.reportShareBlurb)((0, clientReport_1.clientReportDoc)(base({ injuries: { status: 'error', items: [] } })));
    ...
    const worse = (0, clientReport_1.reportShareBlurb)((0, clientReport_1.clientReportDoc)(base({
        injuries: { status: 'error', items: [] },
        measurements: { status: 'error', items: [] },
    })));
...
{
    const d = (0, clientReport_1.clientReportDoc)(base({ training: { status: 'error', items: TRAINING } }));
...
/* ── 14. white label: the brand is the tenant's, never a constant ─────────
   ...
   building. */
{
    const d = (0, clientReport_1.clientReportDoc)(base({ brand: 'Ironhaus Strength' }));
    ...
    ok((0, clientReport_1.clientReportDoc)(base({ brand: '' })).html.includes('Repple'), 'an empty brand falls back rather than printing nothing');
}
/* ── 15. the caveat list is exactly the sections that are not whole ───────*/
{
    eq((0, clientReport_1.reportCaveats)(base()).length, 0, 'four whole reads produce no caveats at all');
    eq((0, clientReport_1.reportCaveats)(base({ training: { status: 'loading', items: TRAINING } })).length, 1, 'a section still loading produces a caveat — it is not silently treated as read');
}
