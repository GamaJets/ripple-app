...
const inWindow = new Date(NOW + 3 * 3600000).toISOString(); // 3 hours away
const farOff = new Date(NOW + 30 * 24 * 3600000).toISOString(); // a month away
const policy = (over = {}) => ({
    applies: true, noticeHours: 24, fee: 40, currency: 'NZD', ...over,
});
const pick = (opts, scope) => {
    const o = opts.find((x) => x.scope === scope);
    if (!o)
        throw new Error(`no ${scope} option`);
    return o;
};
...
let sweep = 0;
for (const startsAt of [inWindow, farOff, new Date(NOW - 600000).toISOString()]) {
    for (const p of [
        policy(),
        ...
        for (const upcoming of [0, 1, 2, 8, 52]) {
            const opts = (0, recurring_1.cancelOptions)({ startsAt, policy: p, upcoming, now: NOW });
            const series = pick(opts, 'series');
            sweep++;
            ok(series.charges === false, `ending a series charged with policy ${JSON.stringify(p)} at ${startsAt} over ${upcoming} sessions`);
            ok(series.verdict === null, 'ending a series carries no fee verdict — there is no fee to have a verdict about');
            ...
            // once, on its own.
            eq(series.affects, Math.max(0, upcoming - 1), 'ending a series affects every occurrence except the next one');
            ...
            ok(!/fee of|[$£€]|NZD|GBP|AED|USD/.test(series.detail), 'and the sentence for it quotes no amount and no currency');
        }
    }
}
...
ok(/never charges a cancellation fee/i.test(recurring_1.RECURRING_END_RULE), 'the rule says plainly that ending charges nothing');
ok(/however much notice is left/i.test(recurring_1.RECURRING_END_RULE), 'and that the notice window does not change that');
ok(/next session stays booked/i.test(recurring_1.RECURRING_END_RULE), 'and that the next session survives it');
/* ── The mirror: one occurrence is still an ordinary cancellation ─────────── */
{
    const opts = (0, recurring_1.cancelOptions)({ startsAt: inWindow, policy: policy(), upcoming: 8, now: NOW });
    ...
    ok(one.charges !== pick(opts, 'series').charges, 'the two choices are priced differently — that difference IS the feature');
}
{
    const one = pick((0, recurring_1.cancelOptions)({ startsAt: farOff, policy: policy(), upcoming: 8, now: NOW }), 'occurrence');
...
{
    const unpriced = pick((0, recurring_1.cancelOptions)({ startsAt: inWindow, policy: policy({ fee: null }), upcoming: 4, now: NOW }), 'occurrence');
    ...
    const unknown = pick((0, recurring_1.cancelOptions)({ startsAt: inWindow, policy: null, upcoming: 4, now: NOW }), 'occurrence');
...
{
    const started = pick((0, recurring_1.cancelOptions)({
        startsAt: new Date(NOW - 600000).toISOString(), policy: policy(), upcoming: 3, now: NOW,
    }), 'occurrence');
...
for (const kind of ['in-time', 'no-policy', 'unknown', 'unpriced']) {
    const line = (0, recurring_1.occurrenceDetail)({ kind }, 24);
    ok(/rest of the standing appointment is untouched/i.test(line), `the '${kind}' sentence says the series survives`);
}
ok(/rest of the standing appointment is untouched/i.test((0, recurring_1.occurrenceDetail)({ kind: 'fee', amount: 40, currency: 'NZD' }, 24)), "the 'fee' sentence says it too");
// Repple records the fee and does not take it. A member who thinks the app has
// charged them will not pay their coach.
ok(/Repple doesn’t take this payment/.test((0, recurring_1.occurrenceDetail)({ kind: 'fee', amount: 40, currency: 'NZD' }, 24)), 'and the fee sentence says who actually collects it');
...
{
    const nzd = (0, recurring_1.occurrenceDetail)({ kind: 'fee', amount: 40, currency: 'NZD' }, 24);
    const gbp = (0, recurring_1.occurrenceDetail)({ kind: 'fee', amount: 40, currency: 'GBP' }, 24);
    ok(nzd !== gbp, 'the same amount in two currencies reads differently');
    const none = (0, recurring_1.occurrenceDetail)({ kind: 'fee', amount: 40, currency: null }, 24);
    ok(/\b40\b/.test(none) && !/[$£€]/.test(none), 'an unknown currency prints the bare figure and invents no symbol');
    ...
    ok(!/currency/i.test(nzd) && !/currency/i.test(gbp), 'while a fee in a currency the gym did choose carries no such clause');
}
ok(!/AED|USD|\$/.test(recurring_1.RECURRING_END_RULE + recurring_1.RECURRING_CREDIT_NOTE + recurring_1.RECURRING_CLASH_NOTE), 'none of the standing sentences names a currency');
...
ok(/Nothing comes off a session pack when the dates/i.test(recurring_1.RECURRING_CREDIT_NOTE), 'the credit note still promises that booking the dates costs nothing');
ok(/not paid for in advance/i.test(recurring_1.RECURRING_CREDIT_NOTE), 'and says outright that future weeks are not paid for up front');
...
ok(/marked done, one at a time/i.test(recurring_1.RECURRING_CREDIT_NOTE), 'and it says when a credit actually comes off, so the sessions are not read as free');
ok(!/settles what’s owed/i.test(recurring_1.RECURRING_CREDIT_NOTE), 'and no longer tells the client to settle it with their coach, which was the app admitting it had lost track');
/* ── A clash skips a date, it does not sink the series ────────────────────── */
eq((0, recurring_1.clashLine)(0, []), null, 'nothing clashed, so no line is drawn at all');
eq((0, recurring_1.clashLine)(0, ['2026-09-08']), null, 'and a stray date with a zero count draws nothing either');
{
    const line = (0, recurring_1.clashLine)(1, ['2026-09-08']);
    ...
    ok(!!line && /Everything else was created/.test(line), 'and says the rest went in — a clash is not a failure of the arrangement');
}
ok(/2 dates were skipped/.test((0, recurring_1.clashLine)(2, ['a', 'b']) ?? ''), 'two clashes read as two');
ok(/skipped rather than double-booked/i.test(recurring_1.RECURRING_CLASH_NOTE), 'the clash note states the policy');
eq((0, recurring_1.createdLine)(0), null, 'nothing created says nothing, rather than announcing a success it did not have');
ok(/1 session booked/.test((0, recurring_1.createdLine)(1) ?? ''), 'one occurrence reads as one');
ok(/8 sessions booked/.test((0, recurring_1.createdLine)(8) ?? ''), 'eight read as eight');
ok(/next 8 weeks/.test((0, recurring_1.createdLine)(8) ?? ''), 'and the horizon is the one the server actually uses');
eq(recurring_1.SERIES_HORIZON_DAYS, 56, 'the horizon matches run_session_series_materialiser’s default');
...
eq((0, recurring_1.shapeSeries)(null).length, 0, 'no rows shape to no series');
eq((0, recurring_1.shapeSeries)([]).length, 0, 'and an empty read is empty, not a crash');
{
    const shaped = (0, recurring_1.shapeSeries)([
...
eq((0, recurring_1.shapeSeries)([raw({ upcoming: '8' })])[0].upcoming, 8, 'a numeric string count is a number');
eq((0, recurring_1.shapeSeries)([raw({ upcoming: null })])[0].upcoming, 0, 'an absent count is zero, not NaN');
eq((0, recurring_1.shapeSeries)([raw({ client_name: '  ' })])[0].clientName, null, 'a blank name is nothing to draw, not a row of spaces');
eq((0, recurring_1.shapeSeries)([raw({ client_name: null })])[0].clientName, null, 'and a client reading their own arrangement is handed null rather than their own name');
eq((0, recurring_1.shapeSeries)([raw({ minute: 45 })])[0].minute, 45, 'quarter hours survive the shaping');
/* ── How it reads on the screen ───────────────────────────────────────────── */
eq((0, recurring_1.clockLabel)(7, 0), '7:00 am', 'seven in the morning');
eq((0, recurring_1.clockLabel)(18, 45), '6:45 pm', 'quarter to seven in the evening');
eq((0, recurring_1.clockLabel)(0, 0), '12:00 am', 'midnight is twelve, not zero');
eq((0, recurring_1.clockLabel)(12, 30), '12:30 pm', 'and half past noon is pm');
eq((0, recurring_1.seriesLabel)({ dow: 2, hour: 7, minute: 0 }), 'Every Tuesday at 7:00 am', 'the whole arrangement in one line');
...
{
    const m = (0, recurring_1.memberSeriesLabel)({ dow: 2, hour: 7, minute: 0 });
    ok(m.includes((0, format_1.weekdayName)(2)), 'it names the day the way the reader writes it');
    ok(m.includes((0, format_1.fmtClock)(7, 0)), 'and the time the way the reader writes it');
    ...
    ok((0, recurring_1.memberSeriesLabel)({ dow: 2, hour: 18, minute: 45 }).includes((0, format_1.fmtClock)(18, 45)), 'quarter to seven in the evening is still quarter to seven in the evening');
}
eq(recurring_1.DOW_NAMES[0], 'Sunday', 'Sunday-first, matching Date.getDay() and extract(dow)');
eq(recurring_1.DOW_NAMES.length, 7, 'seven days');
...
// preview is.
{
    ...
    const dates = (0, recurring_1.seriesDates)(2, 7, 0, 8, from);
    ...
    ok(dates.every((d) => d.getHours() === 7 && d.getMinutes() === 0), 'and every one is at seven in the morning — which is the daylight-saving assertion: '
...
}
{
    ...
    const dates = (0, recurring_1.seriesDates)(2, 7, 0, 2, tuesdayEarly);
    eq(dates[0].getDate(), 1, 'this morning still counts');
    // And is not, when it has.
    const tuesdayLate = new Date(2026, 8, 1, 9, 0, 0);
    ok((0, recurring_1.seriesDates)(2, 7, 0, 2, tuesdayLate)[0].getDate() !== 1, 'an hour that has passed does not');
}
eq((0, recurring_1.seriesDates)(2, 7, 0, 0, new Date(2026, 8, 1)).length, 0, 'zero weeks is zero dates');
/* ── The sentence for ending, at both ends of the count ───────────────────── */
ok(/nothing is removed and nothing is charged/i.test((0, recurring_1.seriesDetail)(0, farOff)), 'ending an arrangement with nothing after it removes nothing and says so');
ok(/removes 1 later session/.test((0, recurring_1.seriesDetail)(1, farOff)), 'one later session reads as one');
ok(/removes 51 later sessions/.test((0, recurring_1.seriesDetail)(51, farOff)), 'fifty-one read as fifty-one');
ok(/No cancellation fee is charged for any of them, however close they are/.test((0, recurring_1.seriesDetail)(51, farOff)), 'and the count does not change the price, which is nothing');
ok(/next session stays booked/i.test((0, recurring_1.seriesDetail)(51, farOff)), 'and the next one is explicitly said to survive, so nobody taps this meaning to cancel it');
...
// actually reaches it.
for (const none of ['', '   ', null, undefined, 'next Tuesday']) {
    const line = (0, recurring_1.seriesDetail)(0, none);
    ok(!/stays booked/i.test(line), `no next occurrence (${JSON.stringify(none)}) promises no surviving session`);
    ok(/no sessions on the books at all/i.test(line), 'and says plainly that there are none, rather than implying one is left');
    ok(/nothing is removed and nothing is charged/i.test(line), 'and still states the price, which is nothing');
}
...
ok(/next session stays booked/i.test((0, recurring_1.seriesDetail)(0, farOff)), 'one session on the books and nothing after it: that one still survives, and is said to');
ok(/no sessions after this one/i.test((0, recurring_1.seriesDetail)(0, farOff)), 'and "after this one" is only said when there IS a this one');
ok(/next session stays booked/i.test((0, recurring_1.seriesDetail)(3, inWindow)), 'a next occurrence inside the notice window survives too — that is the whole point of it');
// Straight through `cancelOptions`, because that is what the screens call and
// the empty string is what they pass.
ok(!/stays booked/i.test(pick((0, recurring_1.cancelOptions)({ startsAt: '', policy: policy(), upcoming: 0, now: NOW }), 'series').detail), 'and the option built for a series with no next occurrence carries no such promise either');
 ...
 */
for (const missing of ['', '   ', 'not-a-date']) {
    const opts = (0, recurring_1.cancelOptions)({ startsAt: missing, policy: policy(), upcoming: 4, now: NOW });
    ok(!opts.some((o) => o.scope === 'occurrence'), `no occurrence option is offered for a startsAt of ${JSON.stringify(missing)}`);
...
{
    const opts = (0, recurring_1.cancelOptions)({ startsAt: farOff, policy: policy(), upcoming: 4, now: NOW });
 ...
 */
{
    ...
    const mine = (0, recurring_1.seriesOccurrencesIn)([
        { ...base, startsAt: tue1 },
        { ...base, startsAt: tue2 },
    ...
    const withOthers = (0, recurring_1.seriesOccurrencesIn)([
    ...
    eq((0, recurring_1.seriesOccurrencesIn)([{ ...base, trainerId: 'tr2', startsAt: tue1 }], SERIES, from, until).length, 0, 'and neither is the same slot with somebody else');
    eq((0, recurring_1.seriesOccurrencesIn)([{ ...base, clientId: 'cl2', startsAt: tue1 }], SERIES, from, until).length, 0, 'nor somebody else’s booking');
    eq((0, recurring_1.seriesOccurrencesIn)([{ ...base, durationMin: 30, startsAt: tue1 }], SERIES, from, until).length, 0, 'nor a half-hour session in the same hour');
    // Status and window.
    eq((0, recurring_1.seriesOccurrencesIn)([{ ...base, status: 'cancelled', startsAt: tue1 }], SERIES, from, until).length, 0, 'a cancelled session is not going to be cancelled again');
    eq((0, recurring_1.seriesOccurrencesIn)([{ ...base, startsAt: tue1 }], SERIES, Date.parse(tue1), until).length, 0, 'the window is exclusive at the near end — a session at "now" is not in the future');
    eq((0, recurring_1.seriesOccurrencesIn)([{ ...base, startsAt: tueGMT }], SERIES, from, Date.parse(tueGMT)).length, 0, 'and exclusive at the far end');
    ...
    eq((0, recurring_1.seriesOccurrencesIn)([{ ...base, startsAt: '2026-08-31T22:00:00.000Z' }], tokyo, from, until).length, 1, 'a Tokyo arrangement is matched in Tokyo’s clock');
    eq((0, recurring_1.seriesOccurrencesIn)([{ ...base, startsAt: '2026-08-31T22:00:00.000Z' }], SERIES, from, until).length, 0, 'and the same instant is not a London Tuesday morning');
    ...
    eq((0, recurring_1.zonedSlot)(tue1, 'Not/AZone'), null, 'a zone that cannot be read says so');
    eq((0, recurring_1.zonedSlot)('not a date', 'Europe/London'), null, 'and so does an unreadable instant');
    eq((0, recurring_1.seriesOccurrencesIn)([{ ...base, startsAt: tue1 }, oneOff], { ...SERIES, tz: 'Not/AZone' }, from, until).length, 2, 'and a series whose zone cannot be read counts everything rather than hiding a cancellation');
    const slot = (0, recurring_1.zonedSlot)(tue1, 'Europe/London');
