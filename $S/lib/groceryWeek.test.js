...
const listOf = (c, coachDay) => {
    const g = (0, meals_1.groceryFromWeek)((0, meals_1.planWeek)(c, coachDay));
    return meals_1.DEPTS.map((d) => `${d}:${(g.byDept[d] ?? []).map((it) => `${it.item}=${it.qty}${it.unit}`).join(',')}`).join(';');
};
/* ── the week is a week ────────────────────────────────────────────────── */
const base = client();
eq((0, meals_1.planWeek)(base).length, meals_1.PLAN_WEEK_DAYS, 'a plan week is seven days');
ok((0, meals_1.planWeek)(base).every((d) => d.length === 4), 'and each of them has the meals-per-day the member set');
// Day zero is the plan as it stands today, not a re-seed. This is the join
// between the two weeks that did not exist before.
eq((0, meals_1.planWeek)(base)[0].map((m) => m.n).join('|'), (0, meals_1.buildPlan)(base).plan.map((m) => m.n).join('|'), 'day one of the week is the day the member is looking at');
...
const before = (0, meals_1.planWeek)(base)[0][0];
const swapped = client({ mealOverride: { 0: (0, meals_1.swapIndex)(base.diet, before.slot, before.idx) } });
const after = (0, meals_1.planWeek)(swapped)[0][0];
ok(after.n !== before.n, 'the fixture actually swaps the meal');
eq((0, meals_1.planWeek)(swapped)[0].map((m) => m.n).join('|'), (0, meals_1.buildPlan)(swapped).plan.map((m) => m.n).join('|'), 'the swapped day is still the day on screen');
...
const written = Array.from({ length: meals_1.PLAN_WEEK_DAYS }, (_, d) => ({ 0: 3 + d, 1: 5 + d }));
const coachDay = (d) => written[d] ?? null;
const coachWeek = (0, meals_1.planWeek)(base, coachDay);
for (let d = 0; d < meals_1.PLAN_WEEK_DAYS; d++) {
    eq(coachWeek[d].map((m) => m.n).join('|'), (0, meals_1.buildPlan)({ ...base, mealOverride: written[d] }).plan.map((m) => m.n).join('|'), `day ${d + 1} of the week is the day the coach wrote`);
}
ok(listOf(base, coachDay) !== listBefore, "and the list is for the coach's week rather than the generated one");
...
const partial = (d) => (d === 3 ? null : written[d]);
eq((0, meals_1.planWeek)(base, partial)[3].map((m) => m.n).join('|'), (0, meals_1.planWeek)(base)[3].map((m) => m.n).join('|'), 'a day the coach did not write keeps the generated one');
/* ── and the list itself still adds up ─────────────────────────────────── */
const g = (0, meals_1.groceryFromWeek)((0, meals_1.planWeek)(base, coachDay));
ok(Object.keys(g.byDept).length >= 3, 'the list spans several departments');
ok(g.mealCount > 0, 'and counts the distinct meals in it');
ok(meals_1.DEPTS.every((d) => (g.byDept[d] ?? []).every((it) => it.qty > 0)), 'every quantity is a real quantity — a zero on a shopping list is an item nobody buys');
