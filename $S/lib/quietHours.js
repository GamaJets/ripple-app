...
exports.QUIET_ORDER_NOTE = exports.QUIET_ZONE_NOTE = exports.QUIET_HELD_NOT_DELAYED = exports.SUGGESTED_QUIET = void 0;
exports.hourInWindow = hourInWindow;
exports.deviceZone = deviceZone;
exports.quietFromRow = quietFromRow;
exports.inQuiet = inQuiet;
exports.hourLabel = hourLabel;
exports.windowLabel = windowLabel;
exports.quietAvailability = quietAvailability;
exports.zoneMovedNote = zoneMovedNote;
 ...
 */
exports.SUGGESTED_QUIET = { fromHour: 22, toHour: 7 };
/**
 ...
 */
function quietFromRow(row) {
    if (!row)
        return null;
    const hour = (v) => (typeof v === 'number' && Number.isInteger(v) && v >= 0 && v <= 23 ? v : null);
    const from = hour(row.from_hour);
    const to = hour(row.to_hour);
    const tz = typeof row.tz === 'string' ? row.tz.trim() : '';
    if (from === null || to === null || from === to || !tz)
        return null;
 ...
 */
function quietAvailability(enforced) {
    if (enforced === true)
        return { available: true, note: null };
    if (enforced === false) {
        return {
            available: false,
            note: 'Quiet hours are not switched on for this server yet. The part that holds them back is not in the app — it '
    ...
    return {
        available: false,
        note: 'Whether this server applies quiet hours could not be read, so nothing is offered here. That is a read that '
...
exports.QUIET_HELD_NOT_DELAYED = 'Inside these hours your phone stays quiet. The notification is still written into your notifications list, so '
...
exports.QUIET_ZONE_NOTE = 'These hours are read in the timezone your phone was in when you set them, because the notification is sent from '
...
exports.QUIET_ORDER_NOTE = 'This sits on top of everything above it. A category you have muted stays muted at every hour, and with Push '
 ...
 */
function zoneMovedNote(stored, device) {
    if (!stored || !device || stored === device)
        return null;
