...
exports.MAX_SESSION_FEE = void 0;
exports.parseSessionFee = parseSessionFee;
exports.sessionFeeFieldValue = sessionFeeFieldValue;
exports.parseGymName = parseGymName;
exports.isBrandColor = isBrandColor;
exports.brandColorOf = brandColorOf;
/**
 ...
 */
exports.MAX_SESSION_FEE = 999999.99;
/**
 ...
 */
function parseSessionFee(input) {
    ...
    if (/^-/.test(bare) || /-/.test(raw))
        return { kind: 'bad', reason: 'A session fee cannot be negative.' };
    ...
    if (!Number.isFinite(fee))
        return { kind: 'bad', reason: 'That is not an amount.' };
    ...
    if (fee === 0)
        return { kind: 'bad', reason: 'A fee of 0 would value every delivered session at nothing. Clear the field instead if you have not set one.' };
    if (fee > exports.MAX_SESSION_FEE)
        return { kind: 'bad', reason: 'That is more than Repple will record as a session fee — check the zeros.' };
    return { kind: 'fee', fee };
}
/**
 ...
 */
function sessionFeeFieldValue(fee) {
 ...
 * never as a value.
 */
function parseGymName(input) {
    const name = String(input ?? '').trim().replace(/\s+/g, ' ');
    if (!name)
        return { kind: 'bad', reason: 'A gym needs a name.' };
    ...
    if (name.length > 80)
        return { kind: 'bad', reason: 'That is longer than a gym name — 80 characters at most.' };
