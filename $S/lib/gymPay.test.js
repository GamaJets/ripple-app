...
{
    const pay = new Map([['t1', rate({ sessionRateCents: 4500 })]]);
    eq((0, gymPay_1.rateForSession)(sess({ rateCents: 3000 }), pay, 2500), 3000, 'a rate snapshotted at delivery WINS — it is what was agreed for that session, and a rise in March must not repay January');
    eq((0, gymPay_1.rateForSession)(sess(), pay, 2500), 4500, 'with no snapshot, what this gym pays THIS coach');
    eq((0, gymPay_1.rateForSession)(sess({ trainerId: 't9' }), pay, 2500), 2500, 'and a coach with no rate of their own falls back to the gym’s standard fee');
    eq((0, gymPay_1.rateForSession)(sess({ trainerId: 't9' }), pay, null), null, 'nothing at any layer is UNPRICED, which is not free');
    // Zero is a decision somebody made — a volunteer, an owner coaching their own
    // clients — and must not fall through to the gym fee.
    const free = new Map([['t1', rate({ sessionRateCents: 0 })]]);
    eq((0, gymPay_1.rateForSession)(sess(), free, 2500), 0, 'a rate of zero is a claim the gym made, not an absent rate');
}
{
    ...
    const priced = (0, gymPay_1.withResolvedRates)(raw, pay, 2500);
    ...
    eq(raw.find((s) => s.id === 'a').rateCents, null, 'the caller’s own rows are NOT mutated — a screen showing a rate the record does not hold is the confusion rate_cents exists to prevent');
    const lines = (0, gymSessions_1.payrollByTrainer)(priced, gymSessions_1.PAY_DELIVERED_ONLY, null, NOW);
    const ana = lines.find((l) => l.trainerId === 't1');
    eq(ana.cents, 7500, 'the payroll figure is 4500 + 3000');
    const settleable = (0, gymSessions_1.settleableSessions)(priced.filter((s) => s.trainerId === 't1'), gymSessions_1.PAY_DELIVERED_ONLY, NOW);
    eq(settleable.length, 2, 'both are settleable — neither is unpriced any more');
    eq((0, gymSessions_1.settlementAmount)(settleable), 7500, 'and the amount handed over equals the figure on screen, which is the disagreement this whole shape exists to prevent');
    ...
    const bare = (0, gymPay_1.withResolvedRates)([sess({ id: 'z', trainerId: 't9' })], pay, null);
    eq(bare[0].rateCents, null, 'unpriced stays unpriced');
    eq((0, gymSessions_1.settleableSessions)(bare, gymSessions_1.PAY_DELIVERED_ONLY, NOW).length, 0, 'and is not settleable');
}
/* ── one currency, or none ─────────────────────────────────────────────────── */
eq((0, gymPay_1.payCurrency)([], 'GBP'), 'GBP', 'no stated rates means the gym’s own currency');
eq((0, gymPay_1.payCurrency)([rate({ currency: 'GBP' })], 'GBP'), 'GBP', 'rates that agree with the gym state it');
eq((0, gymPay_1.payCurrency)([rate({ currency: 'GBP' }), rate({ trainerId: 't2', currency: 'EUR' })], 'GBP'), null, 'two currencies among the rates is no payroll total at all');
eq((0, gymPay_1.payCurrency)([rate({ currency: 'EUR' })], 'GBP'), null, 'a rate in one currency at a gym priced in another is a real disagreement, not a fallback');
eq((0, gymPay_1.payCurrency)([rate({ currency: null })], 'GBP'), 'GBP', 'a coach with no rate states nothing and does not disagree');
...
eq((0, gymPay_1.payCurrency)([rate({ currency: '   ' })], 'GBP'), 'GBP', 'a currency of nothing but spaces states nothing and does not blank the gym’s total');
eq((0, gymPay_1.payCurrency)([rate({ currency: 'gbp' })], 'GBP'), 'GBP', 'and a lowercase tag is the same currency as the gym’s, not a second one');
/* ── a rate, as somebody types it ──────────────────────────────────────────── */
{
    const cents = (s, c = 'GBP') => { const r = (0, gymPay_1.parseRate)(s, c); return r.kind === 'rate' ? r.cents : r.kind; };
    ...
    eq(cents('45', null), 'bad', 'and with no currency a rate is just a number — refused, because this is what somebody is paid');
    ...
    const typed = (minor) => `${Math.floor(minor / 100)}.${String(minor % 100).padStart(2, '0')}`;
    eq(cents(typed(INT4_MAX)), INT4_MAX, 'the largest rate the column can hold is accepted — the boundary is inclusive');
    eq(cents(typed(INT4_MAX + 1)), 'bad', 'and one minor unit past it is refused here, rather than by the database after the form has closed');
}
eq((0, gymPay_1.payRateBlocker)('45', '', '', 'GBP'), null, 'a session rate alone is fine');
eq((0, gymPay_1.payRateBlocker)('', '8', 'per_attendee', 'GBP'), null, 'a class rate with a counting method is fine');
ok((0, gymPay_1.payRateBlocker)('', '8', '', 'GBP') != null, '"80" and "8 a head" are the same digits and different money — a class rate must say which');
ok((0, gymPay_1.payRateBlocker)('', '', 'per_class', 'GBP') != null, 'a counting method with no rate pays nothing');
...
{
    const fromSession = String((0, gymPay_1.payRateBlocker)('45', '', '', null));
    ok((0, gymPay_1.payRateBlocker)('45', '', '', null) != null, 'no currency, no rate — what somebody is paid is a permanent record');
    ok(/currency/i.test(fromSession), 'and the refusal names the missing thing as the currency, not the rate');
    ok(/actually paid/.test(fromSession), 'and says why that cannot be waved through: a rate is what somebody is actually paid');
    ok(!/^Session rate:/.test(fromSession), 'not attributed to a box, because neither box is what is wrong');
    ok(!/Nothing can be worked out from it/.test(fromSession), 'and not the parser\u2019s generic complaint, which describes a difficulty rather than a setting');
    ...
    const fromClass = String((0, gymPay_1.payRateBlocker)('', '8', 'per_class', null));
    ok((0, gymPay_1.payRateBlocker)('', '8', 'per_class', null) != null, 'a class rate alone with no currency is refused on the same ground');
    eq(fromClass, fromSession, 'and reads the same, because it is the same missing setting and the same reason');
}
eq((0, gymPay_1.payRateBlocker)('', '', '', null), null, 'clearing everything needs no currency, because nothing is denominated');
...
async function writeAssertions() {
    {
        ...
        eq(cleared.row.currency, null, 'clearing both rates clears the currency with them — a row with no amounts on it is denominated in nothing');
    ...
    }
    {
        ...
        const threw = async (answer) => {
            try {
                await (0, gymPay_1.saveTrainerPay)(refuse(answer), 'gym-1', {
                    trainerId: 't1', sessionRateCents: 4500, classPayKind: null,
...
eq((0, gymPay_1.classPayAmount)('per_class', 8000, null), 8000, 'a flat class rate ignores the headcount');
eq((0, gymPay_1.classPayAmount)('per_class', 8000, 12), 8000, 'even when there is one');
eq((0, gymPay_1.classPayAmount)('per_attendee', 800, 12), 9600, 'per head multiplies');
eq((0, gymPay_1.classPayAmount)('per_attendee', 800, 0), 0, 'an empty class is a real zero — nobody came');
// The distinction that keeps a coach from being paid nothing because the
// paperwork is missing.
eq((0, gymPay_1.classPayAmount)('per_attendee', 800, null), null, 'a per-head class with no register is UNKNOWN, not free');
eq((0, gymPay_1.classPayAmount)('per_class', -1, null), null, 'a negative rate prices nothing');
...
eq((0, gymPay_1.classPayAmount)('per_class', 0, null), 0, 'a deliberate zero class rate pays zero rather than reading as unpriced');
eq((0, gymPay_1.classPayAmount)('per_attendee', 0, 12), 0, 'and per head, twelve times nothing is still nothing');
{
    const paid = rate({ classPayKind: 'per_attendee', classRateCents: 800 });
    eq((0, gymPay_1.classPayBlocker)(paid, 12, false), null, 'a priced class with a register can go on payroll');
    ok((0, gymPay_1.classPayBlocker)(paid, 12, true) != null, 'and cannot go on it twice');
    ok((0, gymPay_1.classPayBlocker)(paid, null, false) != null, 'a per-head class with no register is refused with words');
    ok((0, gymPay_1.classPayBlocker)(undefined, 12, false) != null, 'a coach with no class rate has nothing to be paid');
    ok((0, gymPay_1.classPayBlocker)(rate({ classPayKind: 'per_class', classRateCents: 8000, currency: null }), null, false) != null, 'a rate with no currency cannot be written down');
    eq((0, gymPay_1.classPayBlocker)(rate({ classPayKind: 'per_class', classRateCents: 8000 }), null, false), null, 'a flat-rate class needs no register at all');
}
/* ── adjustments ───────────────────────────────────────────────────────────── */
eq((0, gymPay_1.adjustmentSign)('bonus'), 1, 'a bonus adds');
eq((0, gymPay_1.adjustmentSign)('reimbursement'), 1, 'and so does money the coach spent being paid back');
eq((0, gymPay_1.adjustmentSign)('deduction'), -1, 'a deduction subtracts');
eq((0, gymPay_1.adjustmentSign)('advance'), -1, 'and so does pay already handed over');
ok(gymPay_1.ADJUSTMENT_KINDS.every((k) => Math.abs((0, gymPay_1.adjustmentSign)(k)) === 1), 'every kind has a sign and none has none');
eq((0, gymPay_1.adjustmentBlocker)('50', 'Covered Saturday', 'GBP'), null, 'an amount and a reason is an adjustment');
ok((0, gymPay_1.adjustmentBlocker)('', 'Covered Saturday', 'GBP') != null, 'no amount, no line');
ok((0, gymPay_1.adjustmentBlocker)('0', 'Covered Saturday', 'GBP') != null, 'an adjustment of nothing changes nothing');
ok((0, gymPay_1.adjustmentBlocker)('50', '', 'GBP') != null, 'a line with no reason on it is the one a coach queries and nobody can answer');
ok((0, gymPay_1.adjustmentBlocker)('50', 'Covered Saturday', null) != null, 'and it has to say what money it is in');
ok((0, gymPay_1.adjustmentBlocker)('-50', 'Covered Saturday', 'GBP') != null, 'the amount is typed POSITIVE — the sign comes from the kind, never from the person typing');
/* ── what a run comes to ───────────────────────────────────────────────────── */
eq((0, gymPay_1.runTotal)({ sessionCents: 7500, sessions: 2, classCents: 9600, classes: 1, adjustmentCents: -2000, adjustments: 1 }), 15100, 'sessions plus classes plus a signed adjustment');
eq((0, gymPay_1.runTotal)({ sessionCents: null, sessions: 0, classCents: 9600, classes: 1, adjustmentCents: 0, adjustments: 0 }), null, 'a run containing unpriced work is not a smaller run — it is one nobody should hand over');
eq((0, gymPay_1.runTotal)({ sessionCents: 0, sessions: 0, classCents: 9600, classes: 1, adjustmentCents: 0, adjustments: 0 }), 9600, 'a coach who taught classes and delivered no one-to-ones is owed for the classes');
eq((0, gymPay_1.runCurrencyBlocker)(['GBP', 'GBP', null]), null, 'one currency, and a silent part, is one currency');
ok((0, gymPay_1.runCurrencyBlocker)(['GBP', 'EUR']) != null, 'two currencies on one run is not a total');
eq((0, gymPay_1.runCurrencyBlocker)([null, null]), null, 'nothing stated is nothing to disagree about');
ok((0, gymPay_1.reversalReasonBlocker)('') != null, 'taking a payroll run back needs a reason');
eq((0, gymPay_1.reversalReasonBlocker)('Paid before the transfer cleared'), null, 'and a reason is a reason');
 ...
 */
{
    const adj = (kind, amountCents, currency) => ({ kind, amountCents, currency });
    const one = (0, gymPay_1.adjustmentsTotal)([adj('bonus', 5000, 'GBP'), adj('deduction', -1500, 'GBP')]);
    eq(one.cents, 3500, 'adjustments in one currency add up');
    eq(one.currency, 'GBP', 'and the total states that currency');
    const two = (0, gymPay_1.adjustmentsTotal)([adj('bonus', 5000, 'GBP'), adj('reimbursement', 4000, 'EUR')]);
    ...
    const split = (0, gymPay_1.adjustmentsTotal)([
    ...
    eq((0, gymPay_1.adjustmentsTotal)([]).cents, null, 'no adjustments is not an adjustment of nothing');
    eq((0, gymPay_1.adjustmentsTotal)([adj('bonus', 100, null)]).cents, null, 'and an adjustment stating no currency is not silently the gym’s');
    eq((0, gymPay_1.adjustmentsTotal)([adj('bonus', 100, ' gbp ')]).currency, 'GBP', 'spacing and case are one currency, not two — the same normalisation sharedCurrency does');
}
 ...
 */
{
    ...
    eq((0, gymPay_1.runScopeOf)('2026-08-15', AUG), 'on', 'a line dated inside the period is on the run');
    eq((0, gymPay_1.runScopeOf)('2026-08-01', AUG), 'on', 'the first day is inside it');
    eq((0, gymPay_1.runScopeOf)('2026-08-31', AUG), 'on', 'and so is the last');
    eq((0, gymPay_1.runScopeOf)('2026-09-01', AUG), 'later', 'a bonus deliberately dated 1 September is not August’s cost');
    eq((0, gymPay_1.runScopeOf)('2026-07-30', AUG), 'earlier', 'an unsettled line from before the period has no other run coming, so it joins this one');
    eq((0, gymPay_1.runScopeOf)(null, AUG), 'undated', 'a line whose date could not be read is placed in no period');
    eq((0, gymPay_1.runScopeOf)('', AUG), 'undated', 'and neither is an empty one');
    eq((0, gymPay_1.runScopeOf)('not a date', AUG), 'undated', 'nor a value that is not a date at all');
    const rows = [
        { id: 'a', on: '2026-08-04' },
        { id: 'b', on: '2026-09-02' },
        { id: 'c', on: '2026-06-19' },
    ...
    eq((0, gymPay_1.scopedToRun)(rows, (r) => r.on, AUG).map((r) => r.id).join(','), 'a,c', 'the run takes the period’s own lines and the stranded earlier ones, and nothing from the future');
}
