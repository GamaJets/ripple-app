...
exports.packDeadline = packDeadline;
exports.bookedBy = bookedBy;
...
const SILENT = { kind: 'silent', text: null, urgent: false, toBook: null, daysLeft: null };
...
function cadence(toBook, daysLeft) {
    if (toBook <= 0 || daysLeft <= 0)
        return null;
    const every = Math.floor(daysLeft / toBook);
    if (every <= 0)
        return null;
    if (every === 1)
        return 'about one a day';
    if (every >= 7)
        return null; // comfortably spaced; saying so is noise
...
function packDeadline(input) {
    ...
    // about the same pack.
    const window = (0, packExpiry_1.packWindow)({ expiresOn, expiredAt: null, sessionsExpired: 0 }, today);
    if (window !== 'soon')
        return SILENT;
    const daysLeft = (0, packExpiry_1.daysLeftOn)({ expiresOn, expiredAt: null, sessionsExpired: 0 }, today);
    const day = (0, packExpiry_1.expiryDayLabel)(expiresOn);
    if (daysLeft == null || !day)
        return SILENT;
 ...
 */
function bookedBy(upcoming, lastDay) {
