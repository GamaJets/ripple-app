...
exports.HK_WRITE_ACTIVITIES = exports.DURATION_SOURCE_LABEL = void 0;
...
exports.humanActivity = humanActivity;
exports.sessionKcal = sessionKcal;
exports.sessionDistanceMeters = sessionDistanceMeters;
exports.planSession = planSession;
exports.planWrite = planWrite;
exports.summariseResult = summariseResult;
exports.readLedger = readLedger;
exports.ledgerAvailable = ledgerAvailable;
exports.writeUnavailableReason = writeUnavailableReason;
...
function groupSessions(log) {
    const by = new Map();
    for (const e of log) {
        if (!e || typeof e.t !== 'string' || !Number.isFinite(Date.parse(e.t)))
            continue;
        ...
        if (g)
            ...
            by.set(key, { key, t: e.t, entries: [e] });
    }
    return [...by.values()].sort((a, b) => Date.parse(b.t) - Date.parse(a.t));
}
exports.DURATION_SOURCE_LABEL = {
 ...
 */
function sessionDuration(entries) {
    let zoneSecs = 0;
    for (const e of entries) {
        if (!e.zones)
            continue;
        const total = (0, hr_1.zoneSecondsTotal)(e.zones);
        if (Number.isFinite(total) && total > zoneSecs)
            zoneSecs = total;
    }
    if (zoneSecs > 0)
        return { seconds: Math.round(zoneSecs), source: 'zones' };
    let cardioSecs = 0;
    for (const e of entries) {
    ...
    if (cardioSecs > 0)
        return { seconds: Math.round(cardioSecs), source: 'cardio' };
    for (const e of entries) {
 ...
 */
exports.HK_WRITE_ACTIVITIES = new Set([
...
function entryActivity(e) {
    ...
    if (Array.isArray(e.sets) && e.sets.length > 0 && !e.cardio)
...
function sessionActivity(entries) {
    ...
    if (!unmapped && seen.size === 1) {
        ...
        if (exports.HK_WRITE_ACTIVITIES.has(activity)) {
...
const KM_PER = { km: 1, k: 1, mi: 1.609344, mile: 1.609344, miles: 1.609344, m: 0.001 };
/**
 ...
 */
function sessionDistanceMeters(entries) {
    const found = [];
    for (const e of entries) {
        const c = e.cardio;
        if (!c)
        ...
        if (factor == null)
            continue; // an unrecognised unit is not a distance we can state
        found.push(d * factor * 1000);
    }
    return found.length === 1 ? Math.round(found[0]) : null;
}
const names = (entries) => entries.map((e) => String(e.exercise || '').trim()).filter(Boolean);
/** Turn one session into a workout to write, or a stated reason it cannot be. */
function planSession(g) {
    ...
    if (!exports.HK_WRITE_ACTIVITIES.has(act.activity)) {
...
function summariseResult(r) {
    ...
    if (r.alreadyWritten > 0)
        parts.push(`${r.alreadyWritten} already there.`);
...
function lazy(mod) {
    try {
        ...
        // eslint-disable-next-line @typescript-eslint/no-var-requires
        else if (mod === 'react-native')
            m = require('react-native');
        // eslint-disable-next-line @typescript-eslint/no-var-requires
        else if (mod === 'health')
            m = require('react-native-health');
        return m?.default ?? m;
    }
    catch {
        return null;
 ...
 */
async function appendLedger(key, rec) {
    const s = storage();
    if (!s)
        return;
    try {
...
function writeUnavailableReason() {
    const rn = lazy('react-native');
    if (!rn)
        return 'Apple Health is only available in the Repple app.';
    if (rn.Platform?.OS !== 'ios')
        return 'Writing to Apple Health is iPhone-only — Health does not exist on this platform.';
    if (!rn.NativeModules?.AppleHealthKit) {
        return 'Needs the Repple app build. HealthKit is native code and is not present in Expo Go or the iOS Simulator without it.';
    }
    if (!nativeHk())
        return 'The Apple Health module in this build cannot save workouts. A newer build is needed.';
    if (!ledgerAvailable()) {
...
function saveOne(p) {
    return new Promise((resolve, reject) => {
        const k = nativeHk();
        if (!k)
            return reject(new Error('The Apple Health module is not available.'));
        if (!exports.HK_WRITE_ACTIVITIES.has(p.activity)) {
        ...
        try {
            k.saveWorkout(options, (err, res) => {
                if (err)
                    return reject(new Error(typeof err === 'string' ? err : err?.message || 'Apple Health refused the write.'));
 ...
 */
async function writeSessions(log, auth, onProgress) {
