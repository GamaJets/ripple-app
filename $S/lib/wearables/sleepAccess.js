...
exports.SLEEP_ASKED_KEY = void 0;
exports.canRememberSleepAsk = canRememberSleepAsk;
exports.hasAskedForSleep = hasAskedForSleep;
exports.markSleepAsked = markSleepAsked;
exports.shouldAutoAskForSleep = shouldAutoAskForSleep;
/**
 ...
 * been asked about it.
 */
exports.SLEEP_ASKED_KEY = 'repple.hk.sleepAsked.v1';
function storage() {
    try {
 ...
 */
function canRememberSleepAsk() {
    return storage() != null;
}
/** True once Repple has requested SleepAnalysis on this device. Never throws. */
async function hasAskedForSleep() {
    const s = storage();
    if (!s)
        return false;
    try {
        return (await s.getItem(exports.SLEEP_ASKED_KEY)) != null;
    }
    catch {
 ...
 */
async function markSleepAsked() {
    const s = storage();
    if (!s)
        return;
    try {
        await s.setItem(exports.SLEEP_ASKED_KEY, new Date().toISOString());
    }
    catch {
 ...
 */
function shouldAutoAskForSleep(i) {
