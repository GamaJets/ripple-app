...
const EVERY = meals_1.ALLERGENS.map((a) => a.id);
/** What the engine thinks is in a dish, tested through the exported surface a
 *  screen actually calls. */
const found = (n, ...items) => (0, meals_1.mealAllergens)({ n, ing: items.map((i) => [i, 1, 'g', 'Pantry & Other']) }, EVERY);
const has = (a, n, ...items) => found(n, ...items).includes(a);
...
eq((0, meals_1.poolGaps)('vegan', 'Breakfast', ['dairy']).length, 0, 'a vegan breakfast can be built without dairy');
eq((0, meals_1.planGaps)('vegan', ['Breakfast', 'Lunch', 'Dinner', 'Snack'], ['dairy']).length, 0, 'and so can a whole vegan day');
eq((0, meals_1.allergenGapNote)((0, meals_1.planGaps)('vegan', ['Breakfast', 'Lunch', 'Dinner', 'Snack'], ['dairy'])), null, 'so the member is told nothing, which is the truth');
...
const note = (0, meals_1.allergenGapNote)(['dairy']);
...
// filter was not honoured. A silent plan with dairy in it is the failure.
{
    ...
    const ids = meals_1.ALLERGENS.map((a) => a.id);
    let silent = 0;
    let checked = 0;
    ...
    for (let mask = 1; mask < (1 << ids.length); mask++) {
        ...
        for (const diet of DIETS) {
            for (const slot of SLOTS) {
                ...
                const gaps = (0, meals_1.poolGaps)(diet, slot, avoid);
                ...
                for (let i = 0; i < Math.min(size, 12); i++) {
                    checked++;
                    const meal = (0, meals_1.mealAt)(diet, slot, i, avoid);
                    const inIt = (0, meals_1.mealAllergens)(meal, avoid);
                    if (inIt.length && gaps.length === 0)
                        silent++;
                }
            }
        }
    ...
    eq(silent, 0, 'no generated meal contains an excluded allergen while the member is told nothing — the combination case used to be exactly this');
}
...
{
    eq((0, types_1.readDiet)('balanced'), 'meat', 'an unrecognised diet reads as the column’s own default');
    eq((0, types_1.readDiet)('vegan'), 'vegan', 'and a real one is untouched');
    eq((0, types_1.readDiet)(null), 'meat', 'as is a null');
    let threw = false;
    try {
        // Cast deliberately: this is the shape of a row that has been in the
        // database, not a shape the type system allows.
        const meal = (0, meals_1.mealAt)('balanced', 'Breakfast', 0, []);
