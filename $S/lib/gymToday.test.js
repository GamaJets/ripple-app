...
const gymToday_1 = require("./gymToday");
...
const utcDayStart = (at) => new Date(new Date(at).toISOString().slice(0, 10) + 'T00:00:00Z').toISOString();
const hours = (w) => (Date.parse(w.toISO) - Date.parse(w.fromISO)) / 3600000;
...
const la = (0, gymToday_1.gymTodayWindow)(LA, laMorning);
...
ok((0, gymToday_1.inWindow)(laMorning, la), 'the instant asked about is inside its own day');
...
eq((0, gymZone_1.gymDay)(lastNight, LA), '2026-08-31', 'that class ran on the 31st at the gym');
ok(!(0, gymToday_1.inWindow)(lastNight, la), 'so it is not in today’s window');
ok(lastNight >= Date.parse(utcDayStart(laMorning)), 'and the UTC window this replaces did count it — which is the bug');
...
const dxb = (0, gymToday_1.gymTodayWindow)(DUBAI, dubaiMorning);
...
eq((0, gymZone_1.gymDay)(earlyBird, DUBAI), '2026-09-01', 'a 1am arrival is on the 1st at the gym');
ok((0, gymToday_1.inWindow)(earlyBird, dxb), 'and is counted in today');
ok(earlyBird < Date.parse(utcDayStart(dubaiMorning)), 'where the UTC window this replaces dropped it — the same bug pointing the other way');
/* ── the two days a year that are not 24 hours ──────────────────────────── */
const back = (0, gymToday_1.gymTodayWindow)(LONDON, Date.parse('2026-10-25T12:00:00Z'));
...
const fwd = (0, gymToday_1.gymTodayWindow)(LONDON, Date.parse('2026-03-29T12:00:00Z'));
...
for (const [label, z] of [
    ...
    const w = (0, gymToday_1.gymTodayWindow)(z, laMorning);
    eq(w.basis, 'reader', `${label} is not a zone, so the day is the reader’s`);
    eq(w.zone, null, `${label} leaves no zone on the window`);
    eq(w.note, gymZone_1.NO_ZONE_NOTE, `${label} carries the shared sentence saying so`);
    ok((0, gymToday_1.inWindow)(laMorning, w), `${label} still produces a window containing the instant`);
}
...
{
    const w = (0, gymToday_1.gymTodayWindow)(null, laMorning);
    const d = new Date(laMorning);
    const p2 = (n) => String(n).padStart(2, '0');
    eq(w.day, `${d.getFullYear()}-${p2(d.getMonth() + 1)}-${p2(d.getDate())}`, 'the fallback day is the local calendar day');
    eq(w.fromISO, new Date(d.getFullYear(), d.getMonth(), d.getDate()).toISOString(), 'and it opens at local midnight');
    ok(hours(w) >= 23 && hours(w) <= 25, 'and is a day long, give or take a clock change');
}
/* ── the window is half-open ────────────────────────────────────────────── */
ok((0, gymToday_1.inWindow)(la.fromISO, la), 'midnight belongs to the day it opens');
ok(!(0, gymToday_1.inWindow)(la.toISO, la), 'and the next midnight belongs to the next day, not both');
ok(!(0, gymToday_1.inWindow)(null, la), 'nothing is not inside a window');
ok(!(0, gymToday_1.inWindow)('not a date', la), 'and neither is an unreadable instant');
ok((0, gymToday_1.inWindow)(new Date(laMorning), la), 'a Date is accepted as well as an ISO string');
...
for (const z of [LA, DUBAI, LONDON]) {
    for (const at of [laMorning, dubaiMorning, Date.parse('2026-12-31T23:30:00Z')]) {
        const w = (0, gymToday_1.gymTodayWindow)(z, at);
        eq(w.day, (0, gymZone_1.gymDay)(at, z), `${z} at ${new Date(at).toISOString()}: the day is gymDay’s day`);
        ok((0, gymToday_1.inWindow)(at, w), `${z} at ${new Date(at).toISOString()}: and the window contains it`);
    }
}
// A Date and the millisecond count it holds are the same instant.
eq((0, gymToday_1.gymTodayWindow)(LA, new Date(laMorning)).fromISO, la.fromISO, 'a Date argument is the same as a number');
if (errors.length) {
