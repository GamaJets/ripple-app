...
exports.wholeMoney = exports.minorMoney = exports.THREE_DECIMAL = exports.ZERO_DECIMAL = void 0;
exports.currencyDecimals = currencyDecimals;
exports.moneyIn = moneyIn;
exports.majorFromMinor = majorFromMinor;
exports.minorFromWhole = minorFromWhole;
exports.minorFromDecimal = minorFromDecimal;
exports.readMinorAmount = readMinorAmount;
exports.sumTaken = sumTaken;
exports.combineTaken = combineTaken;
exports.sumRecurring = sumRecurring;
exports.feeMismatches = feeMismatches;
exports.since = since;
exports.monthStart = monthStart;
exports.packLeft = packLeft;
exports.packRunOut = packRunOut;
...
exports.ZERO_DECIMAL = new Set(['bif', 'clp', 'djf', 'gnf', 'jpy', 'kmf', 'krw', 'mga', 'pyg', 'rwf', 'ugx', 'vnd', 'vuv', 'xaf', 'xof', 'xpf']);
/**
 ...
 */
exports.THREE_DECIMAL = new Set(['bhd', 'jod', 'kwd', 'omr', 'tnd']);
/**
...
function currencyDecimals(currency) {
    ...
    if (!cur)
        return null;
    if (exports.ZERO_DECIMAL.has(cur))
        return 0;
    if (exports.THREE_DECIMAL.has(cur))
        return 3;
    return 2;
}
/**
 ...
 */
function moneyIn(amount, currency, minor) {
    ...
    return `${cur.toUpperCase()} ${whole.toLocaleString((0, locale_1.appLocale)(), { minimumFractionDigits: dp, maximumFractionDigits: dp })}`;
}
/** A Stripe amount, in minor units. */
const minorMoney = (amount, currency) => moneyIn(amount, currency, true);
exports.minorMoney = minorMoney;
/** A whole-unit amount somebody typed — a session rate, a revenue target. */
const wholeMoney = (amount, currency) => moneyIn(amount, currency, false);
exports.wholeMoney = wholeMoney;
/**
 ...
 */
function minorFromWhole(whole, currency) {
    const dp = currencyDecimals(currency);
    if (dp == null || whole == null || !Number.isFinite(whole))
        return null;
 ...
 */
function minorFromDecimal(raw, currency) {
    const dp = currencyDecimals(currency);
    if (dp == null || raw == null)
        return null;
 ...
 * charged.
 */
function readMinorAmount(typed, currency, chargeable = true) {
    ...
    if (!raw)
        return { ok: false, reason: 'Type an amount.' };
 ...
 */
function sumTaken(rows) {
    const by = new Map();
    let unlabelled = 0;
    let unpriced = 0;
    for (const r of rows) {
        if (r.amount_cents == null || !Number.isFinite(r.amount_cents)) {
            unpriced += 1;
            continue;
        }
        ...
        if (pot) {
            pot.minorUnits += r.amount_cents;
            pot.count += 1;
        }
        else
            by.set(cur, { currency: cur, minorUnits: r.amount_cents, count: 1 });
    }
...
function combineTaken(...parts) {
    ...
    for (const part of parts) {
        ...
        for (const p of part.pots) {
            ...
            if (pot) {
                pot.minorUnits += p.minorUnits;
                pot.count += p.count;
            }
            else
                by.set(p.currency, { currency: p.currency, minorUnits: p.minorUnits, count: p.count });
        }
    }
    const pots = [...by.values()].sort((a, b) => (b.minorUnits - a.minorUnits) || a.currency.localeCompare(b.currency));
    return { pots, unlabelled, unpriced };
...
function sumRecurring(rows) {
    ...
    for (const r of rows) {
        ...
        if (pot) {
            ...
            by.set(k, { currency: cur, interval: iv, minorUnits: r.amount_cents, count: 1 });
    }
 ...
 */
function packLeft(r) {
    if (r.sessions_total == null || !Number.isFinite(r.sessions_total))
        return null;
