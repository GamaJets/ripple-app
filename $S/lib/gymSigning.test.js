...
eq((0, gymSigning_1.attributionOf)('member'), 'member', 'the member signing for themselves is carried through');
eq((0, gymSigning_1.attributionOf)('staff'), 'staff', 'and so is a desk entry');
eq((0, gymSigning_1.attributionOf)('unknown'), 'unknown', 'and so is a row from before this was recorded');
...
for (const junk of [null, undefined, '', 'MEMBER', 'gym', 0, {}, ['member']]) {
    eq((0, gymSigning_1.attributionOf)(junk), 'unknown', `an unrecognised attribution (${JSON.stringify(junk)}) is unknown, never a claim about who signed`);
}
ok(['member', 'staff', 'unknown'].every((k) => gymSigning_1.ATTRIBUTION_LABEL[k] && gymSigning_1.ATTRIBUTION_NOTE[k]), 'every attribution has words an owner reads, and a note saying what it is worth');
ok(gymSigning_1.ATTRIBUTION_NOTE.unknown.includes('not known'), 'the unknown note says outright that it is not known, rather than implying a desk entry');
{
    const t = (0, gymSigning_1.tallyAttribution)([
    ...
    const empty = (0, gymSigning_1.tallyAttribution)([]);
...
const gave = (agreementId, attribution = 'member') => ({
...
});
{
    ...
    const rows = (0, gymSigning_1.forMember)([v1, v2], [gave('w1')]);
    ...
    ok((0, gymSigning_1.waitingOn)(rows[0]), 'so the new wording is still waiting on her');
    eq((0, gymSigning_1.waitingCount)(rows), 1, 'and it counts as one thing outstanding');
}
{
    const w = agree({ id: 'w1' });
    const rows = (0, gymSigning_1.forMember)([w], [gave('w1', 'staff')]);
    ...
    eq((0, gymSigning_1.waitingOn)(rows[0]), false, 'it is not asked for a second time — the gym does hold something');
...
}
{
    ...
    const rows = (0, gymSigning_1.forMember)([g], []);
    eq(rows[0].refusal, gymSigning_1.GUARDIAN_REFUSAL, 'a guardian consent cannot be given from the minor’s own account');
    eq((0, gymSigning_1.waitingOn)(rows[0]), false, 'and it is not counted as waiting on them: there is no button, so counting it sends somebody hunting for one');
    eq((0, gymSigning_1.waitingCount)(rows), 0, 'nothing here is waiting on this member');
    ...
    eq((0, gymSigning_1.outstanding)(rows[0]), true, 'IT IS STILL AN UNSIGNED DOCUMENT THE GYM IS MISSING');
    eq((0, gymSigning_1.blockedOn)(rows[0]), true, 'and it is unsigned by somebody other than this member');
    eq(JSON.stringify((0, gymSigning_1.agreementStanding)(rows)), JSON.stringify({ waiting: 0, blocked: 1, outstanding: 1 }), 'the count and the button are two different questions and now have two answers');
    const line = (0, gymSigning_1.agreementSummary)(rows);
    ok(!/Nothing is waiting on you/.test(line), 'A MINOR IS NEVER TOLD THEY ARE CLEAR WHILE THE GUARDIAN CONSENT IS UNSIGNED');
    ...
    const given = (0, gymSigning_1.forMember)([g], [gave('g1', 'staff')]);
    eq((0, gymSigning_1.outstanding)(given[0]), false, 'a guardian consent on file is not missing');
    eq((0, gymSigning_1.blockedOn)(given[0]), false, 'and nobody is being waited on for it');
    eq((0, gymSigning_1.agreementSummary)(given), 'Nothing is waiting on you.', 'so the screen may say so');
}
{
    // The three other shapes of the summary line.
    const g = agree({ id: 'g1', kind: 'guardian_consent', title: 'Guardian consent' });
    const w = agree({ id: 'w1', title: 'Liability waiver' });
    eq((0, gymSigning_1.agreementSummary)((0, gymSigning_1.forMember)([w], [gave('w1')])), 'Nothing is waiting on you.', 'everything signed, and nobody else owes anything');
    eq((0, gymSigning_1.agreementSummary)((0, gymSigning_1.forMember)([w], [])), '1 document waiting on you.', 'one the member can sign themselves reads as it always did');
    const both = (0, gymSigning_1.agreementSummary)((0, gymSigning_1.forMember)([w, g], []));
    ...
    eq(JSON.stringify((0, gymSigning_1.agreementStanding)((0, gymSigning_1.forMember)([w, g], []))), JSON.stringify({ waiting: 1, blocked: 1, outstanding: 2 }), 'and the standing is the two of them');
    ...
    ok(/2 documents your gym asks for are still unsigned/.test((0, gymSigning_1.agreementSummary)((0, gymSigning_1.forMember)([g, g2], []))), 'two of them read as two');
    ok(/they can only be given/.test((0, gymSigning_1.agreementSummary)((0, gymSigning_1.forMember)([g, g2], []))), 'and the pronoun follows the count');
}
{
    // Unsigned first, then required before optional. What is waiting on the
    // reader is what the screen is for.
    const rows = (0, gymSigning_1.forMember)([
    ...
    eq(rows.map((r) => r.id).join(','), 'must,opt,done', 'the required unsigned one is first, the optional unsigned one next, and what is done is last');
}
...
eq((0, gymSigning_1.signingBlocker)(pending(), 'Sara Ahmed', true, true), null, 'read it, ticked it, typed a name: that is a signature');
ok((0, gymSigning_1.signingBlocker)(pending(), 'Sara Ahmed', true, false) != null, 'agreeing to a document you have not opened is not agreeing to anything');
ok((0, gymSigning_1.signingBlocker)(pending(), 'Sara Ahmed', false, true) != null, 'and neither is a name with no agreement beside it');
ok((0, gymSigning_1.signingBlocker)(pending(), '', true, true) != null, 'the name is the signature, so there is no signature without one');
ok((0, gymSigning_1.signingBlocker)(pending(), '   ', true, true) != null, 'and whitespace is not a name');
ok((0, gymSigning_1.signingBlocker)(pending(), 'S', true, true) != null, 'nor is a single letter');
eq((0, gymSigning_1.signingBlocker)(pending({ refusal: gymSigning_1.GUARDIAN_REFUSAL }), 'Sara Ahmed', true, true), gymSigning_1.GUARDIAN_REFUSAL, 'a refusal outranks everything else, and says why in the words the member reads');
ok((0, gymSigning_1.signingBlocker)(pending({ signedAt: '2026-02-01T10:00:00Z' }), 'Sara Ahmed', true, true) != null, 'and a version already signed is not offered again');
ok(gymSigning_1.SIGNING_RULE.includes('cannot be edited'), 'the sentence shown before the tap says the record cannot be edited, because it cannot');
    ...
    const client = {
        ...
        from(table) {
            const q = {
                ...
                select: (cols) => { selects.push({ table, cols, filters: [] }); return q; },
                eq: (col) => { selects[selects.length - 1].filters.push(col); return q; },
                order: () => q,
...
void (async () => {
    {
        ...
        await (0, gymSigning_1.signAsMember)(client, 'tenant-1', { agreementId: 'w2', version: 2, signedName: '  Sara Ahmed  ' });
        ...
        eq(row.member_id, 'uid-sara', 'attributed to the account the SESSION reports, which is the one fact a caller cannot forge');
        ...
        ok(!('attribution' in row), 'attribution is NOT sent: it is derived in the database from auth.uid(), and an app that could set it is the original defect one level up');
        ok(!('witnessed_by' in row), 'no witness is sent: the person signing is the person here, and a witness beside a member signature would be staff attesting to something they were not present for');
        ok(!('signed_by' in row), 'and the writer is not sent either — supabase/parts/520 stamps it, so it cannot be claimed');
    }
    {
    ...
    }
    {
        ...
        await (0, gymSigning_1.fetchMySignatures)(client);
        eq(selects.length, 1, 'one read');
        eq(selects[0].filters.length, 0, 'and it filters on nothing: the session decides whose signatures these are, not a caller-supplied id');
        ok(selects[0].cols.includes('attribution'), 'and it reads the attribution, because a signature whose origin is not read renders as one whose origin is not recorded');
    }
    {
        ...
        const { client, selects } = fakeClient('uid-sara');
        await (0, gymSigning_1.fetchGymAgreements)(client, 'tenant-1');
        eq(selects[0].filters.join(','), 'tenant_id,active', 'the agreements read is scoped to this gym and to the version in force');
        ok(selects[0].cols.includes('body'), 'and it reads the BODY: a signing screen without the wording on it records agreement to a document nobody was shown');
    }
    if (errors.length) {
