...
const ids = (i) => (0, checklist_1.buildChecklist)(i).items.map((x) => x.id);
const labelOf = (i, id) => (0, checklist_1.buildChecklist)(i).items.find((x) => x.id === id)?.label ?? '';
// ── nothing known → nothing claimed ──
const empty = (0, checklist_1.buildChecklist)(NOTHING);
...
ok(!(0, checklist_1.buildChecklist)({ ...NOTHING, waterGoalGlasses: 8 }).gaps.some((g) => g.id === 'water'), 'a client who HAS a water goal is not told to set one');
// Separately, so somebody who has set one is not told to set it.
ok(!(0, checklist_1.buildChecklist)({ ...NOTHING, stepGoal: 8000 }).gaps.some((g) => g.id === 'steps'), 'a client who HAS a step goal is not told to set one');
ok((0, checklist_1.buildChecklist)({ ...NOTHING, stepGoal: 8000 }).gaps.some((g) => g.id === 'sleep'), 'and setting the step goal does not silence the sleep note with it');
...
ok(!ids({ ...NOTHING, kcalTarget: 2140, proteinTargetG: 152 }).includes('steps'), 'no step goal must mean no step row — never a fallback 10,000');
...
ok(!(0, checklist_1.buildChecklist)({ ...NOTHING, kcalTarget: 2140, proteinTargetG: 152 }).gaps.some((g) => g.id === 'macros'), 'a client who HAS macro targets must not be told to go and set them');
// ── the training row comes from the plan, on the days the plan schedules ──
const ppl = (0, programs_1.buildProgram)('muscle', 18).days; // Mon / Wed / Fri
ok((0, checklist_1.scheduledFocus)(ppl, 1) === 'Push', 'Monday of a PPL plan is Push');
ok((0, checklist_1.scheduledFocus)(ppl, 3) === 'Pull', 'Wednesday of a PPL plan is Pull');
ok((0, checklist_1.scheduledFocus)(ppl, 0) === null, 'Sunday is not in a Mon/Wed/Fri plan and must not borrow the nearest day');
ok((0, checklist_1.scheduledFocus)(ppl, 2) === null, 'Tuesday is not a training day in a Mon/Wed/Fri plan');
ok((0, checklist_1.scheduledFocus)([], 1) === null, 'a plan with no days schedules nothing');
...
{
    const week = [1, 2, 3, 4, 5, 6, 0].map((wd) => (0, checklist_1.scheduledDay)(ppl, wd)); // Mon → Sun
    ...
    ok((0, checklist_1.scheduledDay)([], 1) === null, 'a plan with no days schedules nothing');
    ok((0, checklist_1.scheduledDay)([{ day: 'Nonesuch', focus: 'X' }], 1) === null, 'a day naming no real weekday lands on none of them');
    ok((0, checklist_1.scheduledDay)([{ day: 'monday', focus: 'X' }], 1)?.focus === 'X', 'the match is on the first three letters, case-insensitively');
    ...
    for (const wd of [0, 1, 2, 3, 4, 5, 6]) {
        ok((0, checklist_1.scheduledFocus)(ppl, wd) === ((0, checklist_1.scheduledDay)(ppl, wd)?.focus ?? null), `focus and day agree on weekday ${wd}`);
    }
}
// ── coach items ──
const coached = (0, checklist_1.buildChecklist)({
    ...NOTHING,
    waterGoalGlasses: 8,
    coachItems: [
...
ok(coachIds[0] === (0, checklist_1.coachHabitId)('a1b2'), 'a coach item is keyed on its row id');
ok(coachIds.every((id) => id.startsWith(checklist_1.COACH_ID_PREFIX)), 'coach ids must be namespaced away from derived ones');
ok(coached.items.some((i) => i.label === 'Ten minutes of mobility'), 'a coach label is trimmed, not dropped');
ok(coached.items.find((i) => i.id === (0, checklist_1.coachHabitId)('c3d4'))?.icon !== '', 'a coach item with no icon still gets one');
// Renaming the label must not move the storage key, or the tick history detaches.
const renamed = (0, checklist_1.buildChecklist)({ ...NOTHING, coachItems: [{ id: 'a1b2', label: 'Something else entirely' }] });
...
// ── the hero percentage ──
ok((0, checklist_1.donePercent)(0, 0) === null, 'an empty checklist has no percentage — this is the divide-by-zero that rendered NaN%');
ok((0, checklist_1.donePercent)(3, 0) === null, 'a done count over an empty list is still nothing to show');
ok((0, checklist_1.donePercent)(0, 4) === 0, 'nothing ticked is 0%, which is a real answer');
ok((0, checklist_1.donePercent)(2, 4) === 50, 'half ticked is 50%');
ok((0, checklist_1.donePercent)(4, 4) === 100, 'all ticked is 100%');
ok((0, checklist_1.donePercent)(5, 4) === 100, 'a stale done count must not exceed 100%');
ok((0, checklist_1.donePercent)(1, NaN) === null, 'a non-finite total has no percentage');
ok((0, checklist_1.donePercent)(1, -4) === null, 'a negative total has no percentage — clamping it to 0% would state a fact');
...
ok((0, checklist_1.donePercent)(1, 201) === 1, 'one habit ticked out of 201 is not nought per cent — the day has been started');
ok((0, checklist_1.donePercent)(200, 201) === 99, 'one habit outstanding out of 201 is not a hundred per cent — a box is still open');
ok((0, checklist_1.donePercent)(0, 201) === 0, 'but nothing ticked on a long list is still a real nought');
ok((0, checklist_1.donePercent)(201, 201) === 100, 'and everything ticked on a long list is still a real hundred');
