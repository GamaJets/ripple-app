...
exports.SESSION_GAP_MIN = exports.AGREEMENT_TOLERANCE_MIN = exports.FAMILY_ORDER = void 0;
exports.nightKey = nightKey;
exports.recentNights = recentNights;
exports.foldSleepIntervals = foldSleepIntervals;
exports.nightsFromIntervals = nightsFromIntervals;
exports.mergeSleepNight = mergeSleepNight;
exports.mergeSleepNights = mergeSleepNights;
exports.markNightsUnread = markNightsUnread;
exports.formatSleepHours = formatSleepHours;
 ...
 */
exports.FAMILY_ORDER = ['oura', 'whoop', 'fitbit', 'garmin', 'watch', 'phone', 'manual', 'unknown'];
const familyRank = (f) => {
    const i = exports.FAMILY_ORDER.indexOf(f);
    return i < 0 ? exports.FAMILY_ORDER.length : i;
};
/**
 ...
 */
exports.AGREEMENT_TOLERANCE_MIN = 15;
/**
 ...
 */
function recentNights(count, now = new Date()) {
    const out = [];
 ...
 */
function foldSleepIntervals(intervals) {
    const spans = [];
 ...
 */
function nightsFromIntervals(intervals, gapMin = exports.SESSION_GAP_MIN) {
...
const basisRank = (b) => (b === 'asleep' ? 0 : 1);
const byPrecedence = (a, b) => basisRank(a.basis) - basisRank(b.basis) ||
 ...
 */
function mergeSleepNight(night, readings, failed = []) {
    ...
    for (const r of usable) {
        if (r.basis !== chosen.basis || seen.has(r.family))
            continue;
        seen.add(r.family);
        independent.push(r);
    }
    ...
    const agreement = independent.length < 2 ? 'single' : spreadMin <= exports.AGREEMENT_TOLERANCE_MIN ? 'corroborated' : 'conflicting';
    return {
        night,
        outcome: 'measured',
 ...
 */
function mergeSleepNights(reads, nights) {
    const all = [];
    const failed = [];
    for (const rd of reads || []) {
        if (!rd)
            continue;
        if (rd.status === 'error')
            failed.push(rd.provider);
        if (rd.status === 'ready')
            all.push(...(rd.readings || []));
    }
 ...
 */
function markNightsUnread(nights, asked = []) {
    const failed = [...new Set(asked)];
    return (nights || []).map((n) => {
        if (n.outcome === 'measured')
            return { ...n, failed: [...new Set([...n.failed, ...failed])] };
        return { ...n, outcome: 'unknown', failed: [...new Set([...n.failed, ...failed])] };
    });
}
