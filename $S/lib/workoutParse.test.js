...
const workoutParse_1 = require("./workoutParse");
...
// ── the bug ────────────────────────────────────────────────────────────────
{
    const lb = (0, workoutParse_1.parseWorkoutText)('bench 3x8 @135', 'lb');
    ok(lb.length === 1, `one lift from a pounds sentence, got ${lb.length}`);
    const w = lb[0]?.sets[0]?.[1] ?? -1;
    ok(near(w, 135 * units_1.KG_PER_LB), `"@135" from a pounds member is 61.2kg, got ${w}`);
    ...
    const kg = (0, workoutParse_1.parseWorkoutText)('bench 3x8 @135', 'kg');
    ok(near(kg[0]?.sets[0]?.[1] ?? -1, 135), 'the same sentence from a kg member is 135kg');
}
// ── a suffix still means what it says ──────────────────────────────────────
{
    const explicit = (0, workoutParse_1.parseWorkoutText)('squat 5x5 100kg', 'lb');
    ok(near(explicit[0]?.sets[0]?.[1] ?? -1, 100), 'writing "100kg" while set to pounds is deliberate and stays 100kg');
    const lbs = (0, workoutParse_1.parseWorkoutText)('squat 5x5 225lb', 'kg');
    ok(near(lbs[0]?.sets[0]?.[1] ?? -1, 225 * units_1.KG_PER_LB), 'and "225lb" while set to kg converts, got ' + lbs[0]?.sets[0]?.[1]);
}
...
{
    const curls = (0, workoutParse_1.parseWorkoutText)('curl @20 10 10', 'lb');
    ...
    ok(sets.every(([, w]) => near(w, 20 * units_1.KG_PER_LB)), 'each at 9kg');
}
// ── the "5x100" reading, also a bare number ────────────────────────────────
{
    const heavy = (0, workoutParse_1.parseWorkoutText)('deadlift 5x315', 'lb');
    const s = heavy[0]?.sets ?? [];
    ok(s.length === 1 && s[0][0] === 5, `"5x315" is five reps, got ${JSON.stringify(s)}`);
    ok(near(s[0]?.[1] ?? -1, 315 * units_1.KG_PER_LB), `at 142.9kg, got ${s[0]?.[1]}`);
}
...
{
    const d = (0, workoutParse_1.parseWorkoutText)('bench 3x8 60kg, squat 100kg 5 5 5', 'kg');
    ok(d.length === 2, `two clauses, got ${d.length}`);
    ok(d[0]?.exercise === 'Bench' && d[1]?.exercise === 'Squat', `names survive, got ${d.map((x) => x.exercise).join('/')}`);
    ...
    const bodyweight = (0, workoutParse_1.parseWorkoutText)('3x12 pushups', 'kg');
    ok(bodyweight[0]?.sets.length === 3 && bodyweight[0].sets.every(([, w]) => w === 0), 'bodyweight stays zero rather than picking up a unit');
}
if (errors.length) {
