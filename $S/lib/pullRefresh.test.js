...
function rig(reload) {
    ...
    const said = [];
    const r = (0, pullRefresh_1.makeRefresher)({
        reload,
...
async function main() {
    /* ── the ordinary pull ─────────────────────────────────────────────────── */
    {
        ...
        t.clock.advance(pullRefresh_1.MIN_SPIN_MS - 1);
        await settle();
    ...
    /* ── a read that failed still ends the spin ────────────────────────────── */
    {
        ...
        t.onRefresh();
        await settle();
        t.clock.advance(pullRefresh_1.MIN_SPIN_MS);
        await settle();
    ...
    }
    {
        ...
        const t = rig((() => { throw new Error('nope'); }));
        t.onRefresh();
        await settle();
        t.clock.advance(pullRefresh_1.MIN_SPIN_MS);
        await settle();
    ...
    /* ── the report: a read that never comes back ──────────────────────────── */
    {
        ...
        t.clock.advance(pullRefresh_1.MAX_SPIN_MS - pullRefresh_1.MIN_SPIN_MS);
        await settle();
    ...
    /* ── and the gesture still works afterwards ────────────────────────────── */
    {
        ...
        t.onRefresh();
        await settle();
        t.clock.advance(pullRefresh_1.MAX_SPIN_MS);
        await settle();
    ...
    /* ── a hung read that answers LATE must not end somebody else's spin ───── */
    {
        const h = held();
        let ran = 0;
        const t = rig(() => { ran += 1; return ran === 1 ? h.promise : new Promise(() => { }); });
        t.onRefresh();
        await settle();
        t.clock.advance(pullRefresh_1.MAX_SPIN_MS);
        await settle();
    ...
    /* ── the two bounds ────────────────────────────────────────────────────── */
    {
        ok(pullRefresh_1.MIN_SPIN_MS > 0 && pullRefresh_1.MIN_SPIN_MS < 1000, 'the floor is long enough to be seen and short enough not to be waited on');
        ok(pullRefresh_1.MAX_SPIN_MS > pullRefresh_1.MIN_SPIN_MS * 10, 'the ceiling is far past any read that was going to answer');
    }
    if (errors.length) {
