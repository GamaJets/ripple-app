...
exports.tagsText = tagsText;
exports.isReachable = isReachable;
exports.contactLine = contactLine;
exports.searchableFields = searchableFields;
exports.isEmptyPatch = isEmptyPatch;
exports.fetchMemberRecords = fetchMemberRecords;
exports.byMember = byMember;
exports.saveMemberRecord = saveMemberRecord;
...
function contactLine(r) {
    if (!r)
        return null;
    const bits = [r.phone?.trim(), r.email?.trim()].filter((b) => !!b);
...
function rowToRecord(r) {
    return {
        ...
        tags: Array.isArray(r.tags) ? r.tags.filter((t) => typeof t === 'string' && t.trim()) : [],
 ...
 */
async function fetchMemberRecords(sb, tenantId) {
    ...
    if (error)
        throw error;
    return (0, rowCap_1.assertWhole)(data, "this gym's member records").map(rowToRecord);
}
 ...
 */
async function saveMemberRecord(sb, tenantId, memberId, patch, updatedBy) {
    const row = {
        tenant_id: tenantId,
        member_id: memberId,
    ...
    if (patch.phone !== undefined)
        row.phone = nullIfBlank(patch.phone);
    if (patch.email !== undefined)
        row.email = nullIfBlank(patch.email);
    if (patch.emergencyName !== undefined)
        row.emergency_name = nullIfBlank(patch.emergencyName);
    if (patch.emergencyPhone !== undefined)
        row.emergency_phone = nullIfBlank(patch.emergencyPhone);
    if (patch.medicalNote !== undefined)
        row.medical_note = nullIfBlank(patch.medicalNote);
    if (patch.note !== undefined)
        row.note = nullIfBlank(patch.note);
    ...
    if (r.error)
        throw r.error;
    (0, wroteRows_1.assertWrote)('That member record', r);
}
