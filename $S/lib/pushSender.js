...
exports.PUSH_FANOUT_LOG_AT = exports.PUSH_SENDER_REFUSED = void 0;
exports.pushSender = pushSender;
exports.logFanout = logFanout;
/**
 * The rule.
 *
 ...
 */
function pushSender(bearer, serviceKey, userId) {
    const b = (bearer ?? '').trim();
    const k = (serviceKey ?? '').trim();
    if (k && b && b === k)
        return { senderId: 'server', allowed: true, kind: 'server' };
    const u = (userId ?? '').trim();
    if (u)
        return { senderId: u, allowed: true, kind: 'person' };
...
exports.PUSH_SENDER_REFUSED = 'Sign in to Repple to send a notification.';
/**
 ...
 * told what.
 */
exports.PUSH_FANOUT_LOG_AT = 50;
/** Whether this send is one. */
function logFanout(recipients) {
    return Number.isFinite(recipients) && recipients > exports.PUSH_FANOUT_LOG_AT;
}
