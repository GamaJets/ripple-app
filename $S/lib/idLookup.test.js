...
async function run(p) {
    try {
        return { value: await p, threw: null };
    }
    catch (e) {
...
async function main() {
    ...
    {
        const got = (0, idLookup_1.uniqueIds)(['a', 'b', 'a', null, undefined, '', 'c', 'b']);
        eq(JSON.stringify(got), JSON.stringify(['a', 'b', 'c']), 'duplicates collapse, blanks and nulls fall out, first-seen order survives');
    }
    {
        ...
        const got = (0, idLookup_1.uniqueIds)(['z', 'a', 'm']);
        eq(JSON.stringify(got), JSON.stringify(['z', 'a', 'm']), 'the list is not sorted behind the caller');
    }
    eq((0, idLookup_1.uniqueIds)([]).length, 0, 'nothing in, nothing out');
    eq((0, idLookup_1.uniqueIds)([null, undefined, '']).length, 0, 'a list of blanks is an empty list, not a list of one blank');
    /* ── chunkIds: every id in exactly one chunk ────────────────────────────── */
    {
        const chunks = (0, idLookup_1.chunkIds)(ids(350), 150);
    ...
    {
        const chunks = (0, idLookup_1.chunkIds)(ids(300), 150);
        eq(chunks.length, 2, 'an exact multiple does not produce a trailing empty chunk');
    }
    eq((0, idLookup_1.chunkIds)([], 150).length, 0, 'no ids is no chunks — one empty chunk would send an `in.()` that matches nothing');
    {
        ...
        const chunks = (0, idLookup_1.chunkIds)(ids(3), 0);
    ...
    /* ── the chunk size has to fit in a query string ────────────────────────── */
    {
        ...
        const bytes = idLookup_1.ID_CHUNK * 39;
        ok(bytes < 6000, `a full chunk is ${bytes} bytes of query string, which leaves too little room under the 8KB request line`);
        ok(idLookup_1.ID_CHUNK <= rowCap_1.ROW_CAP, 'a chunk larger than the row cap could truncate on a unique key before paging ever helped');
    }
    /* ── readByIds: every id asked about exactly once ───────────────────────── */
    {
        const t = table();
        const { value } = await run((0, idLookup_1.readByIds)(ids(350), t.page, 'names', { chunk: 150 }));
    ...
    {
        const t = table();
        const { value } = await run((0, idLookup_1.readByIds)(['a', 'a', null, 'b'], t.page, 'names'));
    ...
    {
        const t = table();
        const { value } = await run((0, idLookup_1.readByIds)([], t.page, 'names'));
        eq(value?.length, 0, 'no ids is an empty answer');
        eq(t.asked.length, 0, 'and costs no round trip at all');
    }
    {
        const t = table();
        const { value } = await run((0, idLookup_1.readByIds)([null, undefined, ''], t.page, 'names'));
    ...
    /* ── a chunk bigger than its id list is still finished ──────────────────── */
    {
        ...
        const { value } = await run((0, idLookup_1.readByIds)(ids(150), t.page, 'the ledger entries', { chunk: 150 }));
        ...
        eq(t.asked[1].from, rowCap_1.ROW_CAP, 'and the second picks up exactly where it stopped');
    }
    /* ── a failed chunk is never a short answer ─────────────────────────────── */
    {
        ...
        const page = async (chunk, _from, _to) => {
            call += 1;
            if (call === 2)
                return { data: null, error: boom };
            return { data: chunk.map((id) => ({ id })), error: null };
        };
        const { value, threw } = await run((0, idLookup_1.readByIds)(ids(350), page, 'names', { chunk: 150 }));
    ...
    }
    {
        ...
        // browser tab.
        const t = table(() => 3000);
        const { value, threw } = await run((0, idLookup_1.readByIds)(ids(20), t.page, 'the ledger entries', { chunk: 20 }));
        eq(value, null, 'a runaway chunk does not return a prefix');
        ok(threw instanceof rowCap_1.TruncatedRead, 'it refuses, and says which set it refused');
        ok(String(threw?.message ?? '').includes('the ledger entries'), 'in the caller’s own words, because this message can reach a gym owner');
    }
    if (errors.length) {
