...
const keyboardLift_1 = require("./keyboardLift");
let errors = 0;
...
// Nothing lifted yet.
{
    ...
    const got = (0, keyboardLift_1.liftFor)({ barY, barHeight: BAR_H, applied: 0, keyboardScreenY: KEYBOARD_TOP });
...
// bar would climb 253 points per keyboard event until it left the screen.
{
    ...
    for (let i = 0; i < 6; i++) {
        ...
        lift = (0, keyboardLift_1.liftFor)({ barY: measuredY, barHeight: BAR_H, applied: lift, keyboardScreenY: KEYBOARD_TOP }) ?? -1;
        seen.push(lift);
    }
    ...
    ok('and every event after that leaves it exactly where it is: ' + seen.join(','), seen.every((v) => v === seen[0]));
}
...
// a navigator header, so its answer is short by exactly that header.
{
    ...
    const measured = (0, keyboardLift_1.liftFor)({
...
// because there is no second listener to say so.
{
    const TAB_BAR = 83;
    const rest = WINDOW_H - TAB_BAR - BAR_H;
    eq('a keyboard at the bottom of the window means no lift', (0, keyboardLift_1.liftFor)({ barY: rest - 253, barHeight: BAR_H, applied: 253, keyboardScreenY: WINDOW_H }), 0);
}
...
{
    eq('a bar clear of the keyboard is left alone', (0, keyboardLift_1.liftFor)({ barY: 100, barHeight: BAR_H, applied: 0, keyboardScreenY: KEYBOARD_TOP }), 0);
    const got = (0, keyboardLift_1.liftFor)({ barY: 100, barHeight: BAR_H, applied: 0, keyboardScreenY: KEYBOARD_TOP }) ?? -1;
...
{
    eq('an unlaid-out view (zero height) is not a measurement', (0, keyboardLift_1.liftFor)({ barY: 0, barHeight: 0, applied: 0, keyboardScreenY: KEYBOARD_TOP }), null);
    eq('NaN from the platform is not a measurement', (0, keyboardLift_1.liftFor)({ barY: NaN, barHeight: BAR_H, applied: 0, keyboardScreenY: KEYBOARD_TOP }), null);
    eq('an infinite keyboard edge is not a measurement', (0, keyboardLift_1.liftFor)({ barY: 700, barHeight: BAR_H, applied: 0, keyboardScreenY: Infinity }), null);
    eq('a negative height is not a measurement', (0, keyboardLift_1.liftFor)({ barY: 700, barHeight: -1, applied: 0, keyboardScreenY: KEYBOARD_TOP }), null);
}
...
// bar on the keyboard's edge in every one.
{
    ...
    for (const [name, , tabBar] of shapes) {
        ...
        const got = (0, keyboardLift_1.liftFor)({ barY, barHeight: BAR_H, applied: 0, keyboardScreenY: KEYBOARD_TOP }) ?? -1;
        eq(`${name}: the bar ends level with the keyboard`, barY + BAR_H - got, KEYBOARD_TOP);
    }
}
if (errors) {
    console.error(`keyboardLift: ${errors} failure(s)`);
    process.exit(1);
}
