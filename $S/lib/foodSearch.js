...
exports.mergeFoodResults = mergeFoodResults;
...
function matchTier(terms, nameWords, hayWords) {
    if (!terms.length)
        return 1;
    let landed = 0;
    for (const term of terms)
        if (hayWords.some((w) => w.startsWith(term)))
            landed++;
    if (landed === 0)
        return 3;
    if (landed < terms.length)
        return 2;
    return nameWords[0]?.startsWith(terms[0]) ? 0 : 1;
}
// One word is a kind of food; two or more is a product. See the ranking rule.
const KIND_FIRST = ['common', 'restaurant', 'branded'];
const PRODUCT_FIRST = ['branded', 'common', 'restaurant'];
/**
 ...
 */
function mergeFoodResults(query, sources, limit = 20) {
    const terms = words(query || '');
    const order = terms.length > 1 ? PRODUCT_FIRST : KIND_FIRST;
    const candidates = [];
    const consider = (row, hay, idx) => {
    ...
    sources.restaurant.forEach((d, i) => {
        ...
        const e = (0, restaurant_1.estimateDish)(d, 1);
        consider({
            key: `restaurant:${d.id}`,
            name: e.name,
            source: 'restaurant',
