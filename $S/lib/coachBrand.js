...
exports.RESERVED_BRAND_NAMES = exports.MAX_BRAND_NAME = void 0;
exports.expandHex = expandHex;
exports.bestInkRatio = bestInkRatio;
exports.isReadableBrandColor = isReadableBrandColor;
exports.coachBrandColorOf = coachBrandColorOf;
exports.parseCoachBrandColor = parseCoachBrandColor;
exports.parseCoachBrandName = parseCoachBrandName;
exports.coachBrandNameOf = coachBrandNameOf;
exports.resolveClientBrand = resolveClientBrand;
exports.clientBrandNote = clientBrandNote;
 ...
 */
function bestInkRatio(hex) {
    const six = expandHex(hex);
    if (!six)
        return null;
    return (0, a11y_1.contrastRatio)((0, a11y_1.readableInkOn)(six), six);
}
/**
 ...
 */
function isReadableBrandColor(hex) {
    const r = bestInkRatio(hex);
    return r != null && r >= a11y_1.AA_TEXT;
}
/**
 ...
 */
function parseCoachBrandColor(input) {
    ...
    if (!isReadableBrandColor(six)) {
        ...
        const onBlack = (0, a11y_1.contrastRatio)(a11y_1.INK_ON_LIGHT, six);
        const onWhite = (0, a11y_1.contrastRatio)(a11y_1.INK_ON_DARK, six);
        const fmt = (n) => (n == null ? 'nothing measurable' : `${n.toFixed(1)}:1`);
        return {
            kind: 'bad',
            reason: `Your clients could not read a button label on that colour. Dark text on it reaches ${fmt(onBlack)} and light text reaches ${fmt(onWhite)}, and a label needs ${a11y_1.AA_TEXT}:1. Try a darker or a lighter shade of it.`,
        };
    }
 ...
 */
exports.RESERVED_BRAND_NAMES = [
    brands_1.BRAND.label,
    ...Object.values(brands_1.BRAND.apps).map((a) => a.name),
];
 ...
 */
function parseCoachBrandName(input, reserved = exports.RESERVED_BRAND_NAMES) {
    const name = String(input ?? '').trim().replace(/\s+/g, ' ');
    if (!name)
        return { kind: 'clear' };
    if (name.length > exports.MAX_BRAND_NAME) {
        return { kind: 'bad', reason: `That is longer than a name — ${exports.MAX_BRAND_NAME} characters at most.` };
    }
 ...
 */
function resolveClientBrand(input) {
    if (input.inGym) {
        return {
            source: 'gym',
    ...
    if (!brandName && !color)
        return { source: 'app', name: null, color: null };
 ...
 * that from blurring.
 */
function clientBrandNote(input) {
    const coachHasBranding = !!coachBrandNameOf(input.coach?.brandName) || !!coachBrandColorOf(input.coach?.color);
    if (input.inGym) {
