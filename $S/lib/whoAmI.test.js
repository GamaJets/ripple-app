...
const good = (id) => ({ data: { user: { id } }, error: null });
const bad = (message) => ({ data: { user: null }, error: { message } });
const failed = (r) => r.error !== null;
...
async function main() {
    /* ── the point of the whole module: one request, not eleven ───────────── */
    {
        let calls = 0;
        const g = gate();
        const who = (0, whoAmI_1.shareGetUser)(() => { calls++; return g.p; }, failed);
    ...
    /* ── an answer is reused for the window, and re-read after it ─────────── */
    {
        let calls = 0;
        const c = clock();
        const who = (0, whoAmI_1.shareGetUser)(async () => { calls++; return good('u1'); }, failed, { now: c.now });
        await who.getUser();
        await who.getUser();
        eq(calls, 1, 'a second caller inside the window reuses the landed answer');
        c.advance(whoAmI_1.WHO_AM_I_MS - 1);
        await who.getUser();
    ...
    /* ── NEVER hold a failure ─────────────────────────────────────────────── */
    {
        let calls = 0;
        const c = clock();
        const who = (0, whoAmI_1.shareGetUser)(async () => { calls++; return bad('session missing'); }, failed, { now: c.now });
    ...
    /* ── a failure must not poison the answer that follows it ─────────────── */
    {
        let n = 0;
        const c = clock();
        const who = (0, whoAmI_1.shareGetUser)(async () => (++n === 1 ? bad('offline') : good('u1')), failed, { now: c.now });
        await who.getUser();
    ...
    /* ── NEVER share an answer about somebody else ────────────────────────── */
    {
        const seen = [];
        const c = clock();
        const who = (0, whoAmI_1.shareGetUser)(async (jwt) => { seen.push(jwt); return good(jwt ?? 'me'); }, failed, { now: c.now });
    ...
    {
        const seen = [];
        const who = (0, whoAmI_1.shareGetUser)(async (jwt) => { seen.push(jwt); return good('x'); }, failed);
        await who.getUser();
        await who.getUser('');
    ...
    /* ── forget: signing out must not leave the old person held ───────────── */
    {
        let n = 0;
        const c = clock();
        const who = (0, whoAmI_1.shareGetUser)(async () => good(++n === 1 ? 'first' : 'second'), failed, { now: c.now });
        await who.getUser();
        who.forget();
        const after = await who.getUser();
    ...
    /* ── a throw is a throw, not a response ───────────────────────────────── */
    {
        ...
        const who = (0, whoAmI_1.shareGetUser)(async () => { throw boom; }, failed);
        let caught = null;
        try {
            await who.getUser();
        }
