...
const departureSection_1 = require("./departureSection");
const endCoaching_1 = require("./endCoaching");
const errors = [];
const ok = (cond, msg) => { if (!cond)
    errors.push(msg); };
const eq = (a, b, msg) => ok(Object.is(a, b), `${msg} — got ${JSON.stringify(a)}, wanted ${JSON.stringify(b)}`);
const ended = (reason) => ({ reason, endedAt: '2026-07-01T09:00:00Z' });
const WINDOW = 90;
/** The claims that must survive, whichever branch the sentence takes. */
const saysHowMany = (s, n) => s.includes(String(n));
const saysThePeriod = (s) => s.includes(`${WINDOW} days`);
const saysWhatItIsWorth = (s) => /cheapest thing you will ever learn/.test(s);
const saysWhyNow = (s) => /in six months/.test(s);
/* ── nothing to say ───────────────────────────────────────────────────────── */
//
...
eq((0, departureSection_1.departureSectionNote)(null, WINDOW), null, 'an unread book says nothing at all, rather than "nobody has left you"');
eq((0, departureSection_1.departureSectionNote)((0, endCoaching_1.departureTally)([]), WINDOW), null, 'and an empty window says nothing either — "0 people have left" reads as a compliment');
...
{
    const s = (0, departureSection_1.departureSectionNote)((0, endCoaching_1.departureTally)([ended(null)]), WINDOW);
...
{
    const s = (0, departureSection_1.departureSectionNote)((0, endCoaching_1.departureTally)([ended(null), ended(null), ended('nonsense')]), WINDOW) ?? '';
...
// matters.
{
    const s = (0, departureSection_1.departureSectionNote)((0, endCoaching_1.departureTally)([ended('cost'), ended('cost'), ended('moved')]), WINDOW) ?? '';
...
// to carry all of it.
{
    const s = (0, departureSection_1.departureSectionNote)((0, endCoaching_1.departureTally)([ended('cost'), ended('cost'), ended(null), ended(null), ended(null)]), WINDOW) ?? '';
...
{
    const s = (0, departureSection_1.departureSectionNote)((0, endCoaching_1.departureTally)([ended('cost'), ended(null)]), WINDOW) ?? '';
...
// whatever it is given rather than a figure of its own.
{
    ...
    ok(((0, departureSection_1.departureSectionNote)((0, endCoaching_1.departureTally)(rows), 30) ?? '').includes('30 days'), 'a thirty-day window says thirty days');
    ok(((0, departureSection_1.departureSectionNote)((0, endCoaching_1.departureTally)(rows), 365) ?? '').includes('365 days'), 'and a year says a year — nothing here has a window baked into it');
}
/* ── zone independence ────────────────────────────────────────────────────── */
//
...
{
    const a = (0, departureSection_1.departureSectionNote)((0, endCoaching_1.departureTally)([{ reason: null, endedAt: null }]), WINDOW);
    const b = (0, departureSection_1.departureSectionNote)((0, endCoaching_1.departureTally)([{ reason: null, endedAt: '2026-01-01T00:00:00Z' }]), WINDOW);
