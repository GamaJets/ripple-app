...
exports.DEFAULT_NOTIFY_PREFS = exports.CATEGORIES = void 0;
exports.prefsFromStored = prefsFromStored;
exports.categoryDef = categoryDef;
exports.categoryForRoute = categoryForRoute;
exports.allows = allows;
exports.inQuietHours = inQuietHours;
exports.whenToDeliver = whenToDeliver;
exports.hourToDeliver = hourToDeliver;
exports.deliveryMinute = deliveryMinute;
exports.timeToDeliver = timeToDeliver;
exports.movedNote = movedNote;
exports.quietLabel = quietLabel;
 ...
 */
function prefsFromStored(raw) {
    if (raw == null)
        return exports.DEFAULT_NOTIFY_PREFS;
    let parsed;
    try {
    ...
    for (const c of exports.CATEGORIES) {
        ...
        // never touched.
        if (parsed.off && parsed.off[c.key] === true)
            off[c.key] = true;
    }
    ...
    return {
        ...
        quietFromHour: hour(parsed.quietFromHour, exports.DEFAULT_NOTIFY_PREFS.quietFromHour),
        quietToHour: hour(parsed.quietToHour, exports.DEFAULT_NOTIFY_PREFS.quietToHour),
    };
}
/** The definition for a category, or null. */
function categoryDef(key) {
    return exports.CATEGORIES.find((c) => c.key === key) ?? null;
}
...
function categoryForRoute(route) {
    ...
    for (const [screen, key] of CATEGORY_BY_ROUTE) {
        if (r === screen || r.startsWith(screen + '?'))
            return key;
    }
    return null;
}
 ...
 */
function allows(category, prefs) {
 ...
 */
function inQuietHours(hour, prefs) {
    if (!prefs.quiet)
        return false;
    ...
    return (0, quietHours_1.hourInWindow)(hour, prefs.quietFromHour, prefs.quietToHour);
}
/**
 ...
 */
function whenToDeliver(at, category, prefs) {
    const def = categoryDef(category);
    if (!def?.quietable || !prefs.quiet)
        return new Date(at.getTime());
    if (!inQuietHours(at.getHours(), prefs))
        return new Date(at.getTime());
 ...
 */
function hourToDeliver(hour, category, prefs) {
    const def = categoryDef(category);
    if (!def?.quietable || !prefs.quiet)
        return hour;
...
function intoQuiet(hour, minute, prefs) {
 ...
 */
function timeToDeliver(hour, minute, category, prefs) {
    const def = categoryDef(category);
    if (!def?.quietable || !prefs.quiet || !inQuietHours(hour, prefs))
        return { hour, minute };
 ...
 */
function movedNote(hour, minute, category, prefs, label) {
