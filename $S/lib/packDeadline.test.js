...
const packDeadline_1 = require("./packDeadline");
...
eq((0, packDeadline_1.packDeadline)({ left: 4, expiresOn: null, today: TODAY, bookedByThen: 0 }).kind, 'silent', 'a pack with no window has no deadline to miss');
eq((0, packDeadline_1.packDeadline)({ left: 4, expiresOn: inDays(200), today: TODAY, bookedByThen: 0 }).kind, 'silent', 'a window two hundred days out is not news');
eq((0, packDeadline_1.packDeadline)({ left: 0, expiresOn: inDays(3), today: TODAY, bookedByThen: 0 }).kind, 'silent', 'a spent pack running out costs nobody anything');
eq((0, packDeadline_1.packDeadline)({ left: 4, expiresOn: inDays(-1), today: TODAY, bookedByThen: 0 }).kind, 'silent', 'a window that has already closed belongs to expiryLine, which says what was lost');
eq((0, packDeadline_1.packDeadline)({ left: 4, expiresOn: 'not a date', today: TODAY, bookedByThen: 0 }).kind, 'silent', 'a date that will not read expires nothing');
// The boundary is packExpiry's, not a second copy of it.
eq((0, packDeadline_1.packDeadline)({ left: 4, expiresOn: inDays(packExpiry_1.EXPIRING_SOON_DAYS - 1), today: TODAY, bookedByThen: 0 }).kind, 'toBook', 'inside the shared soon-window this speaks');
eq((0, packDeadline_1.packDeadline)({ left: 4, expiresOn: inDays(packExpiry_1.EXPIRING_SOON_DAYS + 1), today: TODAY, bookedByThen: 0 }).kind, 'silent', 'outside it, it does not');
/* ── the answer somebody wanted ────────────────────────────────────────── */
const covered = (0, packDeadline_1.packDeadline)({ left: 3, expiresOn: inDays(9), today: TODAY, bookedByThen: 3 });
...
const over = (0, packDeadline_1.packDeadline)({ left: 2, expiresOn: inDays(9), today: TODAY, bookedByThen: 5 });
eq(over.kind, 'covered', 'more bookings than sessions is still covered');
eq(over.toBook, 0, 'and never a negative number to book');
/* ── the nudge ─────────────────────────────────────────────────────────── */
const nudge = (0, packDeadline_1.packDeadline)({ left: 4, expiresOn: inDays(8), today: TODAY, bookedByThen: 1 });
...
ok(/about one a day/.test((0, packDeadline_1.packDeadline)({ left: 4, expiresOn: inDays(3), today: TODAY, bookedByThen: 0 }).text ?? ''), 'four in four days is about one a day');
// Comfortably spaced: the cadence clause is dropped rather than padded out.
const roomy = (0, packDeadline_1.packDeadline)({ left: 2, expiresOn: inDays(13), today: TODAY, bookedByThen: 0 });
...
const tight = (0, packDeadline_1.packDeadline)({ left: 5, expiresOn: inDays(2), today: TODAY, bookedByThen: 0 });
...
eq((0, packDeadline_1.packDeadline)({ left: 3, expiresOn: inDays(2), today: TODAY, bookedByThen: 0 }).kind, 'toBook', 'three sessions in three days is still bookable');
eq((0, packDeadline_1.packDeadline)({ left: 4, expiresOn: inDays(2), today: TODAY, bookedByThen: 0 }).kind, 'tight', 'four in three days is not');
/* ── the last day ──────────────────────────────────────────────────────── */
const lastDay = (0, packDeadline_1.packDeadline)({ left: 1, expiresOn: TODAY, today: TODAY, bookedByThen: 0 });
...
const noBalance = (0, packDeadline_1.packDeadline)({ left: null, expiresOn: inDays(5), today: TODAY, bookedByThen: 2 });
...
const noDiary = (0, packDeadline_1.packDeadline)({ left: 4, expiresOn: inDays(5), today: TODAY, bookedByThen: null });
...
eq((0, packDeadline_1.packDeadline)({ left: 4, expiresOn: inDays(5), today: TODAY, bookedByThen: Number.NaN }).kind, 'unknown', 'an unreadable count is treated as no count, never as zero');
/* ── singulars, because these are read by people ───────────────────────── */
ok(/1 session on this pack is not booked/.test((0, packDeadline_1.packDeadline)({ left: 1, expiresOn: inDays(5), today: TODAY, bookedByThen: 0 }).text ?? ''), 'one session is singular all the way through the sentence');
...
eq((0, packDeadline_1.bookedBy)([{ startsAt: lastDayEvening }], '2026-09-12'), 1, 'a session on the evening of the last day is inside the window');
const dayAfter = new Date(2026, 8, 13, 9, 0, 0).toISOString();
eq((0, packDeadline_1.bookedBy)([{ startsAt: dayAfter }], '2026-09-12'), 0, 'and one the next morning is not');
eq((0, packDeadline_1.bookedBy)([], '2026-09-12'), 0, 'an empty diary is zero bookings, which is a real answer');
eq((0, packDeadline_1.bookedBy)([{ startsAt: lastDayEvening }], null), null, 'no window means no count — a caller must not read it as "nothing booked"');
eq((0, packDeadline_1.bookedBy)([{ startsAt: lastDayEvening }], 'nonsense'), null, 'nor an unreadable one');
...
eq((0, packDeadline_1.bookedBy)([{ startsAt: 'nonsense' }, { startsAt: lastDayEvening }], '2026-09-12'), 1, 'a booking whose date will not read is not counted as covering anything');
/* ── the two joined up, which is the whole feature ─────────────────────── */
{
    ...
    const n = (0, packDeadline_1.bookedBy)(upcoming, '2026-09-12');
    eq(n, 3, 'only the bookings before the pack ends count towards it');
    const d = (0, packDeadline_1.packDeadline)({ left: 4, expiresOn: '2026-09-12', today: TODAY, bookedByThen: n });
