...
const prog = (title) => ({ title, focus: ['chest'], note: '', days: [] });
const NOW = Date.parse('2026-09-03T10:00:00.000Z');
const stamp = (msAgo) => new Date(NOW - msAgo).toISOString();
/* ── 1 · an empty answer is an answer ─────────────────────────────────── */
{
    const bytes = (0, programCache_1.packPrograms)({}, {}, stamp(0));
    const back = (0, programCache_1.readPrograms)(bytes, NOW);
    ...
    eq((0, programCache_1.readPrograms)(null, NOW).found, false, 'against never having written a copy at all');
}
/* ── 2 · a prefix is never kept ───────────────────────────────────────── */
{
    eq((0, programCache_1.mayCache)(false), true, 'a whole page is written to the device');
    eq((0, programCache_1.mayCache)(true), false, "a page that hit the row cap is not — nothing downstream will remember it was a prefix");
}
/* ── 3 · unreadable is not empty ──────────────────────────────────────── */
{
    for (const bad of ['', 'not json', '{}', '{"rows":"nope","at":"x"}', '[]', 'null']) {
        eq((0, programCache_1.readPrograms)(bad, NOW).found, false, `unreadable bytes (${JSON.stringify(bad)}) are not an empty programme list`);
    }
    ...
    eq((0, programCache_1.readPrograms)(JSON.stringify({ rows: [], at: 'whenever' }), NOW).found, false, 'a copy with no readable stamp cannot be aged, so it is not served');
}
/* ── 4 · never over a live read ───────────────────────────────────────── */
{
    eq((0, programCache_1.mayServeCached)(false, true), true, 'the copy is drawn when no read has landed');
    eq((0, programCache_1.mayServeCached)(true, true), false, 'and never over a read that did land');
    eq((0, programCache_1.mayServeCached)(false, false), false, 'and not at all when there is no copy');
}
/* ── 5 · two people, one phone ────────────────────────────────────────── */
{
    const a = (0, programCache_1.programCacheKey)('11111111-1111-1111-1111-111111111111');
    const b = (0, programCache_1.programCacheKey)('22222222-2222-2222-2222-222222222222');
...
/* ── 6 · the horizon ──────────────────────────────────────────────────── */
{
    const bytes = (0, programCache_1.packPrograms)({ c1: prog('Block A') }, {}, stamp(programCache_1.PROGRAM_HORIZON_MS - 60000));
    eq((0, programCache_1.readPrograms)(bytes, NOW).found, true, 'a copy inside the horizon is served');
    const old = (0, programCache_1.packPrograms)({ c1: prog('Block A') }, {}, stamp(programCache_1.PROGRAM_HORIZON_MS + 60000));
    eq((0, programCache_1.readPrograms)(old, NOW).found, false, 'and one past it is not — after a month the generic programme is the better answer');
    ...
    ok(programCache_1.PROGRAM_HORIZON_MS > 21 * 24 * 60 * 60 * 1000, 'the horizon outlasts a normal training block');
}
/* ── the round trip ───────────────────────────────────────────────────── */
{
    const programs = { c2: prog('Second'), c1: prog('First') };
    const startsOn = { c1: '2026-09-07' };
    const back = (0, programCache_1.readPrograms)((0, programCache_1.packPrograms)(programs, startsOn, stamp(60000)), NOW);
...
/* ── the shape written ────────────────────────────────────────────────── */
{
    const rows = (0, programCache_1.toCachedRows)({ b: prog('B'), a: prog('A') }, { a: '2026-01-01', zz: '2026-02-02' });
...
/* ── a row this build cannot read costs only itself ───────────────────── */
{
    ...
    const back = (0, programCache_1.readPrograms)(raw, NOW);
...
{
    const future = (0, programCache_1.packPrograms)({ c1: prog('Block') }, {}, new Date(NOW + 86400000).toISOString());
    eq((0, programCache_1.readPrograms)(future, NOW).found, true, "a stamp in the future is a phone whose clock moved, and must not cost the member their only copy");
}
if (errors.length) {
