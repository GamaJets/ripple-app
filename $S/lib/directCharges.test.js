...
eq((0, directCharges_1.modelForAccount)({ charge_model: 'direct' }), 'direct', 'the word direct selects direct charges');
eq((0, directCharges_1.modelForAccount)({ charge_model: 'destination' }), 'destination', 'the word destination selects destination charges');
// Everything else is destination, which is what every account did before today.
eq((0, directCharges_1.modelForAccount)({ charge_model: null }), 'destination', 'a null charge_model is the old model, not the new one');
eq((0, directCharges_1.modelForAccount)({}), 'destination', 'a row with no charge_model column is the old model');
eq((0, directCharges_1.modelForAccount)(null), 'destination', 'no account row at all is not a licence to use direct charges');
eq((0, directCharges_1.modelForAccount)(undefined), 'destination', 'an undefined account row is not a licence either');
eq((0, directCharges_1.modelForAccount)({ charge_model: 'Direct' }), 'destination', 'the match is exact — a capitalised word is not the flag');
eq((0, directCharges_1.modelForAccount)({ charge_model: ' direct ' }), 'destination', 'a padded word is not the flag either');
eq((0, directCharges_1.modelForAccount)({ charge_model: 'dir' }), 'destination', 'a truncated word is not the flag');
eq((0, directCharges_1.modelForAccount)({ charge_model: '' }), 'destination', 'an empty string is not the flag');
// ── which account an existing object lives on ───────────────────────────────
eq((0, directCharges_1.accountForObject)({ stripe_account_id: ACCT }), ACCT, 'an object stamped with an account lives on it');
eq((0, directCharges_1.accountForObject)({ stripe_account_id: null }), null, 'a null account means the platform, which is where every old row is');
eq((0, directCharges_1.accountForObject)({}), null, 'an unstamped row means the platform');
eq((0, directCharges_1.accountForObject)(null), null, 'no row is not an account');
eq((0, directCharges_1.accountForObject)({ stripe_account_id: '   ' }), null, 'whitespace is not an account id and must never become a Stripe-Account header');
eq((0, directCharges_1.accountForObject)({ stripe_account_id: ' ' + ACCT + ' ' }), ACCT, 'a padded id is trimmed rather than refused');
// The request options handed to the Stripe SDK.
eq(JSON.stringify((0, directCharges_1.optionsForObject)({ stripe_account_id: ACCT })), JSON.stringify({ stripeAccount: ACCT }), 'a connected object is called with its account');
eq((0, directCharges_1.optionsForObject)({ stripe_account_id: null }), undefined, 'a platform object is called with no account context');
eq((0, directCharges_1.optionsForObject)({}), undefined, 'an unstamped object is called on the platform');
// ── the platform fee percent ────────────────────────────────────────────────
const pctOf = (raw) => { const r = (0, directCharges_1.platformFeePct)(raw); return r.ok ? r.pct : null; };
eq(pctOf(undefined), directCharges_1.DEFAULT_FEE_PCT, 'an unset secret keeps the default the function has always used');
eq(pctOf(null), directCharges_1.DEFAULT_FEE_PCT, 'a null secret keeps the default');
eq(pctOf(''), directCharges_1.DEFAULT_FEE_PCT, 'an empty secret keeps the default');
eq(pctOf('   '), directCharges_1.DEFAULT_FEE_PCT, 'a whitespace-only secret keeps the default');
...
ok(!(0, directCharges_1.platformFeePct)('ten').ok && (0, directCharges_1.platformFeePct)('ten').ok === false, 'a refusal is a refusal, not a silent default');
// ── the fee amount on a one-off ─────────────────────────────────────────────
const feeOf = (price, pct) => { const r = (0, directCharges_1.applicationFeeCents)(price, pct); return r.ok ? r.fee : 'refused'; };
...
eq((0, directCharges_1.liabilityFrom)(undefined), 'coach', 'an unset secret onboards the coach under the arrangement the owner chose');
eq((0, directCharges_1.liabilityFrom)(''), 'coach', 'an empty secret does not fall back to the platform carrying losses');
eq((0, directCharges_1.liabilityFrom)('   '), 'coach', 'a whitespace-only secret does not either');
eq((0, directCharges_1.liabilityFrom)(null), 'coach', 'a null secret does not either');
eq((0, directCharges_1.liabilityFrom)('coach'), 'coach', 'the coach carries it when the secret says so');
eq((0, directCharges_1.liabilityFrom)(' COACH '), 'coach', 'the secret is read case- and space-insensitively');
eq((0, directCharges_1.liabilityFrom)('platform'), 'platform', 'the legacy arrangement is still reachable by name');
eq((0, directCharges_1.liabilityFrom)(' Platform '), 'platform', 'and it is read case- and space-insensitively too');
eq((0, directCharges_1.liabilityFrom)('platfrom'), 'coach', 'a typo does not silently put Repple back on the hook for a new coach');
eq((0, directCharges_1.liabilityFrom)('express'), 'coach', 'the account type is not the name of the setting');
eq((0, directCharges_1.liabilityFrom)('true'), 'coach', 'only the word platform means the platform');
...
eq((0, directCharges_1.accountTypeFor)('coach'), 'standard', 'a new coach is onboarded onto a Standard account');
eq((0, directCharges_1.accountTypeFor)('platform'), 'express', 'the legacy arrangement is still an Express account');
eq((0, directCharges_1.chargeModelFor)('coach'), 'direct', 'a Standard account sells under direct charges');
eq((0, directCharges_1.chargeModelFor)('platform'), 'destination', 'an Express account sells under destination charges');
...
for (const l of ['coach', 'platform']) {
    const pairedRight = ((0, directCharges_1.accountTypeFor)(l) === 'standard') === ((0, directCharges_1.chargeModelFor)(l) === 'direct');
    ok(pairedRight, `the account type and the charge model must agree for ${l}`);
}
// ── what Stripe says the account is ─────────────────────────────────────────
eq((0, directCharges_1.accountTypeOf)({ account_type: 'standard' }), 'standard', 'Stripe’s word is read back');
eq((0, directCharges_1.accountTypeOf)({ account_type: 'express' }), 'express', 'an Express account says so');
eq((0, directCharges_1.accountTypeOf)({ account_type: 'custom' }), 'custom', 'a Custom account is recorded as itself rather than rounded');
eq((0, directCharges_1.accountTypeOf)({ account_type: 'none' }), 'none', 'an account created from controller properties is recorded as itself');
eq((0, directCharges_1.accountTypeOf)({ account_type: ' Standard ' }), 'standard', 'padding and case do not change what the account is');
eq((0, directCharges_1.accountTypeOf)({ account_type: null }), null, 'a null type means nobody has asked Stripe yet');
eq((0, directCharges_1.accountTypeOf)({}), null, 'a row without the column means nobody has asked Stripe yet');
eq((0, directCharges_1.accountTypeOf)(null), null, 'no row at all is not an account type');
eq((0, directCharges_1.accountTypeOf)({ account_type: 'STANDARD_ACCOUNT' }), null, 'a word Stripe does not use is not silently accepted');
// ── can this account take a direct charge ───────────────────────────────────
const why = (a) => { const r = (0, directCharges_1.canTakeDirectCharges)(a); return r.ok ? 'ok' : 'refused'; };
...
eq(why({ stripe_account_id: ACCT, account_type: 'express', charges_enabled: true, card_payments_status: 'active' }), 'refused', 'a legacy Express account may never take a direct charge, however its charge_model was written');
...
eq(why({ stripe_account_id: ACCT, account_type: null, charges_enabled: true, card_payments_status: 'active' }), 'refused', 'nor may an account nobody has asked Stripe about — a row that is both direct and untyped was edited by hand');
...
const pending = (0, directCharges_1.canTakeDirectCharges)({ ...STD, charges_enabled: true, card_payments_status: 'pending' });
ok(!pending.ok && /verif/i.test(pending.reason), 'a capability refusal tells the reader what is missing');
const wrongKind = (0, directCharges_1.canTakeDirectCharges)({ stripe_account_id: ACCT, account_type: 'express', charges_enabled: true, card_payments_status: 'active' });
...
// satisfy the assertions in this file, the assertions are not doing their job.
{
    ...
    const wrong = (a) => (a?.charge_model === 'destination' ? 'destination' : 'direct');
    ok(wrong(null) !== (0, directCharges_1.modelForAccount)(null), 'defaulting an unknown account to direct charges must fail this file');
    ok(wrong({}) !== (0, directCharges_1.modelForAccount)({}), 'defaulting a column-less row to direct charges must fail this file');
}
{
    // Wrong: send a zero fee instead of omitting the field.
    const wrong = (price, pct) => Math.round((price * pct) / 100);
    ok(wrong(4, 10) !== feeOf(4, 10), 'sending a zero application fee must fail this file');
    ok(wrong(10000, 0) !== feeOf(10000, 0), 'sending a zero fee for a zero percent must fail this file');
}
{
    // Wrong: clamp an over-large fee to fit instead of refusing it.
    const wrong = (price, pct) => Math.min(Math.round((price * pct) / 100), price - 1);
...
}
{
    ...
    const wrong = (_row, coach) => coach.charge_model === 'direct' ? ACCT : null;
    ok(wrong({ stripe_account_id: null }, { charge_model: 'direct' }) !== (0, directCharges_1.accountForObject)({ stripe_account_id: null }), 'resolving an old subscription through the coach’s new model must fail this file');
}
{
    // Wrong: refuse an account whose capability status has not been recorded yet.
    const wrong = (a) => (a.card_payments_status === 'active' ? 'ok' : 'refused');
    ok(wrong({ card_payments_status: null }) !== why({ ...STD, charges_enabled: true, card_payments_status: null }), 'refusing every account that predates the capability column must fail this file');
}
{
    ...
    const wrong = (a) => (a.charges_enabled ? 'ok' : 'refused');
    ok(wrong({ charges_enabled: true }) !== why({ stripe_account_id: ACCT, account_type: 'express', charges_enabled: true }), 'letting a legacy Express account take a direct charge must fail this file');
}
{
    ...
    const wrong = (raw) => ((typeof raw === 'string' ? raw.trim().toLowerCase() : '') === 'coach' ? 'coach' : 'platform');
    ok(wrong(undefined) !== (0, directCharges_1.liabilityFrom)(undefined), 'defaulting an unset secret to the platform must fail this file');
    ok(wrong('platfrom') !== (0, directCharges_1.liabilityFrom)('platfrom'), 'defaulting a typo to the platform must fail this file');
}
{
    ...
    const wrongModel = (l) => (l === 'coach' ? 'destination' : 'direct');
    ok(wrongModel('coach') !== (0, directCharges_1.chargeModelFor)('coach'), 'selling a Standard account under destination charges must fail this file');
    ok(wrongModel('platform') !== (0, directCharges_1.chargeModelFor)('platform'), 'selling an Express account under direct charges must fail this file');
}
if (errors.length) {
