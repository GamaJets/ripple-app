...
async function run(p) {
    try {
        return { rows: await p, threw: null };
    }
    catch (e) {
        return { rows: null, threw: e };
    }
}
async function main() {
    ...
    {
        const t = table(0);
        const { rows } = await run((0, rowCap_1.readAll)(t.page, 'classes', { pageSize: 10 }));
    ...
    }
    {
        ...
        eq(JSON.stringify(t.asked[0]), '[0,9]', 'and the range is INCLUSIVE at both ends, which is what PostgREST .range() means — asking [0,10] would read 11 rows a page and shift every page after it');
    }
    {
        const t = table(25);
        const { rows } = await run((0, rowCap_1.readAll)(t.page, 'classes', { pageSize: 10 }));
        eq(rows?.length, 25, 'a set spanning three pages comes back whole');
        eq(JSON.stringify(rows), JSON.stringify([...Array(25).keys()]), 'IN ORDER AND WITHOUT DUPLICATES — a page that overlapped or skipped would still have the right length');
        eq(JSON.stringify(t.asked), '[[0,9],[10,19],[20,29]]', 'and each page starts exactly where the last one ended');
    }
    ...
    // they were the whole set. Silent truncation, with pagination on top.
    {
        ...
        const { rows, threw } = await run((0, rowCap_1.readAll)(page, 'classes', { pageSize: 10 }));
    ...
    {
        const t = table(1000000);
        const { rows, threw } = await run((0, rowCap_1.readAll)(t.page, 'the door log', { pageSize: 10, ceiling: 35 }));
        eq(rows, null, 'a read past the ceiling refuses rather than walking the table');
        ok(threw instanceof rowCap_1.TruncatedRead, 'and it refuses with the vocabulary the rest of the codebase already reads');
        eq(threw.what, 'the door log', 'named in the owner’s words, because this message reaches a screen');
        eq(threw.cap, 35, 'and it says which ceiling was hit, not the row cap it is not');
    ...
    }
    {
        ...
        // runaway.
        const t = table(30);
        const { rows, threw } = await run((0, rowCap_1.readAll)(t.page, 'classes', { pageSize: 10, ceiling: 30 }));
    ...
    }
    {
        ...
        ok(threw instanceof rowCap_1.TruncatedRead, 'and refuses in the same vocabulary as a runaway');
    }
    /* ── the defaults are the ones the callers rely on ─────────────────────── */
    {
        const t = table(3);
        await run((0, rowCap_1.readAll)(t.page, 'classes'));
        eq(JSON.stringify(t.asked[0]), `[0,${rowCap_1.ROW_CAP - 1}]`, 'the default page is the row cap itself — asking for more than PostgREST will return would make every page short, and every read one page long');
    }
    ok(rowCap_1.PAGE_CEILING > rowCap_1.ROW_CAP, 'and the default ceiling is past the cap, or readAll would refuse the first set that needed it');
    {
        ...
        const t = table(3);
        const { rows } = await run((0, rowCap_1.readAll)(t.page, 'classes', { pageSize: 0 }));
