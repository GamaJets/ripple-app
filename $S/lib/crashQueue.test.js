...
{
    eq((0, crashQueue_1.readCrashQueue)(null), { items: [], read: true }, 'nothing stored is genuinely nothing waiting');
    eq((0, crashQueue_1.readCrashQueue)(''), { items: [], read: true }, 'and so is an empty string');
    eq((0, crashQueue_1.readCrashQueue)('{not json').read, false, 'BYTES NOBODY COULD PARSE ARE NOT "NO CRASHES"');
    eq((0, crashQueue_1.readCrashQueue)('{"a":1}').read, false, 'nor is an object where a list was written');
    eq((0, crashQueue_1.readCrashQueue)('[]'), { items: [], read: true }, 'an empty list is readable and empty');
    ...
    const r = (0, crashQueue_1.readCrashQueue)(mixed);
...
/* ── 2. the crash loop ───────────────────────────────────────────────────── */
{
    ...
    let dupes = 0;
    for (let i = 0; i < 50; i++) {
        const out = (0, crashQueue_1.addCrash)(list, (0, crashQueue_1.newCrash)({ id: `loop${i}`, at: '2026-09-01T10:00:00.000Z', message: 'render exploded' }));
    ...
    let full = [];
    for (let i = 0; i < crashQueue_1.CRASH_CAP; i++)
        full = (0, crashQueue_1.addCrash)(full, crash(i, { id: `f${i}`, message: `distinct ${i}` })).list;
    eq(full.length, crashQueue_1.CRASH_CAP, 'the cap is the cap');
    const after = (0, crashQueue_1.addCrash)(full, crash(999, { id: 'late', message: 'a consequence' }));
    eq(after.result, 'full', 'past the cap it refuses');
    eq(after.list.length, crashQueue_1.CRASH_CAP, 'and the list is unchanged');
    eq(after.list[0].message, 'distinct 0', 'THE FIRST FAILURE IS STILL THERE — evicting oldest-first would keep twenty symptoms and drop the cause');
    ok(!after.list.some((c) => c.id === 'late'), 'and the newest was the one refused');
}
/* ── ordering, and removing what was sent ────────────────────────────────── */
{
    const a = (0, crashQueue_1.newCrash)({ id: 'a', at: '2026-09-03T10:00:00.000Z', message: 'third' });
    const b = (0, crashQueue_1.newCrash)({ id: 'b', at: '2026-09-01T10:00:00.000Z', message: 'first' });
    const c = (0, crashQueue_1.newCrash)({ id: 'c', at: '2026-09-02T10:00:00.000Z', message: 'second' });
    eq((0, crashQueue_1.inCrashOrder)([a, b, c]).map((x) => x.message), ['first', 'second', 'third'], 'oldest first, so the reader meets the cause before the consequence');
    eq((0, crashQueue_1.dropCrash)([a, b, c], 'b').map((x) => x.id), ['a', 'c'], 'a sent report comes out');
    eq((0, crashQueue_1.dropCrash)([a, b, c], 'nope').length, 3, 'and an id that is not there removes nothing');
}
/* ── 3. the moment travels with the report ───────────────────────────────── */
{
    const row = (0, crashQueue_1.crashRow)((0, crashQueue_1.newCrash)({
    ...
    ok(row.message.includes('2026-09-01T22:14:00.000Z'), `A QUEUED CRASH MUST NOT ARRIVE AS TODAY'S CRASH — got "${row.message}"`);
    ...
    ok(row.message.length <= crashQueue_1.MAX_MESSAGE, 'the message fits the column even with the moment prepended');
}
/* ── 4. a crash with nobody signed in is still a crash ───────────────────── */
{
    const anon = (0, crashQueue_1.newCrash)({ id: 'n', at: '2026-09-01T00:00:00.000Z', message: 'crashed on launch' });
    eq(anon.userId, null, 'no uid is null rather than a refusal');
    eq((0, crashQueue_1.crashRow)(anon, null).user_id, null, 'and the row carries the null through');
    eq((0, crashQueue_1.crashRow)(anon, 'u1').user_id, null, 'a crash nobody was signed in for is not attributed to whoever signs in later');
    const added = (0, crashQueue_1.addCrash)([], anon);
...
{
    const big = (0, crashQueue_1.newCrash)({
        id: 'big', at: '2026-09-01T00:00:00.000Z',
        message: 'm'.repeat(crashQueue_1.MAX_MESSAGE + 500), stack: 's'.repeat(crashQueue_1.MAX_STACK + 5000),
    });
    eq(big.message.length, crashQueue_1.MAX_MESSAGE, 'the message is trimmed on the way in');
    eq(big.stack.length, crashQueue_1.MAX_STACK, 'and so is the stack');
    eq((0, crashQueue_1.newCrash)({ id: 'q', at: 'x', message: 'm', stack: '' }).stack, null, 'an empty stack is null, not an empty string');
}
...
{
    eq((0, crashQueue_1.attributableTo)('u1', 'u1'), 'u1', 'the ordinary phone keeps its attribution');
    eq((0, crashQueue_1.attributableTo)('u1', 'u2'), null, 'A CRASH FLUSHED UNDER ANOTHER ACCOUNT IS UNATTRIBUTED, NOT REFUSED — the policy takes null');
    eq((0, crashQueue_1.attributableTo)('u1', null), null, 'and so is one flushed with nobody signed in');
    eq((0, crashQueue_1.attributableTo)(null, 'u2'), null, 'a launch crash is never attributed to whoever signed in afterwards');
    eq((0, crashQueue_1.attributableTo)(null, null), null, 'and stays the null the table was designed to hold');
    // Everything a person debugs from survives the lost attribution.
    const c = (0, crashQueue_1.newCrash)({
        id: 'z', at: '2026-09-02T06:30:00.000Z', message: 'boom', stack: 'at Basement',
        platform: 'ios', appVersion: '3.1.0', userId: 'u1',
    });
    const row = (0, crashQueue_1.crashRow)(c, 'u2');
...
{
    ok(!crashQueue_1.CRASH_KEY.includes('${'), 'the key is a constant, not a template waiting for a uid');
    ok(crashQueue_1.CRASH_KEY.includes('v1'), 'and it is versioned, so a shape change does not read old bytes as new ones');
}
if (errors.length) {
