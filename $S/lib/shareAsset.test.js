...
const unread = (0, shareAsset_1.weekCard)({ brand: 'Warehouse', spanLabel: 'Last 7 days', sessions: null, minutes: null, clients: null });
...
const empty = (0, shareAsset_1.weekCard)({ brand: 'Warehouse', spanLabel: 'Last 7 days', sessions: 0, minutes: 0, clients: 0 });
...
ok(!empty.ok && unread.ok === false && empty.why !== unread.why, 'the two refusals do not share a sentence — one is "try again", the other is "go and coach somebody"');
// A partial read is a real card with fewer lines on it, not a card with a zero.
const partial = (0, shareAsset_1.weekCard)({ brand: 'Warehouse', spanLabel: 'Last 7 days', sessions: 12, minutes: null, clients: null });
...
const mixed = (0, shareAsset_1.weekCard)({ brand: 'Warehouse', spanLabel: 'Last 7 days', sessions: 9, minutes: 540, clients: 0 });
...
const one = (0, shareAsset_1.weekCard)({ brand: 'W', spanLabel: 'Last 7 days', sessions: 1, minutes: 60, clients: 1 });
ok(one.ok && one.card.headline === '1 session coached', 'one session is singular');
const many = (0, shareAsset_1.weekCard)({ brand: 'Warehouse Gym', spanLabel: 'Last 7 days', sessions: 1240, minutes: 74400, clients: 86 });
...
ok(many.ok && many.card.stats[0].value !== '1240' && /\d/.test(many.card.stats[0].value), `a four-figure session count is GROUPED, not printed as bare digits — got "${many.ok ? many.card.stats[0].value : 'not ok'}"`);
...
const week = (0, shareAsset_1.weekCard)({ brand: 'Warehouse Gym', spanLabel: 'Last 7 days', sessions: 18, minutes: 1080, clients: 11 });
if (week.ok) {
    eq(week.card.caption, '18 sessions coached — last 7 days.\n18 hrs coached · 11 clients', 'the caption reads as prose, with the headline figure not restated');
}
else {
    errors.push('a full week should build a card');
}
...
const noBrand = (0, shareAsset_1.weekCard)({ brand: '   ', spanLabel: 'Last 7 days', sessions: 3, minutes: null, clients: null });
...
const noConsent = (0, shareAsset_1.resultCard)({ brand: 'Warehouse', clientName: 'Sarah Jones', spanLabel: '12 weeks in', figures, note: 'Sarah smashed it.' }, { figures: false, name: false });
...
const anon = (0, shareAsset_1.resultCard)({ brand: 'Warehouse', clientName: 'Sarah Jones', spanLabel: '12 weeks in', figures, note: "Sarah's consistency did this. Sarah showed up every week." }, { figures: true, name: false });
...
const named = (0, shareAsset_1.resultCard)({ brand: 'Warehouse', clientName: 'Sarah Jones', spanLabel: '12 weeks in', figures, note: 'Sarah smashed it.' }, { figures: true, name: true });
...
const nameOnly = (0, shareAsset_1.resultCard)({ brand: 'W', clientName: 'Sarah', spanLabel: '12 weeks', figures, note: '' }, { figures: false, name: true });
...
const nothing = (0, shareAsset_1.resultCard)({ brand: 'W', clientName: 'Sarah', spanLabel: '12 weeks', figures: [], note: '' }, { figures: true, name: true });
...
const blankFigure = (0, shareAsset_1.resultCard)({ brand: 'W', clientName: 'Sarah', spanLabel: '12 weeks', figures: [{ label: 'Weight', value: '  ' }], note: '' }, { figures: true, name: true });
...
eq((0, shareAsset_1.scrubName)('Sarah was brilliant', 'Sarah Jones'), 'My client was brilliant', 'a name at the start of a sentence is replaced and the sentence still starts with a capital');
eq((0, shareAsset_1.scrubName)('Huge week from Sarah', 'Sarah'), 'Huge week from my client', 'a name mid-sentence stays lower case');
eq((0, shareAsset_1.scrubName)("Sarah's consistency", 'Sarah'), "My client's consistency", 'the possessive survives as a possessive');
eq((0, shareAsset_1.scrubName)('Sarah Jones did it', 'Sarah Jones'), 'My client did it', 'the full name goes as one replacement, not "my client Jones"');
eq((0, shareAsset_1.scrubName)('Well done SARAH', 'Sarah'), 'Well done my client', 'shouting the name does not evade it');
...
eq((0, shareAsset_1.scrubName)('Same session, same result', 'Sam'), 'Same session, same result', '"Sam" does not match inside "Same"');
eq((0, shareAsset_1.scrubName)('Ali trains on Wednesdays', 'Al'), 'Ali trains on Wednesdays', 'nor "Al" inside "Ali"');
eq((0, shareAsset_1.scrubName)('Read the alignment notes', 'Ali'), 'Read the alignment notes', 'nor "Ali" inside "alignment"');
...
eq((0, shareAsset_1.scrubName)('J. was great, and so was Kim', 'J.'), 'My client was great, and so was Kim', 'a full stop in a name is a full stop, not a wildcard');
// Nothing to do is nothing done — no name, no client, no crash.
eq((0, shareAsset_1.scrubName)('A great twelve weeks', null), 'A great twelve weeks', 'a null name leaves the text alone');
eq((0, shareAsset_1.scrubName)('A great twelve weeks', '  '), 'A great twelve weeks', 'so does a blank one');
eq((0, shareAsset_1.scrubName)('', 'Sarah'), '', 'and an empty caption stays empty');
// Two mentions in a row collapse rather than stuttering.
eq((0, shareAsset_1.scrubName)('Sarah Sarah!', 'Sarah'), 'My client!', 'a doubled name does not become "my client my client"');
eq((0, shareAsset_1.firstName)('Sarah Jones'), 'Sarah', 'the first name is the first word');
eq((0, shareAsset_1.firstName)('  '), null, 'a blank name has no first name');
eq((0, shareAsset_1.firstName)(null), null, 'nor does a missing one');
/* ── 3. text that will not fit is wrapped here, not on the card ───────────── */
eq((0, shareAsset_1.wrapLines)('', 20, 3).length, 0, 'empty text is no lines at all, not one empty line');
eq((0, shareAsset_1.wrapLines)('   ', 20, 3).length, 0, 'whitespace-only text likewise');
eq(JSON.stringify((0, shareAsset_1.wrapLines)('Twelve weeks in', 20, 3)), JSON.stringify(['Twelve weeks in']), 'text that fits is one line');
eq(JSON.stringify((0, shareAsset_1.wrapLines)('Twelve weeks of very hard work', 12, 3)), JSON.stringify(['Twelve weeks', 'of very hard', 'work']), 'a greedy wrap fills each line up to the limit');
const wrapped = (0, shareAsset_1.wrapLines)('one two three four five six seven eight nine ten eleven twelve', 10, 3);
...
const long = (0, shareAsset_1.wrapLines)('#TransformationTuesdayAtTheWarehouse', 12, 3);
...
const mix = (0, shareAsset_1.wrapLines)('Come to #TransformationTuesdayAtTheWarehouse today', 12, 4);
ok(mix.every((l) => l.length <= 12), 'a long word mixed with short ones does not overhang either');
eq((0, shareAsset_1.wrapLines)('anything', 0, 3).length, 0, 'a zero-width line yields nothing rather than looping');
eq((0, shareAsset_1.wrapLines)('anything', 10, 0).length, 0, 'and a zero-line budget likewise');
...
ok((0, shareAsset_1.charsPerLine)(920, 64) > 0, 'a real width and size give a real line length');
ok((0, shareAsset_1.charsPerLine)(920, 64) > (0, shareAsset_1.charsPerLine)(920, 96), 'bigger type fits fewer characters');
ok((0, shareAsset_1.charsPerLine)(1840, 64) > (0, shareAsset_1.charsPerLine)(920, 64), 'a wider card fits more');
eq((0, shareAsset_1.charsPerLine)(0, 64), 0, 'a zero width fits nothing');
eq((0, shareAsset_1.charsPerLine)(920, 0), 0, 'and a zero font size is not a division by zero');
/* ── figures and labels ───────────────────────────────────────────────────── */
eq((0, shareAsset_1.hoursLabel)(60), '1 hrs', 'an hour is an hour');
eq((0, shareAsset_1.hoursLabel)(1440), '24 hrs', 'a whole number of hours carries no decimal point');
eq((0, shareAsset_1.hoursLabel)(90), '1.5 hrs', 'a half hour does');
eq((0, shareAsset_1.hoursLabel)(45), '45 min', 'under an hour stays in minutes rather than rounding to "0 hrs"');
eq((0, shareAsset_1.hoursLabel)(0), '0 min', 'zero is zero minutes — the caller decides whether to print it');
eq((0, shareAsset_1.hoursLabel)(-5), '', 'a negative duration is not a duration');
eq((0, shareAsset_1.hoursLabel)(Number.NaN), '', 'and NaN never reaches a graphic');
eq((0, shareAsset_1.lower)('Last 7 days'), 'last 7 days', 'a capitalised span reads as a fragment mid-sentence');
eq((0, shareAsset_1.lower)('August'), 'August', 'a month keeps its capital');
eq((0, shareAsset_1.lower)('This month'), 'this month', 'so does an ordinary determiner get lowered');
eq((0, shareAsset_1.lower)(''), '', 'and an empty span stays empty');
/* ── the exported file ────────────────────────────────────────────────────── */
const fn = (0, shareAsset_1.assetFilename)('week', new Date(2026, 7, 31, 9, 5));
...
ok((0, shareAsset_1.assetFilename)('result').endsWith('.png'), 'a result card is a PNG too');
eq(shareAsset_1.CARD_SIZES.length, 2, 'two shapes, because a square is never the right answer where 4:5 is accepted');
eq((0, shareAsset_1.cardSize)('story').h, 1920, 'the story canvas is a full-height 9:16');
eq((0, shareAsset_1.cardSize)('post').w, 1080, 'and the post is 1080 wide');
...
ok((0, shareAsset_1.cardSize)('nonsense').w > 0, 'an unknown shape falls back to a real canvas, never a 0×0 one');
...
// to read it.
const withLogo = (0, shareAsset_1.weekCard)({ brand: 'Warehouse', spanLabel: 'Last 7 days', sessions: 9, minutes: 540, clients: 6, logo: 'data:image/png;base64,AAAA' });
...
const noLogo = (0, shareAsset_1.weekCard)({ brand: 'Warehouse', spanLabel: 'Last 7 days', sessions: 9, minutes: 540, clients: 6 });
eq(noLogo.ok ? noLogo.card.logo : 'built nothing', null, 'a coach who has set none gets the card they got before, not a gap');
const blankLogo = (0, shareAsset_1.weekCard)({ brand: 'Warehouse', spanLabel: 'Last 7 days', sessions: 9, minutes: 540, clients: 6, logo: '   ' });
...
const consented = (0, shareAsset_1.resultCard)({ brand: 'W', clientName: 'Sarah Jones', spanLabel: '12 weeks in', figures: [{ label: 'Weight', value: '\u22128.4 kg' }], note: '',
    photo: { uri: 'file:///p.jpg', photoId: 'p1', consent: 'granted' } }, { figures: true, name: false });
ok(consented.ok && consented.card.photo?.uri === 'file:///p.jpg', 'the client\u2019s own permission is what puts their photo on a card');
const refused = (0, shareAsset_1.resultCard)({ brand: 'W', clientName: 'Sarah Jones', spanLabel: '12 weeks in', figures: [{ label: 'Weight', value: '\u22128.4 kg' }], note: '',
    photo: { uri: 'file:///p.jpg', photoId: 'p1', consent: 'absent' } }, { figures: true, name: true });
eq(refused.ok ? refused.card.photo : 'built nothing', null, 'and both of the coach\u2019s own ticks together do not substitute for it');
...
const typedIntoHeadline = (0, shareAsset_1.resultCard)({ brand: 'W', clientName: 'Sarah Jones', spanLabel: "Sarah's 12 weeks", figures: [{ label: 'Weight', value: '\u22128.4 kg' }], note: '' }, { figures: true, name: false });
ok(typedIntoHeadline.ok && !/Sarah/i.test(JSON.stringify(typedIntoHeadline.card)), 'a name typed into the period field does not reach the card either');
ok(typedIntoHeadline.ok && typedIntoHeadline.card.headline === "My client's 12 weeks", 'the headline keeps its shape with the name replaced rather than being emptied');
/* ── report ───────────────────────────────────────────────────────────────── */
...
ok(/could not be read/i.test(shareAsset_1.BRAND_UNREAD_NOTE), 'the brand refusal says the gym could not be read');
ok(/not made rather than made with the wrong one/i.test(shareAsset_1.BRAND_UNREAD_NOTE), 'and says the card is withheld rather than built from a guess');
ok(/cannot be taken back/i.test(shareAsset_1.BRAND_UNREAD_NOTE), 'and why that matters here and not elsewhere');
ok(!/no gym|independent/i.test(shareAsset_1.BRAND_UNREAD_NOTE), 'and never states that the coach has no gym — that is the OTHER answer, and it is a correct one');
ok(/logo is set/i.test(shareAsset_1.LOGO_SET_NOT_FETCHED), 'the logo sentence says the record is fine');
ok(/could not be fetched/i.test(shareAsset_1.LOGO_SET_NOT_FETCHED), 'and that it is the picture that did not arrive');
ok(!/have not added|no logo/i.test(shareAsset_1.LOGO_SET_NOT_FETCHED), 'and never reads as "you have not added one", which is what the null used to say');
for (const line of [shareAsset_1.BRAND_UNREAD_NOTE, shareAsset_1.LOGO_SET_NOT_FETCHED]) {
