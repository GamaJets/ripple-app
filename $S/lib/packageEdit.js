...
exports.PACKAGE_NOT_SAVED = exports.MAX_PRICE_CENTS = void 0;
exports.packageEditBlocker = packageEditBlocker;
exports.packageUpdateRow = packageUpdateRow;
exports.isReprice = isReprice;
exports.repriceNote = repriceNote;
...
exports.MAX_PRICE_CENTS = 9999999;
/**
...
function packageEditBlocker(p) {
    ...
    if (p.name !== undefined) {
        const n = p.name.trim();
        if (!n)
            return 'A package needs a name — it is what your client sees on the payment page.';
        if (n.length > 120)
            return 'That name is too long for a payment page. Keep it under 120 characters.';
    }
    if (p.price_cents !== undefined) {
        ...
        if (c <= 0)
            return 'A price has to be more than nothing. To stop selling this, withdraw it instead.';
        if (c > exports.MAX_PRICE_CENTS)
            return 'That price looks like a typing slip. Check the number of zeros.';
    }
    return null;
}
...
function packageUpdateRow(p) {
    if (packageEditBlocker(p))
        return null;
    const row = {};
    if (p.name !== undefined)
...
function isReprice(p, currentCents) {
    if (p.price_cents === undefined)
        return false;
 ...
 */
function repriceNote(activeSubscribers) {
...
exports.PACKAGE_NOT_SAVED = 'That package was not changed, so it is still on sale at the price and name it had. Nothing has changed — try again.';
