...
//     against it.
const finReview_1 = require("./finReview");
const errors = [];
const ok = (cond, msg) => { if (!cond)
    errors.push(msg); };
const eq = (a, b, msg) => ok(Object.is(a, b), `${msg} — got ${JSON.stringify(a)}, wanted ${JSON.stringify(b)}`);
const fin = (patch) => ({ ...(0, finReview_1.emptyFinances)(), ...patch });
/* ── 1. the gate ───────────────────────────────────────────────────────── */
eq((0, finReview_1.hasFigures)((0, finReview_1.emptyFinances)()), false, 'nothing entered is not enough for a review');
eq((0, finReview_1.hasFigures)(fin({ revenue: 40000 })), true, 'revenue alone is enough — it is what every ratio divides by');
...
eq((0, finReview_1.hasFigures)(fin({ members: 300 })), false, 'a member count is not a financial figure');
eq((0, finReview_1.hasFigures)(fin({ expenses: 22000 })), false, 'costs with no revenue give no margin to review');
eq((0, finReview_1.hasFigures)(fin({ mrr: 18000 })), false, 'recurring revenue is a share of total revenue, not a substitute for it');
/* ── 2. anyEntered: telling the two empty states apart ─────────────────── */
eq((0, finReview_1.anyEntered)((0, finReview_1.emptyFinances)()), false, 'a fresh install has entered nothing');
eq((0, finReview_1.anyEntered)(fin({ members: 300 })), true, 'member counts are something the owner typed');
eq((0, finReview_1.anyEntered)(fin({ churnedMembers: 4 })), true, 'any single field counts — the screen must not say their figures were lost');
eq((0, finReview_1.anyEntered)(fin({ classRevenue: 900 })), true, 'ancillary revenue alone still counts as entered');
...
ok((0, finReview_1.anyEntered)(fin({ members: 300 })) && !(0, finReview_1.hasFigures)(fin({ members: 300 })), 'entered-but-not-reviewable is a state the screen has to be able to see');
/* ── 3. no sentence may lose a word ────────────────────────────────────── */
/** Every prose string a review puts on screen. */
const prose = (f, cur = 'GBP') => {
    const r = (0, finReview_1.reviewFinances)(f, cur);
...
const holed = (s) => {
    if (/ {2}/.test(s))
        return 'double space';
    if (/[.!?] +[a-z]/.test(s))
        return 'lower-case start after a full stop';
    if (/^\s|\s$/.test(s))
        return 'leading or trailing space';
    if (/\b(null|undefined|NaN)\b/.test(s))
        return 'a value name printed as a word';
    return null;
};
...
const losing = (0, finReview_1.reviewFinances)(fin({ revenue: 60000, expenses: 90000, members: 200, newMembers: 20, churnedMembers: 0 }), 'GBP');
...
const evens = (0, finReview_1.reviewFinances)(fin({ revenue: 60000, expenses: 60000, members: 200, newMembers: 12, churnedMembers: 0 }), 'GBP');
...
const earning = (0, finReview_1.reviewFinances)(fin({ revenue: 60000, expenses: 50000, members: 200, newMembers: 7, churnedMembers: 5 }), 'GBP');
ok(earning.score >= 55 && earning.score < 85, `the earning gym lands in the middle band — got ${earning.score}`);
ok(earning.netProfit > 0 && /is profitable/.test(earning.summary), `a gym that is making money is still told so — got "${earning.summary}"`);
...
const noCur = (0, finReview_1.reviewFinances)(fin({ revenue: 60000, expenses: 45000, members: 200, newMembers: 10, churnedMembers: 2 }), null);
...
const gbp = (0, finReview_1.reviewFinances)(fin({ revenue: 60000, expenses: 45000, members: 200, newMembers: 10, churnedMembers: 2 }), 'GBP');
...
for (const [label, f] of cases) {
    const r = (0, finReview_1.reviewFinances)(f, 'GBP');
    ...
    ok(Number.isFinite(r.marginPct) && Number.isFinite(r.churnPct) && Number.isFinite(r.growthPct), `${label}: no ratio may come out NaN or Infinity`);
}
if (errors.length) {
