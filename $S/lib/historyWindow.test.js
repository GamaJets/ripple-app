...
const historyWindow_1 = require("./historyWindow");
const errors = [];
...
const at = (y, m, d) => ({ id: `${y}-${m}-${d}`, t: new Date(y, m - 1, d, 12, 0, 0).toISOString() });
const keys = (l) => l.map((e) => e.id).join(',');
// ── a whole read is untouched, which is the common case by a mile ─────────
{
    const log = [at(2024, 1, 5), at(2024, 2, 6), at(2024, 3, 7)];
    const r = (0, historyWindow_1.wholeMonths)(log, false);
...
// ── a truncated read loses its oldest month, and only that one ────────────
{
    ...
    const r = (0, historyWindow_1.wholeMonths)(log, true);
...
// ── an empty read ─────────────────────────────────────────────────────────
{
    const r = (0, historyWindow_1.wholeMonths)([], true);
