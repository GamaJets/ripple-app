...
const checklistCopy_1 = require("./checklistCopy");
const bulkActions_1 = require("./bulkActions");
const errors = [];
const ok = (cond, msg) => { if (!cond)
    errors.push(msg); };
const eq = (a, b, msg) => ok(Object.is(a, b), `${msg} — got ${JSON.stringify(a)}, wanted ${JSON.stringify(b)}`);
const L = (label, icon = '') => ({ label, icon });
const T = (clientId, name, existing = [], maxSort = 0) => ({ clientId, name, existing, maxSort });
/* ── THE GUARD ─────────────────────────────────────────────────────────── */
ok((0, checklistCopy_1.guardChecklistCopy)('ready', 'ready').allowed, 'two whole reads allow the copy');
// Every one of these would produce duplicates, silently, on somebody's morning.
for (const bad of ['loading', 'partial', 'error']) {
    const g = (0, checklistCopy_1.guardChecklistCopy)('ready', bad);
...
ok(!(0, checklistCopy_1.guardChecklistCopy)('partial', 'ready').allowed, 'a truncated roster holds it too — the people past the cap are people');
ok(/twice|second time/i.test((0, checklistCopy_1.guardChecklistCopy)('ready', 'error').reason ?? ''), 'the refusal names the actual consequence, which is a line arriving twice');
ok(/ticks/i.test((0, checklistCopy_1.guardChecklistCopy)('ready', 'partial').reason ?? ''), 'and that removing a duplicate takes the client’s ticks with it, which is why this is not a warning');
/* ── matching ──────────────────────────────────────────────────────────── */
eq((0, checklistCopy_1.normaliseLabel)('  Ten  minutes of MOBILITY '), 'ten minutes of mobility', 'the key is trimmed, collapsed and case-folded');
...
ok((0, checklistCopy_1.normaliseLabel)('Water on waking') !== (0, checklistCopy_1.normaliseLabel)('Drink water when you wake up'), 'two wordings are two lines — the matcher does not guess at meaning');
/* ── the plan ──────────────────────────────────────────────────────────── */
const lines = [L('Protein at breakfast', '🥚'), L('Ten minutes of mobility'), L('Water on waking')];
const plain = (0, checklistCopy_1.planChecklistCopy)(lines, [T('a', 'Ana'), T('b', 'Ben')]);
...
const appended = (0, checklistCopy_1.planChecklistCopy)(lines, [T('a', 'Ana', [], 7)]);
eq(appended.targets[0].add.map((l) => l.sort).join(','), '8,9,10', 'copied lines append past what is already there');
const dedup = (0, checklistCopy_1.planChecklistCopy)(lines, [T('a', 'Ana', ['  protein AT breakfast '])]);
...
const someSkipped = (0, checklistCopy_1.planChecklistCopy)(lines, [
    T('a', 'Ana'),
...
const doubled = (0, checklistCopy_1.planChecklistCopy)([L('Water'), L('water ')], [T('a', 'Ana')]);
eq(doubled.writes, 1, 'a line repeated in the source is copied once');
/* ── what the coach reads ──────────────────────────────────────────────── */
const brief = (0, checklistCopy_1.copyBrief)(someSkipped, 'Cara');
...
ok(/cannot tick them|stays with you/i.test(brief.body), 'and that the client cannot remove them, which is what makes this different from a message');
ok(/nobody is told|Nobody is told/.test(brief.body), 'and that nothing is sent — a coach expecting a notification will follow up on a conversation that never happened');
ok(/Ben/.test(brief.body), 'and names who gets nothing');
const nothing = (0, checklistCopy_1.copyBrief)((0, checklistCopy_1.planChecklistCopy)(lines, [T('b', 'Ben', lines.map((l) => l.label))]), 'Cara');
...
const none = (0, checklistCopy_1.copyBrief)((0, checklistCopy_1.planChecklistCopy)(lines, []), 'Cara');
ok(!none.actionable, 'nobody ticked is not an action either');
const preview = (0, checklistCopy_1.copyPreview)(someSkipped);
...
/* ── the report ────────────────────────────────────────────────────────── */
const partly = (0, bulkActions_1.bulkReport)('checklist', [
...
ok(/tomorrow morning/i.test(partly.body), 'the success half says when the lines appear, because nothing is sent and nobody is told');
ok(/Nobody was notified/i.test(partly.body), 'in as many words');
const allBad = (0, bulkActions_1.bulkReport)('checklist', [{ clientId: 'a', name: 'Ana', ok: false, why: 'refused.' }]);
