...
//                    unanswered write
const planEdits_1 = require("./planEdits");
const errors = [];
...
/* ── THE ROUND TRIP ───────────────────────────────────────────────────────── */
{
    const back = (0, planEdits_1.readPlanEdits)((0, planEdits_1.writePlanEdits)(FULL));
    ...
    eq((0, planEdits_1.editCount)(FULL), 6, 'six separate changes — a movement both swapped and re-loaded is two things to look at');
    eq((0, planEdits_1.editsForDay)(FULL, 0), 3, 'three of them are about Monday');
    eq((0, planEdits_1.editsForDay)(FULL, 2), 2, 'and two about Wednesday, with the custom movement belonging to no one day');
    eq((0, planEdits_1.isEmptyEdits)(FULL), false, 'a plan with changes is not an unchanged plan');
    eq((0, planEdits_1.isEmptyEdits)(planEdits_1.EMPTY_PLAN_EDITS), true, 'and an untouched one is');
    eq(planEdits_1.PLAN_EDITS_KEY, 'repple.planEdits', 'the storage key is stable, because a renamed key is a wiped plan');
}
/* ── UNREADABLE ───────────────────────────────────────────────────────────
 *
 ...
 */
{
    const never = (0, planEdits_1.readPlanEdits)(null);
    ok(never.read, 'a member who has never changed anything HAS been read');
    eq((0, planEdits_1.isEmptyEdits)(never.edits), true, 'and has no changes');
    const broken = (0, planEdits_1.readPlanEdits)('{ not json');
    ok(!broken.read, 'bytes nobody can parse are not an empty plan');
    eq((0, planEdits_1.isEmptyEdits)(broken.edits), true, 'though what is handed back is safe to render');
    const wrongShape = (0, planEdits_1.readPlanEdits)('[1,2,3]');
    ...
    const messy = (0, planEdits_1.readPlanEdits)(JSON.stringify({
 ...
 */
{
    eq((0, planEdits_1.planEditsNote)(planEdits_1.EMPTY_PLAN_EDITS, true), null, '"0 changes are saved" is not a sentence');
    const shared = (0, planEdits_1.planEditsNote)(FULL, true);
    ok(/coach can see/.test(shared), 'a write the server took says the coach can see it');
    const kept = (0, planEdits_1.planEditsNote)(FULL, false);
    ...
    const unknown = (0, planEdits_1.planEditsNote)(FULL, null);
    ...
    const one = (0, planEdits_1.planEditsNote)({ ...planEdits_1.EMPTY_PLAN_EDITS, removed: ['0:lateral'] }, true);
