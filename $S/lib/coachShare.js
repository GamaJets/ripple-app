...
exports.COACH_ASK_NOT_ADVICE = exports.COACH_ASK_WHAT_NEVER_GOES = exports.COACH_ASK_WHAT_GOES = exports.NAME_TOKEN = exports.COACH_CLIENT_HEALTH = exports.COACH_CLIENT_KEYS = exports.COACH_BUSINESS_KEYS = exports.NOT_MEDICAL_ADVICE = exports.REPORT_CONSENT_BODY = exports.REPORT_CONSENT_TITLE = exports.REPORT_WITHHELD_NOTE = exports.WITHHELD_NOTE = exports.CONSENT_BODY = exports.CONSENT_TITLE = exports.WHERE_IT_GOES = exports.NEVER_SENT = exports.SENT_WITH_PERMISSION = exports.ALWAYS_SENT = exports.HEALTH_KEYS = exports.FITNESS_KEYS = exports.COACH_SHARE_KEY = void 0;
exports.consentFromStored = consentFromStored;
exports.storedConsent = storedConsent;
exports.sharedInjuries = sharedInjuries;
exports.sharedAreas = sharedAreas;
exports.shareableContext = shareableContext;
exports.weeklyFacts = weeklyFacts;
exports.businessAskContext = businessAskContext;
exports.clientAskContext = clientAskContext;
exports.fillName = fillName;
...
exports.COACH_SHARE_KEY = 'repple.coachShare';
/**
 ...
 */
function sharedInjuries(injs = []) {
    return sharedAreas((0, injuries_1.activeInjuries)(injs));
}
/**
 ...
 */
function sharedAreas(list = []) {
    if (!list.length)
        return '';
    return list.map((i) => `${(0, injuries_1.areaLabel)(i.area)} (${i.severity})`).join('; ');
}
/**
 ...
 */
function shareableContext(full, consent) {
    if (consent !== 'yes' && consent !== 'no')
        return null;
    const keys = consent === 'yes'
        ? [...exports.FITNESS_KEYS, ...exports.HEALTH_KEYS]
        : exports.FITNESS_KEYS;
    const out = {};
    for (const k of keys) {
        const v = full[k];
...
exports.WHERE_IT_GOES = 'It is sent to Anthropic’s Claude, through this app’s own server, so that it can answer you. It is not used to train a model and it is not sent to your gym.';
exports.CONSENT_TITLE = 'Before your coach can use your numbers';
exports.CONSENT_BODY = 'The AI coach answers better when it knows your body, your sleep and your injuries. That is health information, so it does not go anywhere until you say it can. You can change this at any time, and either answer lets you use the coach.';
/**
 ...
 */
function weeklyFacts(fitness, health, consent) {
    if (consent !== 'yes' && consent !== 'no')
        return null;
...
exports.REPORT_WITHHELD_NOTE = 'Your summary is written from your training only. It will not mention your weight, your body fat, your muscle, your measurements or your check-ins, because those are not being sent. Every figure on this screen is still yours to read.';
...
exports.NOT_MEDICAL_ADVICE = 'For pain, a new injury, or a diagnosis, see a doctor or physio before training.';
/* ══════════════════════════════════════════════════════════════════════════
 ...
 */
function businessAskContext(full) {
    return pick(full, exports.COACH_BUSINESS_KEYS);
}
/**
 ...
 */
function clientAskContext(full) {
    return pick(full, exports.COACH_CLIENT_KEYS);
}
/**
 ...
 */
function fillName(text, name, coach) {
    let out = String(text ?? '');
    const first = (name || '').trim().split(/\s+/)[0];
    if (first)
        out = out.split(exports.NAME_TOKEN).join(first);
    const c = (coach || '').trim().split(/\s+/)[0];
    if (c)
        out = out.split('{coach}').join(c);
    return out;
}
/** What the coach is told about their assistant, on the screen. Sentence case;
 *  it is prose under a heading. */
exports.COACH_ASK_WHAT_GOES = 'Your own figures go — sessions, clients, adherence, takings and the currency they are in. When you ask about one client, what goes is their goal, how they are coached, whether they are turning up, what they are training and which areas they have flagged as injured.';
exports.COACH_ASK_WHAT_NEVER_GOES = 'No name, no email and nothing that says who anybody is. No weight, body fat, scan, sleep or recovery figure. Nothing written in an injury note or read off a document, and nothing from your messages. Replies come back saying {name} and this screen fills it in.';
exports.COACH_ASK_NOT_ADVICE = 'It answers from the figures it is given and nothing else. It has not met your clients, it cannot see their form, and it is not a substitute for asking them.';
