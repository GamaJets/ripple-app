 ...
 */
{
    eq((0, unstampCheck_1.unstampBlocker)(31, 31), null, 'a run of 31 sessions that freed 31 is reversed');
    eq((0, unstampCheck_1.unstampBlocker)(1, 1), null, 'and a run of one');
    eq((0, unstampCheck_1.unstampBlocker)(0, 0), null, 'a run with no sessions at all — a coach paid only for classes and a reimbursement — reverses cleanly');
}
/* ── THE SHORTFALL ────────────────────────────────────────────────────────
 ...
 */
{
    const why = (0, unstampCheck_1.unstampBlocker)(31, 29);
    ...
    ok(!!why && why.includes('Nothing has been changed'), 'and it says nothing has changed, which is the point of refusing before the settlement is touched');
    ok(!!why && why.includes('owed'), 'and says what going ahead would cost — the coach’s money, not a database inconsistency');
    const one = (0, unstampCheck_1.unstampBlocker)(4, 3);
    ...
    ok(!!(0, unstampCheck_1.unstampBlocker)(4, 2) && (0, unstampCheck_1.unstampBlocker)(4, 2).includes('those sessions stay'), 'and reads as a plural when there is more than one');
    const all = (0, unstampCheck_1.unstampBlocker)(12, 0);
    ok(all !== null, 'the worst case — not one session came loose — is refused like any other');
    ok(!!all && all.includes('only 0 of them'), 'and says so plainly rather than reading as though something worked');
}
/* ── THE SURPLUS ──────────────────────────────────────────────────────────
 ...
 */
{
    const why = (0, unstampCheck_1.unstampBlocker)(10, 12);
 ...
 */
{
    const why = (0, unstampCheck_1.unstampBlocker)(31, null);
    ...
    eq((0, unstampCheck_1.unstampBlocker)(31, undefined), why, 'undefined is the same fact as null');
    eq((0, unstampCheck_1.unstampBlocker)(0, null), why, 'even for a run with no sessions — nobody counted is nobody counted');
}
/* ── THE SENTENCE ─────────────────────────────────────────────────────────
 ...
 */
{
    for (const [claimed, got] of [[31, 29], [10, 12], [5, null]]) {
        const why = (0, unstampCheck_1.unstampBlocker)(claimed, got);
        ok(!!why && /not taken back/.test(why), `the refusal for (${claimed}, ${got}) opens by saying the reversal did not happen`);
        ok(!!why && /[Nn]othing (has been changed|is untouched|has changed)|untouched/.test(why), `and for (${claimed}, ${got}) it says the record is untouched`);
