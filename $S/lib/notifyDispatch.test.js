...
const notifyDispatch_1 = require("./notifyDispatch");
...
const KNOWN = new Set(coachNotify_1.COACH_CHANNELS.map((c) => c.key));
const ROUTES_TRIED = [
...
for (const r of ROUTES_TRIED) {
    const c = (0, notifyDispatch_1.channelForRoute)(r);
    ok(c != null, `${r} is classified`);
    ok(c != null && KNOWN.has(c), `${r} names a channel this build has a switch for`);
    ok(c != null && (0, coachNotify_1.channelDef)(c) != null, `${r}'s channel has a definition`);
}
...
eq((0, notifyDispatch_1.channelForRoute)('/(trainer)/payments'), 'money', 'a subscription failing, a package bought and a pack running out are money');
eq((0, notifyDispatch_1.channelForRoute)('/(trainer)/documents'), 'admin', 'a document a client accepted is paperwork');
eq((0, notifyDispatch_1.channelForRoute)('/(trainer)/client-intake'), 'admin', 'and so is an intake coming back');
/* ── every server-written kind has a channel ───────────────────────────── */
//
// THE assertion. See the header.
const byChannel = new Map();
for (const s of notifyInbox_1.SERVER_WRITTEN) {
    const c = (0, notifyDispatch_1.notificationChannel)(s.route, s.title);
    ok(c != null, `${s.where} writes '${s.title}' and nothing says which switch governs it — `
...
ok((byChannel.get('money') ?? 0) >= 5, 'the money switch governs the several money kinds parts 158–612 write, not one of them');
...
ok((byChannel.get('book') ?? 0) >= 2, 'the coach\'s own book is written by a nightly pass as well as computed on the handset');
...
eq((0, notifyDispatch_1.channelForTitle)('An invoice from your gym'), 'money', 'the gym invoice has no screen and is still money');
eq((0, notifyDispatch_1.channelForTitle)('Your coaching has ended'), 'clients', 'a coaching ending is joining and leaving');
eq((0, notifyDispatch_1.channelForTitle)('A client has signed the release'), 'admin', 'and a signed release is paperwork');
eq((0, notifyDispatch_1.channelForTitle)('A client has signed the'), null, 'matched whole, not as a prefix');
eq((0, notifyDispatch_1.channelForTitle)('a client has signed the release'), null, 'and not case-insensitively — these are literals from SQL');
eq((0, notifyDispatch_1.channelForTitle)('  Your coaching has ended  '), 'clients', 'though surrounding space is not a difference');
...
eq((0, notifyDispatch_1.channelForRoute)('/(trainer)/client-goals?clientId=abc'), 'clients', 'a query string is part of the route and not part of the match');
eq((0, notifyDispatch_1.channelForRoute)('/(trainer)/client-goals-archive'), null, 'a longer route that happens to start with a known one is a different screen');
eq((0, notifyDispatch_1.channelForRoute)('/(trainer)/payments/history'), null, 'and so is a child of one');
eq((0, notifyDispatch_1.channelForRoute)('/(trainer)/nowhere'), null, 'a route this build does not know names no switch');
eq((0, notifyDispatch_1.channelForRoute)(null), null, 'and neither does no route');
eq((0, notifyDispatch_1.channelForRoute)(''), null, 'nor an empty one');
eq((0, notifyDispatch_1.channelForRoute)('   '), null, 'nor whitespace');
/* ── route first, then title ───────────────────────────────────────────── */
eq((0, notifyDispatch_1.notificationChannel)('/(trainer)/payments', 'A client has signed the release'), 'money', 'the route is the structural signal and it wins');
eq((0, notifyDispatch_1.notificationChannel)(null, 'A client has signed the release'), 'admin', 'the title is only reached when there is no route');
eq((0, notifyDispatch_1.notificationChannel)('/(trainer)/nowhere', 'An invoice from your gym'), 'money', 'and also when there is a route nobody has classified — a classification beats none');
eq((0, notifyDispatch_1.notificationChannel)(null, null), null, 'nothing to go on is nothing decided');
...
for (const r of notifyDispatch_1.PUSHED_BY_ITS_WRITER) {
    const d = (0, notifyDispatch_1.dispatchDecision)(r, 'anything', false);
...
eq((0, notifyDispatch_1.dispatchDecision)('/(trainer)/chat?clientId=abc', 'Sam', false).dispatch, false, 'the coach\'s thread carries a clientId and is still the chat that is already pushed');
...
const viaCaller = (0, notifyDispatch_1.dispatchDecision)('/(trainer)/payments', 'A package was bought', true);
...
const unknown = (0, notifyDispatch_1.dispatchDecision)('/(trainer)/nowhere', 'Something new', false);
...
ok(/filter/.test(unknown.why), 'and the reason names the consequence: send-push does not filter a channelless send at all');
/* ── and what it will ──────────────────────────────────────────────────── */
for (const s of notifyInbox_1.SERVER_WRITTEN) {
    const bare = (s.route ?? '').split('?')[0];
    const isChat = notifyDispatch_1.PUSHED_BY_ITS_WRITER.includes(bare);
    const d = (0, notifyDispatch_1.dispatchDecision)(s.route, s.title, false);
    eq(d.dispatch, !isChat, `${s.where} · '${s.title}' — ${isChat ? 'chat is already pushed' : 'nothing else pushes this and it must be dispatched'}`);
    if (d.dispatch) {
        ok(d.channel != null && KNOWN.has(d.channel), `${s.title} is sent on a switch the coach actually has`);
    }
}
...
const pushRoutesWithoutChannel = [...new Set(notifyInbox_1.KNOWN_PUSHES
        .filter((p) => p.route && (0, notifyDispatch_1.notificationChannel)(p.route, p.title) == null)
        .map((p) => p.route))].sort();
eq(pushRoutesWithoutChannel.join(' | '), UNCLASSIFIED_ON_PURPOSE.join(' | '), 'every route this repo pushes answers to one of the coach’s switches, bar the one that deliberately does not');
...
for (const route of ROUTES_TRIED) {
    const d = (0, notifyDispatch_1.dispatchDecision)(route, '', false);
    if (d.dispatch)
        ok(d.channel != null, `${route} is never dispatched without a channel`);
}
if (errors.length) {
