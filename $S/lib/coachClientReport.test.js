...
const coachClientReport_1 = require("./coachClientReport");
...
const sectionOf = (html, heading) => (html.split('<h2>' + heading)[1] ?? '').split('<h2>')[0];
const errors = [];
...
/* ── 1. the session tally: a null outcome is not a missed session ──────────
   ...
   the other way. */
{
    const t = (0, coachClientReport_1.sessionTally)(SESSIONS, 'ready');
    ...
    eq((t.completed ?? 0) + (t.noShow ?? 0) + (t.cancelled ?? 0) + (t.lateCancelled ?? 0) + (t.unrecorded ?? 0), t.booked, 'and the five figures account for every booked session exactly once');
...
}
{
    ...
    const t = (0, coachClientReport_1.sessionTally)([{ startsAt: '2026-08-01T09:00:00Z', outcome: 'rescheduled_by_gym' }], 'ready');
...
{
    eq((0, coachClientReport_1.sessionTally)(null, 'error').state, 'unreadable', 'a failed read is unreadable');
    eq((0, coachClientReport_1.sessionTally)(null, 'error').booked, null, 'and states no count at all');
    eq((0, coachClientReport_1.sessionTally)(SESSIONS, 'loading').state, 'unreadable', 'a read still in flight is unreadable, NOT whole');
    eq((0, coachClientReport_1.sessionTally)([], 'ready').state, 'none', 'a landed empty read is genuinely none');
    eq((0, coachClientReport_1.sessionTally)([], 'ready').booked, 0, 'and zero there is a real figure');
    const p = (0, coachClientReport_1.sessionTally)(SESSIONS, 'partial');
...
{
    const d = (0, coachClientReport_1.coachClientReportDoc)(base({ coachNote: null }));
    ...
    // disclaimer.
    let prose = (d.html + '\n' + d.text).toLowerCase();
    for (const stated of [coachClientReport_1.COACH_REPORT_LIMITS, coachClientReport_1.COACH_REPORT_NO_PHOTOS, coachClientReport_1.COACH_REPORT_NO_RATE, ...coachClientReport_1.COACH_REPORT_PROVENANCE]) {
        prose = prose.split(stated.toLowerCase()).join(' ').split((0, clientReport_1.escapeHtml)(stated).toLowerCase()).join(' ');
    }
    for (const f of FORBIDDEN) {
        ok(!prose.includes(f), `the document must contain no judgement — found "${f}"`);
    }
    ok(d.html.includes((0, clientReport_1.escapeHtml)(coachClientReport_1.COACH_REPORT_LIMITS).slice(0, 60)), 'and it says outright that it contains no assessment or rating');
    ok(d.text.includes(coachClientReport_1.COACH_REPORT_LIMITS), 'in the text fallback as well as the HTML');
}
{
    ...
    const d = (0, coachClientReport_1.coachClientReportDoc)(base({
    ...
    ok(d.text.includes(coachClientReport_1.COACH_REPORT_LIMITS), 'and still carries the limits statement');
    ok(d.text.includes(coachClientReport_1.COACH_REPORT_NO_PHOTOS), 'and still says no photographs are included');
    ok(d.text.includes('The coach did not write anything here'), 'and says the coach wrote nothing rather than leaving a heading over a blank a reader would hunt for');
}
/* ── 3. the coach's own words are the ONE opinion, and are labelled as one ─*/
{
    const d = (0, coachClientReport_1.coachClientReportDoc)(base({ coachNote: 'She is on track and adherence has been excellent.' }));
    ...
    ok(d.text.includes('not a finding of this app and not a clinical assessment'), 'and disclaimed as their opinion rather than the app’s');
    ...
    ok(d.html.indexOf('in their own words') < d.html.indexOf('on track'), 'the attribution is printed above the quote, not under it');
...
}
{
    ...
    const d = (0, coachClientReport_1.coachClientReportDoc)(base({ coachStatus: 'error', coachName: null, coachNote: 'Solid block.' }));
...
{
    eq((0, coachClientReport_1.coachReportCaveats)(base()).length, 0, 'six whole reads produce no caveats at all');
    eq((0, coachClientReport_1.coachReportCaveats)(base({ injuries: { status: 'loading', items: INJURIES } })).length, 1, 'a section still loading produces a caveat — it is not silently treated as read');
    eq((0, coachClientReport_1.coachReportCaveats)(base({ coachStatus: 'error' })).length, 1, 'including the read that names the author');
}
{
    const d = (0, coachClientReport_1.coachClientReportDoc)(base({ injuries: { status: 'error', items: [] } }));
    ...
    ok(!inj.includes('No injuries have been recorded'), 'and never prints "no injuries recorded" over a refused read — the most dangerous sentence this app can produce');
...
{
    const d = (0, coachClientReport_1.coachClientReportDoc)(base({ sessions: { status: 'error', items: null } }));
...
{
    const d = (0, coachClientReport_1.coachClientReportDoc)(base({ sessions: { status: 'partial', items: SESSIONS } }));
    const s = sectionOf(d.html, 'Sessions booked');
   ...
   score, and it is arithmetic over a set containing sessions nobody marked. */
{
    ...
    ok(d.text.includes(coachClientReport_1.COACH_REPORT_NO_RATE), 'and the document says why there is none');
    ok(d.text.includes('It is not a session that was missed and it is not one that went ahead'), 'and states outright what an unrecorded outcome is not');
...
{
    const d = (0, coachClientReport_1.coachClientReportDoc)(base({ weightUnit: 'lb', lengthUnit: 'in', unitNote: 'Shown in pounds, which is what Dana reads in.' }));
...
/* ── 8. a change is subtraction over a WHOLE read, and never over a prefix ─*/
{
    ...
    const p = (0, coachClientReport_1.coachClientReportDoc)(base({ composition: { status: 'partial', items: SCANS } }));
...
/* ── 10. the training day table is capped, and says which slice it shows ──*/
{
    ...
    ok(d.text.includes(`The ${coachClientReport_1.COACH_TRAINING_DAYS_SHOWN} most recent of the 40 days read are listed`), 'a long history says which slice of it is printed');
...
{
    const whole = (0, coachClientReport_1.coachClientReportDoc)(base());
    ok(!(0, coachClientReport_1.coachReportShareBlurb)(whole, 'Dana Okafor').includes('BEFORE YOU SEND IT'), 'a complete document needs no warning');
    ok((0, coachClientReport_1.coachReportShareBlurb)(whole, 'Dana Okafor').includes('no assessment or rating of any kind'), 'but it does say what the document is not, every time');
    const broken = (0, coachClientReport_1.coachClientReportDoc)(base({ injuries: { status: 'error', items: [] }, composition: { status: 'error', items: [] } }));
    const b = (0, coachClientReport_1.coachReportShareBlurb)(broken, 'Dana Okafor');
...
/* ── 12. the brand is the tenant's, and the platform name is not on it ────*/
{
    ...
    ok((0, coachClientReport_1.coachClientReportDoc)(base({ brand: '' })).html.includes('Repple'), 'an empty brand falls back to something rather than printing nothing on a handover');
}
...
{
    const rows = [1, 2, 3];
    eq((0, coachClientReport_1.countableRows)(rows, 'ready'), 3, 'a whole read may be counted');
    eq((0, coachClientReport_1.countableRows)([], 'ready'), 0, 'and an empty whole read counts as none — that is a real answer');
    eq((0, coachClientReport_1.countableRows)(rows, 'partial'), null, 'a truncated read may not be counted, however many rows came back');
    eq((0, coachClientReport_1.countableRows)(rows, 'error'), null, 'nor may a refused one, even holding rows from before it failed');
    eq((0, coachClientReport_1.countableRows)(rows, 'loading'), null, 'nor one still in flight');
    eq((0, coachClientReport_1.countableRows)(null, 'ready'), null, 'and null rows are not nought rows');
    eq((0, coachClientReport_1.countableRows)(undefined, 'ready'), null, 'nor are absent ones');
}
