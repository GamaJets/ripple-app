...
const failedWrite_1 = require("./failedWrite");
...
/* ── classification ────────────────────────────────────────────────────── */
{
    ...
    eq((0, failedWrite_1.writeFate)({ code: '23514', message: 'violates check constraint' }), 'refused', 'a CHECK constraint is the server having read the row and said no');
    eq((0, failedWrite_1.writeFate)({ code: '42501', message: 'row-level security' }), 'refused', 'and so is RLS');
    eq((0, failedWrite_1.writeFate)({ status: 400, message: 'bad request' }), 'refused', 'and so is a 4xx');
    eq((0, failedWrite_1.writeFate)({ code: 'PGRST116', message: 'no rows' }), 'refused', 'and so is PostgREST answering');
}
{
    // Our own ceiling. This is the whole reason the file exists.
    const err = (0, requestTimeout_1.requestTimeoutError)('https://x.supabase.co/rest/v1/gym_payments', 'POST', requestTimeout_1.CALL_CEILING_MS);
    eq((0, failedWrite_1.writeFate)(err), 'unanswered', 'a request WE gave up waiting for tells us nothing about the row');
    eq((0, failedWrite_1.mayRetryWrite)('unanswered'), false, 'so a plain retry button is not offered');
    eq((0, failedWrite_1.mayAssertUnchanged)('unanswered'), false, 'and nothing on the screen may claim the row is absent');
}
{
    ...
    eq((0, failedWrite_1.writeFate)({ status: 503, message: 'service unavailable' }), 'unanswered', 'a 503 is the server up and not having answered about the row');
    eq((0, failedWrite_1.writeFate)({ status: 408, message: 'request timeout' }), 'unanswered', 'and 408 is a timeout by another route');
    eq((0, failedWrite_1.writeFate)({ code: '57014', message: 'statement timeout' }), 'unanswered', 'and a Postgres statement timeout may have rolled back or may not have reached us');
}
{
    ...
    eq((0, failedWrite_1.writeFate)(bare), 'unanswered', 'a bare fetch failure is ambiguous and is treated as such');
    // Unless the screen knows the device was already offline, which is the one
    // piece of positive evidence that nothing was sent.
    eq((0, failedWrite_1.writeFate)(bare, { online: false }), 'unreachable', 'a device that was offline sent nothing');
    eq((0, failedWrite_1.writeFate)(bare, { online: true }), 'unanswered', 'and being online does not make it a refusal');
    eq((0, failedWrite_1.writeFate)(bare, { online: null }), 'unanswered', 'nor does not having asked');
    // Offline beats even a refusal-shaped error: if nothing was sent, nothing was
    // refused, and the shape came from somewhere else.
    eq((0, failedWrite_1.writeFate)({ code: '23514' }, { online: false }), 'unreachable', 'nothing was sent, so nothing was refused');
}
eq((0, failedWrite_1.writeFate)(null), 'unanswered', 'a throw with nothing in it explains nothing');
eq((0, failedWrite_1.writeFate)(undefined), 'unanswered', 'and neither does no throw at all');
/* ── the three sentences ───────────────────────────────────────────────── */
{
    const refusedText = (0, failedWrite_1.failedWriteSentence)('refused', PAYMENT, 'duplicate key value');
...
{
    const unreachableText = (0, failedWrite_1.failedWriteSentence)('unreachable', PAYMENT, 'you are offline');
...
{
    const unknownText = (0, failedWrite_1.failedWriteSentence)('unanswered', PAYMENT, 'No reply within 30s for POST /rest/v1 — request timed out.');
    ...
    ok(/only the\s+reply lost/.test(unknownText) || unknownText.includes('reply lost'), 'and names the case that makes it ambiguous');
    ...
    for (const lie of ['Nothing was saved', 'Nothing was written', 'Nothing was sent',
        ...
        ok(!unknownText.includes(lie), `“${lie}” is a claim about the database and this state has none — got ${unknownText}`);
    }
}
{
    ...
    const three = new Set([
        (0, failedWrite_1.failedWriteSentence)('refused', PAYMENT, 'x'),
        (0, failedWrite_1.failedWriteSentence)('unreachable', PAYMENT, 'x'),
        (0, failedWrite_1.failedWriteSentence)('unanswered', PAYMENT, 'x'),
    ]);
    eq(three.size, 3, 'the three states produce three sentences');
}
/* ── punctuation, so a call site does not have to think about it ───────── */
{
    const withStop = (0, failedWrite_1.failedWriteSentence)('refused', PAYMENT, 'it was refused.');
    ok(!withStop.includes('refused..'), `a reason that ends in a full stop does not gain a second — got ${withStop}`);
    const withoutStop = (0, failedWrite_1.failedWriteSentence)('refused', PAYMENT, 'it was refused');
    ok(withoutStop.includes('it was refused.'), 'and one that does not, gains one');
    const noReason = (0, failedWrite_1.failedWriteSentence)('refused', PAYMENT, null);
    ok(noReason.includes('the database refused the write.'), `a silent error still produces a whole sentence — got ${noReason}`);
    const trailing = (0, failedWrite_1.failedWriteSentence)('refused', { ...PAYMENT, what: 'That payment.', unchanged: 'nothing moved.' }, 'no');
...
{
    const err = (0, requestTimeout_1.requestTimeoutError)('https://x.supabase.co/rest/v1/gym_payments', 'POST', requestTimeout_1.CALL_CEILING_MS);
    const note = (0, failedWrite_1.failedWriteNote)(err, PAYMENT);
    ...
    ok(!note.includes('gym_payments') || note.includes('/rest/v1/gym_payments'), 'and the URL is the redacted path requestTimeout already produces, not a query string of ids');
    const refusedNote = (0, failedWrite_1.failedWriteNote)({ code: '23505', message: 'duplicate key' }, PAYMENT);
