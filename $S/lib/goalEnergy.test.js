...
const BODY = { weightKg: 90, bodyFatPct: 24, activity: 1.4, goal: 'tone', diet: 'meat' };
const TDEE = (0, nutrition_1.maintenanceFor)(BODY).tdee;
...
const weightGoal = (targetKg, targetDays) => ({
    id: 'g1',
    kind: 'weight',
...
const planFor = (goal, series = SERIES, tdee = TDEE) => (0, goalEnergy_1.energyPlanFor)({ goal, weightSeries: series, tdeeKcal: tdee, nowMs: NOW });
const derived = (p, what) => {
    if (p.kind === 'derived')
        return p;
...
ok(near(fast.kcalDelta, (-0.5 * goalEnergy_1.KCAL_PER_KG) / 7, 10), `a 0.5 kg/wk deficit is about 550 kcal/day, got ${fast.kcalDelta}`);
...
ok(Math.abs(crash.plannedRateKg) <= 90 * goalEnergy_1.MAX_LOSS_FRACTION_PER_WEEK + 0.01, `a 90 kg client must not be planned past 0.9 kg/wk, got ${crash.plannedRateKg.toFixed(2)}`);
...
ok(crash.kcal === Math.round((TDEE * (1 - goalEnergy_1.MAX_DEFICIT_FRACTION_OF_TDEE)) / 10) * 10, `and the plan sits exactly on it, got ${crash.kcal}`);
...
const bigTdee = (0, nutrition_1.maintenanceFor)({ weightKg: 130, bodyFatPct: 22, activity: 1.9 }).tdee;
const heavy = derived((0, goalEnergy_1.energyPlanFor)({ goal: weightGoal(110, 70), weightSeries: [{ t: iso(-20), v: 131 }, { t: iso(-1), v: 130 }], tdeeKcal: bigTdee, nowMs: NOW }), '20 kg in 10 weeks for a 130 kg client');
...
ok(crash.etaMs != null && crash.etaMs > crash.targetDateMs, 'and it must say when it does arrive — later than the date that was asked for');
ok(crash.kcal >= goalEnergy_1.MIN_PLAN_KCAL, `no plan may go under ${goalEnergy_1.MIN_PLAN_KCAL} kcal, got ${crash.kcal}`);
ok(crash.kcal >= TDEE * (1 - goalEnergy_1.MAX_DEFICIT_FRACTION_OF_TDEE) - 10, `no plan may cut more than ${goalEnergy_1.MAX_DEFICIT_FRACTION_OF_TDEE * 100}% of maintenance, got ${crash.kcal} against ${Math.round(TDEE)}`);
...
const small = { ...BODY, weightKg: 52, bodyFatPct: 20, activity: 1.3 };
const smallTdee = (0, nutrition_1.maintenanceFor)(small).tdee;
const tight = derived((0, goalEnergy_1.energyPlanFor)({ goal: weightGoal(46, 42), weightSeries: [{ t: iso(-20), v: 52.6 }, { t: iso(-1), v: 52 }], tdeeKcal: smallTdee, nowMs: NOW }), 'a light client on a 6-week deadline');
ok(tight.kcal >= goalEnergy_1.MIN_PLAN_KCAL, `the absolute floor holds for a small client too, got ${tight.kcal}`);
...
const fallback = (p, reason, what) => {
    if (p.kind !== 'enum') {
        errors.push(`${what}: expected the enum fallback, got a derived plan`);
        return;
    }
...
fallback(planFor(weightGoal(89, goalTargets_1.MIN_TREND_DAYS - 1)), 'date-too-soon', 'a date sooner than a trend can be measured');
...
const before = (0, nutrition_1.macrosFor)(BODY);
const withEnum = (0, nutrition_1.macrosFor)({ ...BODY, energyPlan: planFor(null) });
ok(JSON.stringify(before) === JSON.stringify(withEnum), `an 'enum' plan must leave every macro exactly as it was: ${JSON.stringify(before)} vs ${JSON.stringify(withEnum)}`);
ok(JSON.stringify((0, nutrition_1.macrosFor)({ ...BODY, energyPlan: null })) === JSON.stringify(before), 'and so must a null plan');
// ── the derived plan actually reaches the plate ────────────────────────────
const fastMacros = (0, nutrition_1.macrosFor)({ ...BODY, energyPlan: fast });
const slowMacros = (0, nutrition_1.macrosFor)({ ...BODY, energyPlan: slow });
...
ok(fastMacros.protein > before.protein, `a 'tone' client held in a deficit must get MORE protein than the enum gave them, not the same: ${fastMacros.protein} vs ${before.protein}`);
ok(fastMacros.protein === slowMacros.protein, 'protein must not drop as the deficit deepens — the 12-week and 40-week plans carry the same grams');
ok(fastMacros.protein === (0, nutrition_1.macrosFor)({ ...BODY, goal: 'fatloss' }).protein, 'a derived deficit protein target matches the dieting figure the fatloss enum already used');
ok((0, nutrition_1.macrosFor)({ ...BODY, goal: 'muscle', energyPlan: fast }).protein >= (0, nutrition_1.macrosFor)({ ...BODY, goal: 'muscle' }).protein, 'and the floor may only ever raise protein, never lower it');
...
ok((0, nutrition_1.macrosFor)({ ...BODY, energyPlan: bulkPlan }).protein === before.protein, 'a surplus leaves the client’s own protein figure alone');
// Gaining is clamped harder than losing.
const bulkFast = derived(planFor(weightGoal(96, 28)), '6 kg gained in 4 weeks');
ok(near(bulkFast.plannedRateKg, 90 * goalEnergy_1.MAX_GAIN_FRACTION_PER_WEEK, 0.02), `a 90 kg client must not be planned past 0.45 kg/wk of gain, got ${bulkFast.plannedRateKg.toFixed(2)}`);
...
ok(JSON.stringify(planFor(g).observed) === JSON.stringify((0, goalTargets_1.projectionOf)(g, SERIES, NOW)), 'the observed trend must come from projectionOf unchanged — no second rate calculation lives here');
ok((0, goalEnergy_1.observedRateKg)(fast) != null, 'readings spanning four weeks support a measured pace');
...
const shortSeries = [{ t: iso(-(goalTargets_1.MIN_TREND_DAYS - 1)), v: 90.4 }, { t: iso(0), v: 90.0 }];
const shortPlan = planFor(g, shortSeries);
ok((0, goalEnergy_1.observedRateKg)(shortPlan) === null, `readings ${goalTargets_1.MIN_TREND_DAYS - 1} days apart are water, not a trend, and must yield no pace`);
const justEnough = [{ t: iso(-goalTargets_1.MIN_TREND_DAYS), v: 90.4 }, { t: iso(0), v: 90.0 }];
ok((0, goalEnergy_1.observedRateKg)(planFor(g, justEnough)) != null, `and one more day — ${goalTargets_1.MIN_TREND_DAYS} — is the span that does, so the threshold is pinned from both sides`);
ok(shortPlan.kind === 'derived', 'but they are still enough to plan from — the target and the date are what set the rate');
ok((0, goalEnergy_1.observedRateKg)({ kind: 'enum', reason: 'no-goal', observed: null }) === null, 'no goal means no observed pace');
// ── the clamp helpers, directly ────────────────────────────────────────────
ok((0, goalEnergy_1.safeWeeklyRate)(-2, 90) === -0.9, 'a 90 kg client clamps to 0.9 kg/wk of loss (1% of bodyweight)');
ok((0, goalEnergy_1.safeWeeklyRate)(-2, 130) === -1, 'and the absolute 1 kg/wk NHS ceiling catches a heavier one before the percentage does');
ok((0, goalEnergy_1.safeWeeklyRate)(-0.4, 90) === -0.4, 'a rate already inside the range is passed through untouched');
ok((0, goalEnergy_1.safeWeeklyRate)(5, 90) === 0.45, 'gaining clamps to 0.5% of bodyweight a week');
ok((0, goalEnergy_1.safeWeeklyRate)(0, 90) === 0, 'maintenance is a legitimate answer');
ok((0, goalEnergy_1.planKcal)(2400, 0).kcal === 2400, 'no rate, no adjustment');
ok((0, goalEnergy_1.planKcal)(2400, -0.5).kcal === 1850, `−0.5 kg/wk off 2,400 kcal is 1,850, got ${(0, goalEnergy_1.planKcal)(2400, -0.5).kcal}`);
ok((0, goalEnergy_1.planKcal)(2400, -3).floored, 'an impossible rate is reported as floored, not silently delivered');
ok((0, goalEnergy_1.planKcal)(2400, -3).kcal === 1800, 'and lands on 75% of maintenance');
ok((0, goalEnergy_1.planKcal)(1400, -3).kcal === goalEnergy_1.MIN_PLAN_KCAL, 'with the absolute 1,200 floor underneath it for a small maintenance');
