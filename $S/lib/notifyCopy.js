...
exports.CLASS_OFF_ROUTE = exports.CLASS_OFF_TITLE_MANY = exports.CLASS_OFF_TITLE = exports.COACH_DECLINED_ROUTE = exports.COACH_ACCEPTED_ROUTE = exports.PUSH_PARTIAL_NOTE = exports.NOTICE_ROUTE = exports.NOTICE_BODY_MAX = exports.NOTICE_TITLE_MAX = void 0;
exports.clip = clip;
exports.noticeNotification = noticeNotification;
exports.invoiceNotification = invoiceNotification;
exports.invoiceReminderNotification = invoiceReminderNotification;
exports.pushPartialNote = pushPartialNote;
exports.deliverySummary = deliverySummary;
exports.pushConsequence = pushConsequence;
exports.classStartsIn = classStartsIn;
exports.coachAnswerNotification = coachAnswerNotification;
exports.coachAnswerConfirmation = coachAnswerConfirmation;
exports.classOffNotification = classOffNotification;
exports.classOffBuckets = classOffBuckets;
exports.classOffConfirmation = classOffConfirmation;
...
exports.NOTICE_ROUTE = '/(client)/notices';
/* ── clipping ─────────────────────────────────────────────────────────────── */
/**
 ...
 */
function noticeNotification(kind, body, gymName) {
    const b = clip(body ?? '', exports.NOTICE_BODY_MAX);
    if (!b)
        return null;
    const gym = (gymName ?? '').trim();
    const title = kind === 'coach'
        ? 'A notice from your coach'
        : gym ? clip(`A notice from ${gym}`, exports.NOTICE_TITLE_MAX) : 'A notice from your gym';
    return { title, body: b };
}
/* ── an invoice ───────────────────────────────────────────────────────────── */
/**
 ...
 */
function invoiceNotification(inv) {
    const n = (0, coachInvoice_1.invoiceNumber)(inv.seq);
    const amount = (0, coachInvoice_1.money)(inv);
    ...
    return {
        ...
        body: clip(`${line} ${claim} Ask them for a copy of the document.`, exports.NOTICE_BODY_MAX),
    };
}
/**
 ...
 */
function invoiceReminderNotification(inv) {
    const n = (0, coachInvoice_1.invoiceNumber)(inv.seq);
    const amount = (0, coachInvoice_1.money)(inv);
    const what = clip(inv.description || '', 120);
    const line = amount
    ...
    const when = /^\d{4}-\d{2}-\d{2}$/.test(due) ? ` They stated it was due on ${(0, coachInvoice_1.invoiceDayLabel)(due)}.` : '';
    return {
        title: 'A reminder from your coach',
        body: clip(`${line} Their record still shows this one as outstanding.${when} If you have already settled it, tell them — this app is not told when a payment reaches them. Ask them for a copy of the document.`, exports.NOTICE_BODY_MAX),
    };
}
/**
...
exports.PUSH_PARTIAL_NOTE = `${PARTIAL_CAUSE}, so more people may be without one than this says.`;
/**
 ...
 */
function pushPartialNote(recipients) {
    return recipients === 1 ? `${PARTIAL_CAUSE}, so they may not have got one.` : exports.PUSH_PARTIAL_NOTE;
}
/**
...
function deliverySummary(r) {
    ...
    if (r.recipients === 0) {
        ...
        parts.push(`It is posted, and at least ${(0, format_1.num)(r.recorded)} people have it in their notifications. There are more people on this notice than can be written to in one go, so some of them do not — they will see it when they open their notices.`);
    }
        ...
        parts.push(`It is posted, and ${(0, format_1.num)(r.recorded)} ${r.recorded === 1 ? 'person has' : 'people have'} it in their notifications. There are more people to address than this app could read in one go, so some may not have been included.`);
    }
        ...
        parts.push(`It is posted, and ${(0, format_1.num)(r.recorded)} of ${(0, format_1.num)(r.recipients)} people now have it in their notifications.`);
    }
    else {
        parts.push(`It is posted, and ${(0, format_1.num)(r.recorded)} ${r.recorded === 1 ? 'person has' : 'people have'} it in their notifications.`);
    }
    if (r.push === 'queued' && r.pushPartial) {
        ...
        parts.push(`A push was queued as well. ${exports.PUSH_PARTIAL_NOTE} Everyone above still has it in their notifications.`);
    }
 ...
 */
function pushConsequence(kind, recipients) {
    const who = kind === 'coach' ? 'client' : 'member';
    const audience = recipients == null
        ? `every ${who}`
        : `${(0, format_1.num)(recipients)} ${who}${recipients === 1 ? '' : 's'}`;
 ...
 */
function classStartsIn(startsAt, now) {
    ...
    if (ms < HOUR_MS)
        return 'It starts in under an hour.';
    const hours = Math.round(ms / HOUR_MS);
    if (hours === 1)
        return 'It starts in about an hour.';
    if (hours < 24)
        return `It starts in about ${hours} hours.`;
...
exports.COACH_ACCEPTED_ROUTE = '/(client)/my-coach';
/** Where a declined one sends them: the only useful next step. */
exports.COACH_DECLINED_ROUTE = '/(client)/trainers';
/**
 ...
 */
function coachAnswerNotification(accepted, coachName) {
    const who = (coachName ?? '').trim() || 'The coach you asked';
    return accepted
        ? {
            title: 'Your coaching request was accepted',
            body: clip(`${who} has taken you on. Your Coach screen has them now, and anything they set you from here arrives in your app.`, exports.NOTICE_BODY_MAX),
            route: exports.COACH_ACCEPTED_ROUTE,
        }
        : {
            title: 'Your coaching request was declined',
            body: clip(`${who} has declined it. Nothing else on your app has changed, and you can ask a different coach from the directory.`, exports.NOTICE_BODY_MAX),
            route: exports.COACH_DECLINED_ROUTE,
        };
}
/**
 ...
 *                 waiting.
 */
function coachAnswerConfirmation(accepted, clientName, told) {
    ...
    if (told.ok)
        return `${lead} Their phone has been told.`;
    if (told.recorded > 0) {
...
exports.CLASS_OFF_TITLE = 'A class you booked is not running';
/** The plural, for somebody whose whole series went. */
exports.CLASS_OFF_TITLE_MANY = 'Classes you booked are not running';
/** Where a called-off class sends the member. */
exports.CLASS_OFF_ROUTE = '/(client)/classes';
/**
 ...
 * diary.
 */
function classOffNotification(classTitle, classes, reason) {
    const name = (classTitle ?? '').trim() || 'A class';
    const said = (reason ?? '').trim();
    const why = said ? `: ${said}` : '';
    const many = Number.isFinite(classes) && classes > 1;
    return {
        title: many ? exports.CLASS_OFF_TITLE_MANY : exports.CLASS_OFF_TITLE,
        body: clip(many
            ? `${(0, format_1.num)(Math.floor(classes))} of your “${name}” classes have been called off${why}. Your bookings are kept on the record and there is nothing for you to do — your Classes screen has the rest of the timetable.`
            : `“${name}” has been called off${why}. Your booking is kept on the record and there is nothing for you to do — your Classes screen has the rest of the timetable.`, exports.NOTICE_BODY_MAX),
        route: exports.CLASS_OFF_ROUTE,
    };
}
/**
 ...
 */
function classOffConfirmation(cancelled, people, pushed, pushPartial) {
    const n = Math.max(0, Math.floor(Number.isFinite(cancelled) ? cancelled : 0));
    const lead = `${(0, format_1.num)(n)} ${n === 1 ? 'class was' : 'classes were'} called off. Every booking, every check-in and every waiting list is kept.`;
    if (people == null) {
        return `${lead} We couldn’t read who had booked, so nobody has been told — tell them yourself.`;
    }
    if (people === 0)
        return `${lead} Nobody had booked or was waiting, so there was nobody to tell.`;
    const who = `${(0, format_1.num)(people)} ${people === 1 ? 'person had' : 'people had'} booked or ${people === 1 ? 'was' : 'were'} waiting`;
    ...
    if (pushed < people) {
        ...
        if (pushPartial) {
            return `${lead} ${who}. A push was queued to ${(0, format_1.num)(pushed)} of them. ${exports.PUSH_PARTIAL_NOTE} Tell them yourself if the class is soon.`;
        }
        return `${lead} ${who}. A push was queued to ${(0, format_1.num)(pushed)} of them; we couldn’t reach the rest, so tell them yourself if the class is soon.`;
    }
    if (pushPartial) {
        ...
        return `${lead} ${who}, and a push was queued for them. ${exports.PUSH_PARTIAL_NOTE} Tell them yourself if the class is soon.`;
    }
