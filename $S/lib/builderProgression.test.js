...
const builderProgression_1 = require("./builderProgression");
const errors = [];
const ok = (cond, msg) => { if (!cond)
    errors.push(msg); };
const eq = (a, b, msg) => ok(Object.is(a, b), `${msg} — got ${JSON.stringify(a)}, wanted ${JSON.stringify(b)}`);
/** A logged session. `sets` is [reps, weightKg] pairs, as the log stores them. */
const entry = (t, exercise, sets) => ({ t, exercise, sets });
const LOG = [
...
const base = { clientPicked: true, log: LOG, status: 'ready', exercise: 'Bench Press', reps: '8-12' };
/* ── a suggestion, from what they actually did ──────────────────────────── */
const s = (0, builderProgression_1.progressionOffer)({ ...base, unit: 'kg' });
...
const noUnit = (0, builderProgression_1.progressionOffer)(base);
...
eq((0, builderProgression_1.progressionOffer)({ ...base, clientPicked: false }).kind, 'silent', 'with no client picked there is nobody to have a history');
eq((0, builderProgression_1.progressionOffer)({ ...base, log: null, status: 'ready' }).kind, 'silent', 'a read nobody issued draws nothing on every row of the week');
/* ── THE three ways to accuse a client of never having trained ──────────── */
const loading = (0, builderProgression_1.progressionOffer)({ ...base, log: null, status: 'loading' });
...
const failed = (0, builderProgression_1.progressionOffer)({ ...base, log: null, status: 'error' });
...
const truncated = (0, builderProgression_1.progressionOffer)({ ...base, exercise: 'Overhead Press', status: 'partial' });
...
const never = (0, builderProgression_1.progressionOffer)({ ...base, exercise: 'Overhead Press', status: 'ready' });
...
const bodyweight = (0, builderProgression_1.progressionOffer)({ ...base, exercise: 'Press Up', status: 'ready' });
eq(bodyweight.kind, 'gap', 'a bodyweight session offers no load to build from');
if (bodyweight.kind === 'gap') {
    ok(!/have not logged this movement/.test(bodyweight.note), 'but it is NOT reported as a movement they have never done — they did it, with nothing on the bar');
...
const bodyweightPartial = (0, builderProgression_1.progressionOffer)({ ...base, exercise: 'Press Up', status: 'partial' });
if (bodyweightPartial.kind === 'gap') {
    ok(/bodyweight/i.test(bodyweightPartial.note), 'and it outranks the truncation sentence, which would say nothing was logged when something was');
}
...
const partialAnswer = (0, builderProgression_1.progressionOffer)({ ...base, status: 'partial', unit: 'kg' });
...
eq((0, builderProgression_1.loadTapLabel)('62.5 kg'), 'Use 62.5 kg', 'the button says what pressing it will do');
// A button reading "Use —" is worse than no button: it promises a value the
// screen has already admitted it cannot render.
eq((0, builderProgression_1.loadTapLabel)(null), null, 'a load that could not be rendered offers no tap');
eq((0, builderProgression_1.loadTapLabel)(''), null, 'and neither does an empty one');
/* ── and it is not offered for a value already in the box ───────────────── */
eq((0, builderProgression_1.alreadyAt)(62.5, 62.5), true, 'the suggestion already in the box needs no button');
eq((0, builderProgression_1.alreadyAt)(62.5, 62.54), true, 'compared at the resolution the ladder works in');
eq((0, builderProgression_1.alreadyAt)(62.5, 60), false, 'a different load is a different load');
eq((0, builderProgression_1.alreadyAt)(62.5, null), false, 'an empty box is not already at anything');
eq((0, builderProgression_1.alreadyAt)(62.5, undefined), false, 'nor is an absent one');
eq((0, builderProgression_1.alreadyAt)(62.5, NaN), false, 'nor a value that is not a number');
if (errors.length) {
