...
exports.rowToThread = rowToThread;
exports.hasConversation = hasConversation;
exports.sortThreads = sortThreads;
exports.sortUnstarted = sortUnstarted;
exports.splitThreads = splitThreads;
exports.threadPreview = threadPreview;
exports.unreadBadgeLabel = unreadBadgeLabel;
exports.threadWhen = threadWhen;
exports.threadsEmptyNote = threadsEmptyNote;
...
function rowToThread(r) {
    const sender = r?.last_sender === 'client' || r?.last_sender === 'coach' ? r.last_sender : null;
    const kind = r?.last_kind === 'image' || r?.last_kind === 'video' ? r.last_kind : null;
...
const KIND_NOUN = { image: 'a photo', video: 'a video' };
...
function threadWhen(iso, now) {
    ...
    return (0, format_1.fmtAxisDay)(d.getFullYear(), d.getMonth(), d.getDate());
}
/**
 ...
 */
function threadsEmptyNote(status, roster) {
    if (status === 'loading')
        return null;
    if (status === 'error') {
