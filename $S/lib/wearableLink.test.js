...
const wearableLink_1 = require("./wearableLink");
const errors = [];
...
{
    const v = (0, wearableLink_1.describeLink)(facts({ remembered: 'disconnected' }));
...
{
    const v = (0, wearableLink_1.describeLink)(facts({ token: { kind: 'dead', at: NOW, why: 'no-token' } }));
...
{
    const v = (0, wearableLink_1.describeLink)(facts({ token: { kind: 'alive', at: NOW } }));
...
// access and one of them may well be our end.
{
    ...
    for (const token of deaths) {
        const why = token.why;
        const v = (0, wearableLink_1.describeLink)(facts({ token }));
...
{
    const v = (0, wearableLink_1.describeLink)(facts({ remembered: 'connected', token: { kind: 'dead', at: NOW, why: 'refresh-failed' } }));
    ok(v.label !== 'Connected', 'a remembered row must not print "Connected" over a dead token');
    ok(!(0, wearableLink_1.isLinked)(facts({ remembered: 'connected', token: { kind: 'dead', at: NOW, why: 'refresh-failed' } })), 'isLinked must agree with describeLink about a dead token');
}
...
{
    const v = (0, wearableLink_1.describeLink)(facts({
    ...
    ok(!/not connected|no longer connected|disconnected/i.test(v.detail), 'a metric gap must never be worded as the account being disconnected');
...
{
    const v = (0, wearableLink_1.describeLink)(facts({ token: { kind: 'alive', at: NOW }, metric: { name: 'sleep', proof: { kind: 'ok', at: NOW } } }));
    ok(v.state === 'live', 'a metric that read fine must leave the link "live"');
}
// ── the transient ───────────────────────────────────────────────────────────
{
    const v = (0, wearableLink_1.describeLink)(facts({ remembered: 'connecting' }));
...
{
    ok((0, wearableLink_1.classifyRefusal)(null, false).why === 'no-token', 'no reason at all means there is no stored row');
    ok((0, wearableLink_1.classifyRefusal)('', true).level === 'account', 'no reason at all is an account fact even on a live account');
    ok((0, wearableLink_1.classifyRefusal)('expired_no_refresh_token', true).level === 'account', 'an expired grant is an account fact');
    ok((0, wearableLink_1.classifyRefusal)('expired_no_refresh_token', true).why === 'expired-no-refresh', 'the expired-grant reason must survive');
    ok((0, wearableLink_1.classifyRefusal)('refresh_failed', true).why === 'refresh-failed', 'a failed renewal must survive');
    ...
    ok((0, wearableLink_1.classifyRefusal)('whoop_unauthorized', true).level === 'metric', 'one endpoint refusing a token that is otherwise working is a METRIC fact');
    ok((0, wearableLink_1.classifyRefusal)('whoop_unauthorized', false).level === 'account', 'one endpoint refusing a token nothing has proven is all we know: treat it as the account');
    // An unrecognised reason must not be optimistically read as "fine".
    ok((0, wearableLink_1.classifyRefusal)('something_new_from_the_server', true).level === 'account', 'an unrecognised refusal must not be assumed harmless');
}
...
// that the two screens are now saying the same thing about the account.
{
    ...
    const devices = (t, m) => (0, wearableLink_1.describeLink)(facts({ token: t, metric: m }));
    const recovery = (t, m) => (0, wearableLink_1.describeLink)(facts({ token: t, metric: m }));
    // Step 1 — WHOOP signed in, the daily roll-up working, sleep never asked for.
    const alive = { kind: 'alive', at: NOW };
    const refusedSleep = { name: 'sleep', proof: { kind: 'refused', at: NOW } };
    ...
    const dead = { kind: 'dead', at: NOW + 2000, why: 'refresh-failed' };
    ...
    const cleared = { kind: 'none' };
    const grantedSleep = { name: 'sleep', proof: { kind: 'ok', at: NOW + 3000 } };
