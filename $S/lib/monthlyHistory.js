...
function monthWindow(now, count) {
    ...
    for (let i = count - 1; i >= 0; i--) {
        ...
        out.push({ key: (0, exports.monthKey)(d), label: exports.MONTH_LABELS[d.getMonth()] });
    }
    return out;
}
/**
 ...
 */
function seriesFor(window, snapshots) {
    return window.map((m) => {
...
const recordedCount = (series) => series.filter((v) => v != null).length;
exports.recordedCount = recordedCount;
/**
 ...
 */
function historyDelta(series, currentValue) {
    const prev = series[series.length - 2];
    if (prev == null || currentValue == null)
        return 0;
 ...
 * write-back.
 */
function sanitiseSnapshots(raw) {
    const out = {};
 ...
 */
function missingOnServer(local, server) {
    const out = {};
