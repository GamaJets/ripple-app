...
exports.wholeMonths = wholeMonths;
 ...
 */
function wholeMonths(log, truncated) {
    if (!truncated || log.length === 0)
        return { log, droppedMonth: null };
    let oldest = null;
    for (const e of log) {
        const k = (0, longView_1.monthKey)(e.t);
        if (k == null)
            continue;
        if (oldest == null || k < oldest)
            oldest = k;
    }
    if (oldest == null)
        return { log, droppedMonth: null };
    const kept = log.filter((e) => (0, longView_1.monthKey)(e.t) !== oldest);
