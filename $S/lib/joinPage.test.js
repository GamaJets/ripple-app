...
ok(Object.prototype.hasOwnProperty.call(table, brands_1.DEFAULT_BRAND_ID), `the site-brands table has no "${brands_1.DEFAULT_BRAND_ID}" entry, which is the host-not-recognised fallback — a *.pages.dev preview would render with no store links`);
for (const id of ids) {
    const page = table[id];
    const brand = brands_1.BRANDS[id];
    if (!brand) {
    ...
    eq(JSON.stringify(page.hosts), JSON.stringify(brand.linkHosts), `${id}: hosts must be brand.linkHosts, apex then www — these are the hosts the page matches on, and a missing one serves that brand's own domain somebody else's app`);
    ok(page.appleAppId === null || /^[0-9]+$/.test(String(page.appleAppId)), `${id}: appleAppId must be digits, or null for a brand with no listing yet — got ${JSON.stringify(page.appleAppId)}`);
}
// ── the markup a reader with no JavaScript gets ───────────────────────────
const baked = table[brands_1.DEFAULT_BRAND_ID];
if (html && baked) {
    ok(html.includes(`play.google.com/store/apps/details?id=${baked.androidPackage}`), `the static Play link in web/join.html is not ${baked.androidPackage} — with scripting off the page would offer a different app than the table says`);
    if (baked.appleAppId) {
        ok(html.includes(`apps.apple.com/app/id${baked.appleAppId}`), `the static App Store link in web/join.html is not id${baked.appleAppId}`);
    }
    ok(html.includes(`>${baked.clientApp}<`) || html.includes(`on ${baked.clientApp}<`), `web/join.html's baked markup no longer names ${baked.clientApp} — the no-JS default and the table have parted company`);
}
if (errors.length) {
