...
exports.WHAT_EVIDENCE_LOOKS_LIKE = exports.EVIDENCE_GOES_TO_STRIPE = exports.DISPUTE_MONEY_IS_ALREADY_GONE = void 0;
exports.isClosed = isClosed;
exports.needsResponse = needsResponse;
exports.disputeStatusLabel = disputeStatusLabel;
exports.disputeReasonLabel = disputeReasonLabel;
exports.daysToRespond = daysToRespond;
exports.deadlineLine = deadlineLine;
exports.disputeTone = disputeTone;
exports.disputeRow = disputeRow;
 ...
 *  closed. */
const FINISHED = new Set(['won', 'lost', 'warning_closed']);
...
function isClosed(status, closedAt) {
    if (closedAt)
        return true;
 ...
 */
function disputeReasonLabel(reason) {
 ...
 */
function daysToRespond(d, now = Date.now()) {
    if (isClosed(d.status, d.closedAt))
        return null;
 ...
 */
function deadlineLine(d, when, now = Date.now()) {
    if (isClosed(d.status, d.closedAt))
        return null;
    if (!when) {
    ...
    if (ms <= 0) {
        const n = Math.max(1, Math.floor(-ms / 86400000));
        return `The date for evidence was ${when} — about ${n} day${n === 1 ? '' : 's'} ago. Stripe decides on what was submitted by then.`;
    }
    if (ms < 86400000)
        return `Evidence is due within the DAY, by ${when}. After that Stripe decides on what has been submitted.`;
    if (ms < 2 * 86400000)
        return `Evidence is due TOMORROW, ${when}. After that Stripe decides on what has been submitted.`;
 ...
 */
function disputeTone(d, now = Date.now()) {
    if (isClosed(d.status, d.closedAt))
        return null;
    if (!needsResponse(d.status))
        return 'warn';
...
exports.DISPUTE_MONEY_IS_ALREADY_GONE = 'The money on a disputed charge is taken back the moment the dispute opens, not when it is decided. If it goes your way it comes back; if it does not, it stays with the client, and on your own charges Stripe’s dispute fee comes out of your balance as well.';
/** Where the work actually happens, said so nobody waits for a button here that
 *  is never coming. */
exports.EVIDENCE_GOES_TO_STRIPE = 'Evidence is submitted in your own Stripe dashboard. This app is told that a case exists and when it closes; it is never told what you sent, and it cannot send anything for you.';
/** And what to send, because "submit evidence" is not an instruction anybody
 *  can act on at nine in the evening. */
exports.WHAT_EVIDENCE_LOOKS_LIKE = 'Send something even if it feels thin — an empty response loses by default. What you have is usually enough: the sessions you delivered and when, your messages with them, and anything they signed.';
 ...
 */
function disputeRow(d, who, eventAt) {
