 ...
 */
function coverageFor(exerciseNames, videos, coachId, 
...
illustratedSlugs = null) {
    const mineSlugs = new Set();
    const academySlugs = new Set();
    for (const v of videos) {
        const slug = v.exerciseId || (0, exerciseId_1.exerciseSlug)(v.name);
        if (!slug)
            continue;
        if (coachId && v.trainerId === coachId)
            mineSlugs.add(slug);
        else if (v.trainerId == null)
            academySlugs.add(slug);
    }
    ...
    for (const raw of exerciseNames) {
        const slug = (0, exerciseId_1.exerciseSlug)(raw);
