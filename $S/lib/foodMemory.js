...
exports.MAX_FAVOURITES = exports.favouritesKey = void 0;
exports.foodKey = foodKey;
exports.recentFoods = recentFoods;
exports.frequentFoods = frequentFoods;
exports.readFavourites = readFavourites;
exports.writeFavourites = writeFavourites;
 ...
 */
function recentFoods(entries, limit = 8) {
 ...
 */
function frequentFoods(entries, limit = 8, minCount = 2) {
    return [...fold(entries).values()]
...
const favouritesKey = (uid) => `repple.foodFavourites:${uid}`;
exports.favouritesKey = favouritesKey;
...
function readFavourites(raw) {
    ...
    for (const row of parsed) {
        ...
        if (!name || !nums.every((k) => typeof r[k] === 'number' && Number.isFinite(r[k])))
            continue;
        foods.push({
...
function writeFavourites(list) {
    return JSON.stringify(list.slice(0, exports.MAX_FAVOURITES));
}
function isFavourite(list, name) {
 ...
 */
function toggleFavourite(list, food) {
    const k = foodKey(food.name);
    if (!k)
        return [...list];
    if (list.some((f) => f.key === k))
        return list.filter((f) => f.key !== k);
    return [{ ...food, key: k }, ...list.filter((f) => f.key !== k)].slice(0, exports.MAX_FAVOURITES);
}
