...
exports.CATALOGUE_BASE_LOCALE = exports.TRANSLATION_LOCALES = void 0;
exports.isTranslationLocale = isTranslationLocale;
exports.catalogueLocale = catalogueLocale;
exports.indexTranslations = indexTranslations;
exports.displayName = displayName;
exports.displayDescription = displayDescription;
exports.fallbackTag = fallbackTag;
exports.fallbackNote = fallbackNote;
 ...
 */
exports.TRANSLATION_LOCALES = ['de', 'es'];
/** The language `exercises.name` and `exercises.description` are written in. */
exports.CATALOGUE_BASE_LOCALE = 'en';
function isTranslationLocale(x) {
    return typeof x === 'string' && exports.TRANSLATION_LOCALES.includes(x);
}
/**
 ...
 */
function indexTranslations(rows, locale) {
    const out = new Map();
    for (const r of rows || []) {
        if (!r || r.locale !== locale)
            continue;
 ...
 */
function displayName(english, exerciseId, translations, want) {
    const base = clean(english) ?? '';
    if (!want)
        return { text: base, locale: exports.CATALOGUE_BASE_LOCALE, isFallback: false };
    const hit = clean(translations?.get(exerciseId)?.name ?? null);
    if (hit)
        return { text: hit, locale: want, isFallback: false };
    return { text: base, locale: exports.CATALOGUE_BASE_LOCALE, isFallback: true };
}
/**
 ...
 */
function displayDescription(english, exerciseId, translations, want) {
    const base = clean(english);
    if (!want)
        return base == null ? null : { text: base, locale: exports.CATALOGUE_BASE_LOCALE, isFallback: false };
    const hit = clean(translations?.get(exerciseId)?.description ?? null);
    if (hit)
        return { text: hit, locale: want, isFallback: false };
    return base == null ? null : { text: base, locale: exports.CATALOGUE_BASE_LOCALE, isFallback: true };
}
/**
 ...
 */
function fallbackNote(name, description) {
    const both = name.isFallback && !!description?.isFallback;
    if (both)
        return 'This movement has not been translated yet, so its name and description are shown in English.';
    if (name.isFallback)
        return 'This movement has not been translated yet, so its name is shown in English.';
    if (description?.isFallback)
        return 'This movement has a translated name, but its description has not been translated yet and is shown in English.';
    return null;
}
/**
 ...
 */
function matchesSearch(term, english, display) {
 ...
 */
function validateTranslations(rows, knownExerciseIds) {
    const problems = [];
    const seen = new Set();
    for (const r of rows) {
        const id = clean(r?.exerciseId) ?? '';
        ...
        if (!isTranslationLocale(loc)) {
            problems.push(`"${id || '(no exercise)'}" is translated into "${loc || '(nothing)'}", which is not a catalogue language. `
                + `The supported set is ${exports.TRANSLATION_LOCALES.join(', ')} — a row in any other locale is stored, read by nothing, `
        ...
        if (!knownExerciseIds.has(id)) {
            problems.push(`"${id}" is translated into "${loc}" and there is no such exercise in the catalogue. `
                + 'The foreign key would reject this when the part is applied, half way through the seed.');
        }
        const key = `${id}\u0000${loc}`;
        if (seen.has(key)) {
            problems.push(`"${id}" is translated into "${loc}" twice in the same set. `
        ...
        if (clean(r?.name) == null && clean(r?.description) == null) {
            problems.push(`"${id}" has a "${loc}" row that translates neither the name nor the description. `
