...
exports.DEFAULT_WINDOW_DAYS = exports.PART_COST = exports.PART_LABEL = exports.PART_ORDER = exports.slicePartial = exports.sliceFailed = exports.sliceReady = exports.sliceLoading = void 0;
exports.sliceCapped = sliceCapped;
exports.rowsOf = rowsOf;
exports.rowsToShow = rowsToShow;
exports.isWholeSlice = isWholeSlice;
exports.truncationNote = truncationNote;
exports.sliceNote = sliceNote;
exports.sliceDash = sliceDash;
exports.brokenParts = brokenParts;
exports.pendingParts = pendingParts;
exports.truncatedParts = truncatedParts;
exports.completeness = completeness;
exports.partialWarning = partialWarning;
exports.truncationWarning = truncationWarning;
exports.memberIds = memberIds;
exports.buildDossier = buildDossier;
exports.buildDossiers = buildDossiers;
exports.retentionRead = retentionRead;
exports.doorLogActive = doorLogActive;
exports.attendanceCaveat = attendanceCaveat;
const gymSessions_1 = require("./gymSessions");
const gymPasses_1 = require("./gymPasses");
const sliceLoading = () => ({ state: 'loading' });
exports.sliceLoading = sliceLoading;
const sliceReady = (rows) => ({ state: 'ready', rows });
exports.sliceReady = sliceReady;
const sliceFailed = (reason) => ({ state: 'failed', reason });
exports.sliceFailed = sliceFailed;
/**
 ...
 */
const slicePartial = (rows, cap) => ({ state: 'partial', rows: rows.length > cap ? rows.slice(0, cap) : rows, cap });
exports.slicePartial = slicePartial;
/**
 ...
 * it meant.
 */
function truncationNote(what, cap) {
...
function brokenParts(rec) {
    const out = [];
    for (const part of exports.PART_ORDER) {
        const s = rec[part];
        if (s.state === 'failed') {
            out.push({ part, label: exports.PART_LABEL[part], cost: exports.PART_COST[part], reason: s.reason });
        }
    }
    return out;
}
/** The parts still in flight. */
function pendingParts(rec) {
    return exports.PART_ORDER.filter((p) => rec[p].state === 'loading');
}
/** The parts that came back, and came back short. */
function truncatedParts(rec) {
    return exports.PART_ORDER.filter((p) => rec[p].state === 'partial');
}
/**
...
function completeness(rec) {
    if (brokenParts(rec).length)
        return 'broken';
    if (pendingParts(rec).length)
        return 'loading';
 ...
 */
function truncationWarning(rec) {
    const cut = truncatedParts(rec);
    if (!cut.length)
        return null;
    const names = cut.map((p) => exports.PART_LABEL[p]);
    const list = names.length === 1
        ? names[0]
        : `${names.slice(0, -1).join(', ')} and ${names[names.length - 1]}`;
    const costs = cut.map((p) => exports.PART_COST[p]).join('; ');
...
function buildDossier(memberId, rec, now = Date.now()) {
    ...
    return {
        ...
        unmarked: sess ? sess.filter((s) => (0, gymSessions_1.isAwaitingOutcome)(s, now)).length : null,
        passes: pss,
        ...
        passVisitsLeft: pss ? pss.filter((p) => p.covers !== 'pt').reduce((a, p) => a + (0, gymPasses_1.remainingUses)(p), 0) : null,
        ptCreditsLeft: pss ? pss.filter((p) => p.covers === 'pt').reduce((a, p) => a + (0, gymPasses_1.remainingUses)(p), 0) : null,
...
function buildDossiers(rec, now = Date.now()) {
...
exports.DEFAULT_WINDOW_DAYS = 56;
/**
 ...
 */
function retentionRead(d, opts) {
    const now = opts.now ?? Date.now();
    const windowDays = opts.windowDays ?? exports.DEFAULT_WINDOW_DAYS;
    const halfDays = windowDays / 2;
    const half = halfDays * 86400000;
    const midpoint = now - half;
    const start = now - 2 * half;
    const inRecent = (iso) => within(iso, midpoint, now);
    const inEarlier = (iso) => within(iso, start, midpoint);
    ...
    const stillTrainingOffTheTimetable = d.bookings != null && d.visits != null &&
    ...
    const absentFromLiveDoorLog = d.visits != null && opts.doorLogActive && recentVisits === 0 && expected;
    const absenceUnknownBecause = d.status === 'frozen' ? 'frozen'
 ...
 */
function attendanceCaveat(rec) {
    const active = doorLogActive(rec);
    if (active === null)
        return null; // the failure banner already says this
    if (active)
        return null;
...
/* ── helpers ───────────────────────────────────────────────────────────────── */
function pick(rows, keep) {
    return rows == null ? null : rows.filter(keep);
}
function trend(recent, earlier) {
...
function nameFor(memberId, ms, vis, pss) {
    for (const m of ms ?? [])
        if (m.memberName?.trim())
            return m.memberName.trim();
    for (const v of vis ?? [])
        if (v.memberName?.trim())
            return v.memberName.trim();
    for (const p of pss ?? [])
