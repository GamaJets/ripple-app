...
exports.MAX_NUDGE_COOLDOWN = exports.MIN_NUDGE_COOLDOWN = exports.goalPct = exports.goalText = void 0;
exports.parseRate = parseRate;
exports.rateText = rateText;
exports.payEstimate = payEstimate;
exports.parseGoal = parseGoal;
exports.goalsEmptyLine = goalsEmptyLine;
exports.goalSaveLine = goalSaveLine;
exports.rateFieldNote = rateFieldNote;
exports.parseCooldown = parseCooldown;
exports.cooldownText = cooldownText;
exports.cooldownNote = cooldownNote;
/**
 * Read a typed rate.
 *
 ...
 */
function payEstimate(rate, present) {
    if (rate == null || present == null)
        return null;
    if (!Number.isFinite(rate) || !Number.isFinite(present))
        return null;
...
const goalText = (value) => (value > 0 ? String(value) : '');
exports.goalText = goalText;
/** Clamp a progress fraction to 0..1. A goal of 0 has no progress to draw. */
const goalPct = (current, goal) => goal > 0 ? Math.max(0, Math.min(1, current / goal)) : 0;
 ...
 * themselves.
 */
function goalsEmptyLine(status, revenue, clients) {
    if (revenue > 0 || clients > 0)
        return null;
    if (status === 'loading')
        return 'Reading your targets…';
    if (status === 'error') {
 ...
 */
function rateFieldNote(status) {
    if (status === 'loading')
        return 'Reading the rate you saved…';
    if (status === 'error') {
 ...
 */
exports.MIN_NUDGE_COOLDOWN = 1;
exports.MAX_NUDGE_COOLDOWN = 365;
function parseCooldown(text) {
    ...
    if (!Number.isSafeInteger(n))
        return { kind: 'invalid' };
    if (n < exports.MIN_NUDGE_COOLDOWN || n > exports.MAX_NUDGE_COOLDOWN)
        return { kind: 'invalid' };
    return { kind: 'value', value: n };
}
