...
const sent = (0, coachShare_1.clientAskContext)(CLIENT);
...
for (const k of coachShare_1.COACH_CLIENT_HEALTH) {
...
const withNew = (0, coachShare_1.clientAskContext)({ ...CLIENT, ...INVENTED });
...
const sparse = (0, coachShare_1.clientAskContext)({ goal: 'Get stronger' });
...
const biz = (0, coachShare_1.businessAskContext)(BUSINESS);
...
eq(biz.takenThisMonth, 'AED 12,400.00', 'the takings figure the digest is told to quote exactly actually reaches the model');
eq(biz.sessionsStillUnmarked, 9, 'and the unmarked count, without which "congratulations on a quiet month" is the answer to nine unrecorded sessions');
eq(biz.howTheyCoach, 'entirely online', 'and how they coach, without which the rule about not suggesting anything needing a room can never fire');
...
for (const k of ['takenThisMonth', 'sessionsStillUnmarked', 'howTheyCoach']) {
    ok(!coachShare_1.COACH_CLIENT_KEYS.includes(k), `${k} is a fact about the coach's own book, not about a client`);
}
...
for (const k of coachShare_1.COACH_CLIENT_HEALTH) {
    ok(!coachShare_1.COACH_BUSINESS_KEYS.includes(k), `${k} is not smuggled into the business allowlist`);
}
ok(!coachShare_1.COACH_BUSINESS_KEYS.includes('name'), 'and neither is a name');
ok(!coachShare_1.COACH_CLIENT_KEYS.includes('name'), 'the client allowlist does not name anybody either');
/* ── the name comes back locally ───────────────────────────────────────── */
eq((0, coachShare_1.fillName)('Hey {name}, good week?', 'Ana Ferreira'), 'Hey Ana, good week?', 'the reply says {name} and the screen fills it in — which is where the name was always going to end up');
eq((0, coachShare_1.fillName)('Hey {name} — {coach}', 'Ana Ferreira', 'Sam Doyle'), 'Hey Ana — Sam', 'and the coach token too, first names both');
// An unknown name leaves the token VISIBLE. "Hey {name}" is obviously
// unfinished and gets fixed; "Hey ," gets sent.
eq((0, coachShare_1.fillName)('Hey {name}', null), 'Hey {name}', 'an unknown name leaves the placeholder rather than a blank');
eq((0, coachShare_1.fillName)('Hey {name}', '   '), 'Hey {name}', 'whitespace is not a name');
eq((0, coachShare_1.fillName)('Hey {name} and {name}', 'Ana'), 'Hey Ana and Ana', 'every occurrence is filled, not just the first');
/* ── the redaction ─────────────────────────────────────────────────────── */
eq((0, coachShare_1.sharedAreas)([]), '', 'no disclosures is an empty string, so the caller writes its own sentence');
eq((0, coachShare_1.sharedAreas)([{ area: 'knee', severity: 'moderate' }]), 'Knee (moderate)', 'area and severity, and nothing else');
ok(!(0, coachShare_1.sharedAreas)([{ area: 'knee', severity: 'moderate' }]).includes(','), 'no note is appended, whatever else the row carries');
...
const injs = [
    { id: '1', area: 'knee', severity: 'moderate', status: 'active', at: '2026-01-01', note: 'grade 2 MCL sprain per MRI' },
    { id: '2', area: 'shoulder', severity: 'mild', status: 'recovered', at: '2025-06-01' },
];
eq((0, coachShare_1.sharedInjuries)(injs), 'Knee (moderate)', 'recovered disclosures are left out and the note still never travels');
/* ── what the screen tells the coach ───────────────────────────────────── */
ok(/name/i.test(coachShare_1.COACH_ASK_WHAT_NEVER_GOES), 'the coach is told the name does not go');
ok(/\{name\}/.test(coachShare_1.COACH_ASK_WHAT_NEVER_GOES), 'and that the placeholder is why replies read the way they do');
ok(/injur/i.test(coachShare_1.COACH_ASK_WHAT_GOES), 'and what does go includes the injury areas, because that is the part that changes an answer');
ok(/document/i.test(coachShare_1.COACH_ASK_WHAT_NEVER_GOES), 'and that nothing off a document travels');
ok(coachShare_1.COACH_ASK_NOT_ADVICE.length > 40, 'and that it has not met anybody');
ok(coachShare_1.NAME_TOKEN === '{name}', 'one placeholder convention for the whole coach app');
if (errors.length) {
