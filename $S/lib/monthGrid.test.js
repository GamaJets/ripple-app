...
ok((0, monthGrid_1.isLeapYear)(2024), '2024 is a leap year');
ok(!(0, monthGrid_1.isLeapYear)(2023), '2023 is not');
ok((0, monthGrid_1.isLeapYear)(2000), '2000 is a leap year — divisible by 400');
ok(!(0, monthGrid_1.isLeapYear)(1900), '1900 is not — divisible by 100 and not by 400');
ok(!(0, monthGrid_1.isLeapYear)(2100), 'nor is 2100');
/* ── February, both ways ──────────────────────────────────────────────────── */
eq((0, monthGrid_1.daysInMonth)(2024, 1), 29, 'February in a leap year');
eq((0, monthGrid_1.daysInMonth)(2023, 1), 28, 'February in a non-leap year');
eq((0, monthGrid_1.daysInMonth)(2000, 1), 29, 'February 2000');
eq((0, monthGrid_1.daysInMonth)(1900, 1), 28, 'February 1900');
// A leap February fills exactly four columns more than a common one, which is
// the property the grid actually depends on.
eq((0, monthGrid_1.monthGrid)(2024, 1).days - (0, monthGrid_1.monthGrid)(2023, 1).days, 1, 'the leap day is one extra cell and not a whole extra row of them');
ok((0, monthGrid_1.monthGrid)(2024, 1).cells.filter((c) => c === 29).length === 1, 'a leap February draws a 29 exactly once');
ok((0, monthGrid_1.monthGrid)(2023, 1).cells.every((c) => c !== 29), 'a common February draws no 29 at all');
/* ── month lengths, checked by what they add up to ────────────────────────── */
for (const year of [1999, 2000, 2023, 2024, 2026, 2027, 2100]) {
    let total = 0;
    for (let m = 0; m < 12; m++) {
        const n = (0, monthGrid_1.daysInMonth)(year, m);
        ok(n >= 28 && n <= 31, `${year}-${m}: a month is 28 to 31 days, got ${n}`);
        total += n;
    }
    eq(total, (0, monthGrid_1.isLeapYear)(year) ? 366 : 365, `${year}: the twelve months make the year`);
}
...
eq((0, monthGrid_1.weekdayOf)(2023, 0, 1), 0, '1 January 2023 was a Sunday');
...
{
    const walk = (y, m, d, days, what) => {
        let prev = (0, monthGrid_1.weekdayOf)(y, m, d);
        let cy = y, cm = m, cd = d;
        for (let i = 0; i < days; i++) {
            cd += 1;
            if (cd > (0, monthGrid_1.daysInMonth)(cy, cm)) {
                cd = 1;
                const n = (0, monthGrid_1.stepMonth)(cy, cm, 1);
                cy = n.year;
                cm = n.month;
            }
            const now = (0, monthGrid_1.weekdayOf)(cy, cm, cd);
...
for (let year = 1999; year <= 2032; year++) {
    for (let month = 0; month < 12; month++) {
        const g = (0, monthGrid_1.monthGrid)(year, month);
        ...
        const days = g.cells.filter((c) => c != null);
        eq(days.length, g.days, `${where}: as many days as the month has`);
        eq(days.length, (0, monthGrid_1.daysInMonth)(year, month), `${where}: and it agrees with daysInMonth`);
        ...
        for (let i = 0; i < g.cells.length; i++) {
            ...
            if (d == null)
                continue;
            eq(i % 7, (0, weekStart_1.dayIndexInWeek)((0, monthGrid_1.weekdayOf)(year, month, d)), `${where}: day ${d} is in its own weekday's column`);
        }
    }
}
...
eq((0, weekStart_1.jsDayForIndex)(0), weekStart_1.WEEK_STARTS_ON, 'column zero is the day the week starts on');
{
    ...
    for (let year = 2020; year <= 2030; year++) {
        for (let month = 0; month < 12; month++) {
            const g = (0, monthGrid_1.monthGrid)(year, month);
            const idx = (0, weekStart_1.dayIndexInWeek)((0, monthGrid_1.weekdayOf)(year, month, 1));
            if (idx === 0) {
                openers++;
                eq(g.leading, 0, `${year}-${month + 1} opens the week: no leading blanks`);
            }
            if (idx === 1) {
                seconds++;
                eq(g.leading, 1, `${year}-${month + 1} opens one day in: one leading blank`);
            }
        }
    }
...
/* ── rows ─────────────────────────────────────────────────────────────────── */
{
    const g = (0, monthGrid_1.monthGrid)(2026, 8);
    const rows = (0, monthGrid_1.gridRows)(g);
...
{
    const n = (0, monthGrid_1.stepMonth)(2026, 11, 1);
...
{
    const p = (0, monthGrid_1.stepMonth)(2026, 0, -1);
...
for (let month = 0; month < 12; month++) {
    ...
    for (const delta of [1, 5, 11, 13, -1, -7, -25]) {
        const there = (0, monthGrid_1.stepMonth)(2026, month, delta);
        const back = (0, monthGrid_1.stepMonth)(there.year, there.month, -delta);
...
eq((0, monthGrid_1.normalMonth)(2026, 12).year, 2027, 'month 12 of 2026 is 2027');
eq((0, monthGrid_1.normalMonth)(2026, 12).month, 0, 'and is January');
eq((0, monthGrid_1.normalMonth)(2026, -1).year, 2025, 'month -1 of 2026 is 2025');
eq((0, monthGrid_1.normalMonth)(2026, -1).month, 11, 'and is December');
eq((0, monthGrid_1.normalMonth)(2026, 5).month, 5, 'an in-range month is left alone');
...
// run, and it is where a `month + 1 > 11` written by hand goes wrong.
{
    ...
    for (let i = 0; i < 12; i++) {
        seen.add(`${y}-${m}`);
        const n = (0, monthGrid_1.stepMonth)(y, m, 1);
        y = n.year;
        m = n.month;
    }
...
eq((0, monthGrid_1.isoFromParts)(2026, 8, 7), '2026-09-07', 'month index is one-based on the wire');
eq((0, monthGrid_1.isoFromParts)(2026, 0, 1), '2026-01-01', 'and both halves are padded');
eq((0, monthGrid_1.isoFromParts)(2026, 11, 31), '2026-12-31', 'and December is 12');
...
for (let month = 0; month < 12; month++) {
    const g = (0, monthGrid_1.monthGrid)(2024, month);
    for (const d of g.cells) {
        if (d == null)
            continue;
        const iso = (0, monthGrid_1.isoFromParts)(2024, month, d);
        ok((0, programStart_1.isStartDate)(iso), `${iso} is a date this app will store`);
...
{
    const now = new Date();
    const [y, m, d] = (0, monthGrid_1.todayParts)(now);
    ...
    const g = (0, monthGrid_1.monthGrid)(y, m);
    ...
    const iso = (0, monthGrid_1.isoFromParts)(y, m, d);
    eq(iso, `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')}`, "today's cell writes the reader's own calendar day");
    ok((0, programStart_1.isStartDate)(iso), 'and it is a date this app will store');
}
...
// one the READER is in, which is what the local getters say.
{
    ...
    const [y, m, d] = (0, monthGrid_1.todayParts)(at);
...
/* ── which month the sheet opens on ───────────────────────────────────────── */
{
    ...
    eq((0, monthGrid_1.openMonth)('2027-03-02', now).year, 2027, 'a typed date opens its own year');
    eq((0, monthGrid_1.openMonth)('2027-03-02', now).month, 2, 'and its own month');
    eq((0, monthGrid_1.openMonth)('', now).year, 2026, 'an empty field opens the reader\'s year');
    eq((0, monthGrid_1.openMonth)('', now).month, 8, 'and the reader\'s month');
    eq((0, monthGrid_1.openMonth)(null, now).month, 8, 'so does null');
    eq((0, monthGrid_1.openMonth)(undefined, now).month, 8, 'and undefined');
    eq((0, monthGrid_1.openMonth)('2026-0', now).month, 8, 'a half-typed date falls back rather than throwing');
    eq((0, monthGrid_1.openMonth)('next tuesday', now).month, 8, 'and so does prose');
    ...
    ok(!(0, programStart_1.isStartDate)('2026-02-30'), 'the 30th of February is not a storable date');
    ok(Number.isFinite((0, monthGrid_1.openMonth)('2026-02-30', now).month), 'but it still opens a month');
}
...
{
    const a = (0, monthGrid_1.monthGrid)(2026, 1);
    const b = (0, monthGrid_1.monthGrid)(2026, 1);
