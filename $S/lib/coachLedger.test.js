...
const EMPTY = { pots: [], unlabelled: 0, unpriced: 0 };
const taken = (...rows) => (0, coachMoney_1.sumTaken)(rows.map((r) => ({ ...r, created_at: '2026-09-01T00:00:00Z' })));
const strand = (key, label, status, t = EMPTY) => ({ key, label, status, taken: t });
/* ── 1. a total exists only when every read was whole ─────────────────────── */
{
    ...
    const whole = (0, coachLedger_1.ledger)([
    ...
    const half = (0, coachLedger_1.ledger)([
        strand('p', 'one-off sales', 'ready', sales),
    ...
    ok(/not a statement that you were paid nothing/.test(half.reason ?? ''), 'and says out loud that an empty figure is not a statement of having been paid nothing');
    ...
    const capped = (0, coachLedger_1.ledger)([
    ...
    const flight = (0, coachLedger_1.ledger)([
    ...
    const none = (0, coachLedger_1.ledger)([]);
...
{
    const l = (0, coachLedger_1.ledger)([
...
eq((0, coachLedger_1.joinLabels)([]), 'nothing', 'no labels is the word nothing, never an empty string mid-sentence');
eq((0, coachLedger_1.joinLabels)(['renewals']), 'renewals', 'one label stands alone');
eq((0, coachLedger_1.joinLabels)(['sales', 'renewals']), 'sales and renewals', 'two are joined with and');
eq((0, coachLedger_1.joinLabels)(['sales', 'renewals', 'fees']), 'sales, renewals and fees', 'three take commas then and');
/* ── 4. whole units are not cents ─────────────────────────────────────────── */
{
    ...
    const s = (0, coachLedger_1.sumMajor)([{ amount: 40, currency: 'AED' }, { amount: 25, currency: 'AED' }]);
    ...
    const mixed = (0, coachLedger_1.sumMajor)([{ amount: 40, currency: 'AED' }, { amount: 25, currency: 'GBP' }]);
    ...
    const tied = (0, coachLedger_1.sumMajor)([{ amount: 30, currency: 'GBP' }, { amount: 30, currency: 'AED' }]);
    eq(tied.pots.map((p) => p.currency).join(','), 'AED,GBP', 'equal pots sort by currency code, not by arrival');
    eq((0, coachLedger_1.sumMajor)([{ amount: 30, currency: 'AED' }, { amount: 30, currency: 'GBP' }]).pots.map((p) => p.currency).join(','), 'AED,GBP', 'and reversing the input does not reverse the answer');
    ...
    eq((0, coachLedger_1.sumMajor)([{ amount: 10, currency: 'aed' }, { amount: 5, currency: ' AED ' }]).pots.length, 1, 'case and padding do not fork a currency into two pots');
    const holes = (0, coachLedger_1.sumMajor)([
    ...
    eq((0, coachLedger_1.sumMajor)([]).pots.length, 0, 'no fees is no pots');
    ...
    ok((0, coachLedger_1.sumMajor)([{ amount: 0, currency: 'AED' }]).pots[0].units === 0, 'a recorded fee of zero is a pot of zero, not an unpriced row');
}
/* ── 5. no spend recorded is not zero spend ───────────────────────────────── */
{
    const s = (0, coachLedger_1.sumSpend)([
    ...
    const zero = (0, coachLedger_1.sumSpend)([{ spend: { cents: 0, currency: 'GBP' } }]);
    ...
    const noCode = (0, coachLedger_1.sumSpend)([{ spend: { cents: 500, currency: '' } }]);
    ...
    eq((0, coachLedger_1.sumSpend)([]).unrecorded, 0, 'no codes is no unrecorded codes');
    // Two currencies of ad spend is two currencies of ad spend.
    const two = (0, coachLedger_1.sumSpend)([
    ...
    const tied = (0, coachLedger_1.sumSpend)([
...
{
    const set = (0, coachLedger_1.denominate)('aed', 'ready');
    ...
    const unset = (0, coachLedger_1.denominate)(null, 'ready');
    ...
    const unread = (0, coachLedger_1.denominate)('AED', 'error');
    ...
    ok(unread.ok === false && !/gym owner/.test(unread.note), 'and does NOT send the coach to a settings screen where nothing is wrong');
    eq((0, coachLedger_1.denominate)('', 'ready').ok, false, 'an empty string is the same silence as a null');
    eq((0, coachLedger_1.denominate)('  ', 'ready').ok, false, 'and so is whitespace');
    eq((0, coachLedger_1.denominate)('A', 'ready').ok, false, 'a code too short to be ISO 4217 is not a currency');
    ...
    eq((0, coachLedger_1.denominate)(null, 'loading').ok, false, 'nothing is denominated while the read is in flight');
}
/* ── 7. an empty ledger says different things under different reads ───────── */
{
    const inReady = (0, coachLedger_1.ledgerEmptyLine)('in', 'ready');
    const inError = (0, coachLedger_1.ledgerEmptyLine)('in', 'error');
    ...
    ok(/not because nobody has paid you/.test(inError), 'it says the opposite explicitly, because the confident zero is the expensive sentence');
    ...
    ok(/cash and transfers do not/.test(inReady), 'the honest empty says what this figure does not include');
    const outError = (0, coachLedger_1.ledgerEmptyLine)('out', 'error');
    ...
    ok(/Still reading/.test((0, coachLedger_1.ledgerEmptyLine)('in', 'loading')), 'loading says loading');
    ok(/one request/.test((0, coachLedger_1.ledgerEmptyLine)('in', 'partial')), 'truncation says truncation');
}
/* ── 8. the standing notes, which are the point of the screen ─────────────── */
{
    ...
    ok(/never subtracted/.test(coachLedger_1.NO_NET_NOTE), 'the no-net note says the two are never subtracted');
    ok(/net figure/.test(coachLedger_1.NO_NET_NOTE), 'and names the thing it is refusing to compute');
    // Gross, and said so. Stripe is the authority on money that moved.
    ok(/before Stripe/.test(coachLedger_1.STRIPE_AUTHORITY_NOTE), 'takings are stated as pre-fee');
    ok(/Stripe dashboard/.test(coachLedger_1.STRIPE_AUTHORITY_NOTE), 'and the record of what landed is named');
    ok(!/earn(ings|ed)\b/.test(coachLedger_1.STRIPE_AUTHORITY_NOTE), 'the word earnings is not used for a gross charge');
    // A period figure names what it counts, or two readers read it two ways.
    ok(/date the money was charged/.test(coachLedger_1.PERIOD_NOTE), 'the period note says which date decides the month');
    ok(/record was written/.test(coachLedger_1.PERIOD_NOTE), 'and which one does not');
    // Prose is sentence case; these are notes, not labels.
    for (const [name, s] of [['NO_NET_NOTE', coachLedger_1.NO_NET_NOTE], ['STRIPE_AUTHORITY_NOTE', coachLedger_1.STRIPE_AUTHORITY_NOTE], ['PERIOD_NOTE', coachLedger_1.PERIOD_NOTE]]) {
