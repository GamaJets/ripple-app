...
exports.END_AT_PERIOD_IS_KINDER = exports.END_NOW_TAKES_THE_REST = exports.REFUND_IS_FINAL = exports.REFUND_FEES_NOTE = exports.REFUND_DOES_NOT = exports.REFUND_PART_IS_EXACT = void 0;
exports.refundableRow = refundableRow;
exports.refundableCents = refundableCents;
exports.isPartlyRefunded = isPartlyRefunded;
exports.isFullyRefunded = isFullyRefunded;
exports.refundBlocker = refundBlocker;
exports.refundAmountBlocker = refundAmountBlocker;
exports.refundConfirmLine = refundConfirmLine;
exports.refundBalanceNote = refundBalanceNote;
 ...
 */
function refundableRow(kind, row, stripeRef) {
...
function isPartlyRefunded(r) {
    const left = refundableCents(r);
    const already = Number.isFinite(r.refundedCents) ? Math.max(0, r.refundedCents) : 0;
    return already > 0 && left > 0;
}
function isFullyRefunded(r) {
    const gross = Number(r.amountCents);
    if (!Number.isFinite(gross) || gross <= 0)
        return false;
    const already = Number.isFinite(r.refundedCents) ? Math.max(0, r.refundedCents) : 0;
 ...
 */
function refundAmountBlocker(minorUnits, max) {
 ...
 */
function refundConfirmLine(amount, who, part) {
    return part
 ...
 */
exports.REFUND_PART_IS_EXACT = 'The amount you type is the amount that goes back, exactly. Nothing is rounded, and nothing is added to it or taken off it.';
/**
 ...
 */
function refundBalanceNote(model) {
    if (model === 'direct') {
 ...
 */
exports.REFUND_DOES_NOT = 'A refund gives money back and does nothing else. It does not cancel a subscription, it does not put a session credit back on a pack, and it does not tell your client anything — Stripe emails them a refund receipt and the rest is yours to say.';
/**
 ...
 */
exports.REFUND_FEES_NOTE = 'Repple’s share of the original charge comes back with the refund, in proportion. What Stripe charged to process the payment in the first place is Stripe’s and is usually not returned, so a sale refunded in full can still leave you short by that. Your Stripe dashboard is the record of what actually moved.';
/**
 ...
 */
exports.REFUND_IS_FINAL = 'A refund cannot be taken back. If it was a mistake, the only way round it is for them to pay again, which means a new checkout at today’s price.';
/**
 ...
 */
exports.END_NOW_TAKES_THE_REST = 'Ending it now stops the coaching immediately and returns nothing. They have paid for the period they are in, and this takes the rest of it off them without giving the money back — refund it separately if that is what you mean.';
 ...
 *  coach reads first. */
exports.END_AT_PERIOD_IS_KINDER = 'Stopping at the end of the period lets them use what they have already paid for, and nothing is charged again after that. It is the right answer nearly always, and it can be undone until the day it ends.';
