...
exports.UNDO_WINDOW_MS = void 0;
exports.stage = stage;
exports.isDue = isDue;
exports.remainingMs = remainingMs;
exports.due = due;
exports.undo = undo;
exports.hiddenIds = hiddenIds;
/**
 ...
 */
function stage(queue, next, _nowMs) {
...
function isDue(u, nowMs, windowMs = exports.UNDO_WINDOW_MS) {
    if (!Number.isFinite(u.atMs) || !Number.isFinite(nowMs))
        return true;
    return nowMs - u.atMs >= windowMs;
}
/** How much of the window is left, in whole milliseconds. Never negative. */
function remainingMs(u, nowMs, windowMs = exports.UNDO_WINDOW_MS) {
    if (!Number.isFinite(u.atMs) || !Number.isFinite(nowMs))
        return 0;
    return Math.max(0, u.atMs + windowMs - nowMs);
}
/** Split a queue into what must be written now and what is still takeable back. */
function due(queue, nowMs, windowMs = exports.UNDO_WINDOW_MS) {
    const commit = [];
    const kept = [];
    for (const q of queue)
        (isDue(q, nowMs, windowMs) ? commit : kept).push(q);
