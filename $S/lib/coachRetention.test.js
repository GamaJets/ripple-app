...
ok(interventions_1.FOLLOW_UP_CONTACT_DAYS + clientDrift_1.DEFAULT_WINDOWS.historyDays <= interventions_1.FOLLOW_UP_READ_DAYS, 'every contact a screen offers for judgement has its own baseline inside the read');
ok(interventions_1.FOLLOW_UP_CONTACT_DAYS > clientDrift_1.DEFAULT_WINDOWS.historyDays, 'and the contact window reaches past the drift window, or the loop only ever sees calls too recent to judge');
/* ── a client who came back ───────────────────────────────────────────────
 *
 * Three sessions a week for two months, a drop to nothing, a phone call, and
 * three sessions a week again. */
const twice = (fromDays, toDays, perWeek) => {
    const out = [];
    for (let d = fromDays; d > toDays; d--) {
        ...
        if (d % 7 < perWeek)
            out.push({ at: ago(d), kind: 'workout' });
    }
    return out;
};
const CALL = 40;
...
const contact = (id, daysAgo) => ({
    id, memberId: 'm1', at: ago(daysAgo), channel: 'call',
    byId: null, byName: 'Front desk', outcome: 'reached', note: null,
});
const one = (0, interventions_1.assessAllFollowUps)([contact('c1', CALL)], () => recovered, {
    now: NOW, readFromMs: NOW - interventions_1.FOLLOW_UP_READ_DAYS * DAY,
});
 ...
 * about a person. */
const tooOld = (0, interventions_1.assessAllFollowUps)([contact('c2', interventions_1.FOLLOW_UP_READ_DAYS + 30)], () => recovered, {
    now: NOW, readFromMs: NOW - interventions_1.FOLLOW_UP_READ_DAYS * DAY,
});
...
eq((0, interventions_1.summariseFollowUps)(null), null, 'an unread contact list produces no tally at all');
eq((0, interventions_1.loopHeadline)(null), null, 'and no headline to put over it');
eq((0, interventions_1.loopHeadline)((0, interventions_1.summariseFollowUps)([])), null, 'nor does an empty one: a coach who has made no calls does not need a report about them');
/* ── counts, never a rate ─────────────────────────────────────────────────
 *
...
const tally = (0, interventions_1.summariseFollowUps)([...one, ...tooOld]);
...
const head = (0, interventions_1.loopHeadline)(tally);
...
ok(!/worked|failed|caused/i.test(head), 'nothing here claims a contact worked: everybody contacted was contacted because they were drifting, so there is no comparable group who were left alone');
ok(/followed/i.test(interventions_1.WHY_NO_RATE) && !/%/.test(interventions_1.WHY_NO_RATE.replace(/percentage/g, '')), 'and the sentence the screen prints beside the counts says so in as many words');
if (errors.length) {
