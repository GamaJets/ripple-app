...
exports.closeBlocker = closeBlocker;
exports.closedMonthBlocker = closedMonthBlocker;
exports.reopenBlocker = reopenBlocker;
exports.fetchCloses = fetchCloses;
exports.liveCloseFor = liveCloseFor;
exports.driftSince = driftSince;
 ...
 */
function snapshotOf(c, currency) {
 ...
 */
function closeBlocker(monthKey, ended, existing) {
 ...
 */
function closedMonthBlocker(day, rows) {
    if (rows == null)
        return null;
 ...
 */
async function fetchCloses(sb, tenantId) {
...
function liveCloseFor(monthKey, rows) {
 ...
 * was closed.
 */
function driftSince(stored, now, fmt) {
    const out = [];
    const check = (label, was, is) => {
        if (was === is)
            return;
 ...
 */
async function closeMonth(sb, tenantId, monthKey, snap, closedBy, note) {
 ...
 */
async function reopenMonth(sb, closeId, reason, by) {
    ...
    if (r.error)
        throw r.error;
    (0, wroteRows_1.assertWrote)('Reopening that month', r);
}
...
async function namesFor(sb, ids) {
    ...
    try {
        rows = await (0, idLookup_1.readByIds)(ids, (chunk, from, to) => sb.from('profiles').select('id, full_name')
            .in('id', chunk).order('id', { ascending: true }).range(from, to), 'the names of the people who closed these months');
    }
    catch {
        return new Map();
    }
