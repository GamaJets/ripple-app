...
{
    eq((0, coachCosts_1.costBlockers)(draft()).length, 0, 'a complete draft has nothing wrong with it');
    const empty = (0, coachCosts_1.costBlockers)({ description: '', amountText: '', currency: null, category: 'rent', paidOn: '' });
    ...
    // person who typed it.
    const noCur = (0, coachCosts_1.costBlockers)(draft({ currency: null }));
    ...
    eq((0, coachCosts_1.costBlockers)(draft({ amountText: '0' })).length, 1, 'a cost of nothing is refused');
    eq((0, coachCosts_1.costBlockers)(draft({ amountText: '-40' })).length, 1, 'and so is a negative one');
    eq((0, coachCosts_1.costBlockers)(draft({ category: 'not-a-category' })).length, 1, 'and a category nobody offers');
}
/* ── 2. the amount, where a hundredfold error would enter ─────────────────
   ...
   amounts out. */
{
    eq((0, coachCosts_1.costBlockers)(draft({ amountText: '450.50' })).length, 0, 'a two-place amount goes through');
    eq((0, coachCosts_1.costBlockers)(draft({ amountText: '450,50' })).length, 0, 'and so does a comma decimal — half the world types it');
    eq((0, coachCosts_1.costBlockers)(draft({ currency: 'JPY', amountText: '45000' })).length, 0, 'a yen amount is a whole number');
    eq((0, coachCosts_1.costBlockers)(draft({ currency: 'JPY', amountText: '450.50' })).length, 1, 'and a decimal in one is refused rather than rounded');
    ...
    eq((0, coachCosts_1.costBlockers)(draft({ currency: 'KWD', amountText: '450.000' })).length, 0, 'a three-place amount is an amount');
    const kw = (0, coachCosts_1.costBlockers)(draft({ currency: 'KWD', amountText: '450.005' }));
...
{
    const t = (0, coachCosts_1.costsTaken)([cost(), cost({ id: 'c2', amountCents: 12000, category: 'insurance' })]);
    ...
    const mixed = (0, coachCosts_1.costsTaken)([cost(), cost({ id: 'c2', currency: 'AED', amountCents: 60000 })]);
    ...
    const holed = (0, coachCosts_1.costsTaken)([cost(), cost({ id: 'c2', currency: null })]);
    ...
    eq((0, coachCosts_1.costsTaken)([cost({ amountCents: null })]).unpriced, 1, 'and one with no amount at all is counted too');
    ...
    eq((0, coachCosts_1.costsTaken)([cost()])?.pots[0].count, 1, 'the count is of rows, not of currencies');
}
/* ── 4. where it went, per category AND per currency ──────────────────────*/
{
    const by = (0, coachCosts_1.costsByCategory)([
    ...
    eq((0, coachCosts_1.costsByCategory)([]).length, 0, 'and an empty book has no categories at all');
    // Two currencies inside one category are two lines, not a sum, for the same
    // reason they are two pots in the total.
    const twoCur = (0, coachCosts_1.costsByCategory)([
    ...
    // "unknown".
    eq((0, coachCosts_1.categoryLabel)('something_added_later'), 'something_added_later', 'an unrecognised category prints as itself');
    eq((0, coachCosts_1.categoryLabel)(null), 'Not stated', 'and a missing one says so rather than being blank');
    eq((0, coachCosts_1.categoryLabel)('rent'), 'Rent or Chair Fee', 'and a known one gets its label');
}
...
{
    ok((0, coachCosts_1.costsEmptyLine)('error').includes('could not be read'), 'a failed read says so');
    ok((0, coachCosts_1.costsEmptyLine)('error').includes('not a statement that you have recorded none'), 'and says outright that it is not a claim about the record');
    ok((0, coachCosts_1.costsEmptyLine)('partial').includes('nothing here is a total'), 'a truncated read states no total');
    eq((0, coachCosts_1.costsEmptyLine)('loading'), 'Still reading.', 'and a read in flight says only that');
    ok((0, coachCosts_1.costsEmptyLine)('ready').includes('have not recorded'), 'a whole read with nothing in it is a real answer');
    ok((0, coachCosts_1.costsEmptyLine)('ready').includes('Rent'), 'and names what belongs here');
}
   ...
   function here that could produce one. */
{
    ...
    ok(coachCosts_1.COSTS_ARE_NEVER_NETTED.includes('no profit figure anywhere in this app'), 'and the screen says so out loud, because an absent figure reads as an omission');
    ok(coachCosts_1.COSTS_ARE_NEVER_NETTED.includes('different currencies'), 'naming one of the reasons it could not be computed');
    ...
    ok(coachCosts_1.COSTS_ARE_NOT_TAX_ADVICE.includes('allowable'), 'no cost is marked allowable');
    ok(!coachCosts_1.COST_CATEGORIES.some((c) => /deduct|allowab|tax|vat/i.test(c.id + c.label + c.note)), 'and no category implies a tax treatment');
    // The two things already counted, which a coach must not write down twice.
    ok(coachCosts_1.COSTS_NOT_TWICE.includes('Repple plan') && coachCosts_1.COSTS_NOT_TWICE.includes('ad spend'), 'the double-count warning names both of them');
    ...
    const ids = coachCosts_1.COST_CATEGORIES.map((c) => c.id);
    ok(!ids.includes('advertising') && !ids.includes('platform'), 'and neither is offered as a category in the first place');
    ok(coachCosts_1.COST_IS_YOUR_WORD.includes('has been checked against'), 'a recorded cost is the coach’s own word and says so');
}
